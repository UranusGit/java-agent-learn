...
>
> **本章范围**：
> - 消息持久化之外，还有事件流持久化（agent_events 表，用于时间线与审计）；
> - 双写一致性对账任务（DB 与 JSONL 不一致时的自动修复）；
> - 启动时孤儿恢复（进程崩溃后修复"父亲已写但孩子未写"的孤儿结果）。
...
- 想做长程任务（长程任务篇） → 完全没法跨 session 续接
- 想做上下文压缩（上下文工程篇） → 没法分清"压缩前 / 压缩后"
...
>
> 别看复杂，**这套机制是后面上下文压缩、长程任务、错误恢复的共同基础**。这一章不打好，后面几章都没法做。
...
## 10. 对照：与上一章 DB-only 的恢复能力差异

| 维度 | 上一章 DB-only | 本章 JSONL + DB 双写 |
|------|--------------|----------------------|
...
| 雷 | 现象 | 规避 |
|----|------|------|
| 双写不一致（DB 成功 JSONL 失败） | 查询有 / 重启丢 | 错误恢复篇加对账任务 |
...
进入下一节：**08-WebSocket 重连**——让前端断网重连时不丢消息。
