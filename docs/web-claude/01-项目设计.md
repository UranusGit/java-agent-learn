...
> 依据：调研笔记的结论与未决问题清单。
> 用途：为后续实验手册（环境准备起）提供架构蓝图、MVP 切片、模块边界。
...
> 设计文档**不直接给代码**——所有代码在环境准备之后的实验手册里。本文是"地图"，那些手册是"路"。
...
flowchart TD
    ...
    subgraph AGENT["Agent 层（核心）"]
        ...
        HE["HarnessEngine（长程任务专用）<br/>- InitializerAgent（首次脚手架生成）<br/>- HarnessGuard（immutable_paths / 单 feature 约束）<br/>- TestVerifier（防假通过）<br/>- ProtocolStateMachine（boot/work/verify/commit）<br/>- MultiAgentBus（Planner/Coder/Tester/Reviewer 通信）"]
    end
    AGENT --> SVC["Service 层<br/>- SessionService（JSONL + DB 索引）<br/>- TaskService（durable Cron + 文件锁）<br/>- SandboxManager（Docker 容器调度）<br/>- ArtifactService（S3 + 元数据）<br/>- HookService（HTTP hook + 沙箱内 shell hook）<br/>- FeatureListService（长程任务核心）<br/>- ProgressService（追加日志）<br/>- ContextCompactor（4 级压缩）<br/>- SubagentService + Scheduler（DAG 调度）<br/>- McpClientManager（MCP 协议）<br/>- AgentEventBus（事件总线）<br/>- RetryTemplate + CircuitBreaker<br/>- QuestionBus + PendingQuestion（HITL 人工介入）"]
...
**核心表关系**（次要表：hooks / subagents / mcp_servers / agent_events / pending_questions / decision_audit / dead_letters）：

```mermaid
erDiagram
    TENANT ||--o{ USER : "归属（tenant_id）"
    USER ||--o{ SESSION : "创建（user_id）"
    SESSION ||--o{ MESSAGE : "包含（session_id，parent_uuid 形成 DAG）"
    SESSION ||--o{ TASK : "发起（session_id）"
    SESSION ||--o{ ARTIFACT : "产出（session_id）"
    TASK ||--o{ TASK_RUN : "多次执行（task_id）"
    TASK o|--o{ TASK : "依赖（task_dependencies DAG）"
    TASK ||--o{ ARTIFACT : "产出（task_id 可空）"
    USER ||--o{ API_KEY : "持有（user_id）"
    TENANT ||--o{ PERMISSION_RULE : "配置（tenant_id）"
    ...
    SESSION {
        ...
        uuid user_id FK
        string status
        uuid parent_session_id "fork 用"
    }
    MESSAGE {
        uuid id PK
        uuid session_id FK
        uuid parent_uuid "DAG"
        string role
        jsonb content
    }
    TASK {
...
#### 2.2.8 Subagent / MCP / Events / PendingQuestion（新增表）
...
Agent Engine 通过 ai-serving 推理网关 SDK 调用模型，网关负责多模型路由、限流、fallback、成本归集，并返回流式响应。
...
全链路 trace 的 span 嵌套关系：`agent_run → model_call（关联到 ai-serving 网关）→ tool_use → sandbox_exec`，聚合到 Jaeger / Tempo。
...
| **v2.0** | **Harness 完整范式** | **Harness 工程 Guardrails + 上下文工程 + 错误恢复** |
| **v2.1** | **Subagent + MCP** | **Subagent DAG 编排 + MCP 协议** |
| **v2.2** | **HITL 全场景** | **全链路事件流 + AskUser + 审批中心** |

> v2.x 系列是"网页版 Claude"完整工程范式落地的关键阶段。
...
设计阶段完成。下一步进入 **学习材料阶段**，分阶段手把手实验：

1. 环境准备：JDK 21 / Spring Boot 4 / Docker / 前端工具链；
2. 项目骨架：Spring Boot 项目 + WS + 最简对话；
3. Agent-Loop：AgentLoop + State + 单工具；
4. 工具系统与权限：Read/Write/Bash + Permission；
5. 沙箱接入：Docker 沙箱；
6. Session 持久化：JSONL + DB；
7. Artifacts：前端 artifact 面板；
8. WebSocket 重连：seq + 重放；
9. 集成 ai-serving：接推理网关 + 多租户；
10. 长程任务：feature_list + Cron；
11. Hooks 系统：事件 + http hook；
12. 附录-部署与排错：docker-compose + 常见问题。
