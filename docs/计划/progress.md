...
```mermaid
flowchart LR
    P1["P1 命令行聊天机器人<br/>难度 ⭐ · 阶段 1"] --> P2["P2 个人知识库问答<br/>难度 ⭐⭐ · 阶段 3"]
    P2 --> P3["P3 单文件代码评审助手<br/>难度 ⭐⭐⭐ · 阶段 4"]
    P3 --> P4["P4 智能运维助手<br/>难度 ⭐⭐⭐⭐ · 阶段 5"]
    P4 --> P5["P5 企业级多租户客服系统<br/>难度 ⭐⭐⭐⭐⭐ · 阶段 6"]
    P5 -->|"毕业标准"| Grad["主线毕业"]
    P5 -.->|"延伸方向"| P6["P6 多 Agent 研究助手 (可选)<br/>难度 ⭐⭐⭐⭐⭐ · 阶段 7-8"]
```
...
```mermaid
flowchart LR
    MCP["1. MCP 生态<br/>(独立可做 · 必做)"] --> LLMOps["2. LLMOps 全栈<br/>(前置: 阶段 3 评估方法论)"]
    LLMOps --> Cost["3. 成本工程<br/>(前置: LLMOps)"]
    Cost --> Reli["4. Agent 可靠性工程<br/>(前置: 阶段 4-5)"]
    JDK["5. JDK 21+ 现代特性<br/>(穿插学习, 不单独阶段)"]
```
