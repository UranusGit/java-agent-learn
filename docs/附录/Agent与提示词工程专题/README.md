...
```
提示词线（怎么写）：01 入门 ──→ 07 写作技巧           ← 01 的深化
Agent 线（怎么做）：02 心智模型 ──→ 03 上手路径 ──→ 04/05 方向 ──→ 06 验证
交汇（写代码前）：  08 Agent 提示词实战               ← 07 的技巧 × 02 的 Agent
收官（动手串一遍）：09 练手项目                       ← 把 01-08 装成一辆能跑的车
架构师（终局）：   10 路线 ──→ 11 决策 ──→ 12 复盘 ──→ 13 自测 ──→ 14 眼界
```
