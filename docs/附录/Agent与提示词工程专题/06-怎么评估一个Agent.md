...
```mermaid
flowchart TD
    RESULT["评估结果"] --> L1["① 规则断言<br/>代码比对结构化输出<br/>成本最低、客观"]
    L1 --> L2["② LLM-as-judge<br/>大模型当阅卷老师<br/>覆盖开放答案、需校准"]
    L2 --> L3["③ 人工抽检<br/>最难的 10 条做兜底<br/>成本最高、金标准"]
```
...
```
测试集 12 条 → 改 prompt 前通过 8 条（67%）
            → 改 prompt 后通过 10 条（83%）→ 这次改动有效，保留
            → 改 prompt 后通过 7 条（58%）→ 回滚，别留"这次碰巧成功"的版本
```
