...
```
第 1-2 天  读完本专题 01+02，在任意聊天界面把"五要素 + 结构化输出"玩一遍
第 3-4 天  框架起步（spring-ai 或 langchain4j 快速起步），做"单次调用 + JSON"
第 5-7 天  加记忆 → 加第一个工具 → 戴护栏（02-防止Agent失控）
第 8-10 天 做评估集，跑通过率
第 11-14 天 完整做一个实战项目（个人助理 或 智能运维）
```
...
timeline
    ...
    section 第 3-4 天
        框架起步（spring-ai 或 langchain4j）<br/>做单次调用 + JSON
    section 第 5-7 天
        加记忆 → 加第一个工具 → 戴护栏
    section 第 8-10 天
        做评估集，跑通过率
    section 第 11-14 天
        完整做一个实战项目<br/>（个人助理 或 智能运维）
```
