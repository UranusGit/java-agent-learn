...
> **这份文档是什么**：写作技巧和 Agent 提示词实战教你"怎么写"，这一篇给你**"现成能抄"的案例**——7 个最常见的场景，每个都配：**提示词原文 → Spring AI 代码 → LangChain4j 代码 → 要点**。
>
> 前提：你理解提示词工程五要素，掌握写作技巧（具体 / 给例子 / 留退路 / 一次一变量），并会写工具描述与决策规则。
> ⚠️ 代码均为**学习参考**，API 以你用的框架版本为准（可跑版本以官方文档为准）。
...
mindmap
  root((7 个场景案例库))
    场景一 五要素客服 → 提示词工程 §2
    场景二 结构化输出 → 提示词工程 §3
    场景三 Few-shot 分类 → 写作技巧
    场景四 工具描述 @Tool → Agent 提示词实战 §2
    "场景五 Agent System Prompt → Agent 提示词实战 §3/§6"
    场景六 RAG 增强提示词 → RAG 入门
    场景七 LLM-as-judge → 评估 §3
```

---

## 1. 场景一：五要素客服提示词（对应提示词工程 §2）
...
> 要点：System 只放**不变的规则**，User 模板放**会变的变量**，输出格式**单独强调**——这就是"拆三段"。

---

## 2. 场景二：结构化输出（对应提示词工程 §3）
...
> 要点：反例（"带讽刺的夸不算积极"）教模型**别怎么做**；示例风格就是隐式指令（写作技巧"给例子"）。

---

## 4. 场景四：工具描述 @Tool（对应 Agent 提示词实战 §2）
...
> 要点：工具描述五件事（作用 / 何时用+何时别用 / 参数 / 返回 / 失败行为）是 **Agent 提示词的精华**。两框架的 `@Tool` 就是写这段提示词的入口。

---

## 5. 场景五：Agent System Prompt——决策规则 + 防注入（对应 Agent 提示词实战 §3/§6）

**提示词**（练手项目的客服 Agent 完整版）：
...
> 要点：**System Prompt 写"决策规则"，不写"话术"**；"不可信数据"声明是防注入第一道闸。
...
> 要点：检索代码会变（向量库 / 切分策略），**但"资料 + 问题 + 找不到就明说"这段增强提示词是固定的**。工程化（自动注入 / 引用溯源）看 RAG 工程化实战。

---

## 7. 场景七：LLM-as-judge 评估提示词（对应评估 §3）
...
> 要点：**判卷提示词要给出"可对齐的评分标准"**，否则评判结果不可复现。把 score 跑进评估集，通过率就有了。

---

## 8. 复用与版本管理（对应提示词工程 §4 / 案例复盘）

- **提示词别写死在业务代码里**：Spring AI 用 `PromptRepo` / 资源文件外部化；LangChain4j 用模板类。
- **每段提示词都建一条评估用例**：改动后跑评估集的通过率，涨了才保留。
...
- 写作技巧 —— 这些提示词背后的写作技巧
- Agent 提示词实战 —— 工具描述 / 决策规则 / 防注入
- Spring AI 教程 / LangChain4j 教程 —— 可跑版本
- Prompt 工程深入 —— CoT / ReAct / Reflexion 的框架代码模板

案例抄完，回练手项目把它们拼成一个真的 Agent——**提示词只有跑进系统里，才算学会了。**
