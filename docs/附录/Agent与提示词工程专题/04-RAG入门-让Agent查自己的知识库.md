...
> **这份文档是什么**：Agent 最常见的两种落地方向之一（另一种是 MCP，让 Agent 接外部工具生态）。讲清 RAG 解决什么问题、五步管道、什么时候该用 RAG（而不是微调或直接塞长上下文）。不贴代码，代码在深处文档里。
>
> 前置：你会写结构化的提示词（五要素 + 结构化输出），并理解 Agent 的工具与循环（即 01、02 的能力）。
> 进阶（要写代码）：可以用框架 100 行跑通"问 PDF"，再逐步升级检索策略。
...
> **五步里，真正决定效果的是 ① 切分和 ④ 检索**——文档切得好不好、检索得准不准，直接决定答案质量。优化方向就是调分块策略（块大小、重叠）和检索参数（召回数、相似度阈值）。
...
> 关键就一句：**"找不到就明说，不要编造"**——这是抑制 RAG 幻觉的第一道闸（提示词写作里"退路"技巧在 RAG 里的应用）。
...
区别只在返回内容：订单工具返回结构化数据，RAG 工具返回**一段文本**。所以 Agent 会自己决定"这个问题要不要先查知识库"——这正是"模型在循环里做决策"的体现。
...
1. 先写 100 行代码让"问 PDF 一个问题"跑通，**连 docker 都不用起**（内存向量库）。
2. 跑通后再逐章升级：生产级存储 → 切分优化 → 查询改写 → 混合检索 → 重排 → 引用溯源 → 评估闭环 → 上生产。
3. 想学原理和选型（向量库对比、Embedding 选型），重点比较向量库的部署复杂度与 Embedding 模型的中文适配。
4. 想串成完整项目，参考 RAG 端到端案例，把"检索 → 生成 → 溯源"串成一条完整链路。
...
完成后进入下一节：**05-MCP 入门**——让 Agent 接外部工具生态。
