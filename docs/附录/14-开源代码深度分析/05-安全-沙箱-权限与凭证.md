...
|---|---|---|
| Landlock allow-list + fail-closed | JVM SecurityManager（已弃用）/ seccomp / 容器 | allow-list 比 deny-list 安全；「未授予即拒绝」是沙箱黄金法则（对应 [教程 64-安全与权限控制]） |
| `approval/request` waterfall HITL | 自定义审批拦截器 | 审批审计对必须 turn 包裹、fail-closed unless allowed-once（对应 [教程 61-Human-in-the-Loop与审批流]） |
