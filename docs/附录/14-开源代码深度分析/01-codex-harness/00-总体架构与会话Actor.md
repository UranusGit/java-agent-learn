...
1. **单写者 Actor**：并发 bug 的根源是"多个执行流同时改一份状态"。把写者减到一，`submission_loop` 里"读 active_turn → 决定 start/steer → 写 turn state"天然原子——锁这个概念从系统中**消失**，而不是被正确使用。消灭错误类，永远优于管理错误。
2. **双通道不对称**：命令有界（512）、事件无界，源于两侧容错语义不同——命令迟到可接受（用户多等一瞬，`send().await` 反压），事件缺失不可接受（UI 状态缺块）。反压只加在"能等"的一侧。
3. **SessionTask 单点收尾**：统计/落盘/终态事件若散落在各任务里，每加一种任务类型必漏一项；集中到 `on_task_finished`，新增类型"免费"继承完整生命周期——开闭原则的收益不在扩展时，而在**不遗漏**时。
4. **持久化先于终态事件**：终端产品的信任建立在"账实相符"上，TurnAborted 若先于 flush 发出，崩溃重放就会出现"被中止了却查无此事"。

**好处**：无锁、无竞态类 bug；三段式取消语义完备（协作退出→100ms 宽限→强杀）；steering 让"不打断也不丢失"同时成立，而不必在丢工作和丢输入之间二选一。

**代价**：单消费者串行封顶了吞吐，一个 turn 卡住会阻塞后续 Op（Interrupt 靠 cancellation_token 旁路存活）；无界事件通道在消费端死亡时可能积压内存。省掉的锁换成了必须守住的纪律——全链路 await 点套 or_cancel、AbortOnDropHandle 防泄漏，纪律一破，协作取消形同虚设。

**适用边界**：交互式流式对话、需要取消/steering 的长会话引擎——必学；纯批处理管道（无中途输入、无取消需求、失败即重跑）用普通队列消费者即可，actor 骨架属于过度设计；若会话内还需多任务真并行，则要引入任务内并发模型，本骨架只管会话级串行。
