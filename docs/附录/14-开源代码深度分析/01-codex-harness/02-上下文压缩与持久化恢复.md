...
1. **不可变快照 + 版本号**：可变历史的并发地狱是"读者读到一半历史被改"。`Arc` 整体替换让读路径零锁零拷贝、写路径天然原子，`history_version` 让持有旧快照者可检测失效——与 00 篇单写者同源：消灭错误类而非管理错误。
2. **append-only JSONL 每行自完整**：崩溃恢复的**最简正确**格式——损坏最多丢尾行，不存在半写文件引发的级联解析失败；配"pending 确认后才移除 + 幂等 persist"，写入可靠性不需要事务协议加持。
3. **静态一次注入，动态最小化重注入**：token 预算是硬约束，重复注入的信息是纯浪费——world_state diff（无变化不注入）把预算只花在增量上。
4. **token 服务器优先**：压缩/截断决策用服务器上报值，本地启发式只做粗下界预警——两个数字打架时信计费的那个。

**好处**：三条压缩路径统一走 `CompactTask`，算法可插拔而前端/持久化零感知；恢复入口是投影方法族（New/Cleared/Resumed/Forked）纯函数，恢复逻辑可单测；崩溃演练只丢尾行、状态一致。

**代价**：历史每次重写整体分配新 Vec——用内存换并发简单性；压缩本身多付一次模型调用的延迟与 token 成本；mid-turn 重注入位置（最后一条真实用户消息之前）是与模型训练假设绑定的**隐式契约**，换模型/换版本必须重新验证，不懂这条的团队压缩后模型行为劣化还找不到原因。

**适用边界**：长会话/长任务 Agent——压缩与恢复是命门，三路径+JSONL 重放必学；短会话（几十轮内）可省压缩与差异注入，但 `HistorySnapshot`+version 的不可变快照思想任何会话引擎都值得——它成本极低且消灭的是一整类并发 bug；审计合规场景（历史回放/留存）则 JSONL 是必选项。
