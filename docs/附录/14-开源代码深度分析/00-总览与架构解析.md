...
2. **事件矩阵即扩展点目录**：Java 侧对应 [教程 03-React前端与AgenticUI/04-流式工具调用与事件协议] 的事件协议与 [教程 02-SpringAI核心机制/04-Advisor链与拦截器]。但这里的事件域（session/agent/capability）划分更细，且带 dispatch 模式契约。
