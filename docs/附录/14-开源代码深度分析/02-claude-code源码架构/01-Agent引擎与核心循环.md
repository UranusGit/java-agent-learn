...
**本篇哲学一句话**：循环是骨架、流式是架构约束而非优化、可靠性靠终止路径与配对完整性这类"硬不变量"。**适用场景**：自研会话引擎、流式交互产品、需要中断/恢复能力的长任务系统。**不适用场景**：单轮无状态调用（如一次性摘要）——循环护栏全套是纯开销。**选型建议**：五类终止路径与消息配对完整性是必抄项（缺了就是生产事故预定）；并发模型建议学 [../01-codex-harness/00-总体架构与会话Actor] 的单写者 Actor（比本篇生成器+级联更不易写错），扣留自愈机制保留。
