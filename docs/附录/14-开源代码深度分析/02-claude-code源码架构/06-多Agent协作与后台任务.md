...
**本篇哲学一句话**：多 Agent 首先是组织学——协调者换权限安全与责任归属，邮箱拥抱最终一致，物理隔离落地认知隔离。**适用场景**：长任务（跑测试/构建）需要后台化、任务可自然拆成责任边界、需要无人值守定时化的系统。**不适用场景**：单轮问答、需要统一判断的紧耦合任务——硬拆多 Agent 只是把协调成本伪装成架构。**选型建议**：两层任务模型与 Cron Jitter/自动过期性价比最高；协调者+邮箱等真有并行分工需求再上；上之前先用 [../01-codex-harness/00-总体架构与会话Actor] 的标准自查单 Agent 循环是否已写对。
