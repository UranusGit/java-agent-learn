...
**哲学根源**：本篇的模块排序本身就是哲学表达——骨架（codex）先行因为正确性是信任的地基，治理（claude-code）第二因为行为边界要在能力扩张前立好，缝化与事件化（dsh）最后因为抽象税只有在规模证明后才值得付。**推荐**：V1 阶段拒绝一切插件框架与多 Agent（dsh/claude-code 的机制此时都是负债）；V2 阶段拒绝第二真相源（此时开始为 V3 的事件化留接口——日志每行自完整 JSON）；V3 阶段拒绝为换而换（每个缝化决策都要能回答"换提供者的概率 × 改造成本"）。**一句话总纲**：每个阶段都有"不做清单"，做对不做清单比做完功能清单更难也更重要。
