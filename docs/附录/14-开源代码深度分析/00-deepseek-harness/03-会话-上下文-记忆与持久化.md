...
**适用场景**：
- 会话是用户资产，崩溃后必须完整恢复（孤儿 turn 合成 `interrupted` 闭合、JSONL link+unlink 原子写）；
- 会话量大到「列表读永不加载全日志」（投影缓存 + 冷读阶梯三档降级）；
- 上下文超窗要优雅压缩且成本敏感（shadow-price 协议 + summarizer 前缀缓存复用）；
- 需要 Token 归因、全文检索、遥测、反馈等任意多读侧视图（单写多读投影）。

**不适用场景**：
- 一次性对话、无持久化需求（整套 fold/投影机制是净成本）；
- 读侧必须点查单字段（追加式日志只能 fold，点查需另建索引并接受「缓存永不超前日志」约束）；
- 无法接受「未识别事件拒读」的强格式策略（向前兼容需显式 `ignorable` 标记）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「崩溃闭合孤儿 turn」与「缓存永不超前日志」两条——恢复时合成 `turn/end {interrupted}` 而非截断，读缓存写前先保证日志已落盘；其次是「前缀缓存复用」，压缩/命名等辅助调用复用对话自身前缀，KV cache 不被击穿，Token 成本大幅降低。
