...
| vendoring + rescope | 内嵌依赖 / shade 插件 | 完全拥有框架层的成本与收益权衡（对应 [附录 07-架构决策方法论]） |
...
**适用场景**：
- 大型 monorepo（几十至上百包）要让质量不靠人自觉（run-gates 依赖图并行 + per-file 100% 覆盖 + snapshot 三态）；
- 文档与代码漂移是长期痛点（gen-* 生成 + verify-* 字节比对，stale 即失败）；
- 框架层是核心资产（vendoring + rescope + 18 条修改日志 + check-vendor-manifest 制度化）；
- Mermaid/文档链接/翻译配对也要进门禁（verify-mermaid 真解析、verify-md-links、translation-pairing）。

**不适用场景**：
- 单模块小项目（三层门禁 + 生成器闭环的成本远超收益，本篇开头已定调）；
- 团队无力维护生成器本身（生成式文档的代价是生成器也要进门禁）；
- 需要紧跟上游快速升级的依赖（vendoring 的本地 diff 维护会拖慢升级）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「生成式文档 + 字节比对」——从注解/运行时生成配置与工具目录，CI 里 stale 即失败，这是文档不漂移的治本方案；其次是「snapshot 三态」：CI 强制 replay 只读、本地才 record，把「故意更新」与「意外漂移」区分开。
