...
|---|---|---|
| `ctx.subagents` 具名 provider 注册表 | Spring 多实现注入 | 「多个 child 运输方式共存」时用注册表，「一个执行语义」时用单引擎（对应 [教程 00-基础与核心/09-多Agent协作]） |
| Activation 可续作（inbox 唯一队列） | 会话状态机 | 续作不建第二套状态机，用 Agent quiescence + owned-child set 推导（对应 [教程 02-SpringAI核心机制/02-Agent状态管理]） |
| workflow worker + vm context 脚本 | 脚本引擎/DSL | 模型写脚本、host 执行扇出是 Agent 编排的高级形态（对应 [教程 08-架构师进阶/02-Agent工作流编排]） |
| Ralph fresh-round + handoff | 多轮反思 | 「共享工作区为权威、handoff 只补充」防止上下文污染（对应 [教程 08-架构师进阶/03-自我反思与Agent评估]） |
| goal durable phase + 进程内 activation | 长任务状态 | 「状态可重放、激活过程本地」兼顾恢复与安全（对应 [教程 08-架构师进阶/06-长任务持久化与中断恢复]） |
...
**适用场景**：
- 多 Agent 委托且运输方式会共存（`ctx.subagents` 具名 provider：spawn/fork/ACP/Claude Code/Codex/dsh-sdk）；
- 任务需要续作而非一次性 run（Activation + inbox 唯一队列，驻留态从数据推导）；
- 长期目标驱动自动续作但要人工可控（goal 的 durable phase 与 `armed/disarmed` 激活分离，resume 后必须人工授权）；
- 会话内定时唤醒（schedule 到点以普通 turn 回原会话，绝不 `steer()` 打断）。

**不适用场景**：
- 单 Agent 单轮对话（编排层整体是净成本）；
- 需要「委托失败悄悄降级继续跑」的宽容语义（本设计是 fail-loud：`UNSUPPORTED_CAPABILITY` 直接拒绝）；
- 想要外部通知渠道的定时（`ScheduleDeliveryMode = 'session-local'`，无冷会话调度器）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「编排不侵入主循环」——委托/后台/定时/目标全部做成监听 `agent/*` 事件的外层策略，主循环只认领-驱动-记录；其次是「一次性 run 与可续作 Activation 分开」，用不同抽象表达不同语义，避免为续作造出「中间结果包装」的第二套状态机。
