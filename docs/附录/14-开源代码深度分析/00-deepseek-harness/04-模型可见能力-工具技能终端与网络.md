...
|---|---|---|
| 能力缝三层分离（定义/提供者/消费者） | `ToolCallback` + 实现分离 | 工具 schema 与执行解耦，换实现不换模型面（对应 [教程 00-基础与核心/03-工具调用]） |
| `fs/*` 事件门策略 | Advisor 拦截 `@Tool` 调用 | 「策略以事件存在、删掉仍能裸跑」比硬编码在工具里更灵活（对应 [教程 02-SpringAI核心机制/01-Advisor链与拦截器]） |
| `scrubbedParentEnv` + 显式合并 | 进程环境传递 | 默认 scrubbed、显式放行是进程安全的黄金法则（对应 [教程 04-企业级架构主干/11-安全与权限控制]） |
| MCP 桥 `mcp__server__tool` | Spring AI MCP Client | 与 Spring AI 的 MCP 工具前缀约定同构（对应 [教程 02-SpringAI核心机制/07-MCP协议]） |
...
**适用场景**：
- Shell/文件系统/检索等能力的后端会变（本机 → 沙箱 → 远程），要求换 provider 不换模型提问方式（能力缝三分离）；
- 模型需要「结果 + 正交状态」而非异常（error-as-field：`timedOut`/`aborted`/`denied` 独立上报）；
- 策略要能独立装卸（`fs/*` 事件门，删掉策略插件工具仍能裸跑）；
- 工具要桥接外部进程/MCP（`mcp__<server>__<tool>` 命名 + `scrubbedParentEnv()` 先剥再显式合并）。

**不适用场景**：
- 只有一两个工具、后端永不变（拆定义/提供者/消费者三套包纯属增加体积——「提供者会变的能力才值得缝」）；
- 调用方全部可信、无外部输入（hostile-peer 心态的防御是净啰嗦）；
- 追求最小 schema（explicit > implicit 会让每个 spec 全必填）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「Provider 注册能力而非工具」——模型面 schema 唯一归属消费者包（`@Tool` 定义与执行实现分离），换后端零改模型面；其次是 error-as-field，用 Java 21 `sealed interface` + 穷尽 switch 表达「结果 + 正交状态」，杜绝把「超时」误读成「成功」。
