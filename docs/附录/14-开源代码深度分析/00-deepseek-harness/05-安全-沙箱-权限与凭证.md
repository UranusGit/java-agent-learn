...
**适用场景**：
- 模型可能执行命令/写文件/访问网络的任何生产 Agent（fail-closed 地板：无 backend 抛 `SANDBOX_UNAVAILABLE`、审批无 answerer 即拒绝）；
- 需要 HITL 审批且审计可重放（`approval/asked`/`decided` 审计对 + turn 包裹）；
- 密钥高频轮换（`CredentialRef` 引用化 + 每操作解析，轮换零重启）；
- 无人值守场景要确定性拒绝（`never` 策略先于 waterfall 判定，prepend 不可绕过）。

**不适用场景**：
- demo 级、工具全部只读无副作用（审批/沙箱/凭证三缝是明显的过度设计）；
- 平台无内核沙箱可用且不能接受 `partial` enforcement（windows-acl 只限写、硬链接可越界）；
- 无法承担「更多拒绝」的体验代价（fail-closed 意味着模型偶尔要走审批/升级）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「凭证不落地 + 每操作解析」——配置只存引用、provider 持值，日志与错误诊断永不引用秘密；其次是「审批审计对必须 turn 包裹」与「严格变宽升级表」，这两条直接决定 HITL 是否可审计、升级路径是否可穷举。
