...
读完后你会发现：**「一切皆事件、可逆注册、会话日志为唯一真相、能力缝可整体替换」**这套组合拳，正是生产级 Agent 系统在管控分离与可扩展性上追求的终极形态。它与 [教程 04-企业级架构主干/00-管控分离架构] 的 Control/Data Plane 思想、[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议] 的事件协议设计高度同构。

> 想深入本体系的对应知识？→ [教程 04-企业级架构主干/00-管控分离架构] [教程 03-React前端与AgenticUI/04-流式工具调用与事件协议] [教程 09-前沿专题/00-SpringAI2-Agentic架构]
...
> 思想对照：这与 Java 生态的「依赖倒置 + 策略模式」等价，但 Cordis 用「服务 key + 事件 + 可逆效果」把插件边界提升为一等公民——插件之间零静态依赖，全部通过运行时的 ctx 与事件解耦。这正是 [教程 04-企业级架构主干/00-管控分离架构] 中 Control Plane 与 Data Plane 解耦的运行时实现。
...
1. **会话日志即真相（Session log as source of truth）**：所有模型可见的输入必须能由日志重建（官方 runtime invariant）。对应 Java 侧「历史记录持久化与合规」[教程 04-企业级架构主干/05-历史记录持久化与合规]。Spring AI 的 ChatMemory 只是内存态，deepseek-harness 把「可重放」做成了编译期不变式。
2. **事件矩阵即扩展点目录**：Java 侧对应 [教程 03-React前端与AgenticUI/04-流式工具调用与事件协议] 的事件协议与 [教程 02-SpringAI核心机制/01-Advisor链与拦截器]。但这里的事件域（session/agent/capability）划分更细，且带 dispatch 模式契约。
3. **能力缝 = 可整体替换的 SPI**：对应 [教程 04-企业级架构主干/00-管控分离架构] 的 Control/Data Plane。换一个 fs/subprocess 提供者，整个沙箱世界随之迁移——这正是「管控分离」在运行时层级的终极表达。
...
> **不适用场景**：只想快速写一个 demo Agent 的入门者（应先读 [教程 00-基础与核心/00-Agent核心概念] [教程 00-基础与核心/02-ChatClient与对话模型]）；想直接抄代码而非吸收设计的开发者（本系列是分析文档，不提供完整可用源码）。
