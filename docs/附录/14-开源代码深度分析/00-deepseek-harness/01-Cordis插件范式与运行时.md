...
> **架构师视角**：这解决了 Java 生态里「Bean 卸载时无法回滚全部注册」的痛点。可逆效果让**热装插件、灰度替换**成为内置能力，而非工程技巧。对应 [教程 04-企业级架构主干/09-灰度发布与版本管理]。
...
**适用场景**：
- 要设计插件化 Agent 框架的扩展机制（服务仓库 + 类型化事件 + 可逆效果三件套）；
- 系统需要热装插件、灰度替换、优雅卸载（可逆 effect 是地基）；
- 扩展点要区分「观察权」与「决策权」（waterfall 的短路语义）；
- 多团队并行扩展同一产品，要求插件之间零静态依赖。

**不适用场景**：
- 单体应用、扩展点固定且少（ctx/effect 的抽象成本高于收益，本篇文末已提示）；
- 团队不打算维护「每个注册都有 disposer」的纪律（可逆性会退化为半吊子）；
- 只需一次性初始化、无卸载需求的服务（Spring Bean 生命周期已够用）。

**Spring AI Java 项目应优先抄哪一条**：优先抄「waterfall 不调 `next()` 即短路」——用责任链 + 显式委派实现策略拦截（`WebFilter` 链的心智），比抛异常中断更可控；其次是「注册/注销成对」的可逆注册（`ctx.effect()` 返回精确 disposer 的纪律），为灰度发布与热替换铺路。
