...
> **定位**：本文是对 [教程 30-高级RAG与AgenticRAG §GraphRAG] 的深入展开——把 GraphRAG 从概念（LLM 抽三元组+图遍历）落到工程：图谱 Schema 设计、实体抽取与消歧、增量更新、社区摘要、图谱质量评估、与 Spring 技术栈的集成。读者画像：要在生产实现 GraphRAG 的开发者。前置阅读：[教程 30 §2]、[教程 05-RAG检索增强生成]。
