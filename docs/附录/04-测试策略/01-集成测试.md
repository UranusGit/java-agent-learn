...
> 「本文是对 [教程 04-企业级架构主干/01-微服务拆分与Agent部署 §3-§5] 的深入展开」
...
>
> 「本文属 Agent 测试金字塔的 **L2 集成层**：Mock 掉 LLM 后验证装配与协作的结构正确性；特别警惕"沙漏形态"——集成层塌陷后评估失败将无法归因，只能烧 Token 盲试，层间经济关系见 [附录 04-测试策略/03-测试金字塔与分层投入 §4.2]」
