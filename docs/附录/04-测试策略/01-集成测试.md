...
class VectorStoreIntegrationTest {
    ...
    void shouldRetrieveRelevantDocuments() {
        ...
        vectorStore.add(List.of(
            new Document("Java 21 引入了虚拟线程", Map.of("category", "java")),
            new Document("Spring Boot 4.1 支持自动配置", Map.of("category", "spring")),
            new Document("Python 是数据科学的首选语言", Map.of("category", "python"))
        ));

        // 等待索引
        await().atMost(5, SECONDS).untilAsserted(() ->
            assertThat(vectorStore.similaritySearch(SearchRequest.query("Java").withTopK(3)))
                .hasSize(3)
        );

        // 验证检索相关性
        List<Document> results = vectorStore.similaritySearch(
            SearchRequest.query("虚拟线程是什么").withTopK(1)
        );
