...
> 想深入？→ ADR 解决"决策怎么记录可查"；决策之后"团队怎么真的按决策走"是另一半工程——Golden Path、采纳度量与平台运营模式见姊妹篇 [附录 07-架构决策方法论/01-平台工程与组织采纳]。
...
- **嵌入流程**：把 ADR 作为 PR 流程的一部分，而不是一个独立的"文档工作"，否则它一定会被跳过。ADR 进 PR 只是交付时间轴上的一站——它与评审、评测、红队等闸门的排序及发布检查单，见姊妹篇 [附录 07-架构决策方法论/02-工程协作流程]。
