...
> **定位**：本附录第 06 篇（前置：[附录 06-WebFlux与响应式编程/00-WebFlux从零入门 §7]、[附录 06-WebFlux与响应式编程/01-Reactor核心 §4]）。前五篇告诉你"怎么用"调度器，这一篇打开引擎室：**Scheduler 全家族选型、flatMap 并发编排三兄弟、EventLoop 与阻塞的战争、Loom 虚拟线程与响应式的合流**。读完你应该能回答：这条管道的每一段跑在哪个线程上、为什么、怎么验证。API 版本：reactor-core 3.8.6 + reactor-netty 1.3.6，全部签名经本地 jar `javap` 实证。
...
**默认情况下什么都不做，你的整条管道都跑在 EventLoop 上**（这就是 [00-WebFlux从零入门 §7] 三条铁律的由来）。Scheduler 的意义是**按需把某段管道"搬家"到更合适的线程池**。
...
两条规则（[01-Reactor核心 §4] 提过结论，这里给机理）：
...
EventLoop 只有 N 个（=核数）。它同时负责**所有连接**的 I/O 事件分发。任何一个 EventLoop 线程阻塞 500ms，这 1/N 的连接**全部**无响应——不是这条请求慢，是随机一批用户集体卡顿。这就是 [00-WebFlux从零入门 §7] 把"禁阻塞"列为铁律 1 的机理。
...
**架构师判断**：虚拟线程让"阻塞写的代价"大幅下降，但没有消灭 [00-WebFlux从零入门 §1] 的核心差异——**SSE 长连接的内存与调度成本仍以响应式最低**。Agent 网关（海量 SSE）选 WebFlux、业务服务（JDBC 密集）可选虚拟线程 MVC，通过 HTTP 解耦两者，是 2026 年的常见组合。「线程模型 vs 响应式的完整选型对照？→ [附录 06-WebFlux与响应式编程/04-WebFlux-vs-MVC]」
...
下一篇 [附录 06-WebFlux与响应式编程/07-WebFlux测试与性能调优] 把镜头转向工程化：如何测试这些线程行为、如何压测与调优 Netty。
