...
> **stop 后数据还在吗？** 看**有没有挂载数据卷**（见上文第 5 章）。没挂卷的话，删容器（`docker rm`）数据就没了；只是 stop 不删，数据还在。
...
> **不必死记这些变量名**。教程给什么就照抄什么。出问题了再去镜像文档查那个变量的含义。本系列附录里 Kafka 相关教程中那串 `-e` 命令就是这种写法。
