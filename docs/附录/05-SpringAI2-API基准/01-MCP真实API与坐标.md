...
> **定位**：本文是对 [教程 02-SpringAI核心机制/07-MCP协议 §API] 的深入展开，也是全体系的 **MCP API 真实性基准**：真实 starter 坐标、真实客户端类型（`McpSyncClient` 而非虚构的 `org.springframework.ai.mcp.McpClient`）、`@Tool` 暴露为 MCP 工具的正确姿势、Streamable HTTP 传输。前置阅读：[教程 02-SpringAI核心机制/07-MCP协议]。
