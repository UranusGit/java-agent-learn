...
记住三个关键点：① 生产者推、消费者**拉**（pull 模型 = 天然背压，消费不过来就少 poll，见 [附录 06-WebFlux与响应式编程/02-背压与流量控制]）；② 高水位（HW）之前的数据才对消费者可见；③ **位移提交发生在消费之后**，这决定了 at-least-once 语义与重复处理问题（[17-Kafka/03]）。
