...
>
> **读者画像**：读完 [17-Kafka/00-08]、要在 Spring Boot 4.1 + WebFlux 技术栈里真正落地事件驱动 Agent 服务的工程师。
>
> **前置阅读**：[17-Kafka/02-消费者与消费组 §6]（长调用防线）；[17-Kafka/03-投递语义与事务 §5-6]（幂等/Outbox）；[附录 07-企业级架构模式/02-事件驱动Agent架构]（架构模式层——本文是其工程实现层）。
>
...
| MANUAL / MANUAL_IMMEDIATE | 代码里 `Acknowledgment.acknowledge()` | **Agent 服务推荐**：与业务结果显式对齐（[17-Kafka/02 §3]） |
...
public void project(AgentEvent event, ConsumerRecord<String, AgentEvent> rec) {
    ...
    // 位移提交由容器在事务内完成——输出与进度原子化（[17-Kafka/03 §4]）
}
```

注意边界：事务只保证 Kafka 内原子；`snapshotStore` 若是外部 DB，事务包不住它——**DB + Kafka 双写仍走 Outbox**（[17-Kafka/03 §6]），事务版投影适合"纯 Kafka 输入→纯 Kafka 输出"的管道。
...
**必须刻进肌肉的三条铁律**（WebFlux 语境，呼应 [附录 06-WebFlux与响应式编程] 与 [教程 42-响应式错误处理]）：
...
2. **接收器不是每请求一个**——`KafkaReceiver` 建立在长生命周期订阅上，与 Spring 单例 Bean 同生命周期；流断了用 `Flux.defer(() -> receiver.receive())` 包装重启（重试语义参考 [教程 42 §3] 的 retryWhen 纪律）。
...
[教程 22-全链路可观测性] 的 Trace 要跨服务、跨事件继续有效，靠三件事：
...
2. **Observation API**：`KafkaTemplate` 与监听容器开启 observation 后，send/ consume 产生 Span，**trace 上下文经 `DefaultKafkaHeaderMapper` 写入消息 headers（W3C traceparent）**——消费端取出继续父 Span，LLM 调用 → 事件发布 → 下游投影的链路在一个 Trace 里闭环（这正是 [教程 22] gen_ai 语义约定在事件链上的延伸）。
3. **业务语义指标**：DLT 速率、重试耗尽率、每主题 lag 换算的"审计落后分钟数"——第三层 SLO 指标（[17-Kafka/08 §3]）。

## 6. 总装：事件驱动 Agent 平台蓝图

```mermaid
graph TB
    subgraph CP["控制面（呼应教程 20-管控分离）"]
        POL["策略/配额/租户配置"]
        REG["工具与模型注册表（CDC 分发）"]
    end
    subgraph DP["数据面"]
...
这张蓝图把九篇机制各就各位：网关侧 Outbox 保"库与事件一致"（§6 of 03）；编排器是消费组（§2 of 02），长 LLM 靠分发-处理分离（§6 of 02）；投影管道用事务（§4 of 03）；快照主题压实（§2 of 04）；成本计量是 Streams 窗口聚合（§4 of 06）；知识库同步走 CDC（§3 of 07）；配额与 ACL 在骨干层兜底（§2 of 08）；trace 穿 headers 全链贯通（§5）。**它同时是 [附录 07-企业级架构模式/02-事件驱动Agent架构] 各模式（Event Sourcing/Saga/CQRS）的物理实现**。
...
1. **监听器里同步调 LLM/DB 且无 subscribeOn/虚拟线程**——[17-Kafka/02 §6] 的踢组事故在 Spring 里的形态。
...
Spring 集成层的口诀：**容器 AckMode 对齐业务语义、错误分流"可重试退避/不可重试死信"、事务包 Kafka 内原子、Outbox 包跨系统一致、reactor-kafka 记住"发射线程即 poll 线程"、trace 沿 headers 穿针引线**。到此，Kafka 主题 10 篇从全景概念到平台蓝图闭环——它与 [附录 07-企业级架构模式/02-事件驱动Agent架构]（模式层）、[项目 13-事件溯源Agent运行时平台]（项目实战层）构成"机制—模式—实战"三点连线。
