...
> **定位**：本文是对 [教程 32-自我反思与Agent评估] 与 [教程 36-数据飞轮与持续改进] 的深入展开——自研评估之外，业界事实标准工具（Langfuse/Ragas/promptfoo/OpenLLMetry）与 Spring AI Observation 的集成方式，以及"自研 vs 采用"的选型。读者画像：要搭建评估闭环但不想全手写的团队。前置阅读：[教程 32 §评估体系]、[教程 16 §OTel 集成]。

---

## 1. 自研 vs 采用：先回答选型问题

本体系教程 32/36 全部手写评估（学习价值），生产选型的对照：
...
Spring AI 原生 Observation 已经产出 gen_ai 语义约定的 Span（[教程 16 §gen_ai]）。接 Langfuse 只需把 OTLP 指到它（或经 OTel Collector 转发）：
...
otel:
  ...
  traces:
    exporter: otlp
# 预期效果: ChatClient/ChatModel/Tool/VectorStore 的 Span 树(教程 16)
#          直接出现在 Langfuse 的 Trace 视图中——零代码改动
```

**Trace 关联会话与用户**（Langfuse 数据集质量的关键）：Span 的 session.id/user.id 属性从业务上下文注入（Baggage 传播，[教程 16 §Baggage]）。
...
flowchart LR
    ...
    RES -->|"③ 回流"| LF["Langfuse/仪表板<br/>数据飞轮的评估输入(教程 36)"]
```
...
```yaml
# promptfooconfig.yaml —— CI 中的提示词回归门禁（接 [教程 23 §灰度] 的影子验证）
...
flowchart TB
    ...
    L3 --> L4["改进: Prompt/RAG/模型<br/>(微调决策框架, 教程 36)"]
    L4 --> L5["灰度: 教程 23<br/>(影子验证=先跑 promptfoo)"]
    L5 --> L1
```

这就是 [教程 36 §飞轮] 的工具化版本：飞轮的每一环都有标准工具承接，自研部分只剩"业务判断"（什么指标达标、如何改）——**把工程留给工具，把判断留给自己**。
...
| Ragas 指标迷信 | LLM-as-Judge 的指标本身有偏差（[教程 32 §Judge 偏差]），趋势比绝对值有用 |
| 侧车无人维护 | Python 侧车进 CI 与告警体系，别做"一次性脚本" |
| 双写 Trace（OTel+SDK 手动） | 二选一（[附录 12-02 §javaagent 纪律]） |
