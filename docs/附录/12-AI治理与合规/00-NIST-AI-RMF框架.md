...
"指定 AI 责任人"落不到具体操作上就等于没指定——GOVERN 的可执行形态是一张**人员 × 操作的权限矩阵**：每一类高风险操作，谁执行（R）、谁担责（A）、谁被咨询（C）、谁被告知（I），四个格子都不允许为空，且 **A 每行有且仅有一个**（"都有责任"="没人负责"）。以 Agent 平台最常见的五类操作为例：

| 操作 | 业务团队 | AI 评审委员会 | 平台团队 | 值班 on-call | AI 责任人 |
|------|---------|--------------|---------|-------------|----------|
| Prompt 版本上线 | R | C（高风险用例） | I（版本登记） | I | **A** |
| 工具注册与权限分级 | R（声明） | C | R（ schema 审核） | I | **A** |
| 护栏红线规则变更 | C | C | R | I | **A** |
| 灾备切换宣布 | I | I | C（预案与预检） | R（执行切换） | **A**（宣布权） |
| P0/P1 事件定级与升级 | I | I | C | R（初判） | **A**（终判） |

其中灾备切换的宣布权要特别说明：切换演练与预检可以按 [教程 04-企业级架构主干/17-高可用与灾备设计 §5] 的 runbook 反复操练，但"正式宣布切换"这个动作必须是具名的人——因为它同时触发对外通告、计费影响与合规留痕，是不可逆的组织级决策，runbook 只授权到"执行"，不授权到"宣布"。

矩阵里专门有一列 **AI**（Agent 本身），规则只有一条：**AI 可为 R、不可为 A**。AI 可以是切换预检的执行者（跑健康检查、给出切换建议、生成演练报告），可以起草 Prompt 回归报告供上线评审，但 Accountable 意味着"对后果承担解释与追责义务"——AI 无法被追责，把 A 写给 AI 等于这一行没有负责人。这条也是评审委员会审查自动化审批 proposals 时的固定判据：凡是提案里出现"AI 审批通过即生效"且无人类 A 兜底的流程，一律退回（审批流的人类卡点设计见 [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]）。矩阵本身作为治理文档进入 `governance/` 目录，随组织架构与人员变动每季度复审。
...
其中"第三方依赖盲区"最容易停在口头——"在合同中加入 SLA、数据使用条款、审计权条款"说出口容易，条款怎么审、审出红旗怎么办是专门的合同工程：见 [附录 12-AI治理与合规/06-模型供应商合同与SLA审查]。
