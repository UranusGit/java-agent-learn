...
「本文是对 [教程 08-架构师进阶/09-Agent治理与合规框架 §4-§5] 的深入展开」

> [教程 08-架构师进阶/09-Agent治理与合规框架] 本文是治理合规系列的第三篇，聚焦两个硬性合规领域——以 GDPR 为代表的数据隐私法规落地，以及 AI 系统中的偏见检测方法。两者都是 AI 系统上线前的"必检项"。
...
/**
 * 偏见检测门禁：在 CI 流水线中运行，
 * 如果模型在受保护群体上的表现差异超过阈值，则阻断发布。
 */
@Component
public class BiasDetectionGate {

    private final ChatClient agentClient;
    private final BiasDatasetRepository datasets;

    public BiasReport runGate(String modelVersion, String datasetName) {
        List<BiasCase> cases = datasets.load(datasetName);
        Map<String, List<BiasCase>> byGroup = cases.stream()
            .collect(Collectors.groupingBy(BiasCase::protectedGroup));

        // 计算每组的准确率
        Map<String, Double> accuracyByGroup = new HashMap<>();
        for (var entry : byGroup.entrySet()) {
            long correct = entry.getValue().stream()
                .filter(c -> {
                    String ans = agentClient.prompt().user(c.input()).call().content();
                    return c.matcher().matches(ans);
                }).count();
            accuracyByGroup.put(entry.getKey(), (double) correct / entry.getValue().size());
        }

        // 计算组间差异（demographic parity）
        double maxAcc = Collections.max(accuracyByGroup.values());
        double minAcc = Collections.min(accuracyByGroup.values());
        double disparity = maxAcc - minAcc;

        BiasReport report = new BiasReport(modelVersion, accuracyByGroup, disparity);

        // 门禁：组间准确率差异 > 5% 则阻断
        if (disparity > 0.05) {
            throw new BiasGateException(
                "偏见门禁未通过: 组间差异 " + disparity + " > 0.05, 详情=" + accuracyByGroup);
        }
        return report;
    }
}
...
/**
 * GDPR 被遗忘权的实现：用户请求删除时，
 * 从训练数据、向量库、对话历史三处清除其数据。
 */
@Service
public class RightToBeForgottenService {
    ...
    private final TrainingDataStore trainingData;

    public void forget(String userId) {
        // 1. 向量库：删除该用户的所有索引文档
        // filterByUserId(userId) 需构造 Filter.Expression（对应 VectorStore.delete(Filter.Expression) 重载）
        vectorStore.delete(filterByUserId(userId));

        // 2. 对话历史：加密删除
        conversations.purgeByUser(userId);

        // 3. 训练数据（若有自训练）：标记为"已撤回"，下次训练时排除
        trainingData.markRevoked(userId);

        // 4. 审计：记录删除事实（GDPR 要求证明已执行）
        auditLogger.info("被遗忘权已执行 userId={} timestamp={}", userId, Instant.now());
    }
}
```
