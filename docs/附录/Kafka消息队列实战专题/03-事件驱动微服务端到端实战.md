...
> **这份文档是什么**：Kafka 入门篇 + 进阶篇 的**毕业实战**。前两篇你学了"零件"（`KafkaTemplate` 发、`@KafkaListener` 收、Kafka、Kafka Streams、EDA 概念），这一篇带你**装一辆能跑的车**——从零搭一个**真正能运行的事件驱动微服务系统**，直接用 **`spring-boot-starter-kafka`**（原生 Spring Kafka，不用 Spring Cloud Stream 那层抽象）。
>
> **写给谁**：已经会用 Kafka 收发消息（会配 topic/分区/offset/消费组），也了解 EDA、幂等、重试死信、Saga 等概念，但还没"**做过一个完整系统**"的人。这篇就是那个系统。
>
> **业务场景**：一个最简化的**电商下单**——`订单服务` 下单 → 通知 `库存服务` 扣库存 → 通知 `支付服务` 处理支付。三个独立服务，**不直接调用**，全靠事件通信。如果中途失败（比如库存不够），要**自动回滚**（Saga 分布式事务）。
>
> **你能学到**：把零散知识串起来——事件设计、消费组隔离、幂等表、重试+死信、Saga 补偿事务**落地成真实代码**、可观测性、架构复盘。
>
> **版本前提（已校验）**：Spring Boot 4.1.0 + `spring-boot-starter-kafka`（版本由 Boot BOM 托管，**不需要 Spring Cloud BOM**）。代码照抄能跑。
>
> **和入门篇/进阶篇的关系**：入门篇/进阶篇同样是直接 Kafka（`KafkaTemplate` 发、`@KafkaListener` 收），本篇把它们教的"零件"组装成一个完整的事件驱动系统。**消费组、幂等、重试死信、at-least-once、Saga 这些概念完全通用**，风格与"管数分离实战"第 10 章的手写 Kafka 一致。
...
如果库存不够（`InventoryRejected`），不能让订单卡在"待处理"。要**自动回滚**——Saga 补偿事务模式（本章落地真实代码）：
...
flowchart TD
    OS["订单服务 order-svc"]
    IS["库存服务 inventory-svc"]
    PS["支付服务 payment-svc"]
    K[("Kafka<br/>所有事件都流经 Kafka")]
    OS -->|"OrderCreated"| K
    K -->|"OrderCreated"| IS
    IS -->|"InventoryReserved/Rejected"| K
    K -->|"InventoryReserved"| PS
    K -->|"InventoryReserved/Rejected"| OS
    PS -->|"PaymentCharged"| K
    K -->|"PaymentCharged"| OS
```
...
除了前面已经学过的，本章重点教三个新东西：
1. **消费组隔离**：三个服务订阅同一个 topic，怎么互不干扰（第 5 章）。
2. **Saga 落地**：从概念落到真实代码（第 6 章）。
3. **幂等表实战**：从原理落到数据库设计（第 4 章）。
...
> **注意命名**：全是"已XX"（Created/Reserved/Charged），不是"去XX"（Create/Reserve/Charge）。这是事件，不是命令。
...
**为什么 key 用 `orderId`**：Kafka 里同一个 key 的消息**永远进同一个分区**。用 `orderId` 当 key，某个订单的所有事件（OrderCreated → InventoryReserved/Rejected → PaymentCharged）都落在**同一分区**，消费时天然按订单有序——这是保证"同一订单的事件顺序不错乱"的关键。

> **注意**：保存订单（`repo.save`）和发事件（`kafkaTemplate.send`）是**两步**，不是原子的。崩了会丢事件——这是 Outbox 模式的由来（第 9.3 专门解决）。
...
**为什么必须幂等**：Kafka 是 at-least-once——同一个 `OrderCreated` **可能被投递多次**（比如库存服务处理完了、提交 offset 前崩了、重启重投）。没有幂等表，库存会被**扣两次**。有了幂等表，第二次收到时 `existsById` 为 true，直接跳过。
...
答案在**消费组**（概念前面讲过，这里落地）。在 Spring Kafka 里，消费组就是 `@KafkaListener` 的 `groupId` 属性——这是 Kafka 原生概念，不是框架抽象。
...
```mermaid
flowchart TD
    A["订单服务: 发 OrderCreated（订单=PENDING）"] --> B["库存服务: 库存不够 → 发 InventoryRejected"]
    B --> C["订单服务: 订阅 InventoryRejected → 订单=CANCELLED ← 这一步就是“补偿”"]
```
...
```mermaid
flowchart TD
    A["支付失败"] -->|"PaymentFailed"| B["库存服务<br/>还回库存（InventoryReleased）"]
    B -->|"补偿事件"| C["订单服务<br/>订单=CANCELLED"]
```
...
库存服务扣减时，数据库可能短暂连不上。加重试机制。在 Spring Kafka 里，重试配置在 **`spring.kafka.listener.retry.*`**（listener 层，不是 consumer 层）：
...
重试 3 次还失败（比如真的是 bug，或库存数据不存在），消息进死信 topic，留待人工处理。Spring Kafka 的 DLT 用 `DeadLetterPublishingRecoverer` 实现——自定义一个 `DefaultErrorHandler` Bean，把"重试模板"和"死信恢复器"绑在一起：
...
> **注意事务边界**：这里的 `@Transactional` 用的是 JPA/数据源事务管理器，管的是**数据库操作的原子性**；`KafkaTemplate.send`（发 `InventoryReserved`/`InventoryRejected`）**不在这个事务里**。如果要在"发消息"和"扣库存"之间也保证原子性，那是 Outbox 模式的范畴（见 9.3）。
...
| 用到的知识 | 来自 | 在实战中的体现 |
|-----------|------|--------------|
| KafkaTemplate / @KafkaListener | 入门篇 | 发/收事件（第 3/4 章） |
...
学完这个实战，你可以往这些方向继续：
1. **引入 Kafka Streams**：对事件流做实时聚合（如"每分钟下单量"）。
2. **Schema Registry**：给事件定义带版本的 schema，管理事件结构版本演进，避免改字段崩掉消费者。
3. **CQRS**：写侧发事件，读侧订阅事件建只读库。
4. **服务发现 + 网关**：用 Eureka + Gateway 收口多服务。
5. **Outbox 模式**：解决"DB 操作 + 发事件"的原子性问题（写 DB 和发事件要么都成要么都不成，是个经典难题）。

> **特别推荐研究 Outbox 模式**：本实战里"保存订单 + 发 OrderCreated"是两步（先 `repo.save` 再 `kafkaTemplate.send`），如果 save 成功但 send 前崩了，订单创建了但事件没发——下游永远不知道。Outbox 模式解决这个问题（把"待发事件"也写进同一个 DB 事务，再异步投递）。这是事件驱动系统的**进阶必修课**，下一篇会专门展开。
...
**解决**：Outbox 模式（见 9.3），生产必修。

#### 坑 4：死信 topic 无人盯，等于丢消息
**解决**：死信要有消费者 + 告警（第 7.2）。

#### 坑 5：事件结构改了，老消费者反序列化崩
**解决**：事件类加 `@JsonIgnoreProperties(ignoreUnknown = true)`、只加字段不删字段；进阶做法是上 Schema Registry 管理版本。
...
> **说明**：入门篇/进阶篇与本文统一用 Spring Kafka 原生 API（`KafkaTemplate` 发、`@KafkaListener` 收）。**消费组、幂等、重试死信、at-least-once、Saga 这些概念完全通用**，入门篇/进阶篇学的是"零件"，本文把它们装成系统。
