...
public class DataMaskingAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest req, CallAdvisorChain chain) {
        ...
        // 把 restore map 放入请求上下文，输出层用来还原（随请求链传递，非线程级）
        ChatClientRequest masked = ChatClientRequest.builder()
                .from(req)
                .contextEntry("masking.restore", restores)
                .userText(text)
                .build();
...
```java
@Component
public class ExfiltrationGuardAdvisor implements CallAdvisor {
