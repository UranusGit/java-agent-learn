...
> 「本文是对 [教程 03-React前端与AgenticUI/01-React状态管理 §5] 的深入展开」
>
...
public class DataMaskingAdvisor implements CallAdvisor {
    ...
    // 同一线程复用多请求、一次请求跨线程切换，ThreadLocal 必然串号/丢失（教程 05-Observation可观测/04-自定义Convention与Filter：工业标签与脱敏 §Context 传递）
    @Override
...
// 工具级 DLP 的正确落点是 ToolCallback 包装层（[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §正确落点]）——在工具执行前审查参数。
...
| [06-数据分类分级与密级传导] | 密级边界 | DLP 六层的"敏感"判定不应靠正则逐条猜，而应由数据密级驱动——§3 密级→控制矩阵给出每个控制点该挂哪层 |
