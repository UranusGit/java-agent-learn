...
public class DataMaskingAdvisor implements BaseAdvisor {
    ...
    private final ThreadLocal<Map<String,String>> restoreMap = ThreadLocal.withInitial(HashMap::new);

    @Override
    public AdvisedResponse aroundCall(AdvisedRequest req, CallAroundAdvisorChain chain) {
        String text = req.userText();
        Map<String,String> restores = restoreMap.get();
        ...
        // 把 restore map 放入上下文，输出层用来还原
        req.context().put("masking.restore", restores);

        AdvisedRequest masked = AdvisedRequest.from(req).withUserText(text).build();
...
- **ThreadLocal 清理**：请求结束后必须清理 restore map，防止内存泄露与跨请求污染。
...
@Component
@Order(500)
public class OutputLeakDetector implements BaseAdvisor {
    private final SystemPromptStore sysPromptStore;

    @Override
    public AdvisedResponse aroundCall(AdvisedRequest req, CallAroundAdvisorChain chain) {
        AdvisedResponse resp = chain.nextAroundCall(req);
        String output = resp.response().getResult().getOutput().getText();
...
```java
@Component
public class DlpAuditAdvisor implements BaseAdvisor {
    @Override
    public AdvisedResponse aroundCall(AdvisedRequest req, CallAroundAdvisorChain chain) {
        AdvisedResponse resp = chain.nextAroundCall(req);
...
| [09-AI治理与合规/02-数据隐私与偏见检测] | 隐私合规 | DLP 是隐私合规的工程实现 |
...
把上述六层防御落地到 Spring AI 2.0 的 `BaseAdvisor` 链，配合 Micrometer 指标与审计日志系统，Agent 的数据泄露风险就能被压缩到与传统企业应用相当的水平。但切记——DLP 是"持续博弈"而非"一次到位"，新的泄露手法会随 LLM 能力演化而出现，红队用例库必须随之更新。
