...
public class ClearanceFilteredRetriever {
    ...
    public Mono<List<Document>> retrieve(String query) {
        return Mono.deferContextual(ctx -> {                          // 实证：deferContextual(ContextView → Mono)
            AgentIdentity identity = ctx.getOrDefault("identity", null); // AgentIdentity 快照，见 [附录 08/05] §3
...
检索裁剪的完整展开（组交集、owner 特例、ABAC 出路、合法空结果语义）在 [附录 19-向量数据库与检索工程/02-检索权限与文档级裁剪 §3-§4]，本篇只保留密级这一维的最小骨架。
