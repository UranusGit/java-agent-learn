...
Flux<Event> ttlBuffer = eventSource
    .onBackpressureBuffer(
        500,
        Duration.ofSeconds(30),  // 缓冲超过 30 秒的数据也会被清除
    ...
    public Mono<String> call(String query) {
        return Mono.defer(() -> {
                ...
                if (rateLimiter.acquirePermission()) {
                    ...
                    return Mono.error(new RateLimitExceededException("超过 LLM API 频率限制"));
                }
            })
            .subscribeOn(Schedulers.boundedElastic());  // 限流等待在专用线程
    }
}
```
...
在教程 37（响应式错误处理）中，背压溢出是错误处理的重要场景之一。理解背压机制，才能在 Agent 高并发场景中做出正确的流量控制决策。
