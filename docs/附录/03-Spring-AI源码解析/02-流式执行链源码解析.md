...
>
> **读者画像**：要写 StreamAdvisor（[教程 14] 审计发现全体系零示例）或排查"流式下工具调用/记忆不生效"的工程师。
>
> **前置阅读**：[教程 14-Advisor链与拦截器]；[教程 19-流式工具调用与事件协议]（应用层视角）；[附录 05-SpringAI2-API基准/00-Advisor与ChatMemory]。
>
...
**关键事实**：两条链是**分开执行**的——同一个 Advisor 类如果只实现了 `adviseCall`，在 `stream()` 路径上根本不会被调用（反之亦然）。这就是 [教程 14] "StreamAdvisor 名存实亡"审计发现的机制根源：写了一个 CallAdvisor 就以为全覆盖，流式调用悄悄裸奔。**生产纪律：凡在流式产品上生效的 Advisor，必须实现 StreamAdvisor（或两栖实现）**。
...
[教程 19 §2.2] 应用层讲过"arguments 分片到达"；这里从框架视角补齐（内部实现细节**以仓库源码为准**，此处为机制模型）：
...
1. **观测上中间轮次可见**：gen_ai.chat.model 的 Observation 每轮一个 Span（[教程 22 §3] 的两轮 Span 树就是它）——**外部 Flux 是一个，内部轮次是多个模型调用**。
2. **取消传播有边界**：调用方 cancel Flux → 上游 HTTP 取消；但**已进入工具执行的部分不可中断**（除非工具自身协作式取消，[教程 19 §5]）——HITL 挂起点设计（[教程 28]）因此必须放在工具执行层而不是 Advisor 层。
3. **记忆写入要挂流的终点**：doOnComplete 拿到的是"流完成"信号，完整聚合后写入 ChatMemory；用户中途取消（doOnCancel）时部分回复是否入库是产品决策，但**实现位置就在这两个信号回调里**（[教程 04] 深化点"流式+记忆"的机制落点）。

## 4. 从机制到实现：写一个流式记忆 Advisor

把 §2/§3 的机制组合成 [教程 14] 缺失的完整示例：
...
| 5 | Observation 织入点 | 每轮 Span 的创建位置（对应 [教程 22 §2.2] 时序） |
...
4. **doOnNext 里做重活**——每个 chunk 一次回调，IO 会拖慢整个流的消费速率（背压传导，[附录 06-WebFlux与响应式编程/01]）。
5. **记忆写入只写 doOnComplete**——错误/取消路径丢失写入或重复写入（§4 的三分支）。

## 8. 总结

流式执行链的三把钥匙：**两条链分开执行（两栖 Advisor 纪律）、流的生命周期挂在信号而非方法（doFinally/doOnComplete/doOnCancel 三窗口）、工具调用在流内聚合续流（外层一个 Flux、内层多轮 Span）**。拿着这张地图去读 spring-ai 源码，[教程 14]/[教程 19] 的所有"流式下为什么这样"都有了机制答案。
