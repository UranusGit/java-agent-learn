...
> 「本文是对 [教程 04-企业级架构主干/01-微服务拆分与Agent部署 §2-§5] 的深入展开；Kafka 中间件层机制（生产者/消费者/事务/存储/运维）全面下钻见 [教程 07-Kafka事件骨干/00-Kafka全景与核心概念]」
...
### 4.4 2PC / TCC / 编排式 Saga：选型对比

Saga 不是分布式事务的唯一方案——工程上还有 2PC（两阶段提交）和 TCC（Try-Confirm-Cancel）可选，但 Agent 场景对三者的适配度差异极大。选型的关键不在"哪个更高级"，而在**各方案对参与方的前提要求**：

| 维度 | 2PC（两阶段提交） | TCC | 编排式 Saga（本篇 §4.2/§4.3） |
|------|-----------------|-----|------------------------------|
| 一致性 | 强一致（同步阻塞到全部提交） | 最终一致（Try 预留 + Confirm/Cancel 收尾） | 最终一致（正向执行 + 补偿回滚） |
| 对参与方的前提 | 必须支持 XA 协议（两阶段锁定、可回滚的事务资源） | 每个参与方必须实现 **Try（资源预留/冻结）**、Confirm、Cancel 三个接口 | 只需"能正向执行 + 有一个可达的补偿动作" |
| 阻塞与单点 | 协调者单点，参与者锁定资源等协调者裁决 | 无长期锁定，但业务侵入大（三个接口都要写） | 无锁定，各步独立推进 |
| Agent 场景相关性 | **低**——Agent 工具多是外部 HTTP/SaaS（发邮件、建工单、调三方 API），没有 XA 参与者可言 | **低**——外部系统的副作用无法冻结：你没法让第三方 API"先冻结这封邮件等确认"，Try 阶段对组织边界之外的资源不成立 | **高**——每步只要求正向动作 + 补偿动作，恰是工具调用的自然形态 |

结论收敛为一句话：**Agent 的工具副作用大多发生在组织边界之外（外部 HTTP/SaaS），既做不了 XA 的两阶段锁定，也做不了 TCC 的 Try 阶段资源预留——两个前提性方案在 Agent 侧先天不成立，编排式 Saga（事件驱动推进 + 逆序补偿 + 补偿失败人工介入）是默认也是唯一现实的选择**。TCC 唯一有效的落点在 Agent 自己体系内的资源上（如先冻结租户配额再执行高成本任务、先预占工单号再分发），这类"自家资源可 freeze"的场景按 TCC 做预留、其余跨边界步骤仍走 Saga 补偿。补偿失败的兜底（告警 + 人工介入）已在 §4.3 实现——那是 Saga 方案里"最终一致收敛不了"时的最后闸门。
