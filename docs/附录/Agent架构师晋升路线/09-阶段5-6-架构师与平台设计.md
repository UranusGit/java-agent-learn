...
读 LangChain4j（`langchain4j/langchain4j`）时同理：核心循环在 `langchain4j-core` 与 `langchain4j` 模块里，`AiServices` 的代理生成逻辑在 `langchain4j` 模块的 `service` 包。**对照这两个框架时，永远问同一个问题：工具调用循环放在哪一层、谁来触发递归**——两个框架的答案不同，这就是「设计取舍」。
