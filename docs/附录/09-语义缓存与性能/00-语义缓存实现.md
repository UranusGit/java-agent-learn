...
> 「本文是对 [教程 08-架构师进阶/00-上下文工程 §5-§6] 的深入展开」
...
public Flux<ChatClientResponse> adviseStream(ChatClientRequest request,
    ...
    if (cached.isPresent()) {
        // 缓存命中：将缓存结果作为流式返回
        // record CacheHit(String responseText, double similarity) 的 accessor 是 responseText()（见 §3.1 checkCache 构造）
        String text = cached.get().responseText();
...
public class CacheTTLManager {
    ...
    public Duration getTTL(String query, String response) {
        // 根据 RAG 上下文新鲜度设置 TTL
        boolean hasRagContext = !response.contains("[NO_CONTEXT]")
                && ragContextCache.containsKey(response.hashCode());
