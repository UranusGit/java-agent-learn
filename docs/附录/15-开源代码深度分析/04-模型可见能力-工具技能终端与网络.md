...
| `scrubbedParentEnv` + 显式合并 | 进程环境传递 | 默认 scrubbed、显式放行是进程安全的黄金法则（对应 [教程 25-安全与权限控制]） |
| MCP 桥 `mcp__server__tool` | Spring AI MCP Client | 与 Spring AI 的 MCP 工具前缀约定同构（对应 [教程 10-MCP协议]） |
