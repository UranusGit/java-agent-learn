...
> **定位**：解析 codex 的模型客户端（双层/重试状态机/多 provider）、前端协议（方法枚举/版本化/投影层）、以及四个进阶机制——Guardian 自动审阅、Code Mode、shell 快照、托管 MITM 网络。读者画像：要构建多前端 Agent 平台与"AI 审 AI"能力的 Java 架构师。锚点：[教程 32-模型路由与降级]、[教程 44-多模型协作与供应策略]、[教程 31-安全与权限控制]。行号基于分析时版本。
...
> 对照 [教程 32-模型路由与降级]：降级粘性是关键——同一会话中途在 provider 间抖动会导致上下文格式不兼容（呼应 02 篇"模型切换触发压缩"）。
...
| 集中重试状态机 | `Retry.backoff(...)` + 按请求类别 filter（采样/工具/压缩不同策略） | [教程 42] retryWhen 的分类版 |
| 方法枚举+投影层 | WebSocket 消息 sealed interface（Op/Event，[教程 19] 同构）+ 前端视图服务（内部全量事件 → 各前端投影） | 多端复用一个会话后端 |
