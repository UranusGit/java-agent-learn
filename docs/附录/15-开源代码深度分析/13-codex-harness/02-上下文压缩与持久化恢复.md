...
> **定位**：解析 codex 的上下文管理（版本号化历史、三条压缩路径、pre/mid-turn 重注入、世界状态差异注入）与持久化恢复（append-only JSONL、投影方法族、TurnContext 快照）。读者画像：要解决"长会话上下文爆炸"与"崩溃恢复"的 Java 架构师。锚点：[教程 34-上下文工程]、[教程 40-长任务持久化与中断恢复]、[教程 25-历史记录持久化与合规]。行号基于分析时版本。
...
**为什么不用可变 List**：可变历史要处理"读者读到一半被改"的并发地狱；`Arc` 整体替换 + 版本号让读路径零锁零拷贝、写路径天生原子（对照 [教程 12-Agent状态管理] 的快照思想）。
...
> **静态一次注入，动态最小化重注入**——这一句就是上下文工程的预算哲学（对照 [教程 34-上下文工程] 的五层拼接与 Token 预算分配）。
...
| world_state diff | 自研 `WorldStateSnapshot`（时间/cwd/git）+ baseline 比较，变化才生成增量 user 消息 | 与 [教程 34] 五层拼接的第二层（环境层）对应 |
| JSONL rollout | 事件溯源式追加日志（对照项目板块事件溯源篇的方法论）；Jackson 每行一 JSON；恢复=按 InitialHistory 类型重放 | 幂等写入：`(id, replay)` 唯一约束或去重表 |
| token 服务器优先 | `Usage.getPromptTokens()/getTotalTokens()`（已实证方法名）做决策；本地估算只做预警 | [教程 27-成本治理] 同口径 |
