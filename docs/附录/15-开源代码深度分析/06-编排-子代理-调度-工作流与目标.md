...
|---|---|---|
| `ctx.subagents` 具名 provider 注册表 | Spring 多实现注入 | 「多个 child 运输方式共存」时用注册表，「一个执行语义」时用单引擎（对应 [教程 08-多Agent协作]） |
| Activation 可续作（inbox 唯一队列） | 会话状态机 | 续作不建第二套状态机，用 Agent quiescence + owned-child set 推导（对应 [教程 11-Agent状态管理]） |
| workflow worker + vm context 脚本 | 脚本引擎/DSL | 模型写脚本、host 执行扇出是 Agent 编排的高级形态（对应 [教程 31-Agent工作流编排]） |
| Ralph fresh-round + handoff | 多轮反思 | 「共享工作区为权威、handoff 只补充」防止上下文污染（对应 [教程 32-自我反思与Agent评估]） |
