...
```mermaid
stateDiagram-v2
    [*] --> FLOWING: subscribe() 订阅后水流开始
    FLOWING --> FLOWING: onNext 吐出数据（可多次）
    FLOWING --> DONE: onComplete 水龙头关了（正常结束）
    FLOWING --> BROKEN: onError 水管爆了（出错）
    DONE --> [*]
    BROKEN --> [*]
```
...
```mermaid
flowchart LR
    SRC["Flux 数据源（水管）"] -->|"数据流"| TRANS["变换型<br/>map / filter / flatMap<br/>改变流的内容（加过滤器）"]
    TRANS -->|"改造后的水"| OBS["旁路观察型 do*<br/>doOnNext / doOnComplete<br/>钻洞装仪表盘，只看不改"]
    OBS -->|"水还是原来的水"| SUB["subscribe 订阅<br/>最终消费"]
```
...
```mermaid
flowchart LR
    A["写 .map().filter().doOnNext()<br/>只是描述：如果有人开水龙头该这么处理"] --> B{"有人调用 subscribe()<br/>或框架帮你订阅?"}
    B -->|"否"| C["水流从未流动<br/>代码不会执行"]
    B -->|"是"| D["水龙头打开<br/>整条流水线开始跑"]
```
