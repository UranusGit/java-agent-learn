...
第 00 关你用了 `observe()`（函数式）。这一关我们还需要能"跨多行 + 打事件"的 **Scope 式**。在工程新增 `ObsStep2Config.java`（包 `com.example.obsdemo.step2`）：

```java
package com.example.obsdemo.step2;
...
public class ObsStep2Config {
    ...
    @Bean
    RouterFunction<ServerResponse> step2Routes() {
        // ① 函数式：observe() 自动 start+stop，一行 = 完整生命周期，返回值透传
        return RouterFunctions.route(GET("/life/functional"), this::functional)
                // ② Scope 式：自己 start/stop，能中途 event 多步、跨多行
                .andRoute(GET("/life/scoped"), this::scoped)
                // ③ 主动制造失败，看 onError 如何被自动记录
                .andRoute(GET("/life/error"), this::errorFlow);
    }

    // ---------- ① 函数式（推荐） ----------
    private Mono<ServerResponse> functional(ServerRequest req) {
        String reply = Observation.createNotStarted("life.fn", () -> new Observation.Context(), registry)
    ...
    // ---------- ② Scope 式（需要自己控制时） ----------
    private Mono<ServerResponse> scoped(ServerRequest req) {
        ...
        try (Scope scope = obs.openScope()) {      // OPEN / CLOSE 时刻由此控制
            ...
            return ServerResponse.ok().bodyValue("scoped done");
        } catch (Exception e) {
    ...
    // ---------- ③ 异常路径 ----------
    private Mono<ServerResponse> errorFlow(ServerRequest req) {
        try {
            ...
            return ServerResponse.ok().bodyValue("不会到这里");
        } catch (IllegalStateException e) {
            return ServerResponse.ok().bodyValue("err caught: " + e.getMessage());
        }
    }
...
测试代码（`src/test/java/com/example/obsdemo/step2/LifecycleTest.java`）：
