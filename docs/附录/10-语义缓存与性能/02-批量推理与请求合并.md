...
public class BatchInferenceService {
    ...
    /**
     * 批量分类：将 10000 条评论一次性提交
     */
    public Mono<BatchResult> batchClassify(List<String> reviews) {
        // 1. 构建批量请求文件
        List<BatchRequestItem> items = reviews.stream()
            .map(review -> BatchRequestItem.builder()
        ...
        // 2. 上传文件
        String fileId = batchApi.uploadFile(
            items.stream()
                .map(this::toJsonLine)
        ...
        // 3. 创建 Batch 任务
        BatchJob job = batchApi.createBatch(BatchCreateRequest.builder()
            .inputFileId(fileId)
            .endpoint("/v1/chat/completions")
            .completionWindow("24h")
            .build());

        return pollUntilComplete(job.id());
    }

    private Mono<BatchResult> pollUntilComplete(String batchId) {
        return Mono.defer(() -> {
                BatchJob job = batchApi.getBatch(batchId);
                if ("completed".equals(job.status())) {
                    return Mono.just(downloadResults(job.outputFileId()));
                }
                // 还在处理，等待后重试
                return Mono.delay(Duration.ofSeconds(30))
                    .then(pollUntilComplete(batchId));
            });
    }
}
```
    ...
    private Mono<List<ClassifyResult>> processBatch(
        ...
        return Mono.fromCallable(() ->
                chatClient.prompt()
                    .user(prompt)
                    .call()
                    .entity(new TypeReference<List<ClassifyResult>>() {})
            )
            .subscribeOn(Schedulers.boundedElastic())
