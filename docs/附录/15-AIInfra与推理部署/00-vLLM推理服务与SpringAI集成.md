...
> **定位**：本文是 [教程 08-架构师进阶/10-多模型协作与供应策略]「自建 vs 商用」决策的自建侧下钻：如何用 vLLM 部署自有推理服务，并通过 OpenAI 兼容 API 无缝接入 Spring AI。读者画像：正在做模型供应策略架构决策、或需要控制推理成本的架构师。前置阅读：[教程 04-企业级架构主干/12-模型路由与降级]、[教程 08-架构师进阶/10-多模型协作与供应策略]、[教程 04-企业级架构主干/07-成本治理与Token计量]。
...
2. **数据合规**：金融/政企/医疗场景数据不能出域（呼应 [教程 04-企业级架构主干/06-多租户隔离与资源治理]）
...
「想深入 Prompt/KV Cache 原理？→ [附录 09-语义缓存与性能/01-Prompt缓存与KVCache]」
...
**PD 分离的架构逻辑**：Prefill（首 Token 前的计算）吃算力，Decode（逐 Token 生成）吃显存带宽——两类负载特征相反，混部署互相拖累；分离后各自独立扩缩容。这与你学过的 [教程 04-企业级架构主干/01-微服务拆分与Agent部署]「按负载特征拆分」是同一条架构原则在 GPU 领域的投影。
...
**这正是自建路线最大的工程红利**：模型路由层（[教程 04-企业级架构主干/12-模型路由与降级]）把「商用 API」和「自建 vLLM」抽象为同一种 OpenAI 兼容端点，降级链就是改一个 base-url 的事。
...
**关键结论**：商用 API 按 Token 计费，自建按卡时计费。负载越平稳、越高频，自建单位成本越低；负载尖刺明显则自建浪费（卡闲着也在烧钱）——所以生产上主流是**混合编排**：基线流量走自建、溢出流量走商用 API（呼应 [教程 08-架构师进阶/10-多模型协作与供应策略 §边缘部署]）。

## 5. 常见坑

- **上下文长度≠免费**：长上下文吃 KV Cache 显存，并发数会骤降——上下文预算管理见 [教程 08-架构师进阶/00-上下文工程]
- **量化换质量**：INT8/INT4 量化省显存但伤推理质量，先在评测集上量化对比再上生产（评测方法见 [教程 08-架构师进阶/03-自我反思与Agent评估]）
- **流式背压**：vLLM 流式输出速度可能快于前端消费，WebFlux 侧要有背压策略「想深入？→ [教程 01-WebFlux与响应式编程/02-背压与流量控制]」
