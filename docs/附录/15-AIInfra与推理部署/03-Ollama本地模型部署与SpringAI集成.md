...
| **参数幻觉** | 生成 Schema 里不存在的字段，或把枚举值编错 | 工具参数设计趋简（少层级、短枚举）；`@ToolParam(description=...)` 写清约束；输出侧用结构化输出约束兜底（[教程 02-SpringAI核心机制/03-结构化输出]） |
...
1. **吞吐节奏不同**：本地单机吞吐远低于商用 API，前端打字机效果会更「卡」。这不是 bug，不要在前端用固定节奏动画掩盖——真实反映生成速度，并把「本地推理中」的状态显式传达给用户（SSE 通道设计的完整方法论见 [教程 02-SpringAI核心机制/00-SSE流式通信]）。
