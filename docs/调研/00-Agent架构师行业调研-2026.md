...
### Claude Code 架构分析（本仓库参考资料）
- 《Claude Code 源码学习：如何设计一个强大的 AI Agent》45 章源码分析
- 7 大核心架构模式：Agent 循环、工具注册表、权限中间件、上下文预算、多 Agent 委派、流式处理、渐进降级
- 核心设计原则：流式优先、默认隔离、权限最小化、渐进增强、用户始终可控、优雅降级、可观测性
- 在本项目二（CodeForge）中落地实践
