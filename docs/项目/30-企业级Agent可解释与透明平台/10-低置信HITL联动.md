...
```bash
mvn test -Dtest=ConfidenceRiskRouteTest        # 按 §二 手写四分支/阈值边界用例后执行
# 预期输出（节选）：
#   Tests run: 5, Failures: 0, Errors: 0, Skipped: 0
#   低置信 100% 不落 AUTO；边界 =CONF_HI 与分支注释一致
#   BUILD SUCCESS
```
...
> 各节断言已上移至 §二.1（置信 × 风险路由矩阵）；工作台材料与回流在 §三.1 核对；本表为整篇迭代的回归验收，不重复材料。

| # | 验收项（断言） | 标准 | 复验方式 |
|---|---------------|------|---------|
| 1 | 四分支路由 | BLOCK_REVIEW/HUMAN_CONFIRM/AUTO/FLAG 各按矩阵命中 | 复验：执行 §二.1 核对命令 |
| 2 | 低置信不自动 | 低置信 100% 不落 `AUTO`（触发复核/拦截/标记） | §二.1 步骤5 |
| 3 | 阈值边界一致 | `=CONF_HI` 语义与分支注释一致 | §二.1 步骤5 |
| 4 | 复核有据 | 工作台四类解释材料齐备 | §三/§三.1 |
| 5 | 复核结果回流 | 回流 05 校准/规则优化/09 飞轮 | §二.1 步骤6 |

**回归失败排查**：按 §二.1 失败排查逐条回溯（highConf 判断反/漏 HUMAN_CONFIRM/边界条件不一致/回流钩子未挂）。

## 六、验收对照

> 00-需求分析量化验收⑤（低置信 100% 触发降级/HITL）在本迭代直接收口。

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 低置信 100% 复核/拦截 | 矩阵保证低置信不自动放行（低风险仅标记） | ✅（§二.1 步骤1/4-5） |
| 复核有据 | 解释即审核材料（四类齐备） | ✅（§三/§三.1） |
| 复核结果回流 | 校准修正 + 规则优化 + 飞轮 | ✅（§二.1 步骤6） |
