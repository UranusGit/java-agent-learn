# 项目 11：工业质检与预测性维护 — 02-迭代一：视觉质检多模态

> **定位**：把质检从"人眼 + 判级不一"升级为"边缘 CV 快筛 + 多模态 LLM 精判"的三层接力架构——小模型毫秒级过滤，LLM 只对灰色样本精判并输出可解释判级报告。前沿 02 多模态 + 教程 03 工具调用的落地。本文给出**完整可手写代码**。
>
> 「遇到阻塞？→ [前沿 02-多模态Agent]、[教程 03-工具调用]、[教程 13-结构化输出]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。质检 Agent 判"合格/缺陷"要看得见依据——哪个 ROI 区域、模型置信、缺陷类型。检测 emit 质检事件：`{img, 置信, 缺陷类型, 命中ROI框, 判定}`。质检员看到"区域#3 置信0.87→判为划痕→复检"，可点开查实时 ROI 框；低置信自动转人工复核（过程可见+衔接 HITL）。
...
**材料**：合格/缺陷样品图像集（含难例低对比度）+ 标注真值；检测阈值配置。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「缺陷识别率」场景执行 | 综合缺陷识别率 ≥ 99%（对照人工 + 标准件） |
| 2 | 用材料构造「节拍」场景执行 | L1 CV 快筛 < 15ms（产线节拍内）；L2 灰色样本精判 < 200ms |
| 3 | 用材料构造「判级可解释」场景执行 | 精判报告 100% 含位置/类型/置信度/理由 |
| 4 | 用材料构造「漏检率」场景执行 | 小缺陷漏检率从 14% 降到 < 5% |
| 5 | 用材料构造「数据闭环」场景执行 | 人工确权 100% 回流；难例达标触发增量训练 |
...
> **定位回顾**：v2 完成"AI 精判层"。下一站 [03-迭代二-设备预测性维护.md](03-迭代二-设备预测性维护.md)——把 v1 的 EWMA 升级为预测性维护（RUL + LLM 解释 + 提前预警）。
