...
> **定位**：把"PdM 预警 → 工单 → EAM/ERP"闭环自动化——LLM 生成"建议工单"（含证据链），**落 EAM 前必须 HITL 审批**。2026 现实是"assistive 为主、governed autonomy 起步"（[调研 §工单自动化]）。教程 03-React前端与AgenticUI/04-流式工具调用与事件协议 HITL 的落地。本文给出**完整可手写代码**。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 全篇]、[附录 05-SpringAI2-API基准/02 §ToolCallingManager]、[教程 08-架构师进阶/06-长任务持久化与中断恢复 §挂起]」
...
/** 挂起点（[教程 08-架构师进阶/06-长任务持久化与中断恢复]）：持久化完整工单意图 + 状态机，审批员跨天/跨重启仍可处理 */
...
/**
 * HITL 审批闸门（[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议 §正确落点]）：拦截 submitWorkOrder 工具调用，
...
public class ApprovalService {
    ...
    private final JdbcTemplate jdbcTemplate;   // v5 新增：维修结果回流落库
    ...
    public void onRepairComplete(String deviceId, String rootCause, boolean effective) {
        ...
        jdbcTemplate.update(
                "INSERT INTO repair_history(device_id, root_cause, effective) VALUES (?, ?, ?)",
                deviceId, rootCause, effective);
        // v3 MaintenanceKnowledgeBase 检索到该记录后，作为下次预测的案例上下文（双向闭环）
    }
}
```
...
### 4.9.1 `WorkOrderController.java`（HTTP 入口：/api/workorder/generate）

§4.10 材料 A 的端点由下述最小 Controller 提供：复用 §4.9 `WorkOrderAgent.generateWorkOrder`（请求体即 v3 `MaintenanceInsight` 的 JSON）：
...
public class WorkOrderController {
    ...
    public WorkOrderController(WorkOrderAgent workOrderAgent) {
        this.workOrderAgent = workOrderAgent;
    }

    @PostMapping(value = "/generate", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<SuggestedWorkOrder> generate(@RequestBody MaintenanceInsight insight) {
        return workOrderAgent.generateWorkOrder(insight);
    }
}
```
...
```sh
# A. 生成建议工单（§4.9 WorkOrderAgent.generateWorkOrder，经 §4.9.1 WorkOrderController 暴露；
#    请求体即 v3 MaintenanceInsight 的 JSON）
curl -X POST "http://localhost:8081/api/workorder/generate" \
  -H "Content-Type: application/json" \
  -d '{"deviceId":"pump-01","insight":"轴承振动RMS爬升","rulDays":6,"evidence":["振动RMS爬升","RUL 6天"],"recommendation":"3天内更换6205轴承"}'

# B. 审批回调（§4.8 /api/approvals/{id}）——批准
curl -X POST "http://localhost:8081/api/approvals/APP-0001?approved=true&comment=同意&approver=maintainer-01"

# C. 审批拒绝与重复提交（验证防绕过）
curl -X POST "http://localhost:8081/api/approvals/APP-0002?approved=false&comment=产能不允许&approver=maintainer-01"
```
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 工单时效 | 预测→建议工单 < 1 分钟（§2-1，§6 回归） | ☐ |
| HITL 强制 | 无审批直接落 EAM 的路径不存在（§2-2，§6 回归） | ☐ |
| 证据可复核 | 建议工单 100% 带证据链（§2-3，§6 回归） | ☐ |
| 拒绝防绕过 | 审批拒绝后 Agent 不得重试同一工单（§2-4，§6 回归） | ☐ |
| 闭环回流 | 维修结果 100% 回流喂预测（§2-5，§6 回归） | ☐ |
