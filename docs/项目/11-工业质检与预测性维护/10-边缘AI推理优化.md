...
>
> 「遇到阻塞？→ [教程 08-架构师进阶/10-多模型协作与供应策略 §边缘部署]、[教程 08-架构师进阶/04-Agent性能优化 §批量与并行]、[教程 04-企业级架构主干/12-模型路由与降级]、[教程 08-架构师进阶/06-长任务持久化与中断恢复 §幂等]、[附录 15-AIInfra与推理部署/00-vLLM推理服务与SpringAI集成]、[前沿 10-边缘Agent与端云协同]」
...
flowchart TB
    ...
    PRUNE --> CHECK
    CHECK -->|是| DONE["上线边缘"]
    CHECK -->|否| ESC["升级硬件或<br/>任务回云端（教程 05-Observation可观测/11-深度整合：SpringAI2与Observation的完整结合面 §供应策略）"]
...
> **为什么批处理在 CV 侧而不在 LLM 侧**：LLM 生成是自回归逐 token，多请求合批只省了调度开销（vLLM 的 continuous batching 在云端已做，边缘 Ollama 并发弱）；而 YOLO 推理是"一次前向算一张图"，批 N 张图的耗时远小于 N 次单张——**攒批收益在 CV 侧是数量级的**（[教程 08-架构师进阶/04-Agent性能优化 §批量请求合并]）。
...
**做法**：L1 快筛跑低分辨率全图（1280px，快、省算力）；命中疑似缺陷 ROI 后，**裁剪该 ROI 并放大到原分辨率**（2592px 工业相机原图）再送 L2 VLM——VLM 看的是"高清局部"而非"低清全图"，既省图像 Token 又提升小缺陷检出（[教程 09-前沿专题/02-多模态Agent开发 §6 成本模型]：图像 Token 随分辨率平方增长）。
...
> v4 的 `syncWhenReconnected` 是"把缓冲全量重发"，云端已处理过的样本会被**重复计数**（重复告警、重复统计）。v9 加两件事：① 每条信号带**幂等键**（`deviceId + ts + 指标` 的哈希），云端 ingest 按键去重；② 恢复时先发**对账清单**（时间段 + 计数 + 校验和），云端比对后只回放差异部分（[教程 08-架构师进阶/06-长任务持久化与中断恢复 §幂等重试]）。
...
> v2 的灰色带（置信 0.70-0.95 送 L2、L2 判不了的送 L3）是静态的。v9 把它变成**闭环**：云端复核（L3 会诊 + 人工确权）结果回流，算"边缘直判一致率"——一致率高说明边缘模型在这个工况下可靠，**放宽灰色带**（更多样本边缘直接判级，省成本省时延）；一致率低说明边缘模型在漂移或工况变了，**收窄灰色带**（更多样本送云端复核）。调节必须在**人工设定的安全边界内**（上下限），且综合识别率 ≥ 99% 是不可破的承诺线（[教程 04-企业级架构主干/12-模型路由与降级]）。
...
**回归断言**（§8 测试与验证通过后，按 §2 验收表整体验收）：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 高峰期 1 小时压测 L2 精判 | P95 < 200ms（验收 1）；显存/内存 ≤ 上限 70%（验收 2） |
| 2 | 断网 4 小时剧本 E | L1 全程边缘自治、缓冲零丢失；恢复后对账差异回放、重复上报率 0%（验收 3/4） |
| 3 | 分流闭环回放 | 灰色带随一致率自动调节，综合识别率 ≥ 99% 不破线（验收 5） |

**失败排查**：时延不达标→微批窗口/ROI 裁剪未生效（攒批掩盖单帧耗时）；对账重复→幂等键字段序或清单 checksum 未按 §6 同构；识别率破线→`SplitRatioController.adjust` 的 recall 闸门未接。
