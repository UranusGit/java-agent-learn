...
curl -X POST "http://localhost:8081/api/pdm/explain/pump-01"
```
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 预警提前 | 可预测故障预警提前 ≥ 14 天（§2-1，§6 回归） | ☐ |
| RUL 准确 | RUL 估计 MAE ≤ 20%（§2-2，§6 回归） | ☐ |
| 成本 | LLM 触发率 < 5% 流量、单次洞察 Token ≤ 1k（§2-3，§6 回归） | ☐ |
| 解释可读 | 洞察含征兆/根因/建议/证据（§2-4，§6 回归） | ☐ |
| 误报可控 | 正常设备误预警率 < 5%（§2-5，§6 回归） | ☐ |
