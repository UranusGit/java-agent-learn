...
> **定位**：把 v2 埋下的数据闭环（难例回流）升级为**全生命周期模型管理**——模型版本与产线绑定（换线不停产）、漂移检测（数据漂移 + 误判率漂移双信号自动触发再训练**评估**）、再训练闭环（新样本 → 金标评估 → 影子运行 → 灰度上线）、模型卡片进发布闸门（工艺合规审计）。这是教程 05-Observation可观测/08-流式响应的观测：stream模式下的span闭合 数据飞轮在"质检/PdM 模型资产"上的落地，也是 [附录 12-AI治理与合规/01-模型卡片与数据卡片] 的工程化。前置阅读：[02-视觉质检多模态](02-视觉质检多模态.md)、[10-边缘AI推理优化](10-边缘AI推理优化.md)、[11-多模态融合质检](11-多模态融合质检.md)。
>
> 「遇到阻塞？→ [教程 08-架构师进阶/07-数据飞轮与持续改进 §5 离线评估、§6 优化决策、§7 灰度发布]、[教程 04-企业级架构主干/09-灰度发布与版本管理 §流量切分]、[附录 12-AI治理与合规/01-模型卡片与数据卡片]、[教程 04-企业级架构主干/05-历史记录持久化与合规 §审计]」
...
- [ ] 第 3 行"架构演进"（数据闭环 → 模型资产治理）与 §3-§6 代码产物（ModelRegistry / DriftDetector / RetrainPipelineService / ModelCardService）对应
- [ ] 四个痛点分别被 §3 版本绑定换线 / §4 漂移检测 / §5 再训练闸门 / §6 模型卡片审计解决
...
- [ ] 五项验收（换线 0 停线 / PSI 或误判率 24h 内出报告 / 三道闸门拦劣化 / 灰度一键回切 / 四元组 100% 可查）都能在 §3-§6 代码（ModelRegistry / DriftDetector / RetrainPipelineService / ModelCardService）中找到落地件
- [ ] 验收项 3"三道闸门"对应 §5 `RetrainPipelineService` 的 goldenGate/shadowGate/cardGate + 灰度
...
### 3.3 本节测试与验证（模型台账与换线绑定）

**前置条件**：`model_version` / `model_line_binding` 表已建（§3.1 DDL）；PostgreSQL 第二数据源可用。

```sql
-- A. 换线绑定核对（原型插入：SHADOW 预热 → promoteToPrimary）
INSERT INTO model_line_binding(line_id, model_id, mode, bound_at, bound_by) VALUES ('line-a','vision-vlm@r3','SHADOW', now(), 'tester'),
('line-a','vision-vlm@r2','PRIMARY', now(), 'tester');
```

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 模拟换线 `bindShadow(line-a, vY)` | 新模型 vY 以 `SHADOW` 入库，旧 vX 仍 `PRIMARY`（双版本在线，不影响产线判级） |
| 2 | `promoteToPrimary(line-a, vY)` | 同事务原子切换（vY → PRIMARY，vX → ARCHIVED）；失败抛异常保持原绑定（换线 < 30s、0 停线） |
| 3 | `currentPrimary(line-a)` | 返回当前 PRIMARY 模型 id（审计四元组可查） |
| 4 | 构造未 SHADOW 直接 promote | `promoted != 1` → 抛"影子绑定不存在"（防误切换） |

**失败排查**：切换失败无回滚→`promoteToPrimary` 未包 `@Transactional`（两步 UPDATE 不成原子）；误切换→未校验 SHADOW 行；绑定历史查不到→`bindShadow/promoteToPrimary` 未写满审计字段（bound_by / 时间戳）。
...
**前置条件**：`ModelManagementTest` 可运行（§7）；`inspection_hard_example` 有近 7 天确权数据（确认误判率信号）。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `psi_detects_distribution_shift`（同分布 vs 右移分布） | 同分布 PSI < 0.1；右移后 PSI > 0.2 → `isDrifted=true`（数据漂移信号） |
| 2 | `error_rate_drift_rule`（0.045 vs 0.025 / 绝对值<3% / 环比<50%） | 误判率漂移规则正确：绝对值 > 3% 且环比 +50% 才触发（有标签信号） |
| 3 | 任意一信号超阈值 | 产出 `drift_report` 并触发**再训练评估**（非直接再训练，人工闸门，验收项 2） |

**失败排查**：PSI 全 < 0.2→分箱数 or 分布平移幅度不足；误判率恒 0→`confirmedErrorRate` 的 `inspection_hard_example` 缺近 7 天数据；触发即训练→确认走的是评估闸门而非自动再训练（ADR-011-29）。

## 5. 再训练闭环：金标评估 → 影子 → 灰度

> **闸门哲学**（[教程 05-Observation可观测/08-流式响应的观测：stream模式下的span闭合 §5、§6]）：每次模型变更必须可证明"不劣化"才能前进。三道闸门：**金标评估闸门**（离线 F1 不低于线上 - 1pp）、**影子闸门**（影子期误判率不劣化）、**灰度闸门**（灰度产线识别率不劣化 + 可一键回切）。任何一道不过 → 停在原地，旧版本继续服务。
...
flowchart LR
    ...
    CARD -->|是| GRAY["灰度 10% 产线<br/>（教程 04-企业级架构主干/00-管控分离架构 §流量切分）"]
...
**金标集（golden set）建设**：从 `inspection_hard_example` + 质检升级队列（v10）按缺陷类型分层抽样，每类 ≥ 50 件、总数 ≥ 500 件，人工双盲标注（两人一致才入集）；金标集**版本化**（`train_data` 快照 ID），评估可复现（[教程 08-架构师进阶/03-自我反思与Agent评估 §评估指标体系]——金标集是质检平台的护城河资产，详见 [14-工业前沿与演进方向 §评估驱动]）。

### 5.1 本节核对（再训练闭环三道闸门）

- [ ] 流程图的三道闸门（金标 F1 不低于线上-1pp / 影子误判率不劣化 / 卡片齐全）与 `RetrainPipelineService` 的 goldenGate/shadowGate/cardGate 一致
- [ ] "漂移只触发评估、不自动再训练"与 ADR-011-29、stateDiagram 的 `TRAINING → GOLDEN_EVAL → SHADOWING → GRAY → LIVE` 状态机一致
- [ ] 金标集三要素（分层抽样 ≥500 双盲 / 版本化 / 人工闸门）能复述

## 6. 模型卡片与工艺合规

> 审计的三个问题（[附录 12-AI治理与合规/01-模型卡片与数据卡片]）：这条产线当时用的什么模型？模型拿什么数据训练的？谁批准上线的？——模型卡片（Model Card）+ 绑定历史（§3.1 表）就是答案。v11 把"卡片齐全"设为**发布闸门**：无卡片不上线。
...
**工艺合规联动**：卡片 `limitations` 字段与工艺约束（v6 `ConstraintTable`）互查——模型局限若涉及工艺参数敏感区，发布前必须工艺工程师会签；审计报表按"产线 × 时间段"查绑定历史，输出"当时用的哪个版本、卡片链接、批准人"（呼应 [教程 04-企业级架构主干/05-历史记录持久化与合规 §审计]）。

### 6.1 本节测试与验证（模型卡片与发布闸门）

```sql
-- A. 卡片版本落库（对齐 §3.1 model_version.card_yaml）
INSERT INTO model_version(model_id, kind, artifact_ref, train_data, f1_on_golden, card_yaml, created_at)
VALUES ('vision-vlm@r3','VISION','ggml-model-q4.gguf','snap-2026-08',0.968,
        'model_id: vision-vlm@r3\nkind: VISION\nintended_use: ...\napprover: qc-01', now());
```

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `cardGate(model_id)` 对无卡片 / 缺字段模型 | `hasCompleteCard` 返回 false（无卡片不发布，发布闸门三） |
| 2 | `renderCard(meta)` 生成标准卡片 | YAML 含七必填字段（model_id/kind/…/approver），模板对齐 [附录 13/01] |
| 3 | 审计查询"产线 × 时间段 → 版本 + 卡片 + 批准人" | `model_line_binding` + `model_version.card_yaml` 可答四元组（验收项 5） |
| 4 | 卡片 limitations 与 v6 `ConstraintTable` 互查 | 涉及工艺敏感区的模型在发布前有工艺工程师会签（工艺合规联动） |

**失败排查**：无卡片却被发布→`cardGate` 未作为发布前置闸门被调用；卡片缺字段→`renderCard` 缺 `limitations`/`approver`；审计查不到→`model_line_binding` 未写 bound_by / 时间戳。
...
- [ ] `ModelManagementTest` 三用例（PSI 分布偏移 / 误判率漂移 / 三道闸门拦劣化）分别覆盖 §4 与 §5 的验收断言口径
- [ ] 剧本 F 验证的换线不停产 / 漂移 24h 出报告 / 闸门全拦 / 一键回切在 §2 验收表逐项对应

## 8. 全篇回归验证

**回归断言**（§7 测试与验证通过后，按 §2 验收表整体验收）：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 换产品线模型切换演练 | 0 停线、切换 < 30s（验收 1，影子期预热 + 双版本在线） |
| 2 | 注入 PSI>0.2 / 确权误判率周环比 +50% | 24h 内产出漂移报告并触发再训练评估（验收 2） |
| 3 | 再训练闸门 + 灰度回切演练 | 金标集 F1 不低于线上 -1pp 才影子、3 天不劣化才灰度、无卡片不发布（验收 3）；灰度可一键回切 < 1 分钟、识别率劣化自动回切（验收 4） |
| 4 | 模型卡片抽查 | "产线×时间→版本→训练数据→批准人"四元组 100% 可查（验收 5） |

**失败排查**：切换停线→影子预热/双版本在线未就绪；漂移不触发→PSI 计算窗口或阈值配置错；灰度回切慢→回切命令未走自动化路径；卡片缺字段→模型注册未强制校验四元组。
...
- [ ] 验收表每行的"验证方式"都能落到 §3-§7 的换线演练 / 单测 / 剧本 F
- [ ] "审计可查"由 `model_line_binding` + `model_version.card_yaml` 表结构保证，非事后补录
...
- [ ] ADR-011-28/29/30 分别与正文版本绑定换线（§3）、漂移双信号只触发评估（§4/§5）、无卡片不发布（§6）一一对应
- [ ] 三条 ADR 编号与 13-ADR架构决策记录 的 ADR-011-28~30 衔接、无重复
...
> **本节核对（痛点一句话）**：两个痛点分别指向 [13-ADR架构决策记录]（决策资产总账）与 [14-工业前沿与演进方向]（2026 前沿判定哪些是必修课）——与两篇的立项一致即 PASS。
