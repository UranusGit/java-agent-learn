...
> **定位**：把"班次良率差异"变成"工艺参数建议引擎"——LLM + RAG/约束检索历史工况，生成参数建议，经人工/DCS 确认后反馈效果，回流飞轮。**关键红线：工艺优化是建议引擎，不是闭环控制（直接写 PLC 是工业安全红线）**。教程 00-基础与核心/08-Plan-and-Execute模式 Plan-and-Execute + 教程 05-Observation可观测/04-自定义Convention与Filter：工业标签与脱敏 反思的落地。本文给出**完整可手写代码**。
>
> 「遇到阻塞？→ [教程 00-基础与核心/08-Plan-and-Execute模式]、[教程 08-架构师进阶/03-自我反思与Agent评估]、[教程 08-架构师进阶/07-数据飞轮与持续改进]」
...
/**
 * 历史工况检索。生产换向量检索（[教程 00-基础与核心/05-RAG检索增强生成]）；
...
/** 建议被确认应用后 → 测量效果 → 回流（飞轮：让"哪个参数组合有效"可被检索，[教程 08-架构师进阶/07-数据飞轮与持续改进]） */
@Component
...
curl -X POST "http://localhost:8081/api/process/suggest?product=housing-x&target=提良率"

# B. 越界建议应被拒：显式请求超 maxStep 的大步长（越界即拒，验收项 2）
curl -X POST "http://localhost:8081/api/process/suggest?product=housing-x&target=大幅改参数"

# C. 确认应用 + 效果回流（§4.8 /api/process/applied）
curl -X POST "http://localhost:8081/api/process/applied?product=housing-x&parameter=保压时间&suggestedValue=14&measuredYield=95.2&energyDelta=-0.3"
```
...
- **取舍理由**：飞轮——让"哪个参数组合有效"可被下次检索到（[教程 05-Observation可观测/08-流式响应的观测：stream模式下的span闭合 §数据飞轮]）
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 建议有效 | 采纳建议后良率提升可复现（§2-1，§6 回归） | ☐ |
| 约束强制 | 建议 100% 过确定性约束校验（§2-2，§6 回归） | ☐ |
| 非闭环 | 无直接写 PLC 路径，建议必须经确认（§2-3，§6 回归） | ☐ |
| 效果回流 | 已应用建议的效果 100% 回流 RAG（§2-4，§6 回归） | ☐ |
| 可追溯 | 每次建议含依据 + 确认人（§2-5，§6 回归） | ☐ |
