...
> **定位**：某制造业工厂的质检与设备维护 Agent 平台——产线视觉质检（多模态 LLM 精判）、设备预测性维护（时序 + LLM 解释）、维护工单自动化（HITL）、边缘部署与断网续跑。项目 11 是**完整独立**的项目，不依赖也不引用其他项目。**本项目的核心使命：把教程 39（边缘部署）、前沿 02（多模态）、教程 24（容错）、教程 35（长任务）在"制造业工业场景"落地。**
>
> 「遇到阻塞？→ [教程 39-多模型协作与供应策略 §边缘部署]、[前沿 02-多模态Agent]、[教程 24-容错与弹性设计]、[教程 35-长任务持久化与中断恢复]」
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 时序异常 | [教程 24-容错与弹性设计 §异常检测]、[附录 01-WebFlux与响应式编程] |
| 02 视觉质检 | [前沿 02-多模态Agent]、[教程 03-工具调用] |
| 03 预测性维护 | [教程 34-高级记忆架构 §时序记忆]、[教程 05-RAG] |
| 04 边缘部署 | [教程 39-多模型协作与供应策略 §边缘部署] |
| 05 工单自动化 | [教程 22-Human-in-the-Loop与审批流]、[附录 12 §ToolCallingManager] |
| 06 工艺优化 | [教程 07-Plan-and-Execute模式]、[教程 32-自我反思与Agent评估] |
| 07 多产线隔离 | [教程 20-多租户隔离与资源治理]、[教程 37 §Reactor Context] |
| 08 高可用断网 | [教程 24-容错与弹性设计]、[教程 28-部署与运维 §演练] |
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`ToolCallingManager`/多模态 `.media()`/`entity()` 结构化输出均按 [附录 12-SpringAI2-API基准] 的真实签名书写，**含全部 import、无 `...` 省略，可照抄编译**。
