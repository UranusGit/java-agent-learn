...
> **定位**：某制造业工厂的质检与设备维护 Agent 平台——产线视觉质检（多模态 LLM 精判）、设备预测性维护（时序 + LLM 解释）、维护工单自动化（HITL）、边缘部署与断网续跑。项目 11 是**完整独立**的项目，不依赖也不引用其他项目。**本项目的核心使命：把教程 05-Observation可观测/11-深度整合：SpringAI2与Observation的完整结合面（边缘部署）、前沿 02（多模态）、教程 04-企业级架构主干/01-微服务拆分与Agent部署（容错）、教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断（长任务）在"制造业工业场景"落地。**
>
> 「遇到阻塞？→ [教程 08-架构师进阶/10-多模型协作与供应策略 §边缘部署]、[前沿 02-多模态Agent]、[教程 04-企业级架构主干/10-容错与弹性设计]、[教程 08-架构师进阶/06-长任务持久化与中断恢复]」
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 时序异常 | [教程 04-企业级架构主干/10-容错与弹性设计 §异常检测]、[教程 01-WebFlux与响应式编程/00-WebFlux从零入门] |
| 02 视觉质检 | [前沿 02-多模态Agent]、[教程 00-基础与核心/03-工具调用] |
| 03 预测性维护 | [教程 08-架构师进阶/05-高级记忆架构 §时序记忆]、[教程 00-基础与核心/05-RAG检索增强生成] |
| 04 边缘部署 | [教程 08-架构师进阶/10-多模型协作与供应策略 §边缘部署] |
| 05 工单自动化 | [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]、[附录 05-SpringAI2-API基准/02 §1.3] |
| 06 工艺优化 | [教程 00-基础与核心/08-Plan-and-Execute模式]、[教程 08-架构师进阶/03-自我反思与Agent评估] |
| 07 多产线隔离 | [教程 04-企业级架构主干/06-多租户隔离与资源治理]、[教程 08-架构师进阶/08-响应式错误处理 §Context 传递] |
| 08 高可用断网 | [教程 04-企业级架构主干/10-容错与弹性设计]、[教程 04-企业级架构主干/13-部署与运维 §演练] |
| 09 边缘推理优化（v9，[10](10-边缘AI推理优化.md)） | [教程 08-架构师进阶/04-Agent性能优化 §批量请求合并]、[教程 04-企业级架构主干/12-模型路由与降级]、[教程 08-架构师进阶/06-长任务持久化与中断恢复 §幂等]、[附录 15-AIInfra与推理部署] |
| 10 多模态融合（v10，[11](11-多模态融合质检.md)） | [教程 09-前沿专题/02-多模态Agent开发 §2 消息构造、§6 成本模型]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §风险分级] |
| 11 模型管理（v11，[12](12-预测性维护模型管理.md)） | [教程 08-架构师进阶/07-数据飞轮与持续改进 §5、§7]、[教程 04-企业级架构主干/09-灰度发布与版本管理 §流量切分]、[附录 12-AI治理与合规/01-模型卡片与数据卡片] |
