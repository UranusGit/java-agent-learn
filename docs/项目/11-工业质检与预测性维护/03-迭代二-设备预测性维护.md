# 项目 11：工业质检与预测性维护 — 03-迭代二：设备预测性维护

> **定位**：把 v1 的 EWMA 检测升级为完整的 PdM——"统计预筛 + LLM 唤醒"双轨制、RUL 剩余寿命估计、LLM 生成可读的故障解释与维护建议。教程 39 §时序记忆 + 教程 05 RAG 的落地。本文给出**完整可手写代码**。
>
> 「遇到阻塞？→ [教程 39-高级记忆架构 §时序记忆]、[教程 05-RAG检索增强生成]、[附录 01-LLM基础理论/02-上下文窗口与Token]」
...
v3 无新依赖（知识库为完整可手写的关键词检索；生产换向量检索需引入 `spring-ai-starter-vector-store-pgvector`，见[教程 05]）。
...
/**
 * 设备知识库（手册/历史工单）。生产换向量检索（VectorStore + QuestionAnswerAdvisor，[教程 05]）；
 * 此处为完整可手写的关键词检索实现（概念代码标注——真实 RAG 见 [教程 05 §RAG]）。
 */
@Component
...
**材料**：一台设备的一段时间序振动/温度数据；故障标签；剩余寿命预测对照。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「预警提前」场景执行 | 可预测故障预警提前 ≥ 14 天（回测历史故障） |
| 2 | 用材料构造「RUL 准确」场景执行 | RUL 估计 MAE ≤ 20%（回测 30 起故障） |
| 3 | 用材料构造「成本」场景执行 | LLM 触发率 < 5% 流量（统计预筛生效）；单次洞察 Token ≤ 1k |
| 4 | 用材料构造「解释可读」场景执行 | 洞察含征兆/根因/建议/证据，维护人员可执行 |
| 5 | 用材料构造「误报可控」场景执行 | 正常设备误预警率 < 5%（对照历史正常期） |
...
> **定位回顾**：v3 完成"预测层"。下一站 [04-迭代三-边缘部署.md](04-迭代三-边缘部署.md)——轻量模型放边缘，云端做疑难会诊，断网边缘自治。
