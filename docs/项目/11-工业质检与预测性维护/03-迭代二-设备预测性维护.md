...
> **定位**：把 v1 的 EWMA 检测升级为完整的 PdM——"统计预筛 + LLM 唤醒"双轨制、RUL 剩余寿命估计、LLM 生成可读的故障解释与维护建议。教程 34 §时序记忆 + 教程 05 RAG 的落地。本文给出**完整可手写代码**。
>
> 「遇到阻塞？→ [教程 34-高级记忆架构 §时序记忆]、[教程 05-RAG检索增强生成]、[附录 05-LLM基础理论/02-上下文窗口与Token]」
...
public class PdMController {
    ...
    public Mono<MaintenanceInsight> explain(@PathVariable String deviceId) {
        List<SensorReading> history = sensorQuery.recent(deviceId, 30);
        return insightService.explainSignal(deviceId, history, recentSignalStore.latest())
                .doOnNext(insight -> {
