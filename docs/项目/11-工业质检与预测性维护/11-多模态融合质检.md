...
> **定位**：把质检从"只看视觉"升级为**视觉 + 声学 + 振动三模态融合**——装配缺陷有一类是相机拍不出来的（电机异响、轴承微磨损、齿轮啮合异常），必须靠声学与振动补盲。本篇回答三个工程问题：融合层级选特征级还是决策级（v10 选**决策级**）；原始时序怎么变成 LLM 可读的摘要（确定性特征化，LLM 不读原始波形）；模态结论冲突时怎么办（确定性加权仲裁 + **强制人工升级**，不仲裁到底）。教程 06-TraceId全链路追踪/02-Tracer编程模型：手动开span 多模态 Agent 开发的落地。前置阅读：[02-视觉质检多模态](02-视觉质检多模态.md)、[03-设备预测性维护](03-设备预测性维护.md)。
>
> 「遇到阻塞？→ [教程 09-前沿专题/02-多模态Agent开发 §2 消息构造、§6 成本模型]、[教程 08-架构师进阶/03-自我反思与Agent评估 §评估指标体系]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §风险分级]、[教程 08-架构师进阶/08-响应式错误处理 §Context 传递]」
...
- [ ] 第 3 行"架构演进"（三模态并行提取 → 决策级融合 → LLM 融合叙事 / 冲突人工升级）与 §4-§5 代码产物对应
- [ ] 三个痛点分别被 §4 特征化消费时序、§5 决策级融合补盲、§5.4 冲突人工升级解决
...
- [ ] 五项验收（补盲检出 ≥ 90% / 综合漏检 < 3% / Token ≤ 2k / 时延 < 500ms / 冲突 100% 升级且升级率 < 8%）都能在 §4-§5 代码（SignalDigest / WeightedFusion / FusionJudgeService / QualityEscalationQueue）中找到落地件
- [ ] 验收项 3"Token ≤ 2k"对应 §4 特征化纪律（LLM 读摘要不读原始波形）
...
### 3.3 本节核对（三模态特性与融合层级选型）

- [ ] 三模态各自"看什么 / 检测本质"能复述（视觉看外观、声学看频谱、振动看频带能量），且交叠很小（融价值在补盲）
- [ ] 特征级 vs 决策级的选型理由（零训练 / 可解释 / 单模态可审计 / 对齐样本稀缺）能不看表讲清，并与 ADR-011-25 一致
- [ ] "为什么权重 0.6 而非平均"的不对称性（外观是法定主判据）与 §5.2 代码对应

## 4. 时序信号特征化：振动频谱 → Agent 可读摘要

> **核心纪律（继承 ADR 011-04"LLM 只读结论"）**：LLM 永远不读原始波形——10kHz × 2s 的振动序列直接进上下文是 Token 灾难且模型读不懂。特征化是**确定性代码**的事：统计特征（RMS/峰值/峭度）+ 频域特征（主频带能量分布，简化 DFT），输出一段**结构化文本摘要**，LLM 读摘要写叙事（[教程 09-前沿专题/02-多模态Agent开发 §3 VLM 能力边界]）。
...
> 声学侧与振动侧同构但特征面不同——RMS/峭度换成**过零率 + 谱质心**，麦克风数据来自工位音频采集卡（经 MQTT 进，与 v1 传感器同通道）。完整类如下：
...
/**
 * 声学特征化（完整可手写）：RMS + 峰值 + 过零率（ZCR）+ 谱质心（Spectral Centroid）。
 * 与振动侧同构但特征面不同：撞击/摩擦/松动类缺陷抬高过零率，啸叫/轴承剥落抬高谱质心。
 * 全确定性——同一帧必得同一摘要（可复核、可回归）。
 */
@Component
public class AcousticFeatureExtractor {

    /** 设备阈值（概念代码：按设备型号配置，来源手册/历史基线） */
    private static final double RMS_LIMIT = 0.70;
    private static final double ZCR_SUSPECT = 0.35;
    private static final double ZCR_ALARM = 0.45;

    public SignalDigest digest(String deviceId, double[] samples, double sampleRateHz) {
        double rms = rms(samples);
        double peak = peak(samples);
        double kurtosis = kurtosis(samples, rms);        // 声学瞬态仍保留峭度（冲击性）
        double zcr = zeroCrossingRate(samples);           // 声学特征①：过零率
        double centroid = spectralCentroid(samples, sampleRateHz);   // 声学特征②：谱质心
        String freqDesc = "谱质心 " + String.format(Locale.ROOT, "%.0f", centroid) + " Hz / 过零率 "
                + String.format(Locale.ROOT, "%.2f", zcr);
        String verdict = (rms > RMS_LIMIT || zcr > ZCR_ALARM) ? "ALARM"
                : (rms > RMS_LIMIT * 0.7 || zcr > ZCR_SUSPECT) ? "SUSPECT" : "OK";
        return new SignalDigest("ACOUSTIC", deviceId, rms, peak, kurtosis,
                freqDesc, verdict, "RMS<" + RMS_LIMIT + ", 过零率<" + ZCR_ALARM + "（手册基线）");
    }
    ...
    /** 过零率：符号翻转比例——撞击/摩擦/松动类缺陷显著升高（0~1）。 */
    private double zeroCrossingRate(double[] x) {
        int crosses = 0;
        for (int i = 1; i < x.length; i++) {
            if ((x[i - 1] >= 0) != (x[i] >= 0)) crosses++;
        }
        return (double) crosses / Math.max(1, x.length - 1);
    }

    /** 谱质心：能量加权平均频率（Hz）——高频异常（啸叫/轴承剥落）会抬高。 */
    private double spectralCentroid(double[] x, double sampleRateHz) {
        int usable = Math.min(x.length, 4096);           // 控制计算量
        double num = 0, den = 0;
        for (int k = 1; k < usable / 2; k++) {
            double re = 0, im = 0;
...
**前置条件**：任意确定性振动样本可注入（正弦 / 正弦+冲击）。

**材料**：`MultimodalFusionTest.vibration_digest_is_deterministic_and_sensitive`（§6）。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 注入正弦 → `VibrationFeatureExtractor.digest` | `ruleVerdict="OK"`、峭度 ~3（正常）；同一波形两次 digest 输出完全一致（确定性可复核） |
| 2 | 注入正弦+冲击（`withImpulse`） | 峭度显著抬升（> 正常×2）、`ruleVerdict` ∈ SUSPECT/ALARM（轴承局部缺陷特征） |
| 3 | 喂原始波形 vs 只喂 `SignalDigest.toPrompt()` 给 LLM | LLM 融合叙事输入是摘要文本而非波形（原始波形零 Token，验收项 3） |
| 4 | 对照 `thresholdBasis` 字段 | 输出带阈值依据（手册条款/基线），可审计 |

**失败排查**：确定性失效→`digest` 里有非确定状态（Random/时间戳）；峭度不抬升→`withImpulse` 冲击幅值过小或 `kurtosis` 四阶矩计算错误；摘要读不懂→`toPrompt()` 字段漏 RMS/主频带。
...
### 5.5 本节测试与验证（决策级融合与冲突仲裁）

**前置条件**：`MultimodalFusionTest` 可运行（§6）。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 视觉 OK + 单振动 ALARM（`visual_ok_but_alarm_escalates`） | 强制 `ESCALATE` + `conflict=true`（不对称规则①，视觉 OK 被声振否决） |
| 2 | 视觉缺陷 + 声振全 OK（`visual_defect_stands_without_corroboration`） | 仍判 `DEFECT`（外观法定主判据，不对称规则②） |
| 3 | 融合分落入灰区 / 无模态输入 | 走 `ESCALATE` 进 `QualityEscalationQueue` 人工升级（冲突样本 100% 升级、无一票独裁） |
| 4 | `FusionJudgeService.narrate` | LLM 只复述判定 + 疑点，不重算（读摘要向量，原始波形零 Token） |

**失败排查**：冲突被平均掉→`WeightedFusion` 的不对称规则分支未命中（先查裁决 `visualOk && anyAlarm`）；应升级却放行→灰区分支阈值（`fused >= 0.45` 且 `< 0.75`）未生效；升级未入队→`QualityEscalationQueue` 未接入 `ESCALATE` 分支。
...
**端到端演练**：连续注入 200 件金标工件（150 好 / 50 坏，坏件含 12 件纯异响缺陷）——验证补盲检出 ≥ 90%、综合漏检 < 3%、冲突升级率 < 8%、LLM 单次叙事 Token ≤ 2k（`spring.ai.chat.observations` 观测口径统计，[教程 04-企业级架构主干/02-全链路可观测性]）。

### 6.1 本节核对（单元测试覆盖）

- [ ] `MultimodalFusionTest` 三用例（特征化确定性 / 视觉OK+告警升级 / 视觉缺陷独立成立）分别覆盖 §4 与 §5.2 的迁移验收项
- [ ] 端到端 200 件剧本对应的四项验收口径（补盲 ≥ 90% / 综合漏检 < 3% / 升级率 < 8% / Token ≤ 2k）在 §2 验收表都能逐项对上

## 7. 全篇回归验证

**回归断言**（§6 测试与验证通过后，按 §2 验收表整体验收）：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 声学/振动可检的装配缺陷金标样本回测 | 检出率 ≥ 90%（验收 1）；三模态融合后综合漏检率 < 3%（验收 2） |
| 2 | 融合判级 Token 计量 | 单次 ≤ 2k（验收 3，原始波形零 Token）；全链路 < 500ms（验收 4） |
| 3 | 模态冲突样本注入 | 100% 进人工升级队列，无模态一票独裁（验收 5）；升级率 < 8% |

**失败排查**：漏检率不降→多模态融合权重未收敛或某模态特征化质量差（升级率 >8% 即信号）；Token 超预算→把原始波形喂进了上下文（未走 `SignalDigest.toPrompt()`）；冲突没升级→仲裁逻辑静默采纳了单模态。
...
- [ ] 验收表每行的"验证方式"都能落到 §4-§6 的具体单测 / 演练 / 观测
- [ ] "Token ≤ 2k"由结构保证（摘要进上下文）而非事后统计——与 §4 特征化纪律一致
...
- [ ] ADR-011-25/26/27 分别与正文决策级选型（§3.2）、特征化不读波形（§4）、冲突人工升级（§5.2/§5.4）一一对应
- [ ] 三条 ADR 编号与 13-ADR架构决策记录 的 ADR-011-25~27 衔接、无重复
...
> **本节核对（痛点一句话）**：三个痛点分别指向 v11 的（1）漂移检测与再训练、（2）版本绑定换线不停产、（3）模型卡片+绑定历史审计——与 [12-预测性维护模型管理] 的三模块立项一致即 PASS。
