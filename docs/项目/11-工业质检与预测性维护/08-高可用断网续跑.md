# 项目 11：工业质检与预测性维护 — 08-迭代七：高可用与断网续跑（主体收官）
...
**材料**：断网续跑剧本（边缘侧队列）+ reconnected 对账 + 高可用切换。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「断网续跑」场景执行 | 断网 1 小时，产线检测/决策不中断（演练验证） |
| 2 | 用材料构造「确定性降级」场景执行 | 边缘 LLM 不可用无缝切规则引擎（不报错不丢数据） |
| 3 | 用材料构造「误报治理」场景执行 | 误报率从 22% 降到 ≤ 8%（回测 2 周） |
| 4 | 用材料构造「无根据结论拦截」场景执行 | 无证据的 LLM 结论 100% 转人工/拒绝（规则校验环） |
| 5 | 用材料构造「演练制度化」场景执行 | 季度演练报告归档，剧本 A-D 全过 |
