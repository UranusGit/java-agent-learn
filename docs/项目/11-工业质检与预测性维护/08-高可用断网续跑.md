...
> **定位**：收官迭代——平台自身高可用 + 边缘断网续跑演练 + 模型误报治理。把"产线停线容忍"变成可验证的工程事实。教程 04-企业级架构主干/01-微服务拆分与Agent部署 容错 + 教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台 演练的落地。本文给出**完整可手写代码**。
>
...
```java
// 完整集成演练注入真实 Bean（Spring Boot Test + @MockitoBean CloudReachability）。
// ⚠ Boot 4.1：@MockBean 已移除，用 org.springframework.test.context.bean.override.convention.MockitoBean（以引入依赖后 javap 输出为准）
@SpringBootTest
class HighAvailabilityDrillTest {

    @Autowired LocalBuffer buffer;
    @Autowired EdgeAutonomy edgeAutonomy;
    @org.springframework.test.context.bean.override.convention.MockitoBean CloudReachability reachability;
...
- **取舍理由**："产线停线容忍"必须可验证，不能靠宣称（[教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台 §演练]）
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 断网续跑 | 断网 1 小时产线检测/决策不中断（§2-1，§7 回归 1） | ☐ |
| 确定性降级 | 边缘 LLM 不可用无缝切规则引擎（§2-2，§7 回归 2） | ☐ |
| 误报治理 | 误报率从 22% 降到 ≤ 8%（§2-3，§7 回归 3） | ☐ |
| 无根据结论拦截 | 无证据的 LLM 结论 100% 转人工/拒绝（§2-4，§7 回归 4） | ☐ |
| 演练制度化 | 季度演练报告归档、剧本 A-D 全过（§2-5，§7 回归 5） | ☐ |
