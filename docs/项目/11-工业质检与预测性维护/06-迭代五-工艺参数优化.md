...
**材料**：工艺参数优化问题（目标函数+约束）；参数扫描输入；优化结果对照。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「建议有效」场景执行 | 采纳建议后良率提升可复现（A/B 对照 ≥ 2 周） |
| 2 | 用材料构造「约束强制」场景执行 | 建议 100% 过确定性约束校验（越界即拒） |
| 3 | 用材料构造「非闭环」场景执行 | 无直接写 PLC 路径（建议必须经确认） |
| 4 | 用材料构造「效果回流」场景执行 | 已应用建议的效果 100% 回流 RAG |
| 5 | 用材料构造「可追溯」场景执行 | 每次建议含依据（历史工况引用）+ 确认人 |
