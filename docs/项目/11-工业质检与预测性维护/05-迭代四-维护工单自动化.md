...
**材料**：一次故障事件 → 工单生成剧本；维修记录补全场景；HITL 审批剧本。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「工单时效」场景执行 | 预测→建议工单 < 1 分钟（原人肉协调 2 周） |
| 2 | 用材料构造「HITL 强制」场景执行 | 无审批直接落 EAM 的路径不存在（代码审查+渗透） |
| 3 | 用材料构造「证据可复核」场景执行 | 建议工单 100% 带证据链（信号/根因/RUL） |
| 4 | 用材料构造「拒绝防绕过」场景执行 | 审批拒绝后 Agent 不得重试同一工单 |
| 5 | 用材料构造「闭环回流」场景执行 | 维修结果 100% 回流喂预测（有记录） |
