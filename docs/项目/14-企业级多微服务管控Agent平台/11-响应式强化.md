...
| # | 检验点 | PASS 判据 |
|---|--------|----------|
| 1 | 无阻塞断言 | 按本节代码/命令注释中的预期逐条核对 |
| 2 | 响应式链测试（StepVerifier） | 按本节代码/命令注释中的预期逐条核对 |
| 3 | 审批响应式测试 | 触发高危工具 → 不审批 → 5 分钟自动拒绝（或手动 decide(false)） |
| 4 | 背压验证 | 流式响应限速（背压控制）：消费者慢时通过 buffer/limitRate 控制 |
### 失败排查
