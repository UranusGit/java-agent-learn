...
> **定位**：把 `admin-portal` 与用户控制台从"能看"做到"**好用**"：**React 前端架构**（不生成源码文件，只讲设计与关键 TSX 片段）、**多页面流式前端**（SSE 消费/断线重连/跨页签同步）、**工具可视化看板**（实时事件流）、**审批卡 UI**、**会话管理 + 反馈入口**（联动 13 数据飞轮）。读者画像：想让平台"有人用、用着顺"的读者。前置阅读：[18-迭代十六-安全纵深深化](18-迭代十六-安全纵深深化.md)、[教程 15-React入门与现代前端工程] 到 [教程 18-Agentic-UI设计]。
>
