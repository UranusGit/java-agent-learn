# 09-迭代八：灰度发布与成本治理——版本化灰度 + 预算控制 + 语义缓存

> **定位**：本迭代让平台**敢改、敢花、不失控**：**灰度发布**（编排定义/模型/Prompt 版本化 + 流量切分 + 秒级回滚）、**成本治理**（Token 预算实时拦截、按任务定价、成本归因、成本异常告警）、**语义缓存**（高频查询降本）。读者画像：理解 HITL 与安全，想落地"发布安全 + 成本可治理"的读者。前置阅读：[08-迭代七-HITL审批与安全合规](08-迭代七-HITL审批与安全合规.md)、[教程 29-灰度发布与版本管理]、[教程 27-成本治理与Token计量]。
>
...
//   gen_ai.client.token.usage{type=prompt/completion, tenant_id=..., business_line=...}
...
**下一篇**：10-迭代九-高可用与灾备——多实例自愈、容量规划、断点续跑。
