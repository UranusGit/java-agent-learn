...
| 层 | 策略 | 说明 |
|----|------|------|
| 前端 | `EventSource`/`fetch` 断线指数退避重连 + `Last-Event-ID` 续传 | [教程 03-React前端与AgenticUI/02-React与SSE流式UI] |
...
// （React 端见 [教程 03-React前端与AgenticUI/03-Agentic-UI设计]；后端用 WebFlux Flux + Sinks 广播）
```
