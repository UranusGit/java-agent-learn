...
> **定位**：补齐 AIGC **法定合规层**：**内容标识**（显式标识"AI 生成"+ 隐式水印/元数据——《生成式人工智能服务管理暂行办法》《互联网信息服务深度合成管理规定》与 EU AI Act 均强制）、**内容溯源**（C2PA 凭证链）、**适龄与特殊人群保护**（未成年人模式/敏感话题分级）。这是面向公众服务的**上线前置项**，不做即违规。读者画像：平台要对外提供服务（尤其 C 端）的合规/架构读者。前置阅读：[27-模型内容审核与安全护栏](27-模型内容审核与安全护栏.md)、[08-迭代七-HITL审批与安全合规](08-迭代七-HITL审批与安全合规.md)。
>
...
```bash
# 任意对话/导出内容 → 断言：含显式标注 + 零宽水印可提取 + 响应头 X-AIGC-* 存在
# 延迟压测：标识注入增量 P99 < 5ms
```
...
```java
# 取一条历史内容 → 水印提取溯源码 → C2PA 验签 → 还原（模型/版本/时间/Prompt哈希）
```

### 6.3 适龄测试

```bash
# 未成年人账号：敏感话题被分级过滤、支付工具不可见、超时长提示、夜间拒绝
```

### 6.4 抗篡改测试

```java
# 内容复制/局部删改后 → 水印仍可提取（抽样 80% 以上存活率）
```
...
| # | 检验点 | PASS 判据 |
|---|--------|----------|
| 1 | 标识完整性测试 | 任意对话/导出内容 → 断言：含显式标注 + 零宽水印可提取 + 响应头 X-AIGC-* 存在 |
| 2 | 溯源验证测试 | 按本节代码/命令注释中的预期逐条核对 |
| 3 | 适龄测试 | 未成年人账号：敏感话题被分级过滤、支付工具不可见、超时长提示、夜间拒绝 |
| 4 | 抗篡改测试 | 按本节代码/命令注释中的预期逐条核对 |
### 失败排查
