...
> **定位**：本迭代是项目**工业落地的收官**：把平台推到"故障自愈、容量可算、长任务可续、灾难可恢复"：**高可用**（数据面多实例 + HPA 自动扩缩 + 就绪探针自愈）、**容灾**（多区域多活 + 数据同步 + RPO/RTO）、**容量规划**（压测基线 → 实例数模型）、**长任务断点续跑**（检查点）、**优雅停机/滚动发布**。读者画像：想看到平台"敢上生产"的读者。前置阅读：[09-灰度发布与成本治理](09-灰度发布与成本治理.md)、[教程 33-部署与运维]、[教程 40-长任务持久化与中断恢复]。
>
...
| # | 检验点 | PASS 判据 |
|---|--------|----------|
| 1 | 故障自愈测试 | 杀掉一个 agent-executor Pod → 探针摘除 → HPA 补新 → 请求无感知（≤60s） |
| 2 | 断点续跑测试 | 按本节代码/命令注释中的预期逐条核对 |
| 3 | 压测验证 | 按本节代码/命令注释中的预期逐条核对 |
| 4 | 容灾演练 | 按本节代码/命令注释中的预期逐条核对 |
### 失败排查
