...
> **定位**：把 `agent-executor` 从"单个 Agent 直连模型"升级为**多 Agent 协作平台**：**DAG 工作流编排**（节点/边/条件分支/循环）、**任务委派**（主管 Agent 拆解委派给子 Agent）、三种协作模式（主从/对等/流水线）。读者画像：想让 Agent 从"单打独斗"到"团队分工"的读者。前置阅读：[12-评估与数据飞轮](12-评估与数据飞轮.md)、[教程 09-多Agent协作]、[教程 79-Agent工作流编排]。
>
