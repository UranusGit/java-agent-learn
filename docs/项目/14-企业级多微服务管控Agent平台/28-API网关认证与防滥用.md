...
| # | 检验点 | PASS 判据 |
|---|--------|----------|
| 1 | 认证测试 | 按本节代码/命令注释中的预期逐条核对 |
| 2 | 租户注入测试 | tenantA 的 token → 下游拿到 tenantA（不可伪造）；tenantA 不能访问 tenantB 数据 |
| 3 | 防刷测试 | 按本节代码/命令注释中的预期逐条核对 |
| 4 | 敏感接口测试 | 改配置/审批无二次验证 → 拒绝；有验证 → 放行且全量审计 |
### 失败排查
