...
> **定位**：把 `agent-executor` 从"单个 Agent 直连模型"升级为**多 Agent 协作平台**：**DAG 工作流编排**（节点/边/条件分支/循环）、**任务委派**（主管 Agent 拆解委派给子 Agent）、三种协作模式（主从/对等/流水线）。读者画像：想让 Agent 从"单打独斗"到"团队分工"的读者。前置阅读：[12-迭代十一-评估与数据飞轮](12-迭代十一-评估与数据飞轮.md)、[教程 09-多Agent协作]、[教程 36-Agent工作流编排]。
>
...
| # | 检验点 | PASS 判据 |
|---|--------|----------|
| 1 | DAG 引擎测试 | 定义一个小 DAG（A→B，B→C with 条件）→ 断言执行顺序与条件分支正确 |
| 2 | 任务委派测试 | 按本节代码/命令注释中的预期逐条核对 |
| 3 | 并行控制测试 | 断言：同时运行的子 Agent ≤ 3（flatMap 并发限制） |
| 4 | 工作流版本化测试 | 按本节代码/命令注释中的预期逐条核对 |
### 失败排查
