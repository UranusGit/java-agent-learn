...
> **定位**：把项目 14 散落的 Kafka 用法**统一成事件驱动设计**：事件分类与 Topic 划分、事件 Schema 与演进、幂等与顺序保证、死信队列与重试、消费端语义（at-least-once）。这是平台"异步事件底座"的规范。读者画像：想理解平台事件如何组织、如何不丢不重不漏的读者。前置阅读：[23-数据模型与Schema设计](23-数据模型与Schema设计.md)、[教程 67-Kafka全景与核心概念]。
>
