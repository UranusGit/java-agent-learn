...
> **定位**：对项目 12 做**工业前沿对照**——梳理 2026 年 Google（Agent Bake-Off、生产指南）、AWS、Nebius 关于研发效能 Agent / 企业 Agent 生产落地的公开经验，逐条映射到本平台 11 个迭代（v1-v11）的既有能力，找出缺口并给出**演进路线（v12+）**。读者画像：读完 00-13 想站在行业前沿审视平台设计的读者。前置阅读：[13-ADR架构决策记录](13-ADR架构决策记录.md)、[教程 08-架构师进阶/07-数据飞轮与持续改进]、[教程 08-架构师进阶/09-Agent治理与合规框架]。
>
...
**平台映射**：工具层已是 Spring AI 2.0 `@Tool` + `ToolCallbackProvider` 形态（可平滑接 MCP，[教程 02-SpringAI核心机制/01-MCP协议]）；但四个专家 Agent 目前是进程内 Bean，**未对外发布能力**——兄弟团队（如中台、安全团队）无法复用（§3 N5）。
