...
> **定位**：把"构建/测试失败靠人翻日志"变成"CI 失败诊断 Agent"——日志结构化入库、确定性信号提取、相似度聚类、LLM 每簇根因假设。**关键：诊断只读，重试/修复/开 PR 必须人工审批**。教程 02-SpringAI核心机制/04-结构化输出 §日志 + 教程 04-企业级架构主干/01-微服务拆分与Agent部署 §重试的落地。本文给出**完整可手写代码**（一行不省略，含全部 import）。
>
> **读者画像**：已完成 [03-测试生成Agent](03-测试生成Agent.md)。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/02-全链路可观测性 §日志]、[教程 04-企业级架构主干/10-容错与弹性设计 §重试]、[教程 08-架构师进阶/08-响应式错误处理]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
