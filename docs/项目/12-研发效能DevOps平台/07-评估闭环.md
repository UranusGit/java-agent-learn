...
```sh
# ① 离线 golden 每日回归
curl -s -X POST http://localhost:8081/api/v1/eval/golden
# ② 线上埋点：驳回一条评论（dismissed 必填原因）
curl -s -X POST http://localhost:8081/api/v1/eval/outcome \
  -H "Content-Type: application/json" \
  -d '{"commentId":"<v2返回的comment_id>","outcome":"DISMISSED","reason":"既有约定回调，非误用"}'
# ③ LLM-judge 盲评
curl -s -X POST http://localhost:8081/api/v1/eval/judge \
  -H "Content-Type: application/json" \
  -d '{"id":"c1","file":"OrderService.java","line":12,"severity":"critical","category":"安全","message":"未验证输入","confidence":0.9}'
# ④ 灰度对照
curl -s -X POST http://localhost:8081/api/v1/eval/gray \
  -H "Content-Type: application/json" \
