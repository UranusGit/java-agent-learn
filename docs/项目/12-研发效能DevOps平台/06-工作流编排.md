...
> **定位**：把"评审流程硬编码"升级为"DAG 工作流编排 + 状态机生命周期"——流程结构用 DAG 表达、生命周期用状态机管理、条件分支/并行/审批节点可配置。教程 05-Observation可观测/03-自定义Handler：收集Agent阶段事件流 Agent 工作流编排的落地。本文给出**完整可手写代码**（一行不省略，含全部 import）。
>
> **读者画像**：已完成 [05-多Agent评审流水线](05-多Agent评审流水线.md)。
>
> 「遇到阻塞？→ [教程 08-架构师进阶/02-Agent工作流编排 全篇]、[教程 02-SpringAI核心机制/02-Agent状态管理]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
> 生产实现把 `WorkflowEngine.WorkflowEvent` 落库做断点续跑（[教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 §长任务持久化与中断恢复]）。
...
```sh
curl -s -N -X POST http://localhost:8081/api/v1/workflow/core/102 \
...
```sh
curl -s -N -X POST http://localhost:8081/api/v1/workflow/core/103 \
  -H "Content-Type: application/json" \
  -d '{"diff":{"changedFiles":[{"path":"src/main/java/com/rd/security/AuthService.java","diffHunk":"@@ -10,3 +10,4 @@"}]}}'
curl -s -X POST http://localhost:8081/api/v1/workflow/state/transition \
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 每条验收有代码落点 | 流程可配→§3.4 `WorkflowDefinition/node(id,cfg)`；条件正确→§3.6 `evaluateRisk`；并行→§3.5 `WorkflowEngine` flatMap；状态→§3.7 状态机；可回放→§3.10 SSE 轨迹 |
| 2 | 与 §3.10 本节验证衔接 | §3.10 步骤 1-7 已逐条覆盖五条验收的 PASS 判据 |
...
**回归断言**（§3.10 本节验证通过后，按 §4 验收表整体验收）：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 改评审顺序（DAG 声明变更）后重跑 §3.10 材料 A/B | 流程按新序执行且无需改代码（验收 1）；高风险 PR 100% 走安全评审（验收 2） |
| 2 | 复跑并行三路 + 汇总 | 并行生效，耗时 ≤ 最慢单个（验收 3） |
| 3 | 状态机穷举迁移 + SSE 轨迹回放 | 无非法迁移（验收 4）；执行轨迹可回放（验收 5） |

**失败排查**：改声明不生效→`WorkflowDefinition` 未从配置/DSL 重建；并行退化→`WorkflowEngine` 用 `concatMap` 误串行；非法迁移漏拒→`ReviewStateMachine` 的 CAS 未接入。

## 8. 验收对照

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 流程可配 | 改评审顺序/加节点无需改代码（§7 回归 1） | ☐ |
| 条件正确 | 高风险 PR 强制安全评审 100% 命中（§7 回归 1） | ☐ |
| 并行生效 | 性能/架构/合规评审并行（§7 回归 2） | ☐ |
| 状态完整 | 评审生命周期状态机无非法迁移（§7 回归 3） | ☐ |
| 可回放 | 每次评审的执行轨迹可回放（§7 回归 3） | ☐ |
