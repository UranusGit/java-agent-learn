# 项目 12：研发效能 DevOps 平台 — 06-迭代五：工作流编排

> **定位**：把"评审流程硬编码"升级为"DAG 工作流编排 + 状态机生命周期"——流程结构用 DAG 表达、生命周期用状态机管理、条件分支/并行/审批节点可配置。教程 31 Agent 工作流编排的落地。本文给出**完整可手写代码**（一行不省略，含全部 import）。
>
> **读者画像**：已完成 [05-迭代四-多Agent评审流水线](05-迭代四-多Agent评审流水线.md)。
>
> 「遇到阻塞？→ [教程 36-Agent工作流编排 全篇]、[教程 12-Agent状态管理]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
> 生产实现把 `WorkflowEngine.WorkflowEvent` 落库做断点续跑（[教程 31 §长任务持久化]）。
...
工作流编排稳了，但**"评审质量到底如何"说不清**：采纳率多少？误报率多少？v7 版本是不是比 v6 好？**需要评估闭环**（离线 golden 集 + 线上埋点 + 盲评 + 灰度对照）。→ [07-迭代六-评估闭环.md](07-迭代六-评估闭环.md)
