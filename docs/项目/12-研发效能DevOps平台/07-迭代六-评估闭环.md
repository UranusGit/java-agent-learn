# 项目 12：研发效能 DevOps 平台 — 07-迭代六：评估闭环

> **定位**：让"评审质量"从感觉变成数字——离线 golden 集 + 线上埋点 + LLM-judge 盲评 + 灰度对照四件套。教程 32 §评估 + 附录 04 Eval 的落地。本文给出**完整可手写代码**（一行不省略，含全部 import）。
>
> **读者画像**：已完成 [06-迭代五-工作流编排](06-迭代五-工作流编排.md)。
>
> 「遇到阻塞？→ [教程 37-自我反思与Agent评估 全篇]、[附录 04-测试策略/02-Eval评估]、[教程 41-数据飞轮与持续改进]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
public class OnlineMetrics {
    ...
    /** 误报率 = dismissed / total（React Context 透传 comment_id，WebFlux 正确姿势见 [教程 37]）。 */
...
评估闭环证明平台有用，但**私有代码安全暴露**：代码库 100% 在评估/审查中过 LLM，但"哪些涉密、谁在访问、是否出域"没有治理。**需要私有代码安全**（不出域 + 权限前置 + 审计）。→ [08-迭代七-私有代码安全.md](08-迭代七-私有代码安全.md)
