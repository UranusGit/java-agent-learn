...
> **定位**：让"评审质量"从感觉变成数字——离线 golden 集 + 线上埋点 + LLM-judge 盲评 + 灰度对照四件套。教程 37 §评估 + 附录 04 Eval 的落地。本文给出**完整可手写代码**（一行不省略，含全部 import）。
>
...
public class OnlineMetrics {
    ...
    /** 误报率 = dismissed / total（comment_id 全链路透传，响应式上下文传递见 [教程 42-响应式错误处理]）。 */
