...
>
> 「遇到阻塞？→ [教程 09-多Agent协作 §协作模式]、[教程 80-自我反思与Agent评估 §评估指标体系]、[教程 77-上下文工程 §Token预算]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
**辩论轮上限是预算问题**（[教程 77-上下文工程 §Token 预算]）：每轮辩论 = 2 次 LLM 调用 × 专家数，无上限会指数放大成本。上限 2 轮是实测权衡（第 3 轮边际收敛 < 3%）。
