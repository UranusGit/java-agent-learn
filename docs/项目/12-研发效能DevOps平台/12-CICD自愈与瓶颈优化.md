...
>
> 「遇到阻塞？→ [教程 22-全链路可观测性 §Trace]、[教程 28-Human-in-the-Loop与审批流 §风险分级]、[教程 30-容错与弹性设计 §熔断]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
**为什么必须有熔断**：自动重试的最大风险是"自愈风暴"——一个真 bug 的测试失败被反复重试，烧光 runner 资源还掩盖问题。同签名连续 3 次自愈失败即熔断转人工，是把"自动"关在笼子里的最后一道闸（[教程 30-容错与弹性设计 §熔断]）。
