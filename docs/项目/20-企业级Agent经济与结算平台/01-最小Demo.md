...
> **定位**：用最小闭环回答"怎么敢让 Agent 花钱"：**花钱前**查预算、**花钱后**记一笔、**超预算**当场拒绝。内存实现（后续迭代落库），可测试。前置阅读：[00-需求分析与架构设计](00-需求分析与架构设计.md)、[教程 92-Agent经济与支付集成]。
>
...
| 5 | 包装位置核对 | 拦截逻辑落在 ToolCallback 包装层，未误用 Advisor（对照 [教程 63]） |
