...
> **定位**：已上线 Agent 有历史流量可回放；**新 Agent 没有**。本篇解决冷启动：用 LLM 从任务说明书**合成流量**（典型/边界/对抗三类）、人审入库成"种子回放集"、上线后真实流量滚动替换合成流量（种子集逐步退役）。读者画像：要回答"新 Agent 没流量怎么仿真"的读者。前置阅读：[07-变更影响仿真与审批联动](07-变更影响仿真与审批联动.md)、[教程 08-架构师进阶/03-自我反思与Agent评估]。
>
