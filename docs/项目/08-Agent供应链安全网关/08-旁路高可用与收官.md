...
**材料**（按验收项构造测试输入）：kill 网关/断网注入；主备切换剧本；压测脚本。

**步骤与断言（由量化验收表转断言清单）**：

| # | 操作（对照材料执行） | 预期（PASS 判据） |
|---|--------------------|------------------|
| 1 | 构造「无单点」对应场景，用材料执行 | 杀掉 1/3 网关实例，零业务影响；杀掉 2/3，P99 延迟上升但服务可用 |
| 2 | 构造「分级降级」对应场景，用材料执行 | 注入 LLM 故障：A/B 级继续（带降级标记）+ C 级拒绝；恢复后离线补检执行 |
| 3 | 构造「审计底线」对应场景，用材料执行 | 审计存储故障时全部调用被拒（fail-close 验证） |
| 4 | 构造「降级可见」对应场景，用材料执行 | 任何降级模式切换产生指标 + 告警 + 审计事件 |
| 5 | 构造「看门人安全」对应场景，用材料执行 | 渗透测试：网关凭证不可提取；策略变更全审计 |
| 6 | 构造「演练制度化」对应场景，用材料执行 | 降级矩阵剧本季度演练 |
