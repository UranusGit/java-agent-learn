...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 §熔断降级]、[教程 04-企业级架构主干/11-安全与权限控制 §可用性]、[附录 08-Agent安全深度]」
...
|---------|----------------|---------------------|
| 注入检测 LLM 不可用 | 降级仅 L1 继续（L2/L3 随熔断跳过，记录降级标记） | fail-close（C 级工具本就高风险） |
...
public class DegradeAwarePipeline {
    ...
    public Mono<InjectionResult> checkInjection(ToolCallContext ctx, Rating rating, String content) {
        ...
        if (action == DegradePolicy.Action.ALLOW_DEGRADED) {
            // 降级仅 L1（上文已过检），L2/L3 随熔断跳过——放行 + 审计降级标记
...
### 3.5 `application-secgw.yaml` 增量（部署 + 降级配置）

> 以下增量**追加写入 `application-secgw.yaml`**（两段式约定见 01-§3.0；server/spring.application 节已在基线中，勿重复声明），改完以 secgw profile 重启生效。
...
**前置条件**：v7 已验收，应用已以 secgw profile 运行（`mvn spring-boot:run -Dspring-boot.run.profiles=secgw`，见 01-§3.8；§3.5 增量并入 application-secgw.yaml 后需重启生效）；§3.1–§3.4 类已抄写编译通过（依赖 v5 注入组件与 v4 行为组件）。
...
> 对照 §5 项目收官验收逐项；落地章节即该验收项的证据所在。v1~v7 各篇的量化验收对照见各自篇末「验收对照」节。

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 无单点 | 杀 1/3 实例零业务影响；杀 2/3 服务可用 | ✅ §5.1 断言 1（材料 AB 滚动杀实例压测） |
| 分级降级 | 注入 LLM 故障：A/B 降级 + C 拒绝；恢复后离线补检 | ✅ §5.1 断言 2/3（材料 AC + HALF_OPEN 恢复） |
| 审计底线 | 审计存储故障时全部调用被拒 | ✅ §5.1 断言 4（材料 AD fail-close 验证） |
| 降级可见 | 模式切换产生指标 + 告警 + 审计事件 | ✅ §5.1 断言 5（材料 AE degrade 指标三件套） |
| 看门人安全 | 渗透测试：凭证不可提取；策略变更全审计 | ✅ §5.1 断言 6（HSM/KMS + 审计双写核对） |
| 演练制度化 | 降级矩阵剧本季度演练 | ✅ §5.1 断言 7（剧本执行留档） |
