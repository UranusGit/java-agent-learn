...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。供应链网关拦/放一个工具要可信——用户和合规要看到"为什么放行/为何拒收/签名如何"。准入流水线 emit 准入事件：`{tool, 来源, 签名校验, 评级, 动作}`。签名失败可见（用户看到"该工具签名不匹配，已拒"），评级降级可见（"C 级→转沙箱执行"），衔接 04 沙箱。
...
**材料**（按验收项构造测试输入）：12 个不同来源工具（含 3 个投毒描述变体 + 1 个静默改描述的 mock）；沙箱环境。

**步骤与断言（由量化验收表转断言清单）**：

| # | 操作（对照材料执行） | 预期（PASS 判据） |
|---|--------------------|------------------|
| 1 | 构造「来源清零」对应场景，用材料执行 | 12 个不明工具处置完成：3 补登记、2 拒绝、7 转沙箱 |
| 2 | 构造「投毒拦截」对应场景，用材料执行 | 投毒测试集（30 个恶意描述变体）自动筛查拦截率 ≥ 90%，其余进人工 |
| 3 | 构造「指纹生效」对应场景，用材料执行 | 模拟工具静默改描述，1 小时内被检测冻结 |
| 4 | 构造「沙箱隔离」对应场景，用材料执行 | C 级工具的沙箱实例出网仅白名单、文件系统隔离（渗透验证） |
| 5 | 构造「全量评级」对应场景，用材料执行 | 在线工具 100% 有 A/B/C 评级 |
