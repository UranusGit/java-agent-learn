...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/11-安全与权限控制 全篇]、[教程 02-SpringAI核心机制/07-MCP协议]、[附录 08-Agent安全深度 全部]、[前沿 04-MCP生态全景 §供应链安全]」
...
### 5.3 `application-secgw.yaml`（业务配置基线，两段式配置的 profile 侧）

配置采用**两段式**：`application.yaml` 只留骨架（profile 激活 + `.env` 导入，见 [01-最小Demo搭建 §3.0]），全部业务配置收纳进 `application-secgw.yaml`（即下面全文）。各迭代增量一律追加写入 `application-secgw.yaml`，见对应篇章（03-§3.1 MCP/OAuth、06-§2.1 策略、08-§3.5 降级矩阵）。
...
| 4 | 按 §5.3 配置以 secgw profile 启动应用（`mvn spring-boot:run -Dspring-boot.run.profiles=secgw`，依赖各迭代代码齐备后） | 8443 端口监听、`ssl.client-auth: want` 生效（无证书请求不被直接拒绝） |
