...
>
> 「遇到阻塞？→ [教程 25-安全与权限控制 全篇]、[教程 10-MCP协议]、[附录 08-Agent安全深度 全部]、[前沿 04-MCP生态全景 §供应链安全]」
...
spring:
  ...
  ai:
    ...
    mcp:
      client:
        streamable-http-connections:     # v3 起逐个登记真实工具端点（这里仅示例）
          example-tool:
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`ChatMemory`/`McpSyncClient`/`ToolCallingManager` 等均按 [附录 12-SpringAI2-API基准] 的真实签名书写，可照抄编译；标注「⚠ 修正」处为审计后对齐的基准写法。
