...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/11-安全与权限控制 §零信任]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]、[教程 04-企业级架构主干/03-工具执行可观测与审计 §审计]」
 ...
 * 与 application-secgw.yaml 的 policies 结构一一对应（用 Jackson 直接反序列化）。
 */
...
/**
 * 策略存储（v6）——从 application-secgw.yaml 的 gateway.policies 加载。
...
**约束执行的信任边界**：参数约束必须在网关（PEP）强制执行——工具端的"我保证只读"不可信（被入侵的工具可以说谎）。SQL 的语法级解析（只认 SELECT）比正则黑名单可靠（[教程 03-React前端与AgenticUI/01-React状态管理 §注入防御的层级]）。

### 3.7 本节测试与验证（PDP 评估与资源白名单）

**前置条件**：v5 已验收，应用已以 secgw profile 运行（`mvn spring-boot:run -Dspring-boot.run.profiles=secgw`，见 01-§3.8）；§3.1–§3.6 类已抄写编译；`policies.yml`（§2.1 示例）已放 classpath。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 默认拒绝 | 无策略匹配的 (agent,tool) 组合 100% 被拒 | ✅ §4.1 断言 3（200 组模糊测试） |
| 参数级粒度 | 非日志路径被拒、写禁止、SQL 写语法级拦截 | ✅ §4.1 断言 1/2（材料 U 三条 + 材料 V 审计核对） |
| 最小权限 | 工具×资源组合按声明策略生效 | ✅ §4.1 断言 4（差异矩阵逐格验证） |
| 决策延迟 | PDP 评估 P99 < 5ms（内嵌无 RPC） | ✅ §4.1 断言 5（evaluate 一万次基准） |
| HITL 升级 | 参数越界可确认调用进人工确认队列 | ✅ §4.1 断言 6（CONFIRM_REQUIRED 审计落地） |
