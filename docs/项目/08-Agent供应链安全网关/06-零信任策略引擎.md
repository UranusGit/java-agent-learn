...
**材料**（按验收项构造测试输入）：多个测试 Agent 身份（含伪造）；范围外动作请求；零信任策略变更集。

**步骤与断言（由量化验收表转断言清单）**：

| # | 操作（对照材料执行） | 预期（PASS 判据） |
|---|--------------------|------------------|
| 1 | 构造「默认拒绝」对应场景，用材料执行 | 无策略匹配的 (agent,tool) 组合 100% 被拒（模糊测试） |
| 2 | 构造「参数级粒度」对应场景，用材料执行 | v5 痛点复现：ops-agent 读非日志路径被拒；写完全禁止；SQL 写语句被语法级拦截 |
| 3 | 构造「最小权限」对应场景，用材料执行 | 每个 Agent 的工具×资源组合按声明策略生效（差异矩阵验证） |
| 4 | 构造「决策延迟」对应场景，用材料执行 | PDP 评估 P99 < 5ms（内嵌无 RPC） |
| 5 | 构造「HITL 升级」对应场景，用材料执行 | 参数越界但可确认的调用进入人工确认队列（CONFIRM_REQUIRED 审计） |
