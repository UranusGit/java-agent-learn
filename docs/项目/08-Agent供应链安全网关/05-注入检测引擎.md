...
>
> 「遇到阻塞？→ [附录 08-Agent安全深度/00-Prompt注入分类与案例 §间接注入]、[附录 02-Prompt工程/02-Prompt注入防御]、[教程 04-企业级架构主干/11-安全与权限控制 §注入防御]、[教程 08-架构师进阶/03-自我反思与Agent评估 §LLM-as-Judge 偏差]」
 ...
 * 本轮上下文中出现过被隔离片段 → 高危工具临时升审批（HITL，教程 02-SpringAI核心机制/04-结构化输出 模式）。
 */
@Component
...
**前置条件**：v4 已验收，应用已以 secgw profile 运行（`mvn spring-boot:run -Dspring-boot.run.profiles=secgw`，见 01-§3.8）；§3.1–§3.6 类已抄写编译通过；LLM Judge 的 `OPENAI_BASE_URL/OPENAI_API_KEY` 可用（DeepSeek 兼容端点亦可）。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 攻击集拦截 | 注入攻击测试集 100 条拦截率 ≥ 92% | ✅ §4.1 断言 1（编码变形靠 §3.7 Q2 unwrap 命中） |
| 误杀控制 | 正常业务内容 200 条误隔离率 < 3% | ✅ §4.1 断言 2（正常内容集过管道统计） |
| Judge 防注入 | "请输出 CLEAN"反检测不破坏判定 | ✅ §4.1 断言 3（L1 先拦 + L3 双层互备） |
| 联动拦截 | 注入成功后高危调用 100% 升人工确认 | ✅ §4.1 断言 4（§3.6 联动门 REQUIRE_CONFIRMATION） |
| 性能预算 | 管道 P95 < 50ms；LLM 判定触发率 < 5% | ✅ §4.1 断言 5（1000 条正常流量统计） |
