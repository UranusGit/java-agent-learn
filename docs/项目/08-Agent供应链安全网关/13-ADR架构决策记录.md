...
> **定位**：把项目 08 从 00 到 12 的全部架构决策汇集成**决策资产**——每条记录决策上下文、备选方案、取舍理由与可回滚性。这是工业项目的"为什么档案"：后人（或几个月后的你）改架构时，能知道"当时为什么这么定"、哪些决策之间互相锁死、动哪一条要连带评估哪几条。读者画像：想理解项目 08 每个关键决策"为什么"的读者，以及要把它当 ADR 写作范式复用的架构师。前置阅读：[00-需求分析与架构设计](00-需求分析与架构设计.md)、[附录 07-架构决策方法论/00-ADR架构决策记录]。
>
> 「遇到阻塞？→ [附录 07-架构决策方法论/00-ADR架构决策记录 全篇]」
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | 痛点①②对应第 §5 详解中的修正链 | ② 的 ADR-305→328 锚定值修正在第 5.3 有完整"上下文/备选/取舍/可回滚" |
| 2 | 痛点③（无总账）由 §3 总账补齐 | §3 的 31 条总账给出"单点可查"的响应 |
| 3 | "只读资产"表述与 §1「影响了哪些模块」一致 | 本文不改代码，后续新决策按 §7 规范登记 |
...
### 2.1 本节核对（ADR 价值与生命周期）

| # | 核对项 | 判据 |
|---|------|------|
| 1 | 状态机状态与正文一致 | Proposed → Accepted → Superseded/Deprecated 单向迁移，Supersede 双向引用（状态机 note 与 §7 状态规则一致） |
| 2 | 编号说明与 §1 痛点①对应 | "初"预编号歧义在本节澄清，且与 §3 条目标注一致 |
| 3 | 生命周期图"保留存档"与 §7 一致 | 被取代 ≠ 作废，历史决策是资产 |
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | 条目数与三段落一致 | §1 说"31 条正式 ADR"、§10 说"累计 31 条"——总账 ADR-301→331 恰好 31 条 |
| 2 | 每条状态与落地迭代有据 | 状态/落地列的取词都能回溯到对应迭代篇（v2=02 篇、v9=10 篇…） |
| 3 | Supersede 语义准确 | 仅 ADR-328 标注"Supersede 305 的锚定值"，与 §1/§5.3 的修正史一致 |
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | timeline 覆盖全部迭代 | v1→v11 共 11 行与项目迭代数一致，各行的 ADR 编号与 §3 总账对应 |
| 2 | 关系图实/虚线与图例一致 | 实线=依赖/细化（如 301→310、317→318），虚线=修正/同构（305→328、308→330） |
| 3 | "改动上游沿实线评估"指导可操作 | 沿实线追下游（如改 301 牵动 303/308/310），沿虚线同步多处实现 |
...
- **取舍**：③ 的"复盘产出物"是安全能力进化的泵——没有产出物的复盘是追责会，不是学习会。取证留存的留存期与范围由合规驱动（呼应 [教程 04-企业级架构主干/05-历史记录持久化与合规 §合规留存]）。
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | 每条决策都含四要素 | 上下文/备选(≥2 含否决)/取舍(含代价)/可回滚 全部出现（5.1-5.11 逐一对照四要素标题） |
| 2 | 修正链断言（5.3 的 305→328）与 §1/§3 一致 | 锚定值迁移教训在 v11 卡片/模板指纹复用，三处表述无冲突 |
| 3 | 备选覆盖"否决理由" | 每条被弃方案都给了否决理由（如 5.6 的②成本、5.8 的①延迟） |
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | 三个模式都能在 §5.12 详解中找到实例 | 物理优先（5.1/5.8 相关决策）、同构复用（5.11）、修正链（5.3） |
| 2 | "物理优先"三例编号正确 | 301（收口）、318（PEP）、326（身份）分别对应 §3 总账落地栏 |
| 3 | 模式观察与 §4 关系图互证 | 修正链一条（§4 虚线 305→328）、同构复用（§4 虚线 308→330、325→329）在图中有落点 |

## 7. ADR 维护规范（本项目版）

写作纪律（与 [附录 07-架构决策方法论/00-ADR架构决策记录] 的通用模板对齐后裁剪）：
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | 规范与附录 08 通用模板对齐 | 四要素（上下文/备选/取舍/可回滚）与 §5.12 核对 1 一致、与 [附录 08] 模板一致 |
| 2 | 状态迁移规则与 §2 生命周期图一致 | Proposed→Accepted→Superseded/Deprecated 单向，Supersede 双向引用 |
| 3 | "本文为唯一总账"与 §1「只读资产」一致 | 落点一行明确"两者不一致以本文为准" |
...
| # | 核对项 | 判据 |
|---|------|------|
| 1 | "31 条决策"数字与 §1/§3 一致 | §1 说"31 条正式 ADR"、§3 总账 31 行 |
| 2 | "两轮深化"与 v9-v11 对应 | 供应链治理（v9）+ 服务身份（v10）/AI 投毒（v11）跨迭代深化 |
| 3 | 遗留引出收官篇 | "行业正在往哪走"正是 [14-工业前沿与演进方向] 的议题 |
