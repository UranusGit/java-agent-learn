...
**材料**（按验收项构造测试输入）：注入一次泄漏事件 + 一次供应中断事件；取证样本；演练脚本。

**步骤与断言（由量化验收表转断言清单）**：

| # | 操作（对照材料执行） | 预期（PASS 判据） |
|---|--------------------|------------------|
| 1 | 构造「自动遏制」对应场景，用材料执行 | 高危事件（注入 + 数据外传企图）从检测到遏制 < 60 秒 |
| 2 | 构造「分流正确」对应场景，用材料执行 | 高危自动遏制、低危进人工队列（分流矩阵验证） |
| 3 | 构造「回放完整」对应场景，用材料执行 | 模拟攻击事件，回放时间线完整呈现攻击链与拦截点 |
| 4 | 构造「遏制生效」对应场景，用材料执行 | 冻结工具后全网关范围内该工具调用 100% 被拒 |
| 5 | 构造「飞轮运转」对应场景，用材料执行 | 最近一次事件的复盘产出已落为规则/策略变更（可追溯） |
