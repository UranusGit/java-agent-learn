...
> **定位**：从"检测到威胁"到"处置与学习"——安全事件的自动分诊、遏制、取证回放与复盘反哺。教程 17 §审计回放、教程 24 §熔断降级的落地。**完整可手写代码**：事件模型、分诊/遏制服务、攻击回放时间线重建、审计存储查询。
>
...
public class IncidentTimeline {
    ...
    public static IncidentTimeline reconstruct(AuditStore auditStore, String traceId) {
        List<AuditEvent> events = auditStore.eventsOfTrace(traceId);
