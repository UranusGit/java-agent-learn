...
>
> 「遇到阻塞？→ [附录 08-Agent安全深度/00-Prompt注入分类与案例 §间接注入]、[附录 02-Prompt工程/02-Prompt注入防御]、[教程 25-安全与权限控制 §注入防御]、[教程 32 §LLM-as-Judge 偏差]」
...
public class InjectionLlmJudge {
    ...
    public Mono<InjectionVerdict> judge(String content) {
        ...
        return Mono.fromCallable(() -> judgeClient.prompt()
                ...
                .call()
                .entity(InjectionVerdict.class))          // ⚠ 真实形态：entity(Class)（附录 12-02 §2）
...
→ [06-迭代五-零信任策略引擎.md](06-迭代五-零信任策略引擎.md)
