...
>
> 「遇到阻塞？→ [附录 09-Agent安全深度/01-ToolPoisoning攻击]、[附录 13-AI治理与合规/01-模型卡片与数据卡片]、[教程 31-安全与权限控制 §注入防御]、[前沿 04-MCP生态全景 §供应链安全]」
>
...
模型卡片/数据卡片是 AI 透明度的"两份身份证"——概念、字段与编写规范见 [附录 13-AI治理与合规/01-模型卡片与数据卡片]。本节的增量是**网关侧的核验机制**：卡片不能只是"贴在墙上的一份文档"，必须是入场流水线里的一道闸。

| 核验项 | 卡片声明 | 核验手段 | 不一致处置 |
|--------|---------|---------|-----------|
| 训练数据来源 | "仅公开语料" | 抽样探测：构造已知私有数据探针，输出过强记忆 → 卡片存疑 | 冻结 + 人工评审（ADR-331：软闸门） |
| 评估指标 | "MMLU 78.2" | 网关侧复测（评估集来自 [教程 37-自我反思与Agent评估] 的金标思路） | 偏差 > 阈值降级使用范围 |
...
| # | 测试 | 方法 | 预期 |
|---|------|------|------|
| 1 | 模型哈希拦截 | 篡改一个已登记模型的权重文件字节，跑 `verify` | 返回 `Frozen`（漂移冻结，ADR-307 语义） |
| 2 | 恶意哈希拦截 | 把测试权重哈希加入 `knownMaliciousHashes` | 返回 `Rejected` |
| 3 | 出处校验 | 提交 `revision=null`（主分支漂移形态）与白名单外仓库 | 均拒绝入场 |
| 4 | pickle 降级 | 提交 `PICKLE_PYTORCH` 格式已通过哈希校验的模型 | 返回 `SandboxOnly`，仅沙箱加载 |
| 5 | 模板漂移 | 修改登记模板的一个字符，捕获新指纹做 diff | `contentHash` 变化，触发冻结与回滚 |
| 6 | 卡片证伪 | 卡片声明"Apache-2.0"但仓库 LICENSE 为 GPL | 不准入 |
| 7 | 批次追溯 | 语料中某文档命中注入模式 | 同批次入库文档全部标记重检 |
| 8 | 大文件流式哈希 | 对 2GB 测试文件计算 SHA-256 | 内存占用 < 100MB（流式读取，无 readAllBytes） |
