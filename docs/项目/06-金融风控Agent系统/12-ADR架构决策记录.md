...
> **定位**：全项目 36 条架构决策（ADR-101 ~ ADR-136）的总账——每条含上下文、备选方案、取舍理由、可回滚方式；十条最有复用价值的决策给出详解。监管问询"你们为什么这么设计"、架构师复盘、新人接手，都从这一篇开始。方法论文档见 [附录 07-架构决策方法论/00-ADR架构决策记录]。
>
...
- [ ] 三个理由（监管问询/决策半衰期/防重复决策）能不看正文复述
- [ ] 最小完备结构四要素（上下文/备选/取舍/可回滚）与 §4 十条详解的实际结构一致
...
- [ ] 六个域的编号集合拼起来恰为 101-136 共 36 条，无重号无缺号（对照 §3 总账逐一勾）
- [ ] 域间关系（AUDIT 红线约束 SAFE，其余链式服务）能对着图说出来
...
| ADR-105 | 校准数据（分桶/因子/模型版本）必须入审计 | 02 迭代二 | 生效 |
| ADR-106 | 决策矩阵硬编码在代码里，阈值外置配置 | 02 迭代二 | 生效 |
| ADR-107 | self-check 失败只重试一次 | 02 迭代二 | 生效 |
| ADR-108 | 挂起用受控异常中断生成流 | 03 迭代三 | 生效 |
| ADR-109 | 审批拒绝后禁止 Agent 自动重试 | 03 迭代三 | 生效 |
| ADR-110 | 挂起默认失败安全（EXPIRED 退回重发起） | 03 迭代三 | 生效 |
| ADR-111 | Checkpoint 用 JDBC 文件库持久化 | 03 迭代三 | 生产换 PG（已演进） |
| ADR-112 | 哈希链+每日外部锚定，不上区块链 | 04 迭代四 | 生效 |
| ADR-113 | 审计查询本身被审计 | 04 迭代四 | 生效 |
| ADR-114 | canonical JSON（键排序）后再哈希 | 04 迭代四 | 生效 |
| ADR-115 | 比对用字段级结构化对照 | 05 迭代五 | 生效（细化 104） |
| ADR-116 | 分歧一律人工，不设权重自动仲裁 | 05 迭代五 | 生效 |
| ADR-117 | 复核模型选不同供应商 | 05 迭代五 | 生效 |
| ADR-118 | 并行用 Mono.zip + boundedElastic | 05 迭代五 | 生效 |
| ADR-119 | 双通道脱敏（Prompt 假名化/审计加密原文） | 06 迭代六 | 生效 |
| ADR-120 | 删除权=销毁映射而非删审计记录 | 06 迭代六 | 生效 |
| ADR-121 | 敏感检测 recall 优先（宁可误杀） | 06 迭代六 | 生效 |
| ADR-122 | 超预算收敛触发面+排队，不降级模型质量 | 07 迭代七 | 生效 |
| ADR-123 | 报表从审计链重算，不另存数据源 | 07 迭代七 | 生效（v9 扩展为 130） |
| ADR-124 | 探针检查业务依赖而非只 ping 端口 | 07 迭代七 | 生效 |
| ADR-125 | 确定性规则前置，LLM 只做异步旁路且无放行权 | 09 迭代八 | 生效 |
| ADR-126 | 窗口特征用 Redis ZSET 内嵌，不引 Flink | 09 迭代八 | 生效 |
| ADR-127 | 规则热更新用双缓冲原子切换+影子模式 | 09 迭代八 | 生效 |
| ADR-128 | 旁路过载降级方向为保守侧（DEFERRED） | 09 迭代八 | 生效 |
| ADR-129 | 卡片代码化（record+注册表+入链），不做 wiki | 10 迭代九 | 生效 |
| ADR-130 | 报送从审计链重算并附链证明 | 10 迭代九 | 生效（承 123） |
| ADR-131 | 解释=事件链组装，禁止 LLM 现场再生成 | 10 迭代九 | 生效 |
| ADR-132 | 合规检查点是策略激活的硬门禁 | 10 迭代九 | 生效 |
| ADR-133 | 图存储用邻接表+递归 CTE 起步，Neo4j 为演进 | 11 迭代十 | 生效 |
| ADR-134 | 团伙定性必须三 Agent 会签+人工，禁止自动定罪 | 11 迭代十 | 生效 |
| ADR-135 | 调查类工具最小权限（全部只读） | 11 迭代十 | 生效 |
| ADR-136 | 图数据全假名+边来源入链 | 11 迭代十 | 生效 |
...
- [ ] 36 行编号从 101 连续到 136，无跳号
- [ ] 每行的"出处"列均能对应到 00-11 篇中真实存在的"本迭代的 ADR"小节（抽 5 条核对）
- [ ] 111/123 两条的演进注记与正文状态说明一致（supersede 显式标注、不删旧条目）
...
- [ ] 抽任意一条详解，四要素（上下文/备选/取舍/可回滚）齐全——缺"放弃"的是宣传稿（§1 的判据）
- [ ] 每条备选方案 ≥2 个且非稻草人（如 ADR-102 的三个备选各有真实缺陷论述）
- [ ] 与迭代篇口径一致：ADR-102 对照 03 §2、ADR-116 对照 05 §6.1、ADR-131 对照 10 §3——发现分歧以迭代篇为准修正本篇
...
- [ ] 状态机五态与状态说明的注记一致（全部 Effective，仅 111/123 有演进）
- [ ] "只取代不静默违反"的纪律能举出 CI 清单引用例（TCM 签名变更核对 ADR-102）
...
- [ ] timeline 十段的编号分组与 §2 六域的编号集合一致（仅分组方式不同，总数 36 不变）
- [ ] "决策是被痛点逼出来的"能在各迭代篇"四问"表验证（抽 v3/v5 两篇）
...
- [ ] 对照抽验：任取 3 条总账 ADR 回到出处篇（如 105→02 篇 §5、119→06 篇 §6、125→09 篇 §9）核对"本迭代的 ADR"表内文案一致——发现分歧以迭代篇为准修正本篇

## 8. 总结

> 本节即 §7 自检清单的执行落点：六条复选框全部可勾即 PASS（其中"关键 ADR 在代码评审清单有引用"需对照 CI 配置核对）。
