...
>
> 「遇到阻塞？→ [教程 19-SSE流式通信 §流式链路]、[教程 12-背压与流量控制]、[教程 85-响应式错误处理]、[教程 60-成本治理与Token计量]」
...
> **API 真实性标注**：`ReactiveRedisTemplate` / `ReactiveStringRedisTemplate` 为 Spring Data Redis 真实 API（[教程 57-多页面流式响应与会话管理] 与 [教程 30-微服务拆分与Agent部署 §5.2] 同款用法，全体系已实证）；该依赖不在 pom 基线中，按「需在 pom.xml 中添加依赖」处理。
...
Reactor 自身的窗口原语也真实可用（[教程 11-Reactor核心] 已覆盖）：`Flux.window(Duration)` / `Flux.bufferTimeout(int, Duration)`——单进程内的微批聚合用它们足够，跨进程才需要 Flink。
...
新规则不是直接上线——先在**影子模式**跑：只记录不拦截，与线上规则集双跑比对，达标才切换（复用 v5 灰度思想，[教程 62-灰度发布与版本管理 §流量切分]）：
...
> **API 真实性标注**：`Sinks.many().multicast().onBackpressureBuffer()` 与 `tryEmitNext()` 返回 `Sinks.EmitResult`（`isSuccess()` 判定）均为 Reactor 真实 API（[教程 28-流式工具调用与事件协议] 已实证 Sinks.Many 用法）；`flatMap(fn, 256)` 第二参为并发预取上限（Reactor 真实重载）；`Flux.subscribe(onNext, onError)` 为真实重载。
