...
>
> 「遇到阻塞？→ [教程 10-SSE流式通信 §流式链路]、[附录 06-WebFlux与响应式编程/02-背压与流量控制]、[教程 42-响应式错误处理]、[教程 27-成本治理与Token计量]」
...
| 4 | 编译核对 | `ReactiveRedisTemplate` 为 spring-boot-starter-data-redis-reactive 真实 API，`opsForZSet().add/removeRangeByScore/zCard` 编译通过 |
...
Reactor 自身的窗口原语也真实可用（[附录 06-WebFlux与响应式编程/01-Reactor核心] 已覆盖）：`Flux.window(Duration)` / `Flux.bufferTimeout(int, Duration)`——单进程内的微批聚合用它们足够，跨进程才需要 Flink。
...
| 2 | 审计完整性（从原 §7.5 上移收束）：任取一个已裁决事件回放 | 链上可见"事件 → 窗口特征（规则版本内）→ 裁决 →（旁路意见）"完整链路；`GET /api/audit/verify` intact=true |
