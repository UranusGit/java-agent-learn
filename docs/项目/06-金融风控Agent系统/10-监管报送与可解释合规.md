# 项目 06：金融风控 Agent 系统 — 10-迭代八：监管报送与可解释合规

> **定位**：把"单笔可回放"升级为"体系可解释"——模型卡片/数据卡片文档化并与版本同链、监管报送从审计链自动生成、每次拒贷/放款的完整依据链、新策略上线前的合规评审门。监管验收从"回答问题"变成"出示证据"。**本文给出完整可手写代码（一行不省略）。**
>
> 「遇到阻塞？→ [附录 13-AI治理与合规/01-模型卡片与数据卡片]、[附录 13-AI治理与合规/00-NIST-AI-RMF框架]、[教程 43-Agent治理与合规框架]、[教程 41-数据飞轮与持续改进 §金标集]」
...
卡片内容对齐 [附录 13-AI治理与合规/01-模型卡片与数据卡片] 的字段框架：用途与范围、版本、评估指标与限制、人工监督点、数据来源与分级。金融场景必须多两样：**监管红线声明**（终审人工）与**已知不适用场景**（如跨境材料、非小微客群）。
...
> `monthlyReportService.monthlyReport(month)` 返回 `Mono`，组装器是离线批任务：用 `block()` 前必须确认运行在 `boundedElastic`（EventLoop 上禁 block 的铁律），生产建议改为链式 `flatMap` 组装（见 [教程 42-响应式错误处理 §阻塞边界]）。
...
五道检查：**C1 卡片齐全**（模型/Prompt 版本有已发布卡片）、**C2 金标回归**（历史终审案例重跑，一致率不低于现网基线，[教程 41-数据飞轮与持续改进 §金标集]）、**C3 影子结果**（v8 影子期报告达标）、**C4 合规清单**（监管红线逐项勾选：无机器终审/PII 处理/留存策略）、**C5 签核**（合规员 + 业务负责人双人）。
...
以 [附录 13-AI治理与合规/00-NIST-AI-RMF框架] 的四函数对照本项目机制，回答监管"治理框架落在哪"：
...
1. **卡片门禁联动测试**：发布一个 promptVersion 无卡片的新版本，`ComplianceGate.check` 必须驳回且驳回原因含 C1；补发卡片后重跑通过。
2. **依据链口径一致性测试**：随机抽 50 单，把依据链文档与 `replayFor` 原始事件逐条比对——步骤数量、顺序、seq/hash 引用 100% 一致；文档中不存在链上没有的信息。
3. **报送零差异测试**：同月两次生成报送包，汇总数字与逐笔明细完全一致（链只增不改，新增事件不属于该月则产物不变）；报送包内链尾哈希与当次 `verify()` 结果一致。
4. **门禁驳回路径测试**：分别构造 C2-C5 各一项不满足的变更请求，断言 5 种驳回各 100% 触发且入链。
