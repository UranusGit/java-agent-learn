...
>
> 「遇到阻塞？→ [教程 37-自我反思与Agent评估 §3.3 准确率的评估方法]、[教程 13-结构化输出]」
...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。风控 Agent 拦下/放行一笔要可信——用户和合规要看到"模型给出多高置信、因为哪个信号被判高风险"。风险判定 emit 判定事件：`{reqId, 置信度, 信号[分项], 分级, 动作}`。
