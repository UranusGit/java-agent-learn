...
**失败排查**：③假名替换错位→applyHits 未按 start 倒序；④假名不一致→查询漏了 applicationId 条件；⑥解密失败→Base64 key 含换行或 IV 前置字节序错；⑧destroy 后仍能查到→DELETE 的子查询写错。
...
> 与章节验证的映射：验收 1=§3.10 材料③ 的 L4 段核对、验收 2=材料③⑤（假名化+跨申请隔离）、验收 3=材料②+⑦、验收 4=材料⑦ 的 REVEAL 留痕、验收 5=材料⑧+全篇回归断言 3、验收 6=季度业务报告（规模化执行项）。本表不重复材料。
