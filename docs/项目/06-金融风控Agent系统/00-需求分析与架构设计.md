...
| 02 | 02-迭代一-置信度与风险分级 | 基础篇（v2） | 置信度校准 + 分级路由 |
| 03 | 03-迭代二-HITL审批流 | 基础篇（v3） | ToolCallingManager 装饰器人工闸门 |
| 04 | 04-迭代三-全量审计与决策回放 | 基础篇（v4） | 哈希链审计 + 决策回放 |
| 05 | 05-迭代四-多模型交叉验证 | 基础篇（v5） | 双模型字段级比对 |
| 06 | 06-迭代五-数据脱敏与合规 | 基础篇（v6） | 双通道脱敏 + 合规落地 |
| 07 | 07-迭代六-成本治理与生产部署 | 基础篇（v7） | 预算联动 + 等保部署 + DR |
