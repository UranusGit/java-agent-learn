...
>
> 「遇到阻塞？→ [教程 22-Human-in-the-Loop与审批流]、[教程 19-历史记录持久化与合规]、[教程 38-Agent治理与合规框架]、[附录 09-AI治理与合规]」
...
| ADR-102 | HITL 拦截点在 ToolCallingManager 装饰器，不在 Advisor | Advisor 环绕整个 ChatClient 调用，拦不到"工具意图返回后、执行前"时点（[教程 22 §正确落点]） |
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`ToolCallingManager` 等均按 [附录 12-SpringAI2-API基准] 的真实签名书写，可照抄编译；API 真实性以 `spring-ai-*-2.0.0-sources.jar` 反编译核对为准。
