...
### 4.2 `application.yaml` + `application-bankrisk.yaml`（两段式配置基线）

> 配置采用**两段式**：主配置 `application.yaml` 只做环境装载与 profile 激活；全部业务配置写在 `application-bankrisk.yaml`（`bankrisk` profile 专属）。后续各迭代篇的 yaml 增量（数据源/密钥/预算/流式参数等）一律追加到 `application-bankrisk.yaml`，不再动主配置。启动命令统一为：`mvn spring-boot:run -Dspring-boot.run.profiles=bankrisk`。

`application.yaml`（主配置，只有环境装载与 profile 激活）：
...
`application-bankrisk.yaml`（业务配置，bankrisk profile 专属）：
...
| 3 | 核对 yml | 主配置 `application.yaml` 仅含 `spring.config.import: optional:file:.env[.properties]` 与 `profiles.active: bankrisk`；业务配置在 `application-bankrisk.yaml`：`spring.ai.openai.chat.model` 直接挂模型名（2.0.0 无 options 中缀）、端口 8081、api-key 用 `${DEEPSEEK_API_KEY}` 占位未硬编码 |
...
> 地基篇只交付工程骨架（pom / 两段式配置 / 启动类），无业务代码——验收对照即骨架在规模化意义上的核对（§4.4 + §9 回归的表述落地），业务验收从 v1（01 篇）起逐篇建立。

| # | 验收项 | 标准 |
|---|--------|------|
| 1 | 骨架可编译 | 按 §4.1–4.3 手写后 `mvn clean compile` 100% `BUILD SUCCESS`（§4.4 断言 1 在任意干净环境复现） |
| 2 | 两段式配置合规 | `application.yaml` 无业务键；业务配置全部在 `application-bankrisk.yaml`；端口恰为 8081；密钥 `${DEEPSEEK_API_KEY}` 占位（§4.4 断言 3） |
| 3 | 演进路线完整 | §2 timeline 的 v1-v7 与 01-07 篇标题逐一对应、无缺漏错位（§9 回归断言 3；地基是后续每篇的准入条件） |

> 本节即 §4.4 验证与 §9 回归的规模化表述：地基篇无业务逻辑，三条验收项均已有可执行断言（编译命令 / 配置核对 / 篇目对照），本表不新增操作步骤。
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 重新执行 §4.4 材料（`mvn clean compile`） | `BUILD SUCCESS`——本篇只含骨架，无业务类，编译通过即回归通过 |
| 2 | 两段式配置核对 | `application.yaml` 无业务键（业务配置全部在 `application-bankrisk.yaml`）；`server.port` 恰为 8081；全文无硬编码密钥（`${DEEPSEEK_API_KEY}` 占位） |
| 3 | 演进路线核对 | §2 timeline 的 v1-v7 与 01-07 篇标题逐一对应，无缺漏错位（本篇是全项目的路线图，后续篇目编号变动时回查本表） |

**失败排查**：①编译失败→pom 基线被后续篇增量污染（增量片段应叠加在 §4.1 基线上，不覆盖）；②端口不对→`application-bankrisk.yaml` 未生效（profiles.active 未设或文件名拼写错误）。
