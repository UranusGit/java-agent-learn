# 项目 06：金融风控 Agent 系统 — 04-迭代三：全量审计与决策回放

> **定位**：构建"事后不可抵赖、记录不可篡改、历史可完整回放"的审计体系——哈希链、WORM 留存、决策回放。监管检查的技术底气。**本文给出完整可手写代码（一行不省略）。**
>
> 「遇到阻塞？→ [教程 17-工具执行可观测与审计 §审计数据模型]、[教程 19-历史记录持久化与合规 §WORM 与留存]、[教程 38-Agent治理与合规框架 §行为审计]」
...
**注意与 [教程 19 §留存矩阵] 的对齐**：金融交易相关记录是"不可删除"（WORM），不是"到期物理删除"——两类留存义务并存时取**更严格者**（审计篇当年在这里有矛盾表述，本篇按合规口径为准）。
...
审计立住了，业务侧的新焦虑：**高额度申请（>500 万）的错误代价太大**——单模型预审在该额度段的对照一致率只有 81%。→ [05-迭代四-多模型交叉验证.md](05-迭代四-多模型交叉验证.md)
