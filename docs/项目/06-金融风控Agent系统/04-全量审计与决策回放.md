...
>
> 「遇到阻塞？→ [教程 32-工具执行可观测与审计 §审计数据模型]、[教程 58-历史记录持久化与合规 §WORM 与留存]、[教程 86-Agent治理与合规框架 §行为审计]」
...
| 4 | 权限核对（目标库为 PG 时） | 执行 §3.9 注释的 REVOKE/GRANT 后，business_dba 执行 UPDATE 报权限拒绝 |
...
# ② 回放某申请
curl "http://localhost:8080/api/audit/replay/SO-0001"
```
...
- [ ] 金融审计记录为"不可删除（WORM）"而非"到期物理删除"，与教程 25 §6.2 的更严口径一致
...
> 与章节验证的映射：验收 1=§3.10 材料②、验收 2=每日锚定任务（本篇未展开代码，属规模化执行项）、验收 3=§4.1 核对+§3.11 断言 2、验收 4=§3.10 断言 4、验收 5=对象存储保留模式配置项。本表不重复材料。
