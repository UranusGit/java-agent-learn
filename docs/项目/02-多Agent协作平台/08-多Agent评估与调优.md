...
> **关联锚点**：[教程 08-架构师进阶/03-自我反思与Agent评估](../../教程/08-架构师进阶/03-自我反思与Agent评估.md)、[教程 08-架构师进阶/07-数据飞轮与持续改进](../../教程/08-架构师进阶/07-数据飞轮与持续改进.md)、[教程 04-企业级架构主干/02-全链路可观测性](../../教程/04-企业级架构主干/02-全链路可观测性.md)、[附录 04-测试策略](../../附录/04-测试策略/)、[附录 11-评估与可观测生态](../../附录/11-评估与可观测生态/)。
...
| **架构如何演进** | 执行面（编排引擎）之上增加**评估面**：事件旁路采集（不改执行链路）→ 指标 → 群体反思 → 拓扑实验 → 金标回归闸门（CI 阶段阻断劣化）。评估面与执行面分离，是 [教程 04-企业级架构主干/00-管控分离架构] 思想在质量维度的再次应用 |
...
> 「想深入指标体系设计？→ [教程 08-架构师进阶/03-自我反思与Agent评估 §3]」——单 Agent 与多 Agent 评估指标的完整推导。
...
注意 `EXP --> ENGINE` 这条回边：拓扑实验不是纯观测——它会把实验结论（最优并行度、角色增减）**回写执行面配置**。这正是评估面的最终目的：度量 → 实验 → 回写，形成 [教程 08-架构师进阶/07-数据飞轮与持续改进 §2] 的闭环。
...
单 Agent 的 Reflection（[教程 08-架构师进阶/03-自我反思与Agent评估 §2]）是"自己检查自己的作业"。多 Agent 场景下这有两 个盲区：① 执行 Agent 的系统性偏差自己看不见（翻译 Agent 总觉得自己译得准）；② **跨节点矛盾**只有第三方能发现（node-2 说"用户集中在华东"，node-3 说"华南占比最高"——两个都"高质量"，合在一起是事故）。
...
冲突处置的关键设计：**冲突不自动重跑**。两个矛盾结论里可能有隐含的正确一方（一个节点引用了旧数据），自动重跑可能把对的改错——裁决必须留给人，裁决结果沉淀进金标集，让下一次回归替人记住（[教程 08-架构师进阶/07-数据飞轮与持续改进 §3] 的采集环节）。
...
> 「想深入？→ [教程 08-架构师进阶/03-自我反思与Agent评估 §4 Spring AI Evaluator API]」
...
flowchart TB
    ...
    BLOCK --> TRIAGE["归因：Prompt? 拆解? 路由?<br/>（教程 05-Observation可观测/08-流式响应的观测：stream模式下的span闭合 §6 决策树）"]
    TRIAGE -->|修复后| CI
...
> 「遇到阻塞？→ [教程 08-架构师进阶/03-自我反思与Agent评估 §7 评估的反模式]」
...
至此，平台具备"度量 → 反思 → 实验 → 回写"的自我改进循环（[教程 08-架构师进阶/07-数据飞轮与持续改进] 的编排平台实例）。而 00-08 九篇文档散落在各处的架构决策——DAG 还是黑板、Pub/Sub 还是直接调用、Saga 还是重试——也应该沉淀成可检索的资产了。
