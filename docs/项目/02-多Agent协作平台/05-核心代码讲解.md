# 05-核心代码讲解

> **定位**：对多 Agent 编排平台全项目核心代码做完整梳理和深度解析——Agent 抽象层、工具注册机制、DAG 编排引擎、智能路由策略、审批网关，逐模块拆解关键代码的设计决策和实现细节。读完这篇，你对整个平台的每一行核心代码都有清晰认知。

> **读者画像**：已完成前三篇迭代文档，希望从"能用"升级到"精通"。

> **前置阅读**：[00-需求分析与架构设计](00-需求分析与架构设计.md) ~ [04-迭代三-任务委派与路由](04-迭代三-任务委派与路由.md)。

> **关联教程**：[教程 03-工具调用](../../教程/03-工具调用.md)、[教程 08-多Agent协作](../../教程/08-多Agent协作.md)、[教程 11-Agent状态管理](../../教程/11-Agent状态管理.md)、[教程 14-管控分离架构](../../教程/14-管控分离架构.md)、[教程 22-Human-in-the-Loop与审批流](../../教程/22-Human-in-the-Loop与审批流.md)、[教程 31-Agent工作流编排](../../教程/31-Agent工作流编排.md)。

---

## 1. 全项目架构总览

```mermaid
graph TB
    subgraph 接入层["接入层"]
        AC["AgentController<br/>Agent 对话"]
        OC["OrchestrateController<br/>编排任务"]
        APC["ApprovalController<br/>人工审批"]
    end

    subgraph 编排层["编排层"]
        TP["TaskParser<br/>LLM 任务拆解"]
        DE["DagEngine<br/>DAG 执行"]
        RS["RoutingStrategy<br/>智能路由"]
        AG["ApprovalGateway<br/>审批网关"]
    end

    subgraph Agent层["Agent 执行层"]
        AR["AgentRegistry<br/>注册中心"]
        AE["AgentExecutor<br/>执行引擎"]
        TR["ToolRegistry<br/>工具注册"]
        MB["MessageBus<br/>消息总线"]
    end

    subgraph 支撑层["支撑层"]
        SS["SessionStateStore<br/>Redis 会话"]
        TS["TaskStateStore<br/>DAG 状态"]
        LM["LoadMonitor<br/>负载监控"]
        PT["PerformanceTracker<br/>性能追踪"]
    end

    AC --> AE
    OC --> TP
    OC --> DE
    TP --> DE
    DE --> RS
    DE --> AG
    RS --> AR
    RS --> LM
    RS --> PT
    DE --> AE
    AE --> TR
    AE --> SS
    DE --> TS
    APC --> AG

    style 接入层 fill:#e3f2fd
    style 编排层 fill:#e8f5e9
    style Agent层 fill:#fff3e0
    style 支撑层 fill:#fce4ec
```

---

## 2. AgentDefinition：不可变契约

### 2.1 代码全貌

```java
public record AgentDefinition(
    String agentId,
    String name,
    String description,
    String systemPrompt,
    Set<String> capabilities,
    List<String> toolBeanNames,
    ModelConfig modelConfig
) {}
```

### 2.2 设计决策解析

| 决策 | 选择 | 理由 |
|------|------|------|
| 用 record 而非 class | record | Agent 定义不可变，修改 = 注销再注册 |
| capabilities 用 Set | Set | 能力不重复，查找 O(1) |
| toolBeanNames 用 List | List | 工具有优先级顺序 |
| systemPrompt 是纯字符串 | String | 不用复杂模板对象，保持简单 |

### 2.3 为什么不把工具直接定义在 Agent 内

工具是独立的 Spring Bean，通过 `toolBeanNames` 引用——而不是直接在 `AgentDefinition` 里嵌入工具实现。原因有三：

```mermaid
graph TB
    subgraph 解耦设计["工具与 Agent 解耦"]
        T1["GeneralAgentTools Bean"]
        T2["ResearchAgentTools Bean"]

        A1["general-assistant<br/>引用: generalAgentTools"]
        A2["research-agent<br/>引用: researchAgentTools + generalAgentTools"]
        A3["translator-agent<br/>引用: 无工具"]

        T1 -.->|被引用| A1
        T1 -.->|被引用| A2
        T2 -.->|被引用| A2
    end

    style T1 fill:#e8f5e9
    style T2 fill:#e8f5e9
    style A1 fill:#e3f2fd
    style A2 fill:#e3f2fd
    style A3 fill:#e3f2fd
```

1. **工具复用**——多个 Agent 可以共享同一组工具
2. **配置驱动**——Agent 的工具集通过 YAML 配置，不改代码
3. **测试友好**——工具 Bean 可以独立单元测试

---

## 3. AgentExecutor：执行引擎核心

### 3.1 完整代码解析

```java
@Service
public class AgentExecutor {

    private final ChatClient.Builder chatClientBuilder;
    private final ToolRegistry toolRegistry;
    private final SessionStateStore sessionStore;

    // 核心执行方法
    public Flux<String> execute(AgentDefinition agent,
                                 String userMessage,
                                 AgentSession session) {
        // 1. 构建独立 ChatClient
        ChatClient client = buildClient(agent);

        // 2. 构建 Prompt
        var promptSpec = client.prompt()
            .system(agent.systemPrompt())
            .user(userMessage);

        // 3. 注入历史消息
        if (session != null && !session.messages().isEmpty()) {
            List<Message> history = session.messages().stream()
                .map(this::toSpringMessage)
                .toList();
            promptSpec = promptSpec.messages(history);
        }

        // 4. 注入工具
        List<Object> tools = toolRegistry.resolveTools(agent.toolBeanNames());
        if (!tools.isEmpty()) {
            promptSpec = promptSpec.tools(tools.toArray());
        }

        // 5. 流式执行
        return promptSpec.stream().content();
    }

    private ChatClient buildClient(AgentDefinition agent) {
        var builder = chatClientBuilder
            .defaultSystem(agent.systemPrompt());

        if (agent.modelConfig() != null) {
            var config = agent.modelConfig();
            builder = builder.defaultOptions(DeepSeekChatOptions.builder()
                .withModel(config.model())
                .withTemperature(config.temperature())
                .withMaxTokens(config.maxTokens())
                .build());
        }
        return builder.build();
    }
}
```

### 3.2 五步执行流程

```mermaid
graph LR
    S1["1. 构建 ChatClient<br/>每个 Agent 独立配置"]
    S2["2. 构建 Prompt<br/>system + user"]
    S3["3. 注入历史消息<br/>多轮对话上下文"]
    S4["4. 注入工具<br/>@Tool 方法"]
    S5["5. 流式执行<br/>stream()"]

    S1 --> S2 --> S3 --> S4 --> S5

    style S1 fill:#e3f2fd
    style S2 fill:#e8f5e9
    style S3 fill:#fff3e0
    style S4 fill:#fce4ec
    style S5 fill:#f3e5f5
```

### 3.3 为什么每次都 buildClient

`ChatClient.Builder` 是 Spring AI 提供的工厂，每次调用 `build()` 创建一个独立的 ChatClient 实例。每次执行都重建的原因：

```java
// 错误做法：共享单例
@Bean
ChatClient sharedClient(ChatClient.Builder builder) {
    return builder.defaultSystem("固定提示词").build();
}
// 问题：所有 Agent 共享同一个 systemPrompt

// 正确做法：每次构建
private ChatClient buildClient(AgentDefinition agent) {
    return chatClientBuilder
        .defaultSystem(agent.systemPrompt())  // 每个 Agent 不同
        .build();
}
```

每个 Agent 有不同的 systemPrompt、temperature、maxTokens。用 Builder 每次构建，确保隔离。`ChatClient.Builder` 本身是轻量级的，构建开销可忽略。

> 「遇到阻塞？→ [教程 03-工具调用](../../教程/03-工具调用.md)」

---

## 4. DagEngine：编排引擎深度解析

### 4.1 核心调度逻辑

```java
@Service
public class DagEngine {

    private static final int MAX_CONCURRENCY = 10;

    /**
     * 调度所有就绪节点
     */
    private Flux<DagEvent> scheduleReadyNodes(DagDefinition dag) {
        List<DagNode> readyNodes = findReadyNodes(dag);

        if (readyNodes.isEmpty()) {
            if (allNodesDone(dag)) {
                return Flux.just(completeEvent(dag));
            }
            if (hasFailedNodes(dag)) {
                return Flux.just(failEvent(dag));
            }
            // 有节点在运行中，等待回调
            return Flux.empty();
        }

        return Flux.fromIterable(readyNodes)
            .flatMap(node -> executeNode(dag, node), MAX_CONCURRENCY);
    }

    /**
     * 查找就绪节点：所有前驱已完成
     */
    private List<DagNode> findReadyNodes(DagDefinition dag) {
        Set<String> doneNodes = dag.nodes().stream()
            .filter(n -> n.status() == NodeStatus.DONE)
            .map(DagNode::nodeId)
            .collect(Collectors.toSet());

        return dag.nodes().stream()
            .filter(n -> n.status() == NodeStatus.PENDING)
            .filter(n -> allPredecessorsDone(n, dag.edges(), doneNodes))
            .toList();
    }

    private boolean allPredecessorsDone(DagNode node,
                                        List<DagEdge> edges,
                                        Set<String> doneNodes) {
        return edges.stream()
            .filter(e -> e.to().equals(node.nodeId()))
            .allMatch(e -> doneNodes.contains(e.from()));
    }
}
```

### 4.2 拓扑排序的响应式实现

传统 DAG 拓扑排序是同步递归的。我们用响应式 Flux 实现了事件驱动的调度：

```mermaid
graph TB
    subgraph 同步拓扑["传统同步拓扑排序"]
        S1["计算入度"]
        S2["入度=0 的入队"]
        S3["while 队列非空"]
        S4["取队首 → 执行 → 阻塞等待"]
        S5["完成后减少后继入度"]
        S6["新入度=0 的入队"]

        S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S3
    end

    subgraph 响应式调度["响应式事件驱动调度"]
        R1["findReadyNodes<br/>找到所有可执行节点"]
        R2["flatMap 并行执行"]
        R3["节点完成回调"]
        R4["reschedule<br/>重新检查就绪节点"]

        R1 --> R2 --> R3 --> R4 --> R1
    end

    style 同步拓扑 fill:#fff3e0
    style 响应式调度 fill:#c8e6c9
```

核心差异：响应式调度**不阻塞等待**节点完成。每个节点执行完成后通过回调触发 `reschedule`，重新检查哪些节点变为可执行。这让编排引擎能同时管理大量并发任务。

### 4.3 并行调度关键

```java
// 限制并行度，防止资源耗尽
private static final int MAX_CONCURRENCY = 10;

Flux.fromIterable(readyNodes)
    .flatMap(node -> executeNode(dag, node), MAX_CONCURRENCY)
```

`flatMap` 的第二个参数控制最大并发数。10 个 Agent 同时执行是经验值——DeepSeek API 的并发限制和服务器资源决定了这个上限。

> 「遇到阻塞？→ [教程 31-Agent工作流编排](../../教程/31-Agent工作流编排.md)」

---

## 5. RoutingStrategy：智能路由评分

### 5.1 评分公式

```java
double total = capabilityScore * 0.30   // 能力匹配
             + semanticScore * 0.25     // 语义相似
             + successScore * 0.20      // 历史成功率
             + loadScore * 0.15         // 负载
             + latencyScore * 0.10;     // 延迟
```

### 5.2 各维度评分详解

```mermaid
graph TB
    subgraph 评分流程["单个 Agent 评分流程"]
        I1["输入: DagNode + AgentDefinition"]
        I2["维度 1: capabilityScore<br/>capabilities.contains(所需能力)<br/>→ 0 或 1"]
        I3["维度 2: semanticScore<br/>LLM 打分<br/>→ 0.0 ~ 1.0"]
        I4["维度 3: successScore<br/>历史成功率<br/>→ 0.0 ~ 1.0"]
        I5["维度 4: loadScore<br/>1 - (活跃任务/10)<br/>→ 0.0 ~ 1.0"]
        I6["维度 5: latencyScore<br/>(10000 - 平均ms) / 9000<br/>→ 0.0 ~ 1.0"]
        I7["加权求和<br/>→ 0.0 ~ 1.0"]
        I8["输出: ScoredAgent"]

        I1 --> I2 --> I3 --> I4 --> I5 --> I6 --> I7 --> I8
    end

    style I2 fill:#e8f5e9
    style I3 fill:#e3f2fd
    style I4 fill:#fff3e0
    style I5 fill:#fce4ec
    style I6 fill:#f3e5f5
```

### 5.3 语义评分的 LLM 调用

```java
private Mono<Double> computeSemanticScore(AgentDefinition agent, DagNode node) {
    String prompt = """
        评估任务描述与 Agent 能力描述的匹配度，返回 0.0-1.0。

        Agent 名称：%s
        Agent 描述：%s
        任务描述：%s

        只返回数字。
        """.formatted(agent.name(), agent.description(), node.description());

    return chatClient.prompt()
        .user(prompt)
        .call()
        .content()
        .map(content -> {
            try {
                double score = Double.parseDouble(content.trim());
                return Math.max(0.0, Math.min(1.0, score));  // clamp
            } catch (NumberFormatException e) {
                return 0.5;  // 解析失败给默认分
            }
        })
        .onErrorReturn(0.5);  // 异常降级
}
```

设计要点：

1. **Prompt 极简**——只返回数字，减少 token 消耗
2. **clamp 0-1**——LLM 可能返回越界值，强制限制
3. **默认 0.5**——解析失败或异常时给中等分数，不影响整体路由
4. **异步执行**——返回 Mono，不阻塞调度线程

### 5.4 路由降级策略

```java
public Mono<AgentDefinition> route(DagNode node, RoutingContext context) {
    return compositeStrategy.rankCandidates(node, context)
        .timeout(Duration.ofSeconds(5))
        .flatMap(candidates -> candidates.isEmpty()
            ? fallbackToSimpleMatch(node)
            : Mono.just(candidates.get(0).agent()))
        .onErrorResume(ex -> {
            log.warn("Composite routing failed, falling back: {}",
                ex.getMessage());
            return fallbackToSimpleMatch(node);
        });
}
```

三级降级链：

```mermaid
graph LR
    L1["综合路由<br/>LLM 评分"] -->|超时/异常| L2["简单匹配<br/>findByCapability"]
    L2 -->|无匹配| L3["错误<br/>NoAgentAvailable"]

    style L1 fill:#e8f5e9
    style L2 fill:#fff3e0
    style L3 fill:#ffcdd2
```

> 「遇到阻塞？→ [教程 27-模型路由与降级](../../教程/27-模型路由与降级.md)」

---

## 6. ApprovalGateway：审批网关

### 6.1 阻塞等待机制

审批网关的核心挑战是**阻塞 DAG 执行直到人工操作**。我们用 `Sinks` + Redis 实现响应式等待：

```java
public Mono<ApprovalResult> requestApproval(
        String taskId, String nodeId, ApprovalRequest request) {

    return stateStore.updateNodeStatus(taskId, nodeId, NodeStatus.BLOCKED, null)
        .then(stateStore.saveApprovalRequest(request))
        .then(pendingApprovals.tryEmitNext(request))
        .then(stateStore.waitForApproval(request.requestId(),
            Duration.ofHours(1)));  // 1 小时超时
}
```

```mermaid
sequenceDiagram
    participant DE as DagEngine
    participant AG as ApprovalGateway
    participant TS as TaskStateStore
    participant U as 用户

    DE->>AG: requestApproval(node)
    AG->>TS: 节点状态 = BLOCKED
    AG->>TS: 保存 ApprovalRequest
    AG->>U: SSE 推送 approval_required

    Note over AG: Mono 阻塞等待...

    U->>AG: POST /api/approvals (APPROVE)
    AG->>TS: 保存 ApprovalResult
    AG->>TS: 通知等待的 Mono
    AG-->>DE: 返回 ApprovalResult
    DE->>DE: 继续执行后续节点
```

### 6.2 三种审批结果处理

```java
return approvalGateway.requestApproval(dag.taskId(), node.nodeId(), request)
    .flatMapMany(result -> switch (result.decision()) {
        // 通过 → 标记完成，继续
        case APPROVE -> stateStore.updateNodeResult(
                dag.dagId(), node.nodeId(),
                "Approved: " + result.comment(), NodeStatus.DONE)
            .thenMany(reschedule(dag, node.nodeId()));

        // 驳回 → 终止任务
        case REJECT -> stateStore.updateNodeStatus(
                dag.dagId(), node.nodeId(), NodeStatus.FAILED, null)
            .thenMany(Flux.just(failEvent(dag, "Rejected")));

        // 修改 → 注入修改，继续
        case MODIFY -> stateStore.updateGlobalContext(
                dag.dagId(), result.modifications())
            .then(stateStore.updateNodeResult(
                dag.dagId(), node.nodeId(),
                "Modified: " + result.comment(), NodeStatus.DONE))
            .thenMany(reschedule(dag, node.nodeId()));
    });
```

MODIFY 的设计是最有价值的——人工不只是"是/否"，还能调整任务参数后继续执行。例如审核翻译质量时，审批者可以修改部分翻译后让流程继续。

> 「遇到阻塞？→ [教程 22-Human-in-the-Loop与审批流](../../教程/22-Human-in-the-Loop与审批流.md)」

---

## 7. TaskParser：LLM 任务拆解

### 7.1 结构化输出

```java
return chatClient.prompt()
    .user(prompt)
    .call()
    .entity(DagDefinition.class);
```

`.entity(DagDefinition.class)` 是 Spring AI 2.0 的结构化输出 API——它自动：

1. 在 Prompt 中附加 JSON Schema 指令
2. 解析 LLM 返回的 JSON
3. 反序列化为 Java 对象

### 7.2 Prompt 工程

```java
private static final String PARSE_PROMPT = """
    你是一个任务编排规划师。用户会给你一个复杂任务描述，
    你需要将其拆解为多个子任务，并定义子任务之间的依赖关系。

    可用的 Agent 能力列表：{availableCapabilities}

    输出 JSON 格式的 DAG 定义：
    {
      "nodes": [{"nodeId":"node-1","description":"...","requiredCapability":"..."}],
      "edges": [{"from":"node-1","to":"node-2"}]
    }

    规则：
    1. 每个子任务只需要一种能力
    2. 无依赖关系的子任务不要加边（允许并行）
    3. 子任务数量 2-6 个

    用户任务：{task}
    """;
```

Prompt 设计要点：

| 要素 | 说明 |
|------|------|
| 角色设定 | "任务编排规划师"——让 LLM 进入角色 |
| 可用能力列表 | 动态注入——LLM 只能选择已注册的能力 |
| 输出格式 | JSON Schema 示例——降低格式错误率 |
| 规则约束 | 3 条规则——限制子任务数量、鼓励并行 |

> 「遇到阻塞？→ [教程 07-Plan-and-Execute模式](../../教程/07-Plan-and-Execute模式.md)」

---

## 8. SessionStateStore：会话状态管理

### 8.1 Redis Key 设计

```mermaid
graph TB
    subgraph RedisKey设计["Redis Key 设计"]
        K1["agent:session:{sessionId}<br/>→ JSON(AgentSession)<br/>TTL: 24h"]
        K2["agent:def:{agentId}<br/>→ JSON(AgentDefinition)<br/>永久"]
        K3["agent:cap:{capability}<br/>→ Set(agentId...)<br/>永久"]
        K4["agent:msg:{agentId}<br/>→ Pub/Sub Channel<br/>实时"]
        K5["dag:{taskId}<br/>→ JSON(DagDefinition)<br/>永久（持久化到 PG）"]
    end

    style K1 fill:#e3f2fd
    style K2 fill:#e8f5e9
    style K3 fill:#fff3e0
    style K4 fill:#fce4ec
    style K5 fill:#f3e5f5
```

### 8.2 getOrCreate 模式

```java
public Mono<AgentSession> getOrCreate(String sessionId, String agentId) {
    String key = "agent:session:" + sessionId;
    return redisTemplate.opsForValue().get(key)
        .map(this::deserialize)
        .switchIfEmpty(Mono.defer(() -> {
            AgentSession newSession = new AgentSession(
                sessionId, agentId,
                new ArrayList<>(), new HashMap<>(),
                LocalDateTime.now(), LocalDateTime.now()
            );
            return redisTemplate.opsForValue()
                .set(key, serialize(newSession), Duration.ofHours(24))
                .thenReturn(newSession);
        }));
}
```

关键设计：

1. **`switchIfEmpty` + `Mono.defer`**——defer 确保新建逻辑只在缓存未命中时执行
2. **原子性考虑**——极端情况下两个并发请求可能同时创建。对于会话来说可接受（最终一致），正式环境可用 Redis SETNX
3. **TTL 自动过期**——24 小时后 Redis 自动清理，无需定时任务

> 「遇到阻塞？→ [教程 11-Agent状态管理](../../教程/11-Agent状态管理.md)」

---

## 9. MessageBus：Agent 间通信

### 9.1 Sinks + Redis Pub/Sub

```java
@Service
public class MessageBus {

    // 本地 Sink：每个 Agent 一个接收器
    private final Map<String, Sinks.Many<AgentMessage>> agentSinks =
        new ConcurrentHashMap<>();

    // Redis 订阅：跨实例消息
    @PostConstruct
    public void subscribeToRedisChannels() {
        // 订阅广播频道
        redisTemplate.listenToChannel("agent:msg:broadcast")
            .subscribe(msg -> dispatchToSink("broadcast", msg));

        // 订阅各 Agent 频道
        agentRegistry.findAll()
            .subscribe(agent ->
                redisTemplate.listenToChannel("agent:msg:" + agent.agentId())
                    .subscribe(msg -> dispatchToSink(agent.agentId(), msg))
            );
    }

    private void dispatchToSink(String agentId, String json) {
        AgentMessage msg = deserialize(json);
        agentSinks.computeIfAbsent(agentId,
            k -> Sinks.many().multicast().onBackpressureBuffer()
        ).tryEmitNext(msg);
    }
}
```

### 9.2 两层通信模型

```mermaid
graph TB
    subgraph 两层通信["两层通信架构"]
        subgraph 进程内["进程内（Sinks）"]
            L1["Agent A"]
            L2["Sinks.Many"]
            L3["Agent B"]
            L1 -->|emitNext| L2
            L2 -->|asFlux| L3
        end

        subgraph 跨进程["跨进程（Redis Pub/Sub）"]
            R1["实例 1 的 Agent A"]
            R2["Redis Channel"]
            R3["实例 2 的 Agent B"]
            R1 -->|publish| R2
            R2 -->|subscribe| R3
        end
    end

    style 进程内 fill:#e8f5e9
    style 跨进程 fill:#fff3e0
```

**Sinks（进程内）**：同一 JVM 内的 Agent 间通信，零网络开销，延迟 < 1ms。

**Redis Pub/Sub（跨进程）**：多实例部署时，实例 1 的 Agent 需要与实例 2 的 Agent 通信。Redis 作为消息中转。

> 「遇到阻塞？→ [教程 08-多Agent协作](../../教程/08-多Agent协作.md)」

---

## 10. 关键模式总结

### 10.1 全项目设计模式一览

| 模式 | 应用位置 | 作用 |
|------|---------|------|
| 策略模式 | RoutingStrategy | 路由策略可替换 |
| 注册表模式 | AgentRegistry, ToolRegistry | 动态发现组件 |
| 观察者模式 | Sinks/Flux 事件流 | DAG 事件驱动调度 |
| 模板方法 | AgentExecutor.execute() | 固定五步流程 |
| 降级模式 | 路由降级、错误处理 | 保证可用性 |
| 管控分离 | DagEngine vs AgentExecutor | 编排与执行解耦 |

> 「遇到阻塞？→ [教程 14-管控分离架构](../../教程/14-管控分离架构.md)」

### 10.2 响应式编程模式

整个项目基于 Reactor 响应式编程，关键模式：

```mermaid
graph TB
    subgraph 响应式模式["响应式编程关键模式"]
        M1["Mono / Flux<br/>单值 / 多值异步流"]
        M2["flatMap 并行<br/>多 Agent 并行执行"]
        M3["switchIfEmpty<br/>缓存未命中 fallback"]
        M4["onErrorResume<br/>异常降级"]
        M5["concatWith<br/>保证 done 事件"]
        M6["takeUntil<br/>SSE 流终止条件"]
    end

    style 响应式模式 fill:#e8f5e9
```

| 模式 | 代码示例 | 使用场景 |
|------|---------|---------|
| flatMap 并行 | `Flux.fromIterable(nodes).flatMap(execute, 10)` | DAG 并行调度 |
| switchIfEmpty | `findById(id).switchIfEmpty(error())` | Agent 不存在时报错 |
| onErrorResume | `.onErrorReturn(0.5)` | 评分失败给默认值 |
| concatWith | `.concatWith(just(doneEvent))` | SSE 流追加 done |
| takeUntil | `.takeUntil(e -> e.isDone())` | SSE 自动断开 |

> 「遇到阻塞？→ [教程 37-响应式错误处理](../../教程/37-响应式错误处理.md)」

---

## 11. 架构决策回顾

### 11.1 关键决策清单

```mermaid
graph TB
    subgraph 核心决策["六大架构决策"]
        D1["1. 管控分离<br/>编排引擎不执行 Agent"]
        D2["2. DAG 事件驱动<br/>非阻塞拓扑排序"]
        D3["3. Redis 通信<br/>Pub/Sub + 状态存储"]
        D4["4. LLM 路由<br/>语义匹配 + 评分"]
        D5["5. 审批可修改<br/>APPROVE/REJECT/MODIFY"]
        D6["6. 响应式全链路<br/>WebFlux + Reactor"]
    end

    style 核心决策 fill:#e8f5e9
```

### 11.2 决策权衡分析

| 决策 | 好处 | 代价 |
|------|------|------|
| 管控分离 | 编排和 Agent 独立扩缩容 | 多一层间接调用 |
| DAG 事件驱动 | 高并发、非阻塞 | 调试复杂度高 |
| Redis 通信 | 跨实例、高性能 | 引入 Redis 依赖 |
| LLM 路由 | 语义精准 | 增加延迟 + 成本 |
| 审批可修改 | 灵活性高 | MODIFY 逻辑复杂 |
| 响应式全链路 | 高吞吐 | 学习曲线陡峭 |

每个决策都是权衡——没有银弹。这些选择在"中型企业多 Agent 平台"场景下是最优解，但在超大规模（万级 Agent）或超低延迟（毫秒级路由）场景下可能需要调整。

---

## 12. 性能优化建议

### 12.1 已内建的优化

| 优化 | 位置 | 效果 |
|------|------|------|
| 并行调度 | DagEngine.flatMap | 并行加速 2-5x |
| Redis 能力索引 | AgentRegistry Set | O(1) 能力查找 |
| 滑动窗口状态 | SessionStateStore | 控制上下文长度 |
| 工具调用 AOP | ToolCallLogger/Metrics | 无侵入观测 |
| 路由超时降级 | RoutingStrategy | 5s 超时保底 |

### 12.2 生产环境建议

```mermaid
graph TB
    subgraph 生产优化["生产环境优化方向"]
        O1["路由评分缓存<br/>相同 (agent,task) 不重复调 LLM"]
        O2["DAG 预编译<br/>常见任务模板预生成 DAG"]
        O3["Agent 连接池<br/>复用 ChatClient 实例"]
        O4["结果缓存<br/>幂等任务直接返回缓存结果"]
        O5["分片调度<br/>大 DAG 分片并行"]
    end

    style 生产优化 fill:#fff3e0
```

> 「遇到阻塞？→ [教程 33-Agent性能优化](../../教程/33-Agent性能优化.md)」

---

## 13. 总结

本篇对多 Agent 编排平台的全项目核心代码做了深度解析：

1. **AgentDefinition**——record 定义不可变契约，工具与 Agent 解耦通过 Bean 引用绑定
2. **AgentExecutor**——五步执行流程（构建 Client → Prompt → 历史注入 → 工具注入 → 流式执行），每次构建独立 Client 保证隔离
3. **DagEngine**——响应式事件驱动的拓扑排序，`flatMap` 并行调度，节点完成回调触发重新调度
4. **RoutingStrategy**——五维度评分（能力 30% + 语义 25% + 成功率 20% + 负载 15% + 延迟 10%），LLM 语义匹配 + 历史数据 + 超时降级
5. **ApprovalGateway**——基于 Sinks + Redis 的异步阻塞等待，支持 APPROVE / REJECT / MODIFY 三种审批决策
6. **TaskParser**——LLM 结构化输出将自然语言拆解为 DAG，Spring AI 的 `entity()` 方法自动反序列化
7. **MessageBus**——两层通信模型（进程内 Sinks + 跨进程 Redis Pub/Sub），兼顾性能和可扩展性
8. **六大架构决策**——管控分离、DAG 事件驱动、Redis 通信、LLM 路由、审批可修改、响应式全链路

至此，多 Agent 编排平台的全部 6 篇文档完成：

| 文档 | 核心内容 |
|------|---------|
| [00-需求分析与架构设计](00-需求分析与架构设计.md) | 业务场景、编排架构、API 设计、迭代路线 |
| [01-最小Demo搭建](01-最小Demo搭建.md) | Agent 抽象、注册中心、单 Agent 流式对话 |
| [02-迭代一-单Agent工具链](02-迭代一-单Agent工具链.md) | @Tool 工具注册、状态管理、多轮对话 |
| [03-迭代二-多Agent编排](03-迭代二-多Agent编排.md) | 多 Agent 注册、消息总线、DAG 引擎 |
| [04-迭代三-任务委派与路由](04-迭代三-任务委派与路由.md) | 智能路由、任务委派、审批网关 |
| 05-核心代码讲解（本文档） | 全项目核心代码深度解析 |
