...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | §11.3 摘要表 | 每行与各迭代篇 ADR 原文决策一致、落点链接正确（注意：表内编号沿用各篇标题下的 ADR 号，与 [09-ADR架构决策记录] 全量索引口径一致） |
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 抽查 §3/§4/§5/§6 四段核心代码与对应迭代篇（[02 §6.1]/[03 §7.2]/[04 §3.3]/[04 §6.3]）diff | 逐行一致（仅注释差异允许） |
| 2 | 03 篇与 04 篇的全篇回归验证各重跑一轮 | 全部 PASS——讲解没有掩盖实现退化 |
| 3 | 本篇 §2–§9 的"运行时核对（复用 …）"逐条执行 | 每条引用的既有断言仍通过 |
