...
> **前置阅读**：[教程 00-Agent核心概念](../../教程/00-Agent核心概念.md)、[教程 09-多Agent协作](../../教程/09-多Agent协作.md)、[教程 20-管控分离架构](../../教程/20-管控分离架构.md)。
...
- **取舍理由**：编排层与执行层独立扩缩容；编排可观测集中在引擎，Agent 可插拔。代价是多一层间接调用（[教程 20-管控分离架构]）
