...
> **前置阅读**：[教程 00-Agent核心概念](../../教程/00-Agent核心概念.md)、[教程 08-多Agent协作](../../教程/08-多Agent协作.md)、[教程 14-管控分离架构](../../教程/14-管控分离架构.md)。

> **API 真实性**：文中所有 Spring AI API 均按 [附录 12-SpringAI2-API基准] 的真实签名书写。
...
> 「遇到阻塞？→ [教程 01-Spring-AI框架入门](../../教程/01-Spring-AI框架入门.md)」
...
编排引擎中的"任务解析器"需要强推理能力的 LLM 来拆解复杂任务。DeepSeek 提供高性价比的推理能力，支持 Function Calling 和 JSON 结构化输出，适合做任务规划。接入方式用 `spring-ai-starter-model-openai` + `base-url: https://api.deepseek.com`（统一坐标，见 [教程 02 §2]）。
...
| 迭代一 | 单 Agent 工具链 | [02-迭代一-单Agent工具链](02-迭代一-单Agent工具链.md) | 工具注册、调用、状态管理 |
| 迭代二 | 多 Agent 编排 | [03-迭代二-多Agent编排](03-迭代二-多Agent编排.md) | 注册中心、Agent 间通信、DAG 引擎 |
| 迭代三 | 任务委派与路由 | [04-迭代三-任务委派与路由](04-迭代三-任务委派与路由.md) | 智能路由、审批网关 |
| 核心精讲 | 代码全解析 | [05-核心代码讲解](05-核心代码讲解.md) | 架构决策回顾 |

每一迭代都可独立运行，前一步是后一步的基础。按顺序阅读，每一步的新增核心代码控制在合理范围。
...
│   │   ├── DagEdge.java
│   │   ├── TaskState.java
│   │   └── ...（含 AgentSession、ApprovalRequest 等）
│   ├── store/                           # 存储层
...
> 「遇到阻塞？→ [教程 16-全链路可观测性](../../教程/16-全链路可观测性.md)」
...
- **取舍理由**：编排层与执行层独立扩缩容；编排可观测集中在引擎，Agent 可插拔。代价是多一层间接调用（[教程 14-管控分离架构]）
...
> 迭代过程中的演进 ADR 见各迭代篇末尾（ADR 002-03 ~ 002-11）。
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`ChatMemory` 等均按 [附录 12-SpringAI2-API基准] 的真实签名书写，可照抄编译；包名统一为 `com.example.orchestrator`。
...
6. **工具拦截纪律**：工具执行拦截/观测用 `ToolCallingManager` 装饰器（附录 12-02 §1.3），**禁止用 AOP 拦 `@Tool`**（反射调用绕过代理，收不到）。
