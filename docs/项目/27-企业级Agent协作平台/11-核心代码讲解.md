# 11-核心代码讲解

> **定位**：把项目 27 的核心实现串成主线——以**一次「客服 Agent 委托财务 Agent 生成季度报表」**贯穿各迭代关键代码的协作。前置阅读：[01-最小Demo](01-最小Demo.md) 至 [10-协作市场与生态](10-协作市场与生态.md) 全部。
>
> **铁律 0**：`ChatClient`/`EmbeddingModel`/Observation 用实证基准；协作协议/市场为「概念代码」。

---

## 一、一次跨 Agent 委托的完整旅程

```mermaid
sequenceDiagram
    participant CA as 客服Agent(委托方)
    participant M as CollabMediator
    participant R as 注册中心(02)
    participant P as 授权PDP(05)
    participant FA as 财务Agent(被委托)
    participant V as 校验器(06)
    participant AU as 审计(08)

    CA->>M: delegate("report.generate",{period:Q3},预算¥2)
    M->>M: 验证身份(05 互信)
    M->>P: 最小授权裁决(不放大)
    M->>R: discover("report.generate")(02 精确+语义)
    M->>FA: negotiate(03 协商)
    FA-->>M: ACCEPTED(¥1.2/8min)
    M->>AU: 记录 协商通过
    M->>FA: 异步执行(04)
    FA-->>M: 进度事件(SSE)→COMPLETED
    M->>V: 注入边界+schema校验(06)
    V-->>M: 通过(带trust标记)
    M->>AU: 完整时间线落审计
    M-->>CA: 结果(隔离包裹)+计量事件(10)
```

### 一.1 本节核对（完整旅程）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 旅程环节齐全 | 身份(05)→授权(05)→发现(02)→协商(03)→异步执行(04)→校验(06)→审计(08)→计量(10)，每步标注迭代篇 |
| 2 | 顺序正确 | 沿途顺序与各迭代篇设计一致（先验证再委托、先协商再执行、结果过 06 边界） |

## 二、编排入口（串联各迭代）

```java
// 概念代码：委托主流程
public Mono<DelegationResult> delegate(DelegationRequest req, AuthCredential cred) {
    return Mono.deferContextual(ctx -> {
        Identity id = verify(cred);                                   // 05 身份
        adjudicate(req, id);                                          // 05 最小授权
        AgentCard target = registry.discover(req.capability()).get(0);// 02 发现
        NegotiationResponse ok = negotiate(req, target);              // 03 协商
        String taskId = submit(target, req, ok);                      // 03/04 异步任务
        return observe(taskId)                                        // 08 打点
            .map(raw -> sandboxAndValidate(raw, target))              // 06 边界+校验
            .transform(this::resilient)                               // 07 降级链
            .doOnNext(r -> { metering.record(r); rating.record(r); });// 10 计量+评价
    });
}
```

### 二.1 本节测试与验证（委托主流程串联）

**前置条件**：`delegate(req, cred)` 可执行；全迭代组件（registry/授权/协商/任务/观察/校验/降级/计量）已装配。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 合法凭证 + 授权范围内委托 | 全链走通：verify→adjudicate→discover→negotiate→submit→observe→validate→metering/rating |
| 2 | 无凭证 / 越权委托 | 分别在 verify / adjudicate 被拒，链在早期短路 |
| 3 | 结果不合格 + 降级 | 经 `resilient` 走 07，最终落 outcome/计量合法结果 |
| 4 | 核对副作用纪律 | 任一环节失败不残留悬空副作用（07 补偿承接） |

**失败排查**：①某环节被跳过→`Mono.deferContextual` 链式有一环断返；②重试重复副作用→`resilient` 与 04/07 语义未对齐；③计量/评价未触发→`doOnNext` 挂链位置错误。

## 三、分层职责（读代码抓手）

| 类 | 迭代 | 职责 |
|----|------|------|
| `AgentCardRegistry` | 02 | 注册/心跳/两级发现 |
| `Negotiator`/`DelegationTask` | 03/04 | 协商 + 异步任务 + 进度取消 |
| `AuthorizationPDP` | 05 | 身份互信 + 委托不放大 |
| `ResultSandbox` | 06 | 注入边界 + schema 校验 |
| `DegradationChain`/`Compensator` | 07 | 降级链 + 副作用补偿 |
| `CollabObservability` | 08 | Span/指标/审计回放 |
| `Federation`/`Market` | 09/10 | 跨组织 + 市场 |

### 三.1 本节核对（分层职责）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 每类对应迭代篇 | 表中每类职责都能映射到对应迭代篇（02-10），无重复归属 |
| 2 | 职责单一清晰 | 每类"单一职责"描述与各迭代篇设计一致 |

## 四、最容易踩的三个坑

1. **委托无授权裁决** → 权限放大事故。**解法**：交集裁决前置（05）。
2. **对方返回直接进系统提示** → 间接注入。**解法**：边界包裹 + 指令样检测（06）。
3. **超时盲目重委托** → 副作用重复。**解法**：先查状态再降级（04/07）。

### 四.1 本节核对（三个易踩坑）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 每坑有解法落点 | 坑→解法→迭代篇对应完整（05/06/04/07），即可执行 |
| 2 | 坑与安全验收关联 | 三坑对应 00 验收的"不放大/注入拦截/可降级"三条量化线 |

## 五、全篇回归验证（对照 00 量化指标）

| 指标 | 目标 | 实现 |
|------|------|------|
| 发现匹配正确率 | ≥95% | 02 两级发现 |
| 委托不放大 | 100% | 05 交集裁决 |
| 注入拦截 | 100% | 06 边界 |
| 失败可降级 | 不中断 | 07 降级链 |
| 审计回放 | ≤5s | 08 时间线 |

> **下一步**：代码串讲完成。12-ADR 记录本项目 8 条关键决策；13-前沿做业界对照与演进。
