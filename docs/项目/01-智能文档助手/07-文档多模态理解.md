...
>
> **前置阅读**：[06-长文档理解与上下文工程](06-长文档理解与上下文工程.md)、[教程 06-TraceId全链路追踪/02-Tracer编程模型：手动开span Agent 开发](../../教程/09-前沿专题/02-多模态Agent开发.md)、[教程 02-SpringAI核心机制/03-结构化输出](../../教程/02-SpringAI核心机制/03-结构化输出.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/00-Advisor与ChatMemory.md) 为准。
>
...
> **本节核对（四问完整性）**：① 新增需求四项与 §3-§6 一一对应；② "上一版痛点"四条（表格乱序/图表丢失/扫描件静默失败/引用不定位）均能在 §2.1/§4.1/§5.1 找到病根——两条通过即本节达标。
...
**前置条件**：准备一份"正文 + 表格 + 架构图"混合版面 PDF；PDFBox 显式坐标已引入（`org.apache.pdfbox:pdfbox`，需引入）。

**材料——分区结果核对（验证辅助类，日志统一走 @Slf4j，禁用 System.out）**：
...
/**
 * 版面分区结果核对（验证辅助类，不属于生产代码路径）
 * 日志统一用 @Slf4j 的 log——保持格式/级别/traceId 关联一致，禁止 System.out
 */
@Slf4j
public class LayoutRegionPrinter {

    /** 对每页调 LayoutAnalyzer 后，输出分区清单供人工比对 */
    public void printRegions(List<LayoutRegion> regions) {
        regions.forEach(r -> log.info("分区结果：p{} {} 坐标({},{}) 尺寸({}x{}) 文本预览[{}]",
                r.pageIndex(), r.type(), r.x(), r.y(), r.width(), r.height(),
                r.rawText() == null ? "" : r.rawText().substring(0, Math.min(20, r.rawText().length()))));
    }
}
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn clean compile`（LayoutRegion + 分析器手写后） | 通过 |
| 2 | 材料 PDF 每页跑分区打印 | 表格页输出 TABLE 区、含架构图的页输出 FIGURE 区、正文页输出 TEXT 区；抽查 20 页三区分流正确率 ≥ 85%（验收 1） |
| 3 | 纯文本 PDF | 全部 TEXT 区，无误报 TABLE |
| 4 | 误判样本观察（构造两栏排版文本页） | 即使误判为 TABLE，走表格管线输出等价 Markdown，信息不丢（§2.2 降级设计成立） |

**失败排查**：表格页被判 TEXT→坐标聚类的纵向间隙阈值过松；正文误判 TABLE→跨列跳变判定过严；FIGURE 全漏→XObject 枚举未实现或位置矩阵解析错。
...
**前置条件**：§2.3 分区正确；table_data 旁路表已建；准备含 3 张表格的《预算权限矩阵》PDF。
...
List<TableCell> cells = List.of(
        new TableCell("部门",120,300), new TableCell("上限",280,300),
        new TableCell("研发部",120,316), new TableCell("800",280,316),
        new TableCell("运维部",120,332), new TableCell("300",280,332));
ReconstructedTable t = new TableReconstructor().reconstruct(cells);
// 断言：headers=[部门,上限]；rows 含 [研发部,800]/[运维部,300]；markdown 以 "| 部门 | 上限 |" 开头
```

```bash
curl -X POST http://localhost:8081/api/documents/upload -F "file=@budget-matrix-tables.pdf"
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料 Java 单测 | 行分组/列对齐/Markdown 输出全部正确（Y 同行、X 排序） |
| 2 | 材料 PDF READY 后查旁路 | `SELECT count(*) FROM table_data WHERE document_id='<该文档>'` = 3 |
| 3 | 精确数值问题 | answer="300 元"（与原文一致，验收 3 的 ≥95% 一致率抽样）；citations 含 page 与 tableId |
| 4 | 随机抽 10 个单元格与原文人工比对 | 重建正确率 ≥ 90%（验收 2） |
| 5 | 按 tableId 取回 JSON | `{headers, rows}` 结构完整，可直接拼 Prompt |

**失败排查**：行列错乱→ROW_TOLERANCE(6.0) 与实际行高不符（调容差）；表头识别错→首行无表头特征词（数据行被当表头）；数值答错→JSON 旁路未启用，LLM 在读 Markdown 长表（核对 §6.2 旁路取数链路）；table_data 为空→TABLE 分区没进表格管线（回查 §2.3）。
...
> **VLM 能力边界（什么该交给它、什么不该）** → [教程 06-TraceId全链路追踪/02-Tracer编程模型：手动开span Agent 开发 §3](../../教程/09-前沿专题/02-多模态Agent开发.md)：教程详细对比了 VLM 擅长（版面理解/图表主旨/关系描述）与不擅长（精确小字 OCR/密集表格数值/像素级定位）的任务，以及图像 Token 成本模型。

### 4.3 本节测试与验证（VLM 图表理解）

**前置条件**：接入的 chat 模型支持图像输入；figure_desc 旁路表已建；上传含架构图与趋势图的《安全规范》。
...
curl -X POST http://localhost:8081/api/qa -H "Content-Type: application/json" \
  -d '{"question": "图中有没有负载均衡组件？"}'   # 幻觉探针（图里没有）
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn clean compile`（ChartUnderstandingService 手写后） | 通过（`media(MimeType, Resource)` / `Media.Format.IMAGE_PNG` 为真实 API） |
| 2 | READY 后查 figure_desc | 每张图一行：描述/洞察/imagePath/坐标齐备；向量库出现 level=FIGURE 块 |
| 3 | 材料探针 1 | 答案复述图中真实连接关系；citations 含 figureId |
| 4 | 材料探针 2（幻觉检查） | 图中没有则答"图中未见"，不编造（验收 4 幻觉率 ≤5% 抽样） |
| 5 | 按 figureId 取 imagePath 回显 | 前端能加载原图 |

**失败排查**：编译报 media 不存在→Spring AI 版本/包名（`org.springframework.ai.content.Media`）；模型拒图→所配模型不支持视觉输入；描述空洞→读图提示词缺少页面上下文参数（§4.2 细节①）；幻觉高→"只描述可见内容"约束未进提示词。
...
**前置条件**：tess4j 依赖已加、Tesseract 原生库与 `chi_sim`/`eng` 语言包已装（`TESSDATA_PREFIX` 已设）；准备一份扫描版合同（图片型 PDF，约 30 页）。

**材料**：

```bash
curl -X POST http://localhost:8081/api/documents/upload -F "file=@contract-scanned.pdf"
curl -X POST http://localhost:8081/api/qa -H "Content-Type: application/json" \
  -d '{"question": "合同违约金比例是多少？"}'
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn dependency:tree -Dincludes=net.sourceforge.tesseract` 后 `mvn clean compile` | tess4j 5.11.0 在树中且编译通过 |
| 2 | 材料 PDF 上传前手工调 `needsOcr` | 返回 true（文本层 < 50 字符） |
| 3 | READY 后核对 | chunkCount > 0；块的 `metadata.ocr=true`；ETL 耗时 < 90 秒（验收 5，OCR 跑 etlTaskExecutor） |
| 4 | 材料提问 | 能从 OCR 文本回答违约金比例；citations.fromOcr=true |
| 5 | 对照有文本层的同内容 PDF 检索 | OCR 版 Recall@5 ≥ 75%（阈值放宽 0.05 生效） |
| 6 | OCR 噪声抽查 | 页码/孤立符号行已被 cleanNoise 过滤 |

**失败排查**：`needsOcr=true` 但 READY 后 0 块→OCR 分流没接进 EtlService（静默失败重现）；UnsatisfiedLinkError→Tesseract 原生库未装；中文识别乱码→语言包缺 chi_sim 或 `TESSDATA_PREFIX` 指错；ETL 超时→调线程池并发或降 dpi（低于 300 识别率下降需权衡）。
...
**前置条件**：§3.3/§4.3/§5.3 至少一项已通过；Citation 已升级为迭代五版（page/tableId/figureId/fromOcr）。

**材料——字段完整性核对（复用前几节提问）**：

```bash
curl -X POST http://localhost:8081/api/qa -H "Content-Type: application/json" \
  -d '{"question": "运维部的打车上限是多少？"}' | jq '.citations[0]'
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 表格类提问后核对材料输出 | Citation 含非空 page + tableId；旧三字段（documentTitle/citedSection/matchedChunkId）仍存在 |
| 2 | 图表类提问（§4.3 探针） | Citation 含 figureId，可回显原图 |
| 3 | OCR 文档提问（§5.3 探针） | fromOcr=true |
| 4 | 纯文本文档提问 | page 有值、tableId/figureId 为 null（字段语义不混用，验收 6） |

**失败排查**：page 恒 null→分区/块生成时 pageIndex metadata 未写入；tableId 查无旁路行→table_data 主键与块 metadata 的 tableId 不一致；旧字段丢失→record 重构时误删（向后兼容破坏）。

## 7. 全篇回归验证

> 单节材料已上移至 §2.3 / §3.3 / §4.3 / §5.3 / §6.3，此处只做跨章节回归（对照 §8 验收表）。

**回归断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 迭代二~四文本类评估集重跑 | 不劣化（验收 7）——多模态管线不影响既有文本链路 |
| 2 | 同一进程：混合 PDF 上传 → 表格数值问 → 图表关系问 → 扫描件问 | 四条链路均 PASS，引用定位各自正确 |
| 3 | `level=TABLE/FIGURE` 块整体过滤下线（模拟回滚） | 检索回退纯文本行为（ADR-07-01 可回滚验证） |
| 4 | 对照 §8 验收表逐行打勾 | 7 项全 PASS |

**失败排查**：文本链路劣化→分块路由误把正文送表格/VLM 管线；回滚后仍异常→旁路查询耦合进了主链路（应只在取数阶段启用）。
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 验收表 7 项每项能指到对应小节验证步骤 | 分区→§2.3；重建/精确问答→§3.3；图表→§4.3；扫描件→§5.3；溯源→§6.3；回归→§7 |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | ADR-07-01 与 §4.1 一致 | 描述化入库、无 CLIP 联合检索；level 过滤可下线（对照 §7 步骤 3） |
| 2 | ADR-07-02 与 §3.1 一致 | Markdown 主通道 + table_data 旁路（对照 §3.3 步骤 3/5） |
| 3 | ADR-07-03 与 §5.2 一致 | tess4j 本地、chi_sim+eng、300dpi；OcrService 独立 Bean |
...
> **本节核对（总结一致性）**：总结五点分别对应 §2.3 / §3.3 / §4.3 / §5.3 / §6.3 的验证结论，§7 回归全 PASS 即本文学习闭环。
