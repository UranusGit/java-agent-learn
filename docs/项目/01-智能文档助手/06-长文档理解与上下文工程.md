...
>
> **前置阅读**：[05-GraphRAG 知识图谱深化](05-GraphRAG知识图谱深化.md)、[教程 77-上下文工程](../../教程/77-上下文工程.md)、[教程 82-高级记忆架构](../../教程/82-高级记忆架构.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/00-Advisor与ChatMemory.md) 为准。
>
...
**为什么三层就够**：企业文档的检索需求天然呈三级分布（全文主旨/章节主题/具体条款），三层树已覆盖；更深的层级（RAPTOR 论文的递归聚类）增加摘要成本但命中率增益递减——在 10 万块规模下，收益不抵成本。这一取舍与 [教程 77-上下文工程 §3](../../教程/77-上下文工程.md) 的"Token 预算分层"一致：**上下文预算花在刀刃上**。
...
方案：**窗口记忆 + 超窗摘要化**——保留最近 N 条完整消息，更早的压缩为一条摘要消息。这与 [教程 77-上下文工程 §4](../../教程/77-上下文工程.md) 的压缩策略、[教程 82-高级记忆架构](../../教程/82-高级记忆架构.md) 的"短期→长期"分层同构。
