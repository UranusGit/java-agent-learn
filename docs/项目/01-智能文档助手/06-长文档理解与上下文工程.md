...
```bash
# 上传一段"一段多义"的测试文档（前半报销标准/后半审批流程，同一段落）
curl -X POST http://localhost:8080/api/documents/upload -F "file=@mixed-paragraph.pdf"
# 验证：chunk 数应比段落感知分块多 1（在语义边界处正确切开）
# 抽查：每块只含一个主题，检索"审批流程"不再命中"报销标准"块
```
...
curl -X POST http://localhost:8080/api/qa -H "Content-Type: application/json" \
  -d '{"question": "这份安全规范的核心设计原则是什么？"}'

# 章节级问题（应命中 SECTION 层）
curl -X POST http://localhost:8080/api/qa -H "Content-Type: application/json" \
  -d '{"question": "身份认证一章讲了什么？"}'

# SQL 验证摘要节点存在且带层级元数据
# SELECT metadata->>'level', count(*) FROM vector_store GROUP BY 1;
# 期望：DOC=1 / SECTION=N / CHUNK=M（叶子块占绝大多数）
```

### 6.3 冲突消解验证

```bash
# 上传 V1（effectiveDate=2026-01-01）与 V2（effectiveDate=2026-06-01）两版报销制度
# 提问"打车报销上限多少天？"
# 期望：答案依据 V2，且回答中提示"旧版本为 X 天，已于 2026-06-01 被 V2 取代"
# 连续问 5 次，答案稳定（不再随机在两版间跳）
```

### 6.4 记忆压缩验证

```bash
# 同一 sessionId 连续追问 30 轮（每轮 1 问 1 答 = 2 条消息）
# 第 25 轮后检查：记忆中第一条为"[此前对话摘要]"，总条数稳定在 21 左右
# 验证第 29 轮仍能正确回答"我们前面讨论的第二步找谁？"（摘要保留了问题脉络）
```
