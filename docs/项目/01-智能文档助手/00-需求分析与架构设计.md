...
>
> **前置阅读**：[教程 00-基础与核心/05-RAG检索增强生成 检索增强生成](../../教程/00-基础与核心/05-RAG检索增强生成.md)、[教程 00-基础与核心/06-向量数据库选型](../../教程/00-基础与核心/06-向量数据库选型.md)。
...
| 08 | [ADR 架构决策记录](08-ADR架构决策记录.md) | 阶段三：沉淀收官 | 13 条 ADR 决策资产与三条元规律 |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 不看正文，能说出痛点三组量化数据（检索耗时 / 重复提问率 / 过时文档误用率）的量级 | 15 分钟/次、43%、12% |
| 2 | 能指出管理层目标的核心词 | "自然语言提问 + 秒级 + 基于企业文档" |
...
> **深入理解 RAG 的完整流程** → [教程 00-基础与核心/05-RAG检索增强生成 检索增强生成](../../教程/00-基础与核心/05-RAG检索增强生成.md)：该教程详细讲解了 RAG 的数据摄入阶段（文档解析、文本分块、向量化、存储）和检索增强阶段（问题向量化、向量检索、Prompt 组装、LLM 生成），是理解本项目技术基础的核心前置阅读。
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 能说出 RAG 胜出的三大理由 | 语义理解强、知识实时更新、答案可追溯 |
| 2 | 能对应说出另外三个落选方案各自的关键短板 | 搜索引擎无法理解语义；FAQ 维护成本高；Fine-tuning 更新难/幻觉不可控 |
...
> **向量数据库选型深入** → [教程 00-基础与核心/06-向量数据库选型](../../教程/00-基础与核心/06-向量数据库选型.md)：该教程从查询延迟、召回率、写入吞吐、存储成本四个核心指标出发，详细对比了 PgVector、Redis、Chroma、Milvus、Pinecone 五种向量数据库的优劣，并提供了完整的选型决策框架。
...
> **高级 RAG 策略深入** → [教程 05-Observation可观测/02-组件交互：Registry、Handler、Convention、Filter协作 RAG 与 Agentic RAG](../../教程/08-架构师进阶/01-高级RAG与AgenticRAG.md)：该教程讲解了基础 RAG 的四大瓶颈（单跳检索、语义盲区、无重排、一刀切）以及混合检索多路召回 + Reranker 重排的完整解决方案。
...
> **想深入？→ [附录 07-架构决策方法论/00-ADR架构决策记录](../../附录/07-架构决策方法论/00-ADR架构决策记录.md)**：ADR 三要素（上下文 / 决策 / 取舍）的完整方法论。

### 3.1 本节核对（架构分层与决策一致性）

| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 不看正文指出四层各自的职责与代表组件 | 接入层=REST+SSE；核心层=文档管理/ETL/检索/问答；存储层=PgVector+PG+MinIO；外部依赖=LLM+Embedding |
| 2 | ADR-001~004 与 §3.3 三个决策一一对应，无互相矛盾 | 路线 RAG / PgVector / 异步 ETL / 检索迭代演进 |
| 3 | 架构全景图与分层职责表组件名一致（DocumentService/EtlService/RetrievalService/QaService 等） | 逐项对得上 |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | §4.3 最终版 pom.xml 与技术栈总览表逐项一致 | Boot 4.1.0 / Spring AI 2.0.0（BOM）/ Java 21 / WebFlux / PgVector starter / Tika / JPA / JDBC 均在两处出现 |
| 2 | 业务依赖均无 `<version>`（由 BOM/parent 锁定），密钥类配置无明文 | 逐条扫 pom 无版本冗余 |
| 3 | 能说出 Spring AI 2.0 给 RAG 的三个端到端抽象 | ETL 三接口、VectorStore 统一抽象、QuestionAnswerAdvisor |
...
├── src/main/resources/
│   ├── application.yaml               # 引导层：仅 profile 激活 + .env 导入（见 01 §3.2 两段式）
│   ├── application-docassistant.yaml  # 业务层：端口/数据库/Spring AI 全部业务配置
│   └── prompts/
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | §5.1 模块划分图与 §5.2 目录树的文件一一对应 | api/service/model/config 四包下每个类名两处一致 |
| 2 | 每个 Controller/Service 在 §3.2 分层职责表中能找到所属层 | 无"孤儿"组件 |
...
| **沉淀收官** | 决策资产与前沿演进 | 13 条 ADR 登记簿 + 2026 对照与演进 backlog | [08-ADR 架构决策记录](08-ADR架构决策记录.md)、[09-工业前沿与演进方向](09-工业前沿与演进方向.md) |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 迭代目标矩阵七个条目与 timeline 六个迭代 + 沉淀一致，且各自对应到 01/02/03/05/06/07/08/09 文档 | 编号与文件名逐一匹配 |
| 2 | 量化验收表中每个指标能指到对应迭代篇可复测 | P99<3s→迭代二；上传<500ms、ETL<30s→迭代一 |
| 3 | 非功能需求四象限（性能/可用性/可扩展/可观测）在验收表中均有落点 | 无空象限 |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | ER 图三实体（DOCUMENT/DOCUMENT_CHUNK/QA_LOG）字段与 §7.2 设计说明覆盖一致 | 状态机、分块单元、问答审计各自字段可指出 |
| 2 | DOCUMENT.status 的枚举值与 §5.2 DocumentStatus.java 注释一致 | UPLOADING/PROCESSING/READY/FAILED 四态 |
| 3 | 每条"关键设计说明"都能在后续迭代篇找到落点（分块策略→迭代一、引用审计→迭代二） | 引用关系成立 |
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 五条风险每条都能说出对应应对方案及其落点迭代 | 解析质量→Tika+OCR；分块溢出→Token 分块；召回率→迭代二混合检索；成本→批量 Embedding；幻觉→提示词+引用+阈值 |
| 2 | 无"只有风险没有应对"的孤儿条目 | 逐行核对 |
...
> **本节核对（总结一致性）**：总结四点（痛点清晰/选型合理/架构分层/迭代务实）分别对应 §1、§2+§4、§3、§6 的核对结论，全部通过即本文学习闭环。
...
> **本节核对（使用说明执行检查）**：① 后续手写代码时 API 以附录 05 基准核对过；② 新增依赖均以「pom.xml 追加依赖」块出现而非静默引用——两条均满足即本文规范落地。
