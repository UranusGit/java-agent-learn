...
| **架构如何演进** | 决策记录从"迭代篇内联片段"收敛为"主登记簿（ADR-01~13）+ 内联编号映射"双层结构 |
...
| ADR-13 | 包结构按文档助手业务分层命名，与 demo01 代码仓习惯（controller/tool）的差异为业务选择 | 工程 | 全篇 | — | ✅ 采纳 |

**决策域分布**：检索域 4 条（ADR-06~09）、数据域 4 条（ADR-02/05/10/11）、工程域 4 条（ADR-03/04/12/13）、总体 1 条（ADR-01）——检索质量是 RAG 项目的核心矛盾，决策密度与之相称。
...
| # | 核对项 | 通过判据 |
|---|--------|---------|
| 1 | 13 条 ADR 的"落地篇/原内联编号"列与各迭代篇内联 ADR 一一对应 | 抽查 ADR-06↔[03] ADR-001-07/08、ADR-09↔[05] ADR-05-01/02、ADR-11↔[07] ADR-07-01~03 均能对上 |
| 2 | 决策域分布计数正确 | 检索 4 / 数据 4 / 工程 4 / 总体 1 = 13 |
...
### ADR-13：包结构按文档助手业务分层命名（api/service/model），与 demo01 代码仓习惯的差异为业务选择

- **上下文**（全篇代码）：本项目的包命名为 `api`（Controller + `api/dto`）、`service`、`model`、`advisor`/`repository`/`config`/`exception`/`graph` 等；而 demo01 代码仓的既有习惯是接入层命名 `controller`、工具类代码收在单数 `tool` 包（类型化 Handler Bean 等）。两套习惯并存时，读者容易把本项目的 `api` 命名与"缺 tool 包"误判为疏漏——需要在决策层面说清楚差异是有意为之。
- **备选**：A：强制对齐代码仓习惯（`api`→`controller`、补建 `tool` 包）；B：保留 `api` 等业务分层命名并登记差异说明；C：双套并存渐进迁移。
- **决策**：B。两点差异说明——① **`api` vs `controller`**：本项目把请求/响应契约（DTO）与 Controller 同放 `api` 包（`api/dto` 子包），"接入层 = 接口契约整体"的业务语义优先于代码仓的层名惯例；② **tool 包缺位**：文档助手的主链路（ETL 解析 / 混合检索 / 问答编排）没有 `@Tool` 工具调用面——版面分析、OCR 等能力以 `Service` 形态被 ETL 管线直接编排，不存在"LLM 自主选择的工具注册表"，因此没有需要独立 `tool` 包承载的代码。
- **取舍**：放弃与代码仓习惯的字面一致，换来"包结构反映业务域"的自解释性。代价是跨项目阅读时需要这条 ADR 作映射说明。**当本项目引入 @Tool 工具调用面时**（如把版面分析 / OCR 暴露为 LLM 可调用的工具），应按代码仓习惯补 `tool` 单数包并以类型化 Handler Bean 方式注册——那时本决策的条件前提消失，按"推翻旧决策新增 ADR"流程演进。
- **后果与可回滚**：包重命名是纯机械重构（IDE 一键迁移 + import 改写），无运行时风险；差异本身登记在案，后续维护者不再误判。
...
13 条决策背后有三条反复出现的取舍模式，值得作为后续演进的判断框架：
...
智能文档助手的 13 条 ADR 是它的"决策 DNA"：从"为什么选 RAG"（ADR-01）到"为什么阻塞要收敛 boundedElastic"（ADR-12）再到"包结构差异为什么是业务选择"（ADR-13），每条都有上下文、备选、取舍、可回滚性。三条元规律（先验证再加深 / 换接口先行 / 降级可用绝不丢失）贯穿始终——它们比任何单条决策更值得带走。
