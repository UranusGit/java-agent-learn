...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。事件溯源记录"发生了什么"——工具执行的**每一步过程**都要进事件，且用户可回放。工具管线 emit 工具事件：`{callId, 事件序号, 阶段(编码/传输/执行/回填), 结果}`。用户看到"工具调用进入能力缝→HTTP传输→远端执行→结果回填"的完整事件链（可审计、可回放），失败时定位到阶段。
...
curl -X POST http://localhost:8080/api/chat -d '{"tool":"fetch_url","arg":"..."}'
