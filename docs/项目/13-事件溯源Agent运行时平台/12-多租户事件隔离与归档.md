...
public List<SessionEvent> load(String tenantId, String sessionId) {
    ...
    return jdbc.sql("""
            ...
            ORDER BY seq ASC""")
            .param("tid", tenantId).param("sid", sessionId)
...
# B：同租户正常读写
curl -H "X-Tenant-Id: acme" "http://localhost:8081/api/session/acme_s1/events"
```
...
**前置条件**：按 §6.2 手写 `ErasureService`（三步：删派生物 → `kms.revokeDataKey` 加密销毁 → `appendRaw` 墓碑 `ErasureExecuted`）；`SessionEventStore.appendRaw` 签名已加 tenantId；补一个**行权测试端点**（真实实现对 DBA/合规操作员鉴权）：

```java
    // 行权端点：POST /api/session/{sessionId}/erase —— 驱动 ErasureService.erase 三步
    @PostMapping("/{sessionId}/erase")
    public void erase(@PathVariable String sessionId,
                      @RequestHeader("X-Tenant-Id") String tenantId,
                      @RequestBody EraseRequest req) {
        erasureService.erase(tenantId, sessionId, req.legalBasis(), req.operator());
    }

    public record EraseRequest(String legalBasis, String operator) {}
```

**材料**：

```bash
# 行权：POST /api/session/{id}/erase 驱动 erase(tenantId, sessionId, legalBasis, operator)
curl -X POST -H "X-Tenant-Id: acme" -H "Content-Type: text/plain" \
  "http://localhost:8081/api/session/acme_s1/erase" -d '{"legalBasis":"GDPR-17","operator":"dpo@acme"}'
# 行权后读取：应解密失败（fail-closed 报「已行权不可读」）
curl -H "X-Tenant-Id: acme" "http://localhost:8081/api/session/acme_s1/events"
```

```sql
-- 墓碑链导出（与行权后读取去重，用 SQL 核对事件链）：ErasureExecuted 应在链上、哈希链不断
SELECT type, data FROM session_event WHERE tenant_id='acme' AND session_id='acme_s1' ORDER BY seq;
```
...
- [ ] ADR 013-23 能讲清"加密销毁 + 墓碑"解如何拆开数据可读性与事实可证明性， 它是全项目唯一真正不可逆决策（KMS 30 天窗口内可撤销）
