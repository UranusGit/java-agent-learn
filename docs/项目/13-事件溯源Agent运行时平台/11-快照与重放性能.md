...
time curl "http://localhost:8081/api/session/s1/events"
# 再访问一次（应命中快照，走增量）
time curl "http://localhost:8081/api/session/s1/events"
```
...
# B：重复投递同一条事件 3 次（模拟 at-least-once 重投）——kafka-console-producer 重发同 key 事件
#    key 用 tenantId（分区键，保同租户顺序）；payload 为 Outbox 外发的同一事件 JSON
for i in $(seq 1 3); do
  echo 'acme|{"sessionId":"acme_s1","seq":42,"type":"AssistantMessage","data":{}}' | \
    kafka-console-producer --bootstrap-server localhost:9092 --topic session-events \
      --property parse.key=true --property key.separator=|
done
# C：重投后查 token_attribution 数值是否重复累计（应为 1 份，不重复）
curl "http://localhost:8081/api/session/report/token?businessLine=consumer"
```
...
```bash
# 触发归档一个静默会话，再冷读拉回
curl -X POST "http://localhost:8081/api/archive/archive-session/s1"
curl "http://localhost:8081/api/session/s1/events"   # 应触发冷读拉回
```
...
- [ ] ADR 013-20 的三条备选（固定间隔 / 每 turn 快照 / 双阈值+归档线）各自为什么被否/被选能复述， 可回滚性"高"（快照层停用即降级全量重放、零迁移）
