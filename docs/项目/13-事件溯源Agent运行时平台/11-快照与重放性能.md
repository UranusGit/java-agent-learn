...
> 「遇到阻塞？→ [教程 81-Agent性能优化]、[教程 31-全链路可观测性]；事件外发与消费组扩缩容见 [教程 69-消费者与消费组]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
flowchart LR
    ...
    subgraph R["读侧（物化读模型）"]
        D -->|"消费组按需扩缩容<br/>[教程 02 §2]"| E["ReadModelProjector<br/>幂等 upsert"]
...
| 扩容 | 按写吞吐 | 消费组水平扩（[教程 02]） |

### 4.2 幂等消费：物化表的三条纪律

外发管道是 at-least-once（[教程 03 §2]），读模型必须幂等：
...
> 「需在 pom.xml 中添加依赖」：`spring-kafka`（外发管道）。`@KafkaListener` 消费与生产者机制详见 [教程 76-Spring集成与Agent事件驱动落地]。
...
- **参考**：[教程 70-投递语义与事务]（Outbox 解决双写一致）、[教程 31-全链路可观测性]（管道观测）
