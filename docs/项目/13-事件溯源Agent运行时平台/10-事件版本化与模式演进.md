...
> 「遇到阻塞？→ [教程 58-历史记录持久化与合规]、[教程 83-长任务持久化与中断恢复]；JSONB/Jackson 机制见 [教程 22-结构化输出]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
业务诉求：客服会话要区分「用户原始输入」与「脱敏后输入」（[教程 64-安全与权限控制] 的 DLP 要求）。旧事件里 `content` 是未脱敏原文——**语义要拆成两个字段**，这是教科书级破坏性变更：
...
curl "http://localhost:8080/api/session/s1/events" > before.json
...
- **参考**：[附录 04-测试策略]（回归资产方法论）、[教程 80-自我反思与Agent评估]（金标集思想同构）
