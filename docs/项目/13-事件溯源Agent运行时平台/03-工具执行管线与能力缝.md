...
**前置条件**：本迭代能力缝三分离 + ToolCallingManager 装饰器守卫实现；`application.yml` 工具事件日志开启。

**材料 A——工具事件断言脚本**：

```bash
# 调一个 HTTP 工具（local→http 能力缝；schema 零改动）
curl -X POST http://localhost:8080/api/chat -d '{"tool":"fetch_url","arg":"https://httpbin.org/get"}'
# 断言：日志出现成对的 tool/call 与 tool/result 事件
grep "tool/call" app.log | tail -1 && grep "tool/result" app.log | tail -1
```

**材料 B——守卫注入**：在装饰器 prev 检查点 mock 抛异常（工具前守卫）。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料A | local→http schema 零改动调用成功；tool/call 与 tool/result 成对出现（计数相等） |
| 2 | 材料B | 守卫拒绝时无 tool/result（拦截在 call 前）；会话不崩（返回可读错误） |
| 3 | 注入工具结果异常 | 工具事件仍成对（失败也落 result，覆盖孤儿隐患的第一步） |

**失败排查**：①事件不成对→拦截点只包了执行未包失败路径；②守卫拦截仍走 call→守卫没在 ToolCallingManager 装饰层实现（用了别的位置）；③错误中断会话→异常未转返回。
