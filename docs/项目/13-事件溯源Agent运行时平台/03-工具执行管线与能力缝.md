...
> **定位**：v2——把工具从「@Tool 直连数据源」升级为「**能力缝三分离 + 工具执行拦截**」。工具从此可换后端；工具执行有了稳定拦截点（`ToolCallingManager` 装饰器），审批/超时/记录都可插入。**设计哲学借鉴**：[附录 14-开源代码深度分析/04-模型可见能力]、[附录 14-开源代码深度分析/12-Java工程师借鉴手册 §K1 §K5]。
...
- **参考**：[附录 14-开源代码深度分析/12-Java工程师借鉴手册 §K1]
