...
> 「遇到阻塞？→ [教程 03-工具调用]、[教程 61-Human-in-the-Loop与审批流 §正确落点]、[附录 05-SpringAI2-API基准/02-Tool与Observation真实API §1.3]」
...
> **Advisor 拦不到「工具意图已返回、工具执行前」**——它环绕整个 ChatClient 调用，工具执行发生在 ChatModel 内部循环。`ToolCallingManager` 装饰器正好插在「模型决定调工具」与「工具实际执行」之间（[教程 61-Human-in-the-Loop与审批流 §正确落点]、[附录 05-SpringAI2-API基准/02-Tool与Observation真实API §1.3]）。
...
- **Why**：Advisor 环绕整个 ChatClient 调用，看不到「工具执行」这个时点；`ToolCallingManager` 正好插在「意图已定、执行未发生」之间（[教程 61-Human-in-the-Loop与审批流 §正确落点]）
