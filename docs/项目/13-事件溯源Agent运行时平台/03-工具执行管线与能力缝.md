...
> **定位**：v2——把工具从「@Tool 直连数据源」升级为「**能力缝三分离 + 工具执行拦截**」。工具从此可换后端；工具执行有了稳定拦截点（`ToolCallingManager` 装饰器），审批/超时/记录都可插入。**设计哲学借鉴**：[附录 14-开源代码深度分析/00-deepseek-harness/04-模型可见能力]、[附录 14-开源代码深度分析/00-deepseek-harness/12-Java工程师借鉴手册 §K1 §K5]。
> 「遇到阻塞？→ [教程 00-基础与核心/03-工具调用]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §正确落点]、[附录 05-SpringAI2-API基准/02-Tool与Observation真实API §1.3]」
...
> **Advisor 拦不到「工具意图已返回、工具执行前」**——它环绕整个 ChatClient 调用，工具执行发生在 ChatModel 内部循环。`ToolCallingManager` 装饰器正好插在「模型决定调工具」与「工具实际执行」之间（[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §正确落点]、[附录 05-SpringAI2-API基准/02-Tool与Observation真实API §1.3]）。
...
- [ ] 一句话说清「为什么不是 Advisor」：Advisor 环绕整个 ChatClient 调用，看不到「工具意图已返回、工具执行前」这个时点；`ToolCallingManager` 正好插在「模型决定调工具」与「工具实际执行」之间（[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议 §正确落点]）
...
curl -X POST "http://localhost:8081/api/chat?sessionId=s1" -H "Content-Type: text/plain" -d "订单 ORD-002 金额多少"
# C：审计导出，看工具事件是否成对
...
- **Why**：Advisor 环绕整个 ChatClient 调用，看不到「工具执行」这个时点；`ToolCallingManager` 正好插在「意图已定、执行未发生」之间（[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §正确落点]）
...
- **参考**：[附录 14-开源代码深度分析/00-deepseek-harness/12-Java工程师借鉴手册 §K1]
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| schema 零改动 | 后端从 local 换 http 数据源，schema 零改动（§3.6 材料 A/B） | ☐ |
| 工具事件成对 | `tool/call` 与 `tool/result` 成对进日志（§3.6 步骤） | ☐ |
| 装饰器可插守卫 | 审批/超时守卫经 `ToolCallingManager` 装饰器插入（§3.6 步骤） | ☐ |
