...
> 「遇到阻塞？→ [附录 07-架构决策方法论/00-ADR架构决策记录]、[附录 14-开源代码深度分析/11-设计哲学与架构模式]（决策的哲学源头）」
...
- **取舍理由**：契约面**闭合**（改联合要过编译器），扩展面（事件 data 字段）**开放**（record 自由加字段）——一收一放正是 [附录 14-开源代码深度分析/11-设计哲学与架构模式 §二] 的"assertNever 纪律"在 Java 的原生表达。v8 演进期兑现回报：新增 `UserMessageV2` 忘写投影分支，**编译期**报错而非线上漏投影
