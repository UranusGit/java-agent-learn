...
> **定位**：把项目 13 从 [00] 到 [12] 的全部**架构决策**收束成一份体系化 ADR（Architecture Decision Record）资产——每条记录**决策上下文 / 备选方案 / 取舍理由 / 可回滚性**。这是工业项目的"决策 DNA"：三个月后有人（包括你自己）想改架构时，先来这里查"当时为什么这么定"，而不是推倒重来。各迭代篇内的 ADR 是"决策发生时的现场记录"，本文是**全项目视角的汇总、分级与关联**——两者互补，冲突时以迭代篇为准（现场记录优先）。读者画像：想复盘项目 13 全部关键决策"为什么"的读者、以及把它当 ADR 写作范式的读者。前置阅读：[00-需求分析与架构设计]；ADR 方法论见 [附录 07-架构决策方法论/00-ADR架构决策记录]。
> 「遇到阻塞？→ [附录 07-架构决策方法论/00-ADR架构决策记录]、[附录 14-开源代码深度分析/00-deepseek-harness/11-设计哲学与架构模式]（决策的哲学源头）」
...
- [ ] 能复述汇总要解决的三个跨迭代问题（决策全景/决策关联/决策生命周期），及各自的答案（13 篇散落→§2 总览表；依赖链→§1 决策链图；已升级 vs 未动→§7 时间线）
- [ ] 能举例说明"决策不是孤立的"：ADR 013-20（快照）依赖 013-01（事件溯源）成立、013-23 依赖 013-22——任举一例即 PASS
...
**ADR 自身的生命周期**（与 [附录 07-架构决策方法论] 对齐）：
...
- [ ] 总览表 23 条（013-01~013-23）编号连续无缺漏，每条状态与"详见"落点（本文或迭代篇）一致
- [ ] ADR 生命周期四态（草案/采纳/升级/废弃）能复述：升级 ≠ 推翻（内核不变、实现深化，如 013-08 指针→013-20 聚合态快照），被推翻的永不删除
...
- **取舍理由**：契约面**闭合**（改联合要过编译器），扩展面（事件 data 字段）**开放**（record 自由加字段）——一收一放正是 [附录 14-开源代码深度分析/00-deepseek-harness/11-设计哲学与架构模式 §二] 的"assertNever 纪律"在 Java 的原生表达。v8 演进期兑现回报：新增 `UserMessageV2` 忘写投影分支，**编译期**报错而非线上漏投影
...
- [ ] ADR 013-01（根决策）能讲清四个备选（关系表现态/内存+快照/消息表+平行审计/事件溯源）取舍，及"可回滚性低"（根决策回滚=重建项目、故最早 v1 落地）
- [ ] ADR 013-05 的 sealed 闭合联合、ADR 013-18 的版本信封 + Upcast 各能与本文 3.2/3.3 及 §1 决策链图中的位置对应
...
- [ ] ADR 013-06 的三备选（Advisor/AOP 拦 @Tool/ToolCallingManager 装饰器）能讲清为何 Advisor 拦不到工具执行时点（环绕整个 ChatClient 调用），`ToolCallingManager` 是唯一正确落点
- [ ] ADR 013-07 能指出"切后端 = 改一行配置"与消费者只依赖 `OrderQueryPort` 端口，对应 [03-迭代二 §3.2]
...
- [ ] ADR 013-08 能复述"合成闭合而非截断"（闭合保留已发生事实与审计；恢复本身可重放），ADR 013-09 讲清"常规批量 + 关键点同步 flush"的折中
- [ ] ADR 013-20 能指出这是对 013-09 时代的指针方案"升级而非推翻"，双阈值（事件数 + 时间间隔）补固定间隔方案的慢会话漏洞
...
- [ ] ADR 013-12/013-13 能讲清"写侧唯一入口从单方法升级为单服务"与"治理归 Control Plane、Data Plane 只执行"（违反=网络层不可达）
- [ ] ADR 013-14 指出成本即投影（v9 升级为 Outbox+物化、内核不动）；ADR 013-23 是唯一真正不可逆决策（加密销毁 + 墓碑），能指出其三层解耦（数据可读性 / 事实可证明性 / 审计豁免显式）
...
- [ ] v0~v10 每行能同时报出「痛点」与「该痛点驱动的 ADR 号」——任一行对不上即 FAIL（这是"痛点驱动决策"的验收证据）
- [ ] 时间线与 [00-需求分析 §2] 的演进 timeline、[02]~[12] 各迭代篇一一对应
...
| **要改某条决策** | 找到该 ADR 的"可回滚性"栏——低/中回滚的（013-01/05/12/23）必须走 [附录 07-架构决策方法论] 的推翻流程：新 ADR 引用旧的、说明为什么推翻 |
...
- [ ] 三个场景（新人接手 30 分钟全景 / 改决策查可回滚性并走推翻流程 / 对照自己项目作 checklist）各自的方法能复述
- [ ] 维护纪律三步（现场记录到迭代篇 + 总览表加行 + 更新决策链图）与"被推翻留档不删除"能复述
