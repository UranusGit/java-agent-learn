...
> 「遇到阻塞？→ [教程 08-架构师进阶/06-长任务持久化与中断恢复]、[教程 08-架构师进阶/05-高级记忆架构]、[教程 08-架构师进阶/07-数据飞轮与持续改进]；开放协议背景见 [教程 02-SpringAI核心机制/07-MCP协议]」
>
...
- 「遇到阻塞？→ [教程 08-架构师进阶/06-长任务持久化与中断恢复]」是这一演进的教材锚点——本项目与它的差异在视角：教程讲检查点/预算/死循环防护通用机制，本演进把它们落到**事件族的 fold 规则**上
...
|---------|---------|----------|---------|
| 事实型（订单号/客户档案） | 长 TTL 或无 TTL | 全量进入 | [教程 08-架构师进阶/05-高级记忆架构] 三层记忆 |
| 过程型（上一轮工具结果） | 短 TTL（如 24h） | 过期后折叠为摘要占位 | 同上 |
| 偏好型（用户习惯） | 显式记住/忘记事件控制 | 墓碑式失效（与 [12-迭代十] 同构） | 同上 |

**失效感知**（staleness-aware）落地为投影时的标注：旧事实折叠进历史时附带 `[stale: 30d]` 元信息，让模型自己判断可信度——这不是平台替模型做决定，而是把信息的时间维度如实呈现（呼应 [教程 08-架构师进阶/00-上下文工程] 的上下文装配原则）。
...
3. **飞轮闭环**：模拟回归 → 灰度发布（v7 能力）→ 生产新轨迹 → 再采样——这就是 [教程 08-架构师进阶/07-数据飞轮与持续改进] 的飞轮在事件溯源运行时上的具体转法
...
- 本地审计不变式不放松：**外部 Agent 的每一步在本日志同样留痕**——互操作不豁免审计（[教程 04-企业级架构主干/11-安全与权限控制] 的边界纪律）
...
> 每条前沿方向一行：**预研判据**（什么信号出现才立项）→ **落地验收**（做到什么程度算落地，逐条可检验）。落地的共同底线：一切新能力走事件追加，不改写已落日志（append-only 纪律）。

| 方向 | 预研判据 | 落地验收（做到即 PASS） | 依赖教程锚点 |
|------|---------|------------------------|-------------|
| E2 审批零计算暂停（P0） | v4 挂起审批占内存、实例缩容丢挂起态的真实事故或扩缩容需求出现 | 挂起态入日志：`approval/asked` 事件携带完整恢复上下文（工具意图+参数+fold 态引用）；asked 落日志后**实例可下线**（零计算占用）；`approval/decided` 到达后任意实例 fold 重建上下文从暂停点续跑（批准）或合成 `ToolResult(rejected)` 闭合（拒绝）；与 ADR 013-08 崩溃闭合是同一机制（演练验证：杀实例→批准→新实例恢复成功） | [教程 08-架构师进阶/06-长任务持久化与中断恢复] |
| E4 模拟先行（P0） | 新 Prompt/新模型上线缺回归防线（黄金事件集目前只用于 schema 兼容门） | 轨迹采样：从事件日志抽高价值会话（长任务/含审批/含恢复），脱敏（v10 DLP）后固化为模拟用例；上线前把历史 `UserMessage` 序列重放（工具调用打桩）比对新旧轨迹差异；黄金集测试门从"schema 兼容"扩展到"行为回归"（[10-迭代八 §6] 门禁升级） | [教程 08-架构师进阶/07-数据飞轮与持续改进] |
| E3 记忆 TTL 与失效感知（P1） | 旧事实与新事实同等进入上下文导致回答陈旧/成本虚增 | `deriveMessages` fold 规则引入事件年龄与类型权重：事实型长 TTL 全量进入、过程型短 TTL 过期折叠为摘要占位、偏好型由显式记住/忘记事件（墓碑）控制；旧事实折叠进历史时附带 `[stale: 30d]` 类标注（模型自行判断可信度）；fold 语义变更通过 v8 式等价测试 | [教程 08-架构师进阶/05-高级记忆架构]、[教程 08-架构师进阶/00-上下文工程] |
| E1 长任务语义（P1） | "监控工单直至闭环"类任务横跨数十个会话/多次重启的真实场景出现 | `TaskStarted/TaskProgressed/TaskClosed` 事件族以 sealed permits 走 append-only 扩展纪律（[10-迭代八]）；任务态 = 跨会话事件流投影，快照策略复用 v9 双阈值；重启后任务从投影恢复而非从会话重建 | [教程 08-架构师进阶/06-长任务持久化与中断恢复] |
| E5 A2A 互操作（P2） | 开放协议生态成熟 + 出现真实跨组织 Agent 调用需求（预留期只留缝不实现） | Agent Card 从 Control Plane 配置派生（能力 = 已注册工具面 + 治理策略投影）；`UserMessage.Source` 枚举扩展 `DELEGATED` 标注来源 Agent；**外部 Agent 每一步在本日志同样留痕**（互操作不豁免审计）；协议细节落地前完成专项调研 | [教程 04-企业级架构主干/11-安全与权限控制]、[教程 02-SpringAI核心机制/07-MCP协议]（开放协议背景） |

**验收用法**：P0 两项（E2/E4）的验收全部复用既有机制（事件对 + 回放基建），立项即可按本表演练验收；P1 两项涉及 fold 语义与事件族扩展，落地前重走 v8 式等价测试；P2（E5）预研期验收以"事件模型已留缝"为 PASS，不提前建协议层。
