...
> 「遇到阻塞？→ [教程 14-管控分离架构]、[教程 15-微服务拆分与Agent部署]」
...
public class SessionController {
    ...
    public List<Object> events(@PathVariable String sessionId) {
...
public class TokenMeter {
    ...
    public void record(String businessLine, String sessionId) {
        ...
        registry.counter("gen_ai.client.token.input", "businessLine", businessLine).increment();
        registry.counter("gen_ai.client.token.output", "businessLine", businessLine).increment();
    }
}
```
...
public class ToolController {
    ...
    @PostMapping("/execute")
    public Object execute(@RequestBody ToolExecRequest req) {
        Prompt prompt = Prompt.builder()
                .messages(req.messages())
                .options(ToolCallingChatOptions.builder()
                        .toolCallbacks(req.toolCallbacks()).build())
                .build();
        return toolCallingManager.executeToolCalls(prompt, req.toolOptions(), req.toolDefinitions());
    }

    public record ToolExecRequest(List<?> messages, List<?> toolCallbacks,
...
- **Why**：对应 [教程 14-管控分离架构]——加治理策略 = 改 Control Plane，不动 Data Plane
