...
**材料**：管控面与数据面分离部署；配置下发；故障隔离验证。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「独立扩缩容」场景执行 | 工具服务 3 实例、会话 1 实例，不中断 |
| 2 | 用材料构造「管控分离」场景执行 | 治理只在 Control Plane，Data Plane 零治理逻辑 |
| 3 | 用材料构造「跨服务链路」场景执行 | 一次对话 Trace 跨 会话→LLM→工具 可见 |
| 4 | 用材料构造「写侧唯一入口」场景执行 | 事件表只被会话服务读写 |
| 5 | 用材料构造「契约版本化」场景执行 | API 契约变更需版本化，旧版保留 2 周期 |
