...
> **关联教程**：[教程 08-架构师进阶/00-上下文工程]（五层拼接与压缩总纲）、[教程 02-SpringAI核心机制/04-结构化输出]（`entity(Class, spec)` 真实 API）；API 真实性以 [附录 05-SpringAI2-API基准] 为准。
...
意图分类是**结构化输出**的经典场景——用 `entity` 把 LLM 输出直接绑定到 record（[教程 02-SpringAI核心机制/04-结构化输出]）。
