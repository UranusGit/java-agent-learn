...
**前置条件**：§3 已通过；`SummarizingChatMemory` 已替换默认 ChatMemory；04 的 `saveAll` 已改为覆盖语义（见本节工程修复点 1）。
...
| 3 | 压缩后 `redis-cli LLEN chatmemory:<sid>` | 不翻倍（覆盖语义生效；若翻倍→saveAll 仍是追加） |

**失败排查**：①压缩后历史翻倍→saveAll 未改 delete+rightPushAll；②摘要丢订单号→`SummaryService` Prompt 未显式要求逐条保留硬事实；③压缩触发不了→阈值条件写错（30 条触发）。
