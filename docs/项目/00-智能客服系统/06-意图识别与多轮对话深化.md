...
> **关联教程**：[教程 08-架构师进阶/00-上下文工程]（五层拼接与压缩总纲）、[教程 02-SpringAI核心机制/03-结构化输出]（`entity(Class, spec)` 真实 API）；API 真实性以 [附录 05-SpringAI2-API基准] 为准。
...
「遇到阻塞？→ [教程 08-架构师进阶/00-上下文工程 §分层拼接策略]」——意图路由本质上是上下文工程的**前置决策层**：先决定「这一轮要拼哪几层上下文」，再进 ChatClient。
...
意图分类是**结构化输出**的经典场景——用 `entity` 把 LLM 输出直接绑定到 record（[教程 02-SpringAI核心机制/03-结构化输出]）。
...
> ⚠️ 分类器注入的是统一的无记忆辅助链 `assistantChatClient`（见 2.4 Bean 拆分与「链选型」原则）——它无 RAG / 无工具 / **无记忆**：分类只判定「这一句」是什么意图，不需要任何上下文拼接，更不该背历史。**默认与抽取、摘要共用同一条 `assistantChatClient`，不单独 new**。例外是成本治理：若想让分类单独用小模型（`/text` 类便宜模型），才在 `ChatClientConfig` 另配一个 `ChatClient.builder(chatModel).build()`（`ChatClient.builder(ChatModel)` 静态工厂，javap 实证）并挂不同 model 参数——**意图分类用便宜模型、正式回答用好模型**是成本治理的常见组合（[教程 04-企业级架构主干/07-成本治理与Token计量]）。该链无记忆也不依赖 `chatMemory`，注入它不会构成循环依赖（见 4.2.1 装配警告）。

### 2.4 双 ChatClient：轻量链与完整链

`ChatClient.Builder` 在 Boot 自动装配中是 prototype Bean，可构建多个不同配置的 ChatClient（[教程 00-基础与核心/02-ChatClient与对话模型]）：
...
public class ChatClientConfig {
    ...
    /** 轻量闲聊链：只保留人格与记忆，闲聊不检索、不挂工具（复用同一 ChatMemory）。 */
    @Bean
...
> 三个 Bean 用的是**同一个 prototype Builder 的多次 build**，互不影响。

**「链选型」三原则（本迭代的核心决策，新增辅助任务时的准绳）**：

| 原则 | 含义 | 结论 |
|------|------|------|
| **① 是否需要历史记录？** | 任务要「接上轮对话」才选记忆链；单轮判定绝不要历史 | 需要历史 → `liteChatClient`(闲聊)/`chatClient`(完整)；**不要历史 → `assistantChatClient`** |
| **② 是否需要 RAG / 工具？** | 知识问答要检索、业务办理要工具；纯判定都不要 | 检索/工具 → `chatClient`(完整)；无 → `assistantChatClient` |
| **③ 所有「不需要历史」的任务统一走同一条 `assistantChatClient`** | 分类、抽取、摘要……一律注入它，不各自 new | 新增无状态任务：注入 `assistantChatClient`，不造新块 |

**统一要点**：`assistantChatClient` 是项目里**唯一的**「无需历史 ChatClient」Bean。新任务若不需要历史，一律注入它，而非复制一个新块或用错了链——既避免「链的漂移」（职责不清），也避免「重复 Bean」散落。「谢谢啦」之后的「那我那个订单呢」由闲聊链的记忆接上，与辅助判定无关——两类链在语义上彻底分开。

> ⚠️ **配置类装配铁律（真实踩坑，必看）**：`ChatConfig` 里定义链 Bean 时，**ChatMemory 一律用 @Bean 方法参数注入，禁止用类字段注入**（如 `@Autowired private ChatMemory memory`）。原因：`@Configuration` 类本身会被实例化成一个 Bean，执行它上面**任一** @Bean 方法前必须先把配置类实例化；若配置类字段注入了 ChatMemory，实例化就会拉取 `SummarizingChatMemory` → `SummaryService` → `assistantChatClient`（同一配置类的 @Bean 方法）→ 又等配置类实例，构成**构造器级的实例化环**，启动直接抛 `BeanCurrentlyInCreationException`。方法参数注入把依赖推迟到具体 @Bean 方法被调用时，配置类实例化只需 `ChatClient.Builder`（Boot 自动装配，非本项目 Bean，无环）——`assistantChatClient`（只依赖 Builder）就能先生成，环被拆开。**这才是「辅助链无历史却仍循环」的隐形根因**。
...
/** ChatService.chatStream 改造（同步 chat 同理）：前置意图分类 → 按意图选链（闲聊轻量 / 知识·业务完整）。 */
@Service
public class ChatService {

    private final ChatClient chatClient;            // 完整链（记忆+RAG+工具）
    private final ChatClient liteChatClient;        // 闲聊记忆链
    private final IntentClassifier intentClassifier;

    public ChatService(ChatClient chatClient, ChatClient liteChatClient,
...
```bash
# ① 闲聊走轻量链
curl -N -X POST "http://localhost:8081/api/chat/stream?sessionId=chat-1" \
...
| 3 | `IntentClassifierTest`（30 条标注样本，`src/test/java/com/shop/customer/intent/IntentClassifierTest.java`） | 分类准确率 ≥ 90%；`confidence ∈ [0,1]`；`entity` 反序列化不抛异常 |

**失败排查**：①闲聊也触发检索→路由分支未生效（检查 switch 是否漏 CHITCHAT 分支直落完整链）；②FAQ 未检索→分类把 FAQ 误判为 CHITCHAT（补边界样本）；③`entity` 反序列化抛异常→结构化输出 schema 与 `IntentResult` 字段不匹配。LLM 相关测试分层见 [附录 04-测试策略]。
...
回看 [00-需求分析 §2.1 场景三]：换货要收集**订单号 + 新尺码**才能建工单。04 迭代里这完全靠 LLM 即兴——它可能忘了问尺码就调工具，也可能把上一轮的订单号张冠李戴。业务办理需要**确定性骨架**：缺什么槽就问什么，问全了才执行。这套「填槽-追问」模式源自传统对话系统的 Frame/Slot Filling，在 Agent 时代依然是多步业务最稳的落地方式（[教程 08-架构师进阶/02-Agent工作流编排 §状态机与工作流选型]）。
...
**关键设计**：槽位由 `SlotManager`（Java 代码）判定完整性，**追问话术交给 LLM 生成**——确定性（必须问什么）与灵活性（怎么问得自然）分工。把当前槽位状态拼进 system 上下文（[教程 08-架构师进阶/00-上下文工程 §五层拼接]：System → 工具 Schema → 记忆 → RAG → 用户输入，槽位指令属于 System 层的业务约束段）。
...
public class SlotExtractor {
    ...
    private final ChatClient assistantChatClient;   // 无记忆辅助链——抽取是一次单轮无状态判定，不需要历史

    public SlotExtractor(ChatClient assistantChatClient) {
    ...
    public ExchangeSlotState applyTo(String userMessage, ExchangeSlotState current) {
        SlotValues extracted = assistantChatClient.prompt()
...
/** ChatService.stream —— 在 §2.5 的路由基础上，为 BUSINESS 插入槽位闭环（完整类壳，三分支齐全）。 */
@Service
public class ChatService {

    private final ChatClient client;                 // 完整链（记忆+RAG+工具）
    private final ChatClient liteChatClient;         // 闲聊记忆链
    private final IntentClassifier classifier;       // 意图分类器（注入 assistantChatClient）
    private final SlotManager slotManager;           // 槽位管理器
...
```bash
# ③ 首轮不给订单号，应被追问
curl -N -X POST "http://localhost:8081/api/chat/stream?sessionId=slot-1" \
...
| 3 | `SlotManagerTest`（`src/test/java/com/shop/customer/slot/SlotManagerTest.java`） | 空→填订单号→填尺码→READY 流转正确；`isComplete()` 翻转时机正确；缺槽时 system 指令含「禁止调工具」 |
...
「遇到阻塞？→ [教程 08-架构师进阶/00-上下文工程 §上下文压缩]」
...
> ⚠️ **两个工程修复点**（照抄前必改）：
>
> 1. **装配不再用 `MessageWindowChatMemory`**：`ChatMemoryConfig` 里那个 `chatMemory(...)` Bean 改为返回 `new SummarizingChatMemory(redisChatMemoryRepository, summaryService)`（把 04 的 `@Bean RedisChatMemoryRepository` 注入进来即可，无需再做 `builder().maxMessages(20)`——窗口由本类的 `COMPRESS_THRESHOLD + KEEP_RECENT` 接管）`SummaryService` 注入 2.4 的无记忆辅助链 `assistantChatClient`（**不能注入 `liteChatClient` 或 `chatClient`**——它们都经 `MessageChatMemoryAdvisor` 挂上了 `chatMemory`，会构成构造器循环依赖，见 4.2.1 装配警告）。
 ...
 *
 * 注入 2.4 的无记忆辅助链 assistantChatClient（不带 chatMemory——挂 chatMemory 的 liteChatClient/chatClient
 * 会与 SummarizingChatMemory 构成构造器循环依赖；见 4.2.1 装配）。无 RAG/工具——摘要只是文本浓缩，省一次完整链开销。
...
> **装配（循环依赖警告）**：`SummaryService` 也注入 **2.4 的无记忆辅助链 `assistantChatClient`**（与分类器、`SlotExtractor` 共用同一个 Bean）。摘要同样是单轮无状态浓缩，不需要历史。**严禁注入 `liteChatClient` 或 `chatClient`**——这两个 Bean 在 `ChatClientConfig` 里都通过 `MessageChatMemoryAdvisor.builder(chatMemory)` 挂上了 `chatMemory`（即 `SummarizingChatMemory`）。若 `SummaryService` 注入它们，会得到 `chatMemory → SummaryService → (liteChatClient|chatClient) → chatMemory` 的构造器依赖环，Spring 构造器注入无法用三级缓存破解，启动即报 `BeanCurrentlyInCreationException`。
>
> `SummaryService` 构造器注入 `assistantChatClient`：`public SummaryService(ChatClient assistantChatClient)`。这样依赖图无环：`SummarizingChatMemory(chatMemory) → SummaryService → assistantChatClient`，且 `assistantChatClient` 不依赖任何东西——分类/抽取/摘要三类辅助任务共用它，无记忆、无 RAG、无工具，与 `chatMemory` 完全解耦。若摘要想用独立模型（如更便宜的压缩模型），在 `ChatClientConfig` 单独 `ChatClient.builder(chatModel).build()` 一个专用 Bean 注入即可（方法同 §2.4，本 demo 默认复用 `assistantChatClient`）。
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `SummarizingChatMemoryTest`（`src/test/java/com/shop/customer/memory/SummarizingChatMemoryTest.java`）：写入 35 条历史 | 触发一次压缩；输出 = 1 条摘要 + 10 条原文；摘要文本包含测试数据中的订单号 |
...
**回归断言**（§2.6 / §3.4 / §4.3 本节验证均通过后，最终整体验收）：
...
本迭代把「一条链打天下」拆成了三层精细治理：**意图决定走哪条链**（闲聊轻量/知识检索/业务填槽）、**槽位决定业务何时能执行**（确定性骨架 + LLM 话术）、**压缩决定长会话记多久**（滚动摘要替代硬截断）。三者共同点是：把「上下文拼什么」从 LLM 的即兴行为变成工程可控的决策——这正是上下文工程的核心（[教程 08-架构师进阶/00-上下文工程]）。
