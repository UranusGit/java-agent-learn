...
>
> **前置阅读**：[教程 00-基础与核心/00-Agent核心概念 核心概念](../../教程/00-基础与核心/00-Agent核心概念.md)、[教程 00-基础与核心/01-Spring-AI框架入门 AI 框架入门](../../教程/00-基础与核心/01-Spring-AI框架入门.md)。
...
| # | 核对项 | PASS 判据（能不看正文回答） |
|---|--------|--------------------------|
| 1 | 三大典型场景 | 能复述每个场景的用户诉求与 AI 需具备的能力差异 |
| 2 | 边界场景 | 能举出至少 2 个"AI 不该独自处理"的场景（如退款、维权） |
...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | 用户故事 ↔ 场景映射 | §1 每个场景至少有 1 条用户故事承接，无孤儿场景 |
| 2 | 非功能诉求 | 转人工/HITL、满意度反馈在用例中有对应条目 |
...
> 「遇到阻塞？→ [教程 00-基础与核心/01-Spring-AI框架入门 AI 框架入门](../../教程/00-基础与核心/01-Spring-AI框架入门.md)」
...
> 「遇到阻塞？→ [教程 00-基础与核心/06-向量数据库选型](../../教程/00-基础与核心/06-向量数据库选型.md)」
...
| # | 核对项 | PASS 判据（能不看正文回答） |
|---|--------|--------------------------|
| 1 | 每项选型 | 能说出"为什么选它 + 什么场景不适用 + 替代方案"（架构师三问） |
| 2 | 版本口径 | Spring Boot 4.1.0 / Spring AI 2.0.0 / WebFlux / Java 21，与 pom 基准一致 |
...
> 「想扩展？→ 本图接入层只有 Web 渠道（REST/SSE/WebSocket）；企业里的另一半入口在 IM（钉钉/企微/Slack）——渠道适配层、秒级 ACK 幂等与流式适配见 [教程 04-企业级架构主干/16-IM渠道接入工程 §3-§6](../../教程/04-企业级架构主干/16-IM渠道接入工程.md)」
...
> 「遇到阻塞？→ [教程 01-WebFlux与响应式编程/04-WebFlux-vs-MVC 链与拦截器](../../教程/02-SpringAI核心机制/01-Advisor链与拦截器.md)」
...
| # | 核对项 | PASS 判据（能不看正文回答） |
|---|--------|--------------------------|
| 1 | §4.1 分层图 | 能说出五层各自职责与数据层四个存储的分工（Redis/PGVector/MySQL/LLM API） |
| 2 | §4.2 时序图 | 能按顺序说出 Memory→RAG→Tool 三个 Advisor 在 LLM 调用前后的动作 |
...
> 「遇到阻塞？→ [教程 01-WebFlux与响应式编程/00-WebFlux从零入门 流式通信](../../教程/02-SpringAI核心机制/06-SSE流式通信.md)」
...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | 接口清单 vs 后续迭代实现 | §5.1 六个接口在 01-08 迭代中各有落点（或明确标注"演进方向"） |
| 2 | 数据结构 | `ChatRequest` 字段与 01 篇 Controller 实参一致（sessionId/message） |
...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | 路线 vs 实际文件 | 每个迭代在 `docs/项目/00-智能客服系统/` 有对应编号文件，无跳号 |
| 2 | 每迭代验收 | 路线图中每个迭代有可度量验收口径（非"完成即可"） |

## 7. 项目结构

```
smart-customer-service/
├── src/main/java/com/shop/customer/
│   ├── config/                 # 配置类
...
├── src/main/resources/
│   ├── application.yaml        # 入口配置：仅 .env import + spring.profiles.active=customerservice
│   ├── application-customerservice.yaml   # 业务配置（DeepSeek/Redis/PGVector，基准端口 8081）
│   ├── prompts/                # Prompt 模板
│   │   └── system.st           # 系统提示词
│   └── data/                   # 知识库源文件
│       ├── product-manual.md
...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | 规划结构 vs 实际手写工程 | 手写后的包结构与本节规划一一对应（com.shop.customer 下 web/service/tool/rag/memory/config） |
...
| # | 核对项 | PASS 判据 |
|---|--------|----------|
| 1 | 密钥管理 | 全部配置用 `${ENV_VAR}` 占位，无明文（铁律 9） |
| 2 | WebFlux 铁律 | 后续迭代代码无 ThreadLocal 传上下文、无 EventLoop block |
