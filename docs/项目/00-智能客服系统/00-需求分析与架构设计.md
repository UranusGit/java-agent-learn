...
> 「遇到阻塞？→ [教程 26-向量数据库选型](../../教程/26-向量数据库选型.md)」
...
> 「遇到阻塞？→ [教程 09-SSE 流式通信](../../教程/09-SSE流式通信.md)」
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`ChatMemory` 等均按 [附录 12-SpringAI2-API基准] 的真实签名书写，可照抄编译；标注「⚠ 修正」处为审计后对齐的基准写法。
