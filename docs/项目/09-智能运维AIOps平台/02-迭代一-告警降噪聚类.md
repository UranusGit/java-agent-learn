...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。AIOps 处置告警要可信——运维要看到"哪条告警被聚到哪簇、为何降噪、自动处置做了什么"。告警处理 emit 处置事件：`{alertId, 聚类簇, 降噪理由, 关联根因, 处置动作}`。运维看到"告警A因同窗口+同指标并入簇#3→判为次要→不升级"，而非一堆告警毫无解释。
