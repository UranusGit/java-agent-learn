...
> **定位**：把"2000 条告警/分钟"变成"值班只看 1 份故障简报"——三层降噪流水线（确定性去重 → 语义聚类 → LLM 事件链接与简报）——[教程 08-架构师进阶/04-Agent性能优化 §级联路由] + [教程 04-企业级架构主干/02-全链路可观测性 §标签设计] 的落地。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 08-架构师进阶/04-Agent性能优化 §级联路由]、[教程 04-企业级架构主干/02-全链路可观测性 §标签设计]、[附录 09-语义缓存与性能/00-语义缓存实现 §相似查询缓存]」
...
> 「降噪聚出 incident 之后怎么处置？→ [附录 18-可观测平台实践/01-AI事件响应工程 §4 Runbook 处置卡]——坏输出风暴/成本火情/静默漂移的止血与复盘闭环」
 ...
 * VectorStore 是阻塞调用（PgVectorStore 走 JDBC），用 boundedElastic 桥接（[教程 08-架构师进阶/08-响应式错误处理 §6]）。
 */
@Component
...
### 3.9 `application-aiops.yaml` 追加 pgvector 配置
...
**材料 A——三条相关告警 + 一条重复告警（同一故障簇）**：依次执行四条 curl（同一故障簇的演进 + 末条重复）：

```bash
# A1 簇种子：DB 连接池告警
curl -s -X POST http://localhost:8081/webhooks/alertmanager \
  -H "Content-Type: application/json" -d '{
  "alerts": [{"status":"firing","labels":{"alertname":"DbConnPoolHigh","severity":"critical","service":"order-db","instance":"db-01"},
   "startsAt":"2026-08-22T09:00:00Z","fingerprint":"fp-A1"}]}'
# 预期输出：HTTP 200，无响应体（可加 -w '%{http_code}\n' 确认；下同）

# A2 传播：订单查询变慢（同库不同告警名）
curl -s -X POST http://localhost:8081/webhooks/alertmanager \
  -H "Content-Type: application/json" -d '{
  "alerts": [{"status":"firing","labels":{"alertname":"OrderQuerySlow","severity":"warning","service":"order-db","instance":"db-01"},
   "startsAt":"2026-08-22T09:00:30Z","fingerprint":"fp-A2"}]}'

# A3 扩散：订单服务层慢查询（不同实例）
curl -s -X POST http://localhost:8081/webhooks/alertmanager \
  -H "Content-Type: application/json" -d '{
  "alerts": [{"status":"firing","labels":{"alertname":"OrderQuerySlow","severity":"warning","service":"order-api","instance":"pod-7"},
   "startsAt":"2026-08-22T09:01:00Z","fingerprint":"fp-A3"}]}'

# A4 重复：fp-A1 同 fingerprint 重发（应被 L1 去重，不新建簇）
curl -s -X POST http://localhost:8081/webhooks/alertmanager \
  -H "Content-Type: application/json" -d '{
  "alerts": [{"status":"firing","labels":{"alertname":"DbConnPoolHigh","severity":"critical","service":"order-db","instance":"db-01"},
   "startsAt":"2026-08-22T09:02:00Z","fingerprint":"fp-A1"}]}'
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料 A1–A4 连发 | 各条 HTTP 200；第 4 条（fp-A1 重复）在 5 分钟窗口内被 L1 判定重复，不新建簇 |
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 降噪率 | 告警量降 ≥ 80%；误报率 60% → < 15% | ✅ §3.10 断言 1/2（去重 + 聚类链路；30 天回测统计） |
| 聚类准确 | 相关告警归簇率 ≥ 90% | ✅ §3.10 断言 2/3（材料 A 同簇归一 + last_seen 更新） |
| LLM 成本 | LLM 触发率 < 5%；P99 < 500ms | ✅ §3.10 断言 4/7（needsLLM 门控；低危单告警不触发） |
| 降级不丢 | LLM 故障告警 100% 放行 | ✅ §3.10 断言 5（规则简报回退，ADR-407） |
| 简报质量 | root_cause 人工一致率 ≥ 70% | ✅ §3.10 断言 4（incident 簇读出简报；50 份抽检统计项） |
