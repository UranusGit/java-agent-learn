...
> **定位**：把"2000 条告警/分钟"变成"值班只看 1 份故障简报"——三层降噪流水线（确定性去重 → 语义聚类 → LLM 事件链接与简报）。教程 05-Observation可观测/05-前端展示：SSE推送观测时间线 级联路由 + 教程 02-SpringAI核心机制/03-结构化输出 标签设计的落地。本文代码为**完整可手写**（含全部 import、无省略）。
>
