...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 §弹性设计]、[教程 08-架构师进阶/10-多模型协作与供应策略 §模型编排]、[教程 08-架构师进阶/04-Agent性能优化 §批量与预热]」
...
```yaml
# application-aiops.yaml 追加（两段式约定见 01-§4.9，以 aiops profile 重启生效）
elasticity:
  budget:
