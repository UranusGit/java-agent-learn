# 项目 09：智能运维 AIOps 平台 — 11-迭代九：预测性容量与弹性
...
容量型故障有了前馈防线，但**突发故障的处置仍是人工链路**：值班看到简报 → 打开 runbook 文档 → 手动逐步执行。runbook 是给人看的文档，不是机器可执行的形态；处置步骤的风险、前置检查、回滚方法都锁在自然语言里。**需要把预案变成结构化、可灰度执行、带安全门的 Playbook——自愈闭环**。→ [12-迭代十-自愈闭环与安全处置.md](12-迭代十-自愈闭环与安全处置.md)
