...
> **定位**：把"大促前拍脑袋扩容"变成"数据驱动的容量预警"——时序预测 + LLM 辅助解释。基于调研结论：容量预测是 ROI 最高的 AIOps 场景，算法按时间尺度选型——[教程 08-架构师进阶/04-Agent性能优化] 的性能优化思路 + [教程 08-架构师进阶/10-多模型协作与供应策略 §边缘部署] 参考。本文代码为**完整可手写**（含全部 import、无省略）。
>
...
| 3 | 成本节省 | 按 P95 配置后，资源成本较峰值常备基线降 ≥ 20%（回测 90 天容量记录；行业基准最高可省 30%，见 §2） |
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 预测准确 | 7 天预测 MAE ≤ 15%；P95 覆盖真实峰值 ≥ 90% | ✅ §3.7 断言 1-3（p50/p95/季节偏移核对；90 天回测统计项） |
| 提前预警 | 容量耗尽前 ≥ 7 天预警 | ✅ §3.7 断言 4（P95 超配额 80% 提前告警，HORIZON_DAYS=7） |
| 成本节省 | 资源成本较峰值常备基线降 ≥ 20% | ✅ §3.7 断言 4/5（按 p95 而非峰值常备出建议；90 天容量记录对照统计项） |
| 报告可读 | 报告含瓶颈/建议/依据，值班可执行 | ✅ §3.7 断言 4（advisor 四字段报告 + 材料 B 落库核对） |
| 降级不丢 | 预测故障时规则兜底告警仍生效 | ✅ §3.7 断言 6（LLM 故障确定性告警不受影响，ADR-422） |
