# 项目 09：智能运维 AIOps 平台 — 08-高可用与演练（第一轮收官）

> **定位**：第一轮收官迭代——解决"运维 Agent 平台自身成为新单点"的悖论：服务拆分、熔断降级、混沌演练制度化——[教程 04-企业级架构主干/10-容错与弹性设计 全篇] + [教程 04-企业级架构主干/13-部署与运维 §演练] 的落地。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 全篇]、[教程 04-企业级架构主干/13-部署与运维 §演练]」
...
- [ ] 能复述降级哲学三句：告警绝不丢（fail-open）、LLM 可降级、审计/审批 fail-close。
- [ ] 能说出五服务拆分与多实例（×2）的对应关系。
...
- [ ] 四种故障模式（LLM/知识库/调度/全检测不可用）各自的降级行为能复述，且与 §3.6 yaml 键一一对应。
- [ ] "全部检测不可用 → 告警原样放行"是矩阵的最后一道闸（M4 对应 ADR-407）。
...
/**
 * 每个降噪/检测组件独立熔断器——一个组件故障不拖垮全链（[教程 04-企业级架构主干/10-容错与弹性设计 §熔断]）。
...
/**
 * 降级矩阵（声明式，可热更新）。对应 application-aiops.yaml 的 degradation.* 节点。
 */
@Component
...
### 3.6 `application-aiops.yaml` 追加降级矩阵配置
...
### 3.8 本节测试与验证（熔断、降级矩阵与混沌演练）

**前置条件**：已按 §3.1 添加 resilience4j 与 wiremock（test）；`db/schema-v7.sql` 已执行。

**材料 A——熔断器行为用例**（独立单测或 /tmp 小 main，直接用 `DetectionCircuitBreaker`）：

| 用例 | 操作 | 预期 |
|------|------|------|
| C1 | 对 `execute("llm-judge", ...)` 连续 10+ 次抛 RuntimeException（滑窗 20、失败率 50%） | `state("llm-judge")` 变为 OPEN，`llmAvailable()`=false |
| C2 | OPEN 后再调 `execute` | 不执行 supplier，直接抛 `CallNotPermittedException` |
| C3 | 等 30 秒（waitDurationInOpenState）后再调 | 进入 HALF_OPEN 试探；成功则 CLOSED |

**材料 B——演练报告归档 SQL**：

```sql
SELECT script_name, alert_dropped, result FROM chaos_drill_report ORDER BY executed_at DESC;
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料 A 三用例 | 状态迁移 OPEN/半开/CLOSED 与表一致 |
| 2 | yaml 绑定 | 启动后 `DegradationProperties` 读到 §3.6 的值（`llmUnavailable.alert=rule-based-brief`、`auditUnavailable.all=fail-close`） |
| 3 | 运行 `ChaosDrillTest` 四个剧本 | 剧本 A：dropped=0 且审计计数=100；剧本 B：`runbookCitationAvailable()=false` 且处置通道仍开；剧本 C：队列非空；剧本 D：dropped=0 |
| 4 | 剧本 B 前置真实性 | WireMock `/kb-search` 返回 500 连续失败后，`runbook-index` 断路器 OPEN（若直接 new 桩对象则不触发——需经熔断器访问，见 §3.5 演练说明） |
| 5 | 演练归档 | 四剧本结果写 `chaos_drill_report`（材料 B），`alert_dropped` 全为 0，result=passed |

**失败排查**：①步骤 1 不 OPEN→失败率未达 50%（滑窗内成功过多，加大失败次数）；②步骤 2 值为默认→`@ConfigurationProperties` 未被扫描（确认 `@Component` 或 `@EnableConfigurationProperties`）；③步骤 4 断路器不 OPEN→知识库调用没包进 `execute("runbook-index", ...)`；④剧本 C 队列为空→调度故障注入后没有入队逻辑（确认排队而非丢弃）。

## 4. 全篇回归验证（第一轮收官验收）

> 各节材料与断言已上移至 §3.8（熔断用例 + 混沌剧本）；本表为第一轮整体验收，不重复材料。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 无单点 | 任一服务单实例故障其余接管（告警零丢失） | ✅ §3.8 断言 3 剧本 D（kill 实例 dropped=0；×2 多实例部署） |
| 降级可预期 | 降级矩阵所有项有测试覆盖 | ✅ §3.8 断言 2/4（yaml 绑定核对 + 剧本 B 断路器 OPEN） |
| 告警生命线 | 任何组件故障告警 100% 放行 | ✅ §3.8 断言 3 剧本 A（dropped=0 且审计计数=100，ADR-407） |
| 审计底线 | 审计存储故障时处置 fail-close | ✅ §3.6 degradation.audit-unavailable.all=fail-close（§3.8 断言 2 绑定生效） |
| 演练制度化 | 季度演练报告归档，剧本 A-D 全过 | ✅ §3.8 断言 5（材料 B chaos_drill_report 全 passed） |
...
- [ ] timeline 的 v1-v8 与各迭代篇编号一一对应；复用的三条 ADR（407/413/415）与 [13] 总账一致。
- [ ] 第二轮（v9-v11）指向 [10]/[11]/[12]，无跨项目引用。
