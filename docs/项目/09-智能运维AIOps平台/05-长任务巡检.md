...
## 2. Checkpoint 机制（教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 落地）

> **认知基线**：**checkpoint ≠ durable execution**。Checkpoint 是"定期持久化中间状态，崩溃后从断点恢复"；durable execution（如 Temporal）更进一步，由外部运行时自动恢复执行、不重花 token。v5 采用**教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 的 Checkpoint 机制**（可手写、零外部依赖、够用），跨主机自动恢复的 durable execution（Temporal）记为生产演进方向（ADR-415 备选）。
...
| 等待审批 | — | 挂起持久化（教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 §挂起） | durable sleep（零内存成本） |
...
/**
 * 检查点：保存"恢复所需的最小状态"（教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 §2.2）。
 ...
 * 结合 checkpoints 里的"连续重复步骤检测"，构成死循环防护（教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断 §6）。
 */
@Component
...
| # | 决策 | 理由 |
|---|------|------|
| ADR-415 | 巡检用 Checkpoint 机制（教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断）持久化断点；跨主机 durable execution（Temporal）记为生产演进方向 | checkpoint 可手写、零外部依赖，覆盖"进程重启续跑"的核心需求；跨进程自动恢复是更大工程，按需引入 |
