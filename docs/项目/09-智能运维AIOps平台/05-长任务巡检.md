...
> **定位**：把"定时巡检/压测/长时诊断"从人工值班中解放出来——Checkpoint 化 + 断点恢复 + 预算控制 + 死循环防护。采用 [教程 83-长任务持久化与中断恢复] 的 Checkpoint 机制（可手写、零外部依赖）。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 83-长任务持久化与中断恢复 全篇]、[教程 63-容错与弹性设计 §重试]」
...
## 2. Checkpoint 机制（教程 40 落地）

> **认知基线**：**checkpoint ≠ durable execution**。Checkpoint 是"定期持久化中间状态，崩溃后从断点恢复"；durable execution（如 Temporal）更进一步，由外部运行时自动恢复执行、不重花 token。v5 采用**教程 40 的 Checkpoint 机制**（可手写、零外部依赖、够用），跨主机自动恢复的 durable execution（Temporal）记为生产演进方向（ADR-415 备选）。
...
| 等待审批 | — | 挂起持久化（教程 40 §挂起） | durable sleep（零内存成本） |
 ...
 * 结合 checkpoints 里的"连续重复步骤检测"，构成死循环防护（教程 40 §6）。
 */
@Component
 ...
 * 每步完成后落检查点；预算超限/异常时保存终态并终止。block() 仅在 boundedElastic 上出现（[教程 85-响应式错误处理 §6]）。
 */
@Service
...
| # | 决策 | 理由 |
|---|------|------|
| ADR-415 | 巡检用 Checkpoint 机制（教程 40）持久化断点；跨主机 durable execution（Temporal）记为生产演进方向 | checkpoint 可手写、零外部依赖，覆盖"进程重启续跑"的核心需求；跨进程自动恢复是更大工程，按需引入 |
