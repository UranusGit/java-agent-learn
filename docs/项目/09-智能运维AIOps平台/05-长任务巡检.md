...
> **定位**：把"定时巡检/压测/长时诊断"从人工值班中解放出来——Checkpoint 化 + 断点恢复 + 预算控制 + 死循环防护。采用 [教程 40-长任务持久化与中断恢复] 的 Checkpoint 机制（可手写、零外部依赖）。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 40-长任务持久化与中断恢复 全篇]、[教程 30-容错与弹性设计 §重试]」
 ...
 * 每步完成后落检查点；预算超限/异常时保存终态并终止。block() 仅在 boundedElastic 上出现（[教程 42-响应式错误处理 §6]）。
 */
@Service
