# 项目 09：智能运维 AIOps 平台 — 05-长任务巡检

> **定位**：把"定时巡检/压测/长时诊断"从人工值班中解放出来——Checkpoint 化 + 断点恢复 + 预算控制 + 死循环防护。采用 [教程 08-架构师进阶/06-长任务持久化与中断恢复] 的 Checkpoint 机制（可手写、零外部依赖）。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 08-架构师进阶/06-长任务持久化与中断恢复 全篇]、[教程 04-企业级架构主干/10-容错与弹性设计 §重试]」
...
- [ ] 能说清 checkpoint 与 durable execution 的差别（本进程重启续跑 vs 跨主机自动恢复）。
- [ ] 能指出三维预算是轮次/Token/时长，且超限抛异常终止。

## 2. Checkpoint 机制（[教程 08-架构师进阶/06-长任务持久化与中断恢复] 落地）

> **认知基线**：**checkpoint ≠ durable execution**。Checkpoint 是"定期持久化中间状态，崩溃后从断点恢复"；durable execution（如 Temporal）更进一步，由外部运行时自动恢复执行、不重花 token。v5 采用**[教程 08-架构师进阶/06-长任务持久化与中断恢复] 的 Checkpoint 机制**（可手写、零外部依赖、够用），跨主机自动恢复的 durable execution（Temporal）记为生产演进方向（ADR-415 备选）。
...
| 等待审批 | — | 挂起持久化（[教程 08-架构师进阶/06-长任务持久化与中断恢复] §2） | durable sleep（零内存成本） |
| 事件历史 | 手写 | 检查点字段自带 | 自动管理（continue_as_new 防超限） |
| **结论** | 不可用 | **v5 选型（本文实现）** | 生产级演进方向 |

### 2.1 本节核对（选型理解）

- [ ] 能对表中"崩溃恢复/定时启动/等待审批/事件历史"四维度说出 v5 与 Temporal 的差距。
- [ ] 能解释为什么 v5 不直接上 Temporal（零外部依赖、够用；ADR-415 备选）。
...
- [ ] 图中"load → 空/存在"两分支对应 `execute` 的 `flatMap(opt -> ...)`（§4.5）。
- [ ] 崩溃后重启用同一 taskId，从 quotas 恢复不重跑 certificates——对应幂等键机制。
...
/**
 * 检查点：保存"恢复所需的最小状态"（[教程 08-架构师进阶/06-长任务持久化与中断恢复] §2.2）。
 ...
 * 结合 checkpoints 里的"连续重复步骤检测"，构成死循环防护（[教程 08-架构师进阶/06-长任务持久化与中断恢复] §6）。
 */
@Component
 ...
 * 每步完成后落检查点；预算超限/异常时保存终态并终止。block() 仅在 boundedElastic 上出现（[教程 08-架构师进阶/08-响应式错误处理 §6]）。
 */
@Service
...
### 4.8 `application-aiops.yaml` 追加 Redis
...
### 4.9 本节测试与验证（断点恢复、幂等与预算终止）

**前置条件**：Redis 可连（6379）；`K8sReadClient` 已给出最小实现（证书/配额返回固定数据、异常 Pod 可控返回）；已按 §4.7 添加依赖。

**材料 A——检查点核对命令**（redis-cli）：

```bash
redis-cli KEYS 'aiops:inspection:checkpoint:*'
redis-cli TTL aiops:inspection:checkpoint:<taskId>
redis-cli GET aiops:inspection:checkpoint:<taskId> | head -c 400
```

**材料 B——断点注入方法**：在 `executeStep` 的 "quotas" 分支临时加 `if (System.getenv("CRASH_AT") != null) System.exit(1);`（仅测试副本），或用 `kill -9` 在步骤间杀进程。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 直接调 `engine.execute("t1", InspectionRequest.full("c1"))`（暴露一个临时触发端点或测试类） | 返回 status=completed，completedSteps 为 3 步且摘要非空；材料 A 查不到 t1 的 key（完成即 delete） |
| 2 | 材料 B 崩溃注入后重跑同 taskId | 日志/结果只补跑剩余步骤；已完成步骤不重复执行（幂等键命中跳过） |
| 3 | 恢复一致性 | 步骤 2 最终 completedSteps 与步骤 1 一次性跑完的结果一致（无重复条目） |
| 4 | 预算终止：临时把 `MAX_TURNS` 改为 0（或构造 turnsUsed≥上限的检查点）重跑 | status=budget_exceeded，异常消息含"轮次超限"，且检查点终态被保存 |
| 5 | 检查点 TTL：步骤 2 崩溃后立即查 TTL | TTL > 0（7 天窗口内可恢复） |
| 6 | 定时触发：手动调 `dailyInspection()`（或等 cron） | 三个 cluster 各产生一次执行日志；同日重跑同 taskId 不重头 |

**失败排查**：①步骤 2 重跑从 certificates 开始→检查点未保存（确认每步 `store.save(...).block()` 执行）或 taskId 不一致；②步骤 1 结束 key 未删→`delete` 未生效；③步骤 4 直接抛出未捕获→`BudgetExceededException` 在 try 外被调用（确认 `checkBudget` 在 try 内）；④Redis 读写报错→用了阻塞 RedisTemplate 或连接配置错（WebFlux 铁律：ReactiveRedisTemplate）。

## 5. 全篇回归验证

> 各节材料与断言已上移至 §4.9；本表为整篇迭代的回归验收，不重复材料。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 崩溃恢复 | 中途 kill 后从断点恢复、不重跑已完成步骤 | ✅ §4.9 断言 2/3（材料 B 崩溃注入 + 幂等键命中跳过） |
| 定时触发 | 每日巡检按时启动，结果入知识库 | ✅ §4.9 断言 6（§4.6 InspectionScheduler cron） |
| 预算控制 | Token/轮次/时长超限自动终止 | ✅ §4.9 断言 4（MAX_TURNS=0 → budget_exceeded） |
| 幂等 | 重试不产生重复处置动作 | ✅ §4.9 断言 2（幂等键命中跳过）+ §4.6 同日 taskId 复用 |
| 恢复一致 | 恢复后与未崩溃跑完一致 | ✅ §4.9 断言 3（completedSteps 无重复条目） |

## 6. 本迭代的 ADR

| # | 决策 | 理由 |
|---|------|------|
| ADR-415 | 巡检用 Checkpoint 机制（[教程 08-架构师进阶/06-长任务持久化与中断恢复]）持久化断点；跨主机 durable execution（Temporal）记为生产演进方向 | checkpoint 可手写、零外部依赖，覆盖"进程重启续跑"的核心需求；跨进程自动恢复是更大工程，按需引入 |
...
- [ ] ADR-415 落点 = §4.2/§4.5 检查点存储与引擎；ADR-416 落点 = §4.4 确定性扫描 + LLM 只读结论；ADR-417 落点 = §4.6 幂等 taskId。
- [ ] 三条 ADR 与 [13-ADR架构决策记录 §3] 总账一致。

## 7. v5 的痛点（驱动下一迭代）

巡检自动跑了，但**知识没有沉淀**：每次故障的复盘（postmortem）结论、每次处置的 runbook、每次巡检的异常模式——都散落在各自系统里，下次类似故障 RCA 还是要从头推理。**需要"复盘→runbook→RAG"的知识飞轮**。→ [06-运维知识飞轮.md](06-运维知识飞轮.md)

### 7.1 本节核对（痛点承接）

- [ ] "复盘→runbook→RAG"由 [06 §2] 知识飞轮闭环承接。
