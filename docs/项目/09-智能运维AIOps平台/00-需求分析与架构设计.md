...
> **定位**：某中大型互联网公司 IT 运维团队的 AIOps 平台——从告警风暴中解放值班工程师，用 Agent 完成告警降噪、根因分析、自动处置（HITL）、长任务巡检与运维知识飞轮。项目 09 是**完整独立**的项目，不依赖也不引用其他项目。**本项目的核心使命：把教程 02-SpringAI核心机制/04-结构化输出（可观测）、30（容错）、40（长任务）、28（HITL）在"运维"这个 ROI 最高的企业 AI 场景落地。**
>
...
| ADR-404 | 长任务用 Checkpoint 机制（[教程 08-架构师进阶/06-长任务持久化与中断恢复]）持久化断点 | 进程一死 run 就死；巡检需断点续跑、预算控制；零外部依赖可手写；跨主机 durable execution（如 Temporal）记为生产演进方向 |
...
基线：Spring Boot 4.1 + Spring AI 2.0 + WebFlux + Java 21。数据接入用 Prometheus HTTP API / ELK / OTel；事件流用 Kafka（spring-kafka 4.x 无响应式模板，走 KafkaTemplate + boundedElastic 桥接，见 01 迭代）；长任务用 Checkpoint（[教程 08-架构师进阶/06-长任务持久化与中断恢复]，v5 起）。各迭代引入的新依赖按规范标注「需在 pom.xml 中添加依赖」。
