...
> **定位**：某中大型互联网公司 IT 运维团队的 AIOps 平台——从告警风暴中解放值班工程师，用 Agent 完成告警降噪、根因分析、自动处置（HITL）、长任务巡检与运维知识飞轮。项目 09 是**完整独立**的项目，不依赖也不引用其他项目。**本项目的核心使命：把教程 22（可观测）、30（容错）、40（长任务）、28（HITL）在"运维"这个 ROI 最高的企业 AI 场景落地。**
>
> 「遇到阻塞？→ [教程 31-全链路可观测性]、[教程 63-容错与弹性设计 §熔断/降级]、[教程 83-长任务持久化与中断恢复]、[教程 61-Human-in-the-Loop与审批流]、[教程 84-数据飞轮与持续改进]」
>
...
| ADR-404 | 长任务用 Checkpoint 机制（教程 40）持久化断点 | 进程一死 run 就死；巡检需断点续跑、预算控制；零外部依赖可手写；跨主机 durable execution（如 Temporal）记为生产演进方向 |
...
基线：Spring Boot 4.1 + Spring AI 2.0 + WebFlux + Java 21。数据接入用 Prometheus HTTP API / ELK / OTel；事件流用 Kafka（spring-kafka 4.x 无响应式模板，走 KafkaTemplate + boundedElastic 桥接，见 01 迭代）；长任务用 Checkpoint（教程 40 机制，v5 起）。各迭代引入的新依赖按规范标注「需在 pom.xml 中添加依赖」。
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 告警接入 | [教程 85-响应式错误处理 §事件流]、[教程 10-WebFlux从零入门] |
| 02 降噪 | [教程 81-Agent性能优化 §级联路由]、[教程 31-全链路可观测性 §标签设计] |
| 03 RCA | [教程 31-全链路可观测性 全篇]、[教程 78-高级RAG与AgenticRAG §Agentic 检索] |
| 04 HITL | [教程 61-Human-in-the-Loop与审批流 全篇]、[教程 63-容错与弹性设计 §熔断]、[附录 05-SpringAI2-API基准/02 §1.3] |
| 05 长任务 | [教程 83-长任务持久化与中断恢复 全篇]、[教程 63-容错与弹性设计 §重试] |
| 06 知识飞轮 | [教程 84-数据飞轮与持续改进 全篇]、[教程 05-RAG] |
| 07 容量 | [教程 87-多模型协作与供应策略 §模型编排]（Ollama/vLLM 参考） |
| 08 高可用 | [教程 63-容错与弹性设计 §熔断降级]、[教程 66-部署与运维 §演练] |
| 10 拓扑图谱 | [教程 78-高级RAG与AgenticRAG §GraphRAG]、[附录 10-知识图谱工程 全部] |
| 11 预测弹性 | [教程 63-容错与弹性设计 §弹性设计]、[教程 87-多模型协作与供应策略 §模型编排] |
| 12 自愈闭环 | [教程 61-Human-in-the-Loop与审批流 全篇]、[教程 63-容错与弹性设计 §熔断] |
| 13 ADR 资产 | [附录 07-架构决策方法论 全篇]、[教程 29-管控分离架构 §决策与治理] |
| 14 前沿 | [教程 80-自我反思与Agent评估]、[教程 86-Agent治理与合规框架]、[教程 82-高级记忆架构 §记忆演化与衰减] |
