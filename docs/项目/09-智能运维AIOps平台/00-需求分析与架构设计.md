...
>
> 「遇到阻塞？→ [教程 16-全链路可观测性]、[教程 24-容错与弹性设计 §熔断/降级]、[教程 35-长任务持久化与中断恢复]、[教程 22-Human-in-the-Loop与审批流]、[教程 36-数据飞轮与持续改进]」
>
> **代码使用纪律（照抄前必读）**：本项目所有代码遵循「项目文档不生成 .java 文件、代码由用户手写」的规范。框架 API 全部按 [附录 12-SpringAI2-API基准] 的真实签名书写（`ChatClient`/`@Tool`/`CallAdvisor`/`ToolCallingManager` 等），**可照抄编译**；`AlertNormalizer`、`RcaOrchestrator`、`RemediationApprovalManager` 等为本项目专属业务类，本文给出**完整类**（含全部 import、无省略），你按文档手写即可。凡引入 pom.xml 未声明的新依赖，均标注「需在 pom.xml 中添加依赖」并给出完整 `<dependency>` 片段。
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 告警接入 | [教程 37-响应式错误处理 §事件流]、[附录 01-WebFlux与响应式编程] |
| 02 降噪 | [教程 33-Agent性能优化 §级联路由]、[教程 16 §标签设计] |
| 03 RCA | [教程 16 全篇]、[教程 30-高级RAG与AgenticRAG §Agentic 检索] |
