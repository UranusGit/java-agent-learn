# 项目 09：智能运维 AIOps 平台 — 08-迭代七：高可用与演练（收官）

> **定位**：收官迭代——解决"运维 Agent 平台自身成为新单点"的悖论：服务拆分、熔断降级、混沌演练制度化。教程 24 容错 + 教程 28 部署的落地。本文代码为**完整可手写**（含全部 import、无省略）。
>
> 「遇到阻塞？→ [教程 24-容错与弹性设计 全篇]、[教程 28-部署与运维 §演练]」
...
**降级哲学**：与项目 08 一致——**告警是生命线，绝不丢**；LLM 是可降级环节；审计/审批是底线（fail-close）。
...
/**
 * 每个降噪/检测组件独立熔断器——一个组件故障不拖垮全链（[教程 24 §熔断]）。
...
> **演练说明**：测试里的 `feedAlerts`/`alertStoreCount` 等桩方法，生产化时替换为真实调用（接入层 + 审计库）；跨实例演练用运维脚本 kill Pod 触发，配合 Grafana 验证降级矩阵。剧本 A-D 全过才准归档季度演练报告。
...
八个迭代走完 AIOps 完整弧线：**接入 → 降噪 → 定位 → 处置 → 巡检 → 学习 → 预测 → 自保**。23 条 ADR 中最有复用价值的三个：ADR-407（告警是生命线绝不丢）、ADR-413（渐进自主停在审批后执行）、ADR-415（巡检用 Checkpoint 断点续跑）。
