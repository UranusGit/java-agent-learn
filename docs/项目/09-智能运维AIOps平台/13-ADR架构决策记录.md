...
> **定位**：全项目决策资产篇——33 条 ADR（ADR-401~433）的总账、决策关系与演化时间线；对 11 条关键决策做"上下文 / 备选方案 / 取舍 / 可回滚性"四要素详解。读完本篇应能回答：这个平台**每个关键设计当初为什么这么选、放弃了什么、还能不能改回去**。方法论锚点：[附录 07-架构决策方法论 全篇]。
>
> 「遇到阻塞？→ [附录 07-架构决策方法论 全篇]、[教程 04-企业级架构主干/00-管控分离架构 §决策与治理]、[教程 08-架构师进阶/09-Agent治理与合规框架 §行为审计]」
...
- [ ] 能复述本篇三条交付：33 条 ADR 总账 / 决策关系网（三主线）/ 关键决策四要素详解（≥ 8 条）。
- [ ] 能说出"决策资产化"对平台成熟度的意义：改架构先查 ADR 而非翻代码猜意图。
- [ ] 能指出三个痛点与本篇章节的对应（散落→§3 总账；依赖隐性→§4 关系图；预录细化→§3 状态标"被 X 细化"）。
...
ADR 自身的状态迁移（与 [附录 07-架构决策方法论 §ADR 生命周期] 对齐）：
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 生命周期五阶段与状态机一致 | 预录→迭代→细化→superseded 的状态迁移与 §2 stateDiagram 对齐 |
| 2 | 能区分"细化"与"推翻" | 细化 = 迭代 ADR 展开预录号（401→406 等）；推翻 = superseded 回引旧号（本项目暂无） |
| 3 | 编号纪律 | ADR-401~405 预录 / 406 起迭代，不重不跳；601 与 406 的相近是细化关系不是错误 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 总数准确 | 预录 5（401-405）+ 第一轮 18（406-423）+ 第二轮 10（424-433）= **33 条**，逐一存在且状态标"有效/被细化" |
| 2 | 编号连续 | 401~433 无跳号、无重号（可对照各迭代篇末 ADR 表逐条复核） |
| 3 | 预录细化关系正确 | 401→406/407、402→409/410、403→412/413、404→415、405→418/419 与对应迭代篇一致 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 时间线无遗漏 | timeline 覆盖 预录（401-405）→ 第一轮两段（406-411 / 412-417 / 418-423）→ 第二轮三段（424-426 / 427-429 / 430-433） |
| 2 | 三主线成员归属正确 | 安全线 5 条 / 成本线 6 条 / 知识线 5 条，能背出并说明归类理由 |
| 3 | 联动评审可直接运用 | 动安全线任一条，能立刻说出需连带评审的其余四条（如放开 413 → 需回答 431/432） |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 抽查任意一条（随机挑 3 条） | 每条都能不看原文复述：上下文（防什么事故）/ 备选（弃用了什么）/ 取舍（代价）/ 可回滚（改回去的触发条件） |
| 2 | 可回滚性标注与代码现状一致 | 例如 ADR-415"可升级到 durable execution" 与 [14 §7] 判据表联动；ADR-431 自动熔断与 §3.5 KillSwitch 一致 |
| 3 | 四要素写满 | 11 条详解均已覆盖 上下文/备选/取舍/可回滚 四要素，无缺项 |
...
- [ ] 五个模式（确定性优先 / 生命线 fail-open 底线 fail-close / 反馈闭环 / 渐进自主 / 知识可信性）各能举出成员编号并一句话提炼。
- [ ] 能复述"决策主题迁移 = 平台成熟度刻度"：第一轮偏安全/成本，第二轮偏闭环/校准。
...
- [ ] 能按规范处理一个虚拟场景：新迭代要改 ADR-413（放开全自主）——知道要新增 ADR（434 顺延）、联动评估安全线其余四条（431/432/428/407）、完整写四要素。
- [ ] 能说出 superseded 的正确动作（不删除不重号、新号回引 `supersedes ADR-4XX`、旧条目标"已被 X 取代"）。
...
- [ ] 能仅凭 §5 四要素向同事讲清任意一条 ADR"防什么事故/代价/何时可回改"。
- [ ] 能说出 33 条 ADR 作为"决策免疫系统"的三种价值（回答为什么 / 重构连带评审 / superseded 留痕）。
