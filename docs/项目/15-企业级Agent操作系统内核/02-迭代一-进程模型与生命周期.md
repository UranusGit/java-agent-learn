# 02-迭代一：进程模型与生命周期——PID/状态机/挂起恢复/父子进程
...
```bash
# 1. 长任务进程跑到第 3 步挂起→快照落盘→内核重启→恢复从第 3 步续跑
# 2. 父 spawn 两子→wait 分别收 0/1 退出码
# 3. 父先退→子被 init 收养→/proc 无泄漏
```
...
**下一篇**：[03-迭代二-系统调用与能力模型](03-迭代二-系统调用与能力模型.md)。
