...
> 过程可见性是工业级标配。在 OS 内核视角，**Agent 进程的执行过程就是它的 syscall 序列**——把 [SyscallGateway.dispatch（三节）](03-迭代二-系统调用与能力模型.md) 的审计记录升级为**用户可见的过程事件流**，用户能实时看到"进程正卡在 INVOKE_MODEL→ 已返回 Ok → 下一步 CALL_TOOL"。
