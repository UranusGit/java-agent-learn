...
> **定位**：进阶进程模型：**进程组**（一组相关进程统一治理——组级信号/组级配额/组级退出语义）、**fork-join 编排**（并行子任务 fork 出去、join 收敛结果——多 Agent 并行的内核原生形态）、**守护进程**（daemon 化常驻服务 Agent——systemd 式开机自启/崩溃拉起/依赖顺序）。读者画像：要内核原生跑多 Agent 并行与常驻服务的读者。前置阅读：[07-IPC与命名空间](07-IPC与命名空间.md)、[教程 79-Agent工作流编排]。
>
