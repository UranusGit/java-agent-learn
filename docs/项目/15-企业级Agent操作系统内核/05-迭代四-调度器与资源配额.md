# 05-迭代四：调度器与资源配额——优先级 / Token cgroup / 抢占

> **定位**：建立**调度器与资源配额（cgroup 式）**：进程优先级（实时/交互/批处理三档）、**Token/并发/记忆配额组**（按租户/业务线成组限账）、**抢占**（高优先级进程可抢占低优先级的模型调用排队位）。读者画像：要让"关键 Agent 永不被批处理挤死"的读者。前置阅读：[04-迭代三-记忆文件系统](04-迭代三-记忆文件系统.md)、[教程 26-多租户隔离与资源治理]、[教程 38-Agent性能优化]。
>
...
```bash
# 1. 抢占：BATCH 打满并发池 → 提交 RT 任务 → ≤100ms 开始调用；被让位的 BATCH 排队恢复
# 2. 配额：cgroup 日 Token 用尽 → INVOKE_MODEL 返回 ENOMEM → 进程 exit(3)
# 3. 公平：batch 组提交 10x 负载 → 交互组 P99 波动 ≤10%
```
...
**下一篇**：[06-迭代五-中断与信号系统](06-迭代五-中断与信号系统.md)。
