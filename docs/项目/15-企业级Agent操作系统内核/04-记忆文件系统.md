...
> **定位**：建立**记忆文件系统（memory-fs）**：把 Agent 记忆组织为**分层挂载的文件系统**——会话记忆=进程私有 tmpfs、长期记忆=持久磁盘、共享知识=只读挂载；**路径语义**（`/mem/{scope}/{key}`）统一 READ/WRITE_MEM syscall；**页缓存与驱逐**（热记忆缓存 LRU）。读者画像：要让记忆"有结构、可共享、可治理"的读者。前置阅读：[03-系统调用与能力模型](03-系统调用与能力模型.md)、[教程 82-高级记忆架构]。
>
