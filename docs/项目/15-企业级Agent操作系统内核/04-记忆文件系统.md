...
> **定位**：建立**记忆文件系统（memory-fs）**：把 Agent 记忆组织为**分层挂载的文件系统**——会话记忆=进程私有 tmpfs、长期记忆=持久磁盘、共享知识=只读挂载；**路径语义**（`/mem/{scope}/{key}`）统一 READ/WRITE_MEM syscall；**页缓存与驱逐**（热记忆缓存 LRU）。读者画像：要让记忆"有结构、可共享、可治理"的读者。前置阅读：[03-系统调用与能力模型](03-系统调用与能力模型.md)、[教程 08-架构师进阶/05-高级记忆架构]。
>
...
**最小可执行载体**（载体同 [01 §4.3](01-最小内核Demo搭建.md)：JUnit + `mvn test`；本篇剧本替换为"三层路径解析 + malformed 校验"，纯 Java 直测 `MemPath`）：

```java
@Test
void 三层路径解析正确_malformed抛异常() {
    assertThat(MemPath.parse("/mem/session/10/history"))
            .isEqualTo(new MemPath("session", "10", "history"));           // 断言①：session 层路由
    assertThat(MemPath.parse("/mem/longterm/assistant/facts"))
            .isEqualTo(new MemPath("longterm", "assistant", "facts"));     // longterm 层
    assertThat(MemPath.parse("/mem/shared/t1/kb/faq"))
            .isEqualTo(new MemPath("shared", "t1", "kb/faq"));             // shared 层（split 基数 5，key 可含斜杠）
    assertThatThrownBy(() -> MemPath.parse("/tmp/x"))                      // 断言③：malformed
            .isInstanceOf(IllegalArgumentException.class);
}
// mvn test -Dtest=MemPathTest → Tests run: 1, Failures: 0, Errors: 0
```
...
**最小可执行载体**（载体同 §三.1；本节剧本替换为"LRU 驱逐语义"，用 `LinkedHashMap(accessOrder=true)` 表达——容量 2 为材料B 缩小版）：

```java
@Test
void LRU驱逐_热key保留_尾出队() {
    var cache = new LinkedHashMap<String, String>(16, 0.75f, true) {   // accessOrder=true：读也计新鲜度
        @Override protected boolean removeEldestEntry(Map.Entry<String, String> e) {
            return size() > 2;                            // 页缓存容量 2
        }
    };
    cache.put("k1", "v1"); cache.put("k2", "v2");
    cache.get("k1");                                      // 热读：k1 刷新为最新
    cache.put("k3", "v3");                                // 触发驱逐：LRU 尾=k2
    assertThat(cache).containsKey("k1").doesNotContainKey("k2");   // 断言②：热 key 保留、尾出队
}
// mvn test -Dtest=MemCacheTest → Tests run: 1, Failures: 0, Errors: 0
```
...
**最小可执行载体**（载体同 §三.1；本节剧本替换为"WRITE_MEM 两道判定语义"——EROFS 先于 ENOSPC，与 §五 注释代码一一对应）：

```java
@Test
void 只读层EROFS_超配额ENOSPC() {
    record WriteCheck(boolean readOnlyLayer, int quotaBytes, int incomingBytes, int usedBytes) {
        String verdict() {                                // WRITE_MEM 服务端判定序
            if (readOnlyLayer) return "EROFS";            // ① shared 只读层先拒
            return usedBytes + incomingBytes > quotaBytes ? "ENOSPC" : "OK";   // ② 配额
        }
    }
    assertThat(new WriteCheck(true, 100, 10, 0).verdict()).isEqualTo("EROFS");     // 断言②：shared 写拒绝
    assertThat(new WriteCheck(false, 100, 10, 95).verdict()).isEqualTo("ENOSPC");  // 断言①：超配额拒
    assertThat(new WriteCheck(false, 100, 10, 50).verdict()).isEqualTo("OK");
}
// mvn test -Dtest=MemQuotaTest → Tests run: 1, Failures: 0, Errors: 0
```
