...
## 六、验证包（手工测试与验证）
**前置条件**：memfs 三层（kernel/shared/session）+LRU 缓存+配额+只读实现。

**材料 A——越权剧本**：进程 scope 只含 session 层，尝试写 shared；**材料 B——容量压测**：写入超缓存容量的 key；**材料 C——配额剧本**：写入超配额数据。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 三层路径各读写一次 | 正确路由；材料A 越权 EPERM |
| 2 | 重复读同 key | P99 ≤20ms（缓存命中）；材料B LRU 驱逐后回源正确（驱逐项重读数据一致） |
| 3 | 进程挂起→恢复 | session 层随快照持久化；恢复后 get(conversationId) 内容完整 |
| 4 | 材料C 超配额 | ENOSPC；shared 层写 → EROFS |

**失败排查**：②驱逐丢数据→脏页未回写就驱逐；③恢复缺数据→session 层不在快照范围（只存了进程态）；④EROFS 未拦→只读在客户端约束（应服务端强制）。
