# 06-迭代五：中断与信号系统——信号表 / HITL 挂起 / 取消 / kill

> **定位**：建立**中断与信号系统**：**信号表**（SIG_APPROVE 审批结果/SIG_CANCEL 取消/SIG_YIELD 让出/SIG_KILL 强杀/SIG_TIMER 超时）、**异步投递与同步点**（编排循环的 syscall 边界检查信号——对应 OS 的中断返回检查）、**kill 语义**（强杀不等于撕毁模型调用——已计费调用自然结束、进程状态终结）。读者画像：要让外部世界能"打断"Agent 的读者。前置阅读：[05-迭代四-调度器与资源配额](05-迭代四-调度器与资源配额.md)、[教程 28-Human-in-the-Loop与审批流]。
>
...
```bash
# 1. 高危工具 → WOULD_BLOCK 挂起（线程/配额零占用）→ 批准唤醒续跑；驳回走拒绝分支
# 2. SIG_CANCEL：跑到第 N 步收信号 → exit(2)+快照 → 可恢复
# 3. SIG_KILL：立即 EXITED；在途调用完成后无僵尸（/proc 无泄漏）
# 4. 信号观测：审批等待 P50/P99、取消原因分布可查
```
...
**下一篇**：[07-迭代六-IPC与命名空间](07-迭代六-IPC与命名空间.md)。
