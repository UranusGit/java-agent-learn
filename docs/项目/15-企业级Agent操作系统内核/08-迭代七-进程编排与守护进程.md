...
```bash
# 1. fork-join：3 子并行（含 1 失败）→ failFast 组内其余收到 SIG_CANCEL，父收 failedFast
# 2. 组信号：SIG_CANCEL(pgid) 一次取消整组（/proc 组内全 EXITED(2)）
# 3. daemon：kill 常驻进程 → ≤5s 拉起；连续杀 5 次 → STOPPED+告警（不再拉）
# 4. 组配额：3 子进程 Token 消耗 → 组账本聚合正确，超组限联动限制
```
