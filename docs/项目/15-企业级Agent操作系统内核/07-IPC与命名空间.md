...
> **定位**：完成内核六大机制的最后一块：**IPC**（进程间通信——管道 pipe/共享黑板 blackboard/信箱 mailbox 三形态）与**命名空间**（ns——租户视图隔离：同租户进程互相可见、跨租户不可见；配额组与 ns 绑定）。读者画像：要多 Agent 协作又要硬隔离的读者。前置阅读：[06-中断与信号系统](06-中断与信号系统.md)、[教程 09-多Agent协作]、[教程 26-多租户隔离与资源治理]。
>
...
## 五、验证包（手工测试与验证）
**前置条件**：pipe（有界缓冲）+信箱+黑板订阅+租户 ns 隔离实现。

**材料 A——跨租户剧本**；**材料 B——端到端剧本**：主管+两 worker（黑板+信箱+wait 组合）。
...
| 2 | 材料A：租户A 进程 SEND_MSG 到租户B | EPERM；跨 ns 网关注册后可通且审计留痕 |
| 3 | 黑板 put | 订阅者 ≤50ms 收到通知并唤醒（唤醒=自动开步） |
| 4 | 材料B 全流程 | 协作任务完整跑通（结果聚合正确） |

**失败排查**：①无背压→缓冲无界（OOM 风险）；③唤醒慢→通知走轮询（应事件推送）；④死锁→wait 与信箱顺序反了（先收结果再 wait）。
