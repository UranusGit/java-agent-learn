...
> **定位**：完成内核六大机制的最后一块：**IPC**（进程间通信——管道 pipe/共享黑板 blackboard/信箱 mailbox 三形态）与**命名空间**（ns——租户视图隔离：同租户进程互相可见、跨租户不可见；配额组与 ns 绑定）。读者画像：要多 Agent 协作又要硬隔离的读者。前置阅读：[06-中断与信号系统](06-中断与信号系统.md)、[教程 09-多Agent协作]、[教程 59-多租户隔离与资源治理]。
>
