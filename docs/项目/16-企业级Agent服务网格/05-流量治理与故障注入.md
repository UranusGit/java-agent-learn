...
本迭代治理都来自**同一份 `MeshPolicy` YAML（一~四节）**，Sidecar 本地执行。下面给出一段**可编译的完整实现**——`GovernanceEventListener` 挂在 Sidecar 治理执行器上，把每次策略判定的结果经 Reactor Sinks 流式发出（Sinks 类型对齐 [教程 05-Observation可观测/02-组件交互：Registry、Handler、Convention、Filter协作/附录 18] 已实证基准）：
