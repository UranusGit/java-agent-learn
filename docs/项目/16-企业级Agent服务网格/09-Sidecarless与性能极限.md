...
> **定位**：把网格推到**性能极限**：**Sidecarless 评估与落地**（eBPF/内核层旁路——延迟 <1ms 的极端场景；能力对比与降级取舍）、**混合模式**（同网格 Sidecar 与 Sidecarless 共存——按 Agent 分级选用）、**延迟治理**（全链路延迟预算表：劫持/检查/策略/mTLS 各环节预算与测量）。读者画像：有毫秒级敏感 Agent（实时风控/语音）的读者。前置阅读：[08-网格安全深化与语义策略](08-网格安全深化与语义策略.md)、[教程 81-Agent性能优化]。
>
