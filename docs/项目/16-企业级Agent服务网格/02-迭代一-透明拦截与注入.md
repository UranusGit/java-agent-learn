...
```bash
# 1. 打标签 → 滚动重启 → Pod 描述可见 sidecar/init 容器；Agent 零改动被接管
# 2. 启动顺序：Sidecar 故意延迟 10s 就绪 → Agent 等待后才发首请求（无逃逸）
# 3. 逃逸：尝试从 Agent 容器 curl 上游 → 被 iptables 拦（或上游 mTLS 拒绝）
# 4. 规模化：50 个存量 Deployment 滚动注入，服务零报错
```
...
**下一篇**：[03-迭代二-控制面与xDS动态下发](03-迭代二-控制面与xDS动态下发.md)。
