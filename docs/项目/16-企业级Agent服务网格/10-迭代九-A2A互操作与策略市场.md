...
> **定位**：打通网格**对外生态**：**A2A 互操作**（SVID↔Agent Card 映射、跨组织 Agent 委托——网格身份体系对接 A2A 协议）、**策略市场**（治理策略资产化——特征库/限流模板/语义规则包在组织内共享与订阅）。读者画像：要跨团队/跨组织 Agent 协作与规模化治理复用的读者。前置阅读：[09-迭代八-Sidecarless与性能极限](09-迭代八-Sidecarless与性能极限.md)、[前沿 00-A2A协议]。
>
...
```bash
# 1. Card 闭环：发布→外组织发现→SVID 验签通过
# 2. 委托：scope 内能力可调、全程双向审计；越权尝试 EPERM
# 3. 市场：订阅 llm-security-baseline → v2.4 发布 → 自动同步生效；撤销推送回退
```
