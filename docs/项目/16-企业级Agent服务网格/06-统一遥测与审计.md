...
> **定位**：建立**统一遥测**：Sidecar 在拦截点**自动生成**三支柱（指标/日志/Trace——OTel 标准，Agent 零埋点）+ **Token 计量**（出站响应解析 usage → 按身份归集，成本治理数据源）+ **出站 PII 清洗**（遥测落库前脱敏——可观测不能变成泄露面）。读者画像：要"接入即全可观测"的读者。前置阅读：[05-流量治理与故障注入](05-流量治理与故障注入.md)、[教程 31-全链路可观测性]、[教程 60-成本治理与Token计量]。
>
