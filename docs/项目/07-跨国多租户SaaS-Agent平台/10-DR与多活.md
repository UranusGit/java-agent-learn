# 项目 07：跨国多租户 SaaS Agent 平台 — 10-DR 与多活（基础篇收束）

> **定位**：基础十迭代收束——区域内容灾与跨区容灾的两级体系：数据主权约束下的 DR 设计、RTO/RPO 承诺、多活演进与容灾演练制度化。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/13-部署与运维 §DR 与多活]、[教程 04-企业级架构主干/10-容错与弹性设计]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
### 3.1 本节核对（两级体系与 RTO/RPO）

- [ ] 一级（AZ 同步 RPO=0）/二级（同法域异步 RPO<5min）的分工与触发条件能复述
- [ ] ADR-230 的"同法域 DR"约束下，EU 备区选址（法兰克福/阿姆斯特丹）与极端降级路径能说出
- [ ] 四级故障的 RTO/RPO 表与 §5 验收项 1/2 的数字一致
...
#### 4.4.1 本节测试与验证（fencing 与切换协调）

**前置条件**：`FencingService` 已接 Redis；`DnsSwitch` 有测试 stub（记录调用顺序）；写入口已挂 `canWrite` 校验。

**材料——fencing 令牌核对**：

```bash
redis-cli GET fencing:write-token:eu-primary
```
...
|---|------|------------------|
| 1 | 正常态材料 | 返回 `GRANTED` |
| 2 | `revokeWriteToken("eu-primary")` 后材料 | 返回 `REVOKED`；写入口请求被拒（fail-closed） |
| 3 | 删除该 key 模拟 Redis 故障/漂移 | `canWrite` 默认拒绝（defaultIfEmpty REVOKED，防脑裂双写） |
| 4 | 调 `failoverCoordinator.failover("eu-primary","eu-backup")` | stub 记录的执行顺序 = 撤销旧主令牌 → 授予新主令牌 → DNS 切换（fencing 优先铁律） |
| 5 | FailoverResult | durationMs 有值；两 region 字段正确 |

**失败排查**：①顺序错（先切 DNS）→`failover` 链拼反；②key 缺失时仍可写→漏了 `defaultIfEmpty("REVOKED")`；③新主拿不到令牌→grant 的 region 名与写入口校验的 region 名不一致。

#### 4.4.2 本节核对（fencing 与切换协调）

- [ ] fencing 优先的执行顺序（撤销旧主令牌 → 授予新主令牌 → DNS 切换）能复述，并说明为什么不能先切 DNS（防双主）
- [ ] `canWrite` 的 fail-closed（defaultIfEmpty REVOKED）在「Redis 故障/漂移」场景下的行为能说清
- [ ] 演练频率表（月 AZ/季桌面/年区域）与 §7 的 Runbook 要求一致
...
> 本表即全篇回归口径：§4.4.1（fencing/切换协调）与 §4.5（演练制度化）通过后逐条对照本表复核（项 1/2/5 需对应频率的演练窗口：月 AZ / 年区域 / 一季度）。
