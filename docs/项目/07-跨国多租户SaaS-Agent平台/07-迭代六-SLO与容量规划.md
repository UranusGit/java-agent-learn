# 项目 07：跨国多租户 SaaS Agent 平台 — 07-迭代六：SLO 与容量规划

> **定位**：把"感觉稳定"变成"承诺可兑现"——SLO 定义与错误预算、面向 SSE 长连接的容量压测、扩容模型。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 33-部署与运维 §容量规划]、[教程 30-容错与弹性设计]、[教程 38-Agent性能优化]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
export default function () {
  ...
  const res = http.post(
    ...
    payload,
    {
      ...
      timeout: '120s',                        // SSE 整个流时长，不是单请求（教程 28 §SSE 压测的坑）
    }
  );
...
> 「遇到阻塞？→ [教程 28 §压测]——SSE 压测的两个坑：客户端超时要按"整个流时长"设置（不是单请求）；必须断言心跳/首帧到达（否则挂在代理缓冲上压了个寂寞，[教程 09 §8 代理配置]）」
...
**供应商 TPM 是水平扩不掉的天花板**（ADR-222）——应用层扩容到 20 实例，供应商 TPM 满了照样 429。供应商配额管理（多 Key 池、多供应商分摊，[教程 39 §API Key 池]）是容量规划的隐藏维度。
...
第一份 Enterprise 合同的法务批注回来："客户数据（含对话内容）不得离开欧盟。"当前平台是单区域部署——**数据驻留从"路线图项"变成"签约阻塞项"**。→ [08-迭代七-全球数据驻留.md](08-迭代七-全球数据驻留.md)
