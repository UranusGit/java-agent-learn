# 项目 07：跨国多租户 SaaS Agent 平台 — 03-租户配额与限流

> **定位**：分级配额体系——Token 日配额（Lua 原子预扣+修正）、租户级并发限流（突发保护）、超限行为分级（Free 限流 / Pro 降速 / Enterprise 宽限+通知）。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/06-多租户隔离与资源治理 §资源配额]、[教程 04-企业级架构主干/10-容错与弹性设计 §限流]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
- [ ] Lua 返回值三态（正数=预扣成功 / -1 超硬限 / -2 超软限）与图中三种分级行为能对应
- [ ] Free→拒、Pro→缓（60s）、Enterprise→宽限 20% 的分流依据（商业分级）能复述
- [ ] "预扣防边跑边超、修正保精度"的因果链能讲清（ADR-208）
...
### 3.2 `application-saaagent.yaml` 新增（Redis）
...
public class QuotaService {
    ...
    // 原子脚本：GET+判断+INCRBY 三步合一条 Lua，杜绝并发超卖（[教程 03-React前端与AgenticUI/02-React与SSE流式UI §5.1 Token 配额]）
...
#### 3.5.1 本节测试与验证（配额原子性与并发限量）

**前置条件**：Redis 可连（6379）；`QuotaService` / `ConcurrencyLimitService` 已装配；两租户 A（FREE，日 1000）/ B（PRO，日 100_000，并发 5）。

**材料——Redis 核对命令**：

```bash
# ① 预扣后查当日余量
redis-cli GET "quota:token:<tenantA>:$(date +%F)"
# ② 并发会话集合
redis-cli SMEMBERS "sessions:<tenantB>"
redis-cli TTL "sessions:<tenantB>"
# ③ 1000 并发 preCharge 压测后核对：quota:token:<tenantA>:<today> 恰好 = 1000（零超卖，未超）
redis-cli GET "quota:token:<tenantA>:$(date +%F)"
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 对 A 租户单次对话 | 材料① 出现 ≈ 预扣估计量的数值 |
| 2 | 对 A 租户连续对话至超 1000 | 返回 SOFT_EXCEEDED（-1 区间）；再冲到 1200 上限得 HARD_EXCEEDED（-2） |
| 3 | `settle` 修正 | 实际用量 < 预扣时余量回升（decrement 生效） |
| 4 | 1000 并发 `preCharge`（limit=1000, est=1） | 成功次数 ≤ 1000，零超卖（Lua 原子性） |
| 5 | B 租户开 6 个不同 sessionId 并发 | 第 6 个 `tryAcquireSession` 返回 false；同 sessionId 重复占用幂等返回 true |
| 6 | 材料② | SCARD ≤ 5；TTL ≤ 7200 且 > 0（EXPIRE 自愈） |

**失败排查**：①超卖→脚本被拆成 GET+INCRBY 两条非原子命令；②TTL=-1→SADD 分支后未刷 EXPIRE；③预扣不生效→key 日期部分与 `LocalDate.now()` 时区不一致。
...
#### 3.9.1 本节测试与验证（对话链路闸门与 429 语义）

**前置条件**：§3.5.1 通过；§3.7 的全局异常处理（429 映射）已生效；三档租户 token 各一枚（可在 DB 把租户 plan 改为 PRO/ENTERPRISE 后重新登录获取）。

**材料——超限探针 curl**：

```bash
# ① Free 租户打到当日硬限后再对话
curl -i -N -X POST "http://localhost:8081/conversations/<会话ID>/messages" \
  -H "Authorization: Bearer <Free租户JWT>" \
  -H "Content-Type: application/json" -d '{"message":"再来一次"}'
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料① | HTTP 429；响应头含 `Retry-After`（到午夜秒数）与升级引导语义（Free 分级文案） |
| 2 | Pro 租户超硬限（软限区内） | 429 + 60s 降速语义（RATE_SLOWED），次日自动恢复 |
| 3 | Enterprise 租户进入软限区 | 正常 200 放行（宽限 20%），且触发管理员通知 |
| 4 | 正常对话完成后 `redis-cli GET quota:token:...` | 数值 = 实际用量（修正后），非预扣估计量 |
| 5 | Pro 租户第 6 个并发会话 curl | 429 并发拒绝；关闭第 5 个后立即可用（releaseSession 生效） |
| 6 | 全链路无 `.block()`（生产代码 grep） | 配额链路全程响应式 |

**失败排查**：①429 无 Retry-After→异常映射分支漏设响应头；②Enterprise 也被硬拒→`allowedWithEnterpriseGrace` 判定未接进闸门；③修正后余量不回→`settle` 未在 concatWith 完成路径上调用。
...
> 本表即全篇回归口径：§3.5.1（原子性/修正/并发）与 §3.9.1（分级行为/429）全通过后，逐条对照本表复核（项 1 的 1000 并发与项 6 的 P99 需压测补齐）。
...
- [ ] ADR-208/209/210/234 四条决策分别能指向 §3.4/§3.6 分级/双层限量/§3.5 自愈的具体代码位置
- [ ] §6 痛点（Enterprise 要自选模型 + 高峰被挤）与 v4"租户级模型路由"形成因果衔接（§6 为收束章，不另设验证）

## 6. v3 的痛点

配额立住了，但 Enterprise 客户提出新诉求："我们要用自己偏好的模型/指定更便宜的模型控制成本"——当前所有租户共用一个模型路由。同时运营发现 Pro 与 Enterprise 共用模型时，Enterprise 客户的高峰体验被 Free 流量挤压。→ [04-租户级模型路由.md](04-租户级模型路由.md)
