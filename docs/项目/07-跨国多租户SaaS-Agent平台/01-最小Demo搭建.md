...
>
> 「遇到阻塞？→ [教程 01-Spring-AI框架入门]、[教程 02-ChatClient与对话模型]、[教程 10-SSE流式通信]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
第二家客户签约在即，v1 立刻暴露致命问题：**所有查询按 user_id 过滤、tenant_id 只是躺在表里**——试用期的第二家客户理论上可以通过会话 ID 探测到第一家的数据（越权读写）。SaaS 的信任前提是"租户互为不可信方"，隔离必须是**硬边界**而不是"代码自觉"。→ [02-迭代一-多租户数据隔离.md](02-迭代一-多租户数据隔离.md)
