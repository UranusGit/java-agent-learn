...
**材料——round-trip 探针（测试类骨架，概念代码）**：

```java
// AesGcmCipherRoundTripTest：加密→解密必须还原；篡改必须报错
byte[] dek = "0123456789abcdef0123456789abcdef".getBytes();   // 32 字节 AES-256
String cipherText = cipher.encrypt(dek, "tenant-secret-key-material");
assert cipher.decrypt(dek, cipherText).equals("tenant-secret-key-material");
```
