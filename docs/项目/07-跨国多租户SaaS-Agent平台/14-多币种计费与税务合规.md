...
>
> 「遇到阻塞？→ [教程 27-成本治理与Token计量 §计费正确性]、[教程 25-历史记录持久化与合规 §不可变记录]、[教程 49-Agent经济与支付集成]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
public record TaxRule(
        String countryCode,          // 'DE'/'FR'/... 或 'US-CA'（州级）
...
| # | 测试 | 方法 | 通过标准 |
|---|------|------|---------|
| 1 | 汇率锁定 | 同月账单在快照日后 1/7/30 天重算三次 | 金额逐分一致（快照日与锁定率持久化在账单上） |
| 2 | 税务判定矩阵 | 5 辖区 × B2B/B2C × 有/无税号 = 20 用例进 CI | 判定与 tax_rule 配置逐条一致；逆向征税发票带标准备注 |
| 3 | 三段式校验 | 1000 张随机发票构造 | `net+tax==gross` 构造校验零失败；发票模板含税号/税率/币种/快照率 |
| 4 | 红冲重放 | 争议-红冲-退款后重放财务月 | 净收款 = Σ发票 − Σ红冲 = 渠道流水（三方对账） |
| 5 | 冷启动爬坡 | 新租户注册后连打配额边界 | 首日第 21% 用量即 429；第 8 天全量；老租户不受影响 |
| 6 | 注册风控 | 批量脚本注册 50 个（同 IP 段+一次性邮箱） | ≥ 90% 进人工审核队列；误伤抽样 < 2% |
