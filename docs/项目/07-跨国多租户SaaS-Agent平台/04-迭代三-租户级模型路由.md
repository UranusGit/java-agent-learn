# 项目 07：跨国多租户 SaaS Agent 平台 — 04-迭代三：租户级模型路由与成本归因

> **定位**：模型路由从"平台一刀切"升级为"按租户级别与偏好路由"——Enterprise 专属高性能模型、Pro 标准模型、Free 廉价模型，且单租户成本可精确归因（为 v5 计费铺路）。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 27-模型路由与降级 全篇]、[教程 21-成本治理与Token计量 §归因]；API 真实性以 [附录 12-SpringAI2-API基准] 为准」
...
| **影响了哪些模块** | 新增 routing 模块（路由决策/健康/归因）；AgentChatService 在调用前解析路由并 `.model(...)` 注入；TenantRepository 加 model_preference 查询 |
...
### 3.1 路由策略配置（`application.yml`，控制面配置支持热更新）
...
/** 被动健康（教程 27 §被动健康为主）：由错误观测标记不可用模型，定期衰减。 */
@Component
...
产品上线正式收费（Pro 计划开放），财务提要求："月底怎么给每个租户出账单？拿什么保证账单数字是对的？"——目前只有 Prometheus 指标，指标是采样聚合、可重置的，**不能直接当账单数据源**。→ [05-迭代四-计费与账单.md](05-迭代四-计费与账单.md)
