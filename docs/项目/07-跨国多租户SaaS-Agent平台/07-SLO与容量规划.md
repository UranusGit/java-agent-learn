...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/13-部署与运维 §容量规划]、[教程 04-企业级架构主干/10-容错与弹性设计]、[教程 08-架构师进阶/04-Agent性能优化]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
export default function () {
  ...
  const res = http.post(
    ...
    payload,
    {
      ...
      timeout: '120s',                        // SSE 整个流时长，不是单请求（教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §10.1 代理和超时）
    }
  );
...
> 「遇到阻塞？→ [教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §10.1 代理和超时]——SSE 压测的两个坑：客户端超时要按"整个流时长"设置（不是单请求）；必须断言心跳/首帧到达（否则挂在代理缓冲上压了个寂寞）」
...
**供应商 TPM 是水平扩不掉的天花板**（ADR-222）——应用层扩容到 20 实例，供应商 TPM 满了照样 429。供应商配额管理（多 Key 池、多供应商分摊，[教程 05-Observation可观测/11-深度整合：SpringAI2与Observation的完整结合面 §4 多供应商冗余 / §API Key 池]）是容量规划的隐藏维度。
...
**失败排查**：①TTFT 全 0/无数据→响应被代理缓冲（教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §10.1 的坑，需关代理缓冲或换流式压测器）；②HTTP 大量失败→压测租户撞配额 429（先放宽配额）；③HPA 不动→自定义指标 adapter 未接 `active_sse_connections`。
