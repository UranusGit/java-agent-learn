...
**前置条件**：V4 迁移已执行（tenant.zone 列）；`ZONE=EU` 环境启动 EU 区实例；存在一个 zone=EU 的租户与一枚其 JWT（含 zone 声明）。

**材料——错区探针 curl**：

```bash
# ① 用 EU 租户的 JWT 打 US 区实例（ZONE=US 启动，端口 8081）
curl -i -X POST "http://localhost:8081/conversations/<会话ID>/messages" \
