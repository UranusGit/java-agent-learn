# 项目 07：跨国多租户 SaaS Agent 平台 — 08-全球数据驻留

> **定位**：EU/US 数据分区——租户数据按法域物理隔离、跨区协作的合规边界、欧盟供应商优先路由。GDPR 跨境传输要求的架构落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/13-部署与运维 §多区域]、[附录 12-AI治理与合规/02-数据隐私与偏见检测 §跨境传输]、[教程 08-架构师进阶/09-Agent治理与合规框架]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
- [ ] 内容数据四类（会话/记忆/向量/用量事件）能列举且都只落本区存储
- [ ] 三个全局服务（tenant 元数据/认证哈希/指标）各自"为什么可以无内容"能答出
- [ ] 跨区元数据的合规依据由法务出具、技术只保证"不含内容"的分工边界能复述
...
#### 3.5.1 本节测试与验证（zone 绑定与边缘兜底重定向）

**前置条件**：V4 迁移已执行（tenant.zone 列）；EU/US 两区实例已启动（见材料①）；存在一个 zone=EU 的租户与一枚其 JWT（含 zone 声明）。

**材料——两区启动命令与错区探针**：

```bash
# ① 两区实例启动（两段式配置 + 区域 profile；ZONE 环境变量注入 app.zone）
#    EU 区（基准端口 8081）：
mvn spring-boot:run -Dspring-boot.run.profiles=saaagent,eu
#    US 区（第二实例，端口 8082，ZONE=US）：
ZONE=US mvn spring-boot:run -Dspring-boot.run.profiles=saaagent,us --server.port=8082

# ② 用 EU 租户的 JWT 打 US 区实例（端口 8082）→ 307 重定向到 EU 区
curl -i -X POST "http://localhost:8082/conversations/<会话ID>/messages" \
  -H "Authorization: Bearer <EU租户JWT>" \
  -H "Content-Type: application/json" -d '{"message":"hi"}'
# ③ 同 JWT 打 EU 区实例（端口 8081）→ 200/SSE
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料② | HTTP 307 + `Location: https://eu.acme-agents.com`（绝不在本区处理） |
| 2 | 材料③ | 正常 200/SSE（zone 匹配） |
| 3 | JWT 载荷解码 | 含 `zone` 声明（§3.4 的 issue 链路） |
| 4 | DB 核对 `SELECT zone FROM tenant WHERE id=<EU租户>` | 为 `EU`；尝试直接 UPDATE zone 走迁移工单流程而非运行时开关（ADR-223，本验证只确认无运行时切换接口） |
| 5 | 白名单路径（/api/auth/） | 不做 zone 校验直接放行 |

**失败排查**：①307 未触发→JWT 缺 zone 声明（v8 JwtService 未抄）或 `app.zone` 未配；②307 到错误域名→`redirectTarget` 模板抄错；③两区都 401→两实例 `JWT_SECRET` 不一致。

### 3.6 EU 区合规供应商路由

```yaml
# application-eu.yaml（EU 区：区域 profile=eu，随 app.zone 生效）
app:
  zone: ${ZONE:EU}
compliance:
  zone: EU
...
# application-us.yaml（US 区：区域 profile=us，无合规过滤）
app:
  zone: ${ZONE:US}
compliance:
  zone: US
...
#### 3.6.1 本节测试与验证（EU 合规路由守卫）

**前置条件**：EU 区实例 `compliance.allowed-suppliers` 已配置；`ComplianceRouter` 已接入 `AgentChatService` 路由段。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | EU 实例上正常对话（目标模型 alias 在 allowed-suppliers 内） | 200 正常放行（guardRoute 原样透传） |
| 2 | 把 allowed-suppliers 改为不含当前模型 alias（模拟全挂） | 请求进入排队模式（429 + PEAK_QUEUE 语义），**不**出现对区外供应商的出站调用 |
| 3 | 抓包/网关日志核对 | 降级期间零跨境 LLM 调用记录（宁可慢不可违） |
| 4 | US 实例（ZONE=US）同样配置为空集合 | 不做合规过滤，正常路由（非 EU 区无过滤） |

**失败排查**：①全挂时仍调外区→`guardRoute` 未接进路由链（漏 `flatMap(compliance::guardRoute)`）；②US 区也被过滤→`deployment.current()` 判定写反；③配置不生效→缺 `@EnableConfigurationProperties(ComplianceProperties.class)`。
...
- [ ] 每区五类资源（DB/Redis/向量库/LLM 网关配置/审计日志）独立部署，无跨区共享存储
- [ ] 全局服务清单仅三条，新增全局服务的"无内容"答辩规则能复述（ADR-225）
...
> 本表即全篇回归口径：§3.5.1（错区重定向）、§3.6.1（合规守卫）通过后逐条对照本表复核（项 1 的抓包审计与项 5 的律师审查需专项补做）。
...
- [ ] ADR-223/224/225/239 分别对应 §3.1 迁移工单 / §3.6.1 排队不跨境 / §3.7 全局最小化 / §3.5.1 边缘兜底
- [ ] §6 痛点（SOC 2 审查要求）与 v9 形成因果衔接（§6 为收束章，不另设验证）
