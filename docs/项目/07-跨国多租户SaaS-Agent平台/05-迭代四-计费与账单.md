# 项目 07：跨国多租户 SaaS Agent 平台 — 05-迭代四：计费与账单

> **定位**：把"计量"升级为"计费"——事件级用量流水（append-only 不可变）、三副本对账、月度账单生成、争议处理。SaaS 的营收正确性问题。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 27-成本治理与Token计量 §计费正确性]、[教程 25-历史记录持久化与合规 §不可变记录]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
public record UsageEvent(
        ...
        long durationMs,
        boolean complete,                // false = 断连但按已有 usage 计（[教程 10 §7.3 流式中断后的部分结果处理]）
...
付费租户到了 47 家，发布节奏出问题：上周一次模型路由策略调整导致 3 家 Pro 租户的 Agent 集体变笨（标准模型池配置错误），客服工单 40+，两家威胁退款。**全量即时发布在 SaaS 是奢侈品**。→ [06-迭代五-三维灰度发布.md](06-迭代五-三维灰度发布.md)
