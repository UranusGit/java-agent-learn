# 项目 07：跨国多租户 SaaS Agent 平台 — 09-迭代八：平台安全与 SOC 2 审计

> **定位**：SOC 2 Type II 备审——租户信任的最后一块拼图：租户级密钥隔离（BYOK 信封加密）、Prompt 注入防护、审计证据链、渗透测试整改。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 25-安全与权限控制 全篇]、[附录 08-Agent安全深度 全部]、[教程 38 §行为审计]；API 真实性以 [附录 12-SpringAI2-API基准] 为准」
...
一切能力就绪，最后一战是**容灾**：EU 大区整体故障（云服务商区域级事故）时，当前架构只能在区内自愈——跨区容灾涉及数据主权（EU 数据不能备到 US 区）与 RTO/RPO 承诺的矛盾。→ [10-迭代九-DR与多活.md](10-迭代九-DR与多活.md)
