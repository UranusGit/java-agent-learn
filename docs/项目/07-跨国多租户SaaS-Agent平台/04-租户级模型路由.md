...
>
> 「遇到阻塞？→ [教程 65-模型路由与降级 全篇]、[教程 60-成本治理与Token计量 §归因]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
/** 被动健康（[教程 32 §6 模型健康检查与自动切换]）：由错误观测标记不可用模型，定期衰减。 */
@Component
...
| 3 | Enterprise（无偏好/高质量偏好） | 命中 flagship；材料 SQL 设 COST_OPTIMIZED 后重试命中 standard |
| 4 | 改偏好后 ≤ TTL 内再次请求 | 新偏好生效（走 TTL 缓存非 JWT，ADR-235） |
...
curl -s "http://localhost:8080/actuator/metrics/saas.llm.tokens" | head
```
