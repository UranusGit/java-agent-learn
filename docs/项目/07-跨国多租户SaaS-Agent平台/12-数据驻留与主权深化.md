...
>
> 「遇到阻塞？→ [教程 33-部署与运维 §多区域]、[教程 43-Agent治理与合规框架]、[附录 13-AI治理与合规/02-数据隐私与偏见检测 §跨境传输]、[教程 31-安全与权限控制 §密钥管理]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
| # | 测试 | 方法 | 通过标准 |
|---|------|------|---------|
| 1 | 分域写入正确性 | EU 域成员在 US 区发起写请求（伪造 dom=EU 打 US 入口） | 307 重定向到 EU 区，US 区零落库（ZoneValidationFilter 升级版单测） |
| 2 | 聚合无内容验证 | 抓取一次 `replicateDaily` 的网络载荷 | 字段白名单校验：无对话文本/无向量/无 PII（CI 里跑 schema 断言） |
| 3 | 台账不可变 | DBA 直连尝试 UPDATE/DELETE 台账 | 触发器拒绝（同 v5 用量事件测试） |
| 4 | kill switch | External 租户在自家 KMS 吊销 KEK 后调 `resolve` | 抛错 fail-closed；恢复 KEK 后自动可用；期间平台无任何绕过路径 |
| 5 | 越权分域 | dom=US 成员尝试读 EU shard 的会话 ID | 404（v2 越权矩阵扩展：分域维度新增用例进 CI） |
| 6 | 监管导出 | 随机时间窗导出 | 台账条目与复制器日志逐笔对得上；导出动作本身出现在台账里 |
