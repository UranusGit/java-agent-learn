...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/09-灰度发布与版本管理 全篇]、[教程 08-架构师进阶/03-自我反思与Agent评估 §5 在线监控 + 离线评估闭环]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
- [ ] 四问的"架构演进"答案（配置指针 + 租户批次，门禁通过才放大范围）与 §3 代码实际一致
- [ ] 新增需求四件事（租户分层 / 批次门禁 / 指针回滚+熔断 / 受影响租户通知）能在 §3 各小节找到落点
...
> ⚠ **装配铁律**：`MetricsClient` / `TenantDirectory` / `Notifier` / `AuditService` 是接口，`ReleaseController`（§3.6）与 `AutoRollbackService`（§3.5）注入它们——**必须提供具体实现 Bean**（真实实现接 Prometheus / 租户目录 / 邮件通知 / 审计表；测试用 stub），否则启动报 `NoSuchBeanDefinitionException`。`TenantCanary` / `ReleaseService` 的装配见 §3.7 `GrayConfig`。
```
...
### 3.7 `GrayConfig.java`（TenantCanary / ReleaseService 装配）

`TenantCanary` 的构造器需要 `Set<UUID> friendlyList`、`ReleaseService` 需要作为单例 Bean——它们都不能靠 `@Component` 自动装配（前者 Spring 无法自动注入 `Set<UUID>`，后者无默认注解），统一在 `GrayConfig` 用 `@Bean` 工厂装配：
...
/** gray 模块装配：TenantCanary / ReleaseService 的 Bean 工厂（friendlyList 在此注入）。 */
@Configuration
public class GrayConfig {

    /** friendlyList = 签约试点租户名单（此处空集示例；生产从配置/DB 注入）。 */
    @Bean
    public TenantCanary tenantCanary() {
        return new TenantCanary(Set.of());
    }
...
> 若不想要 `GrayConfig`，可给 `ReleaseService` 直接加 `@Component`（无参构造可自动装配）；`TenantCanary` 则必须保留 Bean 工厂（`friendlyList` 需显式注入）。接口实现（MetricsClient 等）仍须提供具体 `@Bean`（见 §3.4 装配铁律）。
...
```bash
# ① 查租户批次
curl -s "http://localhost:8081/api/releases/tenants/<tenantId>/batch"
# ② 手工触发门禁评估（试点批）
curl -s -X POST "http://localhost:8081/api/releases/prompt/gate" \
  -H "Content-Type: application/json" -d '{"batch":"PILOT"}'
# ③ 推进/回滚（指针操作）
curl -X POST "http://localhost:8081/api/releases/prompt/advance" \
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 材料① | 返回 INTERNAL / PILOT / P5 / P25 / REST 等批次名，同一租户重复查询结果稳定 |
...
curl -N -X POST "http://localhost:8081/conversations/<会话ID>/messages" \
  -H "Authorization: Bearer <试点批租户JWT>" -H "Content-Type: application/json" \
  -d '{"message":"你好"}'
# 观察日志/响应中的 System Prompt 版本（v1 baseline → v2 切换可见）
```
