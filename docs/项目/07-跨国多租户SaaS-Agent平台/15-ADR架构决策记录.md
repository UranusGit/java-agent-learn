...
> **定位**：全项目 54 条架构决策（ADR-201 ~ ADR-255，编号 229 跳空未使用）的总账——每条含一句话决策、出处与状态；十条最有复用价值的决策给出**上下文/备选/取舍/可回滚**详解。Enterprise 客户尽调"你们为什么这么设计"、架构师面试复盘、新人接手，都从这一篇开始。方法论文档见 [附录 07-架构决策方法论/00-ADR架构决策记录]。
>
...
- [ ] 三个场景（客户尽调即决策考古 / 决策半衰期比代码长 / supersede 关系说不清的后果各是什么）能不看正文复述
- [ ] ADR 最小完备四要素（上下文/备选/取舍/可回滚）能默背，并能对"只写决定不写放弃=宣传稿"给出认同理由
- [ ] 用 ADR-205（连接级状态事故）举例，能指出它属于「决策半衰期比代码长」这类教训的沉淀
...
- [ ] 六组决策域（隔离配额 201/205-210/233-234、计费定价 202/214-216/236/251-255、发布承诺 203/217-222/237-238、主权安全 204/223-228/239-240/243-246、容灾接入 230-232/241-242/247-250、起步上下文 231-232）能各说一句话定位
- [ ] 图上一组边的关系能用一句话解释：`ISO -. 信任边界约束 .-> BILL & REL`（隔离是计费与发布的信任前提）
- [ ] 数字区间与 §3 总账表头（ADR-201~ADR-255）范围一致，无越界
...
- [ ] 台账逐条字段完整：每条含「编号/一句话决策/出处迭代/状态」四要素，无空栏；54 条条数（201-255，229 跳空）能数清
- [ ] 能指出四条演进注记（201 的 v2 修正、233 的 v2 细化 201、239 的 v12 扩展、249 的 v12 细化 230）为何是「不删旧条目」的存续关系
- [ ] 抽查任一条（如 ADR-224 宁可慢不可违）能说出出处（08 迭代七）与一句话决策
...
- [ ] 十条详解（201/205/208/214+215/217/220/223+225/224/230+241/251）能各说一句「上下文→取舍」要点，不用看正文
- [ ] 抽查任一条，能核对四要素完整（上下文/备选/取舍/可回滚）——缺任意一栏即 FAIL
- [ ] 面试复用型（205 池化连接教训 / 220 SLI 产品知识 / 224 宁可慢不可违 / 230 不跨境）能用一句话向别人讲清取舍与不可回滚理由
...
stateDiagram-v2
    ...
    Superseded --> [*]
    Deprecated --> [*]
    note right of Effective: 本项目 54 条全部处于 Effective<br/>其中 201(修正)/233(细化 201)/239(扩展)/249(细化 230)<br/>带演进注记——只演进不推翻
```
...
- [ ] 状态机六态（Proposed→Accepted→Effective→Revised/Superseded/Deprecated→Effective 回环/终态）能从右图对照出来
- [ ] 「只演进不静默违反」这一纪律能落地：违反生效 ADR 的评审拦截（CI 引用 ADR 编号，如 ADR-205）
- [ ] 能指出当前 54 条均处 Effective，其中 201（修正）/233（细化 201）/239（扩展）/249（细化 230）带演进注记——不删除旧条目
...
- [ ] timeline 里每个版本（v0-v13）的 ADR 号段与 §3 台账的出处列一一对应，无缺漏错位
- [ ] 任一版本能答出「它上一迭代的痛点是什么→本版本 ADR 是答案」（如 v6 灰度=答案 v5 模型池失误）
- [ ] v11/v12/v13 的 ADR（243-246 / 247-250 / 251-255）与对应进阶篇标题（12/13/14）能对上
...
- [ ] 能说出"三条满足两条就该写 ADR"的时机判定（多方案选择 / 约束后续多模块 / 六个月后没人记得为什么）
- [ ] 七条自检清单（上下文/≥2 备选/写清放弃/可回滚/出处状态/supersede/红线不可回滚）能对照任一已写 ADR 逐条过一遍
- [ ] 对"稻草人备选不算"这一条能给出正例与反例各一
...
- [ ] 四行总结（总账 / 详解 / 生命周期 / 最大公约数）与 §2 决策域 / §3 台账 / §5 生命周期 / §4 详解的口径一致
- [ ] 最大公约数四类模式能各举一个实际 ADR 编号对应（如上表），并说明它是本项目的可复用资产
- [ ] 全篇 54 条 ADR 总账完整、无跳号（229 已知跳空）、无静默违反，可作为 Enterprise 尽调与新人接手的唯一入口
