...
>
> 「遇到阻塞？→ [教程 33-部署与运维 §多区域]、[教程 30-容错与弹性设计 §超时与重试]、[教程 10-SSE流式通信 §断线重连]、[附录 10-语义缓存与性能/00-语义缓存实现]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
SSE 长连接（[教程 10-SSE流式通信]）对亲和有两个要求：断线重连回到**同一驻留区**（会话状态在那），最好回到**同一实例**（跨实例流式广播缓冲见 [教程 24-多页面流式响应与会话管理]）。传统做法是全局会话表（cookie→节点映射）——**反模式**：全局表本身违背 v8"全局服务最小化"（ADR-225），且成为跨区一致性负担。
...
- **实例级亲和**（尽力）：PoP 在回源负载均衡头上带 `affinity` 头；后端组按头哈希选节点。实例挂了亲和漂移到组内其他节点——SSE 恢复靠断点续传机制（Last-Event-ID，见 [教程 10-SSE流式通信 §断线重连]），不依赖节点存活。
...
语义缓存（[附录 10-语义缓存与性能/00-语义缓存实现]）对 FAQ 型 Agent 的收益极大（相似问题直接命中缓存答案，跳过 LLM 调用）。但把语义缓存推到边缘 PoP 有一个 v8 以来从未松动的约束要面对：**缓存的答案是内容数据**——US 租户的缓存条目放到 Tokyo PoP，就是"内容跨境"。
...
| # | 测试 | 方法 | 通过标准 |
|---|------|------|---------|
| 1 | 就近接入 | 全球 6 探测点拨测（东京/新加坡/法兰克福/伦敦/纽约/圣保罗） | TLS 建连 PoP 为地理最近点；回源目标=租户驻留区 100% |
| 2 | 会话亲和 | SSE 断连重连 1000 次（随机 PoP 入口） | 100% 重回会话驻留区；同实例命中率 > 90%（后端组存活时） |
| 3 | 切换禁跨区写 | 演练中令主区 fencing=REVOKED，全链路写压测 | 全部 429+Retry-After；网络抓包零"写请求到达异法域区" |
| 4 | 缓存不越区 | EU 租户高频同问，抓取 Tokyo PoP 缓存桶 | 桶内零 EU 租户条目（未 opt-in）；opt-in 后条目带 zone 标签 |
| 5 | 灰度-缓存联动 | v6 灰度指针切换瞬间查询 | 切换后首次请求零旧缓存命中（前缀清空生效） |
| 6 | 缓存计量 | 命中 1000 次后对账 | 事件流水 1000 条 `model=CACHE_HIT`，账单金额与等价 token 一致 |
