# 项目 10：数据智能分析平台 — 02-SQL 安全护栏

> **定位**：给 Text2SQL 加"护栏"——数据库层强制（只读角色 + 单语句校验 + 边界控制）+ 查询层兜底（超时/LIMIT/EXPLAIN 预检）。**核心原则：安全在 DB 层，不在 prompt 层**（prompt 指令无法实施访问控制）。教程 04-企业级架构主干/02-全链路可观测性 §SQL 注入 + 教程 03-React前端与AgenticUI/02-React与SSE流式UI §RLS 的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/11-安全与权限控制 §SQL 注入]、[教程 04-企业级架构主干/06-多租户隔离与资源治理 §数据层隔离]」
