...
> **定位**：把项目 10 从 00 到 13 的**架构决策**汇集成体系 ADR（Architecture Decision Record）——每条记录**决策上下文 / 备选方案 / 取舍理由 / 后果 / 可回滚性**。这是本项目的"决策资产"：三个月后改架构时（或面试复盘时），能知道"当时为什么这么定、还剩哪些路没走"。读者画像：想理解项目 10 每个关键决策"为什么"的读者，以及把它当数据平台决策范式的读者。前置阅读：[00-需求分析与架构设计](00-需求分析与架构设计.md)、[10-核心代码讲解](10-核心代码讲解.md)、[附录 07-架构决策方法论]。
>
...
> **本节核对（四问一句话）**：本篇"资产化/不改代码/决策可回溯"定位与 §3 决策域地图、§5 十条详解、§7 自检规范一致（三处是落实"新增哪些决策资产"的三个载体）。
...
**ADR 五要素**（[附录 07-架构决策方法论 §记录模板]）：上下文（当时的问题与约束）、决策（选了什么）、备选（还考虑过什么、为何不选）、后果（代价与收益）、可回滚性（回退到哪、怎么退）。

### 2.1 本节核对（ADR 的价值与五要素）

- [ ] flow chart 中"无 ADR→推倒重来/重复踩坑"与"有 ADR→有据可查+回滚路径"的对比成立
- [ ] 五要素模板能在 §5 十条详解中找到完整呈现（上下文/决策/备选/后果/可回滚逐条都有），非仅有结论
...
- [ ] 六域（安全/语义/路由/呈现/成本质量/发布治理）与六类风险一一对应
- [ ] 每个 subgraph 内的 ADR 编号可反查落地篇（如 A1 里 ADR-502/506→[02]、B4 里 ADR-530~533→[11]），无悬浮无出处编号
- [ ] 箭头顺序（SEC→SEM→ROUTE→OUT→COST→REL）与项目 v1→v12 演进顺序一致（跨域"先安全后语义"等）
...
- [ ] v1–v12 十二站与九个主体迭代 + v10/v11/v12 三篇对应，演进主线（能查→安全→准确→…→治理）自洽
- [ ] 每站标注的 ADR 编号区间与对应迭代篇内的 ADR 一致（如 v11→ADR-534~537，见 [12 §6]）
...
### 5.1 本节核对（十条关键 ADR 的五要素完整性）

- [ ] 十条 ADR（§5 十条详解）每一组都含五要素——上下文 / ≥2 备选（含不选理由）/ 决策 / 后果（取舍）/ 可回滚性，无"只给结论"条目
- [ ] 每条 `<ADR 编号>` 可反查迭代篇（如 ADR-502/506→[02]、ADR-524/525/526→[08]），编号与落地篇一致
- [ ] 自检能答三问（呼应 [10 §1](10-核心代码讲解.md)）：为什么安全在 DB 层（ADR-502/506）、为什么语义层优先（ADR-501/509/511）、为什么数值不用 LLM 判（ADR-524/525/526）
...
> **本节核对（v10–v12 索引）**：索引表 ADR-530~542 逐条与 [11]/[12]/[13] 各篇 §6 ADR 表编号一致（双向可追溯），无编号错位或落地篇张冠李戴。
...
> **本节核对（维护规范）**：自检五条（五要素齐备 / 可反查 / ≥2 备选 / 可回滚 / 推翻成文）与上文"维护规范"表（必填/编号/推翻/引用）对应；flowchart 的"越界→新 ADR+superseded"流程已在正文成文，非口头规则。
...
> **本节核对（总结一句话）**：篇末三个"最有复用价值"ADR（501/509、502/506、524/525）与 §5 详解、[10 §1](10-核心代码讲解.md) 六个设计互为印证（"为什么"在 ADR 留了底）。
