...
> **定位**：把"每次问都花钱"变成"相似问题不重复花钱"——两级缓存（精确 + 语义）+ Token 预算 + 按租户成本归因。教程 21 §成本治理 + 附录 07 §语义缓存的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 21-成本治理与Token计量]、[附录 07-语义缓存与性能/00-语义缓存实现]、[教程 33-Agent性能优化 §缓存策略]」
    ...
    public Optional<QueryResult> semanticHit(String question, String tenantId) {
        List<Document> hits = vectorStore.similaritySearch(
                SearchRequest.builder().query(question).topK(1)
                        .similarityThreshold(THRESHOLD).build());   // 附录 12-02 §4 新式
        if (hits.isEmpty()) {
...
/**
 * 复用 Spring AI 原生 gen_ai.client.token.usage（教程 21 §指标体系）。
...
private Mono<QueryResult> semanticAndMiss(String question, UserContext user) {
    return Mono.fromSupplier(() -> semanticCacheService.semanticHit(question, user.tenantId()))
            .flatMap(opt -> opt.isPresent()
                    ? Mono.just(opt.get()).flatMap(r -> cacheService.exactPut(question, user.tenantId(), r))
