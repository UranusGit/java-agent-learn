# 项目 10：数据智能分析平台 — 09-迭代八：灰度发布与多租户（主体迭代收官）
...
**材料**：多租户账号 + 灰度流量切分剧本 + 两版本 Agent。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「部门隔离」场景执行 | 销售部看不到财务指标/数据（指标集 + RLS 双隔离） |
| 2 | 用材料构造「口径灰度」场景执行 | 试点部门改口径，golden 集门禁达标才放量 |
| 3 | 用材料构造「自动回滚」场景执行 | 口径变更致 golden 通过率 < 92%，1 窗口内回滚 |
| 4 | 用材料构造「配额生效」场景执行 | 各部门配额独立（销售爆不影响财务） |
| 5 | 用材料构造「灰度可见」场景执行 | 每次口径/策略灰度有决策留痕（实验记录） |
