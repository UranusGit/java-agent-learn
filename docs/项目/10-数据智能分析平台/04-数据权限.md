...
> **定位**：行级 RLS + 列级脱敏 + 角色数据可见性——给不同人"看到该看的"。核心：权限在 DB 层（RLS 强制），响应式侧用 Reactor Context 传租户。教程 03-React前端与AgenticUI/02-React与SSE流式UI §数据层隔离 + 教程 05-Observation可观测/09-Advisor与RAG观测：检索质量可观测 §Context 传递的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/06-多租户隔离与资源治理 §数据层隔离]、[教程 08-架构师进阶/08-响应式错误处理 §Context 传递]」
...
| **影响了哪些模块** | DB 层启用 RLS + 列权限/视图；新增 `TenantContextFilter`（WebFilter 写 Reactor Context）+ `RegionAwareQueryExecutor`（set_config 事务级）；查询执行统一走 `RegionAwareQueryExecutor` |
| **架构如何演进** | 请求链路：WebFilter 解析 JWT → Reactor Context 携带租户 → 执行层 deferContextual 取租户 → `set_config` 注入 RLS 会话变量 → 查询 |
...
> **本节核对（四问一句话）**：链路"WebFilter → Reactor Context → set_config（事务级）→ 查询"四步与 §4.3/§4.4 代码一一对应；三条 v3 痛点均有对策。
 ...
 * 教程 05-Observation可观测/09-Advisor与RAG观测：检索质量可观测 §Context 传递——请求级上下文只经 contextWrite，不下沉到实例字段。
 */
@Component
...
### 4.4 `RegionAwareQueryExecutor.java`（set_config 事务级 + 查询）
...
/**
 * 从 Reactor Context 取 region → set_config('app.current_region', ..., true)（事务级）→ 执行查询。
 * 注意：PG 的 SET/SET LOCAL 是 SQL 字面量语句，不接受 JDBC 参数绑定（?），
 * 带变量的注入必须走 set_config（第三个参数 true = 事务级，等价 SET LOCAL）。
 * 连接池下"事务级绑定"防串号：连接归还池后事务级变量不残留。
        ...
        return Mono.deferContextual(ctx -> {
            ...
            return Mono.fromCallable(() ->
                    transactionTemplate.execute(status -> {
                        // set_config 第三参 true = 事务级（等价 SET LOCAL）——RLS 策略读取 app.current_region 过滤行
                        jdbcTemplate.queryForObject(
                                "SELECT set_config('app.current_region', ?, true)",
                                String.class, tenant.region());
...
> ⚠ `set_config(..., true)`（事务级）必须与查询同事务；`RegionAwareQueryExecutor` 已用 `TransactionTemplate` 包裹。若两个方法各开连接，会话变量不会生效——这是本迭代最容易写错的地方。
...
**失败排查**：①步骤 2 不过滤→RLS 策略未建或连库账号是表 owner/superuser（FORCE RLS 才约束 owner，superuser 永远绕过——换普通账号测）；②掩码失效→`customer_masked` 查询账号没有视图权限，或 `has_role` 函数未按角色实现（该函数需自行在 DB 创建，示意见 §4.1 注释）；③串号→`set_config(..., true)` 与查询不在同一事务（§4.5 ⚠ 的高发错误，检查是否都包在同一个 `TransactionTemplate.execute` 内；另核对未误写成 `jdbcTemplate.update("SET LOCAL ... = ?")`——PG 的 SET 语句不接受参数绑定，运行期直接语法错误）；④404 变 403→存在性保护属应用层响应映射，检查 Controller 的异常处理。
...
| ADR-514 | Reactor Context 传 region + set_config 事务级 | WebFlux 禁 ThreadLocal；连接池下事务级防串号；`SET LOCAL` 不接受参数绑定故用 set_config |
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| RLS 强制 | 绕过应用直连 DB 也无法跨 region 读（§4.7 材料 A + §7 回归 1） | ☐ |
| 列级掩码 | PII 列按角色掩码，privacy_officer 可看原文（§4.7 材料 B） | ☐ |
| 越权矩阵 | 全接口 × 越权场景 0 泄露，CI 全绿（§7 回归 1） | ☐ |
| 无串号 | 500 并发混合 region 事务零串号（§4.7 步骤 5 + §7 回归 3） | ☐ |
| 存在性保护 | 越权访问返回 404 非 403（§7 回归 2） | ☐ |
