...
权限可控了，但**多数据源暴露**：订单库/用户库/报表库三个库，Agent 不知道该查哪个；跨库关联（"华东客户的订单"要 join 用户库）LLM 现场猜 join 会错。**需要 Data Catalog 路由**。→ [05-迭代四-多数据源AgenticRAG.md](05-迭代四-多数据源AgenticRAG.md)
