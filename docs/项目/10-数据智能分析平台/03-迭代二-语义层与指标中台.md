# 项目 10：数据智能分析平台 — 03-迭代二：语义层与指标中台

> **定位**：把"Text2SQL 猜口径"升级为"NL→语义层 DSL→确定性 SQL 编译"——指标口径统一，覆盖的问题准确率趋近 100%。**这是本项目区别于"套壳 Text2SQL"的核心架构决策。** 教程 30 §语义检索 + 教程 12 §结构化输出的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 30-高级RAG与AgenticRAG §Agentic 检索]、[教程 12-结构化输出]、[附录 05-LLM基础理论/02-上下文窗口与Token]」
...
public class MetricCompiler {
    ...
    public CompileResult compile(MetricIntent intent) {
        ...
        if (intent.timeRange() != null && !intent.timeRange().isBlank()) {
            ...
            if (parts.length == 2) {
                conds.add(metric.timeColumn() + " BETWEEN DATE ? AND DATE ?");
...
public class SemanticLayerService {
    ...
    private Mono<MetricIntent> resolveIntent(String question) {
        ...
        return Mono.fromCallable(() -> chatClient.prompt()
                ...
                .call()
                .entity(MetricIntent.class))     // 真实重载 entity(Class)，附录 12-02 §2
...
口径统一了，但**数据权限暴露**：业务 A 的销售经理问"全国销售额"，平台返回了全部——**行级权限（只能看自己区域）+ 列级脱敏（不能看客户明细）没有接入**。给不同人"看到该看的"是数据分析的信任前提。→ [04-迭代三-数据权限.md](04-迭代三-数据权限.md)
