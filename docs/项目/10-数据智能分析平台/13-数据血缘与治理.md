...
>
> 「遇到阻塞？→ [教程 02-SpringAI核心机制/02-Agent状态管理]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §审批触发]、[教程 04-企业级架构主干/05-历史记录持久化与合规]、[附录 06-企业级架构模式]」
>
...
> `@Scheduled` 需在启动类加 `@EnableScheduling`（Spring 标准注解，非 Spring AI）。巡检任务跑在调度线程，不占 WebFlux EventLoop——与 [教程 08-架构师进阶/08-响应式错误处理 §调度隔离] 同一纪律。
...
/**
 * 变更影响评估：改表前必答"这列被谁用"。
 * 判定：命中 VALUE 依赖的活跃指标或订阅报表 → BLOCK（转人工审批，[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议 §审批触发]）
    ...
    private boolean check(QualityRule rule) {
        try {
            ...
            return switch (rule.ruleType()) {
                case "FRESHNESS" -> freshness(rule, threshold);        // max(target_column) 距今不超 max_age_minutes（如 orders.order_date）
...
**前置条件**：`quality_rules` 写入一条 `orders` 的 FRESHNESS 规则（目标列 `order_date`——orders 表无 `updated_at` 列，时效以最新订单日期计；`max_age_minutes=60`，severity=FREEZE）；`MetricFreezeGateway` 注入可用；启动类已 `@EnableScheduling`。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 人为停更 `orders`（`order_date` 距今 > 60 分钟——orders 无 `updated_at` 列，时效列用 `order_date`；如需真正更新时间，先 `ALTER TABLE orders ADD COLUMN updated_at TIMESTAMPTZ DEFAULT now()` 并配套触发器），触发 `patrol()` | `quality_results` 落 `passed=false`；FREEZE 规则调 `freezeTableDependents("orders",...)`——§7 `freshnessViolationFreezes` + 验收 4（1 小时内冻结） |
...
| ADR-540 | 命中 VALUE 依赖即 BLOCK 转 HITL 审批 | 数据变更是高危操作（影响清单必答）；复用 [教程 03-React前端与AgenticUI/04-流式工具调用与事件协议 §审批触发] 模式 |
...
class LineageAndImpactTest {
    ...
    @Autowired
    private ChangeImpactService changeImpact;   // 依赖 LineageGraphService（查 lineage_edges），经 Spring 装配
...
import org.springframework.boot.test.context.SpringBootTest;   // ⚠ 需引入依赖 org.springframework.boot:spring-boot-starter-test（本地未下载，未 javap 实证；以引入依赖后 javap 输出为准）
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.test.context.bean.override.convention.MockitoBean;   // ⚠ Boot 4.1：@MockBean 已移除，用 @MockitoBean（同上，以引入依赖后 javap 输出为准）
...
class DataQualityRuleTest {
    ...
    @MockitoBean
    private MetricFreezeGateway freezeGateway;   // 只捕获冻结调用，不真冻 v8 指标

    @Test
    void freshnessViolationFreezes() {
        // 规则：orders.order_date 距今不超 60 分钟（种子订单日期固定 → 等价"人为停更"），severity=FREEZE
        jdbcTemplate.update("DELETE FROM quality_results WHERE rule_id = 'fresh-orders'");
        jdbcTemplate.update("""
                INSERT INTO quality_rules(id, rule_type, target_table, target_column, threshold, severity, enabled)
                VALUES ('fresh-orders', 'FRESHNESS', 'orders', 'order_date',
                        '{"max_age_minutes": 60}'::jsonb, 'FREEZE', true)
        ...
        Boolean passed = jdbcTemplate.queryForObject(
                "SELECT passed FROM quality_results WHERE rule_id = 'fresh-orders'", Boolean.class);
        assertThat(passed).isFalse();   // 违规落 quality_results（order_date 距今远超 60 分钟）
        verify(freezeGateway).freezeTableDependents(eq("orders"), contains("FRESHNESS"));
    }

    @Test
    void brokenRuleDoesNotFreeze() {
        // 阈值 JSON 非法（max_null_rate 写成字符串）→ check 异常 → passed=true（fail-open to warn）
        jdbcTemplate.update("DELETE FROM quality_results WHERE rule_id = 'broken-rule'");
        jdbcTemplate.update("""
                INSERT INTO quality_rules(id, rule_type, target_table, target_column, threshold, severity, enabled)
                VALUES ('broken-rule', 'COMPLETENESS', 'orders', 'pay_amount',
                        '{"max_null_rate": "0.05"}'::jsonb, 'FREEZE', true)
                ON CONFLICT (id) DO UPDATE SET threshold = EXCLUDED.threshold, enabled = true
                """);
        reset(freezeGateway);
        ...
        Boolean passed = jdbcTemplate.queryForObject(
                "SELECT passed FROM quality_results WHERE rule_id = 'broken-rule'", Boolean.class);
        assertThat(passed).isTrue();    // 规则故障 ≠ 数据故障：fail-open to warn
        verify(freezeGateway, never()).freezeTableDependents(anyString(), anyString());   // 不冻结
    }
}
```
