...
> **定位**：主体迭代收官篇——多部门（销售/财务/运营）隔离的租户治理 + 指标口径/查询策略的灰度发布。教程 29 §灰度 + 教程 26 §多租户的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 62-灰度发布与版本管理]、[教程 59-多租户隔离与资源治理]」
...
/**
 * 确定性分桶：sha256 + floorMod（避开 abs(hashCode)%100 溢出，教程 29 §流量切分）。
...
**材料 A——配置绑定核对 SQL**（`DepartmentProperties` 读取 `departments` 前缀，无需入库）：

```sh
# 启动后经 actuator 或日志核对绑定的 dept 指标集
GET /actuator/env/departments.sales.metrics     # 期望 ["gmv","order_count","active_users"]
GET /actuator/env/departments.finance.budget_tokens  # 期望 300000
```
...
**失败排查**：①配置读不到→`@ConfigurationProperties(prefix="departments")` 的 setter 未写或未加 `@Component` 注册；②分桶结果偶变→`sha256Long` 实现错或输入含变化键（userId 换了）；③启动报绑定失败→yaml 缩进/类型与 `Department` 字段不匹配（`budget_tokens` 对应 `budgetTokens`）。
...
- [ ] `strategyVersion` 的分桶复用 §4.8 的 `BucketService.bucketFor`（"A"/"B"），同部门同用户分桶幂等可复现
...
| ADR-529 | 确定性分桶用 sha256 + floorMod | 幂等可复现；避开 abs(hashCode)%100 的溢出（教程 29 §流量切分） |
