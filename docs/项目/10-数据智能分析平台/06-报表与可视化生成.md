...
> **定位**：从"返回表格"升级为"交付洞察"——LLM 产结构化 JSON（chart spec + 叙事），Java 确定性渲染 ECharts。教程 13 §结构化输出 + 附录 14 Agent 交互设计的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 22-结构化输出]、[附录 13-Agent交互设计/00-Agent用户体验设计]、[教程 27-Agentic-UI设计 §生成式 UI]」
...
/**
 * LLM 只输出受控的 chart spec（不是自由 HTML）——教程 18 §4.3 防注入。
...
**前置条件**：[05 §4.10] 已通过；前端 React 工程可跑（教程 24-27 前置）；`ReportGenerator` 已接入查数链路尾部。
...
| ADR-520 | chart spec 受限 DSL 非任意 HTML | 防间接注入导致 XSS（教程 18 §4.3） |
