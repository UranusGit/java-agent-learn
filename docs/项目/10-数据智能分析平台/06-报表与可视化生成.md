...
**材料**：报表问题集；可视化意图样本；生成结果对照。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「图表正确」场景执行 | chart 类型与数据匹配率 ≥ 90%（人工标注 100 份） |
| 2 | 用材料构造「叙事可信」场景执行 | insights 100% 基于数据（无编造数字） |
| 3 | 用材料构造「信任建立」场景执行 | 报表 100% 附 SQL 引用 |
| 4 | 用材料构造「安全」场景执行 | chart spec 是受限 DSL（无任意 HTML/脚本） |
| 5 | 用材料构造「渲染性能」场景执行 | 单报表渲染 < 1s（P95） |
