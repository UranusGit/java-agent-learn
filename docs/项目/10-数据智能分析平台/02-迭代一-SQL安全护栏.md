...
> 过程可见性是工业级标配（[附录 14/01]）。数据 Agent 回答"给我 GMV 报表"要看得见过程——模型生成哪条 SQL、护栏为何判定（拦截/只读/超时）、查了几行。执行 emit 数据事件：`{q, 生成SQL, 护栏判定, 命中行数, 结果}`。用户看到"生成SQL→命中'全表扫描'风险→自动加LIMIT→返回前100行"。
