...
> **定位**：给 Text2SQL 加"护栏"——数据库层强制（只读角色 + 单语句校验 + 边界控制）+ 查询层兜底（超时/LIMIT/EXPLAIN 预检）。**核心原则：安全在 DB 层，不在 prompt 层**（prompt 指令无法实施访问控制）。教程 31 §SQL 注入 + 教程 26 §RLS 的落地。本文给出**完整可手写代码**（一行不省略）。
>
...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。数据 Agent 回答"给我 GMV 报表"要看得见过程——模型生成哪条 SQL、护栏为何判定（拦截/只读/超时）、查了几行。执行 emit 数据事件：`{q, 生成SQL, 护栏判定, 命中行数, 结果}`。用户看到"生成SQL→命中'全表扫描'风险→自动加LIMIT→返回前100行"。

**护栏判定必须可见**：多语句注入被拒、全表扫描被拦截、权限不足被拒——每一步拦截都发事件，用户/审计知道"为什么这个查询没执行"（而非以为数据查不到）。SQL 只在事件里给**脱敏结论与行数**，不泄库内数据。
