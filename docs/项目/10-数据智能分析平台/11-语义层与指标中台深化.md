...
> **定位**：把 v3 的"指标字典"升级为**指标治理体系**——口径统一不止是一份静态字典，而是"提名 → 评审 → 版本化发布 → 退役"的生命周期；语义层 schema 演进有**兼容性规则**（改口径是破坏性变更，必须走 v9 灰度门禁）；业务术语字典（同义词/黑话/缩写）与 LLM 对齐，**Text2SQL 先查语义层再生成 SQL**。读者画像：已跑通 v3 语义层、想知道企业里"指标字典怎么管起来、怎么长年不死掉"的读者。前置阅读：[03-语义层与指标中台](03-语义层与指标中台.md)、[09-灰度发布与多租户](09-灰度发布与多租户.md)、[教程 02-SpringAI核心机制/04-结构化输出]、[教程 08-架构师进阶/01-高级RAG与AgenticRAG]。
>
> 「遇到阻塞？→ [教程 08-架构师进阶/01-高级RAG与AgenticRAG §Agentic 检索]、[教程 04-企业级架构主干/09-灰度发布与版本管理]、[教程 00-基础与核心/05-RAG检索增强生成 §2]、[附录 05-SpringAI2-API基准]」
>
...
```sh
# A. 已登记同义 → 归一化为内部标记（术语"流水"已在 business_glossary 登记 流水→gmv）
curl -X POST "http://localhost:8081/api/query?userId=u1" \
  -H "Content-Type: text/plain" -H "X-Tenant-Id: tenant-default" -H "X-Region: east" \
  -d "上月流水多少"    # 期望：命中精确典，响应含 gmv 的确定性编译 SQL（不降级 ad hoc）

# B. 未登记且低相似（如"上个卖季的动销"）→ 原样返回
curl -X POST "http://localhost:8081/api/query?userId=u1" \
  -H "Content-Type: text/plain" -H "X-Tenant-Id: tenant-default" -H "X-Region: east" \
  -d "上个卖季的动销"    # 期望：不强行归一化，走 ad hoc 降级路径
```
...
class SemanticGovernanceTest {
    ...
    @Autowired
    private SemanticSchemaCompat compat;

    @Autowired
    private MetricGovernanceService governance;
