...
>
> 「遇到阻塞？→ [教程 22-结构化输出]、[教程 78-高级RAG与AgenticRAG]、[教程 59-多租户隔离与资源治理 §数据权限]、[教程 64-安全与权限控制 §SQL 注入]、[教程 60-成本治理与Token计量]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
| 2 | `mvn spring-boot:run` | 应用启动到 8080，无密钥占位符解析失败（`${DEEPSEEK_API_KEY}` 未导出会启动报错，属预期防线） |
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 Text2SQL | [教程 22-结构化输出 §entity]、[教程 05-RAG检索增强生成 §2] |
| 02 SQL 护栏 | [教程 64-安全与权限控制 §SQL 注入]、[教程 59-多租户隔离与资源治理 §RLS] |
| 03 语义层 | [教程 78-高级RAG与AgenticRAG §Agentic 检索]、[教程 22-结构化输出 §entity] |
| 04 数据权限 | [教程 59-多租户隔离与资源治理 §数据层隔离]、[教程 85-响应式错误处理 §Context 传递] |
| 05 多源 | [教程 78-高级RAG与AgenticRAG §Agentic 检索] |
| 06 报表 | [教程 22-结构化输出]、[附录 13-Agent交互设计] |
| 07 成本 | [教程 60-成本治理与Token计量]、[附录 09-语义缓存与性能] |
| 08 数据质量 | [教程 80-自我反思与Agent评估]、[教程 84-数据飞轮与持续改进] |
| 09 灰度多租户 | [教程 62-灰度发布与版本管理]、[教程 59-多租户隔离与资源治理 §多租户] |
| 10 语义层深化 | [教程 78-高级RAG与AgenticRAG]、[教程 62-灰度发布与版本管理 §版本门禁]、[教程 05-RAG检索增强生成 §2] |
| 11 评测护栏升级 | [教程 80-自我反思与Agent评估]、[教程 84-数据飞轮与持续改进]、[附录 04-测试策略/02-Eval评估] |
| 12 血缘治理 | [教程 61-Human-in-the-Loop与审批流 §审批触发]、[教程 58-历史记录持久化与合规]、[附录 06-企业级架构模式] |

### 6.1 本节核对（知识点对照）

- [ ] 12 个迭代每一行至少引用 1 个真实存在的教程/附录篇目，锚点章节号不越界
- [ ] 无重复覆盖冲突：同一教程章节被多个迭代引用时，引用的角度不同（如教程 35 在 03/05/10 分别承担语义匹配/Agentic 检索/版本门禁）
