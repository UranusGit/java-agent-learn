...
> **定位**：本文是项目 10 的起点——一个「**自然语言查数 → 安全可审计 → 口径统一 → 权限可控 → 报表交付 → 成本可治理**」的企业级自助数据智能分析平台。项目 10 是**完整独立**的项目，不依赖也不引用其他项目。**本项目的核心使命：把教程 12（结构化输出）、30（Agentic RAG）、20（数据权限）、25（安全）、21（成本治理）在"数据分析"这个企业 AI 落地最普遍的场景落地。** 全部代码为**完整可手写**（一行不省略），由你照抄手写（项目规范不生成 .java 文件）。
>
> 「遇到阻塞？→ [教程 12-结构化输出]、[教程 30-高级RAG与AgenticRAG]、[教程 20-多租户隔离与资源治理 §数据权限]、[教程 25-安全与权限控制 §SQL 注入]、[教程 21-成本治理与Token计量]；API 真实性以 [附录 12-SpringAI2-API基准] 为准」
...
| 迭代 | 落地的教程知识 |
|------|---------------|
| 01 Text2SQL | [教程 12-结构化输出 §entity]、[教程 05-RAG §Schema] |
| 02 SQL 护栏 | [教程 25 §SQL 注入防御]、[教程 20 §RLS] |
| 03 语义层 | [教程 30 §语义检索]、[教程 29 §上下文工程] |
| 04 数据权限 | [教程 20 §数据层隔离]、[教程 37 §Reactor Context] |
| 05 多源 | [教程 30-高级RAG与AgenticRAG §Agentic 检索] |
| 06 报表 | [教程 12 §结构化输出]、[附录 11-Agent交互设计] |
| 07 成本 | [教程 21-成本治理与Token计量]、[附录 07-语义缓存与性能] |
| 08 数据质量 | [教程 32-自我反思与Agent评估]、[教程 36-数据飞轮] |
| 09 灰度多租户 | [教程 23-灰度发布与版本管理]、[教程 20 §多租户] |
...
1. **框架 API 全部真实**：Spring AI 2.0.0 的 `ChatClient`/`@Tool`/`CallAdvisor`/`entity()` 等均按 [附录 12-SpringAI2-API基准] 的真实签名书写，**每个 Java 类含全部 import、无 `...` 省略，可照抄编译**。
