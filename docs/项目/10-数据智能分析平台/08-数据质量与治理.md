...
> **定位**：防"clean-looking wrong answers"——指标对账 + 漂移检测 + 查询正确性评估。教程 37 §评估 + 教程 41 §数据飞轮的落地。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 37-自我反思与Agent评估]、[教程 41-数据飞轮与持续改进]、[附录 04-测试策略/02-Eval评估]」
