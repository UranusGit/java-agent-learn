...
**材料**：含空值/重复/异常值的脏数据样本；质量规则集；质量评分校验。

**步骤与断言（由验收表转断言清单）**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 用材料构造「对账覆盖」场景执行 | 核心指标 100% 有 control query 对账 |
| 2 | 用材料构造「漂移捕获」场景执行 | 注入"列改名"场景，1 天内被冻结告警 |
| 3 | 用材料构造「golden 回归」场景执行 | 200 问执行准确率 ≥ 90%（退化即阻断发布） |
| 4 | 用材料构造「冻结可见」场景执行 | 漂移指标返回结果带"待复核"标注 |
| 5 | 用材料构造「无静默错」场景执行 | 对账发现漂移后不返回未标注的可信结果 |
