...
**决策依据是频率差异**（[教程 17 §4]）：token 每秒 20-80 个，不缓冲会打满主线程；工具事件每轮几个，缓冲反而引入"卡片延迟出现"的怪异感（违反 3 秒规则）。**背压要施加在高频源上，不能一刀切**——这与后端 Reactor 只对快生产者加 buffer/onBackpressure 是同一判断标准（[附录 06-WebFlux与响应式编程/02-背压与流量控制]）。
