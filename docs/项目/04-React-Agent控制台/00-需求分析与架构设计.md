...
>
> **前置阅读**：[教程 03-React前端与AgenticUI/00-React入门与现代前端工程]、[教程 03-React前端与AgenticUI/01-React状态管理]、[教程 03-React前端与AgenticUI/02-React与SSE流式UI]、[教程 03-React前端与AgenticUI/03-Agentic-UI设计]
> **后端对接**：本项目自带一个**完整可手写的参考后端**（WebFlux + Spring AI 2.0 Agent 服务），它是对前端依赖契约的落地实现——事件协议见 §3.2，完整依赖清单见 §6。前端与后端各为一套独立工程，通过 REST + SSE 通信（[教程 02-SpringAI核心机制/00-SSE流式通信]、[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议]）。
...
| 抱怨 | 背后的产品缺陷 |
|------|---------------|
| "它卡住的时候我不知道它在干嘛" | 工具执行/思考过程不可见（[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §1 30 秒白屏]） |
...
| 层 | 选型 | 理由 | 备选与放弃原因 |
|----|------|------|---------------|
| 框架 | React 19 | 本体系教程线；生态最全 | Vue 3——同样可行，但与教程 00-基础与核心/03-工具调用 不衔接 |
| 构建 | Vite | 毫秒级 HMR、零配置（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §4.1]） | Webpack——启动慢、配置重 |
| 语言 | TypeScript strict | 前后端契约的类型保障（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §4.2]） | JS——放弃编译期契约校验 |
| 客户端全局状态 | Zustand | selector 订阅防连坐渲染（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §3.2]） | Redux Toolkit——样板代码多；Context——高频值不适用 |
| 服务端状态 | TanStack Query v5 | 缓存/失效/重验证开箱即用（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §4]） | 手写 loading/error——重复造轮子 |
...
| 流式消费 | fetch + ReadableStream 自研封装 | POST + 鉴权头 + 取消（[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §1.1]） | EventSource——不支持 POST/自定义头，弃 |
| 后端框架 | Spring Boot 4.1 + WebFlux | 响应式 SSE 原生支持（[教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §4]） | Spring MVC——不支持响应式背压 |
| 后端 AI | Spring AI 2.0（ChatClient） | 本体系技术栈主线（[附录 05-SpringAI2-API基准]） | 1.0 旧 API——已过审计淘汰 |

**不引入的东西（克制清单）**：UI 组件库（MUI/AntD）——本项目组件全部手写以服务学习目标；状态机库（XState）——useReducer 已够（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §2.2]）；React 19 的 useOptimistic 暂不用于 v1（发送消息的乐观上屏在迭代二再引入）。
...
分层与依赖规则（呼应后端的分层思想，[教程 02-SpringAI核心机制/04-Advisor链与拦截器 §分层]）：
...
> 契约的完整事件定义（TS + Java 双面）、脱敏约定（args 脱敏、resultPreview 截断）见 §6.5 与 [教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §2.2]。若后端尚未实现某事件，前端按"未知 type 安全跳过"降级（[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §6 协议版本化]）。

### 3.3 三层状态模型落地

直接采用 [教程 01-WebFlux与响应式编程/06-线程模型与调度器 §5] 的三层模型：

- **全局层（Zustand）**：`sessions[]`、`activeSessionId`、`sidebarCollapsed`
- **服务端层（TanStack Query）**：会话列表（`['sessions']`）、历史消息（`['session-messages', id]`）、配额（`['quota']`）
- **本地流式层（useReducer + ref）**：连接状态机、thought/toolCalls/answerText、pendingApproval；rAF 批量缓冲（[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §4]）
...
每个迭代的"上一版痛点 → 新需求 → 架构影响 → 验收"以四问表形式在各自篇目中展开（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §演进式原则] 的项目版落地）。
...
| # | 决策 | 上下文 | 备选 | 理由 |
|---|------|--------|------|------|
| ADR-1 | fetch+RS 而非 EventSource | 对话需 POST+鉴权头 | EventSource | 能力硬伤（[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §1.1]） |
| ADR-2 | 流式状态不进全局 store | token 高频更新 | 全局 Zustand | 防全站连坐渲染（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §1 原则]） |
| ADR-3 | 事件协议 types/ 单一来源 | 前后端契约漂移风险 | 各组件自定义类型 | 编译期约束；协议变更先改契约 |
| ADR-4 | 审批恢复走新流 | SSE 挂几小时等审批不现实 | 长连接等待 | 连接成本与代理超时（[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5]） |
| ADR-5 | 不引 UI 组件库 | 学习型项目 | MUI/AntD | 手写组件是学习目标本身 |
| ADR-6 | 后端交付"参考实现"而非空契约 | 前端无法脱离真实 SSE 联调 | 只给接口文档 | 完整可手写后端是学习目标的一部分（教程 02-SpringAI核心机制/00-SSE流式通信 的落地） |
...
### 6.2 参考后端配置（两段式，对齐 demo01 代码仓习惯）

`application.yaml` 只放 `.env` 导入与 profile 激活；业务配置全部在 `application-console.yaml`（启动时 `-Dspring-boot.run.profiles=console` 激活）：

```yaml
# application.yaml —— 仅两行：.env 导入 + profile 激活（其余一切进 console 段）
spring:
  config:
...
```yaml
# application-console.yaml —— console profile 业务配置
server:
  port: 8081
spring:
  ai:
    openai:
      ...
      chat:
        model: deepseek-v4-flash                  # Spring AI 2.0.0：无 options 中缀，参数直挂
        timeout: 600s
```
...
export interface BaseEvent {
  id: number;      // 会话级单调递增，断线重连去重依据（教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §3.2）
  ts: number;      // 毫秒时间戳
}
...
> **协议设计要点**（[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §2.2] 决策表的落地）：
...
| 3 | 检查 `application.yaml` / `application-console.yaml` | 两段式齐全（yaml 仅 .env 导入 + profile 激活；console 段端口 8081）；api-key 为 `${DEEPSEEK_API_KEY}` 占位，无明文密钥 |
...
| 工程 | 两套工程完整依赖清单（pom.xml / package.json / application.yaml+application-console.yaml / vite.config.ts / tsconfig.json） |
...
代码使用约定：框架 API 全部真实（Spring AI 2.0 的 `ChatClient`/`@Tool`/`ChatMemory` 按 [附录 05-SpringAI2-API基准] 签名书写，可照抄编译）；事件协议后端 sealed 接口与前端联合类型一一对应；凡涉及业务演示的确定性触发逻辑（如"消息含'订单'就调工具"）均显式标注为演示规则，生产由 `ToolCallingManager` 装饰器替换（[教程 02-SpringAI核心机制/00-SSE流式通信 §3.2]）。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 工程骨架可编译 | pom.xml / package.json / 两段式配置照抄后 `mvn compile` 与 `tsc -b` 双零错误（§6.6 第 1-2 条） | 待实测打钩 |
| 契约双面对齐 | TS `AgentEvent` 联合与 Java sealed 12 种事件逐一对齐，`id` 字段两翼都有（§6.6 第 4 条） | 待实测打钩 |
| 密钥零明文 | api-key 为 `${DEEPSEEK_API_KEY}` 占位，两段式配置无硬编码（§6.6 第 3 条） | 待实测打钩 |
| 架构守卫 | 02 篇完成后组件零直接 fetch、六个端点有对应 Controller、三层状态归属正确（§3.4） | 待实测打钩 |
| 迭代路线兑现 | 01-08 各篇验收/回归结论覆盖 §4 迭代表对应行（§9 第 3 条） | 待实测打钩 |
