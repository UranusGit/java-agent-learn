...
>
> **前置阅读**：已完成 06；[教程 17-React与SSE流式UI §3 断线重连]、[教程 24-多页面流式响应与会话管理 §5 会话广播]
> 「遇到阻塞？→ [教程 10-SSE流式通信 §代理与缓存]、[教程 24 §服务端游标]」
...
离线可用 ≠ 假装在线。三件事必须做到：① 全局 `useOnlineStatus`（`navigator.onLine` + `online/offline` 事件）驱动"离线模式"横幅；② 离线时输入框不禁用而是转投离线队列（§5）；③ 缓存数据打上"可能过期"标记（SWR 的 `offline: true` 响应让 Query 层把 `dataUpdatedAt` 判为陈旧）。**宁可诚实地旧，不要自信地错**——这与后端"降级要显式"的原则同构（[教程 30-容错与弹性设计 §降级]）。
...
重发的消息可能在服务端已处理（发送成功但响应丢失）。**幂等责任在后端**：`ChatRequest` 增加 `clientMessageId`，后端按 `(sessionId, clientMessageId)` 去重——重复请求返回同一轮次的事件流或"已处理"响应，不再触发第二次 LLM 调用（Token 白烧是成本事故，[教程 27-成本治理与Token计量 §2]）。前端只保证：**不猜结果、失败保留、成功才删**。这与 04 的"事件 id 去重"构成完整的双向幂等（服务端对客户端消息幂等，客户端对服务端事件幂等）。
...
1. **离线壳**：DevTools Application → 勾 Offline → 刷新页面：应用壳与上次缓存会话列表可见、显示离线横幅、输入转入队列（UI 显示"已排队 N 条"）。
2. **SSE 回归（关键）**：SW 激活后完整跑一遍 04 §7 验收清单（断线 30 秒恢复、去重、取消）——**SW 上线必须带着断线恢复的回归测试**，这是 ADR 04-14 的验证条款。
3. **双页签**：开两个页签，A 新建会话→B 侧边栏 1 秒内出现；A 删除→B 消失。
4. **双设备**：两台机器（或两个浏览器 profile + 不同 token）同账号：A 发消息→B 进入同一会话≤3 秒可见；B 在轮次进行中进入能看到进行中的流（追赶+接续）。
5. **离线队列**：Offline 下发 5 条→关闭浏览器→重开→恢复在线→按序自动重发；构造"发送成功但响应丢失"（后端测试端点模拟超时）验证幂等（后端消息数=5 而非 10）。
6. **桌面**：`tauri build` 产物冷启动 <2s，SSE 正常（WebView 不缓冲——若发现缓冲，检查 Tauri 的 WebView 配置与系统代理）。
