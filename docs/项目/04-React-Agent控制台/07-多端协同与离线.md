...
>
> **前置阅读**：已完成 06；[教程 03-React前端与AgenticUI/02-React与SSE流式UI §3 断线重连]、[教程 04-企业级架构主干/04-多页面流式响应与会话管理 §5 会话广播]
> 「遇到阻塞？→ [教程 02-SpringAI核心机制/06-SSE流式通信 §代理与缓存]、[教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §服务端游标]」
...
Service Worker 拦截全局 fetch——**一旦无脑缓存，SSE 与 POST 全部被破坏**（代理缓存是 SSE 的头号杀手，[教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §10.1 Nginx proxy_buffering off] 的浏览器版翻版）。策略必须按资源类型分流：
...
离线可用 ≠ 假装在线。三件事必须做到：① 全局 `useOnlineStatus`（`navigator.onLine` + `online/offline` 事件）驱动"离线模式"横幅；② 离线时输入框不禁用而是转投离线队列（§5）；③ 缓存数据打上"可能过期"标记（SWR 的 `offline: true` 响应让 Query 层把 `dataUpdatedAt` 判为陈旧）。**宁可诚实地旧，不要自信地错**——这与后端"降级要显式"的原则同构（[教程 04-企业级架构主干/10-容错与弹性设计 §降级]）。
...
3. **触发时机用"进入 + 低频轮询 + 广播信号"三合一**：进入会话必追、`refetchInterval` 30 秒兜底、页签间 BroadcastChannel 收到信号立刻追。**不做长连接推送**（那需要后端广播基建，[教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §5]——参考后端刻意不引入，属项目 14 的微服务主题）。
...
export interface OutboxItem {
  clientMessageId: string;    // 幂等键（UUID），与后端 (sessionId, clientMessageId) 联合去重
  sessionId: string;
  message: string;
  queuedAt: number;
}

/** 最小事务封装：单 store 读/写，Promise 化（错误沿 req.onerror / transaction.onabort 传播）。 */
...
重发的消息可能在服务端已处理（发送成功但响应丢失）。**幂等责任在后端**：`ChatRequest` 增加 `clientMessageId`，后端按 `(sessionId, clientMessageId)` 去重——重复请求返回同一轮次的事件流或"已处理"响应，不再触发第二次 LLM 调用（Token 白烧是成本事故，[教程 04-企业级架构主干/07-成本治理与Token计量 §2]）。前端只保证：**不猜结果、失败保留、成功才删**。这与 04 的"事件 id 去重"构成完整的双向幂等（服务端对客户端消息幂等，客户端对服务端事件幂等）。
...
| 双端同步 ≤3s | 游标追赶生效 | 教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §5（广播，本项目用轮询+追赶替代） | 待实测打钩 |
