...
>
> **前置阅读**：已完成 07；[教程 03-React前端与AgenticUI/03-Agentic-UI设计 §3 核心组件设计]
> 「遇到阻塞？→ [教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3 组件与事件]、[附录 13-Agent交互设计/00-Agent用户体验设计 §流式体验]」
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四个痛点与三组交付对应 | ①↔Announcer、②↔roving tabindex+焦点管理、③④↔i18n/逻辑属性/tokens |
| 2 | 后端零改动的理由成立 | ErrorEvent.code+params 契约在 00 §6.5 已预留 |
...
| 2 | 读屏可用 | VoiceOver/NVDA 走查主流程：发送→工具执行→审批→完成，播报完整不重复；token 期间不逐字播报（全程播报条数 ≤ 12 条里程碑摘要；token 期间播报延迟 ≤ 2s/条；无逐字复读） |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 六条验收均可判定 | 键盘走查/读屏走查/4.5:1/CI 扫描 0/RTL 镜像/兜底文案，各有工具口径 |
| 2 | 对比度与 Lighthouse 阈值量化 | AA 4.5:1（正文）3:1（控件）、a11y ≥95 分 |
...
两条最容易漏的规则：① **新内容出现不抢焦点**（流式输出、工具卡片弹出时焦点留在输入框——抢焦点会让正在打字的用户"丢字"）；② **对话框关闭必回焦**（回到打开它的按钮，否则焦点掉回 body，读屏用户瞬间"迷路"）。审批卡是两者的混合体：**视觉上不抢焦（浮层），播报上 assertive，键盘上 `Ctrl+.` 之外的 Tab 顺序自然到达**——HITL 的可达性直接决定审批是否真的"human"（[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §前端形态]）。

### 3.4 本节测试与验证（a11y 三大战役）

> **前置**：Announcer、roving tabindex、焦点管理已落进组件；macOS VoiceOver（Cmd+F5）或 NVDA、axe DevTools 可用。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 拔掉鼠标，按 §3.2 快捷键表走完整主流程（切会话→发消息→Ctrl+. 停止→↑↓ 展开工具卡→审批拒绝+备注→Esc） | 全程无一步卡死，焦点始终可感知 |
| 2 | VoiceOver 开启后跑一轮带工具+审批的对话 | token 期间不逐字播报（至多每 2s 一条里程碑摘要）；approval_request 触发 assertive 即时播报；round_end 播报完成摘要 |
| 3 | 检查 DOM | 全应用只有一个 role=status 与一个 role=alert；打字机区无 aria-live |
| 4 | 流式输出中焦点保持在输入框继续打字 | 无丢字、无焦点抢夺 |
| 5 | 打开审批卡再 Esc 关闭 | 焦点回到打开它的触发元素（不是 body） |
| 6 | axe DevTools 扫描主界面 | violations = 0 |
| 7 | Lighthouse a11y | ≥95 分 |

**失败排查**：读屏复读 → 打字机区误设 aria-live（回查铁律 1）；播报互相踩踏 → 出现了第二个 live 节点；Esc 后迷路 → 对话框关闭未记录触发元素回焦；axe 报对比度 → 转入 §5.2 门禁修复。
...
/** 订阅语言/方向切换（React 侧在根部组件用 useEffect 订阅，setState 触发全树重渲染）。 */
export function onLocaleChanged(listener: () => void): () => void {
  localeListeners.add(listener);
  return () => localeListeners.delete(listener);
}

/** 通知所有订阅者：语言与 dir 已切换（由 setLocale 内部调用）。 */
function emitLocaleChanged(): void {
...
### 4.4 本节测试与验证（i18n 三件套）

> **前置**：i18n/ 三语言包落盘、t() 接入全部组件、ErrorMapper 落盘、CI 扫描脚本就绪（或先本地跑等价正则）。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 切换 zh/en/ar | 界面即时切换（无刷新）；`<html lang>` 与 `dir` 同步变化（阿语 dir=rtl） |
| 2 | 切到超长伪语言（文案加长 40% 的假串） | 布局不破版、无溢出（提前暴露德语式长词） |
| 3 | 切阿拉伯文 | 侧边栏/气泡镜像；`<pre>` 与进度条保持 LTR；无横向滚动条 |
| 4 | t('不存在的key') | 返回英文兜底，再不行返回 key 本身（兜底链生效） |
| 5 | 触发一个后端错误（已登记 code，如 RATE_LIMITED） | 显示当前语言的本地化文案 + 参数（重试秒数）替换正确 |
| 6 | 触发一个未登记 code | 显示英文兜底文案 + 折叠的原始 message 可复制 |
| 7 | CI（或本地）跑裸文案扫描正则 | JSX 中文/英文裸字符串 0 发现 |

**失败排查**：切语言部分不更新 → 该组件直接写了字符串未走 t()；占位符不替换 → params key 与 {name} 拼写不一致；RTL 破版 → 还有物理属性（margin-left/text-align:left）残留，按 §4.2 表替换。
...
@media (prefers-color-scheme: dark) {    /* 未显式选择时跟随系统：与 [data-theme="dark"] 同组变量 */
  :root:not([data-theme]) {
    --color-bg: #0f1115;
    --color-text-primary: #e8e8e8;       /* 对比度 14:1，双双超过 AA 4.5:1 */
    --color-accent: #6f9bff;
...
> **前置**：theme.css 双主题 tokens、ThemeToggle 三态、FOUC 内联脚本落盘。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | ThemeToggle 切 light/dark/system | `data-theme` 切换，组件零改动即换肤；system 跟随 `prefers-color-scheme` |
| 2 | 选 dark 后刷新页面 | 首帧即暗色（FOUC 内联脚本生效，无白屏闪） |
| 3 | DevTools 对比度检查器逐项过语义色（明/暗两套） | 正文 ≥4.5:1、控件（含工具卡状态色）≥3:1 |
| 4 | 切主题后跑一轮流式输出 | 打字机光标、`<pre>` 代码高亮在暗色下可见正常 |

**失败排查**：刷新闪白 → FOUC 脚本未内联在 `<head>` 或时序晚于首帧；暗色下状态色看不清 → 直接调 tokens 而非组件内色值；system 不跟随 → matchMedia 监听缺失。
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 两条 ADR 均含备选与可回滚路径 | 04-17→移除 Announcer 即回退、04-18→只改 t() 层迁 i18next |
| 2 | 能复述 04-17 的频率论证 | 渲染 60Hz vs 播报 0.5Hz，三个数量级不可同通道 |

---

## 7. 全篇回归验证

> 章节级验证已前移（§3.4 a11y / §4.4 i18n / §5.3 主题）。以下为整体验收对照，依赖各节已 PASS。
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 收束逻辑成立 | 四条进阶线（性能/多端/a11y/i18n）全部实测闭环后才转收束 |
| 2 | 前沿方向归属明确 | 生成式 UI 与长任务范式指向 10 篇 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四行交付物对应三个章节验证 | §3.4/§4.4/§5.3 覆盖 a11y/i18n/错误/主题 |
| 2 | §7 验收对照六行全部打钩 | 全部实测通过后进阶篇收口，才进入 09 台账篇 |
