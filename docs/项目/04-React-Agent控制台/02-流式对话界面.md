# 项目 04：React Agent 控制台 — 02-多会话与三层状态

> **定位**：v1 的"刷新失忆 + 单会话枷锁 + useState 拥挤"痛点驱动本次迭代：引入会话模型、Zustand + TanStack Query + 本地流式层的三层状态架构。完成后用户拥有完整的多会话对话体验。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 03-React前端与AgenticUI/01-React状态管理 §5 三层状态模型]、[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §3 Zustand]、[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §4 TanStack Query]」
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四个 v1 痛点与本迭代模块一一对应 | ①↔SessionStore+落库、②↔sessionId 路由化、③↔reducer、④↔Query 无限分页 |
| 2 | 能说清三层状态各管什么 | Zustand=客户端全局、Query=服务端真相、reducer=本地高频流式 |

## 2. 目标与量化验收

| # | 目标 | 验收 |
|---|------|------|
| 1 | 会话可持久 | 创建会话→对话→刷新页面→历史完整恢复（后端内存存储 + Query 拉取）；刷新后历史首屏渲染 ≤ 1s（本地缓存命中，localhost 无网络瓶颈） |
| 2 | 切换无串流 | 会话 A 流式中切到 B，B 界面无 A 内容；回切 A 时流已终止且 UI 明确；切换操作响应延迟 < 100ms，Network 面板无残留的 `/api/chat` 流 |
| 3 | 上下文隔离 | 两个会话交替对话，各自上下文独立（后端 sessionId 路由生效）；交替 ≥ 5 轮交叉核对，回答内容与各自会话历史一致不串 |
| 4 | 删除正确 | 删除当前会话跳转会话列表；无悬挂请求报错；删除后 Network 无 4xx/5xx 残留请求 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 六条验收可观察 | 每条都能用浏览器操作 + DevTools（Network/Performance）判定 |
| 2 | 验收与痛点闭环 | 痛点①↔验收1、②↔验收2/3、④↔验收6（分页与缓存） |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 能对照图说出 v2 的五个组件各自消费哪层状态 | Sidebar→Z+Q、MessageHistory→Q、LiveStreamArea→本地流式层 |
| 2 | 能说清虚线边 `round_end 失效 messages` 的含义 | 流结束后本地层反向通知服务端层重拉，保证服务端是真相源 |
...
> **绑定风格说明**：`create(@RequestBody(required = false) Map<String, String> body)` 保留显式绑定是刻意的——前端 `api.createSession` 以 JSON body 提交 `{"title": ...}`（可选），隐式参数绑定（`public Session create(String title)`）只覆盖查询参数/表单，无法解析 JSON body。类级 `@RequestMapping("/api/sessions")` + 方法级 `@GetMapping/@PostMapping/@DeleteMapping` 的标准 Controller 形态，字段注入对齐 demo01 `src` 的 `ChatController`。
...
> **绑定风格说明**：`chat(@RequestBody ChatRequest request)` 保留显式绑定是刻意的——`/api/chat` 走 POST + JSON body（`sessionId`/`message`），隐式参数绑定（`public String chat(String prompt)`）只适用于 GET 查询参数/表单，无法解析 JSON body。SSE 契约要求结构化请求体，故保留类级 `@RequestMapping("/api")` + 方法级 `@PostMapping` 的标准 Controller 形态（同 01 §4.3）。
...
> **分类纪律自查**（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §1]）：`activeSessionId` 在哪？——它由 URL 路由参数承载（`/sessions/:id`），不进 store。URL 是天然的全局状态存储，还能免费获得深链接与浏览器前进后退。
...
> **验收铁律**：在会话 A 流式输出中切换到会话 B——A 的 token 绝不允许出现在 B 的界面上。失败模式几乎都是"闭包捕获了旧 sessionId 的回调仍在写入新状态"，解法就是上述"依赖驱动重建（send 依赖 sessionId）+ 切换时 abort + RESET"三件套。「遇到阻塞？→ [教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3.2 useEffect 依赖与清理]、[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §5.1 会话切换纪律]」
...
> **为什么失效而不是本地拼接**：服务端是真相源——后端可能在流结束后做记忆写入、消息规整、配额扣减（[教程 00-基础与核心/04-记忆与会话管理]）。本地流只是渲染过程；`round_end` 后 `invalidateQueries` 重拉保证刷新/多页签一致性。代价（一次额外请求）换来一致性，值得。

### 4.11 前端：`main.tsx`（重贴完整）

```tsx
// main.tsx —— 应用入口（v2 与 v1 相同：StrictMode + createRoot 渲染 App，无需改动）
...
### 4.12 本节测试与验证（v2 完整代码）

> **前置**：v1 工程可运行（01 §5.1 已 PASS）；本节 11 个文件已按目录落盘。

**材料**：后端 `mvn compile`；前端 `npx tsc --noEmit`；curl 四条（对应 SessionController 的 CRUD/历史）；浏览器 `http://localhost:5173`。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn compile` | BUILD SUCCESS；MessageChatMemoryAdvisor / MessageWindowChatMemory / InMemoryChatMemoryRepository 引用全部解析（与附录 05-00 基准一致） |
| 2 | `npx tsc --noEmit` | 零类型错误（models.ts / api.ts / uiStore.ts / useSessions.ts / useChatStream v2 / 五个组件） |
| 3 | `curl -X POST localhost:8081/api/sessions -H 'Content-Type: application/json' -d '{"title":"测试"}'` | 返回 `{"id":"s-1","title":"测试","createdAt":...}` |
| 4 | `curl localhost:8081/api/sessions` | 返回数组含刚建的会话 |
| 5 | `curl -N -X POST localhost:8081/api/chat -H 'Content-Type: application/json' -d '{"sessionId":"s-1","message":"你好"}'` | token 流末尾 round_end；随后 `curl localhost:8081/api/sessions/s-1/messages` 返回 user + assistant 两条记录 |
| 6 | `curl -X DELETE localhost:8081/api/sessions/s-1` 后再 GET messages | 204/空；历史随会话删除 |

**失败排查**：Advisor 类找不到 → Spring AI 版本不是 2.0.0 或 import 错（应为 `org.springframework.ai.chat.client.advisor.MessageChatMemoryAdvisor`）；messages 返回空 → appendMessage 落在 `Mono.defer` 里，确认流被完整消费而非中途取消；DELETE 后列表仍显示 → 前端 useDeleteSession 的 invalidateQueries 没执行。
...
- **取舍理由**：token 每秒 20-80 个，进全局 store 会连坐全站渲染（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §1 原则]）。reducer 是"连接状态机"的天然载体（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §2.2]），且与后端状态图同构

### ADR 04-04：activeSessionId 放 URL 而非 Zustand
- **决策**：`/sessions/:id` 路由参数承载当前会话；`key={sessionId}` 强制 remount
- **取舍理由**：URL 免费提供深链接、浏览器前进后退、刷新保持当前会话（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §3.3]）；放 store 则刷新丢状态

### ADR 04-05：流结束后 invalidate 而非本地拼接
- **决策**：`round_end` 事件触发 `invalidateQueries(['session-messages', id])`
- **取舍理由**：服务端是真相源，本地拼接在多页签场景必然不一致（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §5 数据流 3]）

### 5.1 本节核对（ADR 演进决策）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 三条 ADR 均能在 §4 代码中指出落点 | 04-03→reducer、04-04→路由参数+key、04-05→invalidateQueries |
| 2 | 能复述 04-03 的量化理由 | token 每秒 20-80 个，进全局 store 连坐全站渲染 |

## 6. 全篇回归验证

> 章节级验证已前移（§4.12 完整代码）。以下为浏览器端整体验收，依赖 §4.12 已 PASS。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 会话可持久 | 创建→对话→刷新→历史完整恢复，首屏 ≤ 1s（§2 验收 1） | 待实测打钩 |
| 切换无串流 | A 流式中切 B 无 A 内容，回切 A 流已终止且响应 < 100ms（§2 验收 2） | 待实测打钩 |
| 上下文隔离 | 两会话交替 ≥ 5 轮交叉核对一致不串（§2 验收 3） | 待实测打钩 |
| 删除正确 | 删当前会话跳列表、无悬挂请求、Network 无 4xx/5xx（§2 验收 4） | 待实测打钩 |
| 资源不泄漏 | 切换 10 次无内存增长（§2 验收 5） | 待实测打钩 |
| 缓存生效 | 30 秒内重复切换不重复请求历史（§2 验收 6） | 待实测打钩 |
| 后端 CRUD 全通 | §4.12 六条 curl（create/list/chat/messages/delete）全部 PASS | 待实测打钩 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 痛点可在运行中的 v2 上复现 | 后端流里抓到 tool/thought 事件，但 UI 上无任何过程展示 |
| 2 | 痛点指向迭代二范围 | 是"事件未接 UI"，不是后端不发事件 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 五行交付物均能对应 §4 真实文件 | 后端 3 个、前端 7 个文件逐一指认 |
| 2 | §6 回归全部 PASS | 六条验收逐条通过后才进入 03 篇 |
