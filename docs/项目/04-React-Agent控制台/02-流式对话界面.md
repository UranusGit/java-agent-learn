...
| 3 | `curl -X POST localhost:8080/api/sessions -H 'Content-Type: application/json' -d '{"title":"测试"}'` | 返回 `{"id":"s-1","title":"测试","createdAt":...}` |
| 4 | `curl localhost:8080/api/sessions` | 返回数组含刚建的会话 |
| 5 | `curl -N -X POST localhost:8080/api/chat -H 'Content-Type: application/json' -d '{"sessionId":"s-1","message":"你好"}'` | token 流末尾 round_end；随后 `curl localhost:8080/api/sessions/s-1/messages` 返回 user + assistant 两条记录 |
| 6 | `curl -X DELETE localhost:8080/api/sessions/s-1` 后再 GET messages | 204/空；历史随会话删除 |
