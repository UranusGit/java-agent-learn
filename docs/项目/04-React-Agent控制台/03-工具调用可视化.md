...
| 2 | `curl -N -X POST localhost:8080/api/chat -H 'Content-Type: application/json' -d '{"sessionId":"s-t","message":"查一下 ORD-001 订单"}'` | 事件顺序：round_start → thought_start/delta/end → tool_start(query_order) → tool_progress → tool_end(success, resultPreview="已发货，金额 199.0 元", durationMs>0) → token 流 → round_end(stop) |
...
| 4 | 从 3 的响应里取 approvalId，`curl -X POST localhost:8080/api/approvals/ap-5 -H 'Content-Type: application/json' -d '{"approved":true,"comment":"ok"}'` | HTTP 202，body 含 `"resumeEndpoint":"/api/chat/resume/r-xxxx"` |
| 5 | `curl -N -X POST localhost:8080/api/chat/resume/r-xxxx -H 'Content-Type: application/json' -d '{"sessionId":"s-t"}'` | approval_resolved → token 流 → round_end(stop) |
