...
>
> 「遇到阻塞？→ [教程 18-Agentic-UI设计 §2 事件协议]、[教程 18 §3 核心组件]、[教程 18 §5 HITL 挂起恢复]、[教程 19-流式工具调用与事件协议]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
| 6 | 参数安全 | 工具参数显示后端脱敏后的 args（前端不自行脱敏——安全责任在后端，[附录 09-Agent安全深度/02-数据泄露防护]） |
 ...
 * 生产环境由 ToolCallingManager 装饰器从真实工具意图中拦截生成（[教程 19 §3.2]）。
 */
@Service
public class ChatService {
    ...
    private Flux<AgentEvent> tailFlow(String sessionId, String roundId, String message) {
        // 演示规则：真实场景由 ToolCallingManager 拦截生成 ToolStart（教程 19 §3.2）
    ...
    private Flux<AgentEvent> toolFlow(String sessionId, String roundId, String name, Map<String, Object> args) {
        ...
        return Flux.concat(
                ...
                Flux.fromCallable(() -> new ToolEnd(nextId(), now(), toolCallId, "success",
                        ...
                        .subscribeOn(Schedulers.boundedElastic())   // 阻塞工具隔离到弹性线程池（教程 19 §4）
        );
    }
...
public class ApprovalController {
    ...
    public ResponseEntity<Map<String, String>> respond(@PathVariable String approvalId,
        ...
        // 202 Accepted + resumeEndpoint：前端据此开新流恢复（教程 18 §5）
...
> **契约纪律**：本文件是协议唯一来源。后端新增事件类型时，这里加一个联合分支 + reducer 加一个 case（漏写编译报错——穷尽检查的价值，[教程 15 §4.2]）。未知 type 的防御保留在 parseSSE 层（协议版本化降级，[教程 18 §6]）。
...
```ts
// reducer/agentRoundReducer.ts —— 与后端状态图同构（教程 18 §2.3）
...
export type RoundAction =
  ...
  | { type: 'TOKEN_BATCH'; content: string }        // rAF 批量缓冲后的 token 块（教程 17 §4.2）
  | AgentEvent;
...
export function useAgentRound(sessionId: string) {
  ...
  const respondApproval = useCallback(async (approved: boolean, comment: string) => {
    ...
    if (approved && res.resumeEndpoint) {
      // 开新流从 Checkpoint 恢复：后端从挂起点继续生成（教程 18 §5）
      const controller = new AbortController();
      abortRef.current = controller;
      await streamChat(
...
```tsx
// components/ThoughtBlock.tsx —— 默认折叠的思考块（教程 18 §3.1）
...
```tsx
// components/ToolCard.tsx —— 过程透明的核心（教程 18 §3.2）
...
```tsx
// components/ApprovalCard.tsx —— HITL 前端形态（教程 18 §3.3）
import { useState } from 'react';
...
- **取舍理由**：**频率差异决定通道**——token 每秒 20-80 个必须背压（[教程 17 §4]）；工具事件每轮几个，缓冲反而引入"卡片延迟出现"的怪异感（违反 3 秒规则）。背压施加在高频源上，不能一刀切
...
- **取舍理由**：SSE 挂几小时等审批不现实（连接成本、代理超时）（[教程 18 §5]）。前端用 `resumeEndpoint` 响应字段开新流，无需自己拼 URL

### ADR 04-08：前端不做脱敏
- **决策**：工具参数展示直接用后端发来的 args
- **取舍理由**：安全责任必须在后端单一化（[附录 09-Agent安全深度/02-数据泄露防护]）。前端过滤是纵深防御的补充，绝不能成为依赖——浏览器里的代码可被绕过，直接看网络包即可
