# 项目 04：React Agent 控制台 — 03-工具调用可视化与 Agentic UI

> **定位**：把 v2 静默丢弃的 thought/tool 事件接到 UI 上——思考块、工具卡片（含并行组）、审批卡与挂起恢复。完成后 Agent 的执行过程对用户完全透明。本文给出**完整可手写代码**（一行不省略）。
>
> 「遇到阻塞？→ [教程 03-React前端与AgenticUI/03-Agentic-UI设计 §2 事件协议]、[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §3 核心组件]、[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5 HITL 挂起恢复]、[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 五个 v2 痛点与本迭代交付一一对应 | ①②↔工具卡片、③↔ThoughtBlock、④↔审批流、⑤↔agentRoundReducer |
| 2 | 演示规则有显式标注 | ChatService 中"消息含订单/退款即触发"被标注为演示规则 |
...
| 3 | 思考可见 | ThoughtBlock 默认折叠，展开可读推理过程（thought_delta 全量拼接无丢字，展开渲染 ≤ 100ms） |
| 4 | 审批流 | 卡片出现→点允许→UI 转 running→恢复流继续输出；拒绝→不恢复 |
| 5 | 截断显式 | 预算截断的回答 FinishBanner 显示警告，非静默结束 |
| 6 | 参数安全 | 工具参数显示后端脱敏后的 args（前端不自行脱敏——安全责任在后端，[附录 08-Agent安全深度/02-数据泄露防护]）；比对 Network 原始响应与 UI 渲染，敏感字段（如金额、账号）零明文直接展示，脱敏一致性 100% |

### 2.1 本节核对（目标与量化验收）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 六条验收均可浏览器观察 | 卡片状态迁移、并行排列、折叠展开、审批交互、截断横幅、参数展示 |
| 2 | 演示触发词覆盖验收路径 | "订单"触发低风险工具、"退款"触发审批，与 §7 回归场景直接对应 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 能说清 v2→v3 的两条重构线 | 协议（token 三事件→全量 12 事件）与状态（useState→轮次状态机） |
| 2 | 能说清审批链路的四步 | approval_request → SSE 关闭（awaiting_approval）→ REST 审批 → resumeEndpoint 开新流 |
 ...
 * 生产环境由 ToolCallingManager 装饰器从真实工具意图中拦截生成（[教程 02-SpringAI核心机制/00-SSE流式通信 §3.2]）。
 */
@Service
public class ChatService {
    ...
    /** 审批通过后从挂起点恢复：继续生成（新流，教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5）。 */
    ...
    private Flux<AgentEvent> tailFlow(String sessionId, String roundId, String message) {
        // 演示规则：真实场景由 ToolCallingManager 拦截生成 ToolStart（教程 02-SpringAI核心机制/00-SSE流式通信 §3.2）
    ...
    private Flux<AgentEvent> toolFlow(String sessionId, String roundId, String name, Map<String, Object> args) {
        ...
        return Flux.concat(
                ...
                Flux.fromCallable(() -> new ToolEnd(nextId(), now(), toolCallId, "success",
                        ...
                        .subscribeOn(Schedulers.boundedElastic())   // 阻塞工具隔离到弹性线程池（教程 02-SpringAI核心机制/00-SSE流式通信 §4）
        );
    }
...
public class ApprovalController {
    ...
    public ResponseEntity<Map<String, String>> respond(@PathVariable String approvalId,
        ...
        // 202 Accepted + resumeEndpoint：前端据此开新流恢复（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5）
...
> **绑定风格说明**：`chat`/`resume` 两个 SSE 端点保留 `@RequestBody` 显式绑定是刻意的——两者都走 POST + JSON body（`ChatRequest`/`ResumeRequest`），隐式参数绑定（`public String chat(String prompt)`）只适用于 GET 查询参数/表单，无法解析 JSON body。类级 `@RequestMapping("/api")` + 方法级 `@PostMapping` 的标准 Controller 形态，字段注入对齐 demo01 `src` 的 `ChatController`。

### 4.6 本节测试与验证（参考后端 v3）

> **前置**：v2 后端可运行（02 §4.12 已 PASS）；本节 5 个文件落盘；`@Tool`/`@ToolParam` import 为 `org.springframework.ai.tool.annotation`。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn clean compile` | BUILD SUCCESS；ChatService 的 12 种事件 record 与 sealed permits 全部解析 |
| 2 | `curl -N -X POST localhost:8081/api/chat -H 'Content-Type: application/json' -d '{"sessionId":"s-t","message":"查一下 ORD-001 订单"}'` | 事件顺序：round_start → thought_start/delta/end → tool_start(query_order) → tool_progress → tool_end(success, resultPreview="已发货，金额 199.0 元", durationMs>0) → token 流 → round_end(stop) |
| 3 | 同端点发送 `message:"我要退款 ORD-002"` | tool_start(refund_order) → approval_request(riskLevel=high) → round_end(finishReason="awaiting_approval")，流随后正常关闭 |
| 4 | 从 3 的响应里取 approvalId，`curl -X POST localhost:8081/api/approvals/ap-5 -H 'Content-Type: application/json' -d '{"approved":true,"comment":"ok"}'` | HTTP 202，body 含 `"resumeEndpoint":"/api/chat/resume/r-xxxx"` |
| 5 | `curl -N -X POST localhost:8081/api/chat/resume/r-xxxx -H 'Content-Type: application/json' -d '{"sessionId":"s-t"}'` | approval_resolved → token 流 → round_end(stop) |
| 6 | 审批拒绝（approved=false） | 返回 `{"status":"rejected"}`；再次 resume 同一 roundId 报"未知挂起轮次"（一次性消费） |

**失败排查**：找不到 `@Tool` 注解 → 包名写成旧版 `org.springframework.ai.tool.annotation` 之外（以附录 05 基准为准）；tool_end 无 resultPreview → executeTool 的 switch 分支没命中 name；resume 报未知轮次 → approvalStore.resolve 已消费或 roundId 抄错；事件 id 不单调 → 多实例/重启导致 AtomicLong 归零（内存版已知边界）。
...
> **契约纪律**：本文件是协议唯一来源。后端新增事件类型时，这里加一个联合分支 + reducer 加一个 case（漏写编译报错——穷尽检查的价值，[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §4.2]）。未知 type 的防御保留在 parseSSE 层（协议版本化降级，[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §6]）。
...
```ts
// reducer/agentRoundReducer.ts —— 与后端状态图同构（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §2.3）
...
export function useAgentRound(sessionId: string) {
  ...
  const respondApproval = useCallback(async (approved: boolean, comment: string) => {
    ...
    if (approved && res.resumeEndpoint) {
      // 开新流从 Checkpoint 恢复：后端从挂起点继续生成（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5）
      const controller = new AbortController();
      abortRef.current = controller;
      await streamChat(
...
```tsx
// components/ThoughtBlock.tsx —— 默认折叠的思考块（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §3.1）
...
```tsx
// components/ToolCard.tsx —— 过程透明的核心（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §3.2）
...
```tsx
// components/ApprovalCard.tsx —— HITL 前端形态（教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §3.3）
import { useState } from 'react';
...
> **前置**：§4.6 后端验证已 PASS；本节文件落盘；浏览器打开 `http://localhost:5173`。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `npx tsc --noEmit` | 零类型错误；reducer 的 switch 对 12 种事件穷尽（漏 case 编译报错） |
| 2 | 发送"查一下 ORD-001 订单" | 出现折叠的思考块；ToolCard 实时 running→success，参数 JSON 与耗时可见 |
| 3 | 展开思考块 | 能读到 thought_delta 拼接的完整内容 |
| 4 | 发送"我要退款 ORD-002" | ApprovalCard 出现（risk-high 样式），输入框禁用/显示停止（status=awaiting_approval），无 answerText |
| 5 | 审批卡点"允许" | Network 出现 POST /api/approvals/{id} 得 202，随后 POST resume 端点开新流，UI 转 running 并继续打字 |
| 6 | 重复 4 后点"拒绝" | 卡片消失，无新请求、无恢复流 |
| 7 | 审批流结束后刷新页面 | 历史里能看到落库的最终消息（invalidate + 落库链路） |
| 8 | DevTools Performance 录制一次工具轮 | React 提交次数远小于 token 帧数（rAF 批量 + React.memo 生效） |

**失败排查**：卡片不出现 → reducer 漏 tool_start case 或 wireEvents 未 dispatch；点允许无恢复 → 响应体 resumeEndpoint 字段名抄错；拒绝后仍恢复 → respondApproval 里 approved 分支判断反了；思考块闪烁重渲染 → ThoughtBlock 未用稳定 key（thoughtId）。
...
- **取舍理由**：**频率差异决定通道**——token 每秒 20-80 个必须背压（[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §4]）；工具事件每轮几个，缓冲反而引入"卡片延迟出现"的怪异感（违反 3 秒规则）。背压施加在高频源上，不能一刀切
...
- **取舍理由**：SSE 挂几小时等审批不现实（连接成本、代理超时）（[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5]）。前端用 `resumeEndpoint` 响应字段开新流，无需自己拼 URL
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 三条 ADR 落点可指认 | 04-06→wireEvents 双通道、04-07→approvalFlow+resumeEndpoint、04-08→ToolCard 直接渲染 args |
| 2 | 能复述 04-06 的频率依据 | token 20-80/秒缓冲、工具事件每轮几个直进 |

## 7. 全篇回归验证

> 章节级验证已前移（§4.6 后端 / §5.8 前端）。以下为整体验收，依赖两节已 PASS。
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 工具透明 | 卡片实时 running→success，参数与耗时可见（§2 验收 1） | 待实测打钩 |
| 并行结构 | 同 parallelGroup 卡片同行排列，真实反映并行（§2 验收 2） | 待实测打钩 |
| 思考可见 | ThoughtBlock 默认折叠，展开 ≤ 100ms 且无丢字（§2 验收 3） | 待实测打钩 |
| 审批流 | 允许→running→恢复流继续输出；拒绝→不恢复、无新流（§2 验收 4） | 待实测打钩 |
| 截断显式 | FinishBanner 显示截断警告非静默（§2 验收 5） | 待实测打钩 |
| 参数安全 | UI 展示与后端响应比对，脱敏一致性 100%（§2 验收 6） | 待实测打钩 |
| 后端事件序 | §4.6 六条 curl 全过（工具流/审批流/恢复/拒绝幂等） | 待实测打钩 |

## 8. 当前痛点（驱动迭代三）

过程透明已达成，但**弱网体验**仍是一块短板：地铁里断线 30 秒，流就死在半截，只能整轮重发（后端 Token 白烧）；usage 只在轮末展示，用户对成本无感知。→ [04-多会话管理与断线恢复.md](04-多会话管理与断线恢复.md)

### 8.1 本节核对（当前痛点）

| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 痛点可在 v3 上复现 | DevTools Network 里把 /api/chat 设为 offline，流中断且只能整轮重发 |
| 2 | 痛点归属迭代三范围 | 断线恢复与用量面板正是 04 篇主题 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四行交付物均能对应本篇真实文件 | 后端 5 个 + 契约 1 个 + 状态 2 个 + 组件 6 个 |
| 2 | §7 回归七条全部 PASS 后才进入 04 篇 | 特别是审批允许/拒绝双路径 |
