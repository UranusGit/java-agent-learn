...
> **定位**：项目 04 全部架构决策的集中台账——把散落在 00-08 各篇的 19 条 ADR（04-00 ~ 04-18）按统一格式（上下文/决策/备选/取舍/可回滚）汇总，并补录奠基决策（传输层 SSE vs WebSocket 的回溯补录）。用途：① 新成员 15 分钟读懂"这个前端为什么长这样"；② 每条决策都有回滚路径——**架构师改主意的成本，是架构能力的直接度量**。格式与方法论锚定 [附录 08-架构决策方法论/00-ADR架构决策记录]。
>
> **前置阅读**：建议读完 01-08 再看本篇（决策卡片大量引用迭代上下文）；急于了解全貌也可先读 §3 台账总表再按需回查。
> 「遇到阻塞？→ [附录 08-架构决策方法论/00-ADR架构决策记录 §格式]、[教程 45-Agent架构反模式与避坑指南]」
...
- **取舍**：**背压只施加在高频源上**——与后端 Reactor 对快生产者加 buffer 的判断标准同构（[附录 06-WebFlux与响应式编程/01-背压与流量控制]）。
...
- **取舍**：安全责任若分散在两端，等于每端都可被单独绕过；后端漏发敏感参数的正确响应是修后端 + 检测告警，不是给前端打补丁（[附录 09-Agent安全深度/02-数据泄露防护]）。
...
- **取舍**：成本数据参与预算控制（>90% 变红），漂移的数据比没有更危险（[教程 27-成本治理与Token计量 §2]）。
...
| 决策域 | ADR | 主要教程/附录锚点 |
|--------|-----|------------------|
| 传输与消费 | 04-00/02/09/10 | [教程 10-SSE流式通信]、[教程 17 §1/§3] |
| 契约与协议 | 04-01/18 | [教程 18 §2]、[教程 19-流式工具调用与事件协议] |
| 状态架构 | 04-03/04/05/11 | [教程 16-React状态管理 §3-§5]、[教程 27 §2] |
| 渲染性能 | 04-06/12/13 | [教程 17 §4]、[教程 38-Agent性能优化] |
| HITL | 04-07 | [教程 18 §5]、[教程 28-Human-in-the-Loop与审批流] |
| 安全 | 04-08 | [附录 09-Agent安全深度/02-数据泄露防护] |
| 多端离线 | 04-14/15/16 | [教程 24 §4-§5]、[教程 30-容错与弹性设计] |
| a11y/i18n | 04-17/18 | [附录 14-Agent交互设计/00-Agent用户体验设计] |
