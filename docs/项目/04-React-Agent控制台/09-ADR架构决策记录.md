...
> 「遇到阻塞？→ [附录 07-架构决策方法论/00-ADR架构决策记录 §格式]、[教程 08-架构师进阶/11-Agent架构反模式与避坑指南]」
...
| **上一版痛点是什么** | ① 决策散落 8 篇文档，找"SSE 为什么不是 WebSocket"要翻 00 §2 选型表 + 01 §4.3 + 教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 三处 ② 各篇 ADR 小节格式不一（有的只有决策+理由，没有备选与回滚）③ 前沿方向（10 篇）要引入新决策时，没有"已决策边界"可对照 |
...
- **可回滚**：协议变更走 `protocolVersion` 协商（04 §5.3 的 426 分支已是回滚的一半）——旧前端拒绝新事件类型的降级路径在 [教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §6]。
...
- **取舍**：**背压只施加在高频源上**——与后端 Reactor 对快生产者加 buffer 的判断标准同构（[教程 01-WebFlux与响应式编程/02-背压与流量控制]）。
...
- **取舍**：至少一次投递 + 消费端幂等——与后端消息队列的语义承诺完全一致（[教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §4]）。
...
- **可回滚**：若后端具备广播能力（[教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §5]），可改为审批完成即时推送——但参考后端刻意不做广播（那是项目 14 的微服务主题，项目独立铁律）。
...
- **取舍**：成本数据参与预算控制（>90% 变红），漂移的数据比没有更危险（[教程 04-企业级架构主干/07-成本治理与Token计量 §2]）。
...
| 决策域 | ADR | 主要教程/附录锚点 |
|--------|-----|------------------|
| 传输与消费 | 04-00/02/09/10 | [教程 02-SpringAI核心机制/00-SSE流式通信]、[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §1/§3] |
| 契约与协议 | 04-01/18 | [教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §2]、[教程 03-React前端与AgenticUI/04-流式工具调用与事件协议] |
| 状态架构 | 04-03/04/05/11 | [教程 03-React前端与AgenticUI/01-React状态管理 §3-§5]、[教程 03-React前端与AgenticUI/03-Agentic-UI设计 §2] |
| 渲染性能 | 04-06/12/13 | [教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §4]、[教程 08-架构师进阶/04-Agent性能优化] |
| HITL | 04-07 | [教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §5]、[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流] |
| 安全 | 04-08 | [附录 08-Agent安全深度/02-数据泄露防护] |
| 多端离线 | 04-14/15/16 | [教程 03-React前端与AgenticUI/00-React入门与现代前端工程 §4-§5]、[教程 04-企业级架构主干/10-容错与弹性设计] |
...
| 回归项 | 验收项 | 标准 | 状态 |
|--------|--------|------|------|
| 台账完整 | 条数准确 | 总表恰好 19 条（04-00 ~ 04-18），无缺号无跳号（§3.1 第 1 条） | 待实测打钩 |
| 出处一致 | 可追溯 | 每条 ADR 在出处篇目真实存在且一句话表述一致（§3.1 第 2 条） | 待实测打钩 |
| 卡片齐全 | 覆盖无遗漏 | 11 张卡覆盖 19 条 ADR，五字段逐卡齐全（§4.1 第 1-2 条） | 待实测打钩 |
| 回滚可复核 | 成本一致 | 每条"回滚成本"与出处篇"可回滚"字段不矛盾（§3.1 第 3 条） | 待实测打钩 |
| 演练可行 | 前提成立 | 两条回滚演练纸面推演 + 代码前提（传输收口/常量位置）成立（§7.1） | 待实测打钩 |
