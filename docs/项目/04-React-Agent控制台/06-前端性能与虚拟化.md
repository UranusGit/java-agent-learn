...
/** 前缀和二分：返回第一个使「前 i 项高度和」> offset 的索引（heightCache 前缀和单调递增）。 */
function findIndexForOffset(heights: number[], offset: number): number {
  let lo = 0, hi = heights.length;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (sumHeights(heights, 0, mid + 1) <= offset) lo = mid + 1;
    else hi = mid;
  }
  return Math.min(lo, heights.length);
}

/** 前缀和区间求和（未实测的高度按 0 计，行高缓存逐步回填）。 */
function sumHeights(heights: number[], start: number, end: number): number {
  let total = 0;
  for (let i = start; i < end; i++) total += heights[i] ?? 0;
  return total;
}

/** 行高实测回填：滚动后把真实 offsetHeight 写回缓存，消除估算误差。 */
function measureInto(cache: number[], index: number, el: HTMLDivElement | null): void {
  if (el) cache[index] = el.offsetHeight;
}
...
两条防线分工明确：**线上 RUM**（真实用户监控）回答"用户实际体验如何"（P75 分位数才作数，均值会骗人）；**CI 合成环境**（Lighthouse + size-limit）回答"这次提交有没有把指标弄坏"。`services/perf.ts` 是唯一出口，埋点与业务代码解耦——与后端"Observation 统一出口"同一分层直觉（[教程 02-SpringAI核心机制/04-结构化输出 §1]）。
