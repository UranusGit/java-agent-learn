...
```java
// 概念代码：路由缓存
package com.example.route.cache;
...
| 验收项 | 标准 | 状态 |
|--------|------|------|
| 会话级缓存命中 | 同会话二次起免 embed（路由层延迟承载，00 量化验收"语义 ≤30ms"） | ✅（§四 回归断言 1） |
| 语义缓存命中率 | 热门查询 ≥ 40% | ✅（§四 回归断言 2） |
| 配置变更缓存失效 | 版本变更清空两级缓存，无误用旧路由 | ✅（§四 回归断言 3） |
| 低置信不缓存 | 防错误复用 | ✅（§四 回归断言 4） |

> 本节核对（一句话）：四项验收拆自 [00-需求分析与架构设计](00-需求分析与架构设计.md) §三 量化验收（语义延迟 ≤30ms 由缓存承载）与本篇 §一 本迭代验收三项，逐条锚定 §四 回归断言编号，无"验收了但没验证"项即 PASS。
