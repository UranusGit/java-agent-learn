...
> **定位**：把"入口路由"的延迟再压低一档。核心：**① 会话内路由缓存**（同一会话后续请求不再重复 embed/分类）② **热门查询语义缓存**（命中过的用户输入直接复用路由结果）③ 缓存一致性（配置变化时失效）。呼应 [81-Agent性能优化](../../教程/81-Agent性能优化.md)、[26-多租户隔离](../../教程/59-多租户隔离与资源治理.md)。前置阅读：[07-兜底降级链与HITL闸门](07-兜底降级链与HITL闸门.md)。
>
...
**缓存 key 必须含租户**（同输入不同租户可能路由不同——多租户矩阵 [26](../../教程/59-多租户隔离与资源治理.md)）。
