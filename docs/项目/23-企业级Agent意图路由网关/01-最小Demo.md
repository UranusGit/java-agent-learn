...
> **定位**：用不到百行造出意图路由的最小骨架：**① 规则路由走通一条 → ② 加一个 EmbeddingModel 语义路由（两个意图示例）→ ③ 一个统一入口 ChatClient 按路由结果分发**。验证三件事：规则命中直达、语义未命中进兜底、分发到正确的 ChatClient。前置阅读：[00-需求分析与架构设计](00-需求分析与架构设计.md)、[教程 09-前沿专题/04-意图路由与路由编排](../../教程/09-前沿专题/04-意图路由与路由编排.md)。
>
