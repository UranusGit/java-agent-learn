...
> **定位**：让"路由到底准不准、兜底了多少"**第一次看得见**。用 Micrometer Observation（配 [教程 33-最小闭环：Agent各阶段输出打印到控制台](../../教程/33-最小闭环：Agent各阶段输出打印到控制台.md)）给路由打点：命中维度（意图×命中/兜底/误分）、命中率/兜底率指标、意图漂移告警。核心转捩点：**路由从'拍脑袋'变成'有指标、可告警、可回放'**。前置阅读：[03-路由表与执行链注册](03-路由表与执行链注册.md)、[教程 31-全链路可观测性](../../教程/31-全链路可观测性.md)。
>
...
**三条关键指标**（配 Grafana/Prometheus，呼应 [教程 31](../../教程/31-全链路可观测性.md)）：
...
**前置条件**：`ObservationRegistry` 已注入；`agent.intent.route` 指标已登记；Prometheus/Grafana 已接入（Mesh 复用 [教程 31]）。
...
**目的是数据飞轮**：告警→补示例句→命中率回升（呼应 [84-数据飞轮与持续改进](../../教程/84-数据飞轮与持续改进.md)、[16-企业级Agent服务网格](../../项目/16-企业级Agent服务网格/00-需求分析与架构设计.md) 的遥测思想）。
...
每次路由记：`requestId, timestamp, input_hash, intent, confidence, selected_route, outcome, latency_stage`。高风险意图（APPROVAL）**必须落全量明文**供事后审计（呼应 [58-历史记录持久化与合规](../../教程/58-历史记录持久化与合规.md)）。
