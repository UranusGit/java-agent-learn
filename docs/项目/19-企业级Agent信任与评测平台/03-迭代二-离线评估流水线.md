...
## 四、评估批次进度可见（结合 RelevancyEvaluator 判分过程）

> 过程可见性是工业级标配。评测流水线是**长时间批量任务**，用户要实时看到批次跑到哪、多少通过多少失败、卡在哪类——不能"提交后等 30min 只给一个最终分数"。可见性直接挂在本迭代真实判分链（[二、离线判分（含 RelevancyEvaluator）](03-迭代二-离线评估流水线.md)）每个 `caseId` 判分后：

```java
// 概念代码：判分后发批次进度事件（对齐 caseId/Evaluator 返回）
record EvalProgress(String batchId, int done, int total, int passed,
                    Map<String,Integer> failByDimension /*忠实度/相关度/安全*/, int currentCaseId) {}
// 每判完一例(RelevancyEvaluator.evaluate 返回)后发: EvalProgress(batchId, done++, total, ..., failByDim, caseId)
```

| 维度 | 事件 | 用户看到 |
|------|------|---------|
| 批次进度 | EvalProgress` | "进度 62/500，通过率 0.91" |
| 维度失败 | failByDimension` | "忠实度维失败 8 例" |
| 单例 | SampleVerdict{caseId,分}` | 点开看该例判分与 reason |

**四条纪律**：① 进度可算（done/total 渲染进度条，样例实时聚簇不刷屏）；② 失败即维报告（按维度聚簇，不等批完才知哪坏，提前定位）；③ SampleVerdict 带 `caseId` 可下钻回原始样本（可审计，呼应信任分可解释）；④ 批可续（进度使中断可断点续跑，不重评已过样例）。
