...
> **定位**：从企业真实痛点出发，完整推演一个基于 MCP 协议的统一工具网关项目的需求分析、架构设计与技术选型。这篇文档不写具体迭代实现，而是回答"为什么做、做什么、怎么做"三个根本问题，并给出**完整可手写的工程蓝图**——最终版 `pom.xml`、`application.yaml`、数据模型 SQL DDL，为后续四篇迭代文档奠定地基。
>
> **读者画像**：架构师、技术负责人、需要从零规划工具中台的全栈开发者。
>
> **前置阅读**：[教程 02-SpringAI核心机制/01-MCP协议](../../教程/02-SpringAI核心机制/01-MCP协议.md)、[教程 00-基础与核心/03-工具调用](../../教程/00-基础与核心/03-工具调用.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/01-MCP真实API与坐标.md) 为准。
...
| ⑤ 决策资产 | 08-ADR架构决策记录 | **按需查阅**：12 条决策 DNA（改架构前先查） |
...
> **深入理解 MCP 协议的全貌** → [教程 02-SpringAI核心机制/01-MCP协议](../../教程/02-SpringAI核心机制/01-MCP协议.md)：该教程详细讲解了 MCP 的 Host/Client/Server 三层架构、JSON-RPC 2.0 通信协议、stdio 与 Streamable HTTP 两种传输方式，以及 MCP 与传统 Tool Calling 的本质区别。
...
### 4.3 完整配置（两段式：`application.yaml` + `application-mcp.yaml`）

> **两段式约定**：`application.yaml` 只做两件事——`spring.config.import` 引入 `.env`（密钥不进版本库）与 `spring.profiles.active: mcp` 激活业务配置；全部业务配置（端口/线程/数据源/MCP 连接）放在 `application-mcp.yaml`。启动命令 `mvn -Dspring-boot.run.profiles=mcp spring-boot:run` 时 Spring Boot 自动加载 `application-mcp.yaml`。
...
**前置条件**：JDK 21、Maven 已安装；本节 `pom.xml` / `application.yaml` + `application-mcp.yaml` 已抄录到工程（迭代零最小集即可：webflux + mcp-client + actuator，其余依赖按演进说明分迭代加入）。

**材料——依赖与配置核对命令**：

```bash
mvn -q dependency:resolve -DincludeArtifactIds=spring-ai-starter-mcp-client
grep -A2 "virtual:" src/main/resources/application-mcp.yaml
```
...
├── src/main/resources/
│   ├── application.yaml                # 主配置：.env 引入 + profile 激活
│   ├── application-mcp.yaml            # 业务配置（端口 8081 / MCP / 数据源）
...
MCP 工具网关集中管理了所有工具的调用入口，安全风险需要特别关注。这部分将在迭代二中详细展开，核心思路是参考 [教程 04-企业级架构主干/11-安全与权限控制](../../教程/04-企业级架构主干/11-安全与权限控制.md) 中的 Agent 安全三道防线模型，在网关层实现统一的输入校验、权限控制和审计追溯。

> **安全架构深入** → [教程 04-企业级架构主干/11-安全与权限控制](../../教程/04-企业级架构主干/11-安全与权限控制.md)：讲解了 Agent 安全的输入安全（Prompt 注入防护）、执行安全（工具权限控制）、输出安全（内容过滤）三道防线体系。

### 9.3 本节核对（风险登记）

- [ ] 四项技术风险各有"应对方案"且指向具体迭代或附录基准，无"待定"
- [ ] "网关成为单点"的应对（无状态化 + 水平扩展）与 §3 WebFlux/虚拟线程选型自洽
- [ ] 安全风险的落点（迭代二 + 教程 04-企业级架构主干/02-全链路可观测性）与 §6.2 目录树 auth/ 包对应
...
| 验收项 | 达标标准 | 本迭代 |
|--------|---------|--------|
| 需求覆盖 | 三重痛点（重复建设/管控缺失/变更成本）→ 三阶段交付（迭代零/一/二） | ✅ |
| 架构完整 | 四层分层（接入/核心/客户端/Server）+ 双重身份 | ✅ |
| 蓝图可编译 | §4 `pom.xml` / `application.yaml` + `application-mcp.yaml` / SQL DDL 照抄可运行 | ✅ |
| 指标可度量 | P99 < 200ms、审计 100% 覆盖、白名单外调用被拒 | ✅ |

### 12.1 全篇回归验证（蓝图落地性）

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn clean compile`（§4.4） | 蓝图最小集编译通过 |
| 2 | 两段式配置校验 | `application.yaml` 仅 .env + profile；业务配置在 `application-mcp.yaml`（端口 8081） |
| 3 | SQL DDL 建表 | `tool_registry` / `tool_permission` 等表创建成功、白名单默认 DENY |
| 4 | 迭代衔接 | 迭代零/一/二各篇验收项可回溯到本蓝图（无凭空能力） |

**失败排查**：①编译失败→依赖坐标或配置键与附录 05-01 不符；②建表失败→外键/默认值顺序错（先建主表再建关联表）。
...
4. **蓝图完整**：给出最终版 `pom.xml`、`application.yaml`/`application-mcp.yaml`、数据模型 SQL DDL，各迭代按演进式路径逐步落地（迭代零单 Server → 迭代一多 Server + 可观测 → 迭代二自建 Server + 容错安全）。
