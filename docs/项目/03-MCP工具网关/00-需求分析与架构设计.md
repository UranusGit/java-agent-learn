...
>
> **前置阅读**：[教程 10-MCP 协议](../../教程/11-MCP协议.md)、[教程 03-工具调用](../../教程/03-工具调用.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/01-MCP真实API与坐标.md) 为准。
...
> **深入理解 MCP 协议的全貌** → [教程 10-MCP 协议](../../教程/11-MCP协议.md)：该教程详细讲解了 MCP 的 Host/Client/Server 三层架构、JSON-RPC 2.0 通信协议、stdio 与 Streamable HTTP 两种传输方式，以及 MCP 与传统 Tool Calling 的本质区别。
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <dependencies>
        ...
        <!-- MCP 客户端：连接下游 MCP Server（迭代零，真实坐标附录 12-01） -->
        <dependency>
            <groupId>org.springframework.ai</groupId>
...
> 演进式说明：迭代一只落地 `audit_logs` 表（见 [02-迭代一 §6.4](02-迭代一-MCP客户端集成.md)）；下表是完整目标 schema，元数据/权限表在后续项目演进中落地。
...
| 风险 | 概率 | 影响 | 应对方案 |
|------|------|------|---------|
| MCP 协议版本变更 | 中 | 高 | 网关层做版本适配抽象，锁定 Spring AI MCP SDK 版本（附录 12-01 基准） |
...
MCP 工具网关集中管理了所有工具的调用入口，安全风险需要特别关注。这部分将在迭代二中详细展开，核心思路是参考 [教程 31-安全与权限控制](../../教程/31-安全与权限控制.md) 中的 Agent 安全三道防线模型，在网关层实现统一的输入校验、权限控制和审计追溯。

> **安全架构深入** → [教程 31-安全与权限控制](../../教程/31-安全与权限控制.md)：讲解了 Agent 安全的输入安全（Prompt 注入防护）、执行安全（工具权限控制）、输出安全（内容过滤）三道防线体系。
...
- **取舍理由**：附录 12-01 §4 基准——HTTP+SSE 已废弃，Streamable HTTP 单端点 POST+mixin 流式响应是现行标准
