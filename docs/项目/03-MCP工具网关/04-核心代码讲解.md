...
> **定位**：对 MCP 工具网关项目中所有关键代码片段做集中梳理和深度解析。每段代码都配有"设计意图——实现要点——代码解读——扩展方向"四层结构，帮助读者不仅看懂"怎么写"，更理解"为什么这样写"。
>
> **读者画像**：已阅读前三篇文档，想要深入理解每段核心代码设计决策的开发者。
>
> **前置阅读**：[01-最小 Demo 搭建](01-最小Demo搭建.md)、[02-迭代一-MCP 客户端集成](02-迭代一-MCP客户端集成.md)、[03-迭代二-自定义 MCP 服务端](03-迭代二-自定义MCP服务端.md)。
...
if (conn == null) {
    throw new IllegalArgumentException(...)   // 配置错误：Server 不存在
}
if (!conn.isAvailable()) {
    throw new IllegalStateException(...)      // 运行时故障：Server 不可用
}
```
...
public ToolCallResult call(ToolCallRequest request) {
    ...
    return ToolCallResult.success(tool.globalId(), result, duration);
}
```
...
注意当前实现是简化版——通配符 `*` 只做 `Set.contains()` 精确匹配，不支持 `filesystem.*` 这种模式匹配。生产环境需要加入前缀匹配逻辑：
...
```java
@Async
public void record(String agentId, String toolName, ...) {
    AuditLog entry = new AuditLog();
    // ... 构建审计日志 ...
    repository.save(entry);  // JPA 写入数据库
}
```
...
Spring AI 在应用启动时扫描所有 `@Tool` 注解方法，通过反射提取方法签名和注解元数据，自动生成对应的 MCP 工具 JSON Schema。这个过程对开发者完全透明——写一个 Java 方法，加两个注解，就得到了一个标准 MCP 工具。
...
如果需要同时使用 stdio 和 SSE 两种传输方式，可以并列配置：
...
graph TB
    subgraph 自检["代码质量自检清单"]
        ...
        Q3["资源管理<br/>✓ Pattern 预编译为 static final<br/>✓ 连接移除时关闭 McpClient<br/>✓ 审计日志异步写入不阻塞"]
...
6. **MCP 工具定义**：`@Tool` 注解通过反射自动生成 MCP JSON Schema，对开发者完全透明。`description` 质量直接影响 LLM 的工具选择准确率。
