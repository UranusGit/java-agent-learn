# 04-核心代码讲解

> **定位**：对 MCP 工具网关项目中所有关键代码做集中梳理和深度解析。每段代码都配有"设计意图——实现要点——代码解读——扩展方向"四层结构，帮助读者不仅看懂"怎么写"，更理解"为什么这样写"。本文给出的类清单都是**完整可手写**（含全部 import、无省略），与前三篇可对照阅读。
>
> **读者画像**：已阅读前三篇文档，想要深入理解每段核心代码设计决策的开发者。
>
> **前置阅读**：[01-最小 Demo 搭建](01-最小Demo搭建.md)、[02-MCP 客户端集成](02-MCP客户端集成.md)、[03-自定义 MCP 服务端](03-自定义MCP服务端.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/01-MCP真实API与坐标.md) 为准。

---

## 1. 项目代码全景

```mermaid
graph TB
    subgraph 代码全景["MCP 工具网关代码结构"]
        direction TB
        subgraph API层["API 层"]
            C1["ToolGatewayController<br/>REST 端点"]
            C2["SecureToolGatewayController<br/>认证版端点"]
        end

        subgraph 核心层["核心层"]
            R1["ToolRouter / ResilientToolRouter<br/>工具路由引擎"]
            R2["ToolRegistry / ToolDiscoveryService<br/>工具注册与发现"]
            R3["SecurityValidator<br/>安全校验器"]
            R4["StrategySelector<br/>多模型策略选择器"]
        end

        subgraph 基础层["基础层"]
            B1["McpClientPool<br/>连接池"]
            B2["HealthChecker<br/>健康检查"]
            B3["CircuitBreakerFactory<br/>熔断器工厂"]
            B4["AuditService<br/>审计服务"]
            B5["GatewayMetrics<br/>指标收集"]
        end

        subgraph 工具层["工具层"]
            T1["OrderTools<br/>订单工具"]
            T2["TicketTools<br/>工单工具"]
            T3["ApprovalTools<br/>审批工具"]
        end

        subgraph 模型层["模型层"]
            M1["ToolInfo<br/>ToolCallRequest<br/>ToolCallResult"]
            M2["AgentIdentity<br/>ToolSupplyStrategy"]
            M3["McpConnection<br/>Status 枚举"]
        end
    end

    style 代码全景 fill:#e3f2fd
```

### 1.1 本节核对（全景图与前三篇类清单一致）

- [ ] 图中每个类都能在 01/02/03 篇正文找到完整代码（无凭空多出的类）
- [ ] 三篇引入的类全部入图（ToolRegistry→DiscoveryService 演进关系已标注）
- [ ] 分层归属与 00 篇 §3.2 分层职责表一致（API/核心/基础/工具/模型层）

---

## 2. 数据模型层深度解析

### 2.1 ToolInfo——工具元数据

```java
package com.example.mcp.gateway.model;

/**
 * 工具元数据——描述一个 MCP 工具的能力。
 * 直接映射 MCP Server tools/list 返回的结构。
 *
 * @param serverName   来源 MCP Server 名称，如 "filesystem"
 * @param name         工具名称，如 "read_file"
 * @param description  工具描述（LLM 判断何时调用的唯一依据）
 * @param inputSchema  参数 JSON Schema——MCP SDK 的 ToolInputSchema 原样透传
 */
public record ToolInfo(
        String serverName,
        String name,
        String description,
        Object inputSchema
) {
    public String globalId() {
        return serverName + "." + name;
    }
}
```

**设计意图**：用一个不可变 Record 封装 MCP 工具的完整元数据，`globalId()` 方法解决了多 Server 工具名冲突问题。

**关键决策——为什么用 Record 而不是 Class**：

| 对比维度 | 传统 Class | Java 21 Record |
|---------|-----------|----------------|
| 代码行数 | 30+ 行（getter/setter/equals/hashCode） | 6 行 |
| 不可变性 | 需手动保证（final 字段 + 无 setter） | 天然不可变 |
| 线程安全 | 需考虑可变字段 | 天然线程安全 |
| 模式匹配 | 需要 instanceof + 强转 | 原生支持 deconstruction |

`globalId()` 的设计逻辑：

```mermaid
graph LR
    subgraph 命名空间["工具命名空间设计"]
        S1["Server: filesystem<br/>tools: read_file, write_file"]
        S2["Server: postgres<br/>tools: query, read_file"]
    end

    subgraph 冲突["潜在冲突"]
        C1["两个 Server 都有<br/>read_file 工具"]
    end

    subgraph 解决["globalId 解决方案"]
        R1["filesystem.read_file"]
        R2["postgres.read_file"]
        R3["全局唯一，无冲突"]
    end

    S1 --> 冲突
    S2 --> 冲突
    冲突 --> 解决

    style 冲突 fill:#ffcdd2
    style 解决 fill:#c8e6c9
```

### 2.2 ToolCallResult——统一结果包装

```java
package com.example.mcp.gateway.model;

/**
 * 工具调用结果 DTO——统一成功/失败包装，避免用异常传递工具失败。
 */
public record ToolCallResult(
        String toolName,
        boolean success,
        Object content,
        String errorMessage,
        long durationMs
) {
    public static ToolCallResult success(String toolName, Object content, long durationMs) {
        return new ToolCallResult(toolName, true, content, null, durationMs);
    }

    public static ToolCallResult failure(String toolName, String error, long durationMs) {
        return new ToolCallResult(toolName, false, null, error, durationMs);
    }
}
```

**设计意图**：用统一的结果格式包裹所有工具调用，无论成功还是失败，调用方都收到 `ToolCallResult` 而非异常。

**为什么不用异常传递失败**：

```mermaid
graph TB
    subgraph 异常方案["异常方案（不推荐）"]
        A1["工具失败 → 抛异常"]
        A2["Controller 捕获异常"]
        A3["转 HTTP 500"]
        A4["Agent 收到 500 错误"]
        A5["Agent 无法区分：<br/>工具本身失败 vs 网关故障"]
    end

    subgraph 统一结果["统一结果方案（推荐）"]
        B1["工具失败 → ToolCallResult.failure"]
        B2["Controller 直接返回"]
        B3["HTTP 200 + success=false"]
        B4["Agent 明确知道：<br/>工具调用失败，原因是 X"]
        B5["Agent 可以据此调整策略<br/>（换工具 / 重试 / 告知用户）"]
    end

    style 异常方案 fill:#ffcdd2
    style 统一结果 fill:#c8e6c9
```

`durationMs` 字段的设计意图：让每次调用都自带耗时信息，无需额外的计时中间件。在审计日志、监控告警中直接可用。

### 2.3 本节测试与验证（Record 行为）

**前置条件**：§2.1/§2.2 代码已手写（与 01 篇 §3.5 同源）。

**执行方式**：`mvn test -Dtest=ToolInfoTest,ToolCallResultTest` 驱动 Record 行为断言（globalId/equals/工厂方法）。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `ToolInfoTest`：两个 Server 同名工具各建一个 ToolInfo | `globalId()` 分别为 `filesystem.read_file` / `postgres.read_file`，互不相等 |
| 2 | 同参构造两个 ToolInfo 做 equals | `true`（Record 自动实现） |
| 3 | `ToolCallResult.success("x", "c", 5)` | `success=true`、`errorMessage=null`、`content="c"`；failure 工厂反之 `content=null` |
| 4 | 端到端失败调用（01 篇 §7.1 回归 3 的不存在工具） | HTTP 200 + `success=false`（统一结果方案，非 500） |

**失败排查**：①equals 不等→手写成了 Class 且未实现 equals/hashCode（应使用 Record）；②失败走了 500→Controller 未用 ToolCallResult 包装异常（核 01 篇 §3.7 catch 分支）。

---

## 3. 连接管理核心代码

### 3.1 McpConnection——连接状态机

```java
package com.example.mcp.gateway.pool;

import com.example.mcp.gateway.model.ToolInfo;
import io.modelcontextprotocol.client.McpSyncClient;   // ⚠ MCP SDK 类型（附录 05-01）

import java.time.Instant;
import java.util.List;
import java.util.concurrent.CopyOnWriteArrayList;

/**
 * MCP Client 连接描述符。一个连接对应一个 MCP Server。
 */
public class McpConnection {

    public enum Status {
        CONNECTING,    // 正在握手
        CONNECTED,     // 已连接，可用
        DEGRADED,      // 降级（健康检查失败但未断开）
        DISCONNECTED   // 已断开
    }

    private final String serverName;
    private final McpSyncClient client;   // ⚠ MCP SDK 类型（附录 05-01）
    private volatile Status status;
    private final Instant connectedAt;
    private Instant lastHealthCheck;
    private int consecutiveFailures;

    // 该 Server 暴露的工具列表（由 ToolDiscoveryService 维护）
    private final List<ToolInfo> tools = new CopyOnWriteArrayList<>();

    public McpConnection(String serverName, McpSyncClient client) {
        this.serverName = serverName;
        this.client = client;
        this.status = Status.CONNECTING;
        this.connectedAt = Instant.now();
    }

    /** 标记健康检查结果 */
    public void recordHealthCheck(boolean healthy) {
        this.lastHealthCheck = Instant.now();
        if (healthy) {
            this.consecutiveFailures = 0;
            this.status = Status.CONNECTED;
        } else {
            this.consecutiveFailures++;
            this.status = this.consecutiveFailures >= 3
                    ? Status.DISCONNECTED
                    : Status.DEGRADED;
        }
    }

    /** 连接是否可用 */
    public boolean isAvailable() {
        return status == Status.CONNECTED || status == Status.DEGRADED;
    }

    // getter（完整，不省略）
    public String getServerName() { return serverName; }
    public McpSyncClient getClient() { return client; }
    public Status getStatus() { return status; }
    public Instant getConnectedAt() { return connectedAt; }
    public Instant getLastHealthCheck() { return lastHealthCheck; }
    public int getConsecutiveFailures() { return consecutiveFailures; }
    public List<ToolInfo> getTools() { return tools; }
}
```

**状态机解析**：

```mermaid
stateDiagram-v2
    [*] --> CONNECTING : 创建连接

    CONNECTING --> CONNECTED : 握手成功
    CONNECTING --> DISCONNECTED : 握手失败

    CONNECTED --> DEGRADED : 健康检查失败 1-2 次
    DEGRADED --> CONNECTED : 健康检查恢复
    DEGRADED --> DISCONNECTED : 连续失败 >= 3 次

    DISCONNECTED --> CONNECTING : 重新连接
    DISCONNECTED --> [*] : 手动移除
```

**为什么用 volatile 而不是 synchronized**：

`status` 字段用 `volatile` 修饰而非 `synchronized` 块保护。原因如下：

- `Status` 是引用类型（枚举），`volatile` 保证了引用的可见性和原子性
- `recordHealthCheck` 由健康检查定时任务单线程调用，不存在并发写入
- `isAvailable()` 被多线程读取，`volatile` 保证了读操作的内存可见性
- 避免了 `synchronized` 的锁开销和可能的死锁风险

**consecutiveFailures 的渐进式策略**：

```java
// 节选——出自上方 McpConnection.recordHealthCheck，仅展示渐进式策略核心
// 不是一次失败就断开，而是给 Server 3 次恢复机会
this.status = this.consecutiveFailures >= 3
        ? Status.DISCONNECTED
        : Status.DEGRADED;
```

这避免了网络瞬时抖动导致的误判。单次超时不等于 Server 故障——可能是 GC 停顿、网络丢包、或 Server 正在执行慢查询。3 次连续失败（约 90 秒）才断开，平衡了灵敏度和稳定性。

### 3.2 McpClientPool——连接池的并发设计

```java
package com.example.mcp.gateway.pool;

import io.modelcontextprotocol.client.McpSyncClient;   // ⚠ MCP SDK 真实类型（附录 05-01）
import org.springframework.stereotype.Component;

import java.util.List;
import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

/**
 * MCP Client 连接池。
 * 管理所有 MCP Server 的连接，提供按名称获取、动态注册、移除断开的连接。
 */
@Component
public class McpClientPool {

    // serverName → McpConnection
    private final Map<String, McpConnection> connections = new ConcurrentHashMap<>();

    public void register(String serverName, McpSyncClient client) {
        connections.put(serverName, new McpConnection(serverName, client));
    }

    public McpConnection get(String serverName) {
        McpConnection conn = connections.get(serverName);
        if (conn == null) {
            throw new IllegalArgumentException(
                    "Unknown MCP server: " + serverName +
                    ". Available: " + connections.keySet());
        }
        if (!conn.isAvailable()) {
            throw new IllegalStateException(
                    "MCP server '" + serverName + "' is " + conn.getStatus());
        }
        return conn;
    }

    public Map<String, McpConnection> getAll() {
        return Map.copyOf(connections);
    }

    public List<McpConnection> getAvailable() {
        return connections.values().stream()
                .filter(McpConnection::isAvailable)
                .toList();
    }

    public void remove(String serverName) {
        McpConnection conn = connections.remove(serverName);
        if (conn != null) {
            try {
                conn.getClient().close();
            } catch (Exception ignored) {
                // 关闭时的异常可以忽略
            }
        }
    }
}
```

**为什么用 ConcurrentHashMap 而不是 HashMap + synchronized**：

```mermaid
graph TB
    subgraph 并发方案对比["连接池并发方案对比"]
        S1["方案 A: HashMap + synchronized<br/>读写都加全局锁<br/>并发吞吐量低"]
        S2["方案 B: ConcurrentHashMap<br/>分段锁（JDK 8+ 桶级 CAS）<br/>读写几乎无锁"]
        S3["方案 C: LinkedHashMap（只读快照）<br/>启动后不变，无需并发保护"]
    end

    style S1 fill:#ffcdd2
    style S2 fill:#c8e6c9
    style S3 fill:#e3f2fd
```

连接池的场景是"多读少写"——注册和移除只在启动和故障时发生，但 `get()` 每次工具调用都会执行。`ConcurrentHashMap` 的无锁读 + CAS 写非常适合这个场景。

**异常设计的分层**：

```java
// 节选——出自上方 McpClientPool.get，仅展示异常分层
// 区分两种错误：配置问题 vs 运行时故障
if (conn == null) {
    throw new IllegalArgumentException(
            "Unknown MCP server: " + serverName);   // 配置错误：Server 不存在
}
if (!conn.isAvailable()) {
    throw new IllegalStateException(
            "MCP server '" + serverName + "' is " + conn.getStatus());
    // 运行时故障：Server 不可用
}
```

`IllegalArgumentException` 表示"你请求了一个不存在的 Server"——这是调用方的配置错误，需要修改代码或配置。`IllegalStateException` 表示"Server 存在但当前不可用"——这是运行时状态，可能自动恢复。

### 3.3 本节测试与验证（状态机与异常分层）

**前置条件**：02 篇 §4/§5 已完成；本节代码与 02 篇 §4.2/§4.3 同源（讲解版）。

**执行方式**：`mvn test -Dtest=McpConnectionTest,McpClientPoolTest` 驱动状态机与异常分层断言。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `McpConnectionTest`：`recordHealthCheck(false)` 调 1、2 次 | status=DEGRADED 且 `isAvailable()=true`（渐进式未隔离） |
| 2 | 连续第 3 次 false | status=DISCONNECTED 且 `isAvailable()=false` |
| 3 | 之后一次 true | status 回 CONNECTED、consecutiveFailures 归零（自动恢复） |
| 4 | `McpClientPoolTest`：`get("不存在")` | 抛 `IllegalArgumentException`（配置错误语义） |
| 5 | 隔离状态下 `get("postgres")` | 抛 `IllegalStateException` 且消息含状态名（运行时故障语义） |
| 6 | 状态机图逐边核对 | stateDiagram 六条迁移与 §3.1 代码分支一一对应 |

**失败排查**：①DEGRADED 被拒→`isAvailable()` 漏了 DEGRADED；②恢复后仍 DISCONNECTED→healthy 分支漏重置计数；③异常类型反了→核对 §3.2 分层注释。

---

## 4. 路由引擎核心代码

### 4.1 ResilientToolRouter——容错调用链

```java
package com.example.mcp.gateway.router;

import com.example.mcp.gateway.model.ToolCallRequest;
import com.example.mcp.gateway.model.ToolCallResult;
import com.example.mcp.gateway.model.ToolInfo;
import com.example.mcp.gateway.pool.McpClientPool;
import com.example.mcp.gateway.pool.McpConnection;
import com.example.mcp.gateway.registry.ToolDiscoveryService;
import com.example.mcp.gateway.resilience.CircuitBreakerFactory;
import io.github.resilience4j.circuitbreaker.CallNotPermittedException;
import io.github.resilience4j.decorators.Decorators;
import io.github.resilience4j.retry.Retry;
import io.github.resilience4j.retry.RetryConfig;
import io.modelcontextprotocol.spec.McpSchema.CallToolRequest;
import org.springframework.stereotype.Component;

import java.time.Duration;
import java.time.Instant;
import java.util.List;
import java.util.Map;
import java.util.function.Supplier;

/**
 * 增强版工具路由引擎——集成熔断 + 重试 + 降级。
 */
@Component
public class ResilientToolRouter {

    private final ToolDiscoveryService discovery;
    private final McpClientPool pool;
    private final CircuitBreakerFactory cbFactory;

    public ResilientToolRouter(ToolDiscoveryService discovery,
                               McpClientPool pool,
                               CircuitBreakerFactory cbFactory) {
        this.discovery = discovery;
        this.pool = pool;
        this.cbFactory = cbFactory;
    }

    public ToolCallResult call(ToolCallRequest request) {
        long start = System.currentTimeMillis();

        // 1. 查找工具
        ToolInfo tool = discovery.find(request.toolName());
        if (tool == null) {
            return ToolCallResult.failure(
                    request.toolName(),
                    "Tool not found: " + request.toolName(),
                    System.currentTimeMillis() - start);
        }

        // 2. 获取连接
        McpConnection conn = pool.get(tool.serverName());
        if (conn == null || !conn.isAvailable()) {
            return ToolCallResult.failure(
                    tool.globalId(),
                    "MCP Server not available: " + tool.serverName(),
                    System.currentTimeMillis() - start);
        }

        // 3. 构建容错调用链：重试 → 熔断 → 降级
        var circuitBreaker = cbFactory.getOrCreate(tool.serverName());
        var retry = buildRetry(tool.serverName());

        Supplier<Object> callSupplier = () -> conn.getClient().callTool(
                new CallToolRequest(tool.name(), safeArguments(request)));

        var decorated = Decorators.ofSupplier(callSupplier)
                .withCircuitBreaker(circuitBreaker)
                .withRetry(retry)
                .withFallback(
                        List.of(CallNotPermittedException.class, Exception.class),
                        e -> degradedResult(tool, e))
                .decorate();

        try {
            Object result = decorated.get();
            return ToolCallResult.success(
                    tool.globalId(),
                    result,
                    System.currentTimeMillis() - start);
        } catch (Exception e) {
            return ToolCallResult.failure(
                    tool.globalId(),
                    "Tool execution failed after resilience: " + e.getMessage(),
                    System.currentTimeMillis() - start);
        }
    }

    /** 构建重试策略——只对瞬时故障重试，不对业务错误重试 */
    private Retry buildRetry(String serverName) {
        return Retry.of("mcp-retry-" + serverName,
                RetryConfig.custom()
                        .maxAttempts(3)
                        .waitDuration(Duration.ofMillis(500))
                        .retryOnException(e ->
                                !(e instanceof IllegalArgumentException))
                        .build());
    }

    /** 降级结果——熔断打开或连续失败时的兜底响应 */
    private Object degradedResult(ToolInfo tool, Throwable e) {
        return Map.of(
                "degraded", true,
                "tool", tool.globalId(),
                "message", "Service temporarily unavailable. Circuit breaker is open.",
                "fallbackAt", Instant.now().toString());
    }

    private Map<String, Object> safeArguments(ToolCallRequest request) {
        return request.arguments() != null ? Map.copyOf(request.arguments()) : Map.of();
    }
}
```

**Decorators 链的执行顺序**：

```mermaid
graph TB
    REQ["工具调用请求"] --> CB_CHECK{"熔断器检查"}

    CB_CHECK -->|"CLOSED"| RETRY["重试层"]
    CB_CHECK -->|"OPEN"| FALLBACK["降级：返回兜底结果"]

    RETRY --> ATTEMPT1["第 1 次尝试"]
    ATTEMPT1 -->|"成功"| SUCCESS["返回结果"]
    ATTEMPT1 -->|"失败（瞬时）"| ATTEMPT2["第 2 次尝试<br/>等待 500ms"]
    ATTEMPT2 -->|"成功"| SUCCESS
    ATTEMPT2 -->|"失败"| ATTEMPT3["第 3 次尝试"]
    ATTEMPT3 -->|"成功"| SUCCESS
    ATTEMPT3 -->|"失败"| FALLBACK

    FALLBACK --> DEGRADED["ToolCallResult<br/>携带降级标记"]

    style SUCCESS fill:#c8e6c9
    style FALLBACK fill:#fff9c4
    style DEGRADED fill:#e3f2fd
```

**重试策略的幂等性考量**：

```java
private Retry buildRetry(String serverName) {
    return Retry.of("mcp-retry-" + serverName,
            RetryConfig.custom()
                    .maxAttempts(3)
                    .waitDuration(Duration.ofMillis(500))
                    .retryOnException(e ->
                            !(e instanceof IllegalArgumentException))
                    .build());
}
```

`retryOnException` 的过滤条件是关键设计：

```mermaid
graph TB
    subgraph 异常分类["重试决策矩阵"]
        R1["网络超时<br/>→ 可重试（瞬时故障）"]
        R2["JSON-RPC 超时<br/>→ 可重试（瞬时故障）"]
        R3["Server 内部错误<br/>→ 可重试（可能恢复）"]
        W1["参数错误<br/>→ 不重试（必然失败）"]
        W2["权限不足<br/>→ 不重试（配置问题）"]
        W3["工具不存在<br/>→ 不重试（配置问题）"]
    end

    style R1 fill:#c8e6c9
    style R2 fill:#c8e6c9
    style R3 fill:#c8e6c9
    style W1 fill:#ffcdd2
    style W2 fill:#ffcdd2
    style W3 fill:#ffcdd2
```

重试只对"可能恢复的瞬时故障"有意义。参数错误、权限不足这类确定性失败，重试 100 次结果也一样——白白浪费资源和延迟。

### 4.2 降级结果设计

```java
private Object degradedResult(ToolInfo tool, Throwable e) {
    return Map.of(
            "degraded", true,
            "tool", tool.globalId(),
            "message", "Service temporarily unavailable. Circuit breaker is open.",
            "fallbackAt", Instant.now().toString());
}
```

**为什么降级结果用 Map 而非 null**：

- `null` 无法区分"工具返回了 null"和"工具被降级了"
- `Map` 携带了降级原因和时间戳，让调用方（尤其是 LLM）能理解当前状态
- LLM 看到 `degraded: true` 后可以告知用户"该服务暂时不可用"，而非将空结果当作真实数据

### 4.3 本节核对（与 03 篇容错实现一致）

- [ ] `Decorators` 链执行顺序图与 03 篇 §4.3 代码的 withCircuitBreaker→withRetry→withFallback 顺序一致
- [ ] 重试决策矩阵的"不重试"分支与 `retryOnException(!(e instanceof IllegalArgumentException))` 对应
- [ ] 端到端熔断/降级行为已由 03 篇 §4.4 验证（本文不再重复故障注入）

---

## 5. 安全核心代码

### 5.1 AgentIdentity——权限模型

```java
package com.example.mcp.gateway.auth;

import java.util.Set;

/**
 * Agent 身份信息。
 */
public record AgentIdentity(
        String agentId,        // Agent 唯一标识
        String agentName,      // Agent 名称
        String tier,           // 权限等级：STANDARD / PREMIUM / ADMIN
        Set<String> allowedTools  // 允许调用的工具集合（白名单）
) {
    /**
     * 检查是否有权调用指定工具。
     */
    public boolean canCall(String toolGlobalId) {
        if ("ADMIN".equals(tier)) return true;
        return allowedTools.contains(toolGlobalId)
                || allowedTools.contains("*");
    }
}
```

**权限层级设计**：

```mermaid
graph TB
    subgraph 权限矩阵["Agent 权限矩阵"]
        T1["ADMIN<br/>全部工具可用<br/>canCall() 直接返回 true"]
        T2["PREMIUM<br/>白名单 + 通配符<br/>allowedTools 含 'filesystem.*'"]
        T3["STANDARD<br/>精确白名单<br/>allowedTools 含 'OrderTools.queryOrder'"]
    end

    subgraph 通配符["通配符匹配示例"]
        W1["'filesystem.*'<br/>→ 匹配 filesystem.read_file<br/>→ 匹配 filesystem.write_file"]
        W2["'OrderTools.queryOrder'<br/>→ 仅匹配此精确工具"]
        W3["'*'<br/>→ 匹配所有工具"]
    end

    style T1 fill:#ffcdd2
    style T2 fill:#fff9c4
    style T3 fill:#c8e6c9
```

注意当前实现是简化版——通配符 `*` 只做 `Set.contains()` 精确匹配，不支持 `filesystem.*` 这种前缀模式匹配。生产环境需要加入前缀匹配逻辑：

```java
// 节选——生产版扩展方法，追加到上方 AgentIdentity 类中（与简化版 canCall 同名替换）
public boolean canCall(String toolGlobalId) {
    if ("ADMIN".equals(tier)) return true;
    // 精确匹配
    if (allowedTools.contains(toolGlobalId)) return true;
    // 前缀通配符匹配：filesystem.* 匹配 filesystem.read_file
    String serverPrefix = toolGlobalId.substring(0,
            toolGlobalId.indexOf('.') + 1) + "*";
    return allowedTools.contains(serverPrefix);
}
```

### 5.2 SecurityValidator——注入检测

```java
// 摘录自 SecurityValidator（完整类见 03 篇 §5.3）——以下仅展示注入检测核心片段
private static final Pattern SQL_INJECTION = Pattern.compile(
        "(?i)(union\\s+select|drop\\s+table|insert\\s+into|" +
        "delete\\s+from|--|;\\s*drop|or\\s+1\\s*=\\s*1)"
);

// 命令注入检测正则（filesystem 类工具）
private static final Pattern CMD_INJECTION = Pattern.compile(
        "(;|\\|`|\\$\\(|\\$\\{|rm\\s+-rf|cat\\s+/etc)"
);

private void checkInjection(ToolCallRequest request, ToolInfo tool) {
    for (var entry : request.arguments().entrySet()) {
        if (entry.getValue() instanceof String strValue) {
            // 数据库类工具检测 SQL 注入
            if ("postgres".equals(tool.serverName())
                    && SQL_INJECTION.matcher(strValue).find()) {
                throw new SecurityException("SQL injection detected");
            }
            // 文件系统类工具检测命令注入
            if ("filesystem".equals(tool.serverName())
                    && CMD_INJECTION.matcher(strValue).find()) {
                throw new SecurityException("Command injection detected");
            }
        }
    }
}
```

**检测策略的分层设计**：

```mermaid
graph TB
    subgraph 检测层["注入检测三层策略"]
        L1["第一层：正则模式匹配<br/>（快速过滤已知攻击模式）<br/>当前实现"]
        L2["第二层：语义分析<br/>（理解 SQL 结构是否合法）<br/>生产建议：sqlparse 库"]
        L3["第三层：参数化查询<br/>（从根本上杜绝注入）<br/>最佳实践"]
    end

    L1 --> L2
    L2 --> L3

    style L1 fill:#fff9c4
    style L2 fill:#e3f2fd
    style L3 fill:#c8e6c9
```

正则匹配只是第一道防线——它的局限在于：规则是有限的、可被绕过的（如编码变换、注释分割）。真正的防御应该让 MCP Server 的 postgres 工具强制使用参数化查询（Prepared Statement），从根源上杜绝 SQL 注入；filesystem 类工具则需限定路径白名单并拒绝拼接 shell 命令。

**Pattern 预编译的性能考量**：

`Pattern.compile()` 是昂贵操作。将编译好的 `Pattern` 作为 `static final` 常量，在类加载时编译一次，后续所有调用共享同一个编译结果。如果每次校验都 `Pattern.compile()`，在高并发下会成为性能瓶颈。

### 5.3 本节测试与验证（权限模型与生产版前缀匹配）

**前置条件**：03 篇 §5 已完成；本节含生产版 `canCall` 扩展代码（简化版之外的新代码，需单独验证）。

**执行方式**：`mvn test -Dtest=SecurityValidatorTest`（含 §5.1 生产版 canCall 断言）；注入正则抽测可写临时 `main` 或 `jshell` 单测跑 6 条样本。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 简化版 `canCall`：STANDARD + 白名单含 `filesystem.read_file` | 对 `filesystem.read_file` 返回 true；对 `filesystem.write_file` 返回 false（简化版不做前缀匹配——已知局限） |
| 2 | 简化版：ADMIN | 任意工具 true |
| 3 | **生产版（本节新增）**：PREMIUM + 白名单含 `filesystem.*` 调 `filesystem.write_file` | 前缀通配匹配返回 true（§5.1 生产版代码） |
| 4 | 生产版：无任何匹配 | false（默认拒绝） |
| 5 | 注入正则抽测 6 条样本（union select / drop table / `--` / or 1=1 / 正常 SQL / 中文参数） | 前 4 条命中，后 2 条放行 |

**失败排查**：①前缀匹配漏判→`indexOf('.') + 1` 边界写错（`a.b.c` 只取第一段，符合 serverName 语义）；②ADMIN 也走匹配→tier 短路分支顺序错误；③正则误报→`--` 规则会拦注释型正常文本，需在文档化样本中标注误报权衡。

---

## 6. 审计核心代码

### 6.1 异步审计的虚拟线程优势

```java
@Async
public void record(
        String agentId,
        String toolName,
        Map<String, Object> inputParams,
        Object outputResult,
        boolean success,
        long durationMs,
        String errorMessage
) {
    AuditLog entry = new AuditLog();
    entry.setAgentId(agentId);
    entry.setToolName(toolName);
    entry.setInputParams(toJson(inputParams));
    entry.setOutputResult(toJson(outputResult));
    entry.setStatus(success ? "SUCCESS" : "FAILED");
    entry.setDurationMs(durationMs);
    entry.setCalledAt(Instant.now());
    entry.setErrorMessage(errorMessage);
    repository.save(entry);  // JPA 写入数据库（完整签名与 02 篇 §6.3 一致）
}
```

**Java 21 虚拟线程如何改变 @Async**：

```mermaid
graph TB
    subgraph 传统方案["传统 @Async（平台线程池）"]
        T1["核心线程数：2-4"]
        T2["最大线程数：10-20"]
        T3["队列：LinkedBlockingQueue"]
        T4["问题：高并发审计时<br/>队列积压 → 拒绝策略 → 丢日志"]
    end

    subgraph 虚拟线程["Java 21 虚拟线程模式"]
        V1["每个 @Async 任务一个虚拟线程"]
        V2["虚拟线程数：数万"]
        V3["IO 阻塞时自动让出载体线程"]
        V4["优势：无队列积压<br/>无拒绝策略<br/>每条审计日志都能写入"]
    end

    style 传统方案 fill:#ffcdd2
    style 虚拟线程 fill:#c8e6c9
```

在 `application-mcp.yaml` 中开启虚拟线程后，Spring Boot 4.1 的 `@Async` 自动使用虚拟线程执行器（`SimpleAsyncTaskExecutor` with virtual threads）。这意味着即使每秒数千次工具调用产生数千条审计日志，也不会因为线程池满而丢弃日志。

### 6.2 审计日志的容错

```java
// 节选——出自上方 record 方法体的容错逻辑（完整实现见 02 篇 §6.3）
try {
    AuditLog entry = new AuditLog();
    entry.setAgentId(agentId);
    entry.setToolName(toolName);
    repository.save(entry);
} catch (Exception e) {
    // 审计日志写入失败不能影响主业务
    log.error("Failed to write audit log for tool '{}': {}",
            toolName, e.getMessage());
}
```

**为什么审计失败不影响主业务**：

```mermaid
graph LR
    subgraph 调用链["工具调用 + 审计的关系"]
        CALL["工具调用<br/>主业务路径<br/>不能失败"]
        AUDIT["审计写入<br/>辅助路径<br/>允许失败"]
    end

    CALL -->|"成功后"| AUDIT
    AUDIT -.->|"失败时<br/>仅记录错误日志"| LOG["error log<br/>运维可感知"]

    style CALL fill:#c8e6c9
    style AUDIT fill:#fff9c4
    style LOG fill:#ffcdd2
```

如果审计写入失败导致工具调用也失败，那审计系统就从一个"监控工具"变成了一个"故障源"。正确的做法是审计失败时记录 error 级别日志（运维可以通过日志告警感知），然后继续运行。

对于要求"审计不可丢失"的场景（如金融合规），可以引入消息队列做缓冲层：审计日志先写入 Kafka/RabbitMQ，消费者负责写入数据库，队列自身提供重试和死信队列机制。

### 6.3 本节核对（异步与容错设计）

- [ ] 完整审计实现以 02 篇 §6.3 为准（本节为讲解摘录），参数签名一致（7 参 record：agentId/toolName/inputParams/outputResult/success/durationMs/errorMessage）
- [ ] `@Async` 生效前提（启动类 `@EnableAsync` + 虚拟线程开启）已在 02 篇 §3.5/§3.3 落地
- [ ] 审计失败不阻塞主业务的行为可在 02 篇 §6.5 断言 5 复核（响应先回、日志 error 可见）

---

## 7. 可观测核心代码

### 7.1 GatewayMetrics——指标设计

```java
// 节选自 GatewayMetrics（完整类见 02 篇 §6.2）——以下仅展示指标打点方法
public void recordToolCall(String serverName, String toolName,
                            boolean success, Duration duration) {
    Counter.builder("mcp.tool.calls")
            .tag("server", serverName)
            .tag("tool", toolName)
            .tag("result", success ? "success" : "failure")
            .register(registry)
            .increment();

    toolCallTimer.record(duration);
}
```

**Tag 的基数控制**：

```mermaid
graph TB
    subgraph Tag基数["Metrics Tag 基数分析"]
        GOOD["低基数 Tag（安全）<br/>server: 3 个值<br/>result: 2 个值<br/>组合：3 × N × 2"]
        BAD["高基数 Tag（危险）<br/>orderId: 无限<br/>userId: 无限<br/>可能导致内存溢出"]
    end

    subgraph 规则["Tag 基数规则"]
        R1["单 Tag 值数 < 100"]
        R2["所有 Tag 组合数 < 10000"]
        R3["绝不使用 requestId / userId 作为 Tag"]
    end

    style GOOD fill:#c8e6c9
    style BAD fill:#ffcdd2
    style 规则 fill:#e3f2fd
```

`server` 和 `tool` 是低基数 Tag（Server 数有限、工具数有限），适合做 Counter 的维度标签。`result` 只有 success/failure 两个值，也是安全的。但如果把 `orderId` 作为 Tag，每个订单都会创建一个新的 Time Series，最终导致 Prometheus 内存溢出——这是 Micrometer 使用中的常见陷阱。

### 7.2 Observation 的 Span 层级

```java
// 节选自 ToolRouter.call（完整实现见 02 篇 §5.3）——以下仅展示 Span 创建
return Observation.createNotStarted("mcp.tool.call", observationRegistry)
        .lowCardinalityKeyValue("tool", request.toolName())
        .observe(() -> doCall(request));
```

`observe()` 方法内部做了三件事：

```mermaid
sequenceDiagram
    participant Caller as 调用方
    participant Obs as Observation
    participant Reg as ObservationRegistry
    participant Target as doCall()

    Caller->>Obs: observe(supplier)
    Obs->>Reg: 创建 Span: mcp.tool.call
    Reg-->>Obs: Span 已注册
    Obs->>Target: 执行 supplier（doCall）

    Note over Target: 实际工具调用过程

    Target-->>Obs: 返回结果
    Obs->>Reg: 关闭 Span（记录耗时、状态）
    Obs->>Reg: 导出 Metrics（自动计时）
    Obs-->>Caller: 返回结果
```

`lowCardinalityKeyValue` 的命名暗示了一个重要原则：Span 的 tag 必须是低基数的。`tool` 标签记录的是工具名（如 `filesystem.read_file`），基数有限。如果误用 `highCardinalityKeyValue` 来记录高基数值（如 traceId），某些可观测后端可能会拒绝或产生过多 Time Series。

### 7.3 本节核对（指标与 Span 设计）

- [ ] 指标三 Tag（server/tool/result）均为低基数，符合 §7.1 基数规则（组合 < 10000）
- [ ] Span 名 `mcp.tool.call` 与 02 篇 §5.3 `Observation.createNotStarted` 参数一致
- [ ] 实际指标输出已由 02 篇 §6.5 断言 1 验证（prometheus grep）

---

## 8. MCP Server 工具定义核心

### 8.1 @Tool 注解的工作原理

```java
@Tool(description = "根据订单号查询订单详情，包括金额、状态和物流信息")
public OrderDetail queryOrder(
        @ToolParam(description = "订单号，格式 ORD-XXXXXX") String orderId
) {
    return orderService.queryDetail(orderId);
}
```

**@Tool 到 MCP 的自动映射**（依赖显式 `ToolCallbackProvider` Bean，见 [03-迭代二 §3.5](03-自定义MCP服务端.md)）：

```mermaid
graph LR
    subgraph Java["Java 侧"]
        ANNOT["@Tool 注解<br/>description='查询订单...'"]
        PARAM["@ToolParam 注解<br/>description='订单号'"]
        METHOD["queryOrder(String orderId)"]
        PROVIDER["MethodToolCallbackProvider<br/>显式 Bean（关键步骤）"]
    end

    subgraph MCP["MCP 侧（自动生成）"]
        TOOL_JSON["tools/list 响应：<br/>{<br/>  name: 'queryOrder',<br/>  description: '查询订单...',<br/>  inputSchema: {<br/>    type: 'object',<br/>    properties: {<br/>      orderId: {<br/>        type: 'string',<br/>        description: '订单号'<br/>      }<br/>    },<br/>    required: ['orderId']<br/>  }<br/>}"]
    end

    ANNOT -->|"映射"| TOOL_JSON
    PARAM -->|"映射"| TOOL_JSON
    METHOD -->|"反射调用"| TOOL_JSON
    PROVIDER -->|"装配入口"| TOOL_JSON

    style Java fill:#e3f2fd
    style MCP fill:#c8e6c9
```

Spring AI 在应用启动时扫描所有 `ToolCallbackProvider` Bean 中的 `@Tool` 注解方法，通过反射提取方法签名和注解元数据，自动生成对应的 MCP 工具 JSON Schema。**缺 `ToolCallbackProvider` Bean 时工具不生效**（附录 05-01 §3 审计教训）。

### 8.2 description 的质量至关重要

```java
// 好的 description——LLM 能准确判断何时使用此工具
@Tool(description = "根据订单号查询订单详情，包括金额、状态和物流信息。订单号格式：ORD-XXXXXX")
public OrderDetail queryOrder(@ToolParam(description = "订单号，格式 ORD-XXXXXX") String orderId) {
    return orderService.queryDetail(orderId);
}

// 差的 description——LLM 无法判断适用场景
@Tool(description = "查订单")
public OrderDetail queryOrder(@ToolParam(description = "订单号") String orderId) {
    return orderService.queryDetail(orderId);
}
```

MCP 工具的 `description` 会被 LLM 作为判断"何时调用此工具"的关键依据。描述越精确、越包含参数格式和返回内容，LLM 的工具选择准确率越高。这是 MCP 工具设计中容易被忽视但影响巨大的细节。

### 8.3 本节测试与验证（description 质量对照）

**前置条件**：03 篇 §3.6 已通过（mcp-cli 可连）。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | mcp-cli `tools/list` 查看 queryOrder 的 description | 与 §8.2 "好描述"一致（含订单号格式 ORD-XXXXXX） |
| 2 | 查看 inputSchema | `properties.orderId.description` 为 `订单号，格式 ORD-XXXXXX`，`required: ["orderId"]`（@ToolParam→Schema 映射正确） |
| 3 | 把 description 临时改成 `查订单` 再看 tools/list | Schema 变差但结构不变——验证 description 是 LLM 侧质量旋钮、不影响协议 |

**失败排查**：①tools/list 无 description→`@Tool` 漏写 description 属性；②参数描述缺失→`@ToolParam` 未标注（Spring AI 2.0 `@ToolParam` 仅有 `required()`/`description()`）。

---

## 9. 配置代码核心

### 9.1 虚拟线程配置

```yaml
spring:
  threads:
    virtual:
      enabled: true
```

**这行配置的底层变化**：

```mermaid
graph TB
    subgraph 开启前["未开启虚拟线程"]
        B1["Tomcat 线程池：200 个平台线程"]
        B2["每个 HTTP 请求占用 1 个线程"]
        B3["IO 阻塞 → 线程等待"]
        B4["200 并发 → 线程池满 → 拒绝请求"]
    end

    subgraph 开启后["开启虚拟线程"]
        A1["Tomcat 线程池：平台线程 + 虚拟线程"]
        A2["每个 HTTP 请求运行在虚拟线程上"]
        A3["IO 阻塞 → 虚拟线程让出，载体线程处理其他请求"]
        A4["10000+ 并发也能轻松应对"]
    end

    style 开启前 fill:#ffcdd2
    style 开启后 fill:#c8e6c9
```

对于 MCP 工具网关这个 IO 密集型应用，虚拟线程带来的提升是数量级的。每次 MCP 工具调用涉及网络 IO（JSON-RPC 到子进程或远程 Server），在传统模式下线程会阻塞等待，而在虚拟线程模式下，载体线程可以继续处理其他请求。

### 9.2 MCP 客户端配置的层次

```yaml
spring:
  ai:
    mcp:
      client:
        stdio:
          servers-configuration: classpath:mcp-servers.json
```

配置层次解析：

```mermaid
graph TB
    subgraph 配置层次["MCP 客户端配置层次"]
        L1["spring.ai.mcp<br/>MCP 总配置入口"]
        L2["spring.ai.mcp.client<br/>客户端配置"]
        L3["spring.ai.mcp.client.stdio<br/>stdio 传输配置"]
        L4["spring.ai.mcp.client.stdio.servers-configuration<br/>Server 配置文件路径"]
    end

    L1 --> L2
    L2 --> L3
    L3 --> L4

    style 配置层次 fill:#e3f2fd
```

如果需要同时连接远程 Streamable HTTP Server 与本地 stdio Server，并列配置：

```yaml
spring:
  ai:
    mcp:
      client:
        stdio:
          servers-configuration: classpath:mcp-servers.json
        streamable-http:                  # ⚠ 现行传输（附录 05-01 §4）
          connections:
            remote-tools:
              url: https://tools.internal/mcp
```

> ⚠ 修正（审计 2026-08-14）：旧稿的 `sse.connections` 结构对应已废弃的 HTTP+SSE 传输；现行标准是 `streamable-http.connections`（附录 05-01 §4 基准，实证键结构见 05 篇 §2.2）。

### 9.3 本节测试与验证（配置生效性）

**前置条件**：`application-mcp.yaml` 已按本节配置。

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 启动日志线程名抽查 | 请求处理线程为虚拟线程（名称含 `VirtualThread`/不带 tomcat 工作线程号，Boot 4.1 + `threads.virtual.enabled=true`） |
| 2 | yaml 层次核对 | stdio 与 `streamable-http.connections` 并列于 `spring.ai.mcp.client` 下，无旧 `sse.connections` 残留 |
| 3 | 加一条 `streamable-http.connections.remote-tools.url` 指向可用的远程 MCP 端点 | 启动后该 Server 的工具出现在 `/tools`（远程传输生效） |
| 4 | `mvn help:effective-properties` 或直接查 `/actuator/env` | `spring.threads.virtual.enabled=true` 生效 |

**失败排查**：①远程 Server 连不上→URL 不是 MCP 端点或网络不通（查 DEBUG 日志 initialize 失败）；②键名报未知属性→用了废弃的 `sse.connections`（附录 05-01 §4）。

---

## 10. 量化验收表（代码质量）

| 维度 | 检查项 | 达标标准 |
|------|--------|---------|
| **并发安全** | 共享 Map / 状态字段 / 读多写少列表 | `ConcurrentHashMap` / `volatile` / `CopyOnWriteArrayList`；连续 3 次失败才 DISCONNECTED |
| **异常处理** | 工具失败传播方式 | 用 `ToolCallResult` 而非异常；审计失败不阻塞主业务 |
| **API 真实性** | MCP 坐标与类型 | `McpSyncClient` + `CallToolRequest` + `ToolCallbackProvider` 均符合附录 05-01 |
| **可观测性** | Tag 基数 / Span | 低基数 Tag（组合 < 10000）；每次调用有 Observation Span |
| **安全防御** | 调用前校验 | 权限白名单 + 注入检测 + 高危阈值（金额 > STANDARD 5000 / PREMIUM 50000 拒绝） |

**项目级验收对照**：

| 验收项 | 达标标准 | 本迭代 |
|--------|---------|--------|
| 数据模型 | Record 不可变、`globalId()` 全局唯一（无同名冲突） | ✅ |
| 连接管理 | 四级状态机 + 连续 3 次失败才 DISCONNECTED | ✅ |
| 容错链路 | 重试仅瞬时故障、熔断打开降级返回 Map 不抛 5xx | ✅ |
| 安全模型 | 白名单 + 注入检测 + 高危金额阈值（5000/50000） | ✅ |
| 审计可观测 | `@Async` 不阻塞主业务、Tag 组合 < 10000 | ✅ |
| MCP 工具 | `@Tool` 经显式 Provider Bean 暴露、description 质量达标 | ✅ |

### 10.2 全篇回归验证（讲解代码可复现）

**回归断言**（§2.3 / §3.3 / §4.3 / §5.3 / §6.3 / §7.3 / §8.3 各节验证通过后，最终整体验收）：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | `mvn clean compile`（§9 配置 + 全类清单） | 04 篇全部类可编译，无虚构 API（对照附录 05-01） |
| 2 | 端到端三路径：成功 / 工具不存在 / 熔断打开 | 分别返回 `success=true` / `success=false`（Tool not found）/ 降级 Map（`degraded: true`） |
| 3 | 审计不阻塞：停数据库后触发调用 | 主调用返回成功，仅 error 日志（§6.2 行为可复现） |
| 4 | 指标对账：跑 100 次调用后 grep prometheus | `mcp_tool_calls_total` 计数 = 100（低基数 Tag 无爆炸） |

**失败排查**：①编译失败→API 拼写与附录 05-01 不符（`McpSyncClient`/`CallToolRequest`/`ToolCallbackProvider`）；②降级路径未触发→熔断阈值未到（核 §4.1 Decorators 链顺序）。

### 10.1 本节核对（验收表可回溯）

- [ ] 五个维度各能回溯到本篇对应验证（并发安全→§3.3 / 异常处理→§2.3 / API 真实性→§8.3 与附录 05-01 / 可观测→§7.3 / 安全→§5.3）
- [ ] "API 真实性"一行引用的三个类型（`McpSyncClient`/`CallToolRequest`/`ToolCallbackProvider`）与附录 05-01 条目一致

## 11. 代码质量自检清单

```mermaid
graph TB
    subgraph 自检["代码质量自检清单"]
        Q1["并发安全<br/>✓ ConcurrentHashMap 用于共享 Map<br/>✓ volatile 用于跨线程状态字段<br/>✓ CopyOnWriteArrayList 用于读多写少列表"]
        Q2["异常处理<br/>✓ 工具失败用 ToolCallResult 而非异常<br/>✓ 审计失败不阻塞主业务<br/>✓ 区分配置错误和运行时故障"]
        Q3["资源管理<br/>✓ Pattern 预编译为 static final<br/>✓ 连接移除时关闭 McpSyncClient<br/>✓ 审计日志异步写入不阻塞"]
        Q4["可观测性<br/>✓ Metrics Tag 控制基数<br/>✓ 每次调用都有 Observation Span<br/>✓ 审计日志携带完整上下文"]
        Q5["安全防御<br/>✓ 工具调用前做权限校验<br/>✓ 输入参数做注入检测<br/>✓ 高危工具有额外阈值检查"]
    end

    style 自检 fill:#c8e6c9
```

### 11.1 本节核对（自检清单逐项落地）

- [ ] 自检五组的每个 ✓ 项都能指到具体类/字段（如"volatile 用于跨线程状态字段"→`McpConnection.status`）
- [ ] 无仅存在于图而不存在于代码的检查项

---

## 12. ADR 演进决策

### ADR 03-10：所有共享可变状态收敛到并发容器，避免显式锁

- **决策**：工具缓存/连接池用 `ConcurrentHashMap`，连接状态用 `volatile`，工具列表用 `CopyOnWriteArrayList`；不引入 `synchronized`/`ReentrantLock`
- **取舍理由**：读多写少场景下无锁读收益最大；`volatile` 单字段状态 + 单线程写入方（健康检查定时任务）不会产生竞态，避免锁开销与死锁风险

### ADR 03-11：API 真实性是硬约束——存疑写法一律指向附录 05

- **决策**：本项目所有 MCP 代码以附录 05-01 为准（`McpSyncClient`、`CallToolRequest`、`MethodToolCallbackProvider`）；可观测代码以附录 05-02 为准（`Observation`、`Tracer`）
- **取舍理由**：2026-08 审计发现大量虚构/错误 API（`org.springframework.ai.mcp.McpClient`、`@ToolMethod`、单参 `before/after`、`defaultTools(provider)` 混写）——本项目的价值前提是"照抄可编译"。注意：`BaseAdvisor.before/after`（双参带 AdvisorChain）在 2.0.0 是真实存在的（javap 实证，2026-08-16），禁止的是**错误签名**

### 12.1 本节核对（ADR 一致性）

- [ ] ADR 03-10 引用的三类并发容器在 §3 代码中逐一出现
- [ ] ADR 03-11 的基准引用（附录 05-01/05-02）与本篇各验证小节的失败排查引用一致
- [ ] 两条 ADR 在 08 篇 ADR 汇总中有对应条目

---

## 13. 扩展方向

本项目的代码设计为后续扩展预留了清晰的接口：

| 扩展方向 | 实现路径 | 涉及接口 |
|---------|---------|---------|
| **动态注册 MCP Server** | 在运行时添加新 Server 连接到 Pool | `McpClientPool.register()` |
| **自定义供应策略** | 实现 `ToolSupplyStrategy` 接口 | `ToolSupplyStrategy` |
| **插件化安全规则** | 实现 `SecurityRule` 接口，注册到 Validator | `SecurityValidator` |
| **多级缓存** | 在 Router 前加 Caffeine 缓存层 | `ResilientToolRouter` |
| **工具版本管理** | ToolMetadata 增加 version 字段 | `ToolInfo` |
| **GraphQL 端点** | 新增 GraphQL Controller 复用 Router | `ResilientToolRouter` |
| **HITL 审批** | 用 `ToolCallingManager` 装饰器拦工具执行前 | `ToolCallingManager`（附录 05-02 §1.3） |

### 13.1 本节核对（扩展点真实存在）

- [ ] 表中"涉及接口"列的每个符号在当前代码/附录基准中真实存在（`McpClientPool.register()`、`ToolSupplyStrategy`、`ResilientToolRouter` 均为已实现类的方法）
- [ ] 未引用不存在的接口（如 `SecurityRule`——表中列为"实现路径"的待建概念，落地时需先定义）

---

## 14. 总结

本篇对 MCP 工具网关项目的全部核心代码做了深度解析：

1. **数据模型层**：Record 的不可变性和线程安全性使其成为 DTO 的最佳选择。`globalId()` 解决了多 Server 工具名冲突，`ToolCallResult` 的统一结果包装让失败处理更加优雅。

2. **连接管理**：`McpConnection` 实现了四级状态机（CONNECTING → CONNECTED → DEGRADED → DISCONNECTED），渐进式故障检测避免了误判。`ConcurrentHashMap` 的无锁读完美匹配"多读少写"场景。

3. **容错调用链**：Resilience4j 的 `Decorators` 链式组合了熔断、重试、降级三种策略。重试只对瞬时故障生效（参数错误不重试），降级返回结构化 Map 而非 null。

4. **安全模型**：三级 Agent 权限（ADMIN/PREMIUM/STANDARD）+ 工具白名单 + 注入检测 + 高危操作阈值。`Pattern` 预编译为 `static final` 避免重复编译开销。

5. **审计与可观测**：`@Async` + 虚拟线程让审计写入不阻塞主业务。Metrics Tag 严格控制基数防止 Time Series 爆炸。Observation 自动创建 Span 和计时。

6. **MCP 工具定义**：`@Tool` 注解通过显式 `ToolCallbackProvider` Bean 装配后自动生成 MCP JSON Schema。`description` 质量直接影响 LLM 的工具选择准确率。

整个项目的代码设计遵循"简洁而不简陋"的原则——用最少的代码实现完整的生产级功能，每个设计决策都有明确的性能或安全考量。Java 21 的 Record、虚拟线程、模式匹配三个特性在项目中发挥了关键作用，显著降低了代码复杂度。Spring Boot 4.1 + Spring AI 2.0 的自动配置让 MCP Client/Server 的接入几乎零配置，开发者可以专注于业务逻辑而非协议细节。

**下一篇**：[05-MCPStreamableHTTP与OAuth21深化](05-MCPStreamableHTTP与OAuth21深化.md)——传输升级 Streamable HTTP + OAuth 2.1 三重身份授权。

**一句话核对**：总结六点分别对应 §2/§3/§4/§5/§6–7/§8，无新增结论。
