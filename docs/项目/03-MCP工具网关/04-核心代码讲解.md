...
>
> **前置阅读**：[01-最小 Demo 搭建](01-最小Demo搭建.md)、[02-迭代一-MCP 客户端集成](02-迭代一-MCP客户端集成.md)、[03-迭代二-自定义 MCP 服务端](03-迭代二-自定义MCP服务端.md)。API 真实性以 [附录 05-SpringAI2-API基准](../../附录/05-SpringAI2-API基准/01-MCP真实API与坐标.md) 为准。
...
**@Tool 到 MCP 的自动映射**（依赖显式 `ToolCallbackProvider` Bean，见 [03-迭代二 §3.5](03-迭代二-自定义MCP服务端.md)）：
...
Spring AI 在应用启动时扫描所有 `ToolCallbackProvider` Bean 中的 `@Tool` 注解方法，通过反射提取方法签名和注解元数据，自动生成对应的 MCP 工具 JSON Schema。**缺 `ToolCallbackProvider` Bean 时工具不生效**（附录 12-01 §3 审计教训）。
...
> ⚠ 修正（审计 2026-08-14）：旧稿的 `sse.connections` 结构对应已废弃的 HTTP+SSE 传输；现行标准是 `streamable-http-connections`（附录 12-01 §4 基准）。
...
| **API 真实性** | MCP 坐标与类型 | `McpSyncClient` + `CallToolRequest` + `ToolCallbackProvider` 均符合附录 12-01 |
...
### ADR 03-09：API 真实性是硬约束——存疑写法一律指向附录 12

- **决策**：本项目所有 MCP 代码以附录 12-01 为准（`McpSyncClient`、`CallToolRequest`、`MethodToolCallbackProvider`）；可观测代码以附录 12-02 为准（`Observation`、`Tracer`）
- **取舍理由**：2026-08 审计发现大量虚构 API（`org.springframework.ai.mcp.McpClient`、`@ToolMethod`、`BaseAdvisor.before/after`、`defaultTools` 混写）——本项目的价值前提是"照抄可编译"
...
| **HITL 审批** | 用 `ToolCallingManager` 装饰器拦工具执行前 | `ToolCallingManager`（附录 12-02 §1.3） |
