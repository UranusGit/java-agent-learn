...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四项需求可见 | VAD 断句/流式ASR/流式TTS/打断闭环在 §三 关键实现与 §四 管道事件中均有落点，非只列需求 |
| 2 | 四条验收可度量 | 150ms 终稿 / ≤400ms 首句 / ≤200ms 停播 / 无残留，均可作为 §四.1 断言判据 |
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | partial→final 语义 | ASR partial("帮我查北")在 VAD 尾静音 150ms 出 final("帮我查北京天气")，与 §一 "尾静音 150ms 终稿"口径一致 |
| 2 | 打断双信号 | 打断拆为停播(≤100ms)+取消在途(≤100ms)=200ms，与 BARGE-IN/cancel 两条消息对应，无合并不清 |
...
### 四.1 本节测试与验证（全双工流式与打断）

> **前置**：流式 ASR（partial/final）+VAD 终句+流式 TTS+打断通道实现，管道事件已按 §四 埋点。
...
| 5 | 管道事件可见性 | 事件流含 VadBoundary→AsrDelta(final)→Tts 序列；打断时可见 `interrupted=true` 的 AsrDelta 定格 |

**失败排查**：①终句慢→VAD 静默窗参数过大；②超 400ms→TTS 冷启动（合成器常驻或首句预暖）；③漏音→取消只停了播放器未 cancel TTS 流（双停：停播+取消生成）；④事件看不到→变换点未接 §四 `PipelineStageListener`。
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 全程无打断正常对话多轮 | 每轮 partial→final→首句≤400ms→TTS 起播，无环节掉线 |
| 2 | 混合压测（含多次打断+重说） | 四验收项（150ms/400ms/200ms/无残留）PASS 率不降，事件流无错序 |

**失败排查**：混合轮某验收项降级→回查 §四.1 对应行排除项；事件错序→§四 时序对齐/序标签问题。
...
> ### 六.1 本节核对（本迭代痛点）
> "工具调用阻塞首音频"是 §四.1 断言 2 之外的新场景，与下一迭代 [03] 的"填充语先开口/工具并行/抢占"一一对应即 PASS。
...
> ### 七.1 本节核对（验收对照）
> 四项验收均能在 §四.1 断言中找到对应（流式断句→1、首句→2、打断→3、无残留→4），状态为 ✅ 即 PASS。

**下一篇**：[03-实时编排与抢占生成](03-实时编排与抢占生成.md)。
