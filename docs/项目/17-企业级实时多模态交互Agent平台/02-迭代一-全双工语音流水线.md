...
## 四、管道阶段可见（结合 AudioChunk/VAD/打断 的真实管道）

> 过程可见性是工业级标配。**实时项目对可见性要求最高**——对话进行中，用户/运维必须实时知道当前处在管道哪一段（VAD 边界→ASR 转写→意图→工具→TTS 合成），否则打断/误判时无从定位。可见性挂在二、三节真实管道数据流（`AudioChunk` 流入 → ASR 分段 → 生成 → TTS 回流）的**每个变换点**：

```java
// 概念代码：在真实管道变换点发阶段事件（结合 AudioChunk/VAD/bargeIn）
record StageEvent(String utteranceId, Stage stage, String delta, long latencyMs, boolean interrupted) {
    enum Stage { VAD_BOUNDARY, ASR_PARTIAL, ASR_FINAL, INTENT, TOOL, TTS_START, TTS_DELTA, TTS_END }
}
// ASR 出 partial: StageEvent(id, ASR_PARTIAL, text, ms)
// VAD 终句: StageEvent(id, ASR_FINAL, finalText, ms, interrupted)
// 用户打断 bargeInSignal: StageEvent(id, TTS_DELTA, ..., interrupted=true)
```

| 管道段 | 阶段事件（对齐项目真实） | 用户看到 |
|--------|-------------------------|---------|
| VAD | `VAD_BOUNDARY` | "检测到我开始说话" |
| ASR | `ASR_PARTIAL/ASR_FINAL` | 实时字幕（可中途纠正） |
| 生成 | `INTENT`/`TOOL` | "识别为查订单→调工具" |
| TTS | `TTS_START/TTS_DELTA/TTS_END` | "正在合成回复…已返回" |

**四条纪律**：① 每段 Begin/End 事件成对，中断（打断 bargeIn/超时）也发 `End` + `interrupted=true`——用户知道卡在哪段；② 阶段事件带 `latencyMs`——实时项目用它做延迟预算监控（本项目第一性"延迟预算"的可见化）；③ 打断即定格（bargeIn 时把进行中段标记 interrupted、已产出的字幕/音频不丢）；④ AsrDelta 只给转写文本，不把内部置信度外泄（与对外话术分离）。
