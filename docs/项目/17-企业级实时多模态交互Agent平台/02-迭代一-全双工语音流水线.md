...
> 过程可见性是工业级标配（[附录 14/01-中间过程可见性设计]）。**实时项目对可见性要求最高**——对话进行中，用户/运维必须实时知道当前处在管道哪一段（VAD 边界→ASR 转写→意图识别→工具调用→TTS 合成），否则打断/误判时无从定位。

二、节的管道每段 emit 实时阶段事件：`{callId, 阶段, 状态, delta, 延迟}`：

| 管道段 | 事件 | 用户看到 |
|--------|------|---------|
| VAD | VadBoundary{utterance_ready}` | "检测到我开始说话" |
| ASR | AsrDelta{text, partial/final}` | 实时字幕（可中途纠正） |
| 意图 | IntentRecognized{intent}` | "识别为查询订单" |
| 工具 | ToolCall{name, ts_begin/end}` | "正在查订单…已返回" |
| TTS | TtsStart/TtsDelta` | "正在合成回复" |

**四条纪律**：
1. **每段成对 Begin/End**：管道一段开始/结束成对，中断（打断/超时）也发 End(failed)——用户知道卡在哪段。
2. **阶段事件与实测延迟绑定**：每段带 `延迟` 字段，实时项目用它做延迟预算监控（17 项目第一性"延迟预算"的可见化）。
3. **打断即定格**：用户打断时，将进行中的管道段标记 interrupted 并定格该段已产出的部分（不丢字幕/已生成语音）——对齐 05 流中合规的"定格"思想。
4. **不泄露文本预期**：AsrDelta 只给转写文本，不把内部意图置信度外泄给用户（内部话术 vs 对外展示分离）。

**验证**（叠加本迭代验证包）：录一段"查天气"→ 事件流顺序 Vad→Asr(final)→Intent→ToolCall→TtsStart→TtsEnd；中途打断 → 进行中段发 End(interrupted) 且此前 delta 已定格；每段带延迟字段。
