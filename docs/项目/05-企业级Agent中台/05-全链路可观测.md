# 项目 05：企业级 Agent 中台 — 05-全链路可观测

> **定位**：跨服务 Trace 贯通——一次 Agent 调用从 agent-platform → llm-gateway → LLM → tool-service 的完整 Span 树，gen_ai 语义约定统一指标口径。教程 00-基础与核心/03-工具调用 §17 在微服务体系中的落地。**观测代码（网关分段计时 / 工具审计观测 / 标签贯通）完整可手写**。
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/02-全链路可观测性 全篇]、[教程 04-企业级架构主干/03-工具执行可观测与审计 全篇]、[教程 01-WebFlux与响应式编程/08-跨服务流与RedisStreams背压 §跨服务传播]；Observation 真实 API 以 [附录 05-02 §3] 为准」

---

## 1. 需求与上一版痛点（四问）

| 问 | 答 |
|----|----|
| **新增了什么需求** | 任一会话可一键调出完整调用链；P99 延迟按"LLM/工具/编排"分段可查；工具调用参数与结果（脱敏后）入审计库 |
| **影响了哪些模块** | 不加新服务——各服务接 OTel SDK + OTLP 上报；新增 OTel Collector + Prometheus + Grafana 观测平面（部署物） |
| **架构如何演进** | 观测是横切能力：W3C traceparent 跨服务自动传播 + gen_ai 语义约定统一指标 + 审计流独立落库 |
| **上一版痛点是什么** | 排障人肉拼三个系统时间线；指标口径不一；工具审计分散；"慢"无法定位 |

| v4 痛点 | 本次迭代对策 |
|---------|-------------|
| 排障要人肉拼三个系统的时间线 | 全链路 Trace（traceparent 跨服务传播） |
| 指标口径不一（网关记 model 名、业务记业务线） | 统一标签集 + gen_ai 语义约定 |
| 工具调用审计分散 | Tool Call Span 统一结构 + 审计流独立落库 |
| "慢"无法定位（LLM 慢？工具慢？排队慢？） | Span 分层计时（模型 TTFT / 工具执行 / 网关排队） |

### 1.1 本节核对（四问与痛点对策）

- 四问完整；四行对策在 §3 各有落点（传播→§3.1、口径→§3.4、审计→§3.3、分段→§3.2）。

## 2. 目标 Span 树

```mermaid
flowchart TB
    ROOT["agent.session (agent-platform)<br/>business_line=cs session=abc"] --> CHAT["chat.prompt 轮次"]
    CHAT --> GW["llm-gateway.proxy<br/>gateway.queue_ms=12"]
    GW --> MODEL1["gen_ai.client.chat (DeepSeek)<br/>ttft=480ms total=3.2s"]
    CHAT --> TOOL["gen_ai.tool.call (order.query)<br/>args_hash=... duration=150ms"]
    TOOL --> TSC["tool-service.execute<br/>db.query=120ms"]
    MODEL1 -. "第2轮工具后继续生成" .-> CHAT
    CHAT --> MODEL2["gen_ai.client.chat (round 2)<br/>ttft=350ms total=1.8s"]

    style ROOT fill:#fff9c4
    style GW fill:#e3f2fd
    style MODEL1 fill:#f3e5f5
    style TOOL fill:#e8f5e9
```

一次"查询订单并回复"的对话 = 1 个 session span，内含 2 轮模型调用 + 1 次工具调用，跨 3 个服务——链路完整。

### 2.1 本节核对（目标 Span 树）

- 目标树中一次会话 = 1 个 session span + 多个 model/tool 子 span，跨 3 服务；与验收项 1 的口径一致。

## 3. 关键实现

### 3.1 跨服务传播

需在 pom.xml 中添加依赖（各数据面服务）——注意是 **javaagent 挂载方式**（`-javaagent` 参数）而非 starter 依赖方式，避免 SDK 手动配置与自动装配的双重注册冲突（[附录 05-02 §3.2]）：

```xml
<!-- OTel javaagent：作为部署工件（-javaagent:opentelemetry-javaagent.jar），
     不进入运行时 classpath，scope=provided 仅用于随构建分发版本 -->
<dependency>
    <groupId>io.opentelemetry.javaagent</groupId>
    <artifactId>opentelemetry-javaagent</artifactId>
    <version>2.7.0</version>
    <scope>provided</scope>
</dependency>
```

- **HTTP 跳**（agent-platform → llm-gateway）：W3C `traceparent` 头自动传播（javaagent 默认行为）
- **MCP 跳**（agent-platform → tool-service）：MCP Streamable HTTP 本身是 HTTP，traceparent 同样透传——**选择 MCP 的隐性红利**
- **业务语义标签**：`business_line` / `session_id` 通过 Span 属性显式写入（见 §3.4），保证跨服务后标签不丢

各服务部署配置（docker/k8s 环境变量，挂 javaagent + 指向 OTel Collector）：

```yaml
# 所有数据面服务统一注入（以 llm-gateway 为例）
OTEL_SERVICE_NAME: llm-gateway
OTEL_EXPORTER_OTLP_ENDPOINT: http://otel-collector:4317
OTEL_METRICS_EXPORTER: otlp
OTEL_TRACES_EXPORTER: otlp
# 采样策略（ADR-016）：头采样 10% + 错误/慢请求 100% 尾采样保留
OTEL_TRACES_SAMPLER: traceidratio
OTEL_TRACES_SAMPLER_ARG: "0.1"
# JVM 参数
# -javaagent:/opt/otel/opentelemetry-javaagent.jar
```

Spring AI 观测开关（键名随版本核对，[附录 05-02 §3.1]）——按项目统一的两段式配置口径：`application.yaml` 只做 `.env` 导入与 profile 激活，各数据面服务各自的业务配置放 `application-<service>.yaml`：

```yaml
# application.yaml（各数据面服务统一的全局骨架）
spring:
  config:
    import: optional:file:.env[.properties]
  profiles:
    active: agenthub
```

```yaml
# application-agent-platform.yaml（示例：观测开关追加到各服务自己的业务配置，llm-gateway / tool-service 同理）
spring:
  ai:
    chat:
      observations:
        log-prompt: false   # javap 实证：真实键是 log-prompt/log-completion（无 include-prompt-content）——不记录 Prompt 原文（PII 最小化）
        log-completion: false
```

> 启动命令统一带 profile：`mvn spring-boot:run -Dspring-boot.run.profiles=agenthub`；多服务同机启动时用 `--server.port` 区分端口。

#### 3.1.1 本节测试与验证（跨服务传播与采样）

**前置条件**：OTel Collector、Prometheus/Tempo（或等价 Trace 后端）就绪；三个数据面服务均按 §3.1 挂 `-javaagent` 并注入 OTEL_* 环境变量。

**材料——一次含工具调用的会话探针**（业务侧 `/cs/chat` 触发一次工具调用；工具授权按 04 篇 §3.3 配置，cs 线未授权时改用 da 线或已授权工具）：

```bash
# ① 含工具调用的客服会话：贯穿 agent-platform → llm-gateway → tool-service（traceparent 跨服务传播）
curl -N -X POST http://localhost:8080/cs/chat -H "Authorization: Bearer $CRED_VALUE" \
  -H "Content-Type: application/json" \
  -d '{"sessionId":"s-obs-1","userId":"u-obs","message":"查一下订单 SO-2026001 的状态"}'

# ② 直接打网关的代理探针（复用 02 篇 §3.11 材料 B，验证 traceparent 经网关跳）
curl -N -X POST "http://localhost:9090/v1/chat/completions" \
  -H "Authorization: Bearer $CRED_VALUE" -H "Content-Type: application/json" \
  -d '{"model":"default","messages":[{"role":"user","content":"你好"}],"stream":true}'

# ③ 步骤 3 用：连发 20 次请求（10% 头采样下留存约 2 条 Trace）。hey 或 for 循环：
hey -n 20 -c 4 -m POST -T "application/json" -H "Authorization: Bearer $CRED_VALUE" \
  -d '{"sessionId":"s-obs-loop","userId":"u-obs","message":"查一下订单 SO-2026001 的状态"}' \
  http://localhost:8080/cs/chat
# for 循环等价：for i in $(seq 1 20); do curl -s -o /dev/null -X POST http://localhost:8080/cs/chat \
#   -H "Authorization: Bearer $CRED_VALUE" -H "Content-Type: application/json" \
#   -d '{"sessionId":"s-obs-loop","userId":"u-obs","message":"查一下订单 SO-2026001 的状态"}' & done; wait
```

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 发起一次跨 3 服务的会话（含工具调用） | Trace 后端可查到同一 traceId 下 agent-platform/llm-gateway/tool-service 三段 Span（traceparent 贯通） |
| 2 | 检查各服务启动日志 | 无 OTel SDK 双重注册告警（javaagent 方式，ADR-015） |
| 3 | 连发 20 次请求（材料③ hey/for 循环） | 采样后留存 Trace 约 2 条（10% 头采样）；其中出错/慢的请求 100% 保留（尾采样） |
| 4 | 抽查 Span 属性 | `spring.ai.chat.observations.log-prompt=false` 生效——Span 事件中无 Prompt 原文（PII 最小化） |

**失败排查**：步骤 1 链路断在网关→供应商段 → 该跳是出网 HTTPS，供应商侧无 Span 属正常，断点应在 tool-service；步骤 2 双重注册 → 同时引了 SDK starter；步骤 4 出现 Prompt 原文 → 配置键写错（无 `include-prompt-content`，真实键是 `log-prompt/log-completion`）。

### 3.2 网关侧：分段计时（回答"慢在哪"）

**`obs/GatewayObservation.java`（llm-gateway）**——Spring AI 2.0 / Micrometer 真实 API：

```java
package com.acme.gateway.obs;

import io.micrometer.observation.Observation;
import io.micrometer.observation.ObservationRegistry;

import org.springframework.stereotype.Component;

import reactor.core.publisher.Flux;

/** 网关分段计时：llm.gateway.proxy Span，携带 business_line 标签与排队时长。 */
@Component
public class GatewayObservation {

    private final ObservationRegistry registry;

    public GatewayObservation(ObservationRegistry registry) {
        this.registry = registry;
    }

    /** 包装一次上游转发：观测在转发开始处 start、流结束后 stop。 */
    public <T> Flux<T> proxy(String businessLine, long queuedAt, Flux<T> upstream) {
        Observation obs = Observation.createNotStarted("llm.gateway.proxy", registry)
                .lowCardinalityKeyValue("business_line", businessLine)
                .start();
        return upstream
                .doOnComplete(() -> finish(obs, queuedAt))
                .doOnCancel(() -> finish(obs, queuedAt))
                .doOnError(e -> obs.error(e).stop());
    }

    private void finish(Observation obs, long queuedAt) {
        obs.highCardinalityKeyValue("gateway.queue.ms",
                        String.valueOf((System.nanoTime() - queuedAt) / 1_000_000))
                .stop();
    }
}
```

在 `LlmGatewayHandler` 中接入（在路由解析后、转发前取排队起点）：

```java
// LlmGatewayHandler 中新增字段：
// private final GatewayObservation gatewayObservation;

// 转发前：
long queuedAt = System.nanoTime();
Flux<DataBuffer> upstream = webClient.post()
        .uri(route.endpoint() + "/chat/completions")
        .headers(h -> h.setBearerAuth(route.apiKey()))
        .contentType(MediaType.APPLICATION_JSON)
        .bodyValue(ctx.rewriteBodyFor(route, body))
        .retrieve()
        .bodyToFlux(DataBuffer.class);

Flux<DataBuffer> observed = gatewayObservation.proxy(bizLine, queuedAt,
        usageMeteringInterceptor(upstream, ctx));
return ServerResponse.ok()
        .contentType(MediaType.TEXT_EVENT_STREAM)
        .body(BodyInserters.fromDataBuffers(observed));
```

> 观测命名与标签遵循 gen_ai 语义约定（`gen_ai.operation.name`、`gen_ai.request.model` 等，[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §gen_ai 语义约定]；TTFT 从 SSE 首帧时间戳计算——`bodyToFlux(DataBuffer)` 的首个 buffer 到达时刻即可近似首帧）。

#### 3.2.1 本节测试与验证（网关分段计时）

**前置条件**：`GatewayObservation` 已按 §3.2 接入 `LlmGatewayHandler`；Micrometer/OTel 导出已通（§3.1.1 通过）。

**材料——网关代理探针**（复用 02 篇 §3.11 材料 B，直接打网关验证 `llm.gateway.proxy` Span）：

```bash
curl -N -X POST "http://localhost:9090/v1/chat/completions" \
  -H "Authorization: Bearer $CRED_VALUE" -H "Content-Type: application/json" \
  -d '{"model":"default","messages":[{"role":"user","content":"你好"}],"stream":true}'
```

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 经网关发一次流式请求（本节约 §3.2.1 材料） | 出现 `llm.gateway.proxy` Span，含 `business_line` 低基数标签 |
| 2 | 正常完成后查 Span 属性 | `gateway.queue.ms` 存在且为非负毫秒值（排队时长） |
| 3 | 中途断连（Ctrl+C） | Span 仍被 stop（`doOnCancel` 分支），不是悬挂 Span |
| 4 | 上游报错（改错 endpoint 触发） | Span 带错误标记（`obs.error(e).stop()`），错误可从瀑布图直接看到 |

**失败排查**：Span 悬挂不闭合 → 只挂了 start 没在 complete/cancel/error 三分支 stop；queue.ms 恒为 0 → queuedAt 取点放在了转发之后。

### 3.3 工具调用审计流

**`obs/ToolAuditObservationHandler.java`（tool-service）**——Micrometer 真实 `ObservationHandler<ToolCallingObservationContext>`：

```java
package com.acme.tool.obs;

import com.acme.tool.audit.AuditSink;

import io.micrometer.observation.Observation;
import io.micrometer.observation.ObservationHandler;
import io.micrometer.tracing.Tracer;   // ⚠ 需引入依赖 io.micrometer:micrometer-tracing（本地未下载，未 javap 实证）
import org.springframework.ai.tool.observation.ToolCallingObservationContext;   // Spring AI 2.0.0 真实类

/** 工具调用观测处理器：工具调用明细（名称/时长/TraceId）入独立审计库（与业务库分离，ADR-017）。 */
public class ToolAuditObservationHandler implements ObservationHandler<ToolCallingObservationContext> {

    private final AuditSink auditSink;
    private final Tracer tracer;

    public ToolAuditObservationHandler(AuditSink auditSink, Tracer tracer) {
        this.auditSink = auditSink;
        this.tracer = tracer;
    }

    @Override
    public boolean supportsContext(Observation.Context context) {
        return context instanceof ToolCallingObservationContext;
    }

    // Observation.Context 无 getDuration()（javap 实证）；时长需用 ctx.put/get 计时
    private static final Object START_KEY = new Object();

    @Override
    public void onStart(ToolCallingObservationContext ctx) {
        ctx.put(START_KEY, System.nanoTime());   // Observation.Context.put(Object, T)
    }

    @Override
    public void onStop(ToolCallingObservationContext ctx) {
        String toolName = ctx.getToolDefinition().name();
        Long startNanos = ctx.get(START_KEY);
        long durationMs = startNanos != null ? (System.nanoTime() - startNanos) / 1_000_000 : -1;
        // TraceId 真实获取路径：Tracer.currentSpan().context().traceId()（[附录 05-02 §3.1]）
        String traceId = tracer.currentSpan() == null
                ? "n/a"
                : tracer.currentSpan().context().traceId();
        auditSink.emit(toolName, durationMs, traceId);
    }
}
```

**`audit/AuditSink.java` 与 `config/ObservationConfig.java`**：

```java
package com.acme.tool.audit;

/** 审计流：工具调用 100% 有记录，trace_id 可回链。 */
public interface AuditSink {
    void emit(String toolName, long durationMs, String traceId);
}
```

```java
package com.acme.tool.config;

import com.acme.tool.audit.AuditSink;
import com.acme.tool.obs.ToolAuditObservationHandler;

import io.micrometer.observation.ObservationHandler;
import io.micrometer.tracing.Tracer;   // ⚠ 需引入依赖 io.micrometer:micrometer-tracing（本地未下载，未 javap 实证）
import org.springframework.ai.tool.observation.ToolCallingObservationContext;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

@Configuration
public class ObservationConfig {

    /** Spring Boot 自动把 ObservationHandler Bean 组装进 ObservationRegistry。 */
    @Bean
    public ObservationHandler<ToolCallingObservationContext> toolAuditHandler(
            AuditSink auditSink, Tracer tracer) {
        return new ToolAuditObservationHandler(auditSink, tracer);
    }
}
```

**`audit/AsyncAuditSink.java`**（脱敏后入独立审计库的落点示意——生产替换为独立审计存储）：

```java
package com.acme.tool.audit;

import java.util.concurrent.BlockingQueue;
import java.util.concurrent.LinkedBlockingQueue;

import org.springframework.stereotype.Component;

@Component
public class AsyncAuditSink implements AuditSink {

    // 生产环境替换为独立审计库（与业务库分离、保留期 1-5 年、独立访问权限）
    private final BlockingQueue<AuditRecord> queue = new LinkedBlockingQueue<>(10_000);

    @Override
    public void emit(String toolName, long durationMs, String traceId) {
        queue.offer(new AuditRecord(toolName, durationMs, traceId));
    }

    public record AuditRecord(String toolName, long durationMs, String traceId) {}
}
```

审计与 Trace 的分工（[教程 01-WebFlux与响应式编程/07-WebFlux测试与性能调优 §审计 vs 监控]）：Trace 服务于排障（保留 7 天），审计服务于合规（保留按策略 1-5 年），**不同存储、不同保留期、不同访问权限**。

#### 3.3.1 本节测试与验证（工具调用审计流）

**前置条件**：tool-service 已注册 `toolAuditHandler` Bean（§3.3 ObservationConfig）；`micrometer-tracing` 依赖已加（文中标注未 javap 实证，签名以引入后实测为准）。

**材料——触发一次工具调用**（经业务侧会话落到 tool-service，验证审计记录）：

```bash
curl -N -X POST http://localhost:8080/cs/chat -H "Authorization: Bearer $CRED_VALUE" \
  -H "Content-Type: application/json" \
  -d '{"sessionId":"s-obs-2","userId":"u-obs","message":"查一下订单 SO-2026001 的状态"}'
```

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 触发一次工具调用（本节约 §3.3.1 材料） | `AsyncAuditSink` 队列新增一条 AuditRecord：toolName/durationMs/traceId |
| 2 | durationMs 合理性 | > 0 且量级与 tool-service 日志耗时一致（`ctx.put/get` 计时生效，非 -1） |
| 3 | traceId 回链 | 用审计记录的 traceId 去 Trace 后端能查到对应调用链 |
| 4 | 审计存储位置 | 与业务库分离（独立审计存储/队列），非同库同表 |

**失败排查**：无审计记录 → `supportsContext` 没匹配 `ToolCallingObservationContext` 或 Handler 未注册为 Bean；traceId 恒为 "n/a" → Handler 执行线程上无当前 Span（异步切线程导致，需上下文传递）。

### 3.4 业务语义标签贯通（business_line/session_id 跨服务不丢）

`business_line` 等业务标签由业务侧用 Micrometer Tracing 的 `Tracer` 写入当前 Span 属性：

```java
package com.acme.agent.platform.obs;

import io.micrometer.tracing.Span;   // ⚠ 需引入依赖 io.micrometer:micrometer-tracing（本地未下载，未 javap 实证）
import io.micrometer.tracing.Tracer;   // ⚠ 需引入依赖 io.micrometer:micrometer-tracing（本地未下载，未 javap 实证）

import org.springframework.stereotype.Component;

/** 业务语义标签写入当前 Span（session 根 Span 上打标，随 traceparent 贯通全链路）。 */
@Component
public class BusinessLineTagging {

    private final Tracer tracer;

    public BusinessLineTagging(Tracer tracer) {
        this.tracer = tracer;
    }

    public void tag(String businessLine, String sessionId) {
        Span span = tracer.currentSpan();
        if (span != null) {
            span.tag("business_line", businessLine);
            span.tag("session_id", sessionId);
        }
    }
}
```

在 `DefaultAgentChatService.chat()` 入口调用一次 `tag(cmd.businessLine(), cmd.sessionId())`，该标签随 W3C traceparent 传播到 llm-gateway 与 tool-service 的所有下游 Span。

#### 3.4.1 本节测试与验证（业务标签贯通）

**前置条件**：`BusinessLineTagging.tag()` 已在 `DefaultAgentChatService.chat()` 入口调用。

**材料——cs / km 两线会话**（验证 business_line 标签随链路走、不串线）：

```bash
# cs 线会话
curl -N -X POST http://localhost:8080/cs/chat -H "Authorization: Bearer $CRED_VALUE" \
  -H "Content-Type: application/json" -d '{"sessionId":"s-tag-cs","userId":"u-tag","message":"你好"}'
# km 线会话（换线，验证标签不串）
curl -N -X POST http://localhost:8080/km/chat -H "Authorization: Bearer $CRED_VALUE" \
  -H "Content-Type: application/json" -d '{"sessionId":"s-tag-km","userId":"u-tag","message":"你好"}'
```

**步骤与断言**：

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 发一次 cs 线会话 | 根 Span 及下游所有 Span（网关/工具）均带 `business_line=cs`、`session_id` 属性 |
| 2 | 换 km 线再发一次 | 新链路标签为 km——标签随链路走、不串线 |
| 3 | Grafana 按 business_line 聚合 metric | 三线数据可分组查询（标签在 Metric 上也不丢） |

**失败排查**：下游 Span 缺标签 → 标签打在了非根 Span 或 tag 时机晚于下游发起；Metric 上无标签 → meter 打点处未附 tag（02 篇网关已带 business_line，对照检查）。

### 3.5 Grafana 三板斧

1. **服务拓扑图**：基于 Trace 的服务依赖图（agent-platform → gateway/tool → 供应商）
2. **延迟瀑布模板**：任选 session_id → 面板渲染该会话全部 Span 的瀑布图（排障入口）
3. **成本看板**：`gen_ai.client.token.usage` 按业务线/模型的日聚合（与 v2 网关计量互为印证）

### 3.5.1 本节核对（Grafana 三板斧）

- 三个面板各自有数据来源：拓扑图←Trace 服务依赖、瀑布图←按 session_id 的 Span 序列、成本看板←`gen_ai.client.token.usage` 聚合（与 02 篇网关计量互为印证）。

## 4. 验收标准

| # | 验收项 | 标准 |
|---|--------|------|
| 1 | 链路完整 | 随机抽 100 个含工具调用的会话，≥99 条可拉出跨 3 服务的完整 Span 树 |
| 2 | 标签贯通 | business_line 标签在所有服务的 Span/Metric 上不丢失 |
| 3 | 延迟分段 | P99 延迟可按 queue/model-ttft/model-total/tool 拆分查询 |
| 4 | 审计落库 | 工具调用 100% 有审计记录（toolName/durationMs/traceId）且 trace_id 可回链 |
| 5 | 排障时效 | "该会话为什么慢"从 30 分钟人肉拼图 → 1 分钟瀑布图 |
| 6 | 观测开销 | 开启全量 Trace 后服务吞吐下降 < 5%（采样策略按 [教程 01-WebFlux与响应式编程/06-线程模型与调度器 §采样]） |

### 4.1 全篇回归验证（可观测整体）

> 探针材料已按主题上移：传播与采样→§3.1.1、分段计时→§3.2.1、审计流→§3.3.1、标签贯通→§3.4.1。本节只做跨维度回归。

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 抽 100 个含工具调用的历史会话按 traceId 拉链 | ≥99 条可拉出跨 3 服务完整 Span 树（验收项 1） |
| 2 | 任选一个"慢会话"，从瀑布图定位慢段 | ≤1 分钟给出"慢在哪一段"结论（验收项 5）；P99 可按 queue/ttft/total/tool 拆分 |
| 3 | 开全量采样压测一轮（hey 压测命令见 §3.1.1 材料③，对照关闭采样） | 吞吐下降 < 5%（验收项 6） |

**失败排查**：抽样链路缺失率高 → MCP 跳的 traceparent 未透传（检查是否走了非 HTTP 传输）；吞吐降幅大 → 头采样比例配置未生效（仍是全量）。

## 5. 本迭代的 ADR

| # | 决策 | 理由 |
|---|------|------|
| ADR-015 | javaagent 挂载而非 SDK starter + 手写 SdkTracerProvider | 避免双重注册；业务代码零侵入（审计切面除外） |
| ADR-016 | Trace 采样：头采样 10% + 错误/慢请求 100% 尾采样保留 | 全量成本不可接受；排障最需要的是异常链路 |
| ADR-017 | 审计存工具调用明细（toolName/durationMs/traceId），trace_id 可回链 | ObservationHandler 拿不到工具入参原文（`ToolCallingObservationContext` 无入参字段，javap 实证口径）；参数级脱敏原文留待 ToolCallingManager 装饰器阶段深化（[教程 08-架构师进阶/09-Agent治理与合规框架 §审计]） |

### 5.1 本节核对（ADR）

- 三条 ADR 可回指正文落点：ADR-015→§3.1 javaagent 方式、ADR-016→§3.1 采样环境变量、ADR-017→§3.3 审计记录（toolName/durationMs/traceId，trace_id 可回链）。

## 6. v5 的痛点（驱动下一迭代）

链路通了，财务提出了新问题：**"审计报告里的业务线是自己声明的吗？"**——business_line 来自调用方自报（短期凭证签发时绑定），但工具执行、会话存储的资源消耗没有硬隔离；数据线的重查询高峰仍会挤占共享存储的 IO。**隔离需要从"标签"升级为"边界"**。→ [06-业务线隔离与资源治理.md](06-业务线隔离与资源治理.md)

### 6.1 本节核对（v5 痛点衔接）

- 痛点与本篇一致：business_line 确为"标签"而非"边界"（§3.4 只写 Span 属性）；隔离升级（配额/资源边界）由 06 篇承接。
