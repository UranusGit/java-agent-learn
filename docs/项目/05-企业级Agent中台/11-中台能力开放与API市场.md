# 项目 05：企业级 Agent 中台 — 11-迭代九：中台能力开放与 API 市场
...
>
> 「遇到阻塞？→ [教程 31-安全与权限控制 §API Key 与 OAuth]、[教程 20-管控分离架构 §控制面职责]、[教程 26-多租户隔离与资源治理 §配额分级]」
...
| 测试类 | 用例 | 断言 |
|--------|------|------|
| `AgentClientPropertiesTest` | 声明式 YAML 绑定（含 tier 缺省、SLA 缺省 silver） | 属性正确绑定；缺省值生效 |
| `CredentialRefresherTest` | 续期失败沿用旧凭证；成功后原子替换 | `current()` 不出现空窗；续期失败不抛出到业务链 |
| `SlaPolicyServiceTest` | 三档优先级/降级顺序/预算保护组合（含容量压力开关） | 金牌永不因压力降档；铜牌压力时先降 |
| `ApiKeyAuthWebFilterTest` | 无 Key / 伪造 Key / 有效 Key 三类请求 | 401 / 401 / 放行且 Context 带接入方身份；内部头被剥除 |

### 4.2 端到端验证

```sh
# ① 自助接入演练：新起 hr-demo 应用，仅声明 acme.agent.* 配置
#    启动日志应出现：凭证签发成功 + 能力目录校验通过（声明的能力全部在目录中）
# ② 伪造内部头：外部请求带 X-Agent-Ns: cs
curl -s -o /dev/null -w "%{http_code}\n" "http://open-gateway:9070/open/v1/agent/chat" \
  -H "X-Api-Key: <partner-key>" -H "X-Agent-Ns: cs" -d @chat.json
#    预期 200 但审计中 ns = partner 登记身份（伪造头被剥除）
# ③ SLA 抢占：压测打满共享容量，观察铜牌请求先收到 X-Agent-Degraded: tier
```

### 4.3 市场运营验证

月度跑计费批处理：中台收入（各业务线转账单合计）与 `gen_ai.client.token.usage` 聚合 × 价格线逐笔核对，差异 < 0.5%（与 v6 三方对账同一口径纪律）。
...
| 内部头防伪造 | 外部伪造 X-Agent-Ns 100% 被剥除 | 端到端用例 ② |
...
市场开放后接入方数量翻倍，两件事同时暴露：① v8 的容灾演练是**季度手工剧本**，接入方越多"演练覆盖不过来"，且监管要求常态化证据链；② 新接入方没有容量意识，营销线一次活动把共享容量打满，银牌全线受影响——**扩容靠拍脑袋，且扩容不看预算**。→ [12-迭代十-混沌工程与容量治理.md](12-迭代十-混沌工程与容量治理.md)
