...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 全篇]、[教程 04-企业级架构主干/12-模型路由与降级 §6 模型健康检查与自动切换]、[教程 04-企业级架构主干/13-部署与运维 §3.5 Deployment]」
...
「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 §6.4 熔断器的三种状态]、[教程 04-企业级架构主干/12-模型路由与降级 §6.1 健康检查器]——流式调用的故障判定要看"首帧超时"而非"整体超时"，避免长回答被误杀」

#### 3.1.1 本节测试与验证（健康探测）

**前置条件**：`RouteHealthTracker`（被动窗口）与低频主动探活（`@EnableScheduling`）已上线。

**材料——失败流量注入**（toxiproxy 对主供应商做代理，逐级注入超时，观察窗口 FAILURE 判定）：

```bash
# toxiproxy：主供应商 endpoint → 本地代理（llm-gateway 的 cs 主路由指向 localhost:10001）
toxiproxy-cli create deepseek-main -l localhost:10001 -u https://api.deepseek.com:443
# 注入 100% 超时（上游 10s 无响应），使窗口失败率越过 0.5 阈值
toxiproxy-cli toxic add deepseek-main -t timeout -a latency=10000 -a jitter=0
# 观察探活占比（验收项 4）：主动探活标记来源的调用 < 总调用 0.5%
...
**材料——故障注入**（iptables 屏蔽 cs 主供应商出网；IP 用 `getent hosts api.deepseek.com` 解析）：

```bash
# 屏蔽（使 cs 主模型流量 100% 失败）
iptables -A OUTPUT -d <PRIMARY_IP> -p tcp --dport 443 -j DROP
# 触发切换后验证 supplier 标签
curl http://localhost:9090/actuator/prometheus | grep 'gateway.llm.requests'
# 恢复主供应商
iptables -D OUTPUT -d <PRIMARY_IP> -p tcp --dport 443 -j DROP
```
...
**材料——两阶段断连注入**（toxiproxy，复用 §3.1.1 的 `deepseek-main` 代理）：

```bash
# ① 首帧前断连：注入 60s 超时，首字节永远不到（触发静默重试下一路由）
toxiproxy-cli toxic add deepseek-main -t timeout -a latency=60000
# ② 首帧后断连：注入 limit_data，流到 2048 字节后切断（触发 recoverable 错误，不自动重发）
toxiproxy-cli toxic remove deepseek-main --toxic timeouts   # 先移除①
toxiproxy-cli toxic add deepseek-main -t limit_data -a bytes=2048
# 恢复
toxiproxy-cli toxic remove deepseek-main --toxic limit_data
```
...
```bash
# 停掉 prompt-service（docker compose / kubectl 二选一）
docker compose stop prompt-service
# kubectl scale deployment prompt-service --replicas=0
# 运行混沌测试：resolve 返回缓存/出厂值而非抛错
mvn -Dtest=ControlPlaneImmunityTest test
# 恢复
docker compose start prompt-service
```
