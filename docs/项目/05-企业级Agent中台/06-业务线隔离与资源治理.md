...
> **定位**：命名空间从"自报标签"升级为"硬边界"——会话/记忆/审计数据的命名空间隔离、Token 与工具调用的配额治理、按业务线的成本归因报表闭环。教程 59/27 的中台化落地。**隔离与配额代码完整可手写**。
>
> 「遇到阻塞？→ [教程 59-多租户隔离与资源治理 §三层隔离模型]、[教程 60-成本治理与Token计量 §预算与归因]；WebFlux 上下文正确姿势见 [教程 85-响应式错误处理 §6 响应式上下文]」
...
三层隔离的理念来自 [教程 59-多租户隔离与资源治理 §2 多租户隔离三层模型]；与该教程的差异：本项目业务线数量少（3）且同属一个集团，**不采用 schema 级硬隔离**（ADR-004）——比 SaaS 场景轻一级，比"纯标签"重一级。

### 2.1 本节核对（三层模型图）

- 三层各自有 §3 落点：L1 凭证绑定→§3.1、L2 命名空间→§3.1 消费端/审计 ns 列、L3 配额→§3.2。
- 与教程 26 的差异声明（不做 schema 硬隔离）与 ADR-004 一致。

## 3. 关键实现

### 3.1 命名空间强制注入（消灭"自报"）

**`platform/security/NamespaceFilter.java`（agent-platform）**——WebFilter 从验签注入的 `X-Agent-Ns` 头解析 ns 写入 Reactor Context（**禁止 ThreadLocal**——WebFlux 线程切换后失效，[教程 85-响应式错误处理 §6 响应式上下文]）：
...
**`service/QuotaService.java`（policy-service）**——ReactiveRedisTemplate + Lua 原子脚本（响应式铁律，[教程 59-多租户隔离与资源治理 §5.1 Token 配额]）：
...
**为什么预检+扣减必须一条 Lua**：分离的两步（GET 判断 → DECRBY）在并发下必然超卖（[教程 59-多租户隔离与资源治理 §5.1 Token 配额] 的审计教训）。500 并发压测下分离实现实测超卖 3.7%（见 [09-核心代码讲解 §2.4]）。
...
| ADR-020 | 配额用 Redis Lua 原子扣减 | 预检与扣减分离会超卖；本地 Bucket 方案在多实例下失效（[教程 59-多租户隔离与资源治理 §5.1] 审计案例） |
