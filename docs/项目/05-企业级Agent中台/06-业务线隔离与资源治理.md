...
|---|------|------------------|
| 1 | 按验收项「防伪造」构造场景执行 | 外部伪造 X-Agent-Ns 头 100% 被剥除；ns 只能来自凭证链 |
| 2 | 按验收项「配额原子性」构造场景执行 | 500 并发压测单 ns 配额扣减，无超卖（扣减总额 ≤ 配额） |
| 3 | 按验收项「超限降级」构造场景执行 | 预算耗尽的业务线自动切廉价模型，服务不中断，响应带降级标记 |
| 4 | 按验收项「数据隔离」构造场景执行 | 任一业务线无法读到其他线的会话/审计/向量数据（自动化隔离测试矩阵） |
| 5 | 按验收项「存储隔离」构造场景执行 | 数据线高峰 IO 不再影响客服线（存储配额生效） |
| 6 | 按验收项「对账」构造场景执行 | 月度三方数据差异 < 1%，差异自动告警 |
