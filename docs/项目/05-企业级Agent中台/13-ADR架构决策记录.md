...
>
> 「遇到阻塞？→ [附录 07-架构决策方法论/00-ADR架构决策记录]（ADR 通用方法论与模板）」
...
- "散落无复利"的论断与事实一致：ADR-001~037 确实分散在 00/02-12 各篇；ADR 三问（上下文/备选/取舍）在 §四 的必填项中可执行。
...
**决策领域分布**（五个领域为并列分类、无先后关系；计数合计 37，覆盖全部 ADR）：
...
flowchart TB
    ...
    subgraph GWDIM["网关/计量/凭证 (7)"]
        ...
        B5["ADR-026 首帧后不重试"]
        B6["ADR-028 分档规则先行"]
        B7["ADR-029 Key/供应商判定分离"]
    end
    subgraph CPD["控制面与治理 (10)"]
        ...
        C7["ADR-022 小流量不自动门禁"]
        C8["ADR-023 灰度平台化"]
        C9["ADR-024 K8s/应用内灰度分层"]
        C10["ADR-027 控制面免疫"]
    end
    subgraph ISOL["隔离/成本/工具治理 (9)"]
        D1["ADR-004 命名空间"]
        D2["ADR-012 统一 MCP"]
        D3["ADR-013 工具下线宽限"]
        D4["ADR-014 工具组部署"]
    ...
    subgraph OPEN["开放/观测/合规 (9)"]
        E1["ADR-015 javaagent 观测"]
        E2["ADR-016 Trace 采样"]
        E3["ADR-017 审计"]
        E4["ADR-031 市场复用注册中心"]
        E5["ADR-032 SLA 资源映射"]
...
- 索引 37 条编号连续（ADR-001~037）无跳号、无重复；"产生篇"列与各迭代篇实际记载的 ADR 一一对应（抽查：ADR-005~007→02 篇、ADR-028~030→10 篇）。
- 每条均有"领域"与"状态"列；状态非"✅ 采纳"的条目注明了取代者/失效条件。
- 决策领域分布图五个领域计数合计 37（2+7+10+9+9），覆盖全部 ADR 无遗漏（此前版本只画 25 条的失真已修正）。
...
- 每条详解均含五要素：上下文/备选（≥2，含被否方案）/决策取舍/后果与可回滚性；抽查任意两条（如 ADR-036）能对上总索引同编号条目。
...
- 自检清单 4 条可执行：①被否备选有记录（抽查 ADR-036 备选 A/B）②重评触发条件有范例（ADR-002/003/004）③代价诚实（ADR-033 OAuth 二期即代价记录）④Spring AI API 已实证（铁律 0 引用完整）。
...
- 14 篇存在且编号衔接；本篇为决策资产收官篇，无运行时内容，无需运行验证。

## 六、全篇回归验证（ADR 总集一致性）

| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 对照总索引逐条回各产生篇「本迭代的 ADR」 | 37 条编号/领域/状态与各篇记载一致，无跳号、无重复（§二.1） |
| 2 | 分布图五个领域计数合计 | = 37（2+7+10+9+9），覆盖全部 ADR 无遗漏 |
| 3 | 抽 ADR-036 / ADR-019 详解与正文实现 | 详解的备选与取舍与对应篇目实现一致（§三 五要素完整） |
| 4 | 状态字段扫描 | 无"✅ 采纳"但正文已被后续事实推翻的条目（ADR-004 标"条件失效"合规；ADR-025 注明被 ADR-029 修正） |

**失败排查**：任一核对不通过 → 该 ADR 编号/领域/状态与产生篇不同步，回到对应篇目修正后重跑。
