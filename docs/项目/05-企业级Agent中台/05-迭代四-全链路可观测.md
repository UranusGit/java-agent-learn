...
>
> 「遇到阻塞？→ [教程 16-全链路可观测性 全篇]、[教程 17-工具执行可观测与审计 全篇]、[教程 18 §跨服务传播]；Observation 真实 API 以 [附录 12-02 §3] 为准」
...
需在 pom.xml 中添加依赖（各数据面服务）——注意是 **javaagent 挂载方式**（`-javaagent` 参数）而非 starter 依赖方式，避免 SDK 手动配置与自动装配的双重注册冲突（[附录 12-02 §3.2]）：
...
Spring AI 观测开关（application.yml，键名随版本核对，[附录 12-02 §3.1]）：
...
spring:
  ai:
    chat:
      observations:
        include-prompt-content: false   # 不采集 Prompt 原文（PII 最小化）
```
...
**`obs/ToolAuditObservationHandler.java`（tool-service）**——Micrometer 真实 `ObservationHandler<ToolObservationContext>`：
...
import org.springframework.ai.tool.observation.ToolObservationContext;
...
public class ToolAuditObservationHandler implements ObservationHandler<ToolObservationContext> {
    ...
    public void onStop(ToolObservationContext ctx) {
        ...
        long durationMs = ctx.getDuration().toMillis();
        // TraceId 真实获取路径：Tracer.currentSpan().context().traceId()（附录 12-02 §3.1）
...
public class ObservationConfig {
    ...
    @Bean
    public ObservationHandler<ToolObservationContext> toolAuditHandler(
...
链路通了，财务提出了新问题：**"审计报告里的业务线是自己声明的吗？"**——business_line 来自调用方自报（短期凭证签发时绑定），但工具执行、会话存储的资源消耗没有硬隔离；数据线的重查询高峰仍会挤占共享存储的 IO。**隔离需要从"标签"升级为"边界"**。→ [06-迭代五-业务线隔离与资源治理.md](06-迭代五-业务线隔离与资源治理.md)
