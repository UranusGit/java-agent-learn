# 项目 05：企业级 Agent 中台 — 12-迭代十：混沌工程与容量治理
...
| 测试类 | 用例 | 断言 |
|--------|------|------|
| `ChaosGuardWebFilterTest` | 带/不带 X-Chaos-Request 头的请求 | 染色流量 Context 含 scenario；生产流量零写入 |
| `ChaosWatchdogTest` | abortIf 断言越线 / 正常两态（注入固定 PromQL 返回） | 越线时 revokeAll 恰好调用一次且 active 置空 |
| `ScenarioLoaderTest` | 场景 YAML 全量加载（含 S1-S6） | 字段完整、window 可解析、expect 语法合法 |
| `CapacityBudgetGateTest` | 余量充足/不足两态；预留后归还 | Lua 原子性（并发 200 预留不超卖）；归还后余量复原 |

### 4.2 演练验证（场景库实测）

| 场景 | 注入 | 关键断言结果（目标） |
|------|------|--------------------|
| S1 杀实例 | kill -9 单网关实例 | 染色流量成功率 ≥ 99%（其余实例接住，v8 反亲和红利） |
| S2 注入延迟 | toxiproxy 上游 +2s | 降级链触发，染色流量 P99 < 8s；非染色零波及 |
| S3 断数据源 | NetworkPolicy 断 Redis | 配额走"最后已知配额+本地限速"降级（policy 免疫扩展），审计零丢失 |
| S4 Key 耗尽 | mock 429 100% | v9 Key 池摘除 + 降级链，成功率 ≥ 99% |
| S5 控制面全停 | scale 0 三服务 | 数据面 1 小时可用性 100%（v8 剧本 C 的常态化） |

### 4.3 容量验证

k6 阶梯压测实测单实例拐点 → 公式预测大促实例数 → 大促实测对比（目标误差 < 15%）；HPA 压缩到预算门 max 后继续压测，验证"拒绝 + 铜牌降档"路径生效。
