...
>
> 「遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计 全篇]、[教程 04-企业级架构主干/13-部署与运维 §HPA 与容量规划]、[教程 04-企业级架构主干/07-成本治理与Token计量 §预算与归因]」
...
/** 演练场景模型：与 YAML 一一对应（加载见 ScenarioLoader）。 */
...
**`chaos/ScenarioLoader.java`（chaos-runner）**——场景 YAML → Scenario 模型（Jackson YAML；需在 chaos-runner 的 pom.xml 添加依赖 `com.fasterxml.jackson.dataformat:jackson-dataformat-yaml`）：
...
/** 场景 YAML 加载：chaos/scenarios/*.yaml → Scenario 模型（字段完整、window 可解析、expect 语法合法）。 */
@Component
public class ScenarioLoader {

    private final ObjectMapper yamlMapper = new ObjectMapper(new YAMLFactory());

    public Scenario load(String classpathResource) {
        try (InputStream in = getClass().getResourceAsStream(classpathResource)) {
            if (in == null) {
                throw new IllegalStateException("场景文件不存在: " + classpathResource);
            }
            String yaml = new String(in.readAllBytes(), StandardCharsets.UTF_8);
            return yamlMapper.readValue(yaml, Scenario.class);
        } catch (Exception e) {
            throw new IllegalStateException("场景 YAML 解析失败: " + classpathResource, e);
        }
    }
}
```
...
**`chaos/PrometheusClient.java` / `chaos/InjectionController.java` / `chaos/AssertionEvaluator.java`**（chaos-runner 适配器，最小可编译实现）：
...
/** 演练断言的数据源：查 Prometheus PromQL，取瞬时值首个样本。 */
@Component
public class PrometheusClient {
    ...
    public PrometheusClient(WebClient.Builder webClientBuilder) {
        this.prometheus = webClientBuilder.baseUrl(System.getenv("PROMETHEUS_URL")).build();
    }

    public Mono<Double> query(String promql, Duration range) {
        return prometheus.get()
                .uri(uri -> uri.path("/api/v1/query")
                        .queryParam("query", promql)
                        .queryParam("timeout", range.getSeconds() + "s")
                        .build())
                .retrieve()
                .bodyToMono(String.class)
                .map(this::parseFirstValue)
                .onErrorReturn(0.0);   // 查询失败按 0 处理（断言侧再判断）
    }

    private double parseFirstValue(String body) {
        try {
            JsonNode result = objectMapper.readTree(body).path("data").path("result");
            if (result.isArray() && result.size() > 0) {
                JsonNode value = result.get(0).path("value");
                return value.size() >= 2 ? value.get(1).asDouble(0.0) : 0.0;
            }
...
/** 注入控制器：封装 toxiproxy / kubectl 调用，撤销演练注入（止血优先，失败只告警不重抛）。 */
@Slf4j
@Component
public class InjectionController {

    /** 撤销当前场景的全部注入：toxiproxy 删 toxic、kubectl 恢复副本。 */
    public void revokeAll(Scenario scenario) {
        String scenarioId = scenario.scenarioId();
        run("toxiproxy-cli", "toxic", "remove", scenarioId + "-proxy",
                "--toxic", scenarioId + "-toxic");
        // kubectl 恢复被 scale 0 的部署：从注入器参数反查，此处示意
        log.warn("[止血] 已发起注入撤销: scenario={}", scenarioId);
    }
    ...
    private void run(String... cmd) {
        try {
            ...
            log.warn("注入撤销命令执行失败: {} — {}", String.join(" ", cmd), e.getMessage());
        }
    }
}
```
...
/** 断言求值：解析 ">= 0.99" / "< 0.005" 形式的期望串并比较实测值。 */
public final class AssertionEvaluator {

    private AssertionEvaluator() {}

    public static boolean matches(double value, String expect) {
        String e = expect.trim();
        if (e.startsWith(">=")) return value >= Double.parseDouble(e.substring(2).trim());
        if (e.startsWith("<=")) return value <= Double.parseDouble(e.substring(2).trim());
        if (e.startsWith(">")) return value > Double.parseDouble(e.substring(1).trim());
        if (e.startsWith("<")) return value < Double.parseDouble(e.substring(1).trim());
        if (e.startsWith("==")) return Math.abs(value - Double.parseDouble(e.substring(2).trim())) < 1e-9;
        if (e.startsWith("=")) return value == Double.parseDouble(e.substring(1).trim());
        throw new IllegalArgumentException("不支持的期望语法: " + expect);
    }
}
```

> 以上三类为 chaos-runner 的最小可编译实现；chaos-runner 需在 pom.xml 添加 lombok 依赖（项目统一 @Slf4j）。
...
**材料——单测类**（JUnit 5 + Mockito；场景文件 `chaos/scenarios/*.yaml` 需在 classpath）：

```java
// ChaosGuardWebFilterTest：带/不带 X-Chaos-Request 头的请求
class ChaosGuardWebFilterTest {
    @Test
    void 染色流量写入Context生产流量零写入() {
        // 带 X-Chaos-Request: S4 → Reactor Context 含 chaos.scenario=S4
        // 不带 → Context 无该键（生产流量零开销）
    }
}

// ChaosWatchdogTest：abortIf 越线/正常两态（Mockito mock PrometheusClient）
class ChaosWatchdogTest {

    private final Scenario s4 = new ScenarioLoader().load("/chaos/scenarios/s4-key-exhaustion.yaml");

    @Test
    void abortIf越线时止血恰好一次() {
        PrometheusClient prom = mock(PrometheusClient.class);
        InjectionController injection = mock(InjectionController.class);
        when(prom.query(anyString(), any())).thenReturn(Mono.just(0.01));   // 实测 0.01 > 期望 0.005 → 越线
        ChaosWatchdog watchdog = new ChaosWatchdog(prom, injection);
        watchdog.arm(s4);
        watchdog.checkAbort();
        verify(injection, times(1)).revokeAll(s4);   // 恰好一次且 active 置空
    }
    ...
    void 正常时不止血() {
        PrometheusClient prom = mock(PrometheusClient.class);
        InjectionController injection = mock(InjectionController.class);
        when(prom.query(anyString(), any())).thenReturn(Mono.just(0.001));  // < 0.005 → 正常
        ChaosWatchdog watchdog = new ChaosWatchdog(prom, injection);
        watchdog.arm(s4);
        watchdog.checkAbort();
        verifyNoInteractions(injection);
    }
}

// ScenarioLoaderTest：场景 YAML 全量加载
class ScenarioLoaderTest {
    @Test
    void 场景YAML全量加载() {
        Scenario s = new ScenarioLoader().load("/chaos/scenarios/s4-key-exhaustion.yaml");
        assertEquals("S4-key-exhaustion", s.scenarioId());
        assertEquals(3, s.assertions().size());   // availability/degraded-visible/no-cascade
        assertEquals(1, s.abortIf().size());
    }
}
```
...
```bash
# 步骤4 S1 杀单网关实例（染色流量下）
kubectl delete pod -l app=llm-gateway -l chaos=true
# 步骤5 S2 toxiproxy 上游 +2s（延迟注入）
toxiproxy-cli create chaos-proxy -l localhost:10002 -u https://api.deepseek.com:443
toxiproxy-cli toxic add chaos-proxy -t latency -a latency=2000
# 步骤6 S5 控制面三服务 scale 0
kubectl scale deployment config-service policy-service prompt-service --replicas=0
# 步骤7 断言越线（把 abortIf expect 临时改为不可能值后重跑 watchdog）：<30s 撤销注入并归档止血记录
```
...
public class CapacityBudgetGate {
    ...
    /** 自动批准阈值：余量需 ≥ 扩容成本 × 3（留一月缓冲，见 §3.2 图）。 */
    private static final long AUTO_RESERVE_MULTIPLIER = 3L;

    /**
     * 扩容请求：三分支决策（ADR-037，与 §3.2 图一致）。
     * AUTO   = 余量 ≥ 成本×3（留一月缓冲）→ 自动批准 + 预留；
     * HITL   = 余量不足但金牌 SLA 受影响 → 人工审批（预算 vs SLA 取舍，通知 CFO）；
     * REJECT = 余量不足且非金牌 → 拒绝 + 替代方案（铜牌先降档/错峰/缓存）。
     */
    public Mono<GateDecision> tryReserve(String service, int addInstances,
                                         long costPerInstanceMonthly, boolean goldSlaAffected) {
        long reserveAmount = addInstances * costPerInstanceMonthly;
        long autoThreshold = reserveAmount * AUTO_RESERVE_MULTIPLIER;
        return reactiveRedis.execute(reserveScript,
                        List.of("budget:capacity:" + service + ":" + java.time.YearMonth.now()),
                        List.of(String.valueOf(autoThreshold), String.valueOf(monthlyCapacityBudget(service))))
                .next()
                .map(result -> {
                    if (result >= 0) {
                        return new GateDecision(Decision.AUTO, true, result, reserveAmount);
                    }
                    return goldSlaAffected
                            ? new GateDecision(Decision.HITL, false, 0, reserveAmount)
                            : new GateDecision(Decision.REJECT, false, 0, reserveAmount);
                });
    }
    ...
    private long monthlyCapacityBudget(String service) {
        // v11：容量预算表从 config-service / DB 读取；此处按服务返回常量示意
        return switch (service) {
            case "llm-gateway" -> 8_000_000L;
            case "agent-platform" -> 6_000_000L;
            default -> 3_000_000L;
        };
    }

    public enum Decision { AUTO, HITL, REJECT }

    public record GateDecision(Decision decision, boolean approved, long remaining, long reserved) {}
}
```
...
**材料——单测类**（JUnit 5；三分支决策与预留归还）：

```java
// CapacityBudgetGateTest：AUTO/HITL/REJECT 三分支 + 预留后归还（并发预留不超卖）
class CapacityBudgetGateTest {
    @Test
    void 余量充足自动批准() {
        // mock ReactiveRedisTemplate 执行 Lua 返回 ≥ 0 → Decision.AUTO 且 approved=true，已预留
    }

    @Test
    void 余量不足金牌转人工() {
        // Lua 返回 -1 且 goldSlaAffected=true → Decision.HITL（预算 vs SLA 取舍，通知 CFO）
    }

    @Test
    void 余量不足非金牌拒绝() {
        // Lua 返回 -1 且 goldSlaAffected=false → Decision.REJECT（铜牌先降档/错峰/缓存）
    }

    @Test
    void 预留后归还余量复原() {
        // tryReserve(AUTO) 预留 → release() 归还 → 余量复原（零泄漏，验收项"预留可回滚"）
    }
}
```
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 单测：`CapacityBudgetGateTest`——AUTO/HITL/REJECT 三分支；预留后归还 | Lua 原子性（并发 200 预留不超卖）；余量充足走 AUTO、金牌不足走 HITL、非金牌不足走 REJECT；归还后余量复原（零泄漏） |
| 2 | k6 阶梯压测（50→100→200→400 并发） | 得到 P99 越线拐点吞吐；按公式得单实例安全吞吐 S（拐点 × 0.7） |
| 3 | 公式预测大促实例数 vs 大促实测 | 误差 < 15% |
| 4 | HPA 压缩到预算门 max 后继续压测 | 「REJECT + 铜牌降档」路径生效（扩容被门拦下并给出替代方案） |
