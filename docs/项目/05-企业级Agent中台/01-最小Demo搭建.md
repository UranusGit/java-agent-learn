...
>
> 「遇到阻塞？→ [教程 30-微服务拆分与Agent部署 §模块化单体]、[教程 01-Spring-AI框架入门]、[教程 02-ChatClient与对话模型]、[教程 03-工具调用]；API 真实性以 [附录 05-SpringAI2-API基准] 为准」
...
> 拆分时机的判断信号（何时必须拆）见 [教程 30-微服务拆分与Agent部署 §8.1 拆还是不拆？]，本项目的实际触发点在迭代二之前揭晓。
