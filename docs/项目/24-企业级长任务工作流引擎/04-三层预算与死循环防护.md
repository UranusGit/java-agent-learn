...
> **定位**：长任务的经济红线——**绝不"烧钱跑飞"、绝不"死循环空转"**。核心：**① 三层预算闸门（轮次/Token/时长）② 死循环检测（"循环不进展"，而不只是最大轮次）③ 超限 → DEGRADED 转 HITL**。呼应 [60-成本治理与Token计量](../../教程/04-企业级架构主干/07-成本治理与Token计量.md)、[81-Agent性能优化](../../教程/08-架构师进阶/04-Agent性能优化.md)。前置阅读：[03-幂等重试与副作用登记](03-幂等重试与副作用登记.md)。
>
...
**Token 计量**：用 Spring AI 2.0 的 `Usage`（`Usage.outputTokens()` 实证，配 [27-成本治理](../../教程/04-企业级架构主干/07-成本治理与Token计量.md)）在每个工具/模型调用后累加。
...
```bash
mvn test -Dtest=BudgetGateTest        # 按 §二 代码手写三闸超限用例后执行
# 预期输出（节选）：
#   Tests run: 4, Failures: 0, Errors: 0, Skipped: 0
#   轮次/Token/时长三闸各拦截 1 例、未超限放行 1 例
#   BUILD SUCCESS
```
...
```bash
mvn test -Dtest=DeadLoopDetectorTest        # 按 §三 代码手写推进/打转/刷新用例后执行
# 预期输出（节选）：
#   Tests run: 3, Failures: 0, Errors: 0, Skipped: 0
#   原地打转序列触发 true、推进序列恒 false、中途刷新不误判
#   BUILD SUCCESS
```
...
预算超限/死循环触发时，不直接 kill（避免破坏幂等闭合），而是**安全进入 DEGRADED**：当前步收尾 → 状态置 DEGRADED → 转 HITL（呼应 [28-Human-in-the-Loop](../../教程/04-企业级架构主干/08-Human-in-the-Loop与审批流.md)）由人决定：加预算续跑 / 暂停 / 取消。
...
> 各节断言已上移至 §二.1（三层预算闸门）与 §三.1（死循环检测）；本表为整篇迭代的回归验收，不重复材料。

| # | 验收项（断言） | 标准 | 复验方式 |
|---|---------------|------|---------|
| 1 | 轮次闸门 | 超 `maxRounds` 第 4 次返回 `false` | 复验：执行 §二.1 核对命令 |
| 2 | Token 闸门 | `usedTokens > tokenBudget` 即拦截 | §二.1 步骤2 |
| 3 | 时长闸门 | 超 `maxDuration` 即拦截；未超限放行 | §二.1 步骤3-4 |
| 4 | 死循环检测 | 重复指纹 ≥ 窗口触发 `true`；推进序列恒 `false` | 复验：执行 §三.1 核对命令 |
| 5 | 中途刷新不误判 | 指纹变化刷新 `lastProgressRound`，短时内不误杀 | §三.1 步骤3 |
| 6 | 超限安全降级 | 当前步收尾→DEGRADED→转 HITL（不瞬间 kill） | §四（联动 02 状态机） |

**回归失败排查**：按 §二.1/§三.1 失败排查逐条回溯（比较方向写反/Token 未累加/指纹粒度过粗/阈值方向错）。

## 七、验收对照

> 00-需求分析量化验收③（预算超限 100% 被拦截）在本迭代收口。

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 预算超限 100% 拦截 | 轮次/Token/时长任一出界即 DEGRADED，不继续花 Token | ✅（§二.1 步骤1-3） |
| 死循环可检测 | "循环不进展"（重复指纹）被掐断 | ✅（§三.1 步骤2） |
| 预算信息全程可观测 | `usedTokens`/`rounds` 随调用累加可读 | ✅（§二.1 步骤2/§二 代码） |
