...
> **定位**：把"散乱的任务状态"收敛为**单写者 Actor 任务核 + 统一任务生命周期**。核心：**① 每个长任务一个 Actor（单写者、有界命令）② 统一生命周期状态机（RUNNING→PAUSING→PAUSED/COMPLETED/CANCELLED/DEGRADED）③ 命令具备暂停/恢复/取消钩子**。呼应 [22-会话引擎 Actor核](../../项目/22-企业级Agent会话引擎/02-Actor核与任务生命周期.md)。前置阅读：[01-最小Demo](01-最小Demo.md)、[12-教程状态管理](../../教程/02-SpringAI核心机制/02-Agent状态管理.md)。
>
...
```bash
mvn test -Dtest=TaskActorTest        # 按 §三 代码手写单写者/暂停恢复/三段取消用例后执行
# 预期输出（节选）：
...
> 各节断言已上移至 §三.1（Actor 单写者与命令处理）；本表为整篇迭代的回归验收，不重复材料。

| # | 验收项（断言） | 标准 | 复验方式 |
|---|---------------|------|---------|
| 1 | 并发命令不覆盖 | 有界队列反压，`runLoop` 串行处理无并发打架 | 复验：执行 §三.1 核对命令 |
| 2 | 暂停安全收尾 | 当前步跑完后才 PAUSED（不硬杀） | §三.1 步骤2 |
| 3 | 恢复续跑 | PAUSED→RUNNING，利用 Checkpoint 续跑 | §三.1 步骤3 |
| 4 | 取消三段语义 | CANCELLING→安全收尾→CANCELLED | §三.1 步骤4 |
| 5 | 高频命令状态一致 | 任意时刻读取一致，无状态回跳 | §三.1 步骤5 |

**回归失败排查**：按 §三.1 失败排查逐条回溯（多消费者/未 drain 当前步/未收尾即硬杀）。

## 七、验收对照

> 00-需求分析量化验收①（断点续跑 ≥95%）在本迭代获得生命周期前提（可暂停/恢复才能安全续跑），②幂等收口在 03。

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 每任务单写者 | 命令有界队列串行，状态无回跳 | ✅（§三.1 步骤1/5） |
| 生命周期状态机 | 八态迁移与 §二 图一一对应 | ✅（§二.1/§三.1 步骤2-4） |
| 暂停/恢复/取消 | 三命令均按安全收尾语义生效 | ✅（§三.1 步骤2-4） |
