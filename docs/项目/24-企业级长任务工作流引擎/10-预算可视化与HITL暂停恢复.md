...
> **定位**：长任务的**经济运营台 + 人工控制台**。核心：**① 预算实时可视化（每任务/每业务线 Token 成本看板）② 超限/异常的 HITL 暂停/恢复 ③ 预算调整与告警**。呼应 [04-预算闸门](04-三层预算与死循环防护.md)、[20-预算预测](../../项目/20-企业级Agent经济与结算平台/08-预算预测与成本治理.md)、[28-HITL](../../教程/04-企业级架构主干/08-Human-in-the-Loop与审批流.md)。前置阅读：[09-多机分布式续跑](09-多机分布式续跑.md)。
>
...
- **业务线聚合**：所有长任务的成本排行，谁是"烧钱大户"（呼应 [41-数据飞轮](../../教程/08-架构师进阶/07-数据飞轮与持续改进.md) 的成本治理视角）。
...
```bash
mvn test -Dtest=TaskBudgetViewTest        # 按 §二 手写看板聚合/实时递增/成本换算用例后执行
# 预期输出（节选）：
#   Tests run: 4, Failures: 0, Errors: 0, Skipped: 0
#   usedTokens/estCost 与 04 计量一致；用量随调用递增
#   BUILD SUCCESS
```
...
- HITL 裁决记录进审计（07 取消审计延续 / 呼应 [28-HITL 审批流](../../教程/04-企业级架构主干/08-Human-in-the-Loop与审批流.md)）。
...
```bash
mvn test -Dtest=HitlBudgetTest        # 按 §三 手写 DEGRADED→暂停/加预算恢复/取消审计用例后执行
# 预期输出（节选）：
#   Tests run: 5, Failures: 0, Errors: 0, Skipped: 0
#   PAUSED 停住烧钱；授权后从 Checkpoint 续跑；裁决落审计
#   BUILD SUCCESS
```
...
> 各节断言已上移至 §二.1（预算成本看板）与 §三.1（HITL 暂停/恢复状态机）；浪费告警判据在 §四.1 核对；本表为整篇迭代的回归验收，不重复材料。

| # | 验收项（断言） | 标准 | 复验方式 |
|---|---------------|------|---------|
| 1 | 看板实时准确 | `TaskBudgetView` 各字段实时可读、与 04 计量一致 | 复验：执行 §二.1 核对命令 |
| 2 | 成本排行 | 多任务聚合可定位"烧钱大户" | §二.1 步骤3 |
| 3 | DEGRADED 可暂停 | 人工暂停停住烧钱，状态可查 | 复验：执行 §三.1 核对命令 |
| 4 | 加预算恢复续跑 | 授权后从 Checkpoint 续跑，结果一致 | §三.1 步骤3/5 |
| 5 | 裁决审计 | HITL 操作落审计（who/what/when） | §三.1 步骤4 |
| 6 | 浪费告警 | 成本突增/死循环频发触发告警；调预算走审批 | §四（判据核对） |

**回归失败排查**：按 §二.1/§三.1 失败排查逐条回溯（打点延迟/换算错/PAUSED 分支缺拦截/裁决未落审计）。

## 七、验收对照

> 00-需求分析量化验收③（预算超限 100% 被拦截）在此获得"人工可裁决"的运营层补充：拦截后不失控，可暂停/加预算/取消。

| 验收项 | 标准 | 状态 |
|--------|------|------|
| 预算实时可视 | 单任务成本/进度 + 业务线成本排行 | ✅（§二.1 步骤1-3） |
| HITL 可介入 | DEGRADED→暂停/加预算恢复/取消均可用且落审计 | ✅（§三.1 步骤2-4） |
| 恢复结果一致 | 加预算续跑与未暂停路径结果一致 | ✅（§三.1 步骤5） |
| 浪费有告警 | 成本突增/死循环频发告警触发 | ✅（§四 判据/§四.1） |
