...
> **定位**：把 01 的"一口气 turn"升级为**任务化生命周期**：SessionTask 接口统一异构工作（对话/压缩/审阅/_shell）、三段式打断（协作取消→100ms 宽限→强杀）、单点收尾（统计/落盘/终态事件/idle 钩子零遗漏）、**持久化先于终态事件**不变式。读者画像：要回答"打断到底是什么语义"的读者。前置阅读：[01-最小Demo](01-最小Demo.md)、[教程 83-长任务持久化与中断恢复]。
>
> **铁律 0**：生命周期自研「概念代码」。
