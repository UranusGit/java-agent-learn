...
> **定位**：给引擎装上**工具编排层**：工具注册表（exposure/parallel 元数据）、失败回填而非抛异常、读写锁并行门、恰好一次通知、交互式命令会话续航。读者画像：要回答"工具执行怎么不崩会话、怎么并行"的读者。前置阅读：[02-Actor核与任务生命周期](02-Actor核与任务生命周期.md)、[教程 03-工具调用]、[教程 38-Agent性能优化]。
>
