...
> **定位**：把治理好的知识以**生产级检索服务**对外：混合检索（向量+关键词+结构化指标路由）、**重排**（精排 Top-N）、**预算内服务**（P99 ≤80ms、缓存、降级空上下文不阻塞调用方）、**查询理解**（意图分流：文档型/指标型/时效型问题走不同检索路径）。读者画像：要把检索做成被依赖服务的读者。前置阅读：[05-时效与失效引擎](05-时效与失效引擎.md)、[教程 78-高级RAG与AgenticRAG]。
>
