# 02-多源接入与 Connector 框架——文档/DB/API/事件统一入库
...
| # | 核对项 | 判据 |
|---|--------|------|
| 1 | 四问齐全 | 新增需求/影响模块/架构演进/上一版痛点四行均有（承接 01 §五 单文档痛点） |
| 2 | 验收可度量 | 四条验收均有量化判据（≤1 天/只入差异/≤5s/零明文） |
...
### 二.1 本节测试与验证（Connector SPI、增量与 CDC）
...
- [ ] 三类源（DB/API/事件）的入库形态（schema 语义/工具知识/窗口事实）能区分，且都"不搬原始数据"
- [ ] 能说明为何 DB 存"列描述+样本值"而非原始行（不给检索制造噪音、保留 schema 级知识）
...
| # | 操作 | 预期（PASS 判据） |
|---|------|------------------|
| 1 | 连续两次对同一源全量同步 | 第二次仅同步差异（指纹判定），无全量重入 |
| 2 | 四类源各接一个走通 ingest | 均为增量同步且入检索可命中，凭证全走密管引用 |

**失败排查**：回归期全量重入→指纹字段不稳定（回查 §二.1 排查项②）；凭证明文→日志脱敏未覆盖新源。

## 五、本迭代痛点

多源进来后口径打架（同指标两个数）→ 03 语义层。

> 五.1 本节核对（一句话）：痛点（多源口径打架）与 [03] 语义层（指标唯一）一一对应，痛点不被搁置即 PASS。
...
> 六.1 本节核对（一句话）：四条验收（SPI/增量/CDC/密管）分别对应 §一.1（验收口径）、§二.1 步骤 2-4，与正文状态一致即 PASS。

**下一篇**：[03-语义层与指标口径](03-语义层与指标口径.md)。
