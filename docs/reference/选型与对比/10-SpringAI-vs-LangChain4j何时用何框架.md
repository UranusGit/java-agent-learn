...
>
> 本文综合"两框架分工模型（理论范式）"与"企业级选型真相（现实校正）"两套视角，给出**最终决策版**：用一份文档回答"我到底该用哪个"。
>
...
| 视角 | 定位 | 适用读者 |
|------|------|---------|
| **理论范式（分工模型）** | 两框架"接入/兜底 vs 思考/编排"分工 | 想理解两框架**设计哲学差异**的读者 |
| **现实校正（企业选型真相）** | 企业不混用，单框架 + Workflow 是主流 | 想知道**企业真实在做什么**的读者 |
| **本文（决策手册）** | 基于 2026-07 最新事实给出"用哪个"的明确答案 | 准备**选型/启动项目**的读者 |

**重要事实更新**："分工模型"视角写于 Spring AI 1.0 时代，彼时 LangChain4j 在"思考/编排"上有明显优势；Spring AI 2.0 GA（2026-06-12）后这个优势大幅缩水（详见 §2）。本文是 2026-07 的最新答案。
...
"分工模型"视角写于 Spring AI 1.0 时代，论点是"LangChain4j 在思考/编排上有优势"。**Spring AI 2.0 GA（2026-06-12）改变了一切**：
...
## 9. 修订记录：对"分工模型"和"企业选型真相"的更正

### 9.1 "分工模型"视角需要修正的论点

> "分工模型"作为"理解两框架设计哲学差异"的教材仍有价值，但以下论点已过时：

| "分工模型"论点 | 2026-07 真相 | 修正 |
|------------|-----------|------|
...
| "两层分工架构（Spring AI 接入 + LangChain4j 思考）" | 企业不这么做（见 5.2 节的论据） | 标注为"理论范式，实战罕见" |
| 第 9 节演进路径"阶段 3 引入 LangChain4j" | 应改为"阶段 3 引入 Alibaba Graph 或 LangGraph4j"（更贴近企业实战） | 见 §8.5 |

### 9.2 "企业选型真相"视角仍需补充的点

> "企业选型真相"整体结论正确（企业不混用），但 2026-06-12 Spring AI 2.0 GA 后需要补充：

| 补充点 | 内容 |
|-------|------|
| Spring AI 版本 | 标注 2.0 GA（2026-06-12）已发布，"企业选型真相"基于 1.0 GA 写就 |
...
```mermaid
flowchart TD
    T10["分工模型（理论范式）"] -->|"现实校正"| T11["企业选型真相（企业实战）"]
    T11 -->|"版本更新 + 决策落地"| T13["本文（决策手册）"]
```
...
本文是"到底用哪个框架"的最终答案：**Spring Boot 项目选 Spring AI 2.0，Quarkus / 纯 Java 项目选 LangChain4j，不混用**。按本文第 3 节决策树对号入座即可。
