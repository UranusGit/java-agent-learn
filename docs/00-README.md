...
教程中的 P1-P6 是阶段性的综合练习。除此之外，还有**二十个贯穿全部知识点的大型企业级项目**，按内容由浅入深排序：
...
├── 阶段4-生产化/              ← 可靠性/成本/可观测/安全/合规/灾备/红队/供应链/SLO/Prompt版本管理/灰度发布/配额调度/长文本/Embedding/GraphRAG/编排DSL/HITL/边缘部署/异常检测（52篇）
├── 阶段5-架构师/              ← 多Agent / 平台化 / 生命周期 / 速率限制 / 熔断 / 调试 / 隐私 / 事件驱动 / 服务网格 / 数据一致性 / 灰度实验 / 混沌工程 / 元数据治理 / 国际化（19篇）
├── 阶段6-前沿/                ← 持续进阶：多模态/蒸馏/联邦学习/推理优化/安全攻防/商业ROI/自进化/数字孪生/AGI路径/量子安全（16篇，深化版）
│
├── 项目实践/                  ← 二十个企业级大型项目（由简入繁）
...
│
├── 理论字典/                  ← 概念速查（18篇，按需查阅不按顺序）
├── 附录/                      ← 底层背景速成（8篇：Reactor/SSE/Redis/Kafka/向量库/可观测/Docker/GitOps）
└── 调研/                      ← 行业调研报告
```
