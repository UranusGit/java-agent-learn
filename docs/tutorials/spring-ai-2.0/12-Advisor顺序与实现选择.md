# Advisor 顺序与实现选择（Spring AI 2.0）

> 本文回答两个问题：
> 1. Advisor 的 `order` 到底怎么设？默认值是多少？谁在外、谁在内？
> 2. `BaseAdvisor` vs `implements CallAdvisor, StreamAdvisor` vs 只实现一个 —— 三种写法各自适用什么场景？
>
> **基于 Spring AI 2.0.0 GA 反编译事实**，所有常量值和默认 order 都来自真实 class 文件，不是猜的。
>
> 调研日期：2026-07-17
...
`order` 是 `org.springframework.core.Ordered` 接口的返回值，**数值越小越靠外**（越先执行 before、越后执行 after）。

```
order = -2147483448  ← 最外（最先 before、最后 after）
order = 0            ← 中间
order = 100          ← 较内
order = 2147483647   ← 最内（最后 before、最先 after）
```
...
> **不要全用 0**：项目里很多 advisor 都用默认 0 会导致顺序不可控（同 order 时按注册顺序，但不保证跨版本一致）。**永远显式给自定义 Advisor 设 order**。

---

## 2. before/after 执行顺序的可视化
...
- `after` 里想基于"完整响应"判断（比如检测 tool call）：**必须自己聚合**，单 chunk 看不到完整内容
...
## 4. order 实战案例：流式 + 工具 + Memory 的正确排序

这是 [`./11-复现手册-流式与工具调用.md` §15](./11-复现手册-流式与工具调用.md) 实证出来的实战配置。

### 4.1 错误的默认组合

```java
.defaultAdvisors(
    toolCallingAdvisor,                              // order = -2147483348（默认最外）
    MessageChatMemoryAdvisor.builder(memory).build() // order = -2147483448（默认）
)
// → 实际顺序：Memory(-2147483448) 比 Tool(-2147483348) 小 100，Memory 在外
```

**看起来对，但**：自定义 ChatConfig 时如果同时改了 Tool 的 order 到 `Integer.MIN_VALUE`（最小），就会反过来 —— Tool 在外、Memory 在内，触发 `conversationId cannot be null` 500 错误。
...
.defaultAdvisors(
    ...
    new SimpleLogAdvisor()                                       // 自己定 order
)
```
...
// BaseAdvisor —— 同时继承 Call+Stream，并把流程抽象成 before/after
public interface BaseAdvisor extends CallAdvisor, StreamAdvisor {
    ChatClientRequest      before(ChatClientRequest req, AdvisorChain chain);  // 你实现
    ChatClientResponse     after(ChatClientResponse resp, AdvisorChain chain); // 你实现
    Scheduler              getScheduler();                                     // 默认提供
    // adviseCall / adviseStream 已有默认实现，自动调 before → next → after
}
```
...
### 5.5 同时 implements CallAdvisor + StreamAdvisor 的用法
...
### 5.6 为什么 `SimpleLogAdvisor` 项目里写的是 `implements CallAdvisor, StreamAdvisor`

项目里的 `org.demo02.advisor.SimpleLogAdvisor` 走的是第二种方式（不是 BaseAdvisor）。原因是它要在流式下做 `filter` 过滤空 chunk 和 `doOnNext` 逐 chunk 打印 —— 这是 Reactor 流式特有的操作，BaseAdvisor 的 `after` 模板做不了这种"逐 chunk 操作"。
...
**原因 1**：你 override 的是 `Advisor.getOrder()`，但你的 advisor 实例化时构造器里硬编码了 order，构造器优先级高。检查 Builder 是否有 `order(int)` 方法被调过。

**原因 2**：你把 advisor 加进了 `defaultAdvisors(...)`，但调用时又用 `.advisors(spec -> ...)` 加了同名 advisor，导致重复，按注册顺序执行而非 order。

### 6.2 "before 改了 prompt，但 LLM 没看到"
...
**原因**：你只实现了 `CallAdvisor`，没实现 `StreamAdvisor`。流式下框架根本不认识你的 advisor。

**修复**：改 `implements BaseAdvisor`，自动覆盖两种模式。

### 6.4 "order 全用 0，偶尔行为飘忽"
...
- [ ] 能说出 `MessageChatMemoryAdvisor` 和 `ToolCallingAdvisor` 的默认 order 数值和差值含义
- [ ] 能解释"order 越小越靠外"在 before/after 两个阶段分别意味着什么
- [ ] 知道自定义 Advisor 推荐插入的 order 插槽（200/300/400 之间）
- [ ] 能在 30 秒内判断"该用 BaseAdvisor 还是 implements CallAdvisor + StreamAdvisor"
- [ ] 知道流式下 `after` 是逐 chunk 触发，要观察完整响应必须用 `ChatClientMessageAggregator`
- [ ] 能解释为什么项目里的 `SimpleLogAdvisor` 不用 BaseAdvisor（因为流式需要 Reactor 操作符）

---

## 8. 相关文档

- [`./03-中级-Advisor2.0与结构化校验.md`](./03-中级-Advisor2.0与结构化校验.md) —— Advisor 链基础
- [`./11-复现手册-流式与工具调用.md` §15](./11-复现手册-流式与工具调用.md) —— 实战 order 调整案例
- [`./02-初级-ToolCallingAdvisor.md`](./02-初级-ToolCallingAdvisor.md) —— ToolCallingAdvisor 深入
