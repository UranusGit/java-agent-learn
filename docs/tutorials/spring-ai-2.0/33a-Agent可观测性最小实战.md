...
企业级用三层封装解决：**通用上下文值类 + 集中注册中心 + 作用域工具**，让业务代码完全不碰 ThreadLocal。
...
**(d) 作用域工具 `obs/AppContextScope.java`**

把"set + 业务执行 + finally clear"封装成一个方法，业务代码只调它，不碰 ThreadLocal 细节。**finally 保证 clear**，杜绝线程池复用导致的串会话：
```java
package com.example.demo07.obs;
...
/**
 * 上下文作用域工具：把 set + clear 封起来，业务代码不直接操作 ThreadLocal。
 * try/finally 保证用完必 clear（防线程池复用串会话）。
 *
 * 用法：AppContextScope.runWith(AppContextKeys.SESSION_ID, sid, () -> { 业务流 });
 */
public final class AppContextScope {

    /** 在指定上下文值的作用域内执行一个 Supplier，自动 set/clear。 */
    public static <T, R> Supplier<R> withValue(PropagatedContextValue<T> key, T value, Supplier<R> body) {
        return () -> {
            key.set(value);
            try {
                return body.get();
    ...
    /** 在指定上下文值的作用域内执行一个 Callable（抛受检异常时用）。 */
    public static <T> Callable<T> withValue(PropagatedContextValue<?> key, Object value, Callable<T> body) {
        return () -> {
            @SuppressWarnings("unchecked")
            PropagatedContextValue<Object> k = (PropagatedContextValue<Object>) key;
            k.set(value);
            try {
                return body.call();
            } finally {
    ...
    private AppContextScope() {}
}
```

**(e) 业务代码：完全无感**

封装后，业务代码不再碰 ThreadLocal，全部用强类型 key + 作用域工具：
    ...
    public String chat(@RequestParam String prompt,
        ...
        agent.run(prompt)
                .contextWrite(reactor.util.context.Context.of(
                        AppContextKeys.SESSION_ID.key(), sessionId))   // ← 强类型 key
                .subscribe();
        return sessionId;
    }
```

`ObservableAgent.run()`——sessionId 改走 Reactor Context，用 `AppContextScope.withValue` 自动 set/clear：
```java
    public Flux<AgentEvent> run(String userInput) {
        return Flux.deferContextual(ctx -> {
            String sessionId = ctx.getOrDefault(AppContextKeys.SESSION_ID.key(), "unknown");

            // 用 AppContextScope 包住业务流：自动 set + finally clear，不直接碰 ThreadLocal
            Supplier<Flux<AgentEvent>> scoped = AppContextScope.withValue(
                    AppContextKeys.SESSION_ID, sessionId,
                    () -> buildEventStream(userInput, sessionId));   // 把 Step 6 的三段式抽成这个方法
...
`ToolObservationHandler.onStop`——从强类型 key 读：
```java
...
└── src/main/java/com/example/demo07/
    ...
    └── obs/
        ...
        ├── AppContextScope.java           ← Step 7（作用域工具，自动 set/clear）
...
public class ObservableAgent {
    ...
    public Flux<AgentEvent> run(String userInput) {
        return Flux.deferContextual(ctx -> {
            ...
            // Step 7 企业级封装：用 AppContextScope 自动 set/clear，业务代码不碰 ThreadLocal
            Supplier<Flux<AgentEvent>> scoped = AppContextScope.withValue(
                    AppContextKeys.SESSION_ID, sessionId,
                    () -> buildEventStream(userInput, sessionId));
            return scoped.get();
        });
    }

    /** 三段式事件流：opening → deltas → closing（业务逻辑，不含上下文管理）。 */
    private Flux<AgentEvent> buildEventStream(String userInput, String sessionId) {
