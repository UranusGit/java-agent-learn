...
> curl 是手动两终端，时序靠你自己控制（看到 READY 再开终端 2）。**想要更直观的验证（自动 READY 握手 + 工具调用面板 + 流式正文）**，用前端页面——见下方「用页面验证」。

**用页面验证**（推荐，比 curl 直观）：把 [附录 C 的页面](#附录-c调试用-html-页面step-8-后) 放到 `src/main/resources/static/demo07.html`，启动后浏览器打开 `http://localhost:8080/demo07.html`，输入 prompt 点发送。页面自动：先订阅 /sse、收到 READY 才触发 /chat（消除竞态）、实时显示工具调用（工具名/参数/返回值）、正文逐字流式。**这个页面从 Step 6 起一直能用，Step 7（会话隔离）、Step 8（多轮记忆）都用它**——后面不重复贴。
...
**用页面验证多轮记忆**（最直观）：用 [附录 C 页面](#附录-c调试用-html-页面step-8-后)，**不要点「新对话」**——在同一会话里连发两轮（"现在几点了，规划今天下午" → 等回答 → 再发"那明天同一时间呢"）。第二轮能接住第一轮的上下文。然后点「新对话」换会话再问"那明天同一时间呢"——这次 LLM 不知道你在说什么（记忆隔离）。页面用 sessionId 兼任记忆会话 ID，连问就是多轮、点新对话就是清记忆。
