...
实现思路见下一节 **03-多 Tool 编排**：用一个 Router Agent 先判断用户意图，再把请求路由到带不同 Tool 组的子 Agent（HR / IT / 业务），从而每次只向 LLM 暴露相关的一小组工具。
...
```mermaid
flowchart LR
    V1["第一遍：凭直觉写一版描述"] --> V2["第二遍：跑 20 条测试集"]
    V2 --> D{"准确率达标?"}
    D -->|"否，看失败案例"| F["分析失败案例"]
    F --> F1["调错 Tool → 描述不够差异化"]
    F --> F2["参数填错 → 参数描述不清晰"]
    F --> F3["该调不调 → 没说清何时使用"]
    F1 --> V1
    F2 --> V1
...
设计完成后，进入下一节：**02-防止 Agent 失控**——给 Agent 装上迭代上限、超时、循环检测、成本控制这些"安全带"，让它在生产中稳定运行。
