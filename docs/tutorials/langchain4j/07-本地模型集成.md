...
> 前提：你已会用 DeepSeek 远端跑 Chat、会配 `OpenAiEmbeddingModel`（即 05、06 的能力）。
>
> 注：本教程系列不依赖 Ollama，所有"本地模型"需求由 LM Studio 满足。
...
```mermaid
flowchart LR
    App["LangChain4j 应用"] --> Dep["langchain4j-open-ai"]
    Dep --> LMS["LM Studio Server<br/>http://127.0.0.1:1234"]
    LMS --> EP1["/v1/chat/completions<br/>对话模型 qwen2.5-7b-instruct"]
    LMS --> EP2["/v1/embeddings<br/>向量模型 text-embedding-bge-large-zh-v1.5"]
    EP1 --> R1["Chat 角色<br/>（本教程默认走 DeepSeek，本地可选）"]
    EP2 --> R2["Embedding 角色<br/>（本教程默认走本地）"]
```
...
```mermaid
flowchart TD
    Q1{"选择对话模型<br/>按内存"} -->|"极速测试 1GB"| C1["qwen2.5-0.5b-instruct"]
    Q1 -->|"本地开发主力 5GB 推荐"| C2["qwen2.5-7b-instruct"]
    Q1 -->|"质量更高 9GB"| C3["qwen2.5-14b-instruct"]
    Q1 -->|"推理类任务 5GB"| C4["deepseek-r1-distill-qwen-7b"]
    Q2{"选择 Embedding 模型"} -->|"中文默认推荐 1024"| E1["text-embedding-bge-large-zh-v1.5"]
    Q2 -->|"中文轻量 512"| E2["text-embedding-bge-small-zh-v1.5"]
    Q2 -->|"英文/通用 768"| E3["text-embedding-nomic-embed-text-v1.5"]
    Q2 -->|"多语言 1024"| E4["bge-m3"]
```
...
> ⚠️ **HTTP/1.1 配置必须加**：不加会偶发 `HttpTimeoutException`（JDK 21 的 HttpClient 默认走 HTTP/2，与 LM Studio 长响应不兼容）。若真遇到该异常，排查方法在本文档 9.3 节。
...
timeline
    ...
    阶段A 跑通 : 安装 LM Studio
             ...
             : LangChain4j 调用本地 Chat
    阶段B 双角色 : 同时加载 Chat + Embedding 模型
             : 验证 Embedding 维度
             : 最小 RAG demo（本地双模型）
    阶段C 性能 : 测试 Context Size 对内存/速度影响
             : 测 GPU Offload 不同层级速度
             : 监控 GPU 利用率
```
...
完成后进入下一节：**08-DeepSeek 集成**——把云端主力模型用起来，学成本控制与 JSON Mode。
