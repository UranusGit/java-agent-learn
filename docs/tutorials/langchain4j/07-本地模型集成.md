...
```mermaid
flowchart TD
    A["安装 LM Studio<br/>lmstudio.ai"] --> B["Search 标签搜索并下载模型<br/>qwen2.5-7b-instruct / bge-large-zh-v1.5"]
    B --> C["Developer 标签<br/>下拉框加载模型（可同时加载多个）"]
    C --> D["Start Server 启动服务<br/>默认端口 1234"]
    D --> E["验证: curl http://127.0.0.1:1234/v1/models"]
    E --> F["测试 Chat / Embedding 调用"]
```
