...
>
> **文档规模**：约 628 篇文档（教程 58 + 附录 85 + 项目 467 + 前沿 17）。2026-08-17 全部 15 个项目完成进阶深化。**2026-08-21 第十一批"进阶补充 + 深化 10 大方向"全部完成**：新增 9 大旗舰项目（23-31，含意图路由/长任务/记忆/飞轮/协作/可观测/边缘/解释/上下文）、8 教程（50-57）、3 附录（20/21/22）、5 前沿（12-16），并修复全体系迭代码/进阶N- 前缀正文链接、命名铁律固化入 CLAUDE.md。
...
│   ├── 53-企业级数据飞轮闭环.md            # 第十一批新增：四环齿轮/血缘/免疫/联邦
│   ├── 54-Agent间协作协议工程化.md        # 第十一批新增：A2A 能力发现/委托/协作安全
│   ├── 55-边缘Agent与端云协同部署.md      # 第十一批新增：端云五形态/断网自治/TCO
│   ├── 56-Agent可解释与透明工程.md        # 第十一批新增：证据归因/置信度/监管披露
│   └── 57-上下文工程生产化.md              # 第十一批新增：五层管线/配额/缓存层序
│
...
│   ├── 27-企业级Agent协作平台/            # 协作互操作视角（14 篇：AgentCard注册心跳/两级发现/协商两阶段委托/进度取消/身份互信不放大/注入边界/降级补偿/可观测/联邦/市场）第十一批第四旗舰
│   ├── 30-企业级Agent可解释与透明平台/    # 可解释视角（14 篇：时间线分层/证据归因校验/分来源置信校准/解释API快照/反事实/监管披露/质量飞轮/置信×风险HITL）第十一批第五旗舰
│   ├── 26-企业级Agent数据飞轮平台/        # 飞轮收束视角（14 篇：多源采集免疫/五层血缘/评估门禁/归因定向修复/灰度守门/联邦共享/健康度/主动加速）第十一批第六旗舰
│   ├── 28-企业级Agent统一可观测平台/      # 可观测合一视角（14 篇：OTel管道/三视图玻璃板/成本归因/评测挂链/告警根因/SLO错误预算/自动回滚/多租户/容量）第十一批第七旗舰
│   ├── 31-企业级Agent上下文管理平台/      # 上下文中枢视角（14 篇：五层渐进瘦身/压缩三路径/分层配额模板/缓存友好层序/装配留痕漂移/多租户隔离/端云分工）第十一批第八旗舰
│   └── 29-企业级边缘Agent部署平台/        # 边缘部署视角（14 篇：节点纳管/模型分发灰度回滚/断网分层降级/端云分工/边缘安全/远程运维自愈/局域联邦/边缘AB/TCO）第十一批第九旗舰
│
...
│   ├── 21-可观测平台实践/                 # 第十一批新增：OTel 管道/gen_ai 语义
│   └── 22-向量数据库与检索工程/           # 第十一批新增：HNSW/IVF/量化、延迟-召回曲线
│
└── 前沿/                                ← 17 篇（00-16），最后扩展视野
    ├── 11-八旗舰组合蓝图.md            # 体系收官导读：八旗舰组合落地路线
    ├── 00-A2A协议.md
    ├── 12-推理时计算与长程推理.md       # 第十一批新增：推理算力与长程 RL
    ├── 13-Agent代码生成与SWE基准深挖.md # 第十一批新增：SWE-bench 深挖
    ├── 14-Chain-of-Agents与协作协议实证.md # 第十一批新增：协作模式实证
    ├── 15-上下文压缩前沿.md            # 第十一批新增：KV缓存/前缀缓存/无限上下文
    └── 16-多模态人工评测局限与自动化.md # 第十一批新增：多模态评测分层
```
...
| 54 | [Agent 间协作协议工程化](教程/54-Agent间协作协议工程化.md) | AgentCard 能力发现、任务委托协商、身份互信、委托不放大、注入边界、失败补偿 |
| 53 | [企业级数据飞轮闭环](教程/53-企业级数据飞轮闭环.md) | 四环齿轮组、血缘元数据、防污染免疫、联邦飞轮、健康度 |
| 55 | [边缘 Agent 与端云协同部署](教程/55-边缘Agent与端云协同部署.md) | 端云五形态、边缘推理栈、断网自治降级、模型分发灰度、TCO 经济账 |
| 56 | [Agent 可解释与透明工程](教程/56-Agent可解释与透明工程.md) | 可解释三维度、证据归因与校验、分来源置信度、反事实、监管披露 |
| 57 | [上下文工程生产化](教程/57-上下文工程生产化.md) | 五层拼接管线、优先级裁剪、压缩三路径、分层配额、缓存友好层序 |
...
| [21-可观测平台实践](附录/21-可观测平台实践/00-OTel管道与gen_ai语义.md) | OTel Collector 管道、gen_ai 语义约定、尾部采样 | 1 |
| [22-向量数据库与检索工程](附录/22-向量数据库与检索工程/00-索引与检索工程深度.md) | HNSW/IVF/量化索引参数、延迟-召回曲线、分片、混合重排 | 1 |
