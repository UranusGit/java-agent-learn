...
> **承接上一阶段。** 阶段 3 把单体拆成了 5 个可独立部署的服务，立起了全链路 trace、熔断/限流、多模型路由——但那是"**一个客户、一套系统**"：`tenant_id` 只是透传字段，没有隔离、没有计费、没有审计，出了事故没有 SLO 说话。本阶段把它升级成"**能同时服务多家企业客户**"的多租户平台：**租户隔离（数据库行级 RLS + 知识库租户过滤 + 权限/配额）、按 token/租户计费、审计合规（对齐 EU AI Act Article 12 的高风险系统日志留痕）、Prompt Cache + 模型路由降成本、SLO 定义与监控**，预计 **6~8 周**。
>
...
> 把阶段 3 的 5 个服务全面"租户化"：`tenant_id` 从"透传字段"变成**贯穿一切数据访问的硬约束**（应用层强制过滤 + PostgreSQL RLS 兜底 + pgvector 元数据过滤）；立起**配额与计费**（每租户每天/每月 token 上限、实时算钱、月度账单）；用 **Prompt Cache + 复杂度路由 + 独立 LLM Gateway** 把成本压下去；所有敏感操作写**对齐 EU AI Act Article 12（record-keeping）的高风险系统日志留痕审计日志**；最后用 **SLO（可用性/延迟/准确率/成本）** 给平台"好坏定数"——且阶段 3 的评估集通过率**不回退**。
...
- **审计合规是"能扛生产"的底线**：对金融/医疗/法律类企业客户，欧盟 EU AI Act **Article 12（record-keeping）** 等监管要求高风险 AI 系统"自动记录可追溯事件（日志留痕）"。平台要把**审计日志**当一等公民做进去，不是上线前补（教程/18 §9、教程/31 §1.2/§1.3）。
...
| 5 | **审计合规** | 全量敏感操作落 `audit_log`，字段对齐 EU AI Act Article 12 的高风险系统日志留痕要求，可导出、保留期可配置 | 教程/18 §9、教程/31 §1.3 |
...
- **不做完整 CICD / K8s / 生产级发布**（阶段 5 收口为"生产交付最小集"）——本阶段沿用 Docker Compose 部署，重点在"平台能力"，不在"交付流水线"。
...
2. **租户上下文全局传播是第一地基，且身份必须由 API Key 派生**：用 `TenantContext`（ThreadLocal）+ 过滤器注入，服务内所有数据访问都必须带 `tenant_id`（教程/18 §3.2）。**防伪造铁律：租户身份一律由 API Key 解析派生，`X-Tenant-Id` 仅作一致性校验**——客户端不能自己指定租户身份；请求头 `X-Tenant-Id` 与 API Key 绑定的租户不一致即返回 401（教程/18 §3.2、§17.1）。**响应式/异步链路要显式传播**：Reactor 的线程切换、线程池任务、@Async 都会丢 ThreadLocal，必须把租户上下文显式传下去（这是多租户最常见的事故源，教程/18 §17.1）。阶段 3 的 `ToolContext → McpMeta` 透传继续用于工具服务。
...
7. **计费以"模型 API 返回的 usage"为准，实时算钱落 `billing_record`，且全口径收口**：成本 = token × 单价（`pricing` 表），**不要用自己估算的 token 数**，以模型返回的 usage 为准，才能做到"token 计费准确率 100%"（教程/18 §5.2、教程/15 §6.2、理论/15 §3.1）。**计费/配额必须覆盖所有模型调用**：不只对话 LLM——知识服务的 **embedding** 调用、评估服务的 **Judge** 模型调用同样要过 Gateway 的配额与计费（或至少统一落 `billing_record`），否则"每租户成本"会漏掉一大块、月度账单对不上模型账单。每月按租户聚合出账单。
...
9. **审计用切面统一落库，对齐 EU AI Act Article 12 的日志留痕要求**：用 `@Audited` 注解 + AOP 切面，把"谁、何时、对哪个租户、做了什么、输入输出、模型/系统版本、触发原因"结构化落 `audit_log`（教程/18 §9）。EU AI Act **Article 12（record-keeping）** 要求高风险 AI 系统自动记录日志（日志留痕）——本阶段把留痕字段做齐、保留期做成可配置（**5-10 年是行业最佳实践口径（参考 教程/31 §1.3），不是 EU AI Act 的强制条款**；实际保留期按企业合规要求配置）。**只记结构化字段，不落敏感明文**（Prompt 哈希化，教程/15 §11.2）。
...
flowchart TD
    subgraph cp["控制面（平台后台）"]
        ...
        AUDIT["审计服务<br/>audit_log 日志留痕 (Article 12)"]
    end
    subgraph edge["接入层"]
...
| **审计服务** | 合规底线 | `@Audited` 切面统一落 `audit_log`，对齐 EU AI Act Article 12 的留痕字段 + 保留期配置 + 导出（教程/18 §9、教程/31 §1.3） |
...
public class TenantFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse resp,
        ...
        // 身份以 API Key 为准：由它解析出租户（防"自己指定身份"的伪造）
        ...
        // X-Tenant-Id 仅作一致性校验：与 API Key 绑定租户不一致即 401
        String declared = req.getHeader("X-Tenant-Id");
        if (declared != null && !declared.equals(tenantId)) {
            resp.sendError(401, "X-Tenant-Id mismatch"); return;
        }
...
- [ ] 用 A 租户的 API Key 伪造 B 租户的 `X-Tenant-Id` 请求 → 返回 401（**身份以 API Key 派生为准，X-Tenant-Id 仅一致性校验，不一致即 401**）。
...
-- 第 2 个参数 true：未设置 app.current_tenant 时返回 NULL 而不是报错
-- NULL 与任何 tenant_id 都不相等 → 未设租户时 RLS 直接过滤掉所有行，宁可查空也不越权
-- 每个租户敏感表一张，kb_chunk / billing_record 同理
```

2. 数据源/连接层在**每个事务内**用 `SET LOCAL app.current_tenant = '<tenant>'`（用连接器/拦截器统一做，别散落在代码里）。**`SET LOCAL` 只在当前事务生效、事务结束自动清空**，天然规避连接池复用污染的连接——**不要用 `SET`（会跨事务残留到连接池里），必须在事务边界内使用 `SET LOCAL`**。多租户场景严禁在事务外裸 `SET` 后长连接复用。
3. 知识库多租户：灌文档时把 `tenant_id` 写进 chunk metadata，检索时 `filterExpression("tenant_id == '<tenant>'")`（教程/18 §3.4）。
4. **RLS 与 HNSW 向量索引的交互**：pgvector 的 HNSW 索引在 RLS 过滤**之前**做近似检索，索引层不认识 `tenant_id`。所以多租户向量检索**以应用层 metadata 过滤为主、RLS 兜底**——检索路径先用 `filterExpression` 把候选集缩到本租户，再对结果做 RLS 复核；**不要只靠 RLS 兜底过滤向量结果**（会导致跨租户召回后再被过滤，既浪费算力，近似检索的语义也可能失真）。
4. 给阶段 3 的**表归属矩阵**升级为"租户隔离矩阵"：每张表标注"是否开 RLS、租户过滤字段"。

**验证什么：**
- [ ] 漏写 `WHERE tenant_id=` 的查询返回**空集**（RLS 兜底生效）——这是隔离的核心验证。
- [ ] 未设置 `app.current_tenant` 时查询返回空集（`current_setting(..., true)` 兜底 NULL，不报错、不越权）；事务结束后连接归还连接池不残留租户（`SET LOCAL` 生效）。
...
2. **字段对齐 EU AI Act Article 12（record-keeping）的高风险系统日志留痕要求**（自动记录可追溯事件）：事件时间戳、系统/模型版本、租户与用户标识、事件类型（登录/查询/工具调用/知识库修改/配置变更）、触发原因、输入输出（敏感内容只存哈希）、操作结果（成功/失败 + 错误码）：

```java
// 关键片段（完整见 教程/18 §9，字段按 EU AI Act Article 12 留痕要求扩展）
...
3. 保留期做成可配置（默认按企业合规口径设置；**5-10 年是行业最佳实践口径而非 EU AI Act 强制条款，参考 教程/31 §1.3**）；提供导出接口（按租户 × 时间段，CSV）。
...
3. 全量回归：3 租户隔离用例 + 配额/计费用例 + 审计用例 + 阶段 3 评估集（通过率 ≥ 阶段 3 基线 0.8，不回退；多租户化不降低回答质量，目标向 0.85 提升）。
...
> 全部打勾才算本阶段完成。数字口径：3 个测试租户；阶段 3 评估集通过率 **≥ 阶段 3 基线（0.8）不回退，且目标提升到 0.85**（是"提高"不是"仅不回退"）。
...
- [ ] **计费全口径收口**：知识服务 embedding 与评估服务 Judge 的模型调用同样走 Gateway 配额 + 计费（或统一落 `billing_record`），月度账单不漏 embedding / Judge 成本。
...
- [ ] **有审计合规**：敏感操作全量落 `audit_log`，字段含对齐 EU AI Act Article 12 的留痕要求（时间戳/租户/用户/模型版本/输入输出哈希/触发原因），可导出、保留期可配置、不落敏感明文。
...
- [ ] **阶段 3 回归不回退**：评估集通过率 ≥ 阶段 3 基线（0.8）不回退，多租户化没有让回答质量下降（目标向 0.85 提升）；Docker Compose 一键起整套平台。
...
- 阶段 5 要做：**行业 Agent 模板**（金融/医疗/法律各自的合规约束、拒答策略、评估集，教程/29/30/31）、**生态化**（MCP Hub 对外开放、第三方工具接入）、把 Docker Compose 升级为**生产交付最小集**（CI 流水线 + 一键部署 + 回滚，教程/27）。阶段 4 的隔离级别可对高合规客户升级独立 schema。
- 阶段 4 的 `audit_log`（对齐 EU AI Act Article 12 的高风险系统日志留痕）和 SLO 底座，就是阶段 5 行业合规的直接地基。
