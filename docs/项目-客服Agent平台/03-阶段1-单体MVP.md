...
| 资料 | 必读/选读 | 读哪几节、读来干嘛 |
|---|---|---|
| **教程/01-2.0基础重塑** | 必读 | §2.1~2.3（版本与依赖：**流式必须 webflux**）、§3（ChatClient API）、§5（Advisor 链与内置优先级表，决定"记忆→检索→工具"的顺序）、§6（最小可运行 demo，照抄工程骨架） |
...
flowchart TD
    ...
    subgraph core["编排层 (Spring AI 2.0)"]
        CC["ChatClient + Advisor 链<br/>记忆 → RAG → 工具循环 → 计费"]
    end
...
| `kb_chunk` | 知识块 | `doc_source`（来源文档，引用溯源用）、`embedding`（向量）、`metadata`（切块元数据） |
...
| **国产大模型 API** | 生成 | DeepSeek / 通义任一，配置进 `spring.ai.model.chat.*` |
| **pgvector** | 存储 | 一张 `kb_chunk` 表做向量检索（`<=>` 余弦距离）；业务表同库 |
...
3. 写一个最薄的流式接口：`POST /api/chat`，入参 `{ sessionId, message }`，返回 SSE 流。
...
4. 启动后用 curl 验证流式（`-N` 是关键，否则 curl 会缓冲一整包再打印）。
...
public class JdbcChatMemoryStore implements ChatMemoryStore {
    ...
    public void addMessage(String conversationId, Message message) {
        jdbc.update("INSERT INTO message(conversation_id, role, content) VALUES (?,?,?)",
                conversationId, message.getRole().name(), message.getText());
    }
    ...
    public List<Message> getMessagesByConversationId(String conversationId) {
        return jdbc.query(
            ...
            (rs, i) -> {
                ...
                // 按 role 转回 UserMessage / AssistantMessage
...
```java
ChatMemory chatMemory = new MessageWindowChatMemory(jdbcChatMemoryStore, 10); // 最近 10 条
...
2. 起 pgvector（docker，教程/09 第2章），建 `kb_chunk` 表（`CREATE EXTENSION IF NOT EXISTS vector;`）。
...
- [ ] 手写一条 SQL（`SELECT chunk_text, 1-(embedding<=>:q) AS sim FROM kb_chunk ORDER BY sim DESC LIMIT 3`）能看到"这次回答引用了哪几块"——调错时定位用（教程/09 第1章"检索命中"心智）。
...
2. 用 `MethodToolCallbackProvider` 注册进 Advisor 链，并注意 **Advisor 顺序**（记忆 → RAG → 工具，见教程/01 §5.3 优先级表）：

```java
ToolCallbackProvider tools = MethodToolCallbackProvider.builder()
        .toolObjects(new SupportTools())
        .build();

ChatClient chatClient = ChatClient.builder(chatModel)
        .defaultAdvisors(
            new MessageChatMemoryAdvisor(chatMemory),
            QuestionAnswerAdvisor.builder(vectorStore).topK(4).build(),
...
```java
// 用 aggregator 聚合整轮（含工具轮次）再统一记一笔，MVP 够用
var aggregator = new ChatClientMessageAggregator(chatClient.prompt(...).stream());
return aggregator.aggregateStream()
    .doOnComplete(() -> tokenUsageRepo.save(sessionId, model, aggregator.getUsage(), priceTable.estimate(...)));
```
