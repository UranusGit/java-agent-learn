...
### 4.4 动态工具发现（Spring AI 2.0 新特性）

```java
// 运行时动态注册工具
String answer = chatClient.prompt()
        .user("帮我查询信息")
        .tools(ToolCallback.builder()
                ...
                .toolFunction(request -> {
                    String query = request.arguments().get("query").asString();
...
> **遇到阻塞？→ [教程 40-长任务持久化与中断恢复]**：幂等重试、检查点恢复的完整方案。
...
> **想深入？→ [附录 09-Agent安全深度/01-ToolPoisoning攻击.md]**：工具投毒攻击——恶意工具如何劫持 LLM 行为。
> **遇到阻塞？→ [教程 23-工具执行可观测与审计]**：Tool Call Span、参数结果记录、安全审计。
