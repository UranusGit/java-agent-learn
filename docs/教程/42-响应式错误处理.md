...
public class AgentController {
    ...
    private boolean isRetryable(Throwable error) {
        ...
        if (error instanceof org.springframework.web.reactive.function.client.WebClientResponseException) {
            var ex = (WebClientResponseException) error;
...
public class StreamingAgentService {
    ...
    public Flux<AgentStreamEvent> streamChat(String sessionId, String message) {
        ...
        return chatClient.prompt()
            ...
            // 正常完成——清除部分结果缓存
            .doOnComplete(() -> {
                partialResultStore.clear(sessionId);
            })
            // 出错——保存部分结果，发送错误事件
            .onErrorResume(error -> {
            ...
            // 将每个 chunk 包装为事件
            .map(chunk -> {
                if (chunk instanceof AgentStreamEvent event) {
                    return event;  // 已经是事件（错误事件）
                }
                return new AgentStreamEvent(EventType.CONTENT, Map.of("text", chunk));
            });
    }
    ...
    public Flux<String> withBuffer(Flux<String> source) {
        return source
            .onBackpressureBuffer(
                1000,                          // 缓冲区大小
                BufferOverflowStrategy.OVERFLOW // 溢出时发出错误信号
            )
            // 缓冲区溢出时的降级
            .onErrorResume(OverflowException.class, error -> {
...
@Component
public class TracingAdvisor implements CallAdvisor {

    /**
     * 在 Advisor 中读取 Context——实现全链路追踪
     */
    @Override
        ...
        return Mono.deferContextual(ctx -> {
            ...
            return chain.nextCall(request)
                .doOnSuccess(response -> {
                    ...
                    log.info("[{}] Agent call completed in {}ms, tokens={}",
                        traceId, duration, tokens);
                })
                .doOnError(error -> {
                    log.error("[{}] Agent call failed: {}", traceId, error.getMessage());
                });
        });
    }

    @Override
    public Mono<Flux<ChatResponse>> adviseStream(ChatClientRequest request,
...
**下一篇**：[43-Agent治理与合规框架](43-Agent治理与合规框架.md) — 企业级 Agent 上线前的完整治理清单。

---

> **想深入？→ [教程 40-长任务持久化与中断恢复]**：流式中断后的检查点恢复。
> **想深入？→ [教程 44-多模型协作与供应策略]**：降级时切换到哪个备选模型。
