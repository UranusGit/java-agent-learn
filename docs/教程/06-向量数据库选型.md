...
> **想深入？→ [教程 78-高级RAG与AgenticRAG]**：不同向量库在 Agentic RAG 多跳推理场景下的表现差异。
> **遇到阻塞？→ [教程 63-容错与弹性设计](63-容错与弹性设计.md)**：向量数据库不可用时的降级策略。
> **想深入？→ [教程 74-Connect与数据生态]**：业务库/文档变更 → CDC → 自动重嵌入 → 向量库同步的完整管道（知识库与业务库最终一致）。
