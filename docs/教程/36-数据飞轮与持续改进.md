...
>
> **前置阅读**：[35-长任务持久化与中断恢复](35-长任务持久化与中断恢复.md)、[05-RAG检索增强生成](05-RAG检索增强生成.md)。
...
**下一篇**：[37-响应式错误处理](37-响应式错误处理.md) — WebFlux + Spring AI 的错误传播链、降级策略、背压实战。

---

> **想深入？→ [教程 35-长任务持久化与中断恢复]**：飞轮的数据采集需要可靠的任务执行。
> **想深入？→ [教程 38-Agent治理与合规框架]**：用户数据采集的隐私合规要求。
> **想深入？→ [教程 40-Agent架构反模式与避坑指南]**：「无评估闭环」反模式的完整分析。
