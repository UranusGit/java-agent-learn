...
>
> **前置阅读**：[04-记忆与会话管理](04-记忆与会话管理.md)、[03-工具调用](03-工具调用.md)。
...
public class ResilientAgentExecutor {
    ...
    public Mono<String> execute(String taskId, String goal) {
        ...
        return checkpointStore.load(taskId)
            .map(checkpoint -> {
                // 2a. 检查点存在——从断点恢复
                return resumeFromCheckpoint(checkpoint);
            })
            .orElseGet(() -> {
                // 2b. 没有检查点——从头开始
    ...
    private Mono<String> startNewTask(String taskId, String goal) {
        ...
        return chatClient.prompt()
            ...
            .content()
            .flatMap(planJson -> {
                List<String> plan = parsePlan(planJson);
...
Spring AI 2.0 的 `ChatClient` 支持 `TokenCountEstimator` 和 `Advisor`，可以透明地追踪 Token 消耗：
...
@Component
public class BudgetTrackingAdvisor implements BaseAdvisor {
...
public class ReportGenerationAgent {
    ...
    public Mono<String> generateReport(String taskId, String topic) {
        // 预算：复杂分析任务
        BudgetGuard.BudgetConfig budget = BudgetPolicy.forTask("complex_analysis");

        return executor.execute(taskId,
            "分析主题「" + topic + "」并生成完整报告").checkpointConfig(config -> config
            .budget(budget)
            .checkpointInterval(CheckpointInterval.EVERY_STEP)
...
**下一篇**：[36-数据飞轮与持续改进](36-数据飞轮与持续改进.md) — 数据采集、评估闭环、灰度发布的数据飞轮。

---

> **想深入？→ [教程 29-上下文工程]**：上下文窗口管理与压缩，直接影响检查点大小。
> **想深入？→ [教程 38-Agent治理与合规框架]**：检查点中的用户数据隐私合规。
> **想深入？→ [教程 40-Agent架构反模式与避坑指南]**：「无持久化」「无超时无预算」「工具无幂等」三大反模式的完整分析。
