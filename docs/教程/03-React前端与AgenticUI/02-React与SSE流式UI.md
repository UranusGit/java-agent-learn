...
function parseSSE(buffer: string): ParsedEvents {
  ...
  for (const frame of frames) {
    ...
    for (const line of frame.split('\n')) {
      if (line.startsWith(':')) continue;            // 注释行：心跳（见教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §10.3）
...
把 §1 的底层机制封装成 Hook，组件只面对语义化状态（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3.4 自定义 Hook 是前端的领域服务层]）：
...
要点：
- **AbortController 放 useRef** 而非 state——它变了不需要重渲染（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3.3]）
- **依赖数组只有 sessionId**——send 的重建只随会话切换，输入内容变化不会导致 send 重建（对应 [教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3.2 的 SSE 依赖坑]）
...
效果：渲染频率从"每 token 一次"降到"每帧一次"（60Hz 屏幕上 ≤ 60 次/秒），且视觉上 token 是成组出现的——反而更接近人类打字的自然节奏。这是"流式 UI 的背压"——与后端 Reactor 的 buffer/bample 缓冲操作符（[教程 01-WebFlux与响应式编程/02-背压与流量控制 §缓冲策略]）是同一个问题在两端的解法。

### 4.3 配套：状态下沉 + memo

批量缓冲解决"频率"，还要解决"范围"（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §5.3 状态下沉]、[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §4.3]）：
...
组件/Hook 的清理函数（[教程 01-WebFlux与响应式编程/05-WebFlux进阶实战 §3.2 useEffect 清理]）是 React 版的 try-with-resources。**每一条会"逃逸"的资源（连接、rAF、定时器、订阅）都必须有配对的清理**，否则会出现"后台还在收上一个会话的 token"这类诡异 bug。

### 5.2 与后端多页面会话管理对齐

多页签同时打开同一会话时，后端用会话级 Sink fan-out 把一条流推给所有连接（[教程 04-企业级架构主干/04-多页面流式响应与会话管理 §5 多端同步]）。前端侧的配合点：
...
| 流式状态上提全局 | token 更新连坐全站渲染 | 高频状态隔离在流式组件（[教程 01-WebFlux与响应式编程/06-线程模型与调度器 §5]） |
...
**下一篇**：[27-Agentic-UI设计.md](03-Agentic-UI设计.md) — 从"消费文本流"到"渲染 Agent 的思考与行动"：事件协议设计、工具透明化、生成式 UI 与 HITL 交互。
