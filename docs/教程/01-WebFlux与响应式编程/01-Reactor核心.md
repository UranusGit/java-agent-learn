...
> 「本文是对 [教程 00-基础与核心/01-Spring-AI框架入门 §3] 和 [教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §2-§4] 的深入展开」

> **定位**：系统讲解 Reactor 的核心抽象 `Mono` 和 `Flux`，覆盖创建、变换、过滤、组合、错误处理、时间操作、背压等全部操作符类别，以及在 Spring AI 流式输出场景中的实战。
>
> **读者画像**：已读完 [教程 01-WebFlux与响应式编程/00-WebFlux从零入门]（跑通了最小工程与 SSE），需要系统补全 Reactor 操作符体系的开发者。
...
「Sinks 全家族与更细的对比？→ [教程 01-WebFlux与响应式编程/03-Sinks详解]」
...
> 背压的深入讨论见 [01-背压与流量控制](02-背压与流量控制.md)。
