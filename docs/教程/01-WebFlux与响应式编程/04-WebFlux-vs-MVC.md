...
> 完整决策树见 §4 与 §7，此处收拢为两个可直接对照项目的清单。

### 适用场景（选 WebFlux）✅

- **LLM 流式 SSE 是主出口**：token-by-token 推送 + 单调用 10~30s，非阻塞挂起不占线程（§2.1 的三特征）
- **万级并发长连接**：10000 并发 × 20s 的数学账——8 条 EventLoop vs 10000 线程 / 10GB 栈内存（§2.2）
- **全链路有响应式驱动**：WebClient / R2DBC / Reactive Redis 齐备，无阻塞库渗入（§3.3）
- **混合架构的流式接入层**：SSE 端点用 WebFlux，CRUD 与用户服务仍走 MVC + 虚拟线程（§5.2）

### 不适用场景（选 MVC / 虚拟线程）❌

- **非流式一次性调用且并发 < 500 QPS**：§4.1 决策树的 MVC 分支——阻塞 20s 也只是占一条（虚拟）线程
- **团队无响应式经验且 JDK < 21**：学习曲线（§3.1）+ 栈跟踪调试成本（§3.2）压过收益，又没有虚拟线程这条第三路
- **重度依赖阻塞生态**：JDBC / 阻塞 Redis 遍布时，WebFlux 的非阻塞优势被抵消还叠加复杂度（§3.3、§8 结论 4）
- **已有稳定 MVC 系统且无流式新需求**：整体迁移是重写税——按 §5 混合架构，只把新增 SSE 端点做成 WebFlux
