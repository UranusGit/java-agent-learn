...
> **定位**：本文面向已理解 Mono/Flux 基础（前置：[教程 01-WebFlux与响应式编程/00-WebFlux从零入门]、[教程 01-WebFlux与响应式编程/01-Reactor核心]）但从未用过 Sinks 的开发者。讲透 Sinks 是什么、四种形态怎么选、emit/tryEmit/EmitResult 的完整语义、多线程发射的坑，以及 Agent 场景（进度推送、多页面共享 LLM 流）的实战。API 版本：reactor-core **3.8.6**（Boot 4.1.0 → reactor-bom 2025.0.6），全部签名经本地 jar `javap` 实证。
...
- **命令式世界的数据入口**：第三方 SDK 回调（`onMessage`）、后台任务线程要把数据塞进响应式管道——unicast 桥接单消费者（§1、§4.1）
- **一个生产者、多个消费者**：同一会话的 LLM token 流广播给多个页面——multicast + 每会话 sink 的 `ConcurrentHashMap` 路由（§7、§10.2）
- **迟到者补看历史**：刷新页面后要看到之前的输出，`replay().limit(n)` 一条 API 完成"历史回放 + 实时追加"（§4.3）
- **纯信号控制**：优雅关闭、取消信号只需"到达与否"不携带负载，`Sinks.Empty` 表达（§3.2）
- **生产与消费生命周期跨请求解耦**：POST 触发生产、GET 随时挂载消费，两条时间线通过 sink 解耦（§10.2 时序图）

### 不适用场景 ❌

- **副作用必须随订阅发生且生产可表达为订阅驱动管道**：用 `Flux.create/push + share()/cache()` 更声明式，生命周期自动管理（§10.1 决策口诀）
- **跨 JVM/跨实例广播**：Sinks 是单进程内存热源，多实例要 Kafka/Redis Pub-Sub 这类外部事件通道
- **需要持久化与断线完整恢复**：replay 只在内存里存历史，进程重启即失——持久化回放走会话日志/事件溯源
- **单消费者顺序消费且无需热语义**：直接操作符链即可，引入 sink 是多余的一跳外加一处泄漏点（§10.4 的 Map 清理负担）
...
「Sinks 排队的元素如何被下游按需拉取？→ [教程 01-WebFlux与响应式编程/02-背压与流量控制]」
