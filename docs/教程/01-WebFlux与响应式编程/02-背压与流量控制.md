...
> 「本文是对 [教程 08-架构师进阶/08-响应式错误处理 §2-§4] 的深入展开」
...
- **SSE 客户端消费速率不可控**：LLM 产出 100~200 tokens/s，弱网/慢设备消费跟不上，`onBackpressureLatest` 保实时弃历史（§4.1）
- **并行任务结果洪峰**：多 Agent 并行/批量推理用 `flatMap` 并发度参数 + 有界缓冲（`DROP_OLDEST`）控制堆积（§4.2）
- **批量写入下游慢于上游**：Embedding 批量写向量库，`buffer(100) + flatMap(write, 3) + 有界缓冲` 防积压（§4.3）
- **对接有 RPM/TPM 限制的 LLM API**：Resilience4j RateLimiter / Reactor 令牌桶 / flatMap 并发度，防 429（§5）
- **需要量化供需差**：`doOnRequest/doOnNext` + Micrometer 监控 requested/produced/dropped，让背压从隐式行为变成可观测指标（§7）

### 不适用场景 ❌

- **生产速率天然不高于消费速率的低流量链路**：§3.5 的策略选择树永远走不到，背压算子是死代码
- **一条不能丢且消费必然更慢**：Buffer 只能延迟不能提速——真正的解法是扩消费并行度或架构升维，不是换背压策略
- **非响应式栈（MVC + 虚拟线程）**：`request(n)` 拉取模型不存在，API 限流用应用层 RateLimiter/信号量即可
- **单值请求（Mono）**：背压策略作用于多元素流，0/1 元素的 Mono 没有积压问题
...
在教程 05-Observation可观测/09-Advisor与RAG观测：检索质量可观测（响应式错误处理）中，背压溢出是错误处理的重要场景之一。理解背压机制，才能在 Agent 高并发场景中做出正确的流量控制决策。
