...
**设计原则**（来自 [附录 14-Agent交互设计/00-Agent用户体验设计]，本文用 React 落地）：
...
这套 reducer 与后端的审批状态机（[教程 28-Human-in-the-Loop与审批流 §状态机]）在 `awaiting_approval` 挂起/恢复的语义上严格对齐——同一张状态图，两端各实现一半。
...
**交互决策**：默认折叠降低认知负担；但**正在思考时**要有一个活动指示（"正在思考…"骨架屏），让用户知道系统没死——这与 [附录 14-Agent交互设计/00 §中间状态] 的"3 秒规则"一致：超过 3 秒的静默必须有可感知的进度信号。
...
**关键设计**：拒绝是安全默认（"允许"用醒目危险色提示后果，不是诱导点击）；备注写入审计日志——前端交互设计与后端审计要求（[教程 23-工具执行可观测与审计 §审计数据模型]）直接挂钩。
...
**永远不要让 LLM 输出 HTML/JSX 直接渲染**（`dangerouslySetInnerHTML`）。间接 Prompt 注入（[附录 09-Agent安全深度/00-Prompt注入分类与案例 §间接注入]）可以让 RAG 文档内容污染 LLM 输出——若输出可直接变成可执行 UI，等于把 XSS 的钥匙交给攻击者。受限 DSL + 白名单注册表是唯一安全形态：
...
- 用户刷新页面后，通过 `GET /rounds/active` 查回挂起中的轮次，重渲染审批卡（**跨刷新恢复**——会话状态持久化的消费端，[教程 25-历史记录持久化与合规 §对话回放]）
...
**React 路线到此完成**。下一步进入实践：[项目/04-React-Agent控制台](../项目/04-React-Agent控制台/00-需求分析与架构设计.md) — 把教程 15-18 的全部知识组合成一个完整的 Agent 前端项目。后端配套深化见 [教程 19-流式工具调用与事件协议]。
