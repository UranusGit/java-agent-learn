...
>
> **前置阅读**：[22-全链路可观测性](22-全链路可观测性.md)、[23-工具执行可观测与审计](23-工具执行可观测与审计.md)、[43-Agent治理与合规框架](43-Agent治理与合规框架.md)。想深入评估 → [37-自我反思与Agent评估](37-自我反思与Agent评估.md)。
...
过程透明的基座是**全链路事件流**（[22-可观测](22-全链路可观测性.md) 的 Trace + [19-轨迹级评估](../项目/19-企业级Agent信任与评测平台/08-轨迹级评估与模拟沙箱.md) 的轨迹）——但可解释视角要求事件**面向"人类可读的解释"再组织**：
...
**要点**：低置信结论要**显式降级表达**（"未找到直接依据"）或走 HITL（呼应 [28-Human-in-the-Loop](28-Human-in-the-Loop与审批流.md)）——置信度是**触发行为**的信号，不是装饰数字。
...
- **审计合规**：归因链 + 置信度是 [43-治理](43-Agent治理与合规框架.md)/EU AI Act 式"决策解释义务"的直接交付物（呼应 [13-附录治理](../附录/13-AI治理与合规/00-NIST-AI-RMF框架.md)）。
...
> **实操建议**：先把 [22-可观测](22-全链路可观测性.md) 的 Trace 投影成解释时间线（成本最低），再给高风险结论加归因表，最后做置信校准。落地完整平台 → [项目30-企业级Agent可解释与透明平台](../项目/30-企业级Agent可解释与透明平台/00-需求分析与架构设计.md)。
