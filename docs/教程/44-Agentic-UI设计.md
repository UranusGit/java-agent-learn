...
> **定位**：本文是 React 路线的收官篇——从"渲染一条文本流"进化到"渲染一个 Agent 的完整行为"：事件协议设计、思考过程展示、工具调用透明化、生成式 UI（JSON→组件）、HITL 审批交互。读者画像：已读完 [教程 41-43]，能实现流式对话界面的开发者。前置阅读：[教程 43-React与SSE流式UI]、后端事件源见 [教程 45-流式工具调用与事件协议]。
>
...
**设计原则**（来自 [附录 11-Agent交互设计/00-Agent用户体验设计]，本文用 React 落地）：
...
这套 reducer 与后端的审批状态机（[教程 22-Human-in-the-Loop与审批流 §状态机]）在 `awaiting_approval` 挂起/恢复的语义上严格对齐——同一张状态图，两端各实现一半。
...
**交互决策**：默认折叠降低认知负担；但**正在思考时**要有一个活动指示（"正在思考…"骨架屏），让用户知道系统没死——这与 [附录 11-Agent交互设计/00 §中间状态] 的"3 秒规则"一致：超过 3 秒的静默必须有可感知的进度信号。
...
**关键设计**：拒绝是安全默认（"允许"用醒目危险色提示后果，不是诱导点击）；备注写入审计日志——前端交互设计与后端审计要求（[教程 17-工具执行可观测与审计 §审计数据模型]）直接挂钩。
...
- 用户刷新页面后，通过 `GET /rounds/active` 查回挂起中的轮次，重渲染审批卡（**跨刷新恢复**——会话状态持久化的消费端，[教程 19-历史记录持久化与合规 §对话回放]）
...
**React 路线到此完成**。下一步进入实践：[项目/04-React-Agent控制台](../项目/04-React-Agent控制台/00-需求分析与架构设计.md) — 把教程 41-44 的全部知识组合成一个完整的 Agent 前端项目。后端配套深化见 [教程 45-流式工具调用与事件协议]。
