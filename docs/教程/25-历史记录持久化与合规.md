...
```java
// 注意: Spring AI 官方只提供 InMemory / JDBC 两种 ChatMemoryRepository
//（Redis/Mongo 仓库并非官方内置，见附录 12 基准 §2.2）。
...
public class AuditLoggingAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        var conversationId = request.context()
...
> **合规口径修正**：金融/医疗类记录的留存义务是"**不可删除**"（WORM）而非"到期物理删除"——两类义务并存时取更严格者。GDPR 删除权与金融留存的调和方式：**销毁"可识别性"而非销毁记录**（删假名映射、保留脱敏审计链，详见 [项目 06 §v6 删除权] 与 [附录 13-AI治理与合规/02 §GDPR 调和]）。
...
**下一篇**：[26-多租户隔离与资源治理](26-多租户隔离与资源治理.md) — 多租户场景下的会话隔离、数据隔离、资源配额、工具权限分级。

**前置阅读**：
- [04-记忆与会话管理](04-记忆与会话管理.md) — ChatMemory 的基本概念，本篇在此基础上讨论持久化与合规。

**相关阅读**：
- [27-成本治理与Token计量](27-成本治理与Token计量.md) — 审计日志中的 Token 消耗数据如何用于成本治理。
- [28-Human-in-the-Loop与审批流](28-Human-in-the-Loop与审批流.md) — 危险操作的审计追踪。
