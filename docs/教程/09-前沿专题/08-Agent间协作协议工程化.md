...
> **定位**：讲透 Agent 与 Agent 之间**协作的工程化**——把 [09-多Agent协作](../00-基础与核心/09-多Agent协作.md)（编排）升级为**跨团队/跨组织异构 Agent 的互操作协议**（A2A 思路）。涵盖：**AgentCard 注册与能力发现、任务委托与协商、协作安全（身份/授权/注入边界）、失败与补偿**。读完这篇，你能把"Agent 之间怎么可信、可控地协作"做成系统。读者画像：要回答"不同团队/厂商的 Agent 怎么安全协作、不失控"的架构师。
>
> **前置阅读**：[09-多Agent协作](../00-基础与核心/09-多Agent协作.md)、[64-安全与权限控制](../04-企业级架构主干/11-安全与权限控制.md)、[20-MCP协议](../02-SpringAI核心机制/01-MCP协议.md)、[前沿 00-A2A协议](../../前沿/00-A2A协议.md)。
...
单 Agent 内部的 Agent 编排（[09](../00-基础与核心/09-多Agent协作.md)）是"主人调度仆从"；**Agent 间协作协议**解决的是**对等/跨信任域**的互操作——不同团队、不同厂商构建的 Agent 要"能对话、能安全地把活委托出去"。
...
**协商**：接受前双方对齐"能做什么、花多少成本、何时交"——这是协作协议的"合同"环节，避免委托后才知道做不了/太贵（呼应 [20-成本治理](../04-企业级架构主干/07-成本治理与Token计量.md)）。
...
跨组织协作不能用内部 Session，要用**可验证的身份**（如 SPIFFE/SVID 或 OIDC/JWT）——被委托方能验证"来者是谁、属于哪"（呼应 [31-安全](../04-企业级架构主干/11-安全与权限控制.md)、[08-零信任](../../项目/08-Agent供应链安全网关/00-需求分析与架构设计.md)）。
...
对方返回的内容**不自动当指令**——把"跨信任域返回"当**数据**而非**代码/系统提示**（防对方 Prompt 注入反向攻击，呼应 [31-Prompt注入防御](../04-企业级架构主干/11-安全与权限控制.md)、[附录08-Agent安全深度](../../附录/08-Agent安全深度/00-Prompt注入分类与案例.md)）。
...
| 委托方取消 | 被委托方安全收尾（呼应 [24-长任务取消](../../项目/24-企业级长任务工作流引擎/10-预算可视化与HITL暂停恢复.md)） |

**要点**：协作协议要支持**异步、可查询、可取消**的任务语义（长任务委托），而非同步 RPC（呼应 [教程 05-Observation可观测/07-指标治理：Token计量、SLO与基数熔断](../08-架构师进阶/06-长任务持久化与中断恢复.md)）。
...
### 不适用场景 ❌
- 同一应用内多 Agent 编排（[09](../00-基础与核心/09-多Agent协作.md) 即可，无需协议开销）。
