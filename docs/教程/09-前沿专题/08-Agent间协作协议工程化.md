...
>
> **前置阅读**：[09-多Agent协作](../00-基础与核心/09-多Agent协作.md)、[04-企业级架构主干/11-安全与权限控制](../04-企业级架构主干/11-安全与权限控制.md)、[02-SpringAI核心机制/01-MCP协议](../02-SpringAI核心机制/01-MCP协议.md)、[前沿 00-A2A协议](../../前沿/00-A2A协议.md)。
...
A2A 的协议栈：**JSON-RPC 2.0 over HTTP(S)**（请求/响应）+ **SSE**（流式更新与任务事件推送）——与本项目 WebFlux SSE 技术栈天然对齐（呼应 [教程 02-SpringAI核心机制/00-SSE流式通信](../02-SpringAI核心机制/00-SSE流式通信.md)）。消息侧三类实体：
