...
> **定位**：讲透 Agent 平台的"入口咽喉"——意图路由与路由编排（Intent Routing / Routing Orchestration）。在 [65-模型路由与降级](../04-企业级架构主干/12-模型路由与降级.md) 讲的是"按成本/延迟选模型"，本文升级为**"按意图选工作流"**：系统收到一个用户请求，先识别它要干什么，再把它分发给正确的 Agent、工作流、工具或模型。读完这篇，你能把"多业务线共用一个入口"的烟囱，收敛成"一个智能路由网关"。
>
> **读者画像**：已经掌握 ChatClient、工具调用、多 Agent 编排，需要把"松散的多 Agent/多工具"收敛为"统一策略路由"的中高级架构师。
>
> **前置阅读**：[02-ChatClient与对话模型](../00-基础与核心/02-ChatClient与对话模型.md)、[65-模型路由与降级](../04-企业级架构主干/12-模型路由与降级.md)、[09-多Agent协作](../00-基础与核心/09-多Agent协作.md)。想深入 embedding 原理 → [附录 01-LLM基础理论/01-Embedding原理](../../附录/01-LLM基础理论/01-Embedding原理.md)。
...
**路由的价值**：每个业务线用自己专属的 Prompt、工具、记忆、模型、成本预算、审计策略——**而不是用一个臃肿的"万能 Agent"想覆盖一切**。这是"管控分离"理念（[教程 04-企业级架构主干/00-管控分离架构](../04-企业级架构主干/00-管控分离架构.md)）在入口侧的落地。
...
- **低于阈值兜底**：语义路由的命门是**阈值**——太低会把"退换货"误分到"文档问答"，太高又全兜底。阈值需用真实语料标定（见 [附录 20](../../附录/17-路由与推理策略/00-路由表与语义路由.md)）。
...
识别意图后，路由网关要把请求**分发给真正的工作流**。这里复用 [32-模型路由](../04-企业级架构主干/12-模型路由与降级.md) 的"多 ChatClient + @Qualifier"模式，但把路由目标从"模型"升级为"ChatClient + 配套执行链"。
...
**要点**：高风险路由（支付、审批、删改）应设**保守阈值 + HITL 复核**（呼应 [28-Human-in-the-Loop](../04-企业级架构主干/08-Human-in-the-Loop与审批流.md)），宁可通过率低，不可误放行。

---

## 4. 路由观测：命中率、兜底率、意图漂移

路由网关是"数据最多的地方"——每一次请求都留下路由信号。**可观测是意图路由的关键闭环**（呼应 [31-全链路可观测性](../04-企业级架构主干/02-全链路可观测性.md) / [32-工具执行可观测与审计](../04-企业级架构主干/03-工具执行可观测与审计.md)）。
...
用 Micrometer（配 [教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台](../05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台.md)）打点，当某意图命中率跌破基线时触发"新意图涌现"告警，提示补充示例句——**这就是意图路由侧的数据飞轮**（呼应 [84-数据飞轮与持续改进](../08-架构师进阶/07-数据飞轮与持续改进.md)）。
...
高风险业务的每次路由都应记录"输入→意图→置信度→所选工作流→产出"，供事后回放与合规审计（呼应 [58-历史记录持久化与合规](../04-企业级架构主干/05-历史记录持久化与合规.md)）。

---

## 5. 路由 A/B 与灰度

意图路由的规则会变（新增意图、改示例句、调阈值），**上线前必须在灰度中验证误分率不上升**（呼应 [62-灰度发布与版本管理](../04-企业级架构主干/09-灰度发布与版本管理.md)）。
...
> **实操建议**：先用**规则 + 语义路由**跑通用例，观察兜底率；兜底率高再补示例句；只有语义也搞不定的开放意图，才引入 LLM 分类器。别一上来就烧 LLM。落到完整企业平台，见 [项目23-企业级Agent意图路由网关](../../项目/23-企业级Agent意图路由网关/00-需求分析与架构设计.md)；想深入路由表/阈值标定 → [附录 20](../../附录/17-路由与推理策略/00-路由表与语义路由.md)。
