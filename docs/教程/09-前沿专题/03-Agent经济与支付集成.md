...
1. **支付工具进自动白名单**——支付永远是 HITL 候选；"小额全自动"的阈值要明确写进策略引擎（[教程 02-SpringAI核心机制/01-MCP协议 §3.3]）。
2. **幂等只靠网关**——网关幂等键有 TTL；业务幂等表（request_id → receipt）自己落库。
3. **额度单机计数**——多实例必然超卖；原子预扣+冲正（§2）。
4. **审计日志可删改**——资金审计走 append-only + 哈希链（[教程 02-SpringAI核心机制/04-Advisor链与拦截器 §4]），与账本双录。
