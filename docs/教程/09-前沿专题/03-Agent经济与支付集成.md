...
>
> **前置阅读**：[教程 00-基础与核心/03-工具调用]；[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]（超限审批的机制底座）；[教程 04-企业级架构主干/03-工具执行可观测与审计]（交易审计）。
>
...
| ② 超限 HITL | 单笔超阈值 或 首次向新收款方付款 → 挂起审批（[教程 04-企业级架构主干/08-Human-in-the-Loop与审批流] 的 ToolCallback 包装方案直接复用） | 中额/新商户 |
| ③ 硬熔断 | 日预算耗尽 / 异常频率（1 分钟 >N 笔）→ 拒绝并告警 | 兜底（[教程 04-企业级架构主干/07-成本治理与Token计量] 预算熔断的资金版） |

额度是**租户/用户级全局状态**：分布式存储 + 原子扣减（Redis Lua 预扣+冲正，[教程 04-企业级架构主干/07-成本治理与Token计量] 深化点同款），不是单机计数。
...
**企业决策框架**：先问"对手方是谁"——人类用户代付走 PSP；企业内工具生态走配额后结（今天就能上线）；跨组织的机器微支付再评估加密轨道（把它当"外币账户"管理：额度小、隔离、可丢）。三条轨道可并存（分层供应，[教程 08-架构师进阶/10-多模型协作与供应策略] 的多供应商思想平移到资金面）。
...
public PurchaseResult purchaseToolCredit(
    ...
    // ③ 审计事件（Outbox 同事务落库→Kafka，[教程 07-Kafka事件骨干/03-投递语义与事务] §6]）
    auditPublisher.publish(PaymentEvent.of(userId, toolId, receipt, auth));
    return PurchaseResult.success(receipt);
}
```

四条铁律：**幂等键贯穿**（LLM 会重试工具调用——同一 request_id 的重复购买必须返回原结果，[教程 07-Kafka事件骨干/03-投递语义与事务] §5] 的资金版，幂等表落库不是 Redis）；**身份走 ToolContext**（用户/额度上下文从请求注入，LLM 传参可被注入篡改，[附录 05 §1.1]）；**审计先于成功返回**（资金动作的审计是合规要件，[教程 04-企业级架构主干/03-工具执行可观测与审计] 的哈希链/WORM 在此适用）；**失败语义显式**（pending/success/declined 三态返回给 LLM，让它能向用户解释而不是无限重试）。
...
sequenceDiagram
    ...
    P-->>A: 待审批（LLM 向用户说明）
    Note over H: 审批挂起机制复用教程 03-React前端与AgenticUI/04-流式工具调用与事件协议<br/>（含超时升级/双人复核）
    H-->>Z: 批准
...
| 供应商滥用 | 工具商虚报用量 | 用量对账（工具调用 Span 计数 vs 计费账单，[教程 04-企业级架构主干/03-工具执行可观测与审计] 观测即对账依据） |
...
1. **支付工具进自动白名单**——支付永远是 HITL 候选；"小额全自动"的阈值要明确写进策略引擎（[教程 02-SpringAI核心机制/07-MCP协议 §3.3]）。
2. **幂等只靠网关**——网关幂等键有 TTL；业务幂等表（request_id → receipt）自己落库。
3. **额度单机计数**——多实例必然超卖；原子预扣+冲正（§2）。
4. **审计日志可删改**——资金审计走 append-only + 哈希链（[教程 02-SpringAI核心机制/01-Advisor链与拦截器 §4]），与账本双录。
5. **无对账**——工具用量观测（Span 计数）与账单月度对账，虚报只有对账能抓。

## 9. 总结

Agent 支付的工程结构一句话：**LLM 只表达意图，钱由确定性原语走**——三道闸（预算内自动/超限 HITL/硬熔断）、三轨道按对手方选（PSP/配额后结/加密微支付）、四铁律（幂等贯穿/身份走 ToolContext/审计先行/三态返回）。协议与生态的动态（x402、MCP 支付原语）见 [前沿 09-Agent经济与支付]；审批挂起机制回看 [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]；成本计量体系回看 [教程 04-企业级架构主干/07-成本治理与Token计量]。
