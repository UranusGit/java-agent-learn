...
>
> **前置阅读**：[教程 00-基础与核心/05-RAG检索增强生成]、[附录 06-企业级架构模式]。
...
| 5 | 若是 LLM 慢 → 看 gen_ai tag（模型/token 数） | [教程 02-SpringAI核心机制/01-MCP协议] |
...
下一关 [教程 00-基础与核心/08-Plan-and-Execute模式]：**存储工程**——span 数据落到哪（后端选型）、怎么建模（traceId 索引 vs 标签索引）、怎么查（查询路径与 TTL）；随后 [教程 00-基础与核心/09-多Agent协作]（使用运营）与 [教程 02-SpringAI核心机制/00-SSE流式通信]（综合实战收官）。
