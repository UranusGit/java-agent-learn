...
下一关 [教程 00-基础与核心/08-Plan-and-Execute模式]：**存储工程**——span 数据落到哪（后端选型）、怎么建模（traceId 索引 vs 标签索引）、怎么查（查询路径与 TTL）；随后 [教程 00-基础与核心/09-多Agent协作]（使用运营）与 [教程 02-SpringAI核心机制/06-SSE流式通信]（综合实战收官）。
