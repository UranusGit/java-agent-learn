...
>
> **前置阅读**：[教程 06-TraceId全链路追踪/07-管控分离微服务落地：traceId架构 §7.3]（分级采样——存储体量的第一道闸）。
...
下一关 [教程 06-TraceId全链路追踪/09-使用运营：从排障到告警成本与飞轮]：**使用运营**——存起来的 traceId 怎么"用出价值"：告警（SLO/错误预算）、容量规划、按租户成本归因、评估飞轮与治理看板。
