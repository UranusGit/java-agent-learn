...
Micrometer 已把 span 侧的时延/错误聚合成指标（教程 05-Observation可观测/07-指标治理 已实证的 `spring.ai.chat.client` 等时延指标体系）。用 Actuator 直查：
...
> 告警拉响之后呢？——AI 系统的事件有自己的四种形态（坏输出风暴/成本火情/静默漂移/注入爆发），分级、Runbook 处置卡与演练闭环见 [附录 18-可观测平台实践/01-AI事件响应工程 §4-§7]。
...
归因结果的三个去向：①租户账单（SaaS 计量）；②配额联动（Top1 租户超配额触发限流，[教程 多租户] 主题）；③模型路由决策（token 大户路由到便宜模型，成本治理闭环，[附录 17-路由与推理策略]）。**注意**：token 精确值走 `gen_ai.client.token.usage` 指标体系（教程 05-Observation可观测/07-指标治理 实证），span 侧只做链路对账。
