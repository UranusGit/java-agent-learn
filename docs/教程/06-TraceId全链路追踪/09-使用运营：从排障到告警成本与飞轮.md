...
>
> **前置阅读**：[教程 00-基础与核心/07-ReAct推理模式 §7.3]（分级采样）、[教程 00-基础与核心/08-Plan-and-Execute模式]（存储与查询路径）。
...
要点：告警条件用**燃烧速率**（budget burn rate，如 2 小时烧掉 5% 预算才 page），不是单次超时——单点超时是常态噪音。深挖见 [教程 00-基础与核心/07-ReAct推理模式]（SLO 与基数熔断）与 [教程 企业级场景清单]。
...
| 看什么 | 怎么看 | 出问题的动作 |
|--------|--------|-------------|
| 基数健康 | 指标 tag 的 distinct 值数（[教程 00-基础与核心/07-ReAct推理模式] 基数熔断） | 找到塞 userId 的 tag，源头改 tag-fields 白名单 |
...
收官关 [教程 02-SpringAI核心机制/06-SSE流式通信]：**综合实战**——把 00-09 全部能力串成工业巡检 Agent 的可运行闭环。

---

**实证基线**：`MeterRegistry.find(String).timers()`、`Timer.percentile(double,TimeUnit)/max(TimeUnit)`（micrometer-core 1.17.0 真实 API）；`@Scheduled/@EnableScheduling`（Spring 真实注解）；9.4 SQL 显式标注概念示意（对齐 08 关 DDL）；SLO/燃烧速率/飞轮为方法论内容，出处 [教程 00-基础与核心/07-ReAct推理模式]、[教程 04-企业级架构主干/02-全链路可观测性]。
