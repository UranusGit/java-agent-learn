...
| 流中断 | `stream()` 中途 onError | 流式部分结果要决定"保留部分输出+标注"还是整体重试——见教程 05-Observation可观测/09-Advisor与RAG观测：检索质量可观测 |
| 模型过载/降级 | 5xx | 降级路由（教程 04-企业级架构主干/03-工具执行可观测与审计），不要原地重试打爆 |
...
> **遇到阻塞？→ [教程 08-架构师进阶/08-响应式错误处理]**：onErrorResume/onErrorMap/retryWhen 的完整语义与流中断恢复。
...
1. **ChatClient 是线程安全的，但带状态的 Advisor 链不是自动安全**——自定义 Advisor 里放可变字段（计数器/缓存）时，多请求并发共享同一实例；计数要用原子类型，上下文要走 Reactor Context（教程 03-React前端与AgenticUI/02-React与SSE流式UI）。
2. **多轮上下文不会自动带上**——ChatClient 每次调用都是无状态的；"多轮对话"要靠 `MessageChatMemoryAdvisor`（教程 00-基础与核心/00-Agent核心概念 §12），新手最常以为框架帮你记住了历史。
3. **上下文长度是硬预算**——System+历史+工具 Schema+用户输入全算 token；超长不是报错而是静默截断或 400。工具多时 Schema 占用惊人（教程 05-Observation可观测/01-读懂输出：span树与观测生命周期 的五层预算分配）。
