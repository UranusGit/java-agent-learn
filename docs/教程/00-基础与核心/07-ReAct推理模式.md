...
> **遇到阻塞？→ [教程 01-WebFlux与响应式编程/03-Sinks详解 链与拦截器]**：Advisor 的完整生命周期、order 机制、自定义 Advisor 的详细写法。
...
> **想深入？→ [教程 08-架构师进阶/00-上下文工程]**：Token 预算管理、上下文窗口压缩技术。
...
> → [教程 00-基础与核心/08-Plan-and-Execute模式 模式]：Plan-and-Execute 的完整实现和任务分解策略。
...
**解决**：优化工具的 `description`，明确区分适用场景。详见 → [教程 00-基础与核心/03-工具调用]。
...
> → [教程 00-基础与核心/03-工具调用]：@Tool 注解、工具注册、ToolCallingAdvisor 的完整机制。
> → [教程 01-WebFlux与响应式编程/03-Sinks详解 链与拦截器]：Advisor 链的 order 机制、自定义 Advisor 的详细实现。
> 想深入？→ [附录 01-LLM基础理论（ReAct 原论文: arxiv.org/abs/2210.03629）]：ReAct 原论文的完整解读。
> 遇到阻塞？→ [教程 08-架构师进阶/00-上下文工程]：Token 预算管理，控制 ReAct 循环的上下文增长。
