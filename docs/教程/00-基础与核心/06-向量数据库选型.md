...
> **想深入？→ [教程 08-架构师进阶/01-高级RAG与AgenticRAG]**：不同向量库在 Agentic RAG 多跳推理场景下的表现差异。
> **遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计](../04-企业级架构主干/10-容错与弹性设计.md)**：向量数据库不可用时的降级策略。
> **想深入？→ [附录 19-向量数据库与检索工程/03-嵌入模型升级与索引迁移](../../附录/19-向量数据库与检索工程/03-嵌入模型升级与索引迁移.md)**：§5 的"零代码切换"只对同一嵌入模型的换库成立；换嵌入模型（维度/向量空间都变）是大版本事件——迁移策略三选型、回填管线与金标准重跑验证见该篇。
> **想深入？→ [教程 07-Kafka事件骨干/07-Connect与数据生态]**：业务库/文档变更 → CDC → 自动重嵌入 → 向量库同步的完整管道（知识库与业务库最终一致）。
