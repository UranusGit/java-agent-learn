...
>
> **前置阅读**：[03-工具调用](03-工具调用.md)、[35-高级 RAG 与 Agentic RAG](78-高级RAG与AgenticRAG.md)。
...
> **并发提示**：Java 21 的虚拟线程让 `CompletableFuture.supplyAsync` 的线程池开销大幅降低，详见 [38-Agent 性能优化](81-Agent性能优化.md)。
...
**下一篇**：[37-自我反思与 Agent 评估](80-自我反思与Agent评估.md) — Reflection 模式、评估指标体系、A/B 测试与回归测试。

---

> **前置回顾**：[35-高级 RAG 与 Agentic RAG](78-高级RAG与AgenticRAG.md)讲了 Agentic RAG 的反思机制——本章的 IterativeRefinementWorkflow 是它的通用化抽象。
> **性能**：工作流中并行节点的性能优化，详见 [38-Agent 性能优化](81-Agent性能优化.md)。
> **治理**：多 Agent 工作流的资源治理与配额控制，详见 [教程 59-多租户隔离与资源治理]。
> **事件驱动**：工作流的异步/事件驱动形态（命令队列 + 消费组工作机群 + Saga 补偿），详见 [教程 76-Spring集成与Agent事件驱动落地] 与 [附录 06-企业级架构模式/02-事件驱动Agent架构]。
