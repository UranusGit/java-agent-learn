...
不适用场景：单体+单人（Zipkin 内存都嫌重，日志 grep 足够）；合规禁止 span 含 prompt 内容的行业——先上脱敏管道再谈存储（`spring.ai.chat.observations.log-prompt` 保持关闭，[教程 20]）。
...
下一关 [教程 09]：**使用运营**——存起来的 traceId 怎么"用出价值"：告警（SLO/错误预算）、容量规划、按租户成本归因、评估飞轮与治理看板。
