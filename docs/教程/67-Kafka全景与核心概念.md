...
>
> **读者画像**：能熟练使用 Spring Boot 开发 Web 服务、读过 [教程 30-微服务拆分与Agent部署] 的中高级 Java 开发者；对 Kafka 可能零基础，或用过但说不清分区/位移/ISR 的原理。
>
> **前置阅读**：[教程 21-Agent状态管理]（理解会话状态为什么需要事件化）；[附录 06-企业级架构模式/02-事件驱动Agent架构]（本文在中间件层为其"下钻"）。
>
...
|------|--------------|----------------------|
| **长耗时任务占资源** | 一次 ReAct 循环 = 多轮 LLM 调用 + 工具执行，30s~5min；WebFlux 事件循环虽不被阻塞，但任务状态散落在内存中，重启即丢 | 任务变成事件流，消费进度持久化为**位移（Offset）**，进程崩溃后从上次位移继续（呼应 [教程 83-长任务持久化与中断恢复]） |
| **上下游速率失配** | LLM 网关 200 QPS，工具执行 20 QPS，评估管道每天批量跑——直接调用必然互相拖垮 | Kafka 是**持久化缓冲区**：生产者速率与消费者速率彻底解耦，削峰填谷是结构属性而非附加功能 |
| **审计与回放缺失** | 可观测事件写日志文件，无法回放、无法重算 | 事件按保留策略持久存储，**可以重新消费**——审计、评估数据集回放、投影重建都依赖这一点（呼应 [教程 58-历史记录持久化与合规]、[教程 84-数据飞轮与持续改进]） |
...
这个双重身份对 Agent 架构极其重要：**事件溯源需要日志形态（完整事件序列），会话快照需要表形态（每会话最新状态）**——同一个 Kafka 集群用不同的 `cleanup.policy` 就能同时服务两种需求（详见 [教程 71-日志存储与高可用复制 §3]）。
...
- **Record**：key + value + timestamp + headers。headers 常用来透传 W3C `traceparent`（全链路追踪穿透 Kafka，见 [教程 76-Spring集成与Agent事件驱动落地 §6]）。
...
- **ISR（In-Sync Replicas）**：与 Leader 保持同步的副本集合。`acks=all` 的含义是"消息进入 ISR 中所有副本才算成功"——这是 Kafka 可靠性语义的基石（[教程 70-投递语义与事务 §2]）。
- **Controller**：负责 Leader 选举与元数据管理。Kafka 4.x 只有 **KRaft 模式**（ZooKeeper 已在 4.0 移除）：控制器自己组成 Raft 仲裁，元数据本身就是一条 Raft 日志（`__cluster_metadata`），故障切换从"分钟级"降到"毫秒~秒级"（演进细节见 [教程 71-日志存储与高可用复制 §5]）。
- **Consumer Group / Coordinator**：同组消费者瓜分一个订阅主题的全部分区；Group Coordinator（Broker 侧）管理成员与位移。这是"Agent 工作机群横向扩缩容"的机制来源（[教程 69-消费者与消费组]）。
...
记住三个关键点：① 生产者推、消费者**拉**（pull 模型 = 天然背压，消费不过来就少 poll，见 [教程 12-背压与流量控制]）；② 高水位（HW）之前的数据才对消费者可见；③ **位移提交发生在消费之后**，这决定了 at-least-once 语义与重复处理问题（[教程 70-投递语义与事务]）。
...
| 多租户 | 弱（靠 ACL+配额，见 [教程 75-运维监控与安全]） | 好（vhost） | 一般 | **原生多租户** |
| 运维复杂度 | 中（KRaft 后降低） | 低 | 中 | 高（三层架构） |

**Agent 系统的选型结论**（呼应 [教程 87-多模型协作与供应策略] 的决策框架）：
...
2. **KIP-848（4.0 GA）改变了再平衡的心智模型**：Broker 侧 Group Coordinator 直接管理成员心跳与分配，移除了经典协议的"全体停止-重新入组"屏障。对频繁发布的 Agent 服务来说，滚动重启的抖动大幅降低（机制详见 [教程 69-消费者与消费组 §4]）。
...
| `agent.tool.audit` | sessionId | delete, 400d + 分层存储 | 合规审计（→ [教程 32]、[教程 58]） |
| `agent.commands` | agentId | delete, 3d | Agent 工作机群（消费组） |
| `kb.change.events`（CDC） | docId | compact | 知识库→向量库同步（→ [教程 74-Connect与数据生态 §5]） |

### 5.2 设计原则

1. **保留策略决定主题边界**：审计 400 天与指令 3 天混在一个主题，只能按最长保留付存储成本。
2. **Key 决定一切**：key = 顺序边界（分区内有序）+ 压实边界（每键最新值）。选 `sessionId` 而不是 `tenantId`（租户热键倾斜，见 [教程 68-生产者机制与调优] §4]）。
3. **事件粒度宁可粗**：一个"工具执行完成"事件携带完整结果，而不是 5 个细碎事件——事件溯源的回放成本随事件数线性增长。
4. **契约显式化**：事件结构带 `version` 字段或走 Schema Registry（[教程 74-Connect与数据生态] §4]）。
...
- **把 Kafka 当数据库用**：按 key 点查/范围查请用真正的存储（压实主题的"表形态"是为流消费服务的，不是为在线查询服务的，见 [教程 71-日志存储与高可用复制] §6]）
...
1. **"Kafka 保证消息不丢"**——错。默认配置下生产者 `acks` 语义、Broker 落盘时机（OS page cache）、消费者先提交后处理，三个环节都能丢。可靠性是**配置出来的**，见 [教程 70-投递语义与事务]。
2. **"分区越多越好"**——错。分区是并行度，也是 Leader 选举、再平衡、打开文件句柄、端到端延迟的成本单元。按目标吞吐规划（[教程 72-性能调优与容量规划] §2]）。
3. **"消费组就是队列，一条消息只被一个消费者处理"**——半个错。这是**组内**语义；不同组各自独立消费全量。多租户场景要防止"新组误订阅全量审计流"（ACL 治理见 [教程 75-运维监控与安全] §4]）。
...
4. 容量与分区规划在上线前用 `kafka-producer-perf-test.sh` 实测，不要拍脑袋（[教程 72-性能调优与容量规划] §6]）。
...
Kafka 的心智模型是**提交日志**，不是队列：分区是并行与顺序的单元，位移是消费者的游标，ISR 是可靠性的地基，回放是它区别于一切传统 MQ 的灵魂属性。对 Agent 架构师而言，Kafka 是事件驱动架构的物理载体——[附录 06-企业级架构模式/02-事件驱动Agent架构] 讲的 Event Sourcing/Saga/CQRS 落到生产，最终都要回答"事件怎么可靠地流动与留存"，本主题 10 篇就是对这个问题的完整回答。下一篇进入第一条主干：[教程 68-生产者机制与调优]。
