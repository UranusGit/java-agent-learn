...
>
> **前置阅读**：[02-ChatClient 与对话模型](02-ChatClient与对话模型.md)、[24-容错与弹性设计](24-容错与弹性设计.md)。
...
public class MultiModelConfig {
    ...
    public ChatClient largeChatClient(AnthropicChatModel anthropicChatModel) {
    ...
    public ChatClient fallbackChatClient(AnthropicChatModel anthropicChatModel) {
        return ChatClient.builder(anthropicChatModel)
...
public class ModelRouterService {
    ...
    public String chat(String userInput) {
        // 1. 分析复杂度
        TaskComplexity complexity = complexityAnalyzer.analyze(userInput);

        // 2. 选择对应层级的模型
        ChatClient primaryClient = selectPrimaryClient(complexity);

        // 3. 执行调用（带健康检查和降级）
        return callWithFallback(primaryClient, userInput);
    }

    private ChatClient selectPrimaryClient(TaskComplexity complexity) {
        return switch (complexity) {
...
public class GracefulDegradationChain {
    ...
    public GracefulDegradationChain(
        ...
        this.chain = List.of(
            new ModelNode("primary", primary, 10_000),       // 主模型，10 秒超时
            new ModelNode("secondary", secondary, 10_000),   // 备用模型
            new ModelNode("tertiary", tertiary, 5_000),      // 降级模型
...
public class ModelHealthChecker {
    ...
    @Scheduled(fixedRate = 30_000)  // 每 30 秒健康检查一次
    public void checkHealth() {
        for (String model : List.of("primary", "secondary", "tertiary")) {
            boolean healthy = pingModel(model);
...
management:
  endpoints:
    web:
      exposure:
        include: health,info,metrics,routing
  endpoint:
    health:
      show-details: always
```
...
**下一篇**：[28-部署与运维](28-部署与运维.md) — Docker 化、K8s 部署、环境隔离、健康检查、日志收集。

---

> **想深入？→ [教程 24-容错与弹性设计](24-容错与弹性设计.md)**：模型路由的底层就是容错——降级链是容错策略的具体应用。
> **遇到阻塞？→ [教程 29-上下文工程]**：不同模型的上下文窗口大小不同，路由时需要考虑 Token 预算。
