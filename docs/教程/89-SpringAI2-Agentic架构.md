...
> **定位**：本文回答一个全体系悬而未决的问题——**什么时候该用 Spring AI 2.0 原生的 agentic/编排能力，什么时候手写**。对照手写编排（教程 07/08/36）与框架原生抽象，给出选型决策框架。读者画像：读完编排相关章节、准备动手做架构选型的开发者。前置阅读：[教程 79-Agent工作流编排]、[教程 28-流式工具调用与事件协议]。
>
> **为什么现在讲**：本体系教程 00-09、19-32、57-88 的编排、Agent 循环、记忆、评估全部手写实现——这是学习路径的正确选择（理解原理），但生产选型时必须知道框架提供了什么、边界在哪，避免"手写一切"的惯性。
...
flowchart TB
    ...
    B -- 否 --> F["框架原生编排"]
    B -- 是 --> C{"拦截点在工具执行层?"}
    C -- 是 --> F2["框架编排 + ToolCallingManager 装饰<br/>(教程 61/19 姿势)"]
...
| 能力域 | 框架提供 | 本体系对应手写篇 |
|--------|---------|-----------------|
| Agent 生命周期 | Agent 抽象（创建/运行/终止/状态） | 教程 21-Agent状态管理 |
| 工作流编排 | Workflow/DAG 原语（步骤/分支/并行） | 教程 79-Agent工作流编排 |
| 记忆读写分离 | ChatMemory reader/writer 组合形态 | 教程 04/34 |
| 模型路由 | ChatModel 多实例 + 路由 | 教程 65-模型路由与降级 |
| 评估钩子 | 评估集成点 | 教程 80-自我反思与Agent评估 |
| 可观测 | 原生 Observation（gen_ai semconv） | 教程 31/23 |
...
flowchart TB
    subgraph OWN["自研层（控制点）"]
        E1["事件协议/Agentic UI 管道<br/>(教程 27/19)"]
...
1. **先包一层**：手写编排外露的接口先稳定下来（[教程 28-流式工具调用与事件协议] 的事件协议就是这样的稳定契约）
2. **替换内部实现**：手写 DAG 执行器 → 框架 Workflow 原语（接口不变，行为可对照测试）
3. **对照验证**：同一评估集（[教程 80-自我反思与Agent评估]）跑迁移前后，行为等价才切流
