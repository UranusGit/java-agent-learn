...
> **定位**：本文回答一个全体系悬而未决的问题——**什么时候该用 Spring AI 2.0 原生的 agentic/编排能力，什么时候手写**。对照手写编排（教程 07/08/36）与框架原生抽象，给出选型决策框架。读者画像：读完编排相关章节、准备动手做架构选型的开发者。前置阅读：[教程 36-Agent工作流编排]、[教程 19-流式工具调用与事件协议]。
>
...
本体系已手写过：ReAct 循环（教程 07）、Plan-and-Execute（教程 08）、多 Agent 协作（教程 09）、DAG 编排（教程 36）、Checkpoint 长任务（教程 40）。手写的价值是**完全控制**；代价是**重复造轮子 + 自担边界情况**（死循环防护、预算控制、恢复语义都要自己写对）。
...
flowchart TB
    ...
    A -- 非标准(研究型) --> M["手写循环<br/>(教程 07/08 姿势)"]
    B -- 否 --> F["框架原生编排"]
    B -- 是 --> C{"拦截点在工具执行层?"}
    C -- 是 --> F2["框架编排 + ToolCallingManager 装饰<br/>(教程 28/19 姿势)"]
...
| 能力域 | 框架提供 | 本体系对应手写篇 |
|--------|---------|-----------------|
| Agent 生命周期 | Agent 抽象（创建/运行/终止/状态） | 教程 12-Agent状态管理 |
| 工作流编排 | Workflow/DAG 原语（步骤/分支/并行） | 教程 36-Agent工作流编排 |
| 记忆读写分离 | ChatMemory reader/writer 组合形态 | 教程 04/34 |
| 模型路由 | ChatModel 多实例 + 路由 | 教程 32-模型路由与降级 |
| 评估钩子 | 评估集成点 | 教程 37-自我反思与Agent评估 |
| 可观测 | 原生 Observation（gen_ai semconv） | 教程 22/23 |
...
| 4 | 生命周期边界可控（可终止/可恢复） | 写一个 kill -9 恢复用例 | 框架不管恢复→恢复语义必须自研（对照教程 40） |
| 5 | Observation 埋点齐全 | 启动后查 Span 是否覆盖编排层 | 盲区→在自研装饰层补观测而非等框架 |

这张清单的产出物是**选型决策记录**（半页 ADR：上下文/备选/实测结论/取舍），归档方式见「想深入？→ [附录 10-架构决策与取舍]」。
...
选型别靠争论，靠**两天内可跑完的同构对照**：同一个任务（天气问答+一次工具调用）分别用手写循环（教程 07 姿势）与 ChatClient 声明式骨架实现，对照三件事——代码量、拦截点位置、可测试性。以下是框架侧骨架（API 均为已实证基准）：
...
var agent = ChatClient.builder(chatModel)                    // 已实证
    ...
    .defaultTools(new WeatherTools())                        // @Tool 方法集（教程 03）
    .defaultAdvisors(
...
| HITL 拦截点 | 循环内任意处（最灵活） | ToolCallingManager 装饰层（教程 28 落点） |
...
flowchart TB
    subgraph OWN["自研层（控制点）"]
        E1["事件协议/Agentic UI 管道<br/>(教程 18/19)"]
        E2["HITL 审批装饰器<br/>(教程 28)"]
        E3["治理: 配额/审计/脱敏"]
    end
...
**原则：治理与契约自研，编排与集成用框架**——治理（审计/审批/脱敏）是企业的差异化控制点，必须握在手里；编排原语（步骤/分支）是无差异的基础设施，框架维护比你维护可靠。这与 [教程 20 §管控分离] 的分层思想一脉相承：控制面自建、通用执行能力采用标准化组件。

## 5. 迁移路径（从手写到框架）

已有手写系统的迁移不是重写，是**逐层替换**：

1. **先包一层**：手写编排外露的接口先稳定下来（[教程 19-流式工具调用与事件协议] 的事件协议就是这样的稳定契约）
2. **替换内部实现**：手写 DAG 执行器 → 框架 Workflow 原语（接口不变，行为可对照测试）
3. **对照验证**：同一评估集（[教程 37-自我反思与Agent评估]）跑迁移前后，行为等价才切流
4. **保留退出能力**：接口稳定意味着随时可退回手写——框架不是终点是选择

## 5.5 迁移决策记录模板（可复制）

迁移最大的风险不是技术，是**半年后没人记得为什么迁/为什么不迁**。每次选型与迁移用这个模板留档（放入项目 ADR，见「想深入？→ [附录 10-架构决策与取舍]」）：
