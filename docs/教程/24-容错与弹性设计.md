...
spring:
  ai:
    openai:
      chat:
        options:
          # 不幸的是，HTTP 超时需要通过 RestClient 配置
          model: gpt-4o
    retry:
      max-attempts: 3              # 最大重试次数
      backoff:
        initial-interval: 1000     # 初始退避 1 秒
        multiplier: 2.0            # 退避倍数
        max-interval: 10000        # 最大退避 10 秒
...
public class LlmService {
     ...
     * 自定义瞬时异常
     */
    public static class TransientAiException extends RuntimeException {
...
public class InstrumentedLlmService {
    ...
    public InstrumentedLlmService(MeterRegistry registry) {
...
**下一篇**：[25-安全与权限控制](25-安全与权限控制.md) — Agent 安全三道防线、Prompt 注入防御、工具权限分级。

---

> **想深入？→ [教程 27-模型路由与降级]**：多模型管理和自动降级链的设计。
> **遇到阻塞？→ [教程 28-部署与运维]**：健康检查端点、日志收集在生产环境的实践。
