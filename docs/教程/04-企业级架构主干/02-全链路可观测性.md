...
>
> **读者画像**：已经完成 Agent 系统的微服务拆分（[21-微服务拆分与 Agent 部署](01-微服务拆分与Agent部署.md)），需要解决跨服务追踪、性能瓶颈定位、成本监控、异常排查等问题。
>
> **前置阅读**：[21-微服务拆分与 Agent 部署](01-微服务拆分与Agent部署.md)。
...
> **上一篇**：[21-微服务拆分与 Agent 部署](01-微服务拆分与Agent部署.md) — 微服务拆分后，跨服务 Trace 传播依赖本文的 OpenTelemetry 集成。
>
> **下一篇**：[32-工具执行可观测与审计](03-工具执行可观测与审计.md) — 本文的 spring.ai.tool Span 属性详解，深入工具调用的审计需求。
>
> **想深入**：[29-管控分离架构](00-管控分离架构.md) — 控制面的可观测中心如何消费本文产生的 Metrics 和 Trace 数据。
>
> **机制层下钻**：[教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台]（6 篇）— 本文的需求侧到机制层的完整下钻：Observation 门面六件套、核心 API、Boot 自动装配、自定义 Handler/Convention/Filter、Trace 跨 HTTP/Reactor/Kafka 传播、Agent 全链路实战（TTFT/成本/质量/审计三管道）。
