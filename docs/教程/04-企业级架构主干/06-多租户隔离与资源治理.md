...
>
> **前置阅读**：[04-记忆与会话管理](../00-基础与核心/04-记忆与会话管理.md)、[58-历史记录持久化与合规](05-历史记录持久化与合规.md)。
 ...
 * 必须在 MessageChatMemoryAdvisor 之前执行，防止跨租户读取历史。
 */
public class TenantIsolationAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    @Override
    public int getOrder() {
        // 在 MessageChatMemoryAdvisor 之前执行：
        // 其默认顺序为 Advisor.DEFAULT_CHAT_MEMORY_PRECEDENCE_ORDER（≈ HIGHEST_PRECEDENCE + 200）
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request,
        ...
        String tenantId = (String) request.context()
                .get("tenantId");
...
public class TenantGuardVectorStore implements VectorStore {
    ...
    public List<Document> similaritySearch(SearchRequest request) {
        ...
        // 合并用 Filter.Expression 结构拼接（SearchRequest.from 复制原请求，2.0 无 Builder.from()）。
        SearchRequest guardedRequest = request.hasFilterExpression()
                ? SearchRequest.from(request)
                        .filterExpression(new Filter.Expression(
                                Filter.ExpressionType.AND,
                                request.getFilterExpression(),
                                new FilterExpressionTextParser().parse(tenantFilter)))
                        .build()
                : SearchRequest.from(request)
...
-- 应用层在同一事务内设置租户上下文
-- ⚠ 修正: 必须用 SET LOCAL（事务级）——连接级 SET 在连接池下
--   "设完即归还" 对池中真实业务连接无效，RLS 形同虚设（详见项目07 v2 的检讨）
BEGIN;
...
```java
// 正确姿势: 事务内 SET LOCAL（随事务结束自动失效，无连接污染）
// 提示: 事务级绑定 + 连接池的完整方案见 [项目 07 §v2 RLS 事务级绑定]
@Component
public class TenantRlsBinder {
    ...
    /**
     * 在事务边界上绑定租户上下文（配合事务管理器统一收口）。
     * 生产实现: 在 TransactionSynchronization 或 AOP 事务切面中，
     * 于同一连接同一事务内执行 SET LOCAL，业务代码无感。
     */
    public void bindTenant(Connection conn, String tenantId) {
        try (Statement stmt = conn.createStatement()) {
            stmt.execute("SET LOCAL app.current_tenant_id = '" + tenantId + "'");  // 事务级
        } catch (SQLException e) {
            throw new RuntimeException("Failed to bind tenant context", e);
        }
    }
}
```
...
public class TenantStorageQuotaService {
    ...
    private final TenantConfigRepository tenantConfigRepo;
    private final VectorDocumentMetadataRepository documentMetadataRepo;  // 自研：向量文档元数据统计

    public TenantStorageQuotaService(ConversationMetadataRepository metadataRepo,
                                     TenantConfigRepository tenantConfigRepo,
                                     VectorDocumentMetadataRepository documentMetadataRepo) {
    ...
    public boolean canIngestDocuments(String tenantId, int documentCount) {
        // 概念代码：VectorStore 接口无 count 方法（真实 API 仅 add/delete/similaritySearch），
        // 租户文档数由自研元数据统计仓库提供
        long currentDocs = documentMetadataRepo.countByTenantId(tenantId);
...
public class ToolPermissionAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request,
        ...
        // 工具定义挂在 Prompt 的 ToolCallingChatOptions 上（2.0.0 的 ChatClientRequest 只有
        // prompt()/context() 两个成员），按租户过滤后 mutate 重建请求 // Spring AI 2.0.0
        ToolCallingChatOptions options = (ToolCallingChatOptions) request.prompt().getOptions();
        List<ToolCallback> allowed = options.getToolCallbacks().stream()
                .filter(tool -> allowedTools.contains(tool.getToolDefinition().name()))
                .collect(Collectors.toList());
        ChatClientRequest filteredRequest = request.mutate()
                .prompt(new Prompt(request.prompt().getInstructions(),
                        options.mutate().toolCallbacks(allowed).build()))
                .build();

        return chain.nextCall(filteredRequest);
    }
}
```
...
这张配置表是**每请求都要读的热数据**——隔离 Advisor 每次调用都要查租户的并发上限、限额与模型权限。租户规模上来后直查配置库会成为瓶颈：它的读取路径需要 L1 进程内 + L2 Redis 双层缓存与事件驱动失效保护（含热点租户配置过期瞬间的击穿对策），完整方案见 [附录 09-语义缓存与性能/03-多级缓存架构 §3-§5]。
...
class TenantIsolationTest {
    ...
    // 概念代码：TenantContext.runAs 为测试辅助类，在测试线程内注入租户上下文后执行回调
        ...
        TenantContext.runAs("tenant-free", () -> {
            assertThatExceptionOfType(ToolNotAllowedException.class)
                    .isThrownBy(() -> chatClient.prompt()
                            .user("帮我转账 1000 元")
                            .tools(fundTransferTool)  // @Tool 注解方法用 tools()；ToolCallback 实例用 toolCallbacks()，两者 2.0 并存
                            .call());
        });
    }
}
...
**上一篇**：[58-历史记录持久化与合规](05-历史记录持久化与合规.md) — 数据生命周期管理为多租户隔离提供了归档和合规基础。

**下一篇**：[60-成本治理与Token计量](07-成本治理与Token计量.md) — 多租户场景下的 Token 配额需要精细化的成本计量体系。

**相关阅读**：
- [04-记忆与会话管理](../00-基础与核心/04-记忆与会话管理.md) — ChatMemory 的基本概念，本篇在此基础上增加了租户隔离层。
- [61-Human-in-the-Loop与审批流](08-Human-in-the-Loop与审批流.md) — Level 3 危险工具的审批流程。
- [62-灰度发布与版本管理](09-灰度发布与版本管理.md) — 按租户灰度发布新 Prompt 和模型。
