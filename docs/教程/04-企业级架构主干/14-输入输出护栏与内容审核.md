...
> **想深入？→ [附录 08-Agent安全深度/06-数据分类分级与密级传导 §5.5](../../附录/08-Agent安全深度/06-数据分类分级与密级传导.md)**：闸③ 按内容判"能不能过"，密级传导按数据的密级判"该不该人审"——同一个 ToolCallback 包装层挂载点，业务护栏之外再叠一层按密级强制 HITL 的守门。
