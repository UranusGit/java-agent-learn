...
>
> **前置阅读**：[05-RAG 检索增强生成](../00-基础与核心/05-RAG检索增强生成.md)。
...
> 关于可观测性的完整方案，详见 [31-全链路可观测性](02-全链路可观测性.md)。
...
// 关键 API：ChatClientRequest 是 record（prompt() + context()），无 1.x 的 userText()/chatOptions()；
// 改写请求用 request.mutate()（javap 实证）。import 补充：
...
public class PolicyEnforcementAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request,
        ...
        PolicyContext policyCtx = new PolicyContext(
                (String) requestContext.get("userId"),
                (String) requestContext.get("tenantId"),
                (String) requestContext.get("agentId"),
                request.prompt().getOptions() != null
                        ? request.prompt().getOptions().getModel() : "default",
        ...
        switch (decision.decision()) {
            ...
            case DOWNGRADE -> {
                // 切换到策略引擎指定的降级模型：重建 Prompt 的 options + 追加 context
                ChatClientRequest downgraded = request.mutate()
                        .prompt(Prompt.builder()
                                .messages(request.prompt().getInstructions())
                                .chatOptions(OpenAiChatOptions.builder()
                                        .model(decision.downgradeModel())
                                        .build())
                                .build())
                        .context("originalModel", policyCtx.modelId())
                        .context("downgradeReason", decision.reason())
                        .build();
...
> **下一篇**：[21-微服务拆分与 Agent 部署](01-微服务拆分与Agent部署.md) — Agent 服务、工具服务、LLM 网关如何独立部署，服务间通信与 Spring Cloud 集成。
>
> **想深入**：[31-全链路可观测性](02-全链路可观测性.md) — 控制面可观测中心的数据来源和 Span 全链路追踪。
>
> **想深入**：[32-工具执行可观测与审计](03-工具执行可观测与审计.md) — 工具调用的安全审计需求与控制面策略引擎的关系。
