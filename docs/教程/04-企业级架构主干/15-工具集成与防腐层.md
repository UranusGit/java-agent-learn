...
生产实现两个注意点：`ticket.await` 的阻塞等待同样必须发生在非 EventLoop 线程上（与 §5.1 同一纪律）；审批等待期间 SSE 流式连接早已关闭，所以真实的 HITL 是**异步任务 + 中断恢复**模型——写提案持久化、审批回调后从检查点恢复执行，完整机制见 [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流 §5] 与 [教程 08-架构师进阶/06-长任务持久化与中断恢复 §3]（检查点恢复的执行引擎；恢复后的重试安全见同篇 §4 幂等重试）。
