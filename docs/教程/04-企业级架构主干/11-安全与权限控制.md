...
>
> **前置阅读**：[03-工具调用](../00-基础与核心/03-工具调用.md)、[63-容错与弹性设计](10-容错与弹性设计.md)。
...
public class SafePromptService {
    ...
    public SafePromptService(ChatClient chatClient) {
...
public class SecurityAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        // 在发给 LLM 之前，检查用户输入。
        // 2.0 的 ChatClientRequest 只有 prompt()/context()，没有 userText()——
        // 取用户文本须走 request.prompt().getInstructions() + Message.getText()。
        ...
        if (result.blocked()) {
            // 命中拦截：构造一个"已拦截"的响应直接返回，跳过 LLM 调用
        ...
        return chain.nextCall(request);       // 正常通过：交给链上后续 Advisor / 模型调用
    }
...
import org.springframework.security.config.Customizer;   // ⚠ 需引入依赖 org.springframework.boot:spring-boot-starter-security（本地未下载，未 javap 实证；以引入依赖后 javap 输出为准）
...
```java
import org.springframework.security.core.Authentication;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
import org.springframework.security.core.GrantedAuthority;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
import org.springframework.security.core.authority.SimpleGrantedAuthority;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
import org.springframework.security.core.context.ReactiveSecurityContextHolder;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
import org.springframework.security.oauth2.jwt.Jwt;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
...
public class ToolPermissionChecker {
    ...
    public ToolPermissionChecker(UserContextService userContextService, ApprovalService approvalService) {
...
public class SecuredTools {
    ...
    public SecuredTools(OrderService orderService, PaymentService paymentService,
...
> **想深入？→ [附录 08-Agent安全深度/05-用户身份联邦与企业权限传导 §3-§4]**：上面 `@ToolPermission` 是代码里写死的静态分级；企业级落地时权限往往长在 IdP 侧——用户身份如何从企业 IdP 经 JWT claims → Reactor Context 传到 Agent 会话，企业 RBAC/ABAC 如何在每 turn 装配点对工具做动态过滤，见该篇。
...
// ApprovalRepository / ApprovalRequest / ApprovalStatus 为业务域对象（用户自定义，非 Spring AI API）
@Component
public class ApprovalService {
    ...
    public Mono<Boolean> requestApproval(String userId, String toolName) {
        ...
        // 响应式轮询：Flux.interval 驱动，每 5 秒查一次状态（boundedElastic 执行阻塞查询），
        // 不用 Thread.sleep——事件循环零占用，超时自动 onComplete（take(60)）
        return Flux.interval(Duration.ofSeconds(5))
                .take(60)  // 最多 60 次 = 5 分钟，超时走下游空/false 分支
                .map(i -> approvalRepo.getStatus(request.id()))
                .subscribeOn(Schedulers.boundedElastic())  // 阻塞查询调度到弹性线程池
                .filter(status -> status != ApprovalStatus.PENDING)
                .next()  // 取第一个非 PENDING 终态
                .map(status -> status == ApprovalStatus.APPROVED)
                .defaultIfEmpty(false);  // 超时自动拒绝
    }
}
```
...
public class OutputSecurityAdvisor implements BaseAdvisor {
    ...
    private static final Logger log = LoggerFactory.getLogger(OutputSecurityAdvisor.class);

    private final OutputFilter outputFilter;

    public OutputSecurityAdvisor(OutputFilter outputFilter) {
    ...
    public ChatClientResponse after(ChatClientResponse response, AdvisorChain chain) {
        // 对 LLM 的输出进行过滤（2.0：chatResponse().getResult().getOutput().getText()）
    ...
    @Override
    public int getOrder() {
        return Ordered.LOWEST_PRECEDENCE;  // 最后执行，在输出阶段拦截
    }
}
```
...
import org.springframework.security.core.context.SecurityContextHolder;   // ⚠ 需引入依赖 spring-boot-starter-security（本地未下载，未 javap 实证）
import org.springframework.stereotype.Component;

// 概念代码：AOP 切面是命令式写法。WebFlux 下 SecurityContextHolder（ThreadLocal）不可靠，
// 生产应改用 ReactiveSecurityContextHolder / Reactor Context 取用户身份（见教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台 §7.3）。
@Aspect
@Component
...
**下一篇**：[06-向量数据库选型](../00-基础与核心/06-向量数据库选型.md) — 向量库核心指标、4+ 向量库对比、选型决策框架。

---

> **想深入？→ [附录 08-Agent安全深度/01-ToolPoisoning攻击.md]**：Tool Poisoning 攻击的完整攻防分析。
> **遇到阻塞？→ [教程 04-企业级架构主干/10-容错与弹性设计](10-容错与弹性设计.md)**：安全检查失败也是一种"故障"，需要容错策略配合。
