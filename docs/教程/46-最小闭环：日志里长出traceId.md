...
> **定位**：本文是 `TraceId 全链路追踪` 系列（教程 46-56）的第一关。目标极小极实——**只加两个依赖、不改一行 Java 代码**，让 demo01 的每条日志自动长出 `[traceId,spanId]`，一次 Agent 请求的所有日志可以用同一个 traceId 串起来。后续十关都在这个最小闭环上逐层加码。
>
> **读者画像**：已经跑通过 demo01 的 ChatClient + TimeTool（[教程 00]），见过 span 树，但还没让 traceId 出现在自己的日志里。
>
> **前置阅读**：[教程 00]、[教程 01]（Observation 生命周期）。
...
**与 Observation 系列的关系**（[教程 06] 已铺过一次）：Micrometer Tracing 不是另一套 API，它就是一组特殊的 `ObservationHandler`。你已有的全部 Observation 知识（Registry/Handler/Convention/Filter）原样有效，本系列专注 traceId 这条线打穿：**生成 → 编程 → Baggage → 记录 → 跨服务 → 展示 → 微服务架构 → 工业闭环**。
...
你的 demo01 里**已经存在 Observation**（[教程 00] 埋的点）：ChatClient 调用、ChatModel 调用、Tool 调用、HTTP server 请求。tracing 依赖一加，自动装配把 `TracingObservationHandler` 挂进 `ObservationRegistry`，每个 Observation 的 `start/stop` 就被翻译成 span 的入栈/出栈，当前 span 的 traceId/spanId 同步写入 SLF4J MDC——你的日志就这样"免费"长出了链路号。
...
- **以为 traceId 是 Spring AI 的功能**：它是 Micrometer Tracing 的；Spring AI 只负责产生 Observation（[教程 20] 的七个观测点），tracing 把观测变链路。
- **在非请求线程（如自建线程池）里期待 traceId**：span 不随线程自动走，WebFlux 场景要开 `Hooks.enableAutomaticContextPropagation()`（[教程 06 §6.3]，05 关跨服务时同样关键）。
...
下一关 [教程 01]：把方括号里那串 16 进制**读透**——traceId/spanId/parentId 的族谱关系、W3C `traceparent` 报文格式、采样位怎么读、span 树怎么在脑中成像。
