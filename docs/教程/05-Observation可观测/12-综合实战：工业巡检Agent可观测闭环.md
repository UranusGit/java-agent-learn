...
>
> **前置阅读**：[教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台]~[11-深度整合：SpringAI2与Observation的完整结合面] 全部（11 关的整合面地图尤其要先看）。
...
- 验收"看得见、查得到、算得清"的完整闭环——同 traceId 贯穿前端时间线/日志/Zipkin/审计/指标五处（12.5 七条用例全命中）；
- 从 demo 到生产的演进推演——V1 单实例全内存 → V2 多实例 → V3 合规留存 → V4 治理闭环，ADR 风格取舍有据；
- 审计与事件流双 SLA 设计——实时看（会过期、可截断）与事后查（不丢、带责任字段）分成两个 Handler，不是一个带开关的；
- HITL 的前置信号准备——工具名+参数+traceId 已备齐，审批信号齐了，落点留给 `ToolCallingManager`/`ToolCallback` 包装层；
- 管控分离的教学样板——观测管线作为 Data Plane（高频执行）与 Control Plane（低频决策）之间的天然数据面。

**❌ 不适用场景**：

- 多实例生产直接上 V1——Sinks/buffer 全进程内，需 Redis Pub/Sub 聚合事件 + MQ 落审计（V2）；
- 合规留存靠内存/日志凑合——事件+trace 需全量入对象存储/时序库，保留期按法规 6 个月~2 年（V3）；
- 把 HITL 塞进 Advisor——铁律：人工审批落点是 `ToolCallingManager`/`ToolCallback` 包装层，不是 Advisor；
- demo 简写直接出门——结构化日志审计、内存档案在故障态第一次翻车就是生产事故，按 12.6 演进路线升级再上量；
- 工具面继续膨胀——本系列工具刻意精简（TimeTool 两方法），观测面才是主角，加工具前先想观测成本。
