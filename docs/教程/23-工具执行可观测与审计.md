...
>
> **读者画像**：已经理解全链路可观测性（[22-全链路可观测性](22-全链路可观测性.md)），需要深入工具调用层面的监控和安全审计。
>
> **前置阅读**：[22-全链路可观测性](22-全链路可观测性.md)、[03-工具调用](03-工具调用.md)。
...
spring:
  ai:
    observation:
      include-prompt-content: true   # 记录 Prompt（用户输入）——键名以附录 12 与所引版本为准
...
public class ToolAuditHandler
    ...
    private String getAttributeValue(Observation.Context context, String key) {
        // ⚠ 修正: Observation.Context#getKeyValue 是虚构 API——Micrometer 的
        // 上下文属性通过 ObservationContext.getLowCardinalityKeyValues() /
        // contextKeyValues() 取（key-value 对遍历），或由自定义 ObservationContext 暴露专用 getter
        return context.getContextKeyValues().entrySet().stream()
...
public class ToolSecurityAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request,
        ...
        // 拦截工具调用，检查安全策略
        // 如果策略引擎拒绝，记录审计日志并短路返回（不调用 chain.nextCall）
        return chain.nextCall(request);  // 策略通过则放行（工具级拦截的实际落点在
                                          // ToolCallingManager 装饰器，见附录 12-02 基准）
    }
}
...
> **上一篇**：[22-全链路可观测性](22-全链路可观测性.md) — 本文是其工具调用层面的深入展开。
>
> **下一篇**：[24-多页面流式响应与会话管理](24-多页面流式响应与会话管理.md) — 流式响应的连接管理和断线重连。
>
> **想深入**：[20-管控分离架构](20-管控分离架构.md) — 控制面的策略引擎和安全策略是工具审计的上游决策者。
