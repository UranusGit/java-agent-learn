...
>
> 前提：你了解 AI 安全的常见攻击面（prompt injection、数据投毒）与红队方法，并理解 Event Sourcing 对审计追溯的价值。
> 预计：2-3 天
...
- 微调数据全人工审核
- 训练 / 推理用不同 prompt 模板
- 定期对模型做 red team（用对抗性输入主动探测模型的注入与偏见漏洞）
...
公平性也应纳入评估闭环：把"不同性别 / 年龄 / 地区的拒绝率差异"写成自动化 eval 指标，随每次模型或 prompt 改动跑回归。
