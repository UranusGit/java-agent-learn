...
> **前置阅读**：建议先通读 [00-Agent核心概念](../00-基础与核心/00-Agent核心概念.md) 至 [87-多模型协作与供应策略](10-多模型协作与供应策略.md)，本文是对这些教程的总结性回顾。
...
|------|--------|---------|---------|---------|
| **架构设计** | God Agent | 不可维护、不可测试 | 高 | [09-多Agent协作](../00-基础与核心/09-多Agent协作.md) |
| **架构设计** | 单模型无降级 | 单点故障 | 极高 | [44-多模型协作](10-多模型协作与供应策略.md) |
| **代码质量** | 硬编码 Prompt | 不可维护、不可迭代 | 中 | [02-ChatClient](../00-基础与核心/02-ChatClient与对话模型.md) |
| **代码质量** | 工具无幂等 | 数据错误、重复执行 | 极高 | [03-工具调用](../00-基础与核心/03-工具调用.md) |
| **运行安全** | 无超时无预算 | 成本爆炸、死循环 | 极高 | [40-长任务持久化](06-长任务持久化与中断恢复.md) |
| **运行安全** | 安全裸奔 | 数据泄露、恶意操作 | 极高 | [31-安全权限](../04-企业级架构主干/11-安全与权限控制.md) |
| **数据管理** | 上下文溢出 | 性能下降、Token 浪费 | 高 | [77-上下文工程](00-上下文工程.md) |
| **数据管理** | 无持久化 | 用户体验差、数据丢失 | 高 | [25-历史持久化](../04-企业级架构主干/05-历史记录持久化与合规.md) |
| **运营治理** | 无评估闭环 | 盲目迭代、无法验证 | 高 | [80-自我反思与Agent评估](03-自我反思与Agent评估.md) |
| **运营治理** | 无 HITL | 危险操作不可逆 | 极高 | [28-HITL](../04-企业级架构主干/08-Human-in-the-Loop与审批流.md) |
| **平台保障** | 无可观测 | 黑盒运行、无法排障 | 高 | [22-全链路可观测](../04-企业级架构主干/02-全链路可观测性.md) |
| **平台保障** | 忽略成本 | 账单失控、毛利被侵蚀 | 高 | [27-成本治理](../04-企业级架构主干/07-成本治理与Token计量.md) |
...
> **对应教程**：[09-多Agent协作](../00-基础与核心/09-多Agent协作.md) — 多 Agent 协作架构、Orchestrator-Worker 模式、Agent 间通信协议。
...
> **对应教程**：[02-ChatClient与对话模型](../00-基础与核心/02-ChatClient与对话模型.md) — ChatClient 的 Prompt 管理、SystemPrompt 模板化、外部化配置。
...
> **对应教程**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md) — 完整的预算控制体系、死循环三层防护、检查点与崩溃恢复。
...
> **对应教程**：[87-多模型协作与供应策略](10-多模型协作与供应策略.md) — 完整的多模型编排、故障切换、降级链设计。
> **补充参考**：[65-模型路由与降级](../04-企业级架构主干/12-模型路由与降级.md) — 模型路由策略与实现细节。
...
> **对应教程**：[03-工具调用](../00-基础与核心/03-工具调用.md) — 工具定义、参数设计、`@Tool` 注解的完整用法。
> **补充参考**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md) — 幂等重试在崩溃恢复中的应用。
...
> **对应教程**：[77-上下文工程](00-上下文工程.md) — 五层拼接策略、Token 预算分配、上下文压缩、KV Cache / Prompt Cache 利用。
> **补充参考**：[82-高级记忆架构](05-高级记忆架构.md) — 分级记忆系统的深度设计。
...
> **对应教程**：[80-自我反思与Agent评估](03-自我反思与Agent评估.md) — Reflection 模式、评估指标体系、LLM-as-Judge、回归测试。
> **补充参考**：[84-数据飞轮与持续改进](07-数据飞轮与持续改进.md) — 数据飞轮驱动的持续优化闭环。
...
> **对应教程**：[64-安全与权限控制](../04-企业级架构主干/11-安全与权限控制.md) — 输入安全（Prompt 注入防护）、执行安全（工具权限控制）、输出安全（内容过滤）、Spring Security 集成。
...
> **对应教程**：[58-历史记录持久化与合规](../04-企业级架构主干/05-历史记录持久化与合规.md) — 会话持久化方案、合规留存、数据脱敏。
> **补充参考**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md) — 检查点机制与崩溃恢复。
...
// 生产 HITL 的工程落点是 ToolCallingManager 装饰器或 ToolCallback 包装层（见 [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]），
...
> **对应教程**：[61-Human-in-the-Loop与审批流](../04-企业级架构主干/08-Human-in-the-Loop与审批流.md) — HITL 完整设计、风险分级、审批状态机、Spring AI Advisor 拦截实现。
...
> **对应教程**：[31-全链路可观测性](../04-企业级架构主干/02-全链路可观测性.md) — Spring AI 2.0 原生可观测性体系、gen_ai 语义约定、OpenTelemetry 集成、Prometheus + Grafana 监控面板。
> **补充参考**：[32-工具执行可观测与审计](../04-企业级架构主干/03-工具执行可观测与审计.md) — 工具调用的可观测性与审计日志。
...
> **对应教程**：[60-成本治理与Token计量](../04-企业级架构主干/07-成本治理与Token计量.md) — Token 计量指标体系、按租户/用户成本归因、预算上限与优雅降级、模型路由降本。
> **补充参考**：[59-多租户隔离与资源治理](../04-企业级架构主干/06-多租户隔离与资源治理.md) — 多租户资源隔离与配额管理。
...
| # | 反模式 | 一句话 | 正确做法 | 对应教程 |
|---|--------|--------|---------|---------|
| 1 | **God Agent** | 一个 Agent 干所有事，不可维护 | 领域分工 + Router 路由 | [09-多Agent协作](../00-基础与核心/09-多Agent协作.md) |
| 2 | **硬编码 Prompt** | Prompt 散落在代码中 | 外部化 + 版本管理 | [02-ChatClient](../00-基础与核心/02-ChatClient与对话模型.md) |
| 3 | **无超时无预算** | Agent 死循环无限烧钱 | 轮次 + Token + 时间三层限制 | [40-长任务持久化](06-长任务持久化与中断恢复.md) |
| 4 | **单模型无降级** | 模型挂了全站挂 | 多供应商冗余 + 自动降级 | [44-多模型协作](10-多模型协作与供应策略.md) |
| 5 | **工具无幂等** | 重复执行导致数据错误 | 幂等键 + 服务端去重 | [03-工具调用](../00-基础与核心/03-工具调用.md) |
| 6 | **上下文溢出** | 历史消息无限增长 | 窗口管理 + 摘要压缩 | [77-上下文工程](00-上下文工程.md) |
| 7 | **无评估闭环** | 改了不知道好坏 | 评估集 + 回归测试 | [80-自我反思与Agent评估](03-自我反思与Agent评估.md) |
| 8 | **安全裸奔** | 无注入防护、无审计 | 三道防线 + 审计日志 | [31-安全权限](../04-企业级架构主干/11-安全与权限控制.md) |
| 9 | **无持久化** | 重启丢失所有会话 | 外部化存储 + 检查点 | [25-历史持久化](../04-企业级架构主干/05-历史记录持久化与合规.md) |
| 10 | **无 HITL** | 危险操作无审批 | 风险分级 + 人工审批 | [28-HITL](../04-企业级架构主干/08-Human-in-the-Loop与审批流.md) |
| 11 | **无可观测** | 黑盒运行、无法排障 | Micrometer + OTel + Grafana | [22-全链路可观测](../04-企业级架构主干/02-全链路可观测性.md) |
| 12 | **忽略成本** | 不追踪 Token 消耗 | 成本归因 + 预算控制 | [27-成本治理](../04-企业级架构主干/07-成本治理与Token计量.md) |

---

## 18. 结语——整个教程体系的回顾

本篇是整个 Agent 教程体系的收官篇。让我们回顾一下从 [00-Agent核心概念](../00-基础与核心/00-Agent核心概念.md) 到这里的完整旅程：
...
1. **Agent = LLM + 工具 + 记忆 + 规划**——这是出发点（[00-Agent核心概念](../00-基础与核心/00-Agent核心概念.md)）。
2. **可靠性**是生产环境的底线——检查点、幂等、预算控制缺一不可（[40-长任务持久化](06-长任务持久化与中断恢复.md)）。
3. **安全性**是不能妥协的红线——注入防护、数据脱敏、审计日志（[31-安全权限](../04-企业级架构主干/11-安全与权限控制.md)）。
4. **可观测性**是运维的基础——Micrometer + OTel 全链路追踪（[22-全链路可观测](../04-企业级架构主干/02-全链路可观测性.md)）。
5. **成本控制**是商业可持续的保障——Token 计量 + 预算 + 模型路由（[27-成本治理](../04-企业级架构主干/07-成本治理与Token计量.md)）。
6. **持续改进**是长期竞争力——数据飞轮、评估闭环、灰度发布（[41-数据飞轮](07-数据飞轮与持续改进.md)）。
...
> **想深入？→ 回顾所有教程**：从 [00-Agent核心概念](../00-基础与核心/00-Agent核心概念.md) 开始，系统性地掌握 Agent 开发的每一个方面。
> **想深入？→ [教程 00-基础与核心/09-多Agent协作](../00-基础与核心/09-多Agent协作.md)**：多 Agent 架构模式，解决 God Agent 反模式。
> **想深入？→ [教程 04-企业级架构主干/11-安全与权限控制](../04-企业级架构主干/11-安全与权限控制.md)**：三道防线的完整实现。
> **想深入？→ [教程 08-架构师进阶/06-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md)**：检查点、幂等、预算控制的完整方案。
> **想深入？→ [教程 08-架构师进阶/10-多模型协作与供应策略](10-多模型协作与供应策略.md)**：多模型冗余与故障切换。
