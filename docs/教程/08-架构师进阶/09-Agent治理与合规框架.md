...
>
> **前置阅读**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md)、[84-数据飞轮与持续改进](07-数据飞轮与持续改进.md)。
...
### 2.4 教程 04-企业级架构主干/02-全链路可观测性 与本篇的分工

本篇负责"治理框架"（风险分级、透明度文档、审计、合规清单）；攻击面防护（Prompt 注入分类、越权、Tool Poisoning）属于安全主线，见 [教程 04-企业级架构主干/11-安全与权限控制]。两者合起来才是完整的"AI 治理"——治理回答"谁批准上线、留什么证据"，安全回答"攻击从哪来、怎么拦"。
...
**落点二：评估集与微调数据。** 评估集不应手工维护，而应设计成"脱敏源数据 + 生成规则"的可重建产物——删除某个用户的数据就是改源数据然后重新生成评估集（[教程 08-架构师进阶/03-自我反思与Agent评估] 的评估集管理按此设计）。微调数据最棘手：删除=重训，成本极高且不可承受，所以治理动作要**前置**——训练前就限定数据用途、取得单独同意、设定保留期，让"事后删除"尽量不发生在微调数据上。
...
「想深入？→ [教程 05-Observation可观测/00-最小闭环：Agent各阶段输出打印到控制台]：Micrometer Observation 如何作为审计层的数据源，把 gen_ai 语义约定的 Span 直接归档为审计事件。」
...
| **安全** | Prompt 注入防护 | Y | [教程 04-企业级架构主干/11-安全与权限控制] |
| **安全** | 内容过滤 | Y | 拦截有害内容 |
| **安全** | 数据脱敏 | Y | 敏感信息不出域 |
...
检查清单回答"这个 Agent 上线前查什么"，但治理的完整生命周期还有两头没有被清单覆盖：**上线的准入登记**与**终局的下线退役**。资产台账（Agent Inventory）把本篇的能力串到同一个 `agentId` 上：登记身份、负责人、密级、风险等级、依赖模型（引用模型注册表，[教程 08-架构师进阶/10-多模型协作与供应策略 §6.2]）、Prompt 版本、工具清单、数据源与备案状态；上线闸门强制"未登记不部署"（CI/CD 查清单 + 网关运行时兜底校验，衔接 §8.1 检查项）；清单外的调用即影子嫌疑——裸调 LLM API 的发现靠出口画像与账单对账，处置按"先分诊数据敏感性、再定收编或下架"决策，收编路径接平台的 Golden Path。下线退役是全流程五阶段：下线评估（清单反查依赖方）→ 弃用通告与流量归零（弃用协议头见 [教程 04-企业级架构主干/18-API版本管理与产品化 §4.2]，归零由观测数据判定）→ 数据处置（按 [教程 04-企业级架构主干/05-历史记录持久化与合规 §6.2] 留存矩阵执行，§4.6 被遗忘权三落点在删除时逐项对账）→ 证据归档 → 台账销账，备案注销义务在终止服务起二十个工作日内办结。登记什么、影子怎么收编、下线怎么一步步执行，全部展开在 **[附录 12-AI治理与合规/05-Agent资产清单与影子治理]**；台账中"备案状态"列的运营流程见 **[附录 12-AI治理与合规/04-算法备案与监管报送运营]**。
...
| **资产台账与下线** | 未登记不部署、影子收编、下线五阶段——展开见附录 12-05 |
| **上线检查清单** | 合规 / 安全 / 透明 / 审计 / 运营 五大类检查项 |

治理不是上线前的"最后一道手续"，而是与 Agent 能力同生共长的运营体系：本篇的审计层与事件响应衔接 [教程 08-架构师进阶/07-数据飞轮与持续改进] 的监控闭环，数据主体权利能力（§4.6）则会在每一次监管问询和用户行权请求中被反复调用。

**上一篇**：[85-响应式错误处理](08-响应式错误处理.md) — WebFlux 错误传播链与降级策略。

**下一篇**：[87-多模型协作与供应策略](10-多模型协作与供应策略.md) — 模型编排、多供应商冗余、API Key 池管理。

---

> **想深入？→ [教程 08-架构师进阶/07-数据飞轮与持续改进]**：治理框架中的持续监控与飞轮结合。
> **想深入？→ [教程 08-架构师进阶/11-Agent架构反模式与避坑指南]**：「安全裸奔」反模式的完整分析。
> **想深入？→ [教程 04-企业级架构主干/11-安全与权限控制]**：Prompt 注入与访问控制的详细防护方案。
> **想深入？→ [附录 08-Agent安全深度]**：注入分类、Tool Poisoning 与数据泄露防护的全量展开。
> **想深入？→ [附录 12-AI治理与合规/05-Agent资产清单与影子治理]**：资产台账的数据模型、上线闸门、影子收编与下线退役五阶段的完整展开（§8.4）。
