# 82-高级记忆架构

> **定位**：讲透 Agent 的高级记忆系统——三层记忆架构（短期上下文窗口 → 长期向量化存储 → 外部 RAG 检索）、语义记忆 vs 情景记忆的分类学、记忆演化（遗忘曲线 / 衰减 / 重要性评分）、记忆隔离与多租户、在 Spring AI 中实现高级记忆的完整架构设计。读完这篇，你的 Agent 将拥有类似人类的"记忆能力"——能记住重要的、遗忘琐碎的、按需回忆相关的。
>
> **读者画像**：已经掌握 [04-基础记忆管理](../00-基础与核心/04-记忆与会话管理.md) 的 ChatMemory + MessageWindowChatMemory，需要设计企业级长期记忆系统的资深架构师。
>
> **前置阅读**：[04-记忆与会话管理](../00-基础与核心/04-记忆与会话管理.md)、[77-上下文工程](00-上下文工程.md)、[05-RAG 检索增强生成](../00-基础与核心/05-RAG检索增强生成.md)。

---

## 1. 基础记忆的天花板

[04 章](../00-基础与核心/04-记忆与会话管理.md)讲的 `MessageWindowChatMemory` 是一个**滑动窗口**——保留最近 N 条消息，超出的直接丢弃。这在简单多轮对话中够用，但生产级 Agent 会遇到四个硬伤。

```mermaid
graph TB
    subgraph 基础记忆瓶颈["滑动窗口的四大瓶颈"]
        P1["无差别淘汰<br/>重要信息和寒暄一起丢"]
        P2["无跨会话记忆<br/>新会话从零开始"]
        P3["无重要性区分<br/>所有消息同等对待"]
        P4["无遗忘机制<br/>窗口满了一刀切"]
    end

    P1 --> S1["分层存储"]
    P2 --> S2["长期记忆"]
    P3 --> S3["重要性评分"]
    P4 --> S4["记忆演化"]

    style 基础记忆瓶颈 fill:#ffcdd2
    style S1 fill:#c8e6c9
    style S2 fill:#c8e6c9
    style S3 fill:#c8e6c9
    style S4 fill:#c8e6c9
```

### 1.1 真实场景的痛点

| 场景 | 滑动窗口的问题 | 用户感受 |
|------|-------------|---------|
| 长期客服（月度对话） | 上周讨论过的方案全忘了 | "Agent 怎么又问一遍？" |
| 个性化助手 | 不记得用户偏好 | "我每次都要重新说一遍" |
| 多轮复杂咨询 | 早期关键信息被淘汰 | "Agent 忘了我最初说的条件" |
| 多会话场景 | 会话间完全隔离 | "在 A 会话里说的，B 会话不知道" |

**根本原因**：滑动窗口把记忆当作**无差别的消息队列**，而人类记忆是**有层次、有权重、有遗忘**的复杂系统。

---

## 2. 三层记忆架构

### 2.1 人类记忆的类比

在设计 Agent 记忆系统之前，先理解人类记忆的三层结构：

```mermaid
graph TB
    subgraph 人类记忆["人类记忆的三层结构"]
        L1["工作记忆（Working Memory）<br/>当前正在处理的信息<br/>容量 ~7 项，持续数秒"]
        L2["短期记忆（Short-term Memory）<br/>最近几小时的对话<br/>容量有限，会遗忘"]
        L3["长期记忆（Long-term Memory）<br/>数月甚至数年的记忆<br/>分为语义记忆和情景记忆"]
    end

    L1 -.->|"反复出现 → 巩固"| L2
    L2 -.->|"重要事件 → 存储"| L3
    L3 -.->|"回忆线索 → 调用"| L1

    style L1 fill:#c8e6c9
    style L2 fill:#fff9c4
    style L3 fill:#ffe0b2
```

### 2.2 Agent 的三层记忆架构

将人类记忆模型映射到 Agent 系统架构：

```mermaid
graph TB
    subgraph Agent三层记忆["Agent 三层记忆架构"]
        L1["第 1 层：短期工作记忆<br/>= LLM 上下文窗口<br/>当前对话 + 最近 N 轮"]
        L2["第 2 层：长期向量化存储<br/>= VectorStore<br/>会话摘要 / 关键事实 / 用户画像"]
        L3["第 3 层：外部知识检索<br/>= RAG / 数据库 / API<br/>海量文档 / 结构化数据"]
    end

    subgraph 数据流["记忆流转"]
        F1["实时对话<br/>→ 第 1 层"]
        F2["定期压缩摘要<br/>→ 第 2 层"]
        F3["按需检索<br/>→ 第 3 层"]
    end

    L1 -.->|"摘要 + 关键事实提取"| L2
    L2 -.->|"相似记忆检索回<br/>第 1 层"| L1
    L3 -.->|"RAG 结果注入<br/>第 1 层"| L1

    style L1 fill:#c8e6c9
    style L2 fill:#fff9c4
    style L3 fill:#ffe0b2
```

### 2.3 三层对比

| 层级 | 存储 | 容量 | 持久性 | 延迟 | 成本 |
|------|------|------|--------|------|------|
| **第 1 层：工作记忆** | LLM 上下文窗口 | 128K-1M Token | 单次请求 | 0（已在内存） | Token 计费 |
| **第 2 层：长期存储** | VectorStore（PgVector / Redis） | 无限 | 永久（或 TTL） | ~50ms（查询） | 存储成本 |
| **第 3 层：外部检索** | RAG / 数据库 / API | 无限 | 永久 | ~200ms-2s | 取决于源 |

### 2.4 各层职责

```mermaid
graph LR
    subgraph 第1层职责["第 1 层：工作记忆"]
        A1["当前对话上下文"]
        A2["最近 N 轮消息"]
        A3["从第 2/3 层调入的<br/>相关记忆和知识"]
    end

    subgraph 第2层职责["第 2 层：长期存储"]
        B1["会话摘要"]
        B2["关键事实三元组"]
        B3["用户画像与偏好"]
    end

    subgraph 第3层职责["第 3 层：外部检索"]
        C1["企业知识库"]
        C2["业务数据库"]
        C3["实时 API"]
    end

    style 第1层职责 fill:#c8e6c9
    style 第2层职责 fill:#fff9c4
    style 第3层职责 fill:#ffe0b2
```

### 2.5 Java 代码：三层记忆架构实现

```java
@Service
public class ThreeTierMemoryService {

    private final ChatMemory shortTermMemory;           // 第 1 层
    private final VectorStore longTermStore;            // 第 2 层
    private final EmbeddingModel embeddingModel;
    private final QuestionAnswerAdvisor externalRag;    // 第 3 层

    /**
     * 写入记忆：对话结束后，将重要信息存入第 2 层
     */
    public void persist(String userId, String conversationId) {
        // ChatMemory.get(String) 单参：窗口大小由 MessageWindowChatMemory 构建时配置
        List<Message> history = shortTermMemory.get(conversationId);

        // 1. 提取关键事实（实体 + 关系 + 事件）
        List<MemoryFragment> fragments = extractKeyFacts(history);

        // 2. 为每个片段打重要性评分
        fragments.forEach(f -> f.setImportance(scoreImportance(f)));

        // 3. 存入第 2 层（向量化 + 元数据）
        List<Document> docs = fragments.stream()
            .map(f -> Document.builder()
                .text(f.content())
                .metadata(Map.of(
                    "userId", userId,
                    "conversationId", conversationId,
                    "type", f.type().name(),
                    "importance", f.importance(),
                    "timestamp", Instant.now().toString()))
                .build())
            .toList();
        longTermStore.add(docs);

        // 4. 清理第 1 层（可选：保留最近 N 条作为继续对话的上下文）
    }

    /**
     * 读取记忆：对话开始时，从第 2 层检索相关长期记忆注入第 1 层
     */
    public String recall(String userId, String userInput) {
        // 1. 从第 2 层检索与当前输入相关的长期记忆
        List<Document> memories = longTermStore.similaritySearch(
            SearchRequest.builder()
                .query(userInput)
                .topK(5)
                .filterExpression("userId == '" + userId + "'")  // 租户隔离
                .similarityThreshold(0.7)
                .build()
        );

        // 2. 组装为上下文摘要
        String memoryContext = memories.stream()
            .map(Document::getText)
            .collect(Collectors.joining("\n"));

        // 3. 返回给第 1 层使用
        return memoryContext;
    }
}
```
> **需在 pom.xml 中添加依赖**（`QuestionAnswerAdvisor` 所在模块）：2.0.0 起模块名从 `spring-ai-advisors-vector-store` 改为 `spring-ai-vector-store-advisor`（老坐标已不存在）——版本走 `spring-ai-bom`，不写 `<version>`：
>
> ```xml
> <dependency>
>     <groupId>org.springframework.ai</groupId>
>     <artifactId>spring-ai-vector-store-advisor</artifactId>
> </dependency>
> ```

---

## 3. 语义记忆 vs 情景记忆

人类长期记忆分为两种类型，Agent 记忆系统也应该区分：

### 3.1 两种记忆类型

```mermaid
graph TB
    subgraph 长期记忆分类["长期记忆的分类"]
        SM["语义记忆（Semantic Memory）<br/>= 知识、事实、规则"]
        EM["情景记忆（Episodic Memory）<br/>= 个人经历、事件"]
    end

    SM --> SM1["'用户张三是 VIP 客户'"]
    SM --> SM2["'公司退款政策是 7 天无理由'"]
    SM --> SM3["'用户偏好简洁回复'"]

    EM --> EM1["'2026-08-10 用户询问了退款流程'"]
    EM --> EM2["'2026-08-12 用户投诉了物流延迟'"]
    EM --> EM3["'上周用户提到他下周要出差'"]

    style SM fill:#e3f2fd
    style EM fill:#fff9c4
```

### 3.2 两者的区别

| 维度 | 语义记忆 | 情景记忆 |
|------|---------|---------|
| **内容** | 事实、知识、偏好 | 事件、经历、交互记录 |
| **时间性** | 无时间戳（"张三是 VIP"） | 有时间戳（"8 月 10 日发生了什么"） |
| **抽象度** | 高（抽象提取） | 低（具体事件） |
| **变化频率** | 低（稳定事实） | 高（持续产生新事件） |
| **检索方式** | 关键词 / 语义匹配 | 时间范围 + 语义匹配 |
| **遗忘曲线** | 缓慢遗忘 | 较快遗忘（除非重要事件） |
| **存储结构** | Key-Value / 知识图谱 | 时序向量库 |

### 3.3 实现：双类型记忆存储

```java
@Service
public class TypedMemoryService {

    private final VectorStore vectorStore;
    private final KeyValueStore profileStore;  // 存语义记忆

    /**
     * 写入语义记忆（用户画像、稳定偏好）
     */
    public void saveSemanticMemory(String userId, String key, String value) {
        // 语义记忆适合用 Key-Value 存储，更新而非追加
        profileStore.put("user:" + userId + ":" + key, value);
    }

    /**
     * 读取语义记忆
     */
    public String loadSemanticMemory(String userId, String key) {
        return profileStore.get("user:" + userId + ":" + key);
    }

    /**
     * 读取用户全部语义记忆（按前缀聚合）
     */
    public String loadAllSemantic(String userId) {
        return profileStore.getAll("user:" + userId + ":");
    }

    /**
     * 写入情景记忆（交互事件）
     */
    public void saveEpisodicMemory(String userId, String event, String conversationId) {
        Document doc = Document.builder()
                .text(event)
                .metadata(Map.of(
                        "userId", userId,
                        "conversationId", conversationId,
                        "type", "EPISODIC",
                        "timestamp", Instant.now().toString(),
                        "importance", scoreImportance(event)))
                .build();
        vectorStore.add(List.of(doc));
    }

    /**
     * 检索情景记忆（支持时间范围 + 语义相似度）
     */
    public List<Document> recallEpisodic(String userId, String query,
                                          Instant from, Instant to) {
        return vectorStore.similaritySearch(
            SearchRequest.builder()
                .query(query)
                .topK(10)
                .filterExpression("""
                    userId == '%s' && type == 'EPISODIC'
                    && timestamp >= '%s' && timestamp <= '%s'
                    """.formatted(userId, from, to))
                .build()
        );
    }
}
```

### 3.4 从对话中自动提取两类记忆

```java
@Service
public class MemoryExtractor {

    private final ChatClient extractor;

    /**
     * 分析一段对话，提取语义记忆和情景记忆
     */
    public ExtractionResult extract(String userId, List<Message> conversation) {
        String transcript = conversation.stream()
            .map(m -> m.getType() + ": " + m.getText())
            .collect(Collectors.joining("\n"));

        String json = extractor.prompt()
            .system("""
                分析以下对话，提取两种记忆：

                1. 语义记忆（稳定事实）：用户画像、偏好、身份信息
                   格式：{"type": "SEMANTIC", "key": "preference.style", "value": "简洁"}

                2. 情景记忆（具体事件）：发生了什么事
                   格式：{"type": "EPISODIC", "event": "用户咨询了退款流程", "importance": 0.7}

                输出 JSON 数组。
                """)
            .user(transcript)
            .call()
            .content();

        return parseExtraction(json);
    }
}
```

```mermaid
graph TB
    CONV["对话历史"] --> EXTRACT["记忆提取器"]
    EXTRACT --> SEM["语义记忆<br/>'用户偏好简洁回复'<br/>'用户是 VIP'"]
    EXTRACT --> EPIS["情景记忆<br/>'8-12 用户投诉了物流'<br/>'8-10 用户问了退款'"]

    SEM --> KV["Key-Value 存储<br/>（更新覆盖）"]
    EPIS --> VS["向量库<br/>（追加 + 时间戳）"]

    style EXTRACT fill:#fff9c4
    style KV fill:#e3f2fd
    style VS fill:#c8e6c9
```

---

## 4. 记忆演化：遗忘、衰减与重要性

人类不会记住所有事情——我们会**遗忘**。这不是缺陷，而是**特性**——遗忘让我们聚焦于重要的信息。Agent 也需要类似机制。

### 4.1 遗忘曲线

```mermaid
graph TB
    subgraph 艾宾浩斯遗忘曲线["艾宾浩斯遗忘曲线（概念示意）"]
        T1["第 1 天<br/>记忆强度 100%"]
        T2["第 2 天<br/>记忆强度 ~35%"]
        T3["第 7 天<br/>记忆强度 ~25%"]
        T4["第 30 天<br/>记忆强度 ~20%"]
        T5["反复回忆<br/>→ 巩固记忆"]
    end

    T1 --> T2 --> T3 --> T4
    T4 -.->|"回忆一次"| T5
    T5 -.->|"强度回升"| T1

    style 艾宾浩斯遗忘曲线 fill:#fff9c4
```

### 4.2 记忆衰减算法

每条记忆有一个**衰减权重**，随时间递减。但**被回忆**（检索命中）会增强权重。

```java
@Service
public class MemoryDecayService {

    private static final double BASE_DECAY_RATE = 0.95;  // 每天衰减 5%
    private static final double RECALL_BOOST = 1.3;      // 每次回忆增强 30%

    /**
     * 计算一条记忆的当前有效权重
     */
    public double calculateEffectiveWeight(MemoryRecord memory) {
        long daysSinceCreation = ChronoUnit.DAYS.between(
            memory.getCreatedAt(), Instant.now());

        long daysSinceLastRecall = ChronoUnit.DAYS.between(
            memory.getLastRecalledAt(), Instant.now());

        // 基础衰减：时间越久越弱
        double timeDecay = Math.pow(BASE_DECAY_RATE, daysSinceCreation);

        // 回忆增强：被回忆过多少次
        double recallMultiplier = Math.pow(RECALL_BOOST, memory.getRecallCount());

        // 重要性加权
        double importance = memory.getImportance();

        // 最终有效权重
        return importance * timeDecay * recallMultiplier;
    }

    /**
     * 定期清理低权重记忆（遗忘机制）
     */
    @Scheduled(cron = "0 0 3 * * *")  // 每天凌晨 3 点
    public void forgetLowWeightMemories() {
        double forgetThreshold = 0.05;  // 低于此权重的记忆被遗忘

        List<MemoryRecord> all = memoryRepo.findAll();
        List<MemoryRecord> toForget = all.stream()
            .filter(m -> calculateEffectiveWeight(m) < forgetThreshold)
            .filter(m -> m.getImportance() < 0.8)  // 高重要性记忆永不遗忘
            .toList();

        memoryRepo.deleteAll(toForget);
        log.info("遗忘清理：删除了 {} 条低权重记忆", toForget.size());
    }
}
```

### 4.3 重要性评分

不同记忆应该有不同的权重。一次普通的"你好"和"用户透露他有严重过敏史"的重要性天差地别。

```mermaid
graph TB
    subgraph 重要性维度["记忆重要性的五大维度"]
        D1["情感强度<br/>用户情绪越强烈越重要"]
        D2["事实关键性<br/>医疗过敏 > 偏好颜色"]
        D3["复用频率<br/>经常被检索的记忆更重要"]
        D4["时效性<br/>近期事件 > 远期事件"]
        D5["用户显式标注<br/>用户说'记住这个'"]
    end

    style 重要性维度 fill:#e8f5e9
```

```java
@Service
public class ImportanceScorer {

    private final ChatClient scorer;

    public double score(String content, String userId) {
        String result = scorer.prompt()
            .system("""
                评估以下信息的记忆重要性，输出 0.0-1.0 的分数。
                考虑维度：
                - 是否涉及用户安全 / 健康（如过敏史 → 1.0）
                - 是否涉及核心业务（如订单 / 支付 → 0.8）
                - 是否是稳定偏好（如语言偏好 → 0.6）
                - 是否是一次性信息（如临时天气 → 0.2）
                - 是否是寒暄 / 无意义内容（如"你好" → 0.05）
                """)
            .user(content)
            .call()
            .content();
        return Double.parseDouble(result.trim());
    }
}
```

| 信息类型 | 重要性 | 遗忘策略 |
|---------|--------|---------|
| 医疗 / 安全信息 | 1.0 | 永不遗忘 |
| 核心业务数据 | 0.8-0.9 | 永不遗忘 |
| 用户稳定偏好 | 0.5-0.7 | 缓慢衰减 |
| 具体事件细节 | 0.3-0.5 | 正常衰减 |
| 寒暄 / 无意义 | 0.05-0.1 | 快速遗忘 |

### 4.4 记忆巩固：从情景到语义

人类在睡眠时会将**情景记忆**巩固为**语义记忆**——反复出现的事件模式被提取为通用知识。Agent 也可以做类似处理：

```mermaid
graph TB
    subgraph 记忆巩固["记忆巩固过程"]
        E1["多次情景记忆<br/>'用户多次要求简洁回复'<br/>'用户再次说不要废话'"]
        S1["巩固为语义记忆<br/>'用户偏好：简洁回复'"]
        E2["原始情景记忆<br/>可被遗忘"]
    end

    E1 --> S1
    S1 --> KV["Key-Value 存储<br/>（长期稳定）"]
    E1 --> E2 --> DEL["衰减清理"]

    style S1 fill:#e3f2fd
    style E2 fill:#ffe0b2
```

```java
@Scheduled(cron = "0 0 4 * * 0")  // 每周日凌晨 4 点执行巩固
public void consolidateMemories() {
    // 1. 找出高频出现的情景记忆模式
    Map<String, Long> patterns = findRecurringEpisodicPatterns();

    // 2. 将高频模式提取为语义记忆
    patterns.entrySet().stream()
        .filter(e -> e.getValue() >= 3)  // 出现 3 次以上的模式
        .forEach(e -> {
            String semanticKey = extractKey(e.getKey());
            String semanticValue = extractValue(e.getKey());
            profileStore.put(semanticKey, semanticValue);
        });

    // 3. 原始情景记忆降低权重（已巩固到语义层）
    // 下次遗忘清理时会自然回收
}
```

---

## 5. 记忆隔离与多租户

### 5.1 多租户记忆隔离

企业级 Agent 服务多个用户 / 组织，记忆必须严格隔离：

```mermaid
graph TB
    subgraph 多租户隔离["多租户记忆隔离"]
        U1["用户 A 的记忆空间"]
        U2["用户 B 的记忆空间"]
        U3["组织 X 的共享知识"]
    end

    subgraph 隔离层["隔离层（Filter）"]
        F1["userId 过滤"]
        F2["orgId 过滤"]
        F3["权限检查"]
    end

    U1 -.->|"仅自己可读"| F1
    U2 -.->|"仅自己可读"| F1
    U3 -.->|"组织成员可读"| F2

    style 多租户隔离 fill:#e8f5e9
```

### 5.2 隔离实现

```java
@Service
public class MultiTenantMemoryService {

    /**
     * 写入记忆时强制带 userId
     */
    public void save(String userId, String orgId, MemoryRecord memory) {
        memory.setUserId(userId);
        memory.setOrgId(orgId);
        memory.setCreatedAt(Instant.now());
        memoryStore.save(memory);
    }

    /**
     * 读取时强制过滤——绝不跨租户
     */
    public List<MemoryRecord> recall(String userId, String orgId, String query) {
        return vectorStore.similaritySearch(
            SearchRequest.builder()
                .query(query)
                .topK(5)
                .filterExpression("""
                    userId == '%s' || (orgId == '%s' && scope == 'SHARED')
                    """.formatted(userId, orgId))
                .build()
        );
    }
}
```

### 5.3 记忆权限模型

```mermaid
graph TB
    subgraph 权限层级["记忆的三个权限层级"]
        P1["PRIVATE：仅本人可读<br/>个人偏好 / 隐私信息"]
        P2["TEAM：团队可读<br/>项目讨论 / 团队决策"]
        P3["ORG：全组织可读<br/>公司政策 / 公共知识"]
    end

    P1 --> R1["filter: userId == 'xxx'"]
    P2 --> R2["filter: orgId == 'xxx' && scope == 'TEAM'"]
    P3 --> R3["filter: scope == 'ORG'"]

    style P1 fill:#ffcdd2
    style P2 fill:#fff9c4
    style P3 fill:#c8e6c9
```

> **安全警告**：多租户记忆隔离是安全底线。**任何记忆检索都必须带 userId/orgId 过滤条件**，绝不能做全库搜索。建议在代码审查时检查每一条 `similaritySearch` 是否包含 filter。

---

## 6. 完整架构：Spring AI 中的高级记忆

### 6.1 整体架构图

```mermaid
graph TB
    subgraph 完整架构["高级记忆系统完整架构"]
        REQ["用户请求<br/>userId + message"] --> CACHE{"语义缓存命中？"}

        CACHE -->|"是"| RESP["直接返回"]
        CACHE -->|"否"| RECALL["记忆召回<br/>从第 2 层检索相关记忆"]

        RECALL --> CTX["上下文组装<br/>第 1 层 + 召回记忆 + RAG"]
        CTX --> LLM["LLM 推理"]
        LLM --> RESP

        RESP --> EXTRACT["后台：记忆提取<br/>语义 + 情景"]
        EXTRACT --> STORE["写入第 2 层<br/>带重要性评分"]

        STORE --> DECAY["定期：记忆演化<br/>衰减 + 巩固 + 遗忘"]
    end

    subgraph 存储层["存储层"]
        ST1["Redis<br/>语义记忆 KV"]
        ST2["PgVector<br/>情景记忆向量"]
        ST3["知识库<br/>第 3 层 RAG"]
    end

    RECALL -.-> ST1
    RECALL -.-> ST2
    CTX -.-> ST3
    STORE -.-> ST1
    STORE -.-> ST2

    style CACHE fill:#c8e6c9
    style EXTRACT fill:#fff9c4
    style DECAY fill:#ffe0b2
```

### 6.2 核心 Service 整合

```java
@Service
public class AdvancedMemoryAgent {

    private final ThreeTierMemoryService memory;
    private final TypedMemoryService typedMemory;
    private final MemoryExtractor extractor;
    private final MemoryDecayService decay;
    private final ImportanceScorer scorer;
    private final ChatClient agent;

    /**
     * 完整的带记忆 Agent 请求处理
     */
    public String chat(String userId, String userInput) {
        // 1. 召回长期记忆（第 2 层 → 第 1 层）
        List<Document> episodicDocs = typedMemory.recallEpisodic(
            userId, userInput, Instant.now().minus(Duration.ofDays(30)), Instant.now());
        String episodicContext = episodicDocs.stream()
            .map(Document::getText)
            .collect(Collectors.joining("\n"));

        String semanticContext = typedMemory.loadAllSemantic(userId);

        // 2. 组装增强 Prompt
        String enhancedPrompt = """
            [用户画像]
            %s

            [相关历史记忆]
            %s

            [当前问题]
            %s
            """.formatted(semanticContext, episodicContext, userInput);

        // 3. LLM 推理
        String answer = agent.prompt()
            .system(buildSystemPrompt(userId))
            .user(enhancedPrompt)
            .call()
            .content();

        // 4. 异步提取并存储记忆（不阻塞响应；conversationId 由调用方传入更佳）
        CompletableFuture.runAsync(() -> {
            List<Message> conversation = List.of(
                new UserMessage(userInput),
                new AssistantMessage(answer)
            );
            ExtractionResult extracted = extractor.extract(userId, conversation);
            extracted.getAll().forEach(record -> {
                double importance = scorer.score(record.getContent(), userId);
                if ("SEMANTIC".equals(record.getType())) {
                    typedMemory.saveSemanticMemory(userId, record.getKey(), record.getContent());
                } else {
                    typedMemory.saveEpisodicMemory(userId, record.getContent(), userId);
                }
            });
        });

        return answer;
    }
}
```

### 6.3 记忆摘要 Advisor

将三层记忆封装为 Spring AI Advisor，对业务代码透明：

```java
import java.time.Duration;
import java.time.Instant;
import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.CompletableFuture;
import java.util.stream.Collectors;

import org.springframework.ai.chat.client.ChatClient;
import org.springframework.ai.chat.client.ChatClientRequest;
import org.springframework.ai.chat.client.ChatClientResponse;
import org.springframework.ai.chat.client.advisor.api.CallAdvisor;
import org.springframework.ai.chat.client.advisor.api.CallAdvisorChain;
import org.springframework.ai.chat.messages.Message;
import org.springframework.ai.chat.messages.SystemMessage;
import org.springframework.ai.chat.prompt.Prompt;

public class AdvancedMemoryAdvisor implements CallAdvisor {

    private final TypedMemoryService typedMemory;
    private final String userId;
    private final String conversationId;

    @Override
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        // 请求前：从第 2 层召回相关记忆
        String userInput = request.prompt().getUserMessages().stream()
            .map(Message::getText)
            .collect(Collectors.joining("\n"));

        String recalled = typedMemory.recallEpisodic(
            userId, userInput,
            Instant.now().minus(Duration.ofDays(30)), Instant.now());

        // 将记忆作为 system 补充消息注入（改写请求）
        Prompt original = request.prompt();
        List<Message> messages = new ArrayList<>(original.getInstructions());
        if (!recalled.isEmpty()) {
            messages.add(new SystemMessage("[相关历史记忆]\n" + recalled));
        }
        ChatClientRequest effective = request.mutate()
            .prompt(new Prompt(messages, original.getOptions()))
            .build();

        ChatClientResponse response = chain.nextCall(effective);

        // 请求后：异步把本轮问答写入第 2 层情景记忆
        String answer = response.chatResponse().getResult().getOutput().getText();
        CompletableFuture.runAsync(() ->
            typedMemory.saveEpisodicMemory(userId, userInput + " → " + answer, conversationId));

        return response;
    }
}

// 使用
ChatClient client = ChatClient.builder(chatModel)
    .defaultAdvisors(new AdvancedMemoryAdvisor(typedMemory, currentUserId, conversationId))
    .build();
```

---

## 7. 记忆系统的运维

### 7.1 记忆生命周期管理

```mermaid
graph LR
    subgraph 记忆生命周期["一条记忆的完整生命周期"]
        C1["创建<br/>从对话中提取"]
        C2["存储<br/>向量化 + 元数据"]
        C3["检索<br/>被相关 Query 命中"]
        C4["巩固<br/>高频记忆升级为语义记忆"]
        C5["衰减<br/>权重随时间递减"]
        C6["遗忘<br/>低于阈值时清理"]
    end

    C1 --> C2 --> C3
    C3 -.->|"反复命中"| C4
    C2 --> C5 --> C6
    C4 -.->|"巩固后<br/>原始可遗忘"| C5

    style C4 fill:#e3f2fd
    style C5 fill:#fff9c4
    style C6 fill:#ffe0b2
```

### 7.2 合规与隐私

| 合规要求 | 实现方式 |
|---------|---------|
| 用户要求删除所有记忆 | 按 userId 级联删除（GDPR 被遗忘权） |
| 敏感信息不存储 | 在提取阶段做 PII 检测和脱敏 |
| 记忆有 TTL | 设置过期时间，自动清理 |
| 审计追溯 | 记录每条记忆的来源（conversationId + timestamp） |
| 数据本地化 | 记忆存储按地区分库 |

```java
/**
 * GDPR 被遗忘权：删除用户全部记忆
 */
public void forgetUser(String userId) {
    // 删除语义记忆
    profileStore.deleteByPrefix("user:" + userId + ":");

    // 删除情景记忆
    vectorStore.delete(
        new FilterExpressionBuilder().eq("userId", userId).build()
    );

    // 删除会话历史
    chatMemory.clear(userId);

    // 记录审计日志
    auditLog.record("USER_FORGOTTEN", userId, Instant.now());
}
```

---

## 8. 适用场景与不适用场景

### ✅ 适用场景

- 长期陪伴型 Agent（个人助理、心理咨询、学习辅导）
- 个性化推荐型 Agent（需要记住用户偏好）
- 多会话连续服务（跨会话保持上下文）
- 企业级客服系统（记住客户历史交互）
- 需要记忆演化和遗忘的真实智能系统

### ❌ 不适用场景

- 一次性问答（不需要长期记忆）
- 强无状态 API（RESTful 公开服务）
- 数据合规不允许存储用户数据的场景
- 极简原型阶段（先用 MessageWindowChatMemory 跑通）
- 对话轮数极少的场景（短期记忆足够）

---

## 9. 本章总结

| 概念 | 一句话 |
|------|--------|
| **三层记忆架构** | 工作记忆（上下文窗口）→ 长期存储（VectorStore）→ 外部检索（RAG） |
| **语义记忆** | 稳定事实 / 偏好 / 知识，用 Key-Value 存储 |
| **情景记忆** | 具体事件 / 交互记录，用向量库 + 时间戳存储 |
| **记忆衰减** | 权重随时间递减，被回忆时增强 |
| **重要性评分** | 不同信息有不同权重，高重要性永不遗忘 |
| **记忆巩固** | 高频情景记忆巩固为语义记忆 |
| **多租户隔离** | 每次检索强制带 userId/orgId 过滤 |
| **遗忘机制** | 定期清理低权重记忆，聚焦重要信息 |

**进阶篇（34-39）小结**：
- [77-上下文工程](00-上下文工程.md) — 上下文窗口的 RAM 管理
- [35-高级 RAG 与 Agentic RAG](01-高级RAG与AgenticRAG.md) — 从塞文档到自主检索
- [36-Agent 工作流编排](02-Agent工作流编排.md) — DAG / 状态机 / 条件分支
- [37-自我反思与 Agent 评估](03-自我反思与Agent评估.md) — 自评迭代 + 质量度量
- [38-Agent 性能优化](04-Agent性能优化.md) — 延迟 / 成本 / 并发优化
- **82-高级记忆架构** — 三层记忆 + 演化 + 遗忘

**下一篇**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md) — 进阶 II 篇开启：Checkpoint、断点恢复、预算控制。

---

> **前置回顾**：[04-记忆与会话管理](../00-基础与核心/04-记忆与会话管理.md)介绍了 MessageWindowChatMemory——本章是它的"企业级终极进化版"。
> **上下文协同**：记忆的召回结果需要注入上下文窗口，拼接策略详见 [77-上下文工程](00-上下文工程.md)。
> **性能考量**：记忆检索的延迟优化（语义缓存 / 并行检索），详见 [38-Agent 性能优化](04-Agent性能优化.md)。
> **RAG 对比**：第 3 层外部检索与 [05-RAG 检索增强生成](../00-基础与核心/05-RAG检索增强生成.md) 和 [35-高级 RAG](01-高级RAG与AgenticRAG.md) 的技术栈一致，区别在于记忆是"个人化的 RAG"。
