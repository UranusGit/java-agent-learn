...
>
> **前置阅读**：[83-长任务持久化与中断恢复](06-长任务持久化与中断恢复.md)、[05-RAG检索增强生成](../00-基础与核心/05-RAG检索增强生成.md)。
...
public class InteractionLogger {
    ...
    /** 更新显式反馈（点赞/点踩/评分） */
    public void updateFeedback(String logId, UserFeedback feedback) {
        repository.updateFeedback(logId, feedback);
    }

    /** 补充文字反馈 */
    public void updateComment(String logId, String comment) {
        repository.updateComment(logId, comment);
    }
...
/**
 * 交互日志仓库（示意）——生产可换 Spring Data JPA / MongoRepository 实现
 */
interface InteractionLogRepository {
    void save(InteractionLogger.InteractionRecord record);
    void updateFeedback(String logId, InteractionLogger.UserFeedback feedback);
    void updateComment(String logId, String comment);
    Optional<InteractionLogger.InteractionRecord> findNextInteraction(String sessionId, Instant timestamp);
    List<InteractionLogger.InteractionRecord> findByTimestampBetween(Instant from, Instant to);
    List<InteractionLogger.InteractionRecord> findByVersionAndTimestampAfter(String version, Instant since);
    List<InteractionLogger.InteractionRecord> findByExperimentAndGroup(String experimentId, String group);
}
```
...
public class ImplicitFeedbackDetector {
    ...
    public ImplicitFeedbackDetector(InteractionLogRepository repository,
    ...
    /** 简单相似度判断（示意）——生产可用 Embedding 余弦相似度 */
    private boolean isSimilarQuestion(String a, String b) {
        return a.length() > 5 && b.length() > 5 && a.contains(b.substring(0, Math.min(5, b.length())));
    }

    /** 简单负面情绪词典（示意） */
    private boolean containsNegativeSentiment(String text) {
        return text.contains("不对") || text.contains("错了") || text.contains("没用");
    }

    /** 会话放弃判断（示意）——超过 10 分钟无后续交互视为放弃 */
    private boolean sessionAbandoned(String sessionId, Instant timestamp) {
        return repository.findNextInteraction(sessionId, timestamp).isEmpty();
    }
...
/** 剪贴板服务（示意）——前端上报复制事件后查重 */
interface ClipboardService {
    boolean wasCopied(String logId);
}
```
...
public class FeedbackController {
    ...
    public FeedbackController(InteractionLogger interactionLogger) {
    ...
    public Mono<Map<String, String>> thumbs(
        ...
        interactionLogger.updateFeedback(logId,
            up ? InteractionLogger.UserFeedback.THUMBS_UP
               : InteractionLogger.UserFeedback.THUMBS_DOWN);
    ...
    public Mono<Map<String, String>> rate(
        ...
        InteractionLogger.UserFeedback feedback = switch (rating) {
            case 1 -> InteractionLogger.UserFeedback.RATED_1;
            case 2 -> InteractionLogger.UserFeedback.RATED_2;
            case 3 -> InteractionLogger.UserFeedback.RATED_3;
            case 4 -> InteractionLogger.UserFeedback.RATED_4;
            case 5 -> InteractionLogger.UserFeedback.RATED_5;
...
public class AgentMetricsDashboard {
    ...
    private double computeSatisfactionRate(List<InteractionLogger.InteractionRecord> records) {
        ...
        long positive = records.stream()
            .filter(r -> r.userFeedback() == InteractionLogger.UserFeedback.THUMBS_UP
                      || r.userFeedback() == InteractionLogger.UserFeedback.RATED_4
                      || r.userFeedback() == InteractionLogger.UserFeedback.RATED_5
                      || r.userAction() == InteractionLogger.UserAction.ACCEPTED
                      || r.userAction() == InteractionLogger.UserAction.CONTINUED)
            .count();
    ...
    private double computeCompletionRate(List<InteractionLogger.InteractionRecord> records) {
        ...
        long completed = records.stream()
            .filter(r -> r.userAction() != InteractionLogger.UserAction.ABANDONED)
            .count();
    ...
    private double computeModificationRate(List<InteractionLogger.InteractionRecord> records) {
        ...
        long modified = records.stream()
            .filter(r -> r.userAction() == InteractionLogger.UserAction.MODIFIED)
            .count();
    ...
    /** 满意度（按时间窗口）——供告警模块复用 */
    public double getSatisfactionRate(Instant from, Instant to) {
        return getDashboard(from, to).satisfactionRate();
    }

    /** 错误率（按时间窗口）——供告警模块复用 */
    public double getErrorRate(Instant from, Instant to) {
        return getDashboard(from, to).errorRate();
    }
...
public class MetricsAlerter {
    ...
    private final AgentMetricsDashboard dashboard;
    private final AlertService alertService;

    public MetricsAlerter(AgentMetricsDashboard dashboard, AlertService alertService) {
...
public class EvaluationDataset {
    ...
    public List<EvalCase> autoGenerate(List<InteractionLogger.InteractionRecord> records) {
        ...
        records.stream()
            .filter(r -> r.userFeedback() == InteractionLogger.UserFeedback.THUMBS_DOWN)
            .map(this::toEvalCase)
    ...
    /** 把一条交互记录转成评估案例（示意） */
    private EvalCase toEvalCase(InteractionLogger.InteractionRecord record) {
        return new EvalCase(
            record.logId(),
            record.toolCalls().isEmpty() ? "qa" : "tool_use",
            record.userInput(),
    ...
    /** 把高频问题转成评估案例（示意） */
    private EvalCase toEvalCase(String userInput) {
        return new EvalCase(
            "freq-" + Integer.toHexString(userInput.hashCode()),
            "frequent",
            userInput,
            "",
            List.of(),
...
public class RegressionTestRunner {
    ...
    public RegressionTestRunner(EvaluationDataset dataset, ChatClient chatClient) {
    ...
    public RegressionReport run(String agentVersion) {
        ...
        for (EvaluationDataset.EvalCase testCase : dataset.getCases()) {
            // 用 Agent 执行（call().content() 是同步阻塞调用，生产建议包 fromCallable + boundedElastic）
     ...
     * 多维度评估单条结果
     */
    private EvalScore evaluate(EvaluationDataset.EvalCase testCase, String actualResponse) {
    ...
    private double checkCorrectness(EvaluationDataset.EvalCase testCase, String response) {
    ...
    /** 相关性（示意） */
    private double checkRelevance(EvaluationDataset.EvalCase testCase, String response) {
        return response == null || response.isBlank() ? 0 : 1;
    }

    /** 完整性（示意） */
    private double checkCompleteness(EvaluationDataset.EvalCase testCase, String response) {
        return response != null && response.length() > 20 ? 1 : 0.5;
    }

    /** 安全性（示意） */
    private double checkSafety(String response) {
        return response != null && response.contains("敏感") ? 0 : 1;
    }
    ...
    public record EvalCaseResult(
        EvaluationDataset.EvalCase testCase,
...
public class OptimizationDecider {
    ...
    /**
     * 根据问题分类推荐优化策略
     * @param report   当前版本回归报告
     * @param baseline 基线（上一版本）回归报告，用于找退步案例
     */
    public List<OptimizationSuggestion> analyze(RegressionTestRunner.RegressionReport report,
                                                RegressionTestRunner.RegressionReport baseline) {
        List<OptimizationSuggestion> suggestions = new ArrayList<>();

        // 分析退步案例的共性
        List<RegressionTestRunner.EvalCaseResult> regressions = report.regressions(baseline);
    ...
    /** 按错误文本聚类（示意） */
    private Map<String, Long> analyzeErrorPatterns(List<RegressionTestRunner.EvalCaseResult> regressions) {
        return regressions.stream()
            .map(r -> r.actualResponse())
            .collect(Collectors.groupingBy(s -> s.length() > 10 ? "long" : "short", Collectors.counting()));
    }

    /** 领域特定错误判断（示意） */
    private boolean hasDomainSpecificErrors(Map<String, Long> errorPatterns) {
        return errorPatterns.getOrDefault("long", 0L) > 5;
    }
}

/** 优化建议 */
record OptimizationSuggestion(OptimizationType type, String reason, Priority priority, List<String> actions) {}

/** 优化类型 */
enum OptimizationType { RAG_IMPROVEMENT, PROMPT_OPTIMIZATION, MODEL_FINE_TUNING, TOOL_IMPROVEMENT, MODEL_UPGRADE }
...
public class GradualRolloutManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(GradualRolloutManager.class);

    private final RolloutConfigRepository configRepository;
    private final InteractionLogRepository logRepository;
    private final AlertService alertService;

    public GradualRolloutManager(RolloutConfigRepository configRepository,
    ...
    private double computeSatisfaction(List<InteractionLogger.InteractionRecord> records) {
        long positive = records.stream()
            .filter(r -> r.userFeedback() == InteractionLogger.UserFeedback.THUMBS_UP
                      || r.userAction() == InteractionLogger.UserAction.ACCEPTED)
            .count();
    ...
    private double computeErrorRate(List<InteractionLogger.InteractionRecord> records) {
        long failed = records.stream()
            .filter(r -> r.toolCalls().stream().anyMatch(t -> !t.success()))
            .count();
        return (double) failed / records.size();
    }

    private void rollback(RolloutConfig current) {
        configRepository.save(new RolloutConfig(
            current.currentVersion(), current.currentVersion(),
            0, Instant.now(), current.whitelistUsers()));
        log.warn("灰度已回滚到 {}：100%", current.currentVersion());
    }
}

/** 灰度配置 */
record RolloutConfig(
    String currentVersion,
    String newVersion,
    int newVersionPercentage,
    Instant updatedAt,
    List<String> whitelistUsers
) {}

/** 灰度配置仓库（示意） */
interface RolloutConfigRepository {
    RolloutConfig getCurrentConfig();
    void save(RolloutConfig config);
}

/** 灰度指标 */
record RolloutMetrics(
    boolean healthy,
    ...
    static RolloutMetrics insufficientData() {
        return new RolloutMetrics(false, false, 0, 0, 0);
    }

    String reason() {
        if (satisfaction < 0.5) return "满意度低于 50%";
        if (errorRate > 0.1) return "错误率高于 10%";
        return "样本不足";
    }
}
```
...
public class ABTestManager {
    ...
    public ABTestManager(ExperimentRepository experimentRepository,
                         InteractionLogRepository logRepository) {
    ...
    private GroupStats computeStats(List<InteractionLogger.InteractionRecord> records) {
        long positive = records.stream()
            .filter(r -> r.userFeedback() == InteractionLogger.UserFeedback.THUMBS_UP)
            .count();
        long failed = records.stream()
        ...
        return new GroupStats(
            records.isEmpty() ? 0 : (double) positive / records.size(),
            records.isEmpty() ? 0 : (double) failed / records.size(),
            records.size()
        );
    }

    /** 取满意度最高的组为胜者（示意） */
    private String findWinner(Map<String, GroupStats> groupStats) {
        return groupStats.entrySet().stream()
            .max(Map.Entry.comparingByValue(
                java.util.Comparator.comparingDouble(GroupStats::satisfaction)))
            .map(Map.Entry::getKey)
            .orElse("A");
    }

    /** 显著性判断（示意）——样本量过小即不显著 */
    private boolean isStatisticallySignificant(Map<String, GroupStats> groupStats) {
        return groupStats.values().stream().allMatch(g -> g.sampleSize() >= 100);
    }
}

/** 实验配置 */
record ExperimentConfig(String name, String description) {}

/** 实验实体 */
record Experiment(
    String id,
    ExperimentConfig config,
    Map<String, Double> groupDistribution,
    ExperimentStatus status,
    Instant createdAt
) {}

enum ExperimentStatus { RUNNING, COMPLETED, STOPPED }

/** 实验组统计 */
record GroupStats(double satisfaction, double errorRate, int sampleSize) {}
...
interface ExperimentRepository {
    ...
    Experiment findById(String experimentId);
}
```
...
**下一篇**：[85-响应式错误处理](08-响应式错误处理.md) — WebFlux + Spring AI 的错误传播链、降级策略、背压实战。

---

> **想深入？→ [教程 08-架构师进阶/06-长任务持久化与中断恢复]**：飞轮的数据采集需要可靠的任务执行。
> **想深入？→ [教程 08-架构师进阶/09-Agent治理与合规框架]**：用户数据采集的隐私合规要求。
> **想深入？→ [教程 08-架构师进阶/11-Agent架构反模式与避坑指南]**：「无评估闭环」反模式的完整分析。
