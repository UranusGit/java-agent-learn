...
```java
// Spring AI 2.0.0 —— entity(Class) 即走框架默认结构化输出路径
//（"useProviderStructuredOutput().validateSchema().maxAttempts()" 这一整套 spec 链
//   是审计发现的虚构 API，不真实存在——见附录 12-02 基准 §2）
Person person = chatClient.prompt()
        .user("提取人物信息：张三，25岁，在北京")
        .call()
        .entity(Person.class);

// Provider 原生结构化输出/自动重试的开关在 ChatOptions 层配置，且因供应商而异——
// 以所引版本文档为准；业务层的验证与重试封装见 §5.3
```
...
```java
// Spring AI 2.0.0 —— entity() 真实重载只有 Class / ParameterizedTypeReference 两种形态
//（框架不提供 spec-lambda 自动重试/验证——那是虚构 API，见附录 12-02 基准）
Person person = chatClient.prompt()
        .user("提取人物信息：张三，25岁，在北京")
        .call()
        .entity(Person.class);
...
> **想深入？→ [教程 14-Advisor链与拦截器.md]**：理解 Advisor 链如何在结构化输出的前后添加自定义处理逻辑。
