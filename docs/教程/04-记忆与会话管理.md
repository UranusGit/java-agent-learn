...
graph TB
    subgraph 记忆系统["Spring AI 记忆系统"]
        ...
        CMR --> RMR["RedisChatMemoryRepository<br/>Redis 存储"]
        CMR --> MMR["MongoChatMemoryRepository<br/>MongoDB 存储"]
    end
...
> **想深入？→ [教程 29-上下文工程]**：更高级的记忆管理——上下文压缩、Token 预算分配、渐进式披露。
> **想深入？→ [教程 34-高级记忆架构]**：三层记忆架构、记忆演化与衰减。
...
> **遇到阻塞？→ [教程 20-多租户隔离与资源治理]**：多租户场景下的会话隔离、资源配额、数据隔离。
...
| **Redis** | 生产、高并发 | 极快、TTL 自动过期 | 需运维 Redis |
| **MongoDB** | 生产、非结构化 | 灵活的文档结构 | 需运维 MongoDB |

> **遇到阻塞？→ [教程 19-历史记录持久化与合规]**：会话归档、对话回放、审计留存、数据生命周期管理。
...
> **想深入？→ [教程 34-高级记忆架构]**：三层记忆架构（短期→长期→外部 RAG）的完整设计。
...
> **想深入？→ [附录 05-LLM基础理论/02-上下文窗口与Token.md]**：理解上下文窗口限制如何影响记忆策略。
