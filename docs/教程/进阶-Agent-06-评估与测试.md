...
> 📖 **评估学习路径 · 第 1 级 评估入门**：本文件是评估四步路径的起点。
> 下一级 → `主线-SpringAI2.0-12-评估闭环与Prompt版本管理.md`（第 2 级 评估闭环）→ `主线-SpringAI2.0-13-测试工程化.md`（第 3 级 测试工程化）→ `../理论/11-LLMOps.md`（理论）。
...
```mermaid
flowchart TD
    TC["测试用例（TestCase）"] --> EV["evaluate()"]
    EV --> R["recorder 记录 Tool 调用"]
    R --> SC1["toolSelectionScore<br/>Tool 是否命中 expected_tools"]
    R --> SC2["paramScore<br/>参数是否匹配 expected_tool_calls"]
    R --> SC3["answerScore<br/>回答命中 expected_keywords"]
    SC1 --> PASS{"isPassed？<br/>工具调对 且 关键词命中"}
    SC2 --> PASS
    SC3 --> PASS
    PASS -->|"是"| OK["通过"]
    PASS -->|"否"| FAIL["失败"]
...
> 💡 RAGAS 是业界标准的 RAG 自动化评估框架：它用另一个 LLM 当裁判，对"检索质量 + 答案质量"打分，核心指标是 Faithfulness（答案是否忠实于检索内容）与 Relevance（答案是否切题）。起步阶段人工抽检足够，项目做大了再接入它。
...
1. 准备评估集（20-100 条）→ 跑评估 → 分析失败案例 → 改 prompt / Tool 描述 → 回归测试 → 重复 3-5 直到准确率达标（如 80%+）→ 接入 CI/CD 持续监控。

**完整评估闭环**：

```mermaid
flowchart TD
    P1["准备评估集（20-100 条）"] --> P2["跑评估"]
    P2 --> P3["分析失败案例"]
    P3 --> P4["改 prompt / Tool 描述"]
    P4 --> P5["回归测试"]
    P5 --> D{"准确率达标?<br/>如 80%+"}
    D -->|"否"| P3
    D -->|"是"| P6["接入 CI/CD 持续监控"]
```
...
RAG 专用的自动化评估框架，用另一个 LLM 当裁判，对"检索质量 + 答案质量"打分（核心指标 Faithfulness 与 Relevance，详见本文档 5.3 节）。

### 11.3 LangSmith / Langfuse

生产可观测平台：追踪每次请求的 prompt、响应、token 用量与延迟，全链路可视化，定位问题快得多。
