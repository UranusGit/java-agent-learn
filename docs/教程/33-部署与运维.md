...
>
> **前置阅读**：[30-容错与弹性设计](30-容错与弹性设计.md)、[32-模型路由与降级](32-模型路由与降级.md)。
...
spec:
  ...
  # ⚠ 修正: 不用 sessionAffinity——Agent 服务无状态（会话在 Redis/DB 集中管理，
  # 跨实例广播见教程 18 §多页面同步）。粘性会话与水平扩展架构冲突，且 NLB 后不可靠。
...
graph LR
    ...
    subgraph STAGING["预发环境 (staging)"]
        ...
        S_VEC["PgVector<br/>（独立实例）<br/>=== 生产同款 ==="]
...
| **向量库** | ⚠ Chroma（本地）→ 见下 | PgVector | Milvus 集群 |
<!-- 修正: 三环境三种向量库违反"预发与生产一致性"——staging 无法复现 prod 的向量检索行为。
     正确做法: staging 与 prod 同款（PgVector 或 Milvus），dev 允许不同。
     若 prod 是 Milvus，staging 必须是 Milvus（降容量可，换引擎不可）。 -->
...
> **遇到阻塞？→ [教程 32-模型路由与降级](32-模型路由与降级.md)**：多模型部署的配置管理参考本章环境隔离部分。
