...
>
> **前置阅读**：[教程 03]、[教程 19-SSE流式通信]、[教程 26-React与SSE流式UI]。
...
- **`replay().limit(N)` 而非 `multicast()`**：新订阅者先收到最近 N 条（进页面即可见"刚发生过什么"），之后实时推。工控大屏断线重连（EventSource 自动重连）后不丢上下文——呼应 [教程 57-多页面流式响应与会话管理] 的断线重连主题。
...
> 「React 工程化与 SSE UI 细节 → [教程 15-React入门 §工程化]、[教程 26-React与SSE流式UI]」
...
**下一关**：把所有观测用 traceId 串成一条跨阶段链路。→ [教程 06]
