...
> **定位**：本文讲透企业级 Agent 前后端协作的核心契约——流式模式下的工具调用循环（streaming tool calls）、Agent 事件协议的设计与实现、并行工具调用的处理、客户端取消与服务端部分结果。这是教程 09（SSE）与教程 44（Agentic UI）之间的"后端发动机"篇。读者画像：要实现完整 Agent 流式后端的开发者。前置阅读：[教程 03-工具调用 §调用循环]、[教程 09-SSE流式通信]、[教程 44-Agentic-UI设计 §2 事件协议]。
>
> **与教程 44 的分工**：教程 44 站在前端视角定义"消费什么事件"；本文站在后端视角讲"事件从哪里来、怎么生成"——特别是流式模式下的工具循环如何在 Spring AI 2.0 的执行模型中落地。
...
前端消费的事件联合类型见 [教程 44 §2.1]。后端侧的对应物是 sealed interface + 每事件单调递增 id：
    ...
    public Flux<AgentEvent> toEventStream(Flux<ChatResponse> source, SessionContext ctx) {
        return source.concatMap(chunk -> {
            ...
            if (chunk.hasToolCalls()) {
                for (ToolCall tc : chunk.getToolCalls()) {
                    events.add(new ToolStart(nextId(), now(), tc.id(), tc.name(),
                            parseArgs(tc.arguments()), parallelGroupOf(tc)));
                }
            }
            // ② 文本增量 → Token 事件（脱敏管道在此挂接, [教程 25 §输出过滤]）
            if (chunk.hasText()) {
                events.add(new Token(nextId(), now(), redactor.scrub(chunk.getText())));
            }
            // ③ usage/finish → RoundEnd
            if (chunk.getMetadata().getFinishReason() != null) {
...
「遇到阻塞？→ [附录 12-SpringAI2-API基准/02-Tool与Observation真实API §拦截扩展点]——工具执行层面的拦截/观测在 ToolCallingManager 层，不在本管道（本管道只负责'事件化'）」
...
```java
// 装饰 ToolCallingManager: 工具执行前后发事件（HITL 拦截也在这一层, [教程 22 §正确落点]）
public class EventEmittingToolManager implements ToolCallingManager {

    private final ToolCallingManager delegate;
    private final Sinks.Many<AgentEvent, Void> eventSink;   // 事件出口

    @Override
    public ToolExecutionResult executeToolCalls(Prompt prompt,
            ToolCallingChatOptions options, List<ToolDefinition> definitions) {

        List<ToolCall> calls = currentToolCalls(prompt);
        calls.forEach(tc -> eventSink.tryEmitNext(
            new ToolStart(nextId(), now(), tc.id(), tc.name(), parseArgs(tc.arguments()), null)));

        long start = System.nanoTime();
        ToolExecutionResult result = delegate.executeToolCalls(prompt, options, definitions);  // 真实执行

        calls.forEach(tc -> eventSink.tryEmitNext(new ToolEnd(
            nextId(), now(), tc.id(), Outcome.SUCCESS,
            truncate(previewOf(result, tc), 500),          // resultPreview 截断（教程 44 §2.2）
            elapsedMs(start))));
        return result;
    }
}
...
自己控制"LLM 调用 → 解析工具意图 → 执行 → 回填 → 再调用"的循环——ReAct 类自定义模式（[教程 06]）用这种。代价：你要自己处理循环上限、预算控制（[教程 35 §三维预算]）、死循环防护。**只在框架姿势无法表达你的模式时才用**。
...
```java
// 并行执行多个工具——Mono.zip 而非 CompletableFuture（WebFlux 铁律, [教程 06 §并行工具]）
private Flux<AgentEvent> executeParallel(List<ToolCall> calls, int parallelGroup) {
    int group = parallelGroup;   // 同组标记: 前端排成一行（教程 44 §3.2）
...
**部分结果的保留决策**：取消后已生成的 token 保存在会话缓冲区——用户"重新生成"时可以（a）从断点续写（带上下文成本）或（b）全量重来。默认全量重来（续写的拼接质量不稳定），但**缓冲区必须保留**（审计与恢复的依据，[教程 19 §对话回放]）。

## 6. 反模式

| 反模式 | 后果 | 纠正 |
|--------|------|------|
| 事件无 id | 断线恢复无法去重 | 单调 id 进每事件（教程 44 §2.2） |
...
**下一篇**：[46-SpringAI2-Agentic架构.md](46-SpringAI2-Agentic架构.md) — 框架原生的 Agent 编排抽象：什么时候不用手写这一切。
