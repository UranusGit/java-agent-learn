...
| 方法 | 语义 | 工业用法 |
|------|------|---------|
| `tag(k, v)` | span 全程有效的键值对 | 业务属性：`tenant=plant-a`（注意基数！值必须有限可枚举，[教程 07] 基数熔断同理） |
...
两个易错点：① `withSpan` 与 `end` **是两件事**——scope 管日志，end 管记录闭合，缺一个就漏一半；② WebFlux 主链路里**优先用** `Hooks.enableAutomaticContextPropagation()` + Reactor Context（[教程 06 §6.3]），手抓 span 只用于"逃出 Reactor 世界"的边界（如legacy 线程池）。
...
适用场景：消息消费者入口（Kafka 监听器内没有 HTTP 自动 span，[教程 67-Kafka全景与核心概念] 场景）、老代码线程池迁移期、任何"业务子段"计时。
...
下一关 [教程 03]：**Baggage**——tenantId 这类业务属性如何挂在 traceId 上、随整条链路（含跨服务）自动流动、并进 MDC 与日志同行——这是多租户架构（07 关）的关键地基。
