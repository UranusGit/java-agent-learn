...
层级化的 key 让失效粒度可控：`invalidateQueries({ queryKey: ['sessions'] })` 会失效所有租户的会话列表——与后端缓存键的分层设计（[附录 10-语义缓存与性能/00-语义缓存实现 §缓存键设计]）是同一种架构思维。
...
这一分层与后端的三层记忆架构（[教程 39-高级记忆架构 §三层记忆]）形成有趣的镜像：前端也是"短期（流式缓冲）→ 中期（会话状态）→ 长期（服务端数据缓存）"，只是生命周期尺度不同。架构师的跨端抽象能力正体现在这种模式识别上。
...
**下一篇**：[17-React与SSE流式UI.md](17-React与SSE流式UI.md) — 把状态模型接到真实的 SSE 流上：fetch + ReadableStream、事件解析、断线重连与 token 批量渲染。
