...
public class ModelABTestRouter {
    ...
    public ChatModel route(String testName, String userId) {
        ...
        if (config == null) {
            // 没有活跃的 A/B 测试，使用基线模型
            return modelRegistry.get(config.baselineModel());
        }
...
public class CanaryRoutingAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    private AdvisedResponse forwardRequest(ChatClient client,
                                            AdvisedRequest original) {
        // 将原请求的参数转发给选中的 ChatClient
        return client.prompt()
                .system(original.systemText())
                .user(extractUserMessage(original))
                .advisors(a -> {
                    original.adviseContext().forEach(a::param);
                })
                .call()
                .advisedResponse();
    }

    private Flux<AdvisedResponse> forwardStream(ChatClient client,
                                                 AdvisedRequest original) {
        return client.prompt()
                .system(original.systemText())
                .user(extractUserMessage(original))
    ...
    private RequestContext buildContext(AdvisedRequest request) {
        return new RequestContext(
                request.adviseContext().get("userId", String.class),
                request.adviseContext().get("tenantId", String.class),
                request.adviseContext().get(ChatMemory.CONVERSATION_ID, String.class)
        );
    }
...
public class CanaryMetricsCollector {
     ...
     * 记录灰度和稳定版的执行指标。
     */
    public void recordSuccess(CanaryDecision decision, AdvisedResponse response,
                               long durationNanos) {
