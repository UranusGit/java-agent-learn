...
>
> **前置阅读**：[04-记忆与会话管理](04-记忆与会话管理.md)、[05-RAG 检索增强生成](05-RAG检索增强生成.md)。
...
```java
// Spring AI 2.0 中通过扩展字段设置缓存断点
// 以 Anthropic Claude 为例，支持 ephemeral cache_control
ChatClient client = ChatClient.builder(claudeChatModel)
    .defaultSystem(systemPrompt)              // 此处可设 cache_control
    .build();

// 对长文档 RAG，把固定的文档块标记为可缓存
...
public class SemanticCacheAdvisor implements CallAdvisor {
    ...
    @Override
    public ChatClientResponse before(ChatClientRequest request) {
        ...
        if (!hits.isEmpty()) {
            // 命中：构造一个"已缓存"的响应，跳过 LLM 调用
            String cachedAnswer = hits.get(0).getText();
            return ChatClientResponse.cached(cachedAnswer);
        }
        return request;
    }
    ...
    public ChatClientResponse after(ChatClientResponse response) {
        // 未命中路径：把 Q&A 写入缓存库
        String question = response.request().userText();
        String answer = response.content();
        cacheStore.add(List.of(Document.builder().text(answer)
                .metadata(Map.of("question", question, "timestamp", Instant.now().toString()))
                .build()));
        return response;
    }
}
...
public class ContextAwareChatService {
    ...
    public String chat(String userId, String userInput) {
        ...
        // 2. 读取会话历史
        List<Message> history = memory.get(userId, 20);   // lastN 必带
        ...
        memory.add(userId, userInput, answer);
...
**下一篇**：[30-高级 RAG 与 Agentic RAG](35-高级RAG与AgenticRAG.md) — GraphRAG 多跳推理、Agentic RAG 自主决策检索、混合检索与重排。

---

> **前置回顾**：[04-记忆与会话管理](04-记忆与会话管理.md)介绍了 MessageWindowChatMemory 的滑动窗口——本章是它的"企业级进化版"。
> **想深入**：[39-高级记忆架构](39-高级记忆架构.md)将讲透三层记忆（短期→长期→外部 RAG）的完整架构，与本章的上下文工程紧密呼应。
> **性能优化**：[33-Agent 性能优化](38-Agent性能优化.md)会继续讨论 KV Cache 和批量推理对延迟的影响。
