...
>
> **前置阅读**：[09-多Agent协作](09-多Agent协作.md)、[31-安全与权限控制](31-安全与权限控制.md)、[11-MCP协议](11-MCP协议.md)、[前沿 00-A2A协议](../前沿/00-A2A协议.md)。
...
**协商**：接受前双方对齐"能做什么、花多少成本、何时交"——这是协作协议的"合同"环节，避免委托后才知道做不了/太贵（呼应 [20-成本治理](../教程/27-成本治理与Token计量.md)）。
...
跨组织协作不能用内部 Session，要用**可验证的身份**（如 SPIFFE/SVID 或 OIDC/JWT）——被委托方能验证"来者是谁、属于哪"（呼应 [31-安全](../教程/31-安全与权限控制.md)、[08-零信任](../项目/08-Agent供应链安全网关/00-需求分析与架构设计.md)）。
...
对方返回的内容**不自动当指令**——把"跨信任域返回"当**数据**而非**代码/系统提示**（防对方 Prompt 注入反向攻击，呼应 [31-Prompt注入防御](../教程/31-安全与权限控制.md)、[附录09-Agent安全深度](../附录/09-Agent安全深度/00-Prompt注入分类与案例.md)）。
...
**要点**：协作协议要支持**异步、可查询、可取消**的任务语义（长任务委托），而非同步 RPC（呼应 [教程 40-长任务](../教程/40-长任务持久化与中断恢复.md)）。
