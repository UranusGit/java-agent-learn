...
> **前置阅读**：建议先通读 [00-Agent核心概念](00-Agent核心概念.md) 至 [44-多模型协作与供应策略](44-多模型协作与供应策略.md)，本文是对这些教程的总结性回顾。
...
> **对应教程**：[40-长任务持久化与中断恢复](40-长任务持久化与中断恢复.md) — 完整的预算控制体系、死循环三层防护、检查点与崩溃恢复。
...
> **对应教程**：[44-多模型协作与供应策略](44-多模型协作与供应策略.md) — 完整的多模型编排、故障切换、降级链设计。
> **补充参考**：[32-模型路由与降级](32-模型路由与降级.md) — 模型路由策略与实现细节。
...
> **补充参考**：[40-长任务持久化与中断恢复](40-长任务持久化与中断恢复.md) — 幂等重试在崩溃恢复中的应用。
...
> **对应教程**：[34-上下文工程](34-上下文工程.md) — 五层拼接策略、Token 预算分配、上下文压缩、KV Cache / Prompt Cache 利用。
> **补充参考**：[39-高级记忆架构](39-高级记忆架构.md) — 分级记忆系统的深度设计。
...
> **对应教程**：[37-自我反思与Agent评估](37-自我反思与Agent评估.md) — Reflection 模式、评估指标体系、LLM-as-Judge、回归测试。
> **补充参考**：[41-数据飞轮与持续改进](41-数据飞轮与持续改进.md) — 数据飞轮驱动的持续优化闭环。
...
> **对应教程**：[31-安全与权限控制](31-安全与权限控制.md) — 输入安全（Prompt 注入防护）、执行安全（工具权限控制）、输出安全（内容过滤）、Spring Security 集成。
...
> **对应教程**：[28-Human-in-the-Loop与审批流](28-Human-in-the-Loop与审批流.md) — HITL 完整设计、风险分级、审批状态机、Spring AI Advisor 拦截实现。
...
> **对应教程**：[22-全链路可观测性](22-全链路可观测性.md) — Spring AI 2.0 原生可观测性体系、gen_ai 语义约定、OpenTelemetry 集成、Prometheus + Grafana 监控面板。
> **补充参考**：[23-工具执行可观测与审计](23-工具执行可观测与审计.md) — 工具调用的可观测性与审计日志。
...
> **对应教程**：[27-成本治理与Token计量](27-成本治理与Token计量.md) — Token 计量指标体系、按租户/用户成本归因、预算上限与优雅降级、模型路由降本。
> **补充参考**：[26-多租户隔离与资源治理](26-多租户隔离与资源治理.md) — 多租户资源隔离与配额管理。
...
> **想深入？→ [教程 31-安全与权限控制](31-安全与权限控制.md)**：三道防线的完整实现。
> **想深入？→ [教程 40-长任务持久化与中断恢复](40-长任务持久化与中断恢复.md)**：检查点、幂等、预算控制的完整方案。
> **想深入？→ [教程 44-多模型协作与供应策略](44-多模型协作与供应策略.md)**：多模型冗余与故障切换。
