...
>
> **前置阅读**：[教程 01 §1.3]（traceparent 报文）、[教程 03]。
>
...
适用场景：任何 ≥2 个服务的系统；SSE 流式链路（06 关在传播之上做展示）；消息中间件传播（Kafka 场景同思路，header 换成 record header，[教程 67-Kafka全景与核心概念] 结合处）。
...
下一关 [教程 06]：**展示**——把 traceId 时间线用 SSE 推到前端页面，用户看得见"Agent 正在干什么 + 这一步的链路号"，报警体验闭环到 UI。
