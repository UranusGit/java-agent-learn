...
>
> **读者画像**：读过 [教程 07-Kafka事件骨干/01-生产者机制与调优]、[教程 07-Kafka事件骨干/02-消费者与消费组] 的工程师；正在设计"工具执行结果必须不丢不重"的审计/事件溯源管道，或被"重复消费"问题困扰的架构师。
>
> **前置阅读**：[教程 07-Kafka事件骨干/01-生产者机制与调优 §5]（PID/序列号）；[教程 07-Kafka事件骨干/02-消费者与消费组 §3]（位移提交策略）；[教程 04-企业级架构主干/10-容错与弹性设计]（重试/幂等的通用视角）。
>
...
1. **崩溃本身无法避免，语义取决于"提交"与"处理"的先后顺序**——[教程 07-Kafka事件骨干/02-消费者与消费组] §3] 的两种手动提交策略正是对这张图的选择。
...
延续 [教程 07-Kafka事件骨干/01-生产者机制与调优] §3] 的三件套，给出完整核对矩阵——每一行都是一次真实故障注入：
...
| 磁盘坏 / 文件系统损坏 | 消息丢失 | RF=3 + 跨机架/AZ 分布（[教程 07-Kafka事件骨干/08-运维监控与安全] §2]） |
...
```java
// 概念骨架：事务生产者（spring-kafka 封装见 [教程 07-Kafka事件骨干/09-Spring集成与Agent事件驱动落地] §4]）
...
| Kafka Streams `exactly_once_v2` | 无（框架托管上述事务） | 流处理拓扑（[教程 07-Kafka事件骨干/06-KafkaStreams流处理] §7]） |
...
实现要点（呼应 [教程 08-架构师进阶/06-长任务持久化与中断恢复 §4] 的幂等层）：
...
4. Debezium 侧与 Schema 演进的完整展开见 [教程 07-Kafka事件骨干/07-Connect与数据生态 §5]。

Agent 场景对号入座：LLM 网关完成一次调用，要同时写"调用记录表"（成本归因，[教程 04-企业级架构主干/07-成本治理与Token计量]）与发 `agent.llm.telemetry` 事件（计量管道）——Outbox 是标准答案；会话事件同理（[项目 13] 迭代二的事件管线）。
...
- 流处理拓扑：Kafka Streams `exactly_once_v2` 一键获得（[教程 07-Kafka事件骨干/06-KafkaStreams流处理] §7]）
...
3. **transactional.id 没有按实例区分**（如多实例共用一个 id）——互相 fencing，间歇性 `ProducerFencedException`，必现事故。规范：`tx-prefix + 实例序号`（spring-kafka 的 `transaction-id-prefix` 自动处理，见 [教程 07-Kafka事件骨干/09-Spring集成与Agent事件驱动落地] §4]）。
...
语义问题的正确打开方式：**接受 at-least-once，把工程力量投向幂等**；Kafka 事务是"消息+位移"原子化的专用工具（consume-transform-produce/EOS v2）；跨出 Kafka 边界的一致性，靠幂等键与 Outbox。至此读写两侧机制完备，下一篇下钻存储底座——日志分段、保留与压实、ISR 复制、KRaft 控制器：[教程 07-Kafka事件骨干/04-日志存储与高可用复制]。
