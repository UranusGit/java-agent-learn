...
- **取消**：客户端可发 `notifications/cancelled`（携带被取消请求的 id），服务端"尽力而为"中止——协议不保证执行中的副作用真的停，所以**工具实现要幂等/可补偿**（呼应 [教程 83-长任务持久化与中断恢复]）。
...
**④ 审计日志——对抗"事后无法归因"**。记录每次 `tools/call` 的四元组：哪个会话/用户 → 调了哪个 Server 的哪个工具 → 参数 → 结果摘要。没有这份日志，攻击发生了你既不知道谁干的、也说不清干了什么。技术落点：Spring AI 的工具执行 Observation（[教程 32-工具执行可观测与审计]、[教程 03-自定义观测点与扩展点]）+ MCP 网关层的调用镜像；注意脱敏（参数里可能有 PII，见 [附录 08-Agent安全深度/02-数据泄露防护]）。

**⑤ 人工审批（HITL）——对抗"不可逆操作"**。删除、转账、对外发送这类工具，注入防护做得再好也只是概率防御，最后一道闸必须是"人确认"。正确落点在工具执行层（`ToolCallingManager` 装饰器，见 [教程 61-Human-in-the-Loop与审批流]），审批期间 `tools/call` 挂起（配合 §7.1 的进度通知给用户反馈），拒绝则返回 `isError=true` 的工具级错误让 LLM 改道。
...
**下一篇**：[21-Agent状态管理](21-Agent状态管理.md) — 状态机、会话生命周期、上下文管理与并发一致性。

---

> **想深入？→ [附录 05-SpringAI2-API基准/01-MCP真实API与坐标]**：MCP 全部真实坐标、类型与虚构 API 对照表。
> **想深入？→ [教程 29-管控分离架构]**：MCP Server 如何实现 Agent 内核与外部能力的完全解耦。
