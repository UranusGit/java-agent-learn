...
>
> **前置阅读**：[02-ChatClient 与对话模型](02-ChatClient与对话模型.md)、[30-容错与弹性设计](30-容错与弹性设计.md)。
...
**下一篇**：[33-部署与运维](33-部署与运维.md) — Docker 化、K8s 部署、环境隔离、健康检查、日志收集。

---

> **想深入？→ [教程 30-容错与弹性设计](30-容错与弹性设计.md)**：模型路由的底层就是容错——降级链是容错策略的具体应用。
> **遇到阻塞？→ [教程 34-上下文工程]**：不同模型的上下文窗口大小不同，路由时需要考虑 Token 预算。
