...
（配置元数据实证：`management.tracing.baggage.tag-fields` 真实存在。）`work-order-id` 就**不要**进 tag-fields——工单号近乎无限枚举，进 tag 就是基数爆炸（[教程 07] 基数熔断同理）。
...
下一关 [教程 04]：**记录与持久化**——traceId 回写响应头（前端报警有号可查）、结构化日志、以及把"链路档案"落库做审计——展示与记录的存储侧。
