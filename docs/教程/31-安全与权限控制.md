...
>
> **前置阅读**：[03-工具调用](03-工具调用.md)、[30-容错与弹性设计](30-容错与弹性设计.md)。
...
// 生产应改用 ReactiveSecurityContextHolder / Reactor Context 取用户身份（见教程 33 §7.3）。
@Aspect
@Component
...
**下一篇**：[06-向量数据库选型](06-向量数据库选型.md) — 向量库核心指标、4+ 向量库对比、选型决策框架。

---

> **想深入？→ [附录 09-Agent安全深度/01-ToolPoisoning攻击.md]**：Tool Poisoning 攻击的完整攻防分析。
> **遇到阻塞？→ [教程 30-容错与弹性设计](30-容错与弹性设计.md)**：安全检查失败也是一种"故障"，需要容错策略配合。
