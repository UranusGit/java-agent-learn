...
**第 4 层：输出契约层。** 对最终输出格式的机器可校验约束：JSON schema、字段必选性、长度上限、语言。这层**变更频率最高**——下游解析逻辑每调整一次，这里跟着动。失效模式是"格式漂移"：模型在 JSON 前后加寒暄、字段名单复数不一致、把 null 写成字符串 "null"。工程防线有三道：契约写得机器可校验、few-shot 给正例、解析侧用结构化输出兜底（`entity(Class)` 的 provider 级结构化输出，见 [教程 02-SpringAI核心机制/03-结构化输出]）。
...
| **4 格式漂移** | JSON 前后带寒暄、字段名大小写不定、null 表达不一致 | 解析失败日志 + 输出原文 diff | 契约层改为机器可校验断言；few-shot 补该格式的正例；解析侧换 provider 级结构化输出兜底（[教程 02-SpringAI核心机制/03-结构化输出]） |
