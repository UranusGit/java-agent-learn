...
>
> 前提：你已理解 Advisor 链如何包裹 ChatClient 调用（即 03 的能力），这决定了你在哪一层埋点。
> 预计：2 天
...
sequenceDiagram
    ...
    participant LLM1 as "LLM1 GPT-4o-mini"
    participant OA as "Order Agent"
    participant LLM2 as "LLM2 Claude Sonnet"
    participant VS as "VectorStore"
    participant DB as "订单 DB"
    participant API as "物流 API"
    U->>RA: "我上个月订单为什么没发货"
    RA->>LLM1: 判断意图
    LLM1-->>RA: 命中 Order Agent
    RA->>OA: 分发请求
    OA->>LLM2: 主流程推理
    LLM2->>VS: 向量查询
    VS-->>LLM2: 命中 5 个 chunk
    LLM2->>DB: 调 getOrderHistory
    DB-->>LLM2: 订单列表
    LLM2->>API: 调 getShippingStatus
    API-->>LLM2: 物流状态
    LLM2-->>OA: 综合生成最终回复
    OA-->>U: 最终回答
```
...
public class PromptLineageAdvisor implements BaseAdvisor {
    ...
    public ChatClientRequest before(ChatClientRequest req, AdvisorChain chain) {
        ...
        // 工具名列表需在 ChatClient 构造时写入 context（key 自定义，下面用 "toolNames"）。
...
**Prompt 血缘记录的 before / after 时序**：

```mermaid
flowchart TD
    B1["PromptLineageAdvisor.before()<br/>生成 LineageRecord<br/>id / sessionId / prompt 版本 / system + user prompt / toolNames"] --> B2["lineageRepo.save(record)<br/>context 放入 __lineage_id__"]
    B2 --> Call["调用 LLM"]
    Call --> A1["PromptLineageAdvisor.after()<br/>从 context 取 __lineage_id__"]
    A1 --> A2["补写 model_used / total_tokens / response"]
    A2 --> A3["lineageRepo.update(id, model, tokens, output)"]
```
...
**Drift 检测流程（近 7 天 vs 之前 30 天）**：

```mermaid
flowchart TD
    Calc["avg(metric, 7 天) vs avg(metric, 30 天)<br/>delta = recent - baseline"] --> Check{"|delta| &gt; |threshold|?"}
    Check -->|"是"| Alert["report.addAlert(metric, baseline, recent, delta)"]
    Check -->|"否"| None["无告警，持续观察"]
    Alert --> Cause["定位原因：Prompt 改坏 / 新文档投毒 /<br/>供应商模型变化 / 用户 query 分布变化"]
```
...
```mermaid
flowchart TD
    P0["P0 紧急<br/>错误率 &gt; 10%、主 LLM 全挂<br/>通知：电话 + 短信 + IM"] ==> P1["P1 严重<br/>P95 &gt; 10s、faithfulness &lt; 0.7<br/>通知：短信 + IM"]
    P1 ==> P2["P2 警告<br/>Cost 超日预算 80%、Drift 触发<br/>通知：IM"]
    P2 ==> P3["P3 提示<br/>异常流量、单租户异常<br/>通知：IM 群"]
```
