...
>
> **读者画像**：读完 [教程 00-08]、要在 Spring Boot 4.1 + WebFlux 技术栈里真正落地事件驱动 Agent 服务的工程师。
>
> **前置阅读**：[教程 69-消费者与消费组 §6]（长调用防线）；[教程 70-投递语义与事务 §5-6]（幂等/Outbox）；[附录 06-企业级架构模式/02-事件驱动Agent架构]（架构模式层——本文是其工程实现层）。
>
...
| MANUAL / MANUAL_IMMEDIATE | 代码里 `Acknowledgment.acknowledge()` | **Agent 服务推荐**：与业务结果显式对齐（[教程 69-消费者与消费组] §3]） |
...
public void project(AgentEvent event, ConsumerRecord<String, AgentEvent> rec) {
    ...
    // 位移提交由容器在事务内完成——输出与进度原子化（[教程 70-投递语义与事务] §4]）
}
```

注意边界：事务只保证 Kafka 内原子；`snapshotStore` 若是外部 DB，事务包不住它——**DB + Kafka 双写仍走 Outbox**（[教程 70-投递语义与事务] §6]），事务版投影适合"纯 Kafka 输入→纯 Kafka 输出"的管道。
...
**必须刻进肌肉的三条铁律**（WebFlux 语境，呼应 [教程 10-WebFlux从零入门] 与 [教程 85-响应式错误处理]）：
...
[教程 31-全链路可观测性] 的 Trace 要跨服务、跨事件继续有效，靠三件事：
...
2. **Observation API**：`KafkaTemplate` 与监听容器开启 observation 后，send/ consume 产生 Span，**trace 上下文经 `DefaultKafkaHeaderMapper` 写入消息 headers（W3C traceparent）**——消费端取出继续父 Span，LLM 调用 → 事件发布 → 下游投影的链路在一个 Trace 里闭环（这正是 [教程 31] gen_ai 语义约定在事件链上的延伸）。
3. **业务语义指标**：DLT 速率、重试耗尽率、每主题 lag 换算的"审计落后分钟数"——第三层 SLO 指标（[教程 75-运维监控与安全] §3]）。
...
1. **监听器里同步调 LLM/DB 且无 subscribeOn/虚拟线程**——[教程 69-消费者与消费组] §6] 的踢组事故在 Spring 里的形态。
