...
| `PromptChatMemoryAdvisor`（1.x，已移除） | ~~紧凑式记忆注入~~ | 2.0.0 起删除，官方仅保留 `MessageChatMemoryAdvisor`；历史压缩需自研（见 [教程 08-架构师进阶/05-高级记忆架构]） |
...
> **想深入？→ [教程 04-企业级架构主干/02-全链路可观测性]**：如何将 Filter、Interceptor 和 Advisor 的可观测数据统一关联到一个 Trace 链路。
...
> **想深入？→ [教程 00-基础与核心/02-ChatClient与对话模型]**：回顾 ChatClient 的完整 API，理解 Advisor 如何嵌入到 ChatClient 的调用流程中。
> **想深入？→ [教程 04-企业级架构主干/02-全链路可观测性]**：将 Advisor 与 Filter、Interceptor 的可观测数据统一到一条 Trace 链路。
> **想深入？→ [教程 02-SpringAI核心机制/02-Agent状态管理]**：Advisor 上下文在 Agent 状态管理中的关键作用。
