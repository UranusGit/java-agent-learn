...
>
> **前置阅读**：[03-工具调用](../00-基础与核心/03-工具调用.md)。
>
...
> ⚠️ **供应链安全警示（这段配置本身就是风险示范）**：`npx -y` 的语义是"**不问、直接拉取并执行**远程 npm 包"。MCP Server 一旦被投毒（包名抢注、版本劫持、维护者账号被盗），恶意代码就会以**你的应用进程权限**运行——读环境变量、外传数据、横向移动。生产环境的最低要求：①锁定版本（`npx -y pkg@1.2.3` 而非浮动 latest）；②私有镜像源 + 准入扫描；③Server 进程跑在容器/沙箱里做权限隔离；④高危 Server 走审批接入。展开见 §8.2 与 [附录 08-Agent安全深度/01-ToolPoisoning攻击]。
...
- **取消**：客户端可发 `notifications/cancelled`（携带被取消请求的 id），服务端"尽力而为"中止——协议不保证执行中的副作用真的停，所以**工具实现要幂等/可补偿**（呼应 [教程 08-架构师进阶/06-长任务持久化与中断恢复]）。
- **进度**：调用 `tools/call` 时在 `_meta.progressToken` 里带上进度令牌，长任务 Server 会在执行期间回推 `notifications/progress`（进度百分比/消息）——这正是 §教程 01-WebFlux与响应式编程/00-WebFlux从零入门 §4.5"工具执行中给前端发状态"的协议级通道。
...
**④ 审计日志——对抗"事后无法归因"**。记录每次 `tools/call` 的四元组：哪个会话/用户 → 调了哪个 Server 的哪个工具 → 参数 → 结果摘要。没有这份日志，攻击发生了你既不知道谁干的、也说不清干了什么。技术落点：Spring AI 的工具执行 Observation（[教程 04-企业级架构主干/03-工具执行可观测与审计]、[教程 00-基础与核心/03-工具调用]）+ MCP 网关层的调用镜像；注意脱敏（参数里可能有 PII，见 [附录 08-Agent安全深度/02-数据泄露防护]）。

**⑤ 人工审批（HITL）——对抗"不可逆操作"**。删除、转账、对外发送这类工具，注入防护做得再好也只是概率防御，最后一道闸必须是"人确认"。正确落点在工具执行层（`ToolCallingManager` 装饰器，见 [教程 04-企业级架构主干/08-Human-in-the-Loop与审批流]），审批期间 `tools/call` 挂起（配合 §7.1 的进度通知给用户反馈），拒绝则返回 `isError=true` 的工具级错误让 LLM 改道。
...
> **想深入？→ [附录 08-Agent安全深度/01-ToolPoisoning攻击]**：MCP Server 被篡改后的工具投毒攻击与防御方案。
...
**下一篇**：[02-Agent状态管理](02-Agent状态管理.md) — 状态机、会话生命周期、上下文管理与并发一致性。

---

> **想深入？→ [附录 05-SpringAI2-API基准/01-MCP真实API与坐标]**：MCP 全部真实坐标、类型与虚构 API 对照表。
> **想深入？→ [教程 04-企业级架构主干/00-管控分离架构]**：MCP Server 如何实现 Agent 内核与外部能力的完全解耦。
> **想深入？→ [教程 00-基础与核心/03-工具调用]**：回顾 Tool Calling 机制，理解 MCP 在底层复用了同一套调用循环。
> **想深入？→ [附录 08-Agent安全深度/01-ToolPoisoning攻击]**：MCP 安全风险与防御。
