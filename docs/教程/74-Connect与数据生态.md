...
>
> **读者画像**：读完 [教程 00-06]、正在为 RAG 知识库设计同步链路、或为多区域 Agent 平台设计数据复制的工程师与架构师。
>
> **前置阅读**：[教程 70-投递语义与事务 §6]（Outbox 模式——CDC 是其实现通道）；[教程 05-RAG检索增强生成]（知识库 ETL）；[教程 58-历史记录持久化与合规]（数据复制与合规语境）。
>
...
| 双写 | 业务代码同时写 DB 和 Kafka | 一致性无解（[教程 70-投递语义与事务] §6] 反模式） |
...
**方案**（把 [教程 70-投递语义与事务] §6] Outbox 与 RAG ETL 串成一条流水线）：
...
4. **工具注册表同理**：MCP 工具元数据变更 → CDC → 网关缓存失效事件——同一模式复用于"配置变更的实时分发"（[教程 29-管控分离架构] 数据面缓存一致性的一条实现路径）。
...
1. **用双写代替 CDC**——一致性无解（[教程 70-投递语义与事务] §6]），见 Outbox。
...
Connect/Debezium/Schema Registry/MM2 四件套把 Kafka 从"消息管道"升级为"数据平台"：CDC 给了事件驱动**可靠的数据正源**（Outbox 的实现通道），Agent 平台用它打通知识库→向量库的自动同步；注册中心把事件契约从口头约定变成 CI 级强制；MM2 承载 DR 与多区域。下一篇转向运维面——部署拓扑、安全（SASL/ACL/配额）、监控与故障处置：[教程 75-运维监控与安全]。
