...
**下一篇**：[41-数据飞轮与持续改进](41-数据飞轮与持续改进.md) — 数据采集、评估闭环、灰度发布的数据飞轮。

---

> **想深入？→ [教程 34-上下文工程]**：上下文窗口管理与压缩，直接影响检查点大小。
> **想深入？→ [教程 43-Agent治理与合规框架]**：检查点中的用户数据隐私合规。
> **想深入？→ [教程 45-Agent架构反模式与避坑指南]**：「无持久化」「无超时无预算」「工具无幂等」三大反模式的完整分析。
