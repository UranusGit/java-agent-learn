...
这与 [教程 32-模型路由与降级](../教程/32-模型路由与降级.md) 中的模型路由策略高度类似——工具也可以视为一种需要路由的资源。
...
这个架构与我们 [教程 20-管控分离架构](../教程/20-管控分离架构.md) 和 [项目 03-MCP 工具网关](../项目/03-MCP工具网关/00-需求分析与架构设计.md) 的设计高度一致——MCP Gateway 就是管控分离理念在 MCP 生态中的具体实现。
