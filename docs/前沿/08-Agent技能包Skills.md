...
> **定位**：Agent 的"能力复用单元"——Skills 作为 2025 年后主流 Agent 平台的标准扩展机制，正在改变工具/提示词/知识打包分发的方式。本文调研其形态、与 MCP 的关系、对企业架构的影响。前置阅读：[教程 20-MCP协议]、[前沿 04-MCP生态全景]。
...
这与 [教程 77-上下文工程 §五层拼接] 的分层理念完全同构——Skills 是上下文工程在"能力分发"维度的应用：**上下文预算管理成了技能系统的设计约束**。
...
2. **Skill 变更需要灰度**：技能指令的变更直接改变 Agent 行为——与 Prompt 版本管理（[教程 32]）同一套灰度/回滚机制
...
| 技能包分发 | Prompt 版本管理（[教程 32]）+ 配置下发（[项目 05 v3] 控制面） |
