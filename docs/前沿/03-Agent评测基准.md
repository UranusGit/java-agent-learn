...
|------|-------------------|-------------|
| OS 操作 | 工具调用的准确性、错误恢复能力 | [教程 00-基础与核心/03-工具调用](../教程/00-基础与核心/03-工具调用.md) |
| 数据库 | 结构化推理、SQL 生成 | [教程 02-SpringAI核心机制/04-结构化输出](../教程/02-SpringAI核心机制/04-结构化输出.md) |
| 知识图谱 | 多步推理、信息整合 | [教程 00-基础与核心/07-ReAct推理模式 推理模式](../教程/00-基础与核心/07-ReAct推理模式.md) |
| 网页操作 | 序列规划、页面理解 | [教程 05-Observation可观测/03-自定义Handler：收集Agent阶段事件流](../教程/08-架构师进阶/02-Agent工作流编排.md) |
| 策略博弈 | 长程规划、对手建模 | [教程 00-基础与核心/08-Plan-and-Execute模式 模式](../教程/00-基础与核心/08-Plan-and-Execute模式.md) |
...
SWE-bench 的核心挑战之一是 **代码库级别的上下文管理**——Agent 不可能把整个项目塞进上下文窗口，必须使用 RAG 或 Agentic RAG 来精确定位相关代码。这直接关联到我们在 [教程 05-Observation可观测/02-组件交互：Registry、Handler、Convention、Filter协作 RAG 与 Agentic RAG](../教程/08-架构师进阶/01-高级RAG与AgenticRAG.md) 中讨论的技术。
...
大多数评测只关注"能不能完成"，忽略"用多少成本完成"。在实际生产中，一个用 50 步、消耗 50000 Token 完成任务的 Agent 和一个用 5 步、消耗 5000 Token 完成同样任务的 Agent，商业价值天差地别。这与我们在 [教程 03-React前端与AgenticUI/03-Agentic-UI设计 Token 计量](../教程/04-企业级架构主干/07-成本治理与Token计量.md) 中讨论的成本意识高度相关。
...
这与 [教程 08-架构师进阶/07-数据飞轮与持续改进](../教程/08-架构师进阶/07-数据飞轮与持续改进.md) 中的理念一致——评测不应该是一次性的活动，而是持续迭代的闭环。生产中的失败案例是最有价值的评测数据来源。
...
对于 Java Agent 架构师，最实用的建议是：**将评测作为 Agent 开发的第一公民**。在构建任何 Agent 功能时，同步构建对应的测试场景和评估指标——投入至少 30% 的精力到测试集构建、评测自动化和持续监控中。这与我们在 [教程 05-Observation可观测/04-自定义Convention与Filter：工业标签与脱敏 Agent 评估](../教程/08-架构师进阶/03-自我反思与Agent评估.md) 中讨论的自我评估能力共同构成了 Agent 质量保障的双层体系：Agent 内部的自省和外部的系统化评测。
