...
> **定位**：Agent 的"手"——让 Agent 操作真实浏览器/桌面界面完成任务的技術方向。本文调研 2025-2026 年的主流实现、架构形态、风险与 Java 生态的接入方式。前置阅读：[教程 00-基础与核心/00-Agent核心概念]、[教程 00-基础与核心/03-工具调用]。
...
接入即 MCP 客户端集成（[附录 05-01 基准]）：注册 Playwright MCP Server 的工具目录，Agent 的 ReAct 循环自然驱动浏览器。企业接入的位置在**安全网关**（[项目 08] 的形态）：浏览器工具是高危工具（能触达一切 Web），必须过准入评级（C 级沙箱）、egress 域名白名单、注入检测（网页内容是不可信输入——间接注入的主战场）。

## 4. 风险清单（这个方向的独特攻击面）

| 风险 | 说明 | 缓解 |
|------|------|------|
| 间接注入 | 网页内容直接进 Prompt——攻击页面可诱导 Agent 执行恶意操作 | 内容过注入检测管道（[教程 03-React前端与AgenticUI/01-React状态管理/附录 08]）；高危动作 HITL |
...
**最重要的架构判断**：Computer Use 的安全模型 = 间接注入防御 + 高危动作审批的组合——这两者本体系已有完整方案（[附录 08/00]、[教程 04-企业级架构主干/02-全链路可观测性]），浏览器只是把它们的作用面放大到"整个互联网"。
