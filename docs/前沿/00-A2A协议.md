...
[教程 01-WebFlux与响应式编程/01-Reactor核心 协议](../教程/02-SpringAI核心机制/07-MCP协议.md) 中我们深入讨论过，MCP 解决的是 **Agent 如何连接工具和数据源** 的标准化问题。MCP 让一个 Agent 可以统一地调用文件系统、数据库、GitHub 等外部能力。
...
值得注意的是 SSE 流式模式——它与我们 [教程 01-WebFlux与响应式编程/00-WebFlux从零入门 流式通信](../教程/02-SpringAI核心机制/06-SSE流式通信.md) 中讲的 SSE 完全一致。A2A 直接复用了 SSE 作为 Agent 间的流式通信协议，这意味着 Spring WebFlux 天然可以支持 A2A 的流式任务。
