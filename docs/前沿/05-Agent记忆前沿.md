...
> **定位**：本文调研 Agent 记忆研究的前沿方向。我们在 [教程 00-基础与核心/04-记忆与会话管理](../教程/00-基础与核心/04-记忆与会话管理.md) 和 [教程 08-架构师进阶/05-高级记忆架构](../教程/08-架构师进阶/05-高级记忆架构.md) 中已经讨论了工程化的记忆方案（短期记忆、长期记忆、向量检索）。本文将视角推向前沿：记忆演化模型、神经记忆系统、持久化记忆研究、以及"记忆即参数"的范式转变。
>
...
GDPR 的"被遗忘权"（Right to Erasure）对 Agent 记忆系统提出了特殊要求——不仅要删除主存储中的记忆，还要清除所有备份、缓存、向量索引中的对应条目。这在技术上非常困难。这些挑战与 [教程 04-企业级架构主干/11-安全与权限控制](../教程/04-企业级架构主干/11-安全与权限控制.md) 和 [教程 04-企业级架构主干/05-历史记录持久化与合规](../教程/04-企业级架构主干/05-历史记录持久化与合规.md) 中的企业级安全实践直接相关。
...
在 [教程 00-基础与核心/09-多Agent协作 Agent 协作](../教程/00-基础与核心/09-多Agent协作.md) 中讨论的多 Agent 场景下，Agent 之间可以共享记忆，形成集体智能：
...
这与 [教程 08-架构师进阶/07-数据飞轮与持续改进](../教程/08-架构师进阶/07-数据飞轮与持续改进.md) 中的数据飞轮理念高度一致——Agent 的集体经验可以反哺系统优化。
...
graph LR
    subgraph 路线["企业 Agent 记忆演进路线"]
        ...
        S2["Phase 2<br/>多层级 + 摘要压缩<br/>（教程 05-Observation可观测/01-读懂输出：span树与观测生命周期 已覆盖）"]
