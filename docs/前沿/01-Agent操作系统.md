...
Agent OS 中的 Agent 有明确的生命周期状态机，与 [教程 12-Agent 状态管理](../教程/21-Agent状态管理.md) 中讨论的业务层状态不同，这是 **运行时级别** 的状态：
...
这个机制与我们 [教程 82-高级记忆架构](../教程/82-高级记忆架构.md) 和 [教程 77-上下文工程](../教程/77-上下文工程.md) 中讨论的记忆压缩和上下文窗口管理是一致的，Agent OS 将其从应用层提升到了运行时层。
...
这与 [教程 59-多租户隔离与资源治理](../教程/59-多租户隔离与资源治理.md) 中的多租户架构直接对应——Agent OS 提供运行时级别的记忆隔离，而不是依赖应用层手动处理。
...
这个架构与我们在 [教程 29-管控分离架构](../教程/29-管控分离架构.md) 中讨论的"数据面/控制面分离"理念一致——Agent OS 内核就是控制面，Agent 应用是数据面。
