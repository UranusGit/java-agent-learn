...
这个机制与我们 [教程 39-高级记忆架构](../教程/39-高级记忆架构.md) 和 [教程 34-上下文工程](../教程/34-上下文工程.md) 中讨论的记忆压缩和上下文窗口管理是一致的，Agent OS 将其从应用层提升到了运行时层。
...
这与 [教程 26-多租户隔离与资源治理](../教程/26-多租户隔离与资源治理.md) 中的多租户架构直接对应——Agent OS 提供运行时级别的记忆隔离，而不是依赖应用层手动处理。
...
这个架构与我们在 [教程 20-管控分离架构](../教程/20-管控分离架构.md) 中讨论的"数据面/控制面分离"理念一致——Agent OS 内核就是控制面，Agent 应用是数据面。
