...
```mermaid
flowchart TD
    Q1{"需要持久化吗？<br/>重启后要保留历史"}
    Q1 -->|"否"| InMem["MessageWindowChatMemory<br/>（内存，学习/原型用）"]
    Q1 -->|"是"| Q2{"历史保留多少条？"}
    Q2 -->|"≤ 10 条"| M10["maxMessages(10)<br/>（简单问答）"]
    Q2 -->|"10-20 条"| M20["maxMessages(20)<br/>（推荐默认值）"]
    Q2 -->|"> 20 条"| Summary["考虑摘要压缩<br/>（上下文工程）"]
    InMem --> Q3{"需要跨设备同步？"}
    Q3 -->|"否"| Done1["内存即可"]
    Q3 -->|"是"| Redis["Redis/DB 持久化"]

    style InMem fill:#4caf50,color:#fff
    style M20 fill:#4caf50,color:#fff
```
