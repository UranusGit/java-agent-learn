...
**实施进度**：
- ✅ 方向1（意图/语义路由中枢）全部完成：**教程 50-意图路由与路由编排** + **附录 20-路由与推理策略/00-路由表与语义路由** + **项目 23-企业级Agent意图路由网关**（14 篇：00架构→01最小Demo→02语义路由→03路由表→04观测→05命中阈值→06灰度→07兜底HITL→08缓存→09审计→10策略下发→11核心代码→12-ADR(8条)→13前沿）。命名遵循"主题名不带迭代码"铁律。
- ✅ **全体系迭代命名链接大修复**（用户指出命名规范问题后）：系统在"物理文件名早规范（如 `03-租户配额与限流.md`）但正文 700+ 处仍写旧 `N-迭代X-主题.md`"——全量把正文链接中的 `N-迭代X-` 前缀去除（214 文件/650 处），同时修正新写项目在二级目录的跨板引用 `../→../../`；终验=全体系相对链接 0 死链（1230 条）、Mermaid 0。
- ⬜ 待做：方向2-10 按上表 4 波推进。
