...
- 🔄 **方向5（A2A协作平台）进行中**【checkpoint 040990f】：已提交**教程 54-Agent间协作协议工程化** + **项目 27-00 需求架构**（协作中继视角：AgentCard注册/能力发现/任务委托协商/身份互信+最小授权/注入边界/失败补偿/审计/协作市场）。铁律0：A2A/AgentCard 开放协议无官方 SDK（适配层概念代码）；基座 MCP2.0 SDK + ChatClient 实证。**待续：项目 27-01 最小Demo → 02 能力发现 → 03 任务委托 → 04 执行/取消 → 05 身份互信 → 06 注入边界 → 07 失败补偿 → 08 可观测 → 09 跨组织 → 10 协作市场 → 11 核心代码 → 12 ADR → 13 前沿**。
- ⬜ 待做：方向4-10（方向4数据飞轮=项目26/教程53、方向6可观测=项目28/附录21、方向7边缘=项目29/教程55、方向8 XAI=项目30/教程56、方向9上下文工程=项目31/教程57、方向10向量库=附录22 + 前沿12-16）。
