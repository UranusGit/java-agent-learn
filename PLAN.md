...
- ✅ 方向2（Temporal式超长任务工作流引擎）全部完成：**教程 51-超长任务与分布式工作流引擎** + **项目 24-企业级长任务工作流引擎**（14 篇：00架构→01最小Demo→02 Actor任务核→03幂等→04预算死循环→05 DAG多Worker→06断点续跑→07可视取消→08独立引擎SPI→09多机分布→10预算HITL→11核心代码→12-ADR(8条)→13前沿）。Temporal 仅作业界对照（需引入依赖后 javap 实证，不写成已实证 API）。
- ✅ **全体系命名大修复**（用户两次指出命名问题）：①去迭代码前缀（214 文件/650 处）②去进阶N- 前缀（18 文件 + 51 处正文）③修复新写项目二级目录跨板引用 `../→../../` ④**命名铁律固化进 CLAUDE.md**（文件名一律 `XX-主题.md`，禁止迭代码/进阶N- 序号前缀）。终验=全体系相对链接 0 死链、Mermaid 0、0 残留不规范前缀。
- ⬜ 待做：方向3-10 按上表 4 波推进（方向3记忆中枢/方向4数据飞轮/方向5 A2A/方向6可观测/方向7边缘/方向8 XAI/方向9上下文工程/方向10向量库+前沿雷达）。
