...
| 坑 | 原因 | 修复 |
|---|---|---|
| `A->B` 一根线箭头显示异常 | `->` 是最老的实线箭头，显示形式不稳定 | 统一用 `->>`（实线）`-->>`（返回虚线） |
| 生命线激活条画不完整/悬挂 | `activate` / `deactivate` 漏配对 | 每对显式配对，或统一用 `->>+` / `-->>-` 简写 |
| 重复激活报错 | `->>+` 简写和显式 `activate` 混用导致重复激活 | 同一张图里只用一种激活写法 |
| `participant a b as 名字` 报错 | ID 不允许空格 | ID 用英文无空格：`participant ab as 名字` |
| `Note over A, B` 报错 | 引用了未定义的参与者 | `Note over` 引用的参与者必须都已显式定义 |
| 消息文本含 `(` `:` 报错 | 特殊字符与语法定界符冲突 | 消息文本加双引号：`client->>server: "查询 (批量)"` |
| `alt`/`loop` 块后面内容消失 | 组合块漏写 `end` 闭合 | 每个组合块必须以 `end` 结束，嵌套时逐层检查 |
